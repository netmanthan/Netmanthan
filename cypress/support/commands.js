			...
			return cy
				.request({
					...
					headers: {
						...
						"X-netmanthan-CSRF-Token": csrf_token,
					},
				})
				.then((res) => {
...
Cypress.Commands.add("create_records", (doc) => {
	return cy
		.call("netmanthan.tests.ui_test_helpers.create_if_not_exists", { doc: JSON.stringify(doc) })
		.then((r) => r.message);
});

Cypress.Commands.add("set_value", (doctype, name, obj) => {
	return cy.call("netmanthan.client.set_value", {
		doctype,
		name,
		fieldname: obj,
	});
...
Cypress.Commands.add(
	...
	(tablefieldname, row_idx, fieldname, fieldtype = "Data") => {
		let selector = `.netmanthan-control[data-fieldname="${tablefieldname}"]`;
...
Cypress.Commands.add("dialog", (opts) => {
	return cy
		.window({ log: false })
		.its("netmanthan", { log: false })
		.then((netmanthan) => {
			Cypress.log({
				name: "dialog",
				displayName: "dialog",
				message: "netmanthan.ui.Dialog",
				consoleProps: () => {
					return {
						options: opts,
						dialog: d,
			...
			var d = new netmanthan.ui.Dialog(opts);
			d.show();
			return d;
		});
});
...
Cypress.Commands.add("save", () => {
	cy.intercept("/api/method/netmanthan.desk.form.save.savedocs").as("save_call");
...
Cypress.Commands.add("add_role", (user, role) => {
	cy.window()
		.its("netmanthan")
		.then((netmanthan) => {
			const session_user = netmanthan.session.user;
...
const add_remove_role = (action, user, role, session_user) => {
	...
	cy.call("netmanthan.tests.ui_test_helpers.add_remove_role", {
		action: action,
		user: user,
		role: role,
	});
...
Cypress.Commands.add("clear_filters", () => {
	...
	cy.intercept({
		method: "POST",
		url: "api/method/netmanthan.model.utils.user_settings.save",
	}).as("filter-saved");
...
Cypress.Commands.add("select_listview_row_checkbox", (row_no) => {
	cy.get(".netmanthan-list .select-like > .list-row-checkbox").eq(row_no).click();
});
