context("Workspace Blocks", () => {
	before(() => {
		...
		return cy
			...
			.its("netmanthan")
			.then((netmanthan) => {
				return netmanthan.xcall("netmanthan.tests.ui_test_helpers.setup_workflow");
			});
	});
	...
	it.skip("Quick List Block", () => {
		...
		cy.intercept({
			method: "GET",
			url: "api/method/netmanthan.desk.form.load.getdoctype?**",
		}).as("get_doctype");
		...
		cy.intercept({
			method: "POST",
			url: "api/method/netmanthan.desk.reportview.get",
		}).as("refresh-list");
