context.skip("Form Tour", () => {
	before(() => {
		...
		return cy
			...
			.its("netmanthan")
			.then((netmanthan) => {
				return netmanthan.call("netmanthan.tests.ui_test_helpers.create_form_tour");
			});
	});
	...
	it("navigates a form tour", () => {
		...
		cy.get(".netmanthan-driver").should("be.visible");
		cy.get('.netmanthan-control[data-fieldname="first_name"]').as("first_name");
		cy.get("@first_name").should("have.class", "driver-highlighted-element");
		cy.get(".netmanthan-driver").findByRole("button", { name: "Next" }).as("next_btn");
		...
		cy.get('.netmanthan-control[data-fieldname="last_name"]').as("last_name");
		...
		// assert field is highlighted
		cy.get('.netmanthan-control[data-fieldname="phone_nos"]').as("phone_nos");
		...
		cy.get('.grid-row-open .netmanthan-control[data-fieldname="phone"]').as("phone");
		...
		cy.wait(500);
		cy.get(".netmanthan-driver").findByRole("button", { name: "Save" }).should("be.visible");
	});
});
