...
context("Grid Search", () => {
	before(() => {
		...
		return cy
			...
			.its("netmanthan")
			.then((netmanthan) => {
				return netmanthan.xcall(
					"netmanthan.tests.ui_test_helpers.insert_doctype_with_child_table_record",
					{
	...
	it("Test search row visibility", () => {
		cy.window()
			.its("netmanthan")
			.then((netmanthan) => {
				netmanthan.model.user_settings.save("Doctype With Child Table", "GridView", {
		...
		cy.get('.netmanthan-control[data-fieldname="child_table_1"]').as("table");
