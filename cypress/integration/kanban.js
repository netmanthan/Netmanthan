context("Kanban Board", () => {
	before(() => {
		cy.login("netmanthan@example.com");
		cy.visit("/app");
	});
	...
	it("Create ToDo from kanban", () => {
		cy.intercept({
			method: "POST",
			url: "api/method/netmanthan.client.save",
		}).as("save-todo");
	...
	it("Add and Remove fields", () => {
		...
		cy.intercept(
			"POST",
			"/api/method/netmanthan.desk.doctype.kanban_board.kanban_board.save_settings"
		).as("save-kanban");
		cy.intercept(
			"POST",
			"/api/method/netmanthan.desk.doctype.kanban_board.kanban_board.update_order"
		).as("update-order");
		...
		cy.get(".netmanthan-control .label-area").contains("Show Labels").click();
		cy.click_modal_primary_button("Save");
		...
		cy.get_open_dialog()
			.find(
				'.netmanthan-control[data-fieldname="fields_html"] div[data-label="ID"] .remove-field'
			)
			.click();

		cy.wait("@update-order");
		cy.get_open_dialog().find(".netmanthan-control .label-area").contains("Show Labels").click();
		cy.get(".modal-footer .btn-primary").last().click();
	...
	it("Checks if Kanban Board edits are blocked for non-System Manager and non-owner of the Board", () => {
		...
		cy.call("netmanthan.tests.ui_test_helpers.create_test_user", {
			username: noSystemManager,
		});
		cy.remove_role(noSystemManager, "System Manager");
		cy.call("netmanthan.tests.ui_test_helpers.create_todo", { description: "netmanthan User ToDo" });
		cy.call("netmanthan.tests.ui_test_helpers.create_admin_kanban");
		...
		cy.switch_to_user("Administrator");
		cy.call("netmanthan.client.delete", { doctype: "User", name: noSystemManager });
	});
