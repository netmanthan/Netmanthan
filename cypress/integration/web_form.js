context("Web Form", () => {
	before(() => {
		...
		return cy
			...
			.its("netmanthan")
			.then((netmanthan) => {
				return netmanthan.xcall("netmanthan.tests.ui_test_helpers.clear_notes");
			});
	});
	...
	it("Read Only", () => {
		...
		cy.get('.netmanthan-control[data-fieldname="title"] .control-input').should(
			"have.css",
			"display",
			"none"
		);
	...
	it("Navigate and Submit a MultiStep WebForm", () => {
		cy.call("netmanthan.tests.ui_test_helpers.update_webform_to_multistep").then(() => {
