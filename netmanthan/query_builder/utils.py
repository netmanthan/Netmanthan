...
class ImportMapper:
	...
	def __call__(self, *args: Any, **kwds: Any) -> Callable:
		db = db_type_is(netmanthan.conf.db_type or "mariadb")
...
def get_query(*args, **kwargs) -> QueryBuilder:
	from netmanthan.database.query import Engine
...
def DocType(*args, **kwargs):
	return netmanthan.qb.DocType(*args, **kwargs)


def Table(*args, **kwargs):
	return netmanthan.qb.Table(*args, **kwargs)


def patch_query_execute():
	"""Patch the Query Builder with helper execute method
	This excludes the use of `netmanthan.db.sql` method while
	...
	def execute_query(query, *args, **kwargs):
		...
		result = netmanthan.db.sql(query, params, *args, **kwargs)  # nosemgrep
	...
	def prepare_query(query):
		...
		if netmanthan.flags.in_safe_exec:
			from netmanthan.utils.safe_exec import check_safe_sql_query
			...
			if not check_safe_sql_query(query, throw=False):
				...
				if len(callstack) >= 3 and ".py" in callstack[2].filename:
					...
					raise netmanthan.PermissionError("Only SELECT SQL allowed in scripting")
		return query, param_collector.get_parameters()

	builder_class = netmanthan.qb._BuilderClasss
	...
	netmanthan._qb_patched[netmanthan.conf.db_type] = True


def patch_query_aggregation():
	"""Patch aggregation functions to netmanthan.qb"""
	from netmanthan.query_builder.functions import _avg, _max, _min, _sum

	netmanthan.qb.max = _max
	netmanthan.qb.min = _min
	netmanthan.qb.avg = _avg
	netmanthan.qb.sum = _sum
	netmanthan._qb_patched[netmanthan.conf.db_type] = True
