...
def check_admin_or_system_manager(user=None):
	...
	warn(
		...
		'Please use netmanthan.only_for("System Manager") instead.',
	...
	if ("System Manager" not in netmanthan.get_roles(user)) and (user != "Administrator"):
		netmanthan.throw(_("Not permitted"), netmanthan.PermissionError)


def print_has_permission_check_logs(func):
	def inner(*args, **kwargs):
		netmanthan.flags["has_permission_check_logs"] = []
		result = func(*args, **kwargs)
		self_perm_check = True if not kwargs.get("user") else kwargs.get("user") == netmanthan.session.user
		...
		if not result and self_perm_check and raise_exception:
			msgprint(("<br>").join(netmanthan.flags.get("has_permission_check_logs", [])))
		netmanthan.flags.pop("has_permission_check_logs", None)
		return result
...
def has_permission(
	...
	*,
	...
	:param raise_exception:
	        ...
	        If not False, will display a message using netmanthan.msgprint
	...
	if ptype == "share" and netmanthan.get_system_settings("disable_document_sharing"):
		return False
	...
	if doc:
		if isinstance(doc, str):
			doc = netmanthan.get_doc(meta.name, doc)
		perm = get_doc_permissions(doc, user=user, ptype=ptype).get(ptype)
		if not perm:
			push_perm_check_log(
				_("User {0} does not have access to this document").format(netmanthan.bold(user))
			)
	else:
		...
		if not perm:
			push_perm_check_log(
				_("User {0} does not have doctype access via role permission for document {1}").format(
					netmanthan.bold(user), netmanthan.bold(doctype)
				)
			)
	...
	def false_if_not_shared():
		if ptype in ("read", "write", "share", "submit", "email", "print"):
			...
			if doc:
				...
				shared = netmanthan.share.get_shared(
					doctype,
					user,
					rights=rights,
			...
			elif netmanthan.share.get_shared(doctype, user, rights=rights, limit=1):
...
def get_role_permissions(doctype_meta, user=None, is_owner=None):
	...
	if isinstance(doctype_meta, str):
		doctype_meta = netmanthan.get_meta(doctype_meta)  # assuming doctype name was passed
	...
	if not netmanthan.local.role_permissions.get(cache_key):
		perms = netmanthan._dict(if_owner={})
		...
		netmanthan.local.role_permissions[cache_key] = perms

	return netmanthan.local.role_permissions[cache_key]


def get_user_permissions(user):
	from netmanthan.core.doctype.user_permission.user_permission import get_user_permissions
...
def has_user_permission(doc, user=None):
	...
	apply_strict_user_permissions = netmanthan.get_system_settings("apply_strict_user_permissions")
	...
	def check_user_permission_on_link_fields(d):
		...
		meta = netmanthan.get_meta(d.get("doctype"))
...
def has_controller_permissions(doc, ptype, user=None):
	...
	methods = netmanthan.get_hooks("has_permission").get(doc.doctype, [])

	if not methods:
		return None

	for method in reversed(methods):
		controller_permission = netmanthan.call(netmanthan.get_attr(method), doc=doc, ptype=ptype, user=user)
...
def get_all_perms(role):
	...
	perms = netmanthan.get_all("DocPerm", fields="*", filters=dict(role=role))
	custom_perms = netmanthan.get_all("Custom DocPerm", fields="*", filters=dict(role=role))
	doctypes_with_custom_perms = netmanthan.get_all("Custom DocPerm", pluck="parent", distinct=True)
...
def get_roles(user=None, with_standard=True):
	...
	def get():
		if user == "Administrator":
			return netmanthan.get_all("Role", pluck="name")  # return all available roles
		else:
	...
	roles = netmanthan.cache().hget("roles", user, get)
...
def get_perms_for(roles, perm_doctype="DocPerm"):
	...
	return netmanthan.get_all(perm_doctype, fields=["*"], filters=filters)


def get_doctypes_with_custom_docperms():
	"""Returns all the doctypes with Custom Docperms"""

	doctypes = netmanthan.get_all("Custom DocPerm", fields=["parent"], distinct=1)
...
def set_user_permission_if_allowed(doctype, name, user, with_message=False):
	if get_role_permissions(netmanthan.get_meta(doctype), user).set_user_permissions != 1:
...
def add_user_permission(
	...
	"""Add user permission"""
	from netmanthan.core.doctype.user_permission.user_permission import user_permission_exists

	if not user_permission_exists(user, doctype, name, applicable_for):
		if not netmanthan.db.exists(doctype, name):
			netmanthan.throw(_("{0} {1} not found").format(_(doctype), name), netmanthan.DoesNotExistError)
...
def remove_user_permission(doctype, name, user):
	user_permission_name = netmanthan.db.get_value(
		"User Permission", dict(user=user, allow=doctype, for_value=name)
	)
	netmanthan.delete_doc("User Permission", user_permission_name)
...
def clear_user_permissions_for_doctype(doctype, user=None):
	...
	if user:
		filters["user"] = user
	user_permissions_for_doctype = netmanthan.get_all("User Permission", filters=filters)
	for d in user_permissions_for_doctype:
		netmanthan.delete_doc("User Permission", d.name)


def can_import(doctype, raise_exception=False):
	if not ("System Manager" in netmanthan.get_roles() or has_permission(doctype, "import")):
		if raise_exception:
			raise netmanthan.PermissionError(f"You are not allowed to import: {doctype}")
		else:
			return False
	return True
...
def can_export(doctype, raise_exception=False):
	if "System Manager" in netmanthan.get_roles():
		return True
	else:
		role_permissions = netmanthan.permissions.get_role_permissions(doctype)
		has_access = role_permissions.get("export") or role_permissions.get("if_owner").get("export")
		if not has_access and raise_exception:
			raise netmanthan.PermissionError(_("You are not allowed to export {} doctype").format(doctype))
		return has_access
...
def update_permission_property(
	...
	from netmanthan.core.doctype.doctype.doctype import validate_permissions_for_doctype

	out = setup_custom_perms(doctype)

	name = netmanthan.db.get_value(
		"Custom DocPerm", dict(parent=doctype, role=role, permlevel=permlevel, if_owner=if_owner)
	)
	table = DocType("Custom DocPerm")
	netmanthan.qb.update(table).set(ptype, value).where(table.name == name).run()
...
def setup_custom_perms(parent):
	...
	if not netmanthan.db.exists("Custom DocPerm", dict(parent=parent)):
		copy_perms(parent)
		return True
...
def add_permission(doctype, role, permlevel=0, ptype=None):
	...
	custom_docperm = netmanthan.get_doc(
		{
...
def copy_perms(parent):
	...
	for d in netmanthan.get_all("DocPerm", fields="*", filters=dict(parent=parent)):
		custom_perm = netmanthan.new_doc("Custom DocPerm")
...
def reset_perms(doctype):
	...
	netmanthan.db.delete("Custom DocPerm", {"parent": doctype})


def get_linked_doctypes(dt: str) -> list:
	meta = netmanthan.get_meta(dt)
	linked_doctypes = [dt] + [
		d.options
		for d in meta.get(
			"fields",
...
def push_perm_check_log(log):
	if netmanthan.flags.get("has_permission_check_logs") is None:
		return

	netmanthan.flags.get("has_permission_check_logs").append(_(log))
...
def has_child_permission(
	...
	if isinstance(child_doc, str):
		child_doc = netmanthan.db.get_value(
			child_doctype,
			child_doc,
	...
	if not parent_doctype:
		push_perm_check_log(
			_("Please specify a valid parent DocType for {0}").format(netmanthan.bold(child_doctype))
		)
		return False

	parent_meta = netmanthan.get_meta(parent_doctype)
	...
	if parent_meta.istable or not (
		...
		push_perm_check_log(
			_("{0} is not a valid parent DocType for {1}").format(
				netmanthan.bold(parent_doctype), netmanthan.bold(child_doctype)
			)
		)
		return False
	...
	if child_doc:
		...
		if not parentfield:
			push_perm_check_log(
				_("Parentfield not specified in {0}: {1}").format(
					netmanthan.bold(child_doctype), netmanthan.bold(child_doc.name)
				)
			)
			return False
		...
		if parentfield not in valid_parentfields:
			push_perm_check_log(
				_("{0} is not a valid parentfield for {1}").format(
					netmanthan.bold(parentfield), netmanthan.bold(child_doctype)
				)
			)
			return False
		...
		if permlevel > 0 and permlevel not in parent_meta.get_permlevel_access(ptype, user=user):
			push_perm_check_log(
				_("Insufficient Permission Level for {0}").format(netmanthan.bold(parent_doctype))
			)
			return False
