...
def new_site(
	...
	"Create a new site"
	from netmanthan.installer import _new_site, extract_sql_from_archive

	netmanthan.init(site=site, new_site=True)
...
def restore(
	...
	from netmanthan.utils.backups import Backup, get_or_generate_backup_encryption_key
	...
	try:
		...
		if is_partial(decompressed_file_name):
			click.secho(
				"Partial Backup file detected. You cannot use a partial file to restore a netmanthan site.",
				fg="red",
			)
			click.secho(
	...
	# dont allow downgrading to older versions of netmanthan without force
...
def partial_restore(context, sql_file_path, verbose, encryption_key=None):
	from netmanthan.installer import extract_sql_from_archive, partial_restore
	from netmanthan.utils.backups import Backup, get_or_generate_backup_encryption_key
	...
	try:
		...
		with open(decompressed_file_name) as f:
			...
			if "Partial Backup" not in header:
				click.secho(
					"Full backup file detected.Use `bench restore` to restore a netmanthan Site.", fg="red"
				)
				_backup.decryption_rollback()
				sys.exit(1)
...
def _reinstall(
	...
	from netmanthan.installer import _new_site
	...
	try:
		...
		installed = netmanthan.get_installed_apps()
		netmanthan.clear_cache()
	except Exception:
		installed = []
	finally:
...
def install_app(context, apps, force=False):
	...
	from netmanthan.installer import install_app as _install_app
	...
	for site in context.sites:
		...
		for app in apps:
			...
			except netmanthan.IncompatibleApp as err:
				err_msg = f":\n{err}" if str(err) else ""
				print(f"App {app} is Incompatible with Site {site}{err_msg}")
				exit_code = 1
			except Exception as err:
				err_msg = f": {str(err)}\n{netmanthan.get_traceback()}"
...
def list_apps(context, format):
	...
	for site in context.sites:
		...
		apps = netmanthan.get_single("Installed Applications").installed_applications
		...
		if apps:
			...
			installed_applications = netmanthan.get_installed_apps()
			applications_summary = "\n".join(installed_applications)
			summary = f"{site_title}\n{applications_summary}\n"
	...
	if format == "json":
		click.echo(netmanthan.as_json(summary_dict))
...
def add_system_manager(context, email, first_name, last_name, send_welcome_email, password):
	...
	for site in context.sites:
		netmanthan.connect(site=site)
		try:
			netmanthan.utils.user.add_system_manager(email, first_name, last_name, send_welcome_email, password)
			netmanthan.db.commit()
		finally:
			netmanthan.destroy()
	if not context.sites:
...
def disable_user(context, email):
	...
	with netmanthan.init_site(site):
		netmanthan.connect()
		user = netmanthan.get_doc("User", email)
		user.enabled = 0
...
def migrate(context, skip_failing=False, skip_search_index=False):
	...
	from netmanthan.migrate import SiteMigration
...
@click.argument("netmanthan_provider")
@pass_context
def migrate_to(context, netmanthan_provider):
	"Migrates site to the specified provider"
	from netmanthan.integrations.netmanthan_providers import migrate_to

	for site in context.sites:
		netmanthan.init(site=site)
		netmanthan.connect()
		migrate_to(site, netmanthan_provider)
		netmanthan.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def run_patch(context, module, force):
	...
	for site in context.sites:
		...
		try:
			netmanthan.connect()
			netmanthan.modules.patch_handler.run_single(module, force=force or context.force)
		finally:
			netmanthan.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def reload_doc(context, module, doctype, docname):
	...
	for site in context.sites:
		try:
			netmanthan.init(site=site)
			netmanthan.connect()
			netmanthan.reload_doc(module, doctype, docname, force=context.force)
			netmanthan.db.commit()
		finally:
			netmanthan.destroy()
	if not context.sites:
...
def reload_doctype(context, doctype):
	...
	for site in context.sites:
		try:
			netmanthan.init(site=site)
			netmanthan.connect()
			netmanthan.reload_doctype(doctype, force=context.force)
			netmanthan.db.commit()
		finally:
			netmanthan.destroy()
	if not context.sites:
...
def add_to_hosts(context):
	"Add site to hosts"
	for site in context.sites:
		netmanthan.commands.popen(f"echo 127.0.0.1\t{site} | sudo tee -a /etc/hosts")
	if not context.sites:
		raise SiteNotSpecifiedError
...
def backup(
	...
	for site in context.sites:
		try:
			...
			exit_code = 1
			continue
		if netmanthan.get_system_settings("encrypt_backup") and netmanthan.get_site_config().encryption_key:
			click.secho(
...
def remove_from_installed_apps(context, app):
	...
	from netmanthan.installer import remove_from_installed_apps
...
def uninstall(context, app, dry_run, yes, no_backup, force):
	...
	from netmanthan.installer import remove_app
...
def _drop_site(
	...
	from netmanthan.database import drop_user_and_database
	from netmanthan.utils.backups import scheduled_backup
	...
	drop_user_and_database(netmanthan.conf.db_name, db_root_username, db_root_password)

	archived_sites_path = archived_sites_path or os.path.join(
		netmanthan.get_app_path("netmanthan"), "..", "..", "..", "archived", "sites"
	)
...
def move(dest_dir, site):
	...
	old_path = netmanthan.utils.get_site_path()
...
def set_last_active_for_user(context, user=None):
	...
	from netmanthan.core.doctype.user.user import get_system_users
	...
	with netmanthan.init_site(site):
		...
		netmanthan.db.set_value("User", user, "last_active", now_datetime())
		netmanthan.db.commit()
...
def publish_realtime(context, event, message, room, user, doctype, docname, after_commit):
	...
	from netmanthan import publish_realtime
...
def browse(context, site, user=None):
	...
	if site not in netmanthan.utils.get_sites():
	...
	sid = ""
	if user:
		if netmanthan.conf.developer_mode or user == "Administrator":
			...
			sid = f"/app?sid={netmanthan.session.sid}"
		else:
			click.echo("Please enable developer mode to login as a user")

	url = f"{netmanthan.utils.get_site_url(site)}{sid}"
...
def clear_log_table(context, doctype, days, no_backup):
	...
	"""
	from netmanthan.core.doctype.log_settings.log_settings import LOG_DOCTYPES
	from netmanthan.core.doctype.log_settings.log_settings import clear_log_table as clear_logs
	from netmanthan.utils.backups import scheduled_backup
	...
	if doctype not in LOG_DOCTYPES:
		raise netmanthan.ValidationError(f"Unsupported logging DocType: {doctype}")
...
def trim_database(context, dry_run, format, no_backup, yes=False):
	...
	for site in context.sites:
		...
		queried_result = (
			...
			.select(table_name)
			.where(information_schema.tables.table_schema == netmanthan.conf.db_name)
			.where(information_schema.tables.table_type == "BASE TABLE")
			.run()
		)

		database_tables = [x[0] for x in queried_result]
		doctype_tables = netmanthan.get_all("DocType", pluck="name")
		...
		if not TABLES_TO_DROP:
			if format == "text":
				click.secho(f"No ghost tables found in {netmanthan.local.site}...Great!", fg="green")
		else:
			if not yes:
			...
			for table in TABLES_TO_DROP:
				...
				if not dry_run:
					netmanthan.db.sql_ddl(f"drop table `{table}`")

			ALL_DATA[netmanthan.local.site] = TABLES_TO_DROP
		netmanthan.destroy()
...
def get_standard_tables():
	...
	sql_file = os.path.join(
		...
		"netmanthan",
		"database",
		netmanthan.conf.db_type,
		f"framework_{netmanthan.conf.db_type}.sql",
	)
...
def trim_tables(context, dry_run, format, no_backup):
	...
	from netmanthan.model.meta import trim_tables
	from netmanthan.utils.backups import scheduled_backup
	...
	for site in context.sites:
		...
		if not (no_backup or dry_run):
			click.secho(f"Taking backup for {netmanthan.local.site}", fg="green")
		...
		try:
			...
			if format == "table" and not dry_run:
				click.secho(f"The following data have been removed from {netmanthan.local.site}", fg="green")
...
def handle_data(data: dict, format="json"):
	if format == "json":
		...
		print(json.dumps({netmanthan.local.site: data}, indent=1, sort_keys=True))
	else:
