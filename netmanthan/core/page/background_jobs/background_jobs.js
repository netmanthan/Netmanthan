netmanthan.pages["background_jobs"].on_page_load = (wrapper) => {
...
class BackgroundJobs {
	constructor(wrapper) {
		...
		this.page.add_inner_button(__("Remove Failed Jobs"), () => {
			netmanthan.confirm(__("Are you sure you want to remove all failed jobs?"), () => {
				netmanthan
					.call("netmanthan.core.page.background_jobs.background_jobs.remove_failed_jobs")
		...
		this.make_filters();
		this.refresh_jobs = netmanthan.utils.throttle(this.refresh_jobs.bind(this), 1000);
	}
	...
	update_scheduler_status() {
		netmanthan.call({
			method: "netmanthan.core.page.background_jobs.background_jobs.get_scheduler_status",
			callback: (r) => {
	...
	refresh_jobs() {
		...
		netmanthan.call({
			method: "netmanthan.core.page.background_jobs.background_jobs.get_info",
			args,
			callback: (res) => {
				...
				this.$content.html(
					netmanthan.render_template(template, {
						jobs: res.message || [],
					})
				);

				let auto_refresh = this.auto_refresh.get_value();
				if (netmanthan.get_route()[0] === "background_jobs" && auto_refresh) {
