netmanthan.pages["permission-manager"].on_page_load = (wrapper) => {
	let page = netmanthan.ui.make_app_page({
		parent: wrapper,
	...
	$(netmanthan.render_template("permission_manager_help", {})).appendTo(page.main);
	wrapper.permission_engine = new netmanthan.PermissionEngine(wrapper);
};

netmanthan.pages["permission-manager"].refresh = function (wrapper) {
	wrapper.permission_engine.set_from_route();
};

netmanthan.PermissionEngine = class PermissionEngine {
	constructor(wrapper) {
	...
	setup_page() {
		this.doctype_select = this.wrapper.page.add_field({
			...
			options: "DocType",
			change: function () {
				netmanthan.set_route("permission-manager", this.get_value());
			},
		});
		...
		this.page.add_inner_button(__("Set User Permissions"), () => {
			return netmanthan.set_route("List", "User Permission");
		});
		this.set_from_route();
	}
	...
	set_from_route() {
		...
		if (netmanthan.get_route()[1]) {
			this.doctype_select.set_value(netmanthan.get_route()[1]);
		} else if (netmanthan.route_options) {
			if (netmanthan.route_options.doctype) {
				this.doctype_select.set_value(netmanthan.route_options.doctype);
			}
			if (netmanthan.route_options.role) {
				this.role_select.set_value(netmanthan.route_options.role);
			}
	...
	reset_std_permissions(data) {
		let doctype = this.get_doctype();
		let d = netmanthan.confirm(__("Reset Permissions for {0}?", [doctype]), () => {
			return netmanthan
				.call({
					module: "netmanthan.core",
					page: "permission_manager",
		...
		let $d = $(d.wrapper)
			.find(".netmanthan-confirm-message")
		...
		data.message.forEach((d) => {
			let rights = this.rights
				.filter((r) => d[r])
				.map((r) => {
					return __(toTitle(netmanthan.unscrub(r)));
				});
	...
	add_delete_button(row, d) {
		$(
			`<button class='btn btn-danger btn-remove-perm btn-xs'>${netmanthan.utils.icon(
				"delete"
			)}</button>`
		)
			...
			.on("click", () => {
				return netmanthan.call({
					...
					callback: (r) => {
						if (r.exc) {
							netmanthan.msgprint(__("Did not remove"));
						} else {
							this.refresh();
						}
					},
	...
	add_check_events() {
		...
		this.body.on("click", ".show-user-permissions", () => {
			netmanthan.route_options = { allow: this.get_doctype() || "" };
			...
			() => {
				...
				d.set_primary_action(__("Add"), () => {
					...
					netmanthan.call({
						...
						callback: (r) => {
							if (r.exc) {
								netmanthan.msgprint(__("Did not add"));
							} else {
								this.refresh();
							}
						},
	...
	get_link_fields(doctype) {
		return netmanthan.get_children("DocType", doctype, "fields", {
			fieldtype: "Link",
