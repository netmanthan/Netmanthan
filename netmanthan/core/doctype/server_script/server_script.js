...
netmanthan.ui.form.on("Server Script", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			frm.add_custom_button(__("Compare Versions"), () => {
				new netmanthan.ui.DiffView("Server Script", "script", frm.doc.name);
			});
		}
...
if netmanthan.form_dict.message == "ping":
	netmanthan.response['message'] = "pong"
else:
	netmanthan.response['message'] = "ok"
</code></pre>
...
tenant_id = netmanthan.db.get_value(...)
