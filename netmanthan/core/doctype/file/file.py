...
from netmanthan.permissions import get_doctypes_with_read
from netmanthan.utils import call_hook_method, cint, get_files_path, get_hook_method, get_url
from netmanthan.utils.file_manager import is_safe_path
...
class File(Document):
	...
	def autoname(self):
		...
		if self.is_folder:
			...
			self.name = netmanthan.generate_hash(length=10)
	...
	def before_insert(self):
		...
		if self.is_remote_file:
			...
			netmanthan.local.rollback_observers.append(self)
	...
	def validate(self):
		...
		self.file_size = netmanthan.form_dict.file_size or self.file_size
	...
	def validate_attachment_references(self):
		...
		if not self.attached_to_name or not isinstance(self.attached_to_name, (str, int)):
			netmanthan.throw(_("Attached To Name must be a string or an integer"), netmanthan.ValidationError)

		if self.attached_to_field and SPECIAL_CHAR_PATTERN.search(self.attached_to_field):
			netmanthan.throw(_("The fieldname you've specified in Attached To Field is invalid"))
	...
	def on_trash(self):
		if self.is_home_folder or self.is_attachments_folder:
			netmanthan.throw(_("Cannot delete Home and Attachments folders"))
	...
	def get_successors(self):
		return netmanthan.get_all("File", filters={"folder": self.name}, pluck="name")
	...
	def handle_is_private_changed(self):
		...
		private_file_path = Path(netmanthan.get_site_path("private", "files", file_name))
		public_file_path = Path(netmanthan.get_site_path("public", "files", file_name))
	...
	def fetch_attached_to_field(self, old_file_url):
		...
		reference_dict = netmanthan.get_doc(self.attached_to_doctype, self.attached_to_name).as_dict()
	...
	def validate_attachment_limit(self):
		...
		if self.attached_to_doctype and self.attached_to_name:
			attachment_limit = cint(netmanthan.get_meta(self.attached_to_doctype).max_attachments)
			...
			if current_attachment_count >= attachment_limit:
				netmanthan.throw(
					_("Maximum Attachment Limit of {0} has been reached for {1} {2}.").format(
						netmanthan.bold(attachment_limit), self.attached_to_doctype, self.attached_to_name
					),
	...
	def set_folder_name(self):
		...
		if self.attached_to_doctype:
			self.folder = netmanthan.db.get_value("File", {"is_attachments_folder": 1})
	...
	def validate_file_on_disk(self):
		...
		if not os.path.exists(full_path):
			netmanthan.throw(_("File {0} does not exist").format(self.file_url), IOError)
	...
	def validate_duplicate_entry(self):
		if not self.flags.ignore_duplicate_entry_error and not self.is_folder:
			...
			duplicate_file = netmanthan.db.get_value("File", filters, ["name", "file_url"], as_dict=1)

			if duplicate_file:
				duplicate_file_doc = netmanthan.get_cached_doc("File", duplicate_file.name)
	...
	def set_file_name(self):
		if not self.file_name and not self.file_url:
			netmanthan.throw(
				_("Fields `file_name` or `file_url` must be set for File"), exc=netmanthan.MandatoryError
			)
	...
	def generate_content_hash(self):
		...
		try:
			...
			netmanthan.throw(_("File {0} does not exist").format(file_path))
	...
	def make_thumbnail(
		...
		path = os.path.abspath(netmanthan.get_site_path("public", thumbnail_url.lstrip("/")))
		...
		try:
			...
			netmanthan.msgprint(_("Unable to write file format for {0}").format(path))
			return
	...
	def validate_empty_folder(self):
		...
		if self.is_folder and netmanthan.get_all("File", filters={"folder": self.name}, limit=1):
			netmanthan.throw(_("Folder {0} is not empty").format(self.name), FolderNotEmpty)

	def _delete_file_on_disk(self):
		"""If file not attached to any other record, delete it"""
		on_disk_file_not_shared = self.content_hash and not netmanthan.get_all(
			"File",
	...
	def unzip(self) -> list["File"]:
		...
		if not self.file_url.endswith(".zip"):
			netmanthan.throw(_("{0} is not a zip file").format(self.file_name))
		...
		with zipfile.ZipFile(zip_path) as z:
			for file in z.filelist:
				...
				file_doc = netmanthan.new_doc("File")
				try:
					file_doc.content = z.read(file.filename)
				except zipfile.BadZipFile:
					netmanthan.throw(_("{0} is a not a valid zip file").format(self.file_name))
		...
		netmanthan.delete_doc("File", self.name)
		return files
	...
	def get_content(self) -> bytes:
		if self.is_folder:
			netmanthan.throw(_("Cannot get file contents of a Folder"))
	...
	def get_full_path(self):
		...
		if file_path.startswith("/private/files/"):
			...
			netmanthan.throw(_("There is some problem with the file url: {0}").format(file_path))

		if not is_safe_path(file_path):
			netmanthan.throw(_("Cannot access file path {0}").format(file_path))

		if os.path.sep in self.file_name:
			netmanthan.throw(_("File name cannot have {0}").format(os.path.sep))
	...
	def save_file(
		...
		if (
			...
			and netmanthan.get_system_settings("strip_exif_metadata_from_uploaded_images")
		):
		...
		if not ignore_existing_file_check:
			duplicate_file = netmanthan.get_value(
				"File",
		...
		if duplicate_file:
			file_doc: "File" = netmanthan.get_cached_doc("File", duplicate_file.name)
	...
	def check_max_file_size(self):
		...
		if file_size > max_file_size:
			...
			if netmanthan.has_permission("System Settings", "write"):
				msg += ".<br>" + _("You can increase the limit from System Settings.")
			netmanthan.throw(msg, exc=MaxFileSizeReachedError)
	...
	def create_attachment_record(self):
		...
		file_url = (
			quote(netmanthan.safe_encode(self.file_url), safe="/:") if self.file_url else self.file_name
		)
	...
	def add_comment_in_reference_doc(self, comment_type, text):
		if self.attached_to_doctype and self.attached_to_name:
			try:
				doc = netmanthan.get_doc(self.attached_to_doctype, self.attached_to_name)
	...
	def zip_files(files):
		...
		for _file in files:
			if isinstance(_file, str):
				_file = netmanthan.get_doc("File", _file)
...
def on_doctype_update():
	netmanthan.db.add_index("File", ["attached_to_doctype", "attached_to_name"])
...
def has_permission(doc, ptype=None, user=None):
	...
	if ptype == "create":
		return netmanthan.has_permission("File", "create", user=user)
	...
	if doc.attached_to_doctype and doc.attached_to_name:
		...
		ref_doc = netmanthan.get_doc(attached_to_doctype, attached_to_name)
