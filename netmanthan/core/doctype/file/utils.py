...
import netmanthan
from netmanthan import _, safe_decode
from netmanthan.utils import cstr, encode, get_files_path, random_string, strip
from netmanthan.utils.file_manager import safe_b64decode
...
def setup_folder_path(filename: str, new_parent: str) -> None:
	file: "File" = netmanthan.get_doc("File", filename)
...
def get_local_image(file_url: str) -> tuple["ImageFile", str, str]:
	...
	file_path = netmanthan.get_site_path(*file_url_path)

	try:
		image = Image.open(file_path)
	except OSError:
		netmanthan.throw(_("Unable to read file format for {0}").format(file_url))
...
def get_web_image(file_url: str) -> tuple["ImageFile", str, str]:
	# download
	file_url = netmanthan.utils.get_url(file_url)
	r = requests.get(file_url, stream=True)
	try:
		r.raise_for_status()
	except requests.exceptions.HTTPError as e:
		if "404" in e.args[0]:
			netmanthan.msgprint(_("File '{0}' not found").format(file_url))
		else:
			netmanthan.msgprint(_("Unable to read file format for {0}").format(file_url))
		raise

	try:
		image = Image.open(BytesIO(r.content))
	except Exception as e:
		netmanthan.msgprint(_("Image link '{0}' is not valid").format(file_url), raise_exception=e)
...
def remove_file_by_url(file_url: str, doctype: str = None, name: str = None) -> "Document":
	if doctype and name:
		fid = netmanthan.db.get_value(
			"File", {"file_url": file_url, "attached_to_doctype": doctype, "attached_to_name": name}
		)
	else:
		fid = netmanthan.db.get_value("File", {"file_url": file_url})

	if fid:
		from netmanthan.utils.file_manager import remove_file
...
def get_file_name(fname: str, optional_suffix: str | None = None) -> str:
	...
	suffix = optional_suffix or netmanthan.generate_hash(length=6)
...
def extract_images_from_html(doc: "Document", content: str, is_private: bool = False):
	...
	def _save_file(match):
		...
		netmanthan.flags.has_dataurl = True
...
def update_existing_file_docs(doc: "File") -> None:
	...
	file_doctype = netmanthan.qb.DocType("File")
	(
		netmanthan.qb.update(file_doctype)
...
def attach_files_to_document(doc: "Document", event) -> None:
	...
	for df in attach_fields:
		...
		unattached_file = netmanthan.db.exists(
			"File",
			{
				"file_url": value,
