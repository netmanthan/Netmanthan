...
from netmanthan.core.api.file import (
	create_new_folder,
	get_attached_images,
	get_files_in_folder,
	move_file,
...
if TYPE_CHECKING:
	from netmanthan.core.doctype.file.file import File
...
def make_test_doc(ignore_permissions=False):
	...
	d.description = "Test"
	d.assigned_by = netmanthan.session.user
...
def make_test_image_file():
	file_path = netmanthan.get_app_path("netmanthan", "tests/data/sample_image_for_optimization.jpg")
	with open(file_path, "rb") as f:
		file_content = f.read()

	test_file = netmanthan.get_doc(
		{"doctype": "File", "file_name": "sample_image_for_optimization.jpg", "content": file_content}
	).insert()
	# remove those flags
	_test_file: "File" = netmanthan.get_doc("File", test_file.name)
...
class TestSimpleFile(netmanthanTestCase):
	def setUp(self):
	...
	def test_save(self):
		_file = netmanthan.get_doc("File", {"file_url": self.saved_file_url})
		content = _file.get_content()
		self.assertEqual(content, self.test_content)


class TestBase64File(netmanthanTestCase):
	def setUp(self):
		self.attached_to_doctype, self.attached_to_docname = make_test_doc()
		self.test_content = base64.b64encode(test_content1.encode("utf-8"))
		_file: "File" = netmanthan.get_doc(
			{
				"doctype": "File",
...
class TestSameFileName(netmanthanTestCase):
	def test_saved_content(self):
		self.attached_to_doctype, self.attached_to_docname = make_test_doc()
		self.test_content1 = test_content1
		self.test_content2 = test_content2
		_file1 = netmanthan.get_doc(
			{
				"doctype": "File",
		...
		_file = netmanthan.get_doc("File", {"file_url": self.saved_file_url1})
		content1 = _file.get_content()
		self.assertEqual(content1, self.test_content1)
		_file = netmanthan.get_doc("File", {"file_url": self.saved_file_url2})
	...
	def test_saved_content_private(self):
		...
		_file = netmanthan.get_doc("File", {"file_url": _file1.file_url})
		self.assertEqual(_file.get_content(), test_content1)

		_file = netmanthan.get_doc("File", {"file_url": _file2.file_url})
		self.assertEqual(_file.get_content(), test_content2)


class TestSameContent(netmanthanTestCase):
	def setUp(self):
	...
	def test_attachment_limit(self):
		...
		self.assertRaises(netmanthan.exceptions.AttachmentLimitReached, file2.insert)
		limit_property.delete()
		netmanthan.clear_cache(doctype="ToDo")


class TestFile(netmanthanTestCase):
	def setUp(self):
	...
	def tearDown(self):
		try:
			netmanthan.get_doc("File", {"file_name": "file_copy.txt"}).delete()
		except netmanthan.DoesNotExistError:
			pass

	def delete_test_data(self):
		test_file_data = netmanthan.get_all(
			"File",
			pluck="name",
	...
	def tests_after_upload(self):
		...
		file_folder = netmanthan.db.get_value("File", self.saved_name, "folder")
		self.assertEqual(file_folder, _("Home/Test Folder 1"))

	def test_file_copy(self):
		folder = self.get_folder("Test Folder 2", "Home")

		file = netmanthan.get_doc("File", {"file_name": "file_copy.txt"})
	...
	def test_folder_copy(self):
		...
		file = netmanthan.get_doc("File", {"file_name": "folder_copy.txt"})
		file_copy_txt = netmanthan.get_value("File", {"file_name": "file_copy.txt"})
		if file_copy_txt:
			netmanthan.get_doc("File", file_copy_txt).delete()

		self.assertEqual(_("Home/Test Folder 1/Test Folder 3"), file.folder)

	def test_default_folder(self):
		d = netmanthan.get_doc({"doctype": "File", "file_name": _("Test_Folder"), "is_folder": 1})
		d.save()
	...
	def test_on_delete(self):
		...
		self.assertEqual(netmanthan.db.get_value("File", _("Home/Test Folder 1"), "file_size"), 0)
		...
		folder = netmanthan.get_doc("File", "Home/Test Folder 1/Test Folder 3")
	...
	def test_same_file_url_update(self):
		...
		file2 = netmanthan.get_doc("File", file2.name)
	...
	def test_file_url_validation(self):
		test_file: "File" = netmanthan.new_doc("File")
		test_file.update({"file_name": "logo", "file_url": "https://netmanthan.io/files/netmanthan.png"})
	...
	def test_make_thumbnail(self):
		# test web image
		test_file: "File" = netmanthan.get_doc(
			{
				"doctype": "File",
				"file_name": "logo",
				"file_url": netmanthan.utils.get_url("/_test/assets/image.jpg"),
			}
		...
		test_file = netmanthan.get_doc(
			{
				"doctype": "File",
				"file_name": "logo",
				"file_url": netmanthan.utils.get_url("/_test/assets/image"),
			}
		).insert(ignore_permissions=True)
		...
		test_file.db_set("thumbnail_url", None)
		test_file.reload()
		test_file.file_url = netmanthan.utils.get_url("unknown.jpg")
		test_file.make_thumbnail(suffix="xs")
		self.assertEqual(
			json.loads(netmanthan.message_log[0]).get("message"),
			f"File '{netmanthan.utils.get_url('unknown.jpg')}' not found",
		)
		self.assertEqual(test_file.thumbnail_url, None)

	def test_file_unzip(self):
		file_path = netmanthan.get_app_path("netmanthan", "www/_test/assets/file.zip")
		public_file_path = netmanthan.get_site_path("public", "files")
		try:
			import shutil
...
class TestAttachment(netmanthanTestCase):
	...
	def tearDownClass(cls):
		...
		netmanthan.delete_doc("DocType", cls.test_doctype)

	def test_file_attachment_on_update(self):
		doc = netmanthan.get_doc(doctype=self.test_doctype, title="test for attachment on update").insert()
...
class TestAttachmentsAccess(netmanthanTestCase):
	def setUp(self) -> None:
		netmanthan.db.delete("File", {"is_folder": 0})
...
class TestFileUtils(netmanthanTestCase):
	def test_extract_images_from_doc(self):
		...
		self.assertTrue(netmanthan.db.exists("File", {"attached_to_name": todo.name}))
		...
		filename = netmanthan.db.exists("File", {"attached_to_name": todo.name})
		self.assertIn(f'<img src="{netmanthan.get_doc("File", filename).file_url}', todo.description)
...
class TestFileOptimization(netmanthanTestCase):
	...
	def test_optimize_svg(self):
		file_path = netmanthan.get_app_path("netmanthan", "tests/data/sample_svg.svg")
		with open(file_path, "rb") as f:
			file_content = f.read()
		test_file = netmanthan.get_doc(
	...
	def test_optimize_folder(self):
		test_folder = netmanthan.get_doc("File", "Home/Attachments")
...
class TestGuestFileAndAttachments(netmanthanTestCase):
	def setUp(self) -> None:
		netmanthan.db.delete("File", {"is_folder": 0})
		netmanthan.get_doc(
			doctype="DocType",
		...
		d = netmanthan.get_doc(
			{
				...
				"assigned_by": netmanthan.session.user,
			}
		)
		d.save()

		self.assertTrue(
			netmanthan.db.exists(
				"File",
				{
					"file_name": "test_private_guest_attachment.txt",
					"file_url": f.file_url,
