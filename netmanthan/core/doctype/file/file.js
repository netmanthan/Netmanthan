netmanthan.ui.form.on("File", {
	...
	preview_file: function (frm) {
		...
		if (netmanthan.utils.is_image_file(frm.doc.file_url)) {
		...
		} else if (netmanthan.utils.is_video_file(frm.doc.file_url)) {
			$preview = $(`<div class="img_preview">
	...
	optimize: function (frm) {
		netmanthan.show_alert(__("Optimizing image..."));
		frm.call("optimize_file").then(() => {
			netmanthan.show_alert(__("Image optimized"));
		});
	},

	unzip: function (frm) {
		netmanthan.call({
			method: "netmanthan.core.api.file.unzip_file",
			args: {
				name: frm.doc.name,
			},
			callback: function () {
