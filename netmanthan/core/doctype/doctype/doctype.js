...
netmanthan.ui.form.on("DocType", {
	refresh: function (frm) {
		frm.set_query("role", "permissions", function (doc) {
			if (doc.custom && netmanthan.session.user != "Administrator") {
				return {
					query: "netmanthan.core.doctype.role.role.role_query",
					filters: [["Role", "name", "!=", "All"]],
				};
			}
		});

		if (netmanthan.session.user !== "Administrator" || !netmanthan.boot.developer_mode) {
			if (frm.is_new()) {
		...
		if (!frm.is_new() && !frm.doc.istable) {
			if (frm.doc.issingle) {
				frm.add_custom_button(__("Go to {0}", [__(frm.doc.name)]), () => {
					window.open(`/app/${netmanthan.router.slug(frm.doc.name)}`);
				});
			} else {
		...
		if (!netmanthan.boot.developer_mode && !frm.doc.custom) {
		...
		} else if (netmanthan.boot.developer_mode) {
			let msg = __(
	...
	setup_default_views: (frm) => {
		netmanthan.model.set_default_views_for_doctype(frm.doc.name, frm);
	},
});

netmanthan.ui.form.on("DocField", {
	form_render(frm, doctype, docname) {
		...
		let row = netmanthan.get_doc(doctype, docname);
		...
		function update_fieldname_options() {
			...
			netmanthan.model.with_doctype(link_doctype, () => {
				let fields = netmanthan.meta
					.get_docfields(link_doctype, null, {
						fieldtype: ["not in", netmanthan.model.no_value_type],
					})
