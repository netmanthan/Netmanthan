...
from netmanthan.cache_manager import clear_controller_cache, clear_user_cache
...
from netmanthan.database.schema import validate_column_length, validate_column_name
from netmanthan.desk.notifications import delete_notification_count_for, get_filters_for
from netmanthan.desk.utils import validate_route_conflict
from netmanthan.model import (
	child_table_fields,
	data_field_options,
	default_fields,
	no_value_fields,
...
from netmanthan.modules import get_doc_path, make_boilerplate
from netmanthan.modules.import_file import get_file_path
from netmanthan.query_builder.functions import Concat
from netmanthan.utils import cint, is_a_property, random_string
...
class InvalidFieldNameError(netmanthan.ValidationError):
	pass


class UniqueFieldnameError(netmanthan.ValidationError):
	pass


class IllegalMandatoryError(netmanthan.ValidationError):
	pass


class DoctypeLinkError(netmanthan.ValidationError):
	pass


class WrongOptionsDoctypeLinkError(netmanthan.ValidationError):
	pass


class HiddenAndMandatoryWithoutDefaultError(netmanthan.ValidationError):
	pass


class NonUniqueError(netmanthan.ValidationError):
	pass


class CannotIndexedError(netmanthan.ValidationError):
	pass


class CannotCreateStandardDoctypeError(netmanthan.ValidationError):
	pass
...
class DocType(Document):
	...
	def validate(self):
		...
		if not self.is_new():
			self.before_update = netmanthan.get_doc("DocType", self.name)
		...
		if self.default_print_format and not self.custom:
			netmanthan.throw(_("Standard DocType cannot have default print format, use Customize Form"))
	...
	def check_developer_mode(self):
		...
		if netmanthan.flags.in_patch or netmanthan.flags.in_test:
			return

		if not netmanthan.conf.get("developer_mode") and not self.custom:
			netmanthan.throw(
		...
		if netmanthan.conf.get("developer_mode"):
	...
	def setup_fields_to_fetch(self):
		...
		try:
			old_meta = netmanthan.get_meta(netmanthan.get_doc("DocType", self.name), cached=False)
			old_fields_to_fetch = [df.fieldname for df in old_meta.get_fields_to_fetch()]
		except netmanthan.DoesNotExistError:
			old_fields_to_fetch = []

		new_meta = netmanthan.get_meta(self, cached=False)
	...
	def validate_website(self):
		...
		if self.has_web_view:
			...
			if not "route" in [d.fieldname for d in self.fields]:
				netmanthan.throw(_('Field "route" is mandatory for Web Views'), title="Missing Field")
	...
	def ensure_minimum_max_attachment_limit(self):
		...
		from netmanthan.model import attachment_fieldtypes
		...
		if total_attach_fields > self.max_attachments:
			...
			field_label = netmanthan.bold(self.meta.get_field("max_attachments").label)
			netmanthan.msgprint(
	...
	def change_modified_of_parent(self):
		...
		if netmanthan.flags.in_import:
			return
		parent_list = netmanthan.get_all(
			"DocField", "parent", dict(fieldtype=["in", netmanthan.model.table_fields], options=self.name)
		)
		for p in parent_list:
			netmanthan.db.update("DocType", p.parent, {}, for_update=False)
			...
			if d.fieldtype:
				if not getattr(d, "fieldname", None):
					if d.label:
						...
						netmanthan.throw(_("Row #{}: Fieldname is required").format(d.idx), title="Missing Fieldname")
				else:
					if d.fieldname in restricted:
						netmanthan.throw(_("Fieldname {0} is restricted").format(d.fieldname), InvalidFieldNameError)
	...
	def on_update(self):
		...
		try:
			netmanthan.db.updatedb(self.name, Meta(self))
		...
		allow_doctype_export = (
			not self.custom
			and not netmanthan.flags.in_import
			and (netmanthan.conf.developer_mode or netmanthan.flags.allow_doctype_export)
		)
		...
		netmanthan.clear_cache(doctype=self.name)

		# clear user cache so that on the next reload this doctype is included in boot
		clear_user_cache(netmanthan.session.user)

		if not netmanthan.flags.in_install and hasattr(self, "before_update"):
	...
	def setup_autoincrement_and_sequence(self):
		...
		name_type = f"varchar({netmanthan.db.VARCHAR_LEN})"

		if self.autoname == "autoincrement":
			name_type = "bigint"
			netmanthan.db.create_sequence(self.name, check_not_exists=True, cache=netmanthan.db.SEQUENCE_CACHE)
	...
	def sync_global_search(self):
		...
		if set(global_search_fields_before_update) != set(global_search_fields_after_update):
			now = (not netmanthan.request) or netmanthan.flags.in_test or netmanthan.flags.in_install
			netmanthan.enqueue("netmanthan.utils.global_search.rebuild_for_doctype", now=now, doctype=self.name)
	...
	def set_base_class_for_controller(self):
		...
		scrubbed_name = netmanthan.scrub(self.name)
		scrubbed_module = netmanthan.scrub(self.module)
		controller_path = netmanthan.get_module_path(
			scrubbed_module, "doctype", scrubbed_name, f"{scrubbed_name}.py"
		)

		document_cls_tag = f"class {despaced_name}(Document)"
		document_import_tag = "from netmanthan.model.document import Document"
		website_generator_cls_tag = f"class {despaced_name}(WebsiteGenerator)"
		website_generator_import_tag = "from netmanthan.website.website_generator import WebsiteGenerator"
	...
	def before_rename(self, old, new, merge=False):
		...
		if not self.custom and netmanthan.session.user != "Administrator":
			netmanthan.throw(_("DocType can only be renamed by Administrator"))

		self.check_developer_mode()
		self.validate_name(new)

		if merge:
			netmanthan.throw(_("DocType can not be merged"))
	...
	def after_rename(self, old, new, merge=False):
		...
		if self.issingle:
			netmanthan.db.sql("""update tabSingles set doctype=%s where doctype=%s""", (new, old))
			netmanthan.db.sql(
	...
	def rename_files_and_folders(self, old, new):
		...
		for fname in os.listdir(new_path):
			if netmanthan.scrub(old) in fname:
				old_file_name = os.path.join(new_path, fname)
				new_file_name = os.path.join(new_path, fname.replace(netmanthan.scrub(old), netmanthan.scrub(new)))
				shutil.move(old_file_name, new_file_name)

		self.rename_inside_controller(new, old, new_path)
		netmanthan.msgprint(_("Renamed files and replaced code in controllers, please check!"))

	def rename_inside_controller(self, new, old, new_path):
		for fname in ("{}.js", "{}.py", "{}_list.js", "{}_calendar.js", "test_{}.py", "test_{}.js"):
			fname = os.path.join(new_path, fname.format(netmanthan.scrub(new)))
				...
				with open(fname, "w") as f:
					if fname.endswith(".js"):
						...
						file_content = code.replace(
							netmanthan.scrub(old), netmanthan.scrub(new)
		...
		doctype_json_path = os.path.join(new_path, f"{netmanthan.scrub(new)}.json")
		current_data = netmanthan.get_file_json(doctype_json_path)
	...
	def preserve_naming_series_options_in_property_setter(self):
		...
		if not (
                netmanthan.db.table_exists(self.name)
                and netmanthan.get_all(self.name, fields=["name"], limit=1, as_list=True)
		):
			return

		existing_property_setter = netmanthan.db.get_value(
	...
	def make_controller_template(self):
		...
		if self.has_web_view:
			templates_path = netmanthan.get_module_path(
				netmanthan.scrub(self.module), "doctype", netmanthan.scrub(self.name), "templates"
			)
	...
	def make_amendable(self):
		...
		if self.is_submittable:
			docfield_exists = netmanthan.get_all(
	...
	def make_repeatable(self):
		...
		if self.allow_auto_repeat:
			if not netmanthan.db.exists(
				"Custom Field", {"fieldname": "auto_repeat", "dt": self.name}
			) and not netmanthan.db.exists(
	...
	def validate_nestedset(self):
		...
		if not self.nsm_parent_field:
			field_label = netmanthan.bold(_("Parent Field (Tree)"))
			netmanthan.throw(_("{0} is a mandatory field").format(field_label), netmanthan.MandatoryError)

		# check if field is valid
		fieldnames = [df.fieldname for df in self.fields]
		if self.nsm_parent_field and self.nsm_parent_field not in fieldnames:
			netmanthan.throw(_("Parent Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def add_nestedset_fields(self):
		...
		parent_field_name = netmanthan.scrub(parent_field_label)
		self.append(
			"fields",
			{
	...
	def get_max_idx(self):
		...
		max_idx = netmanthan.db.sql("""select max(idx) from `tabDocField` where parent = %s""", self.name)
	...
	def validate_name(self, name=None):
		...
		max_length = netmanthan.db.MAX_COLUMN_LENGTH - 3
		if len(name) > max_length:
			# length(tab + <Doctype Name>) should be equal to 64 characters hence doctype should be 61 characters
			netmanthan.throw(
				_("Doctype name is limited to {0} characters ({1})").format(max_length, name), netmanthan.NameError
			)

		# a DocType name should not start or end with an empty space
		if WHITESPACE_PADDING_PATTERN.search(name):
			netmanthan.throw(_("DocType's name should not start or end with whitespace"), netmanthan.NameError)
...
def validate_series(dt, autoname=None, name=None):
	...
	if not autoname and dt.get("fields", {"fieldname": "naming_series"}):
		...
		if not dt.get("fields", {"fieldname": fieldname}):
			netmanthan.throw(
				_("Fieldname called {0} must exist to enable autonaming").format(netmanthan.bold(fieldname)),
	...
	if autoname and autoname.startswith("field:"):
		...
		if not field or field not in [df.fieldname for df in dt.fields]:
			netmanthan.throw(_("Invalid fieldname '{0}' in autoname").format(field))
		else:
			for df in dt.fields:
	...
	if (
		...
		doctype = netmanthan.qb.DocType("DocType")
		used_in = (
		...
		if used_in:
			netmanthan.throw(_("Series {0} already used in {1}").format(prefix, used_in[0][0]))
...
def validate_autoincrement_autoname(dt: Union[DocType, "CustomizeForm"]) -> bool:
	...
	def get_autoname_before_save(dt: Union[DocType, "CustomizeForm"]) -> str:
		if dt.doctype == "Customize Form":
			property_value = netmanthan.db.get_value(
			...
			if not property_value:
				return netmanthan.db.get_value("DocType", dt.doc_type, "autoname") or ""
	...
	if not dt.is_new():
		...
		if (
			...
			if dt.doctype == "Customize Form":
				netmanthan.throw(_("Cannot change to/from autoincrement autoname in Customize Form"))

			if netmanthan.get_meta(dt.name).issingle:
				return False

			if not netmanthan.get_all(dt.name, limit=1):
...
def change_name_column_type(doctype_name: str, type: str) -> None:
	...
	netmanthan.db.change_column_type(*args)


def validate_links_table_fieldnames(meta):
	"""Validate fieldnames in Links table"""
	if not meta.links or netmanthan.flags.in_patch or netmanthan.flags.in_fixtures or netmanthan.flags.in_migrate:
		return
	...
	for index, link in enumerate(meta.links, 1):
		...
		if not link.parent_doctype:
			...
			netmanthan.throw(message, netmanthan.ValidationError, _("Parent Missing"))

		if not link.table_fieldname:
			message = _("Document Links Row #{0}: Table Fieldname is mandatory for internal links").format(
				index
			)
			netmanthan.throw(message, netmanthan.ValidationError, _("Table Fieldname Missing"))

		if meta.name == link.parent_doctype:
			field_exists = link.table_fieldname in fieldnames
		else:
			field_exists = netmanthan.get_meta(link.parent_doctype).has_field(link.table_fieldname)

		if not field_exists:
			message = _("Document Links Row #{0}: Could not find field {1} in {2} DocType").format(
				index, netmanthan.bold(link.table_fieldname), netmanthan.bold(meta.name)
			)
			netmanthan.throw(message, netmanthan.ValidationError, _("Invalid Table Fieldname"))
...
def _test_connection_query(doctype, field, idx):
	...
	try:
		netmanthan.get_all(doctype, filters=filters, limit=1, distinct=True, ignore_ifnull=True)
	except Exception as e:
		netmanthan.clear_last_message()
		msg = _("Document Links Row #{0}: Invalid doctype or fieldname.").format(idx)
		msg += "<br>" + str(e)
		netmanthan.throw(msg, InvalidFieldNameError)


def validate_fields_for_doctype(doctype):
	meta = netmanthan.get_meta(doctype, cached=False)
...
def validate_fields(meta):
	...
	:param meta: `netmanthan.model.meta.Meta` object to check."""
		...
		if fieldname in Document._reserved_keywords:
			netmanthan.throw(
				_("{0}: fieldname cannot be set to reserved keyword {1}").format(
					netmanthan.bold(docname),
					netmanthan.bold(fieldname),
				),
	...
	def check_link_table_options(docname, d):
		if netmanthan.flags.in_patch or netmanthan.flags.in_fixtures:
			return
		...
		if d.fieldtype in ("Link",) + table_fields:
			...
			if d.options != d.parent:
				options = netmanthan.db.get_value("DocType", d.options, "name")
				if not options:
					netmanthan.throw(
	...
	def check_hidden_and_mandatory(docname, d):
		if d.hidden and d.reqd and not d.default and not netmanthan.flags.in_migrate:
			netmanthan.throw(
	...
	def check_width(d):
		if d.fieldtype == "Currency" and cint(d.width) < 100:
			netmanthan.throw(_("Max width for type Currency is 100px in row {0}").format(d.idx))
	...
	def check_illegal_default(d):
		...
		if d.fieldtype == "Select" and d.default:
			if not d.options:
				...
				netmanthan.throw(
					_("Default value for {0} must be in the list of options.").format(netmanthan.bold(d.fieldname))
				)
	...
	def check_precision(d):
		if (
			...
			netmanthan.throw(_("Precision should be between 1 and 6"))
	...
	def check_unique_and_text(docname, d):
		...
		if getattr(d, "unique", False):
			...
			if not d.get("__islocal") and netmanthan.db.has_column(d.parent, d.fieldname):
				has_non_unique_values = netmanthan.db.sql(
	...
	def check_fold(fields):
		...
		for i, f in enumerate(fields):
			if f.fieldtype == "Fold":
				if fold_exists:
					netmanthan.throw(_("There can be only one Fold in a form"))
				fold_exists = True
				if i < len(fields) - 1:
					nxt = fields[i + 1]
					if nxt.fieldtype != "Section Break":
						netmanthan.throw(_("Fold must come before a Section Break"))
				else:
					netmanthan.throw(_("Fold can not be at the end of the form"))
	...
	def check_search_fields(meta, fields):
		...
		for fieldname in search_fields:
			...
			if (fieldtype_mapper.get(fieldname) in no_value_fields) or (fieldname not in fieldname_list):
				netmanthan.throw(_("Search field {0} is not valid").format(fieldname))
	...
	def check_title_field(meta):
		...
		if meta.title_field not in fieldname_list:
			netmanthan.throw(_("Title field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_image_field(meta):
		...
		if not df:
			netmanthan.throw(_("Image field must be a valid fieldname"), InvalidFieldNameError)
		if df[0].fieldtype != "Attach Image":
			netmanthan.throw(_("Image field must be of type Attach Image"), InvalidFieldNameError)
	...
	def check_is_published_field(meta):
		...
		if meta.is_published_field not in fieldname_list:
			netmanthan.throw(_("Is Published Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_website_search_field(meta):
		...
		if meta.website_search_field not in fieldname_list:
			netmanthan.throw(_("Website Search Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_timeline_field(meta):
		...
		if meta.timeline_field not in fieldname_list:
			netmanthan.throw(_("Timeline field must be a valid fieldname"), InvalidFieldNameError)

		df = meta.get("fields", {"fieldname": meta.timeline_field})[0]
		if df.fieldtype not in ("Link", "Dynamic Link"):
			netmanthan.throw(_("Timeline field must be a Link or Dynamic Link"), InvalidFieldNameError)
	...
	def check_illegal_depends_on_conditions(docfield):
		...
		for field in depends_on_fields:
			...
			if depends_on and ("=" in depends_on) and DEPENDS_ON_PATTERN.match(depends_on):
				netmanthan.throw(_("Invalid {0} condition").format(netmanthan.unscrub(field)), netmanthan.ValidationError)
	...
	def validate_data_field_type(docfield):
		...
		if docfield.fieldtype == "Data" and not (
			...
			if docfield.options and (docfield.options not in data_field_options):
				df_str = netmanthan.bold(_(docfield.label))
				text_str = (
				...
				netmanthan.msgprint(text_str + df_options_str, title="Invalid Data Field", alert=True)

	def check_child_table_option(docfield):
		if netmanthan.flags.in_fixtures:
			return
		...
		if not meta.istable:
			netmanthan.throw(
				_("Option {0} for field {1} is not a child table").format(
					netmanthan.bold(doctype), netmanthan.bold(docfield.fieldname)
				),
	...
	def check_max_height(docfield):
		if getattr(docfield, "max_height", None) and (docfield.max_height[-2:] not in ("px", "em")):
			netmanthan.throw(f"Max for {netmanthan.bold(docfield.fieldname)} height must be in px, em, rem")
	...
	def check_no_of_ratings(docfield):
		if docfield.fieldtype == "Rating":
			if docfield.options and (int(docfield.options) > 10 or int(docfield.options) < 3):
				netmanthan.throw(_("Options for Rating field can range from 3 to 10"))
	...
	for d in fields:
		...
		if not netmanthan.flags.in_migrate:
...
def validate_permissions_for_doctype(doctype, for_remove=False, alert=False):
	...
	doctype = netmanthan.get_doc("DocType", doctype)
...
def clear_permissions_cache(doctype):
	...
	for user in netmanthan.db.sql_list(
		"""
		SELECT
		...
		netmanthan.clear_cache(user=user)
...
def validate_permissions(doctype, for_remove=False, alert=False):
	...
	if not permissions and alert:
		netmanthan.msgprint(_("No Permissions Specified"), alert=True, indicator="orange")
	...
	def check_atleast_one_set(d):
		if (
			...
			netmanthan.throw(_("{0}: No basic permissions set").format(get_txt(d)))
	...
	def check_permission_dependency(d):
		if d.cancel and not d.submit:
			netmanthan.throw(_("{0}: Cannot set Cancel without Submit").format(get_txt(d)))

		if (d.submit or d.cancel or d.amend) and not d.write:
			netmanthan.throw(_("{0}: Cannot set Submit, Cancel, Amend without Write").format(get_txt(d)))
		if d.amend and not d.write:
			netmanthan.throw(_("{0}: Cannot set Amend without Cancel").format(get_txt(d)))
		if d.get("import") and not d.create:
			netmanthan.throw(_("{0}: Cannot set Import without Create").format(get_txt(d)))
	...
	def remove_rights_for_single(d):
		...
		if d.report:
			netmanthan.msgprint(_("Report cannot be set for Single types"))
			d.report = 0
			d.set("import", 0)
			d.set("export", 0)
		...
		for ptype, label in [["set_user_permissions", _("Set User Permissions")]]:
			if d.get(ptype):
				d.set(ptype, 0)
				netmanthan.msgprint(_("{0} cannot be set for Single types").format(label))

	def check_if_submittable(d):
		if d.submit and not issubmittable:
			netmanthan.throw(_("{0}: Cannot set Assign Submit if not Submittable").format(get_txt(d)))
		elif d.amend and not issubmittable:
			netmanthan.throw(_("{0}: Cannot set Assign Amend if not Submittable").format(get_txt(d)))

	def check_if_importable(d):
		if d.get("import") and not isimportable:
			netmanthan.throw(_("{0}: Cannot set import as {1} is not importable").format(get_txt(d), doctype))
			...
			if d.role == "All":
				netmanthan.throw(
					_("Row # {0}: Non administrator user can not set the role {1} to the custom doctype").format(
						d.idx, netmanthan.bold(_("All"))
					),
					title=_("Permissions Error"),
				)

			roles = [row.name for row in netmanthan.get_all("Role", filters={"is_custom": 1})]

			if d.role in roles:
				netmanthan.throw(
					_("Row # {0}: Non administrator user can not set the role {1} to the custom doctype").format(
						d.idx, netmanthan.bold(_(d.role))
					),
					title=_("Permissions Error"),
				)
...
def make_module_and_roles(doc, perm_fieldname="permissions"):
	...
	try:
		if (
			...
			and not netmanthan.db.exists("Domain", doc.restrict_to_domain)
		):
			netmanthan.get_doc(dict(doctype="Domain", domain=doc.restrict_to_domain)).insert()

		if "tabModule Def" in netmanthan.db.get_tables() and not netmanthan.db.exists("Module Def", doc.module):
			m = netmanthan.get_doc({"doctype": "Module Def", "module_name": doc.module})
			if netmanthan.scrub(doc.module) in netmanthan.local.module_app:
				m.app_name = netmanthan.local.module_app[netmanthan.scrub(doc.module)]
			else:
				m.app_name = "netmanthan"
			m.flags.ignore_mandatory = m.flags.ignore_permissions = True
			if netmanthan.flags.package:
				m.package = netmanthan.flags.package.name
				m.custom = 1
			m.insert()
		...
		for role in list(set(roles)):
			if netmanthan.db.table_exists("Role", cached=False) and not netmanthan.db.exists("Role", role):
				r = netmanthan.new_doc("Role")
				r.role_name = role
				r.desk_access = 1
				r.flags.ignore_mandatory = r.flags.ignore_permissions = True
				r.insert()
	except netmanthan.DoesNotExistError as e:
		pass
	except netmanthan.db.ProgrammingError as e:
		if netmanthan.db.is_table_missing(e):
			pass
		else:
			raise
...
def check_fieldname_conflicts(docfield):
	...
	doc = netmanthan.get_doc({"doctype": docfield.dt})
	...
	if docfield.fieldname in method_list + property_list:
		netmanthan.msgprint(msg, raise_exception=not docfield.is_virtual)


def clear_linked_doctype_cache():
	netmanthan.cache().delete_value("linked_doctypes_without_ignore_user_permissions_enabled")
...
def check_email_append_to(doc):
	...
	if doc.subject_field and not subject_field:
		netmanthan.throw(_("Select a valid Subject field for creating documents from Email"))
	...
	if subject_field and subject_field.fieldtype not in [
		...
		netmanthan.throw(_("Subject Field type should be Data, Text, Long Text, Small Text, Text Editor"))
	...
	if doc.sender_field and not sender_field:
		netmanthan.throw(_("Select a valid Sender Field for creating documents from Email"))

	if not sender_field.options == "Email":
		netmanthan.throw(_("Sender Field should have Email in options"))
