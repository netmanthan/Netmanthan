...
import netmanthan
from netmanthan.cache_manager import clear_doctype_cache
...
class TestDocType(netmanthanTestCase):
	def tearDown(self):
		netmanthan.db.rollback()

	def test_validate_name(self):
		self.assertRaises(netmanthan.NameError, new_doctype("_Some DocType").insert)
		self.assertRaises(netmanthan.NameError, new_doctype("8Some DocType").insert)
		self.assertRaises(netmanthan.NameError, new_doctype("Some (DocType)").insert)
		self.assertRaises(
		...
		for name in ("Some DocType", "Some_DocType", "Some-DocType"):
			if netmanthan.db.exists("DocType", name):
				netmanthan.delete_doc("DocType", name)

			doc = new_doctype(name).insert()
			doc.delete()

	def test_making_sequence_on_change(self):
		netmanthan.delete_doc_if_exists("DocType", self._testMethodName)
		...
		self.assertEqual(
			...
			"varchar" if netmanthan.db.db_type == "mariadb" else "character varying",
		)

	def test_doctype_unique_constraint_dropped(self):
		if netmanthan.db.exists("DocType", "With_Unique"):
			netmanthan.delete_doc("DocType", "With_Unique")

		dt = new_doctype("With_Unique", unique=1)
		dt.insert()

		doc1 = netmanthan.new_doc("With_Unique")
		doc2 = netmanthan.new_doc("With_Unique")
		...
		doc1.insert()
		self.assertRaises(netmanthan.UniqueValidationError, doc2.insert)
		netmanthan.db.rollback()
	...
	def test_validate_search_fields(self):
		...
		self.assertRaises(netmanthan.ValidationError, doc.save)
	...
	def test_sync_field_order(self):
		...
		# create test doctype
		test_doctype = netmanthan.get_doc(
			{
				"doctype": "DocType",
				"module": "Core",
				"fields": [
		...
		netmanthan.delete_doc_if_exists("DocType", "Test Field Order DocType")
		if os.path.isfile(path):
			os.remove(path)

		try:
			netmanthan.flags.allow_doctype_export = 1
			test_doctype.save()

			# assert that field_order list is being created with the default order
			test_doctype_json = netmanthan.get_file_json(path)
			...
			with open(path, "w+") as txtfile:
				txtfile.write(netmanthan.as_json(test_doctype_json))
			...
			netmanthan.reload_doctype(test_doctype.name, force=True)
			test_doctype.reload()
			...
			netmanthan.flags.allow_doctype_export = 0
	...
	def test_cancel_link_doctype(self):
		...
		from netmanthan.desk.form.linked_with import cancel_all_linked_docs, get_submitted_linked_docs
		...
		# create doctype data
		data_link_doc = netmanthan.new_doc("Test Linked Doctype")
		data_link_doc.some_fieldname = "Data1"
		data_link_doc.insert()
		data_link_doc.save()
		data_link_doc.submit()

		data_doc = netmanthan.new_doc("Test Doctype")
	...
	def test_ignore_cancelation_of_linked_doctype_during_cancel(self):
		...
		# create doctype data
		data_link_doc_1 = netmanthan.new_doc("Test Linked Doctype 1")
		data_link_doc_1.some_fieldname = "Data1"
		data_link_doc_1.insert()
		data_link_doc_1.save()
		data_link_doc_1.submit()

		data_doc_2 = netmanthan.new_doc("Test Doctype 1")
		...
		data_doc = netmanthan.new_doc("Test Doctype 2")
		data_doc.some_fieldname = "Data1"
		...
		self.assertRaises(netmanthan.LinkExistsError, data_link_doc_1.cancel)
	...
	def test_create_virtual_doctype(self):
		...
		virtual_doc.reload()
		doc = netmanthan.get_doc("DocType", "Test Virtual Doctype")

		self.assertDictEqual(doc.as_dict(), virtual_doc.as_dict())
		self.assertEqual(doc.is_virtual, 1)
		self.assertFalse(netmanthan.db.table_exists("Test Virtual Doctype"))

	def test_create_virtual_doctype_as_child_table(self):
		"""Test virtual DocType as Child Table below a normal DocType."""
		netmanthan.delete_doc_if_exists("DocType", "Test Parent Virtual DocType", force=1)
		netmanthan.delete_doc_if_exists("DocType", "Test Virtual DocType as Child Table", force=1)
		...
		doc = netmanthan.get_doc("DocType", "Test Virtual DocType as Child Table")

		self.assertEqual(doc.is_virtual, 1)
		self.assertEqual(doc.istable, 1)
		self.assertFalse(netmanthan.db.table_exists("Test Virtual DocType as Child Table"))
		...
		parent_doc_entry = netmanthan.get_doc(
	...
	def test_autoincremented_doctype_transition(self):
		netmanthan.delete_doc_if_exists("DocType", "testy_autoinc_dt")
		dt = new_doctype("testy_autoinc_dt", autoname="autoincrement").insert(ignore_permissions=True)
		dt.autoname = "hash"

		dt.save(ignore_permissions=True)

		dt_data = netmanthan.get_doc({"doctype": dt.name, "some_fieldname": "test data"}).insert(
		...
		try:
			dt.save(ignore_permissions=True)
		except netmanthan.ValidationError as e:
			self.assertEqual(
				e.args[0],
	...
	def test_json_field(self):
		...
		json_doc.insert()
		json_doc.save()
		doc = netmanthan.get_doc("DocType", "Test Json Doctype")
		...
		test_json = netmanthan.get_doc("Test Json Doctype", doc.name)
	...
	@patch.dict(netmanthan.conf, {"developer_mode": 1})
	def test_custom_field_deletion(self):
		...
		netmanthan.delete_doc("DocType", child)
		self.assertFalse(netmanthan.get_meta(doctype).get_field(field))
	...
	def test_delete_doctype_with_customization(self):
		...
		# Create 1 record
		original_doc = netmanthan.get_doc(doctype=doctype, custom_field_name="wat").insert()
		...
		restored_doc = netmanthan.get_last_doc(doctype)
		...
		self.assertEqual(netmanthan.get_meta(doctype).get_field(field).default, "DELETETHIS")
		netmanthan.delete_doc("DocType", doctype)
	...
	def test_not_in_list_view_for_not_allowed_mandatory_field(self):
		...
		netmanthan.delete_doc("DocType", doctype.name)
