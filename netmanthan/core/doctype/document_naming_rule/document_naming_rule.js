...
netmanthan.ui.form.on("Document Naming Rule", {
	...
	document_type: (frm) => {
		...
		if (frm.doc.document_type) {
			netmanthan.model.with_doctype(frm.doc.document_type, () => {
				let fieldnames = netmanthan
					.get_meta(frm.doc.document_type)
					.fields.filter((d) => {
						return netmanthan.model.no_value_type.indexOf(d.fieldtype) === -1;
					})
					.map((d) => {
