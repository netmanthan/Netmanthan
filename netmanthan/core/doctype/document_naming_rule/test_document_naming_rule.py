...
class TestDocumentNamingRule(netmanthanTestCase):
	def test_naming_rule_by_series(self):
		naming_rule = netmanthan.get_doc(
			dict(doctype="Document Naming Rule", document_type="ToDo", prefix="test-todo-", prefix_digits=5)
		).insert()

		todo = netmanthan.get_doc(
			dict(doctype="ToDo", description="Is this my name " + netmanthan.generate_hash())
		).insert()
	...
	def test_naming_rule_by_condition(self):
		...
		# another rule
		naming_rule_1 = netmanthan.copy_doc(naming_rule)
		...
		naming_rule_2 = netmanthan.copy_doc(naming_rule)
		...
		todo = netmanthan.get_doc(
			dict(doctype="ToDo", priority="High", description="Is this my name " + netmanthan.generate_hash())
		).insert()

		todo_1 = netmanthan.get_doc(
			dict(doctype="ToDo", priority="Medium", description="Is this my name " + netmanthan.generate_hash())
		).insert()

		todo_2 = netmanthan.get_doc(
			dict(doctype="ToDo", priority="Low", description="Is this my name " + netmanthan.generate_hash())
		).insert()
