...
from netmanthan.utils.background_jobs import get_queues, get_redis_conn
...
class RQJob(Document):
	...
	def stop_job(self):
		try:
			...
			netmanthan.msgprint(_("Job is not running."), title=_("Invalid Operation"))
...
def serialize_job(job: Job) -> netmanthan._dict:
	...
	return netmanthan._dict(
		...
		arguments=netmanthan.as_json(job.kwargs),
		timeout=job.timeout,
...
def for_current_site(job: Job) -> bool:
	return job.kwargs.get("site") == netmanthan.local.site
...
@netmanthan.whitelist()
def stop_job(job_id):
	netmanthan.get_doc("RQ Job", job_id).stop_job()
