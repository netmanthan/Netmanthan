...
netmanthan.ui.form.on("Data Import", {
	setup(frm) {
		netmanthan.realtime.on("data_import_refresh", ({ data_import }) => {
			frm.import_in_progress = false;
			if (data_import !== frm.doc.name) return;
			netmanthan.model.clear_doc("Data Import", frm.doc.name);
			netmanthan.model.with_doc("Data Import", frm.doc.name).then(() => {
				frm.refresh();
			});
		});
		netmanthan.realtime.on("data_import_progress", (data) => {
		...
		frm.set_query("reference_doctype", () => {
			return {
				filters: {
					name: ["in", netmanthan.boot.user.can_import],
				},
			};
		});
	...
	refresh(frm) {
		...
		if (frm.doc.status.includes("Success")) {
			frm.add_custom_button(__("Go to {0} List", [__(frm.doc.reference_doctype)]), () =>
				netmanthan.set_route("List", frm.doc.reference_doctype)
			);
		}
	},
	...
	update_indicators(frm) {
		const indicator = netmanthan.get_indicator(frm.doc);
		if (indicator) {
	...
	show_import_status(frm) {
		netmanthan.call({
			method: "netmanthan.core.doctype.data_import.data_import.get_import_status",
			args: {
				...
				.then((result) => {
					if (result.length > 0) {
						frm.add_custom_button("Report Error", () => {
							...
							netmanthan.request.report_error(fake_xhr, {});
						});
					}
				});
		}
	...
	download_template(frm) {
		netmanthan.require("data_import_tools.bundle.js", () => {
			frm.data_exporter = new netmanthan.data_import.DataExporter(
	...
	show_import_preview(frm, preview_data) {
		...
		netmanthan.require("data_import_tools.bundle.js", () => {
			frm.import_preview = new netmanthan.data_import.ImportPreview({
	...
	export_errored_rows(frm) {
		open_url_post(
			"/api/method/netmanthan.core.doctype.data_import.data_import.download_errored_template",
			{
				data_import_name: frm.doc.name,
			}
		);
	...
	export_import_log(frm) {
		open_url_post(
			"/api/method/netmanthan.core.doctype.data_import.data_import.download_import_log",
			{
				data_import_name: frm.doc.name,
			}
		);
						...
						if (log.success) {
							if (frm.doc.import_type === "Insert New Records") {
								html = __("Successfully imported {0}", [
									`<span class="underline">${netmanthan.utils.get_form_link(
										frm.doc.reference_doctype,
										log.docname,
										true
									)}<span>`,
							...
							let messages = JSON.parse(log.messages || "[]")
								...
								.join("");
							let id = netmanthan.dom.get_unique_id();
							html = `${messages}
	...
	show_import_log(frm) {
		...
		netmanthan.call({
			method: "netmanthan.client.get_count",
			args: {
				doctype: "Data Import Log",
				filters: {
					data_import: frm.doc.name,
