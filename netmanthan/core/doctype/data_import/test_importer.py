...
from netmanthan.utils import format_duration, getdate

doctype_name = "DocType for Import"


class TestImporter(netmanthanTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def test_data_import_from_file(self):
		...
		doc1 = netmanthan.get_doc(doctype_name, "Test")
		doc2 = netmanthan.get_doc(doctype_name, "Test 2")
		doc3 = netmanthan.get_doc(doctype_name, "Test 3")
	...
	def test_data_import_without_mandatory_values(self):
		...
		self.assertEqual(netmanthan.parse_json(import_log[0]["row_indexes"]), [2, 3])
		expected_error = (
			"Error: <strong>Child 1 of DocType for Import</strong> Row #1: Value missing for: Child Title"
		)
		self.assertEqual(
			netmanthan.parse_json(netmanthan.parse_json(import_log[0]["messages"])[0])["message"], expected_error
		)
		expected_error = (
			"Error: <strong>Child 1 of DocType for Import</strong> Row #2: Value missing for: Child Title"
		)
		self.assertEqual(
			netmanthan.parse_json(netmanthan.parse_json(import_log[0]["messages"])[1])["message"], expected_error
		)

		self.assertEqual(netmanthan.parse_json(import_log[1]["row_indexes"]), [4])
		self.assertEqual(
			netmanthan.parse_json(netmanthan.parse_json(import_log[1]["messages"])[0])["message"],
			"Title is required",
		)

	def test_data_import_update(self):
		existing_doc = netmanthan.get_doc(
		...
		updated_doc = netmanthan.get_doc(doctype_name, existing_doc.name)
...
def create_doctype_if_not_exists(doctype_name, force=False):
	if force:
		...
		netmanthan.delete_doc_if_exists("DocType", "Child 2 of " + doctype_name)
...
def get_import_file(csv_file_name, force=False):
	...
	_file = netmanthan.db.exists("File", {"file_name": file_name})
	if force and _file:
		netmanthan.delete_doc_if_exists("File", _file)

	if netmanthan.db.exists("File", {"file_name": file_name}):
		f = netmanthan.get_doc("File", {"file_name": file_name})
	else:
		full_path = get_csv_file_path(file_name)
		f = netmanthan.get_doc(
			doctype="File", content=netmanthan.read_file(full_path), file_name=file_name, is_private=1
		)
...
def get_csv_file_path(file_name):
	return netmanthan.get_app_path("netmanthan", "core", "doctype", "data_import", "fixtures", file_name)
