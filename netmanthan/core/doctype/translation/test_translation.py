...
from netmanthan.translate import clear_cache


class TestTranslation(netmanthanTestCase):
	def setUp(self):
		netmanthan.db.delete("Translation")
	...
	def test_doctype(self):
		...
		for key, val in translation_data.items():
			netmanthan.local.lang = key

			translation = create_translation(key, val)
			self.assertEqual(_(val[0]), val[1])

			netmanthan.delete_doc("Translation", translation.name)
...
def create_translation(key, val):
	translation = netmanthan.new_doc("Translation")
