...
from netmanthan import STANDARD_USERS, _, msgprint, throw
from netmanthan.core.doctype.user_type.user_type import user_linked_with_permission_on_doctype
...
from netmanthan.utils.deprecations import deprecated
from netmanthan.utils.password import check_password, get_password_reset_limit
from netmanthan.utils.password import update_password as _update_password
from netmanthan.utils.user import get_system_managers
from netmanthan.website.utils import is_signup_disabled
...
class User(Document):
	...
	def after_insert(self):
		...
		netmanthan.cache().delete_key("users_for_mentions")
		netmanthan.cache().delete_key("enabled_users")
	...
	def validate(self):
		...
		if (self.name not in ["Administrator", "Guest"]) and (
			not self.get_social_login_userid("netmanthan")
		):
			self.set_social_login_userid("netmanthan", netmanthan.generate_hash(length=39))

	def populate_role_profile_roles(self):
		if self.role_profile_name:
			role_profile = netmanthan.get_doc("Role Profile", self.role_profile_name)
			self.set("roles", [])
	...
	def validate_allowed_modules(self):
		if self.module_profile:
			module_profile = netmanthan.get_doc("Module Profile", self.module_profile)
	...
	def validate_user_image(self):
		if self.user_image and len(self.user_image) > 2000:
			netmanthan.throw(_("Not a valid User Image."))

	def on_update(self):
		# clear new password
		self.share_with_self()
		clear_notifications(user=self.name)
		netmanthan.clear_cache(user=self.name)
		now = netmanthan.flags.in_test or netmanthan.flags.in_install
		self.send_password_notification(self.__new_password)
		netmanthan.enqueue(
			"netmanthan.core.doctype.user.user.create_contact",
			user=self,
		...
		if self.name not in STANDARD_USERS and not self.user_image:
			netmanthan.enqueue(
				"netmanthan.core.doctype.user.user.update_gravatar",
				name=self.name,
				now=now,
				enqueue_after_commit=True,
			)
		...
		if self.time_zone:
			netmanthan.defaults.set_default("time_zone", self.time_zone, self.name)
	...
	def has_website_permission(self, ptype, user, verbose=False):
		...
		return self.name == netmanthan.session.user
	...
	def check_enable_disable(self):
		...
		if not cint(self.enabled) and self.name in STANDARD_USERS:
			netmanthan.throw(_("User {0} cannot be disabled").format(self.name))

		if not cint(self.enabled):
			self.a_system_manager_should_exist()

		# clear sessions if disabled
		if not cint(self.enabled) and getattr(netmanthan.local, "login_manager", None):
			netmanthan.local.login_manager.logout(user=self.name)
	...
	def add_system_manager_role(self):
		...
		if (
			...
			and cint(netmanthan.db.get_single_value("System Settings", "setup_complete"))
		):
	...
	def is_system_manager_disabled(self):
		return netmanthan.db.get_value("Role", {"name": "System Manager"}, ["disabled"])
	...
	def set_system_user(self):
		...
		if self.user_type and not netmanthan.get_cached_value("User Type", self.user_type, "is_standard"):
	...
	def set_roles_and_modules_based_on_user_type(self):
		user_type_doc = netmanthan.get_cached_doc("User Type", self.user_type)
	...
	def validate_share(self, docshare):
		...
		# 		if docshare.share != 1:
		# 			netmanthan.throw(_("Sorry! User should have complete access to their own record."))
		# 	else:
		# 		netmanthan.throw(_("Sorry! Sharing with Website User is prohibited."))
	...
	def reset_password(self, send_email=False, password_expired=False):
		from netmanthan.utils import get_url, random_string
	...
	def send_welcome_mail_to_user(self):
		...
		subject = None
		method = netmanthan.get_hooks("welcome_email")
		if method:
			subject = netmanthan.get_attr(method[-1])()
		if not subject:
			site_name = netmanthan.db.get_default("site_name") or netmanthan.get_conf().get("site_name")
			if site_name:
	...
	def send_login_mail(self, subject, template, add_args, now=None):
		...
		from netmanthan.utils import get_url
		from netmanthan.utils.user import get_user_fullname

		created_by = get_user_fullname(netmanthan.session["user"])
		if created_by == "Guest":
		...
		sender = (
                netmanthan.session.user not in STANDARD_USERS and get_formatted_email(netmanthan.session.user) or None
		)
	...
	def on_trash(self):
		...
		self.enabled = 0
		if getattr(netmanthan.local, "login_manager", None):
			netmanthan.local.login_manager.logout(user=self.name)

		# delete todos
		netmanthan.db.delete("ToDo", {"allocated_to": self.name})
		todo_table = DocType("ToDo")
		(
			netmanthan.qb.update(todo_table)
			.set(todo_table.assigned_by, None)
			.where(todo_table.assigned_by == self.name)
		).run()

		# delete events
		netmanthan.db.delete("Event", {"owner": self.name, "event_type": "Private"})

		# delete shares
		netmanthan.db.delete("DocShare", {"user": self.name})
		# delete messages
		...
		netmanthan.qb.update(table).where(table.user == self.name).set(table.user, None).run()

		# delete notification settings
		netmanthan.delete_doc("Notification Settings", self.name, ignore_permissions=True)
		...
		netmanthan.db.delete("User Permission", {"user": self.name})

	def before_rename(self, old_name, new_name, merge=False):
		netmanthan.clear_cache(user=old_name)
	...
	def after_rename(self, old_name, new_name, merge=False):
		tables = netmanthan.db.get_tables()
		for tab in tables:
			desc = netmanthan.db.get_table_columns_description(tab)
			has_fields = []
			for d in desc:
		...
		if netmanthan.db.exists("Notification Settings", old_name):
			netmanthan.rename_doc("Notification Settings", old_name, new_name, force=True, show_alert=False)

		# set email
		netmanthan.db.update("User", new_name, "email", new_name)
	...
	def remove_disabled_roles(self):
		disabled_roles = [d.name for d in netmanthan.get_all("Role", filters={"disabled": 1})]
	...
	def validate_username(self):
		if not self.username and self.is_new() and self.first_name:
			self.username = netmanthan.scrub(self.first_name)
		...
		if self.username_exists():
			if self.user_type == "System User":
				netmanthan.msgprint(_("Username {0} already exists").format(self.username))
	...
	def suggest_username(self):
		...
		# @firstname
		username = _check_suggestion(netmanthan.scrub(self.first_name))

		if not username:
			# @firstname_last_name
			username = _check_suggestion(
				netmanthan.scrub("{} {}".format(self.first_name, self.last_name or ""))
			)

		if username:
			netmanthan.msgprint(_("Suggested Username: {0}").format(username))
	...
	def validate_user_email_inbox(self):
		...
		if len(email_accounts) != len(set(email_accounts)):
			netmanthan.throw(_("Email Account added multiple times"))
	...
	def find_by_credentials(cls, user_name: str, password: str, validate_password: bool = True):
		...
		login_with_mobile = cint(
			netmanthan.db.get_single_value("System Settings", "allow_login_using_mobile_number")
		)
		login_with_username = cint(
			netmanthan.db.get_single_value("System Settings", "allow_login_using_user_name")
		)
		...
		users = netmanthan.get_all("User", fields=["name", "enabled"], or_filters=or_filters, limit=1)
		if not users:
			return
		...
		if validate_password:
			...
			except netmanthan.AuthenticationError:
	...
	def check_roles_added(self):
		...
		netmanthan.msgprint(
			_("Newly created user {0} has no roles enabled.").format(netmanthan.bold(self.name)),
			title=_("No Roles Specified"),
			indicator="orange",
			primary_action={
				"label": _("Add Roles"),
				"client_action": "netmanthan.set_route",
...
def get_roles(arg=None):
	...
	return netmanthan.get_roles(netmanthan.form_dict["uid"])


@netmanthan.whitelist()
def get_perm_info(role):
	"""get permission info"""
	from netmanthan.permissions import get_all_perms
...
def update_password(new_password, logout_all_sessions=0, key=None, old_password=None):
	...
	if key and not isinstance(key, str):
		netmanthan.throw(_("Invalid key type"))
	...
	if res.get("message"):
		netmanthan.local.response.http_status_code = 410
		return res["message"]
	else:
		user = res["user"]

	logout_all_sessions = cint(logout_all_sessions) or netmanthan.db.get_single_value(
	...
	redirect_to = netmanthan.cache().hget("redirect_after_login", user)
	if redirect_to:
		redirect_url = redirect_to
		netmanthan.cache().hdel("redirect_after_login", user)

	netmanthan.local.login_manager.login_as(user)

	netmanthan.db.set_value("User", user, "last_password_reset_date", today())
	netmanthan.db.set_value("User", user, "reset_password_key", "")
...
def test_password_strength(new_password, key=None, old_password=None, user_data=None):
	from netmanthan.utils.password_strength import test_password_strength as _test_password_strength
	...
	if not user_data:
		user_data = netmanthan.db.get_value(
			"User", netmanthan.session.user, ["first_name", "middle_name", "last_name", "email", "birth_date"]
		)
...
def has_email_account(email):
	return netmanthan.get_list("Email Account", filters={"email_id": email})


@netmanthan.whitelist(allow_guest=False)
...
def ask_pass_update():
	...
	password_list = netmanthan.get_all(
...
def _get_user_for_update_password(key, old_password):
	# verify old password
	result = netmanthan._dict()
	if key:
		user = netmanthan.db.get_value(
		...
		if result.user:
			reset_password_link_expiry = cint(
				netmanthan.db.get_single_value("System Settings", "reset_password_link_expiry_duration")
			)
			if (
		...
		# verify old password
		netmanthan.local.login_manager.check_password(netmanthan.session.user, old_password)
...
def reset_user_data(user):
	user_doc = netmanthan.get_doc("User", user)
...
def verify_password(password):
	netmanthan.local.login_manager.check_password(netmanthan.session.user, password)


@netmanthan.whitelist(allow_guest=True)
def sign_up(email, full_name, redirect_to):
	if is_signup_disabled():
		netmanthan.throw(_("Sign Up is disabled"), title=_("Not Allowed"))

	user = netmanthan.db.get("User", {"email": email})
	if user:
		if user.enabled:
		...
		if netmanthan.db.get_creation_count("User", 60) > 300:
		...
		default_role = netmanthan.db.get_single_value("Portal Settings", "default_role")
		if default_role:
			user.add_roles(default_role)

		if redirect_to:
			netmanthan.cache().hset("redirect_after_login", user.name, redirect_to)
...
def reset_password(user):
	...
	try:
		...
		netmanthan.local.response["http_status_code"] = 404
...
def user_query(doctype, txt, searchfield, start, page_len, filters):
	from netmanthan.desk.reportview import get_filters_cond, get_match_cond

	doctype = "User"
	conditions = []

	user_type_condition = "and user_type != 'Website User'"
	if filters and filters.get("ignore_user_type") and netmanthan.session.data.user_type == "System User":
		user_type_condition = ""
	...
	""".format(
			...
			standard_users=", ".join(netmanthan.db.escape(u) for u in STANDARD_USERS),
			key=searchfield,
...
def get_system_users(exclude_users=None, limit=None):
	...
	system_users = netmanthan.db.sql_list(
...
def get_website_users():
	...
	return netmanthan.db.count("User", filters={"enabled": True, "user_type": "Website User"})
...
def get_permission_query_conditions(user):
	if user == "Administrator":
		...
		return """(`tabUser`.name not in ({standard_users}))""".format(
			standard_users=", ".join(netmanthan.db.escape(user) for user in STANDARD_USERS)
		)
...
def notify_admin_access_to_system_manager(login_manager=None):
	if (
		...
		and netmanthan.local.conf.notify_admin_access_to_system_manager
	):

		site = '<a href="{0}" target="_blank">{0}</a>'.format(netmanthan.local.request.host_url)
		date_and_time = "<b>{}</b>".format(format_datetime(now_datetime(), format_string="medium"))
		ip_address = netmanthan.local.request_ip
...
def handle_password_test_fail(feedback: dict):
	...
	netmanthan.throw(msg=" ".join([warning] + suggestions), title=_("Invalid Password"))


def update_gravatar(name):
	gravatar = has_gravatar(name)
	if gravatar:
		netmanthan.db.set_value("User", name, "user_image", gravatar)
...
def throttle_user_creation():
	...
	if netmanthan.db.get_creation_count("User", 60) > netmanthan.local.conf.get("throttle_user_limit", 60):
		netmanthan.throw(_("Throttled"))


@netmanthan.whitelist()
def get_role_profile(role_profile):
	roles = netmanthan.get_doc("Role Profile", {"role_profile": role_profile})
	return roles.roles


@netmanthan.whitelist()
def get_module_profile(module_profile):
	module_profile = netmanthan.get_doc("Module Profile", {"module_profile_name": module_profile})
	return module_profile.get("block_modules")


def create_contact(user, ignore_links=False, ignore_mandatory=False):
	from netmanthan.contacts.doctype.contact.contact import get_contact_name
	...
	if not contact_name:
		...
		contact = netmanthan.get_doc("Contact", contact_name)
...
def generate_keys(user):
	...
	user_details = netmanthan.get_doc("User", user)
	api_secret = netmanthan.generate_hash(length=15)
	# if api key is not set generate api key
	if not user_details.api_key:
		api_key = netmanthan.generate_hash(length=15)
...
def switch_theme(theme):
	if theme in ["Dark", "Light", "Automatic"]:
		netmanthan.db.set_value("User", netmanthan.session.user, "desk_theme", theme)


def get_enabled_users():
	def _get_enabled_users():
		enabled_users = netmanthan.get_all("User", filters={"enabled": "1"}, pluck="name")
		return enabled_users

	return netmanthan.cache().get_value("enabled_users", _get_enabled_users)
