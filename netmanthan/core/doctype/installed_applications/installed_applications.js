...
netmanthan.ui.form.on("Installed Applications", {
	...
	show_update_order_dialog() {
		const dialog = new netmanthan.ui.Dialog({
			...
			primary_action: function () {
				...
				netmanthan.call({
					method: "netmanthan.core.doctype.installed_applications.installed_applications.update_installed_apps_order",
					freeze: true,
					args: {
						new_order: new_order,
					},
		...
		netmanthan
			.xcall(
				"netmanthan.core.doctype.installed_applications.installed_applications.get_installed_app_order"
			)
			.then((data) => {
				data.forEach((app) => {
				...
				grid.find(".grid-row-check:first").remove() &&
					grid.find(".grid-row-check").replaceWith(netmanthan.utils.icon("menu"));
				dialog.show();
			});
	},
});
