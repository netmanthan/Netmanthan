...
class LogSettings(Document):
	...
	def _remove_unsupported_doctypes(self):
		for entry in list(self.logs_to_clear):
			...
			msg = _("{} does not support automated log clearing.").format(netmanthan.bold(entry.ref_doctype))
			if netmanthan.conf.developer_mode:
				msg += "<br>" + _("Implement `clear_old_logs` method to enable auto error clearing.")
			netmanthan.msgprint(msg, title=_("DocType not supported by Log Settings."))
			self.remove(entry)
	...
	def add_default_logtypes(self):
		...
		for logtype, retention in DEFAULT_LOGTYPES_RETENTION.items():
			if logtype not in existing_logtypes and _supports_log_clearing(logtype):
				if not netmanthan.db.exists("DocType", logtype):
					continue
	...
	def clear_logs(self):
		...
		for entry in self.logs_to_clear:
			...
			kwargs = netmanthan.get_newargs(func, {"days": entry.days})
			func(**kwargs)
...
def run_log_clean_up():
	doc = netmanthan.get_doc("Log Settings")
...
def has_unseen_error_log():
	if netmanthan.get_all("Error Log", filters={"seen": 0}, limit=1):
		return {
			"show_alert": True,
...
def get_log_doctypes(doctype, txt, searchfield, start, page_len, filters):
	...
	doctypes = netmanthan.get_list("DocType", filters=filters, pluck="name")
...
def clear_log_table(doctype, days=90):
	...
	"""
	from netmanthan.utils import get_table_name
	...
	try:
		netmanthan.db.sql_ddl(f"CREATE TABLE `{temporary}` LIKE `{original}`")
		...
		netmanthan.db.sql_ddl(f"RENAME TABLE `{original}` TO `{backup}`, `{temporary}` TO `{original}`")
	except Exception:
		netmanthan.db.rollback()
		netmanthan.db.sql_ddl(f"DROP TABLE IF EXISTS `{temporary}`")
		raise
	else:
		netmanthan.db.sql_ddl(f"DROP TABLE `{backup}`")
