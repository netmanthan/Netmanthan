...
import netmanthan
from netmanthan.core.doctype.log_settings.log_settings import _supports_log_clearing, run_log_clean_up
from netmanthan.tests.utils import netmanthanTestCase
from netmanthan.utils import add_to_date, now_datetime


class TestLogSettings(netmanthanTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def setUp(self) -> None:
		if self._testMethodName == "test_delete_logs":
			self.datetime = netmanthan._dict()
	...
	def test_delete_logs(self):
		...
		activity_log_count = netmanthan.db.count("Activity Log", {"creation": ("<=", self.datetime.past)})
		error_log_count = netmanthan.db.count("Error Log", {"creation": ("<=", self.datetime.past)})
		email_queue_count = netmanthan.db.count("Email Queue", {"creation": ("<=", self.datetime.past)})
...
def setup_test_logs(past: datetime) -> None:
	activity_log = netmanthan.get_doc(
		{
