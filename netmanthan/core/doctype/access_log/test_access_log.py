...
from netmanthan.utils import cstr, get_site_url


class TestAccessLog(netmanthanTestCase):
	def setUp(self):
		# generate keys for current user to send requests for the following tests
		generate_keys(netmanthan.session.user)
		netmanthan.db.commit()
		generated_secret = netmanthan.utils.password.get_decrypted_password(
			"User", netmanthan.session.user, fieldname="api_secret"
		)
		...
		self.file_name = netmanthan.utils.random_string(10) + ".txt"
		self.test_content = netmanthan.utils.random_string(1024)
	...
	def test_make_full_access_log(self):
		...
		last_doc = netmanthan.get_last_doc("Access Log")
	...
	def test_private_file_download(self):
		...
		new_private_file = netmanthan.get_doc(
			{
				"doctype": self.test_doctype,
		...
		private_file_link = get_site_url(netmanthan.local.site) + new_private_file.file_url
