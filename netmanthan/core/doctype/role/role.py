...
class Role(Document):
	def before_rename(self, old, new, merge=False):
		if old in STANDARD_ROLES:
			netmanthan.throw(netmanthan._("Standard roles cannot be renamed"))

	def after_insert(self):
		netmanthan.cache().hdel("roles", "Administrator")
	...
	def disable_role(self):
		if self.name in STANDARD_ROLES:
			netmanthan.throw(netmanthan._("Standard roles cannot be disabled"))
		else:
			self.remove_roles()
	...
	def remove_roles(self):
		netmanthan.db.delete("Has Role", {"role": self.name})
	...
	def on_update(self):
		...
		if self.has_value_changed("desk_access"):
			for user_name in get_users(self.name):
				user = netmanthan.get_doc("User", user_name)
...
def get_user_info(users, field="email"):
	...
	info_list = []
	for user in users:
		user_info, enabled = netmanthan.db.get_value("User", user.get("user_name"), [field, "enabled"])
