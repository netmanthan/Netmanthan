...
class TestRoleProfile(netmanthanTestCase):
	def test_make_new_role_profile(self):
		netmanthan.delete_doc_if_exists("Role Profile", "Test 1", force=1)
		new_role_profile = netmanthan.get_doc(dict(doctype="Role Profile", role_profile="Test 1")).insert()
		...
		random_user = netmanthan.get_doc(
			{
				"doctype": "User",
				"email": random_user,
				"enabled": 1,
