...
from netmanthan.utils import get_formatted_email, get_url, parse_addr
...
class CommunicationEmailMixin:
	...
	def mail_cc(self, is_inbound_mail_communcation=False, include_sender=False):
		...
		* Remove standard users from email list
		...
		if include_sender:
			...
			if netmanthan.session.user not in netmanthan.STANDARD_USERS:
				sender = netmanthan.db.get_value("User", netmanthan.session.user, "email")
			cc.append(sender)
		...
		self._final_cc = [m for m in cc if m and m not in netmanthan.STANDARD_USERS]
		return self._final_cc
	...
	def mail_bcc(self, is_inbound_mail_communcation=False):
		...
		* remove standard users.
		...
		self._final_bcc = [m for m in bcc if m not in netmanthan.STANDARD_USERS]
		return self._final_bcc
	...
	def get_content(self, print_format=None):
		if print_format and netmanthan.db.get_single_value("System Settings", "attach_view_link"):
			return self.content + self.get_attach_link(print_format)
		return self.content

	def get_attach_link(self, print_format):
		"""Returns public link for the attachment via `templates/emails/print_link.html`."""
		return netmanthan.get_template("templates/emails/print_link.html").render(
			{
				"url": get_url(),
		...
		if not hasattr(self, "_outgoing_email_account"):
			if self.email_account:
				...
				if self.sent_or_received == "Sent" and self._outgoing_email_account:
					if netmanthan.db.exists("Email Account", self._outgoing_email_account.name):
	...
	def filter_disabled_users(emails):
		...
		return netmanthan.get_all("User", pluck="email", filters={"email": ["in", emails], "enabled": 0})
	...
	def send_email(
		...
		if input_dict := self.sendmail_input_dict(
			...
			netmanthan.sendmail(**input_dict)
