...
import netmanthan
from netmanthan.core.doctype.communication.communication import Communication, get_emails
from netmanthan.core.doctype.communication.email import add_attachments
from netmanthan.email.doctype.email_queue.email_queue import EmailQueue
from netmanthan.tests.utils import netmanthanTestCase

if TYPE_CHECKING:
	from netmanthan.contacts.doctype.contact.contact import Contact
	from netmanthan.email.doctype.email_account.email_account import EmailAccount

test_records = netmanthan.get_test_records("Communication")


class TestCommunication(netmanthanTestCase):
	def test_email(self):
		valid_email_list = [
		...
		for i, x in enumerate(valid_email_list):
			with self.subTest(i=i, x=x):
				self.assertTrue(netmanthan.utils.parse_addr(x)[1])

		for i, x in enumerate(invalid_email_list):
			with self.subTest(i=i, x=x):
				self.assertFalse(netmanthan.utils.parse_addr(x)[0])
	...
	def test_name(self):
		...
		for x in valid_email_list:
			self.assertTrue(netmanthan.utils.parse_addr(x)[0])

		for x in invalid_email_list:
			self.assertFalse(netmanthan.utils.parse_addr(x)[0])

	def test_circular_linking(self):
		a = netmanthan.get_doc(
			{
		...
		a = netmanthan.get_doc("Communication", a.name)
		a.reference_doctype = "Communication"
		a.reference_name = c.name

		self.assertRaises(netmanthan.CircularLinkingError, a.save)

	def test_deduplication_timeline_links(self):
		netmanthan.delete_doc_if_exists("Note", "deduplication timeline links")
	...
	def test_contacts_attached(self):
		contact_sender: "Contact" = netmanthan.get_doc(
			{
				"doctype": "Contact",
		...
		contact_recipient: "Contact" = netmanthan.get_doc(
			{
				"doctype": "Contact",
		...
		contact_cc: "Contact" = netmanthan.get_doc(
			{
				"doctype": "Contact",
		...
		comm: Communication = netmanthan.get_doc(
			{
				"doctype": "Communication",
				"communication_medium": "Email",
	...
	def test_get_communication_data(self):
		from netmanthan.desk.form.load import get_communication_data

		netmanthan.delete_doc_if_exists("Note", "get communication data")

		note = netmanthan.get_doc(
			{"doctype": "Note", "title": "get communication data", "content": "get communication data"}
		).insert(ignore_permissions=True)

		comm_note_1 = netmanthan.get_doc(
			{
				"doctype": "Communication",
				"communication_type": "Communication",
		...
		comm_note_2 = netmanthan.get_doc(
			{
				"doctype": "Communication",
				"communication_type": "Communication",
	...
	def test_link_in_email(self):
		...
		notes = {}
		for i in range(2):
			netmanthan.delete_doc_if_exists("Note", f"test document link in email {i}")
			notes[i] = netmanthan.get_doc(
				{
					"doctype": "Note",
	...
	def test_signature_in_email_content(self):
		...
		comm_with_signature = netmanthan.get_doc(
			base_communication
			| {
		...
		comm_without_signature = netmanthan.get_doc(
			base_communication
			| {
...
class TestCommunicationEmailMixin(netmanthanTestCase):
	...
	def test_cc(self):
		def test(assertion, cc_list=None, set_user_as=None, include_sender=False, thread_notify=False):
			if set_user_as:
				netmanthan.set_user(set_user_as)
	...
	def test_add_attachments_by_filename(self):
		...
		attached_file_name, attached_content_hash = netmanthan.db.get_value(
			"File",
	...
	def test_add_attachments_by_file_content(self):
		...
		attached_file_name = netmanthan.db.get_value(
			"File",
			{"attached_to_name": comm.name, "attached_to_doctype": comm.doctype},
		)
		attached_file = netmanthan.get_doc("File", attached_file_name)
...
def create_email_account() -> "EmailAccount":
	netmanthan.delete_doc_if_exists("Email Account", "_Test Comm Account 1")
