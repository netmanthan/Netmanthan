...
def make(
	...
	**kwargs,
	...
	if kwargs:
		...
		warn(
			f"Options {kwargs} used in netmanthan.core.doctype.communication.email.make "
			"are deprecated or unsupported",
			category=DeprecationWarning,
		)

	if doctype and name and not netmanthan.has_permission(doctype=doctype, ptype="email", doc=name):
		raise netmanthan.PermissionError(f"You are not allowed to send emails related to: {doctype} {name}")
...
def _make(
	...
	sender = sender or get_formatted_email(netmanthan.session.user)
	...
	if cint(send_email):
		if not comm.get_outgoing_email_account():
			netmanthan.throw(
				...
				exc=netmanthan.OutgoingEmailError,
			)
...
def add_attachments(name: str, attachments: Iterable[str | dict]) -> None:
	...
	for a in attachments:
		if isinstance(a, str):
			attach = netmanthan.db.get_value("File", {"name": a}, ["file_url", "is_private"], as_dict=1)
			file_args = {
				"file_url": attach.file_url,
				"is_private": attach.is_private,
			}
		elif isinstance(a, dict) and "fcontent" in a and "fname" in a:
			# dict returned by netmanthan.attach_print()
			file_args = {
				"file_name": a["fname"],
				"content": a["fcontent"],
				"is_private": 1,
...
def mark_email_as_seen(name: str = None):
	try:
		...
		netmanthan.db.commit()  # nosemgrep: this will be called in a GET request

	except Exception:
		netmanthan.log_error("Unable to mark as seen", None, "Communication", name)

	finally:
		netmanthan.response.update(
			{
				"type": "binary",
...
def update_communication_as_read(name):
	...
	communication = netmanthan.db.get_value("Communication", name, "read_by_recipient", as_dict=True)
