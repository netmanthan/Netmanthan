...
from netmanthan.automation.doctype.assignment_rule.assignment_rule import (
	apply as apply_assignment_rule,
)
from netmanthan.contacts.doctype.contact.contact import get_contact_name
from netmanthan.core.doctype.comment.comment import update_comment_in_doc
from netmanthan.core.doctype.communication.email import validate_email
from netmanthan.core.doctype.communication.mixins import CommunicationEmailMixin
from netmanthan.core.utils import get_parent_doc
...
class Communication(Document, CommunicationEmailMixin):
	...
	def onload(self):
		...
		if (
			...
			email_flag_queue = netmanthan.db.get_value(
	...
	def validate_reference(self):
		if self.reference_doctype and self.reference_name:
			if not self.reference_owner:
				self.reference_owner = netmanthan.db.get_value(
					self.reference_doctype, self.reference_name, "owner"
				)

			# prevent communication against a child table
			if netmanthan.get_meta(self.reference_doctype).istable:
				netmanthan.throw(
			...
			if self.reference_doctype == "Communication":
				...
				if circular_linking:
					netmanthan.throw(
						...
						netmanthan.CircularLinkingError,
					)
	...
	def after_insert(self):
		...
		if self.reference_doctype == "Communication" and self.sent_or_received == "Sent":
			netmanthan.db.set_value("Communication", self.reference_name, "status", "Replied")

		if self.communication_type == "Communication":
			self.notify_change("add")

		elif self.communication_type in ("Chat", "Notification"):
			if self.reference_name == netmanthan.session.user:
				message = self.as_dict()
				message["broadcast"] = True
				netmanthan.publish_realtime("new_message", message, after_commit=True)
			else:
	...
	def set_signature_in_email_content(self):
		...
		signature = user_email_signature or netmanthan.db.get_value(
			"Email Account",
	...
	def get_attachments(self):
		attachments = netmanthan.get_all(
			"File",
	...
	def set_status(self):
		...
		email_rule = netmanthan.db.get_value("Email Rule", {"email_id": self.sender, "is_spam": 1})
		if (
			self.communication_type == "Communication"
			and self.communication_medium == "Email"
	...
	def find_one_by_filters(cls, *, order_by=None, **kwargs):
		name = netmanthan.db.get_value(cls.DOCTYPE, kwargs, order_by=order_by)
	...
	def set_sender_full_name(self):
		if not self.sender_full_name and self.sender:
			if self.sender == "Administrator":
				self.sender_full_name = netmanthan.db.get_value("User", "Administrator", "full_name")
				self.sender = netmanthan.db.get_value("User", "Administrator", "email")
				...
				if not self.sender_full_name:
					self.sender_full_name = netmanthan.db.get_value("User", self.sender, "full_name")

				if not self.sender_full_name:
					first_name, last_name = netmanthan.db.get_value(
	...
	def set_delivery_status(self, commit=False):
		...
		status_counts = Counter(
			netmanthan.get_all("Email Queue", pluck="status", filters={"communication": self.name})
		)
	...
	def set_timeline_links(self):
		contacts = []
		create_contact_enabled = self.email_account and netmanthan.db.get_value(
...
def on_doctype_update():
	...
	netmanthan.db.add_index("Communication", ["reference_doctype", "reference_name"])
	netmanthan.db.add_index("Communication", ["status", "communication_type"])
	netmanthan.db.add_index("Communication", ["message_id(140)"])
...
def has_permission(doc, ptype, user):
	if ptype == "read":
		...
		if doc.reference_doctype and doc.reference_name:
			if netmanthan.has_permission(doc.reference_doctype, ptype="read", doc=doc.reference_name):
				return True
...
def get_contacts(email_strings: list[str], auto_create_contact=False) -> list[str]:
	...
	for email in email_addrs:
		...
		if not contact_name and email and auto_create_contact:
			...
			first_name = netmanthan.unscrub(email_parts[0])
...
def add_contact_links_to_communication(communication, contact_name):
	contact_links = netmanthan.get_all(
		"Dynamic Link",
...
def parse_email(communication, email_strings):
	...
	"""
	if not netmanthan.db.get_value("Email Account", filters={"enable_automatic_linking": 1}):
		return
	...
	for email_string in email_strings:
		if email_string:
			for email in email_string.split(","):
				...
				if doctype and docname and netmanthan.db.get_value(doctype, docname, ignore=True):
...
def get_email_without_link(email):
	...
	"""
	if not netmanthan.get_all("Email Account", filters={"enable_automatic_linking": 1}):
		return email
...
def set_avg_response_time(parent, communication):
	if parent.meta.has_field("avg_response_time") and communication.sent_or_received == "Sent":
		...
		communications = netmanthan.get_list(
			"Communication",
