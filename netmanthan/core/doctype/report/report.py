...
import netmanthan
import netmanthan.desk.query_report
from netmanthan import _, scrub
from netmanthan.core.doctype.custom_role.custom_role import get_custom_allowed_roles
from netmanthan.core.doctype.page.page import delete_custom_role
from netmanthan.desk.reportview import append_totals_row
from netmanthan.model.document import Document
from netmanthan.modules import make_boilerplate
from netmanthan.modules.export_file import export_to_files
from netmanthan.utils import cint, cstr
from netmanthan.utils.safe_exec import check_safe_sql_query, safe_exec
...
class Report(Document):
	def validate(self):
		...
		if not self.module:
			self.module = netmanthan.db.get_value("DocType", self.ref_doctype, "module")

		if not self.is_standard:
			self.is_standard = "No"
			if (
				netmanthan.session.user == "Administrator" and getattr(netmanthan.local.conf, "developer_mode", 0) == 1
			):
		...
		if self.is_standard == "No":
			...
			if netmanthan.db.get_value("Report", self.name, "is_standard") == "Yes":
				netmanthan.throw(_("Cannot edit a standard report. Please duplicate and create a new report"))

		if self.is_standard == "Yes" and netmanthan.session.user != "Administrator":
			netmanthan.throw(_("Only Administrator can save a standard report. Please rename and save."))
	...
	def on_trash(self):
		if (
			self.is_standard == "Yes"
			and not cint(getattr(netmanthan.local.conf, "developer_mode", 0))
			and not netmanthan.flags.in_patch
		):
			netmanthan.throw(_("You are not allowed to delete Standard Report"))
	...
	def set_doctype_roles(self):
		if not self.get("roles") and self.is_standard == "No":
			meta = netmanthan.get_meta(self.ref_doctype)
			if not meta.istable:
	...
	def is_permitted(self):
		...
		if has_common(netmanthan.get_roles(), allowed):
			return True
	...
	def export_doc(self):
		...
		if self.is_standard == "Yes" and (netmanthan.local.conf.get("developer_mode") or 0) == 1:
			export_to_files(
	...
	def execute_query_report(self, filters):
		if not self.query:
			netmanthan.throw(_("Must specify a Query to run"), title=_("Report Document Error"))

		check_safe_sql_query(self.query)

		result = [list(t) for t in netmanthan.db.sql(self.query, filters)]
		columns = self.get_columns() or [cstr(c[0]) for c in netmanthan.db.get_description()]
	...
	def execute_script_report(self, filters):
		...
		if execution_time > threshold and not self.prepared_report:
			netmanthan.enqueue(enable_prepared_report, report=self.name)

		netmanthan.cache().hset("report_execution_time", self.name, execution_time)

		return res

	def execute_module(self, filters):
		# report in python module
		module = self.module or netmanthan.db.get_value("DocType", self.ref_doctype, "module")
		method_name = get_report_module_dotted_path(module, self.name) + ".execute"
		return netmanthan.get_attr(method_name)(netmanthan._dict(filters))

	def execute_script(self, filters):
		# server script
		loc = {"filters": netmanthan._dict(filters), "data": None, "result": None}
	...
	def run_query_report(
		...
		columns, result = [], []
		data = netmanthan.desk.query_report.run(
			self.name,
			filters=filters,
			user=user,
		...
		for d in data.get("columns"):
			if isinstance(d, dict):
				...
				columns.append(
					netmanthan._dict(label=parts[0], fieldtype=fieldtype, fieldname=parts[0], options=options)
				)
	...
	def get_standard_report_columns(self, params):
		if params.get("fields"):
			...
			for df in netmanthan.get_meta(self.ref_doctype).fields:
				if df.in_list_view:
	...
	def get_standard_report_order_by(self, params):
		...
		if params.get("group_by"):
			group_by_args = netmanthan._dict(params["group_by"])
	...
	def build_standard_report_columns(self, columns, group_by_args):
		...
		for (fieldname, doctype) in columns:
			...
			if meta.get_field(fieldname):
				...
				field = netmanthan._dict(fieldname=fieldname, label=label)
	...
	def toggle_disable(self, disable):
		if not self.has_permission("write"):
			netmanthan.throw(_("You are not allowed to edit the report."))

		self.db_set("disabled", cint(disable))


def is_prepared_report_disabled(report):
	return netmanthan.db.get_value("Report", report, "disable_prepared_report") or 0


def get_report_module_dotted_path(module, report_name):
	return (
            netmanthan.local.module_app[scrub(module)]
            + "."
            + scrub(module)
            + ".report."
            + scrub(report_name)
...
def enable_prepared_report(report: str):
	netmanthan.db.set_value("Report", report, "prepared_report", 1)
