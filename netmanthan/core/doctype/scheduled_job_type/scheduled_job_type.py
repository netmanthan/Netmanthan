...
from netmanthan.utils import get_datetime, now_datetime
from netmanthan.utils.background_jobs import enqueue, is_job_enqueued
...
class ScheduledJobType(Document):
	...
	def enqueue(self, force=False):
		...
		if self.is_event_due() or force:
			if netmanthan.flags.enqueued_jobs:
				netmanthan.flags.enqueued_jobs.append(self.method)

			if netmanthan.flags.execute_job:
				self.execute()
			else:
				if not self.is_job_in_queue():
					enqueue(
						"netmanthan.core.doctype.scheduled_job_type.scheduled_job_type.run_scheduled_job",
					...
					netmanthan.logger("scheduler").error(
						f"Skipped queueing {self.method} because it was found in queue for {netmanthan.local.site}"
					)
		return False
	...
	def get_next_execution(self):
		CRON_MAP = {
			...
			"Hourly": "0 * * * *",
			"Hourly Long": "0 * * * *",
			"All": "0/" + str((netmanthan.get_conf().scheduler_interval or 240) // 60) + " * * * *",
		}
	...
	def execute(self):
		...
		try:
			...
			if self.server_script:
				script_name = netmanthan.db.get_value("Server Script", self.server_script)
				if script_name:
					netmanthan.get_doc("Server Script", script_name).execute_scheduled_method()
			else:
				netmanthan.get_attr(self.method)()
	...
	def log_status(self, status):
		# log file
		netmanthan.logger("scheduler").info(f"Scheduled Job {status}: {self.method} for {netmanthan.local.site}")
	...
	def on_trash(self):
		netmanthan.db.delete("Scheduled Job Log", {"scheduled_job_type": self.name})
...
def execute_event(doc: str):
	...
	doc = json.loads(doc)
	netmanthan.get_doc("Scheduled Job Type", doc.get("name")).enqueue(force=True)
	return doc
...
def run_scheduled_job(job_type: str):
	...
	try:
		netmanthan.get_doc("Scheduled Job Type", dict(method=job_type)).execute()
	except Exception:
		print(netmanthan.get_traceback())


def sync_jobs(hooks: dict = None):
	netmanthan.reload_doc("core", "doctype", "scheduled_job_type")
	scheduler_events = hooks or netmanthan.get_hooks("scheduler_events")
...
def clear_events(all_events: list):
	for event in netmanthan.get_all("Scheduled Job Type", fields=["name", "method", "server_script"]):
		...
		if not (is_defined_in_hooks or is_server_script):
			netmanthan.delete_doc("Scheduled Job Type", event.name)
