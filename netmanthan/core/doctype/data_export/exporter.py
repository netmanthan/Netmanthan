...
from netmanthan.utils import cint, cstr, format_datetime, format_duration, formatdate, parse_json
from netmanthan.utils.csvutils import UnicodeWriter
...
class DataExporter:
	...
	def prepare_args(self):
		...
		if self.all_doctypes:
			self.child_doctypes = []
			for df in netmanthan.get_meta(self.doctype).get_table_fields():
	...
	def build_response(self):
		...
		if self.file_type == "Excel":
			...
			netmanthan.response["result"] = cstr(self.writer.getvalue())
			netmanthan.response["type"] = "csv"
			netmanthan.response["doctype"] = self.doctype
	...
	def build_field_columns(self, dt, parentfield=None):
		...
		tablecolumns = []
		table_name = "tab" + dt
		for f in netmanthan.db.get_table_columns_description(table_name):
		...
		_column_start_end = netmanthan._dict(start=0)
		...
		if dt == self.doctype:
			...
			_column_start_end = netmanthan._dict(start=len(self.columns))
	...
	def getinforow(docfield):
		...
		if docfield.fieldtype == "Select":
			...
			return cstr(netmanthan.defaults.get_defaults().date_format)
	...
	def add_data(self):
		...
		netmanthan.permissions.can_export(self.parent_doctype, raise_exception=True)

		# sort nested set doctypes by `lft asc`
		order_by = None
		table_columns = netmanthan.db.get_table_columns(self.parent_doctype)
		if "lft" in table_columns and "rgt" in table_columns:
			order_by = f"`tab{self.parent_doctype}`.`lft` asc"
		# get permitted data only
		self.data = netmanthan.get_list(
		...
		for doc in self.data:
			...
			if self.all_doctypes:
				...
				for c in self.child_doctypes:
					...
					data_row = (
						netmanthan.qb.from_(child_doctype_table)
						.select("*")
	...
	def build_response_as_excel(self):
		filename = netmanthan.generate_hash(length=10)
		...
		netmanthan.response["filename"] = _(self.doctype) + ".xlsx"
