...
from netmanthan.desk.form.linked_with import get_linked_doctypes
...
class UserPermission(Document):
	...
	def on_update(self):
		netmanthan.cache().hdel("user_permissions", self.user)
		netmanthan.publish_realtime("update_user_permissions", user=self.user, after_commit=True)
	...
	def validate_user_permission(self):
		...
		duplicate_exists = netmanthan.get_all(
			self.doctype,
			filters={
				"allow": self.allow,
		...
		if duplicate_exists:
			netmanthan.throw(_("User permission already exists"), netmanthan.DuplicateEntryError)

	def validate_default_permission(self):
		"""validate user permission overlap for default value of a particular doctype"""
		overlap_exists = []
		if self.is_default:
			overlap_exists = netmanthan.get_all(
				self.doctype,
		...
		if overlap_exists:
			ref_link = netmanthan.get_desk_link(self.doctype, overlap_exists[0].name)
			netmanthan.throw(_("{0} has already assigned default value for {1}.").format(ref_link, self.allow))
...
def get_user_permissions(user=None):
	...
	if netmanthan.request and netmanthan.local.form_dict.cmd == "get_user_permissions":
	...
	cached_user_permissions = netmanthan.cache().hget("user_permissions", user)
	...
	try:
		for perm in netmanthan.get_all(
			...
			meta = netmanthan.get_meta(perm.allow)
			add_doc_to_perm(perm, perm.for_value, perm.is_default)

			if meta.is_nested_set() and not perm.hide_descendants:
				decendants = netmanthan.db.get_descendants(perm.allow, perm.for_value)
				for doc in decendants:
					add_doc_to_perm(perm, doc, False)

		out = netmanthan._dict(out)
		netmanthan.cache().hset("user_permissions", user, out)
	except netmanthan.db.SQLError as e:
...
def clear_user_permissions(user, for_doctype):
	netmanthan.only_for("System Manager")
	total = netmanthan.db.count("User Permission", {"user": user, "allow": for_doctype})
...
def insert_user_perm(
	...
	user_perm = netmanthan.new_doc("User Permission")
	user_perm.user = user
