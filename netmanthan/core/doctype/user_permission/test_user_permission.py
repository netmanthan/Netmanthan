...
from netmanthan.core.doctype.user_permission.user_permission import (
	add_user_permissions,
	remove_applicable,
)
from netmanthan.permissions import has_user_permission
from netmanthan.tests.utils import netmanthanTestCase
from netmanthan.website.doctype.blog_post.test_blog_post import make_test_blog


class TestUserPermission(netmanthanTestCase):
	def setUp(self):
		test_users = (
		...
		netmanthan.db.delete("User Permission", {"user": ("in", test_users)})
		netmanthan.delete_doc_if_exists("DocType", "Person")
		netmanthan.db.sql_ddl("DROP TABLE IF EXISTS `tabPerson`")
		netmanthan.delete_doc_if_exists("DocType", "Doc A")
		netmanthan.db.sql_ddl("DROP TABLE IF EXISTS `tabDoc A`")
	...
	def test_default_user_permission_validation(self):
		...
		self.assertRaises(netmanthan.ValidationError, add_user_permissions, param)
	...
	def test_default_user_permission_corectness(self):
		...
		add_user_permissions(param)
		netmanthan.db.delete("User Permission", filters={"for_value": test_blog.name})
		netmanthan.delete_doc("Blog Post", test_blog.name)
	...
	def test_default_user_permission(self):
		...
		for category in ["general", "public"]:
			if not netmanthan.db.exists("Blog Category", category):
				netmanthan.get_doc({"doctype": "Blog Category", "title": category}).insert()
		...
		netmanthan.set_user("test_user_perm1@example.com")
	...
	def test_for_applicable_on_update_from_apply_to_all(self):
		...
		removed_apply_to_all = netmanthan.db.exists("User Permission", get_exists_param(user))
		is_created_applicable_first = netmanthan.db.exists(
			"User Permission", get_exists_param(user, applicable="Comment")
		)
		is_created_applicable_second = netmanthan.db.exists(
	...
	def test_for_apply_to_all_on_update_from_applicable(self):
		...
		is_created = add_user_permissions(param)
		is_created_apply_to_all = netmanthan.db.exists("User Permission", get_exists_param(user))
		removed_applicable_first = netmanthan.db.exists(
			"User Permission", get_exists_param(user, applicable="Comment")
		)
		removed_applicable_second = netmanthan.db.exists(
			"User Permission", get_exists_param(user, applicable="Contact")
		)
	...
	def test_user_perm_for_nested_doctype(self):
		...
		if not netmanthan.db.exists("DocType", "Person"):
			doc = new_doctype(
				"Person",
		...
		parent_record = netmanthan.get_doc(
			{"doctype": "Person", "person_name": "Parent", "is_group": 1}
		).insert()

		child_record = netmanthan.get_doc(
			{
				"doctype": "Person",
				"person_name": "Child",
				"is_group": 0,
		...
		self.assertTrue(has_user_permission(netmanthan.get_doc("Person", parent_record.name), user.name))
		self.assertTrue(has_user_permission(netmanthan.get_doc("Person", child_record.name), user.name))

		netmanthan.db.set_value(
			"User Permission", {"allow": "Person", "for_value": parent_record.name}, "hide_descendants", 1
		)
		netmanthan.cache().delete_value("user_permissions")

		# check if adding perm on a group record with hide_descendants enabled,
		# hides child records
		self.assertTrue(has_user_permission(netmanthan.get_doc("Person", parent_record.name), user.name))
		self.assertFalse(has_user_permission(netmanthan.get_doc("Person", child_record.name), user.name))

	def test_user_perm_on_new_doc_with_field_default(self):
		"""Test User Perm impact on netmanthan.new_doc. with *field* default value"""
		netmanthan.set_user("Administrator")
		user = create_user("new_doc_test@example.com", "Blogger")

		# make a doctype "Doc A" with 'doctype' link field and default value ToDo
		if not netmanthan.db.exists("DocType", "Doc A"):
			doc = new_doctype(
				"Doc A",
				fields=[
					{
		...
		netmanthan.set_user("new_doc_test@example.com")

		new_doc = netmanthan.new_doc("Doc A")
	...
	def test_user_perm_on_new_doc_with_user_default(self):
		"""Test User Perm impact on netmanthan.new_doc. with *user* default value"""
		from netmanthan.core.doctype.session_default_settings.session_default_settings import (
		...
		if not netmanthan.db.exists("Session Default", {"ref_doctype": "DocType"}):
...
def create_user(email, *roles):
	...
	if netmanthan.db.exists("User", email):
		return netmanthan.get_doc("User", email)
