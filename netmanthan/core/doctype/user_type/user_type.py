...
from netmanthan.config import get_modules_from_app
from netmanthan.model.document import Document
from netmanthan.permissions import add_permission, add_user_permission
from netmanthan.utils import get_link_to_form
...
class UserType(Document):
	...
	def clear_cache(self):
		...
		if not self.is_standard:
			netmanthan.cache().delete_value("non_standard_user_types")
	...
	def on_trash(self):
		if self.is_standard:
			netmanthan.throw(_("Standard user type {0} can not be deleted.").format(netmanthan.bold(self.name)))

	def set_modules(self):
		if not self.user_doctypes:
			return

		modules = netmanthan.get_all(
			"DocType",
	...
	def validate_document_type_limit(self):
		limit = netmanthan.conf.get("user_type_doctype_limit", {}).get(netmanthan.scrub(self.name))

		if not limit and netmanthan.session.user != "Administrator":
			netmanthan.throw(
				_("User does not have permission to create the new {0}").format(netmanthan.bold(_("User Type"))),
	...
	def validate_role(self):
		if not self.role:
			netmanthan.throw(_("The field {0} is mandatory").format(netmanthan.bold(_("Role"))))

		if not netmanthan.db.get_value("Role", self.role, "is_custom"):
			netmanthan.throw(
				_("The role {0} should be a custom role.").format(
					netmanthan.bold(get_link_to_form("Role", self.role))
				)
			)

	def update_users(self):
		for row in netmanthan.get_all("User", filters={"user_type": self.name}):
			user = netmanthan.get_cached_doc("User", row.name)
	...
	def update_modules_in_user(self, user):
		block_modules = netmanthan.get_all(
			"Module Def",
	...
	def add_role_permissions_for_user_doctypes(self):
		...
		for row in self.user_doctypes:
			...
			netmanthan.db.set_value("Custom DocPerm", docperm, values)

	def add_select_perm_doctypes(self):
		if netmanthan.flags.ignore_select_perm:
			return
		...
		for doctype in user_doctypes:
			doc = netmanthan.get_meta(doctype)
	...
	def add_role_permissions_for_file(self):
		...
		netmanthan.db.set_value("Custom DocPerm", docperm, {"read": 1, "create": 1, "write": 1})
	...
	def remove_permission_for_deleted_doctypes(self):
		...
		for perm in netmanthan.get_all(
			"Custom DocPerm", filters={"role": self.role, "parent": ["not in", doctypes]}
		):
			netmanthan.delete_doc("Custom DocPerm", perm.name)


def add_role_permissions(doctype, role):
	name = netmanthan.get_value("Custom DocPerm", dict(parent=doctype, role=role, permlevel=0))
...
def get_user_linked_doctypes(doctype, txt, searchfield, start, page_len, filters):
	modules = [d.get("module_name") for d in get_modules_from_app("netmanthan")]
	...
	custom_doctypes = netmanthan.get_all(
...
def user_linked_with_permission_on_doctype(doc, user):
	...
	if not doc.user_id_field:
		netmanthan.throw(_("User Id Field is mandatory in the user type {0}").format(netmanthan.bold(doc.name)))

	if netmanthan.db.get_value(doc.apply_user_permission_on, {doc.user_id_field: user}, "name"):
		return True
	else:
		label = netmanthan.get_meta(doc.apply_user_permission_on).get_field(doc.user_id_field).label

		netmanthan.msgprint(
			_(
				"To set the role {0} in the user {1}, kindly set the {2} field as {3} in one of the {4} record."
			).format(
				netmanthan.bold(doc.role),
				netmanthan.bold(user),
				netmanthan.bold(label),
				netmanthan.bold(user),
				netmanthan.bold(doc.apply_user_permission_on),
			)
		)
...
def apply_permissions_for_non_standard_user_type(doc, method=None):
	...
	if not netmanthan.db.table_exists("User Type") or netmanthan.flags.in_migrate:
		return

	user_types = netmanthan.cache().get_value(
	...
	for user_type, data in user_types.items():
		...
		if doc.get(data[1]) and (
			...
			if not perm_data:
				...
				netmanthan.db.set_value("User Permission", perm_data[0], "user", doc.get(data[1]))
