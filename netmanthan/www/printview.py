...
import netmanthan
from netmanthan import _, get_module_path
from netmanthan.core.doctype.access_log.access_log import make_access_log
from netmanthan.core.doctype.document_share_key.document_share_key import is_expired
from netmanthan.utils import cint, escape_html, strip_html
...
def get_context(context):
	...
	if not ((netmanthan.form_dict.doctype and netmanthan.form_dict.name) or netmanthan.form_dict.doc):
		return {
			"body": f"""
				<h1>Error</h1>
				<p>Parameters doctype and name required</p>
				<pre>{escape_html(netmanthan.as_json(netmanthan.form_dict, indent=2))}</pre>
				"""
		}

	if netmanthan.form_dict.doc:
		doc = netmanthan.form_dict.doc
	else:
		doc = netmanthan.get_doc(netmanthan.form_dict.doctype, netmanthan.form_dict.name)

	set_link_titles(doc)

	settings = netmanthan.parse_json(netmanthan.form_dict.settings)

	letterhead = netmanthan.form_dict.letterhead or None
	...
	make_access_log(
		doctype=netmanthan.form_dict.doctype, document=netmanthan.form_dict.name, file_type="PDF", method="Print"
	)
	...
	body = get_rendered_template(
		...
		meta=meta,
		trigger_print=netmanthan.form_dict.trigger_print,
		no_letterhead=netmanthan.form_dict.no_letterhead,
		letterhead=letterhead,
		settings=settings,
	)
	print_style = get_print_style(netmanthan.form_dict.style, print_format)

	return {
		"body": body,
		"print_style": print_style,
		"comment": netmanthan.session.user,
		"title": netmanthan.utils.strip_html(doc.get_title() or doc.name),
		"lang": netmanthan.local.lang,
		"layout_direction": "rtl" if is_rtl() else "ltr",
		"doctype": netmanthan.form_dict.doctype,
		"name": netmanthan.form_dict.name,
		"key": netmanthan.form_dict.get("key"),
	}
...
def get_print_format_doc(print_format_name, meta):
	...
	if not print_format_name:
		print_format_name = netmanthan.form_dict.format or meta.default_print_format or "Standard"

	if print_format_name == "Standard":
		return None
	else:
		try:
			return netmanthan.get_doc("Print Format", print_format_name)
...
def get_rendered_template(
	...
	print_settings = netmanthan.get_single("Print Settings").as_dict()
	...
	if not netmanthan.flags.ignore_print_permissions:
		validate_print_permission(doc)

	if doc.meta.is_submittable:
		if doc.docstatus.is_draft() and not cint(print_settings.allow_print_for_draft):
			netmanthan.throw(_("Not allowed to print draft documents"), netmanthan.PermissionError)

		if doc.docstatus.is_cancelled() and not cint(print_settings.allow_print_for_cancelled):
			netmanthan.throw(_("Not allowed to print cancelled documents"), netmanthan.PermissionError)
	...
	letter_head = netmanthan._dict(get_letter_head(doc, no_letterhead, letterhead) or {})

	if letter_head.content:
		letter_head.content = netmanthan.utils.jinja.render_template(
			letter_head.content, {"doc": doc.as_dict()}
		)

	if letter_head.footer:
		letter_head.footer = netmanthan.utils.jinja.render_template(
	...
	try:
		...
		netmanthan.throw(
			...
			exc=netmanthan.PrintFormatError,
...
def set_title_values_for_link_and_dynamic_link_fields(meta, doc, parent_doc=None):
	...
	for field in meta.get_link_fields() + meta.get_dynamic_link_fields():
		...
		link_title = netmanthan.get_cached_value(doctype, doc.get(field.fieldname), meta.title_field)
		if parent_doc:
...
def set_title_values_for_table_and_multiselect_fields(meta, doc):
	for field in meta.get_table_fields():
		...
		_meta = netmanthan.get_meta(field.options)
	...
	for field in meta.fields:
		if field.fieldtype == "Text Editor":
			...
			if value and "<!-- markdown -->" in value:
				doc.set(field.fieldname, netmanthan.utils.md_to_html(value))
...
def get_html_and_style(
	...
	if isinstance(doc, str) and isinstance(name, str):
		doc = netmanthan.get_doc(doc, name)

	if isinstance(doc, str):
		doc = netmanthan.get_doc(json.loads(doc))

	print_format = get_print_format_doc(print_format, meta=meta or netmanthan.get_meta(doc.doctype))
	set_link_titles(doc)
	...
	try:
		html = get_rendered_template(
			...
			settings=netmanthan.parse_json(settings),
		)
	except netmanthan.TemplateNotFoundError:
...
def get_rendered_raw_commands(doc, name=None, print_format=None, meta=None, lang=None):
	...
	if not print_format or (print_format and not print_format.raw_printing):
		netmanthan.throw(
			_("{0} is not a raw printing format.").format(print_format), netmanthan.TemplateNotFoundError
		)
...
def validate_print_permission(doc):
	for ptype in ("read", "print"):
		if netmanthan.has_permission(doc.doctype, ptype, doc) or netmanthan.has_website_permission(doc):
			return

	key = netmanthan.form_dict.key
	if key and isinstance(key, str):
		validate_key(key, doc)
	else:
		raise netmanthan.PermissionError(_("You do not have permission to view this document"))


def validate_key(key, doc):
	document_key_expiry = netmanthan.get_cached_value(
		"Document Share Key",
	...
	if document_key_expiry is not None:
		if is_expired(document_key_expiry[0]):
			raise netmanthan.exceptions.LinkExpired
		else:
			return

	# TODO: Deprecate this! kept it for backward compatibility
	if netmanthan.get_system_settings("allow_older_web_view_links") and key == doc.get_signature():
		return

	raise netmanthan.exceptions.InvalidKeyError
...
def get_letter_head(doc, no_letterhead, letterhead=None):
	if no_letterhead:
		return {}
	if letterhead:
		return netmanthan.db.get_value("Letter Head", letterhead, ["content", "footer"], as_dict=True)
	if doc.get("letter_head"):
		return netmanthan.db.get_value("Letter Head", doc.letter_head, ["content", "footer"], as_dict=True)
	else:
		return (
                netmanthan.db.get_value("Letter Head", {"is_default": 1}, ["content", "footer"], as_dict=True) or {}
		)
...
def get_print_format(doctype, print_format):
	if print_format.disabled:
		netmanthan.throw(
			_("Print Format {0} is disabled").format(print_format.name), netmanthan.DoesNotExistError
		)

	# server, find template
	module = print_format.module or netmanthan.db.get_value("DocType", doctype, "module")
	path = os.path.join(
		get_module_path(module, "Print Format", print_format.name),
        netmanthan.scrub(print_format.name) + ".html",
	)
	...
	if os.path.exists(path):
		...
		netmanthan.throw(_("No template found at path: {0}").format(path), netmanthan.TemplateNotFoundError)
...
def make_layout(doc, meta, format_data=None):
	...
	for df in format_data or meta.fields:
		...
		if df.fieldtype == "Signature" and not doc.get(df.fieldname):
			placeholder_image = "/assets/netmanthan/images/signature-placeholder.png"
...
def get_print_style(style=None, print_format=None, for_legacy=False):
	...
	css = netmanthan.get_template("templates/styles/standard.css").render(context)

	if style and netmanthan.db.exists("Print Style", style):
		css = css + "\n" + netmanthan.db.get_value("Print Style", style, "css")
...
def get_visible_columns(data, table_meta, df):
	...
	columns = []
	doc = data[0] or netmanthan.new_doc(df.options)
