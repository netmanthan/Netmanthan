...
"""
netmanthan - Low Code Open Source Framework in Python and JS

netmanthan, pronounced fra-pay, is a full stack, batteries-included, web
...
Read the documentation: https://netmanthanframework.com/docs
"""
import functools
import gc
import importlib
...
from netmanthan.query_builder import (
	get_query,
	get_query_builder,
	patch_query_aggregation,
	patch_query_execute,
)
from netmanthan.utils.caching import request_cache
from netmanthan.utils.data import cint, cstr, sbool
...
__version__ = "14.50.0"
__title__ = "netmanthan Framework"
...
_tune_gc = bool(sbool(os.environ.get("netmanthan_TUNE_GC", True)))
...
def _(msg: str, lang: str | None = None, context: str | None = None) -> str:
	...
	"""
	from netmanthan.translate import get_all_translations
...
def get_lang_dict(fortype: str, name: str | None = None) -> dict[str, str]:
	...
	from netmanthan.translate import get_dict

	return get_dict(fortype, name)


def set_user_lang(user: str, user_language: str | None = None) -> None:
	"""Guess and set user language for the session. `netmanthan.local.lang`"""
	from netmanthan.translate import get_user_lang
...
if TYPE_CHECKING:
	...
	from netmanthan.database.postgres.database import PostgresDatabase
	from netmanthan.model.document import Document
	from netmanthan.query_builder.builder import MariaDB, Postgres
	from netmanthan.utils.redis_wrapper import RedisWrapper
...
def init(site: str, sites_path: str = ".", new_site: bool = False, force=False) -> None:
	"""Initialize netmanthan for the current site. Reset thread locals `netmanthan.local`"""
...
def connect(
	...
	:param site: If site is given, calls `netmanthan.init`.
	:param db_name: Optional. Will use from `site_config.json`.
	:param set_admin_as_user: Set Administrator as current user.
	"""
	from netmanthan.database import get_db
...
def get_traceback(with_context: bool = False) -> str:
	...
	from netmanthan.utils import get_traceback
...
def throw(
	...
	:param msg: Message.
	:param exc: Exception class. Default `netmanthan.ValidationError`"""
	msgprint(
		msg,
		raise_exception=exc,
		title=title,
...
def create_folder(path, with_init=False):
	...
	from netmanthan.utils import touch_file
...
def get_roles(username=None) -> list[str]:
	...
	return netmanthan.permissions.get_roles(username or local.session.user)
...
def sendmail(
	...
	if as_markdown:
		from netmanthan.utils import md_to_html
	...
	from netmanthan.email.doctype.email_queue.email_queue import QueueBuilder
...
def is_whitelisted(method):
	from netmanthan.utils import sanitize_html
...
def read_only():
	def innfn(fn):
		def wrapper_fn(*args, **kwargs):
			...
			# netmanthan.read_only could be called from nested functions, in such cases don't swap the
			# connection again.
...
def only_for(roles: list[str] | tuple[str] | str, message=False):
	"""
	Raises `netmanthan.PermissionError` if the user does not have any of the permitted roles.
...
def clear_cache(user: str | None = None, doctype: str | None = None):
	...
	import netmanthan.utils.caching

	if doctype:
		netmanthan.cache_manager.clear_doctype_cache(doctype)
		reset_metadata_version()
	elif user:
		netmanthan.cache_manager.clear_user_cache(user)
	else:  # everything
		from netmanthan import translate

		netmanthan.cache_manager.clear_user_cache()
		netmanthan.cache_manager.clear_domain_cache()
	...
	netmanthan.utils.caching._SITE_CACHE.clear()
	local.role_permissions = {}
...
def only_has_select_perm(doctype, user=None, ignore_permissions=False):
	...
	permissions = netmanthan.permissions.get_role_permissions(doctype, user=user)
...
def has_permission(
	...
	*,
	...
	Raises `netmanthan.PermissionError` if user isn't permitted and `throw` is truthy
	...
	out = netmanthan.permissions.has_permission(
		doctype,
		ptype,
		doc=doc,
		user=user,
...
def has_website_permission(doc=None, ptype="read", user=None, verbose=False, doctype=None):
	"""Raises `netmanthan.PermissionError` if not permitted.
...
def get_precision(
	...
	from netmanthan.model.meta import get_field_precision
...
def new_doc(
	...
	from netmanthan.model.create_new import get_new_doc

	return get_new_doc(doctype, parent_doc, parentfield, as_dict=as_dict)


def set_value(doctype, docname, fieldname, value=None):
	"""Set document value. Calls `netmanthan.client.set_value`"""
	import netmanthan.client

	return netmanthan.client.set_value(doctype, docname, fieldname, value)
...
def get_doc(*args, **kwargs) -> "Document":
	"""Return a `netmanthan.model.document.Document` object of the given type and name.
	...
	Examples:
	        ...
	        todo = netmanthan.get_doc({"doctype":"ToDo", "description": "test"})
	        todo.insert()

	        # open an existing document
	        todo = netmanthan.get_doc("ToDo", "TD0001")

	"""
	import netmanthan.model.document

	doc = netmanthan.model.document.get_doc(*args, **kwargs)
...
def get_single(doctype):
	"""Return a `netmanthan.model.document.Document` object of the given Single doctype."""
	return get_doc(doctype, doctype)


def get_meta(doctype, cached=True):
	"""Get `netmanthan.model.meta.Meta` instance of given doctype name."""
	import netmanthan.model.meta

	return netmanthan.model.meta.get_meta(doctype, cached=cached)


def get_meta_module(doctype):
	import netmanthan.modules

	return netmanthan.modules.load_doctype_module(doctype)
...
def delete_doc(
	...
	"""Delete a document. Calls `netmanthan.model.delete_doc.delete_doc`.
	...
	import netmanthan.model.delete_doc

	return netmanthan.model.delete_doc.delete_doc(
		doctype,
		name,
		force,
		ignore_doctypes,
...
def reload_doc(
	...
	return netmanthan.modules.reload_doc(module, dt, dn, force=force, reset_permissions=reset_permissions)
...
def rename_doc(
	...
	*,
	...
	Calls `netmanthan.model.rename_doc.rename_doc`
	"""
...
def get_module_path(module, *joins):
	...
	from netmanthan.modules.utils import get_module_app
...
def get_all_apps(with_internal_apps=True, sites_path=None):
	...
	if "netmanthan" in apps:
		apps.remove("netmanthan")
	apps.insert(0, "netmanthan")

	return apps


@request_cache
def get_installed_apps(sort=False, netmanthan_last=False, *, _ensure_on_bench=False):
	"""
	Get list of installed apps in current site.

	:param sort: [DEPRECATED] Sort installed apps based on the sequence in sites/apps.txt
	:param netmanthan_last: [DEPRECATED] Keep netmanthan last. Do not use this, reverse the app list instead.
	...
	if netmanthan_last:
		deprecation_warning("`netmanthan_last` argument is deprecated and will be removed in v15.")
		if "netmanthan" in installed:
			installed.remove("netmanthan")
		installed.append("netmanthan")
...
def get_file_items(path, raise_not_found=False, ignore_empty_lines=True):
	...
	if content:
		content = netmanthan.utils.strip(content)
...
def compare(val1, condition, val2):
	"""Compare two values using `netmanthan.utils.compare`
	...
	return netmanthan.utils.compare(val1, condition, val2)
...
def redirect(url):
	...
	from netmanthan.exceptions import Redirect
...
def redirect_to_message(title, html, http_status_code=None, context=None, indicator_color=None):
	...
	Example Usage:
	        netmanthan.redirect_to_message(_('Thank you'), "<div><p>You will receive an email at test@example.com</p></div>")
...
def build_match_conditions(doctype, as_condition=True):
	...
	import netmanthan.desk.reportview

	return netmanthan.desk.reportview.build_match_conditions(doctype, as_condition=as_condition)


def get_list(doctype, *args, **kwargs):
	"""List database query via `netmanthan.model.db_query`. Will also check for permissions.
	...
	Example usage:
	        ...
	        # simple dict filter
	        netmanthan.get_list("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        netmanthan.get_list("ToDo", fields="*", filters = [["modified", ">", "2014-01-01"]])

	        # filter as a list of dicts
	        netmanthan.get_list("ToDo", fields="*", filters = {"description": ("like", "test%")})
	"""
	import netmanthan.model.db_query

	return netmanthan.model.db_query.DatabaseQuery(doctype).execute(*args, **kwargs)


def get_all(doctype, *args, **kwargs):
	"""List database query via `netmanthan.model.db_query`. Will **not** check for permissions.
	Parameters are same as `netmanthan.get_list`
	...
	Example usage:
	        ...
	        # simple dict filter
	        netmanthan.get_all("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        netmanthan.get_all("ToDo", fields=["*"], filters = [["modified", ">", "2014-01-01"]])

	        # filter as a list of dicts
	        netmanthan.get_all("ToDo", fields=["*"], filters = {"description": ("like", "test%")})
	"""
...
def get_value(*args, **kwargs):
	...
	Alias for `netmanthan.db.get_value`
...
def get_test_records(doctype):
	...
	from netmanthan.modules import get_doctype_module, get_module_path
...
def format_value(*args, **kwargs):
	...
	import netmanthan.utils.formatters

	return netmanthan.utils.formatters.format_value(*args, **kwargs)
...
def get_print(
	...
	from netmanthan.utils.pdf import get_pdf
...
def attach_print(
	...
	from netmanthan.translate import print_language
	from netmanthan.utils import scrub_urls
	from netmanthan.utils.pdf import get_pdf
...
def publish_progress(*args, **kwargs):
	...
	"""
	import netmanthan.realtime

	return netmanthan.realtime.publish_progress(*args, **kwargs)
...
def publish_realtime(*args, **kwargs):
	...
	return netmanthan.realtime.publish_realtime(*args, **kwargs)


def local_cache(namespace, key, generator, regenerate_if_none=False):
	"""A key value store for caching within a request

	:param namespace: netmanthan.local.cache[namespace]
	:param key: netmanthan.local.cache[namespace][key] used to retrieve value
...
def enqueue(*args, **kwargs):
	...
	"""
	import netmanthan.utils.background_jobs

	return netmanthan.utils.background_jobs.enqueue(*args, **kwargs)
...
def enqueue_doc(*args, **kwargs):
	...
	return netmanthan.utils.background_jobs.enqueue_doc(*args, **kwargs)
...
def logger(
	...
	from netmanthan.utils.logger import get_logger
...
def safe_eval(code, eval_globals=None, eval_locals=None):
	...
	from netmanthan.utils.safe_exec import safe_eval
...
def get_active_domains():
	from netmanthan.core.doctype.domain_settings.domain_settings import get_active_domains
...
def get_version(doctype, name, limit=None, head=False, raise_err=True):
	...
	Example
	>>> netmanthan.get_version('User', 'foobar@gmail.com')
	>>>
	[
	        {
	...
	if meta.track_changes:
		...
		from netmanthan.utils import dictify, safe_json_loads, squashify
...
def parse_json(val):
	from netmanthan.utils import parse_json
...
def mock(type, size=1, locale="en"):
	...
	from netmanthan.utils import squashify
...
def validate_and_sanitize_search_inputs(fn):
	...
	def wrapper(*args, **kwargs):
		from netmanthan.desk.search import sanitize_searchfield
...
if _tune_gc:
	...
	# This number is quite small for netmanthan where a single query can potentially create 700+
	# objects easily.
