...
from netmanthan.query_builder import Table
from netmanthan.utils import cstr, encode

Auth = Table("__Auth")


class LegacyPassword(pbkdf2_sha256):
	name = "netmanthan_legacy"
	ident = "$netmanthanl$"
...
def set_encrypted_password(doctype, name, pwd, fieldname="password"):
	...
	except netmanthan.db.DataError as e:
		if netmanthan.db.is_data_too_long(e):
			netmanthan.throw(_("Most probably your password is too long."), exc=e)
		raise e


def remove_encrypted_password(doctype, name, fieldname="password"):
	netmanthan.db.delete("__Auth", {"doctype": doctype, "name": name, "fieldname": fieldname})


def check_password(user, pwd, doctype="User", fieldname="password", delete_tracker_cache=True):
	"""Checks if user and password are correct, else raises netmanthan.AuthenticationError"""
	...
	if not result or not passlibctx.verify(pwd, result[0].password):
		raise netmanthan.AuthenticationError(_("Incorrect User or Password"))
...
def delete_login_failed_cache(user):
	netmanthan.cache().hdel("last_login_tried", user)
	netmanthan.cache().hdel("login_failed_count", user)
	netmanthan.cache().hdel("locked_account_time", user)
...
def delete_all_passwords_for(doctype, name):
	try:
		netmanthan.db.delete("__Auth", {"doctype": doctype, "name": name})
	except Exception as e:
		if not netmanthan.db.is_missing_column(e):
			raise


def rename_password(doctype, old_name, new_name):
	# NOTE: fieldname is not considered, since the document is renamed
	netmanthan.qb.update(Auth).set(Auth.name, new_name).where(
		(Auth.doctype == doctype) & (Auth.name == old_name)
	).run()


def rename_password_field(doctype, old_fieldname, new_fieldname):
	netmanthan.qb.update(Auth).set(Auth.fieldname, new_fieldname).where(
...
def encrypt(txt, encryption_key=None):
	...
	try:
		...
		netmanthan.throw(_("Encryption key is in invalid format!"))
...
def get_encryption_key():
	if "encryption_key" not in netmanthan.local.conf:
		from netmanthan.installer import update_site_config

		encryption_key = Fernet.generate_key().decode()
		update_site_config("encryption_key", encryption_key)
		netmanthan.local.conf.encryption_key = encryption_key

	return netmanthan.local.conf.encryption_key


def get_password_reset_limit():
	return netmanthan.db.get_single_value("System Settings", "password_reset_limit") or 0
