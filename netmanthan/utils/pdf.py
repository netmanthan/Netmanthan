...
from netmanthan.utils.jinja_globals import bundled_asset, is_rtl
...
def get_pdf(html, options=None, output: PdfWriter | None = None):
	...
	try:
		...
		if any([error in str(e) for error in PDF_CONTENT_ERRORS]):
			if not filedata:
				print(html, options)
				netmanthan.throw(_("PDF generation failed because of broken image links"))
...
def prepare_options(html, options):
	...
	# page size
	pdf_page_size = (
            options.get("page-size") or netmanthan.db.get_single_value("Print Settings", "pdf_page_size") or "A4"
	)

	if pdf_page_size == "Custom":
		options["page-height"] = options.get("page-height") or netmanthan.db.get_single_value(
			"Print Settings", "pdf_page_height"
		)
		options["page-width"] = options.get("page-width") or netmanthan.db.get_single_value(
...
def get_cookie_options():
	options = {}
	if netmanthan.session and netmanthan.session.sid and hasattr(netmanthan.local, "request"):
		# Use wkhtmltopdf's cookie-jar feature to set cookies and restrict them to host domain
		cookiejar = f"/tmp/{netmanthan.generate_hash()}.jar"

		# Remove port from request.host
		# https://werkzeug.palletsprojects.com/en/0.16.x/wrappers/#werkzeug.wrappers.BaseRequest.host
		domain = netmanthan.utils.get_host_name().split(":", 1)[0]
		with open(cookiejar, "w") as f:
			f.write(f"sid={netmanthan.session.sid}; Domain={domain};\n")
...
def prepare_header_footer(soup):
	...
	css = netmanthan.read_file(os.path.join(netmanthan.local.sites_path, print_css))
	...
	for html_id in ("header-html", "footer-html"):
		...
		if content:
			...
			html = netmanthan.render_template(
			...
			# create temp file
			fname = os.path.join("/tmp", f"netmanthan-pdf-{netmanthan.generate_hash()}.html")
			with open(fname, "wb") as f:
				f.write(html.encode("utf-8"))

			# {"header-html": "/tmp/netmanthan-pdf-random.html"}
...
def get_wkhtmltopdf_version():
	wkhtmltopdf_version = netmanthan.cache().hget("wkhtmltopdf_version", None)

	if not wkhtmltopdf_version:
		try:
			res = subprocess.check_output(["wkhtmltopdf", "--version"])
			wkhtmltopdf_version = res.decode("utf-8").split(" ")[1]
			netmanthan.cache().hset("wkhtmltopdf_version", None, wkhtmltopdf_version)
		except Exception:
			pass
