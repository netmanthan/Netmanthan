...
def make_boilerplate(dest, app_name, no_git=False):
	...
	app_name = netmanthan.scrub(app_name)
...
def _create_app_boilerplate(dest, hooks, no_git=False):
	netmanthan.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, netmanthan.scrub(hooks.app_title)),
		with_init=True,
	)
	netmanthan.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, "templates"), with_init=True
	)
	netmanthan.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "www"))
	netmanthan.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "pages"), with_init=True
	)
	netmanthan.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "includes"))
	netmanthan.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "config"), with_init=True)
	netmanthan.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "css"))
	netmanthan.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "js"))
	...
	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "__init__.py"), "w") as f:
		f.write(netmanthan.as_unicode(init_template))

	with open(os.path.join(dest, hooks.app_name, "MANIFEST.in"), "w") as f:
		f.write(netmanthan.as_unicode(manifest_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, "requirements.txt"), "w") as f:
		f.write("# netmanthan -- https://github.com/netmanthan/netmanthan is installed via 'bench init'")
	...
	with open(os.path.join(dest, hooks.app_name, "license.txt"), "w") as f:
		f.write(netmanthan.as_unicode("License: " + hooks.app_license))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "modules.txt"), "w") as f:
		f.write(netmanthan.as_unicode(hooks.app_title))
	...
	with open(os.path.join(dest, hooks.app_name, "setup.py"), "w") as f:
		f.write(netmanthan.as_unicode(setup_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "hooks.py"), "w") as f:
		f.write(netmanthan.as_unicode(hooks_template.format(**hooks)))
	...
	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "config", "desktop.py"), "w") as f:
		f.write(netmanthan.as_unicode(desktop_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "config", "docs.py"), "w") as f:
		f.write(netmanthan.as_unicode(docs_template.format(**hooks)))
	...
	if not no_git:
		with open(os.path.join(dest, hooks.app_name, ".gitignore"), "w") as f:
			f.write(netmanthan.as_unicode(gitignore_template.format(app_name=hooks.app_name)))
...
class PatchCreator:
	def __init__(self):
		self.all_apps = netmanthan.get_all_apps(sites_path=".", with_internal_apps=False)
	...
	def _ask_app_name(self):
		...
		self.app_dir = pathlib.Path(netmanthan.get_app_path(self.app))
	...
	def _ask_patch_meta_info(self):
		...
		default_filename = netmanthan.scrub(self.docstring) + ".py"
...
# Desk Notifications
# ------------------
# See netmanthan.core.notifications.get_notification_config
...
#	"netmanthan.desk.doctype.event.event.get_events": "{app_name}.event.get_events"
# }}
#
# each overriding function accepts a `data` argument;
# generated from the base implementation of the doctype dashboard,
# along with any modifications made in other netmanthan apps
...
desktop_template = """from netmanthan import _
...
jobs:
  tests:
    ...
    steps:
      ...
      - name: Setup
        run: |
          pip install netmanthan-bench
          bench init --skip-redis-config-generation --skip-assets --python "$(which python)" ~/netmanthan-bench
          mysql --host 127.0.0.1 --port 3306 -u root -proot -e "SET GLOBAL character_set_server = 'utf8mb4'"
          mysql --host 127.0.0.1 --port 3306 -u root -proot -e "SET GLOBAL collation_server = 'utf8mb4_unicode_ci'"

      - name: Install
        working-directory: /home/runner/netmanthan-bench
        run: |
