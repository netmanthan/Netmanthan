...
from netmanthan.utils import cint, cstr, get_bench_id
from netmanthan.utils.commands import log
from netmanthan.utils.redis_queue import RedisQueue
...
def get_queues_timeout():
	common_site_config = netmanthan.get_conf()
...
def enqueue(
	...
	**kwargs,
	...
	:param now: if now=True, the method is executed via netmanthan.call
	...
	if not is_async and not netmanthan.flags.in_test:
		print(
			_(
	...
	call_directly = now or (not is_async and not netmanthan.flags.in_test)
	if call_directly:
		return netmanthan.call(method, **kwargs)

	try:
		q = get_queue(queue, is_async=is_async)
	except ConnectionError:
		if netmanthan.local.flags.in_migrate:
	...
	if enqueue_after_commit:
		if not netmanthan.flags.enqueue_after_commit:
			netmanthan.flags.enqueue_after_commit = []

		netmanthan.flags.enqueue_after_commit.append(
			{
				"queue": queue,
				"is_async": is_async,
				"timeout": timeout,
...
def enqueue_doc(
	...
	return enqueue(
		"netmanthan.utils.background_jobs.run_doc_method",
		doctype=doctype,
		name=name,
		doc_method=method,
		queue=queue,
...
def run_doc_method(doctype, name, doc_method, **kwargs):
	getattr(netmanthan.get_doc(doctype, name), doc_method)(**kwargs)
...
def execute_job(site, method, event, job_name, kwargs, user=None, is_async=True, retry=0):
	...
	for before_job_task in netmanthan.get_hooks("before_job"):
		netmanthan.call(before_job_task, method=method_name, kwargs=kwargs, transaction_type="job")

	try:
		retval = method(**kwargs)

	except (netmanthan.db.InternalError, netmanthan.RetryBackgroundJobError) as e:
		netmanthan.db.rollback()

		if retry < 5 and (
			isinstance(e, netmanthan.RetryBackgroundJobError)
			or (netmanthan.db.is_deadlocked(e) or netmanthan.db.is_timedout(e))
		):
			# retry the job if
			# 1213 = deadlock
			...
			netmanthan.log_error(title=method_name)
			raise
		...
		for after_job_task in netmanthan.get_hooks("after_job"):
			netmanthan.call(after_job_task, method=method_name, kwargs=kwargs, result=retval)
...
def validate_queue(queue, default_queue_list=None):
	...
	if queue not in default_queue_list:
		netmanthan.throw(_("Queue should be one of {0}").format(", ".join(default_queue_list)))
...
def get_redis_conn(username=None, password=None):
	if not hasattr(netmanthan.local, "conf"):
		raise Exception("You need to call netmanthan.init")

	elif not netmanthan.local.conf.redis_queue:
		raise Exception("redis_queue missing in common_site_config.json")

	global _redis_queue_conn

	cred = netmanthan._dict()
	if netmanthan.conf.get("use_rq_auth"):
		if username:
			cred["username"] = username
			cred["password"] = password
		else:
			cred["username"] = netmanthan.get_site_config().rq_username or get_bench_id()
			cred["password"] = netmanthan.get_site_config().rq_password
	...
	try:
		...
		log(f"Please make sure that Redis Queue runs @ {netmanthan.get_conf().redis_queue}", colour="red")
		raise
...
def enqueue_test_job():
	enqueue("netmanthan.utils.background_jobs.test_job", s=100)
...
def create_job_id(job_id: str) -> str:
	...
	return f"{netmanthan.local.site}::{job_id}"
...
def truncate_failed_registry(job, connection, type, value, traceback):
	...
	from netmanthan.utils import create_batch

	conf = netmanthan.get_conf(site=job.kwargs.get("site"))
