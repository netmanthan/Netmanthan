...
import netmanthan
from netmanthan.model.meta import get_field_currency, get_field_precision
...
def format_value(value, df=None, doc=None, currency=None, translated=False, format=None):
	...
	if isinstance(df, str):
		df = netmanthan._dict(fieldtype=df)
	...
	if not df:
		...
		default_currency = netmanthan.db.get_default("currency")
		...
		if not BLOCK_TAGS_PATTERN.search(value):
			return netmanthan.safe_decode(value).replace("\n", "<br>")

	elif df.get("fieldtype") == "Markdown Editor":
		return netmanthan.utils.markdown(value)

	elif df.get("fieldtype") == "Table MultiSelect":
		values = []
		meta = netmanthan.get_meta(df.options)
		link_field = [df for df in meta.fields if df.fieldtype == "Link"][0]
		for v in value:
			v.update({"__link_titles": doc.get("__link_titles")})
			formatted_value = netmanthan.format_value(v.get(link_field.fieldname, ""), link_field, v)
