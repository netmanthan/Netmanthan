...
class RedisWrapper(redis.Redis):
	...
	def make_key(self, key, user=None, shared=False):
		...
		return f"{netmanthan.conf.db_name}|{key}".encode()
	...
	def set_value(self, key, val, user=None, expires_in_sec=None, shared=False, cache_locally=True):
		...
		if not expires_in_sec and cache_locally:
			netmanthan.local.cache[key] = val
	...
	def get_value(self, key, generator=None, user=None, expires=False, shared=False):
		...
		:param expires: If the key is supposed to be with an expiry, don't store it in netmanthan.local
		"""
		original_key = key
		key = self.make_key(key, user, shared)

		if key in netmanthan.local.cache:
			val = netmanthan.local.cache[key]
	...
	def get_keys(self, key):
		...
		try:
			...
			return [k for k in list(netmanthan.local.cache) if regex.match(cstr(k))]
	...
	def delete_value(self, keys, user=None, make_keys=True, shared=False):
		...
		for key in keys:
			...
			if key in netmanthan.local.cache:
				del netmanthan.local.cache[key]
	...
	def hset(self, name: str, key: str, value, shared: bool = False, cache_locally: bool = True):
		...
		# set in local
		if cache_locally:
			netmanthan.local.cache.setdefault(_name, {})[key] = value
	...
	def hget(self, name, key, generator=None, shared=False):
		_name = self.make_key(name, shared=shared)
		if _name not in netmanthan.local.cache:
			netmanthan.local.cache[_name] = {}

		if not key:
			return None

		if key in netmanthan.local.cache[_name]:
			return netmanthan.local.cache[_name][key]
		...
		if value is not None:
			...
			netmanthan.local.cache[_name][key] = value
		elif generator:
			value = generator()
	...
	def hdel(self, name, key, shared=False):
		...
		if _name in netmanthan.local.cache:
			if key in netmanthan.local.cache[_name]:
				del netmanthan.local.cache[_name][key]
		try:
	...
	def hdel_keys(self, name_starts_with, key):
		...
		for name in netmanthan.cache().get_keys(name_starts_with):
			name = name.split("|", 1)[1]
			self.hdel(name, key)
