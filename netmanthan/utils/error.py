...
EXCLUDE_EXCEPTIONS = (
    ...
    netmanthan.CSRFTokenError,  # CSRF covers OAuth too
    netmanthan.SecurityException,
    netmanthan.InReadOnlyMode,
)
...
def make_error_snapshot(exception):
	if netmanthan.conf.disable_error_snapshot:
		return

	if isinstance(exception, EXCLUDE_EXCEPTIONS) or _is_ldap_exception(exception):
		return

	logger = netmanthan.logger(with_more_info=True)

	try:
		error_id = "{timestamp:s}-{ip:s}-{hash:s}".format(
			timestamp=cstr(datetime.datetime.now()),
			ip=netmanthan.local.request_ip or "127.0.0.1",
			hash=netmanthan.generate_hash(length=3),
		)
		snapshot_folder = get_error_snapshot_path()
		netmanthan.create_folder(snapshot_folder)
		...
		with open(encode(snapshot_file_path), "wb") as error_file:
			error_file.write(encode(netmanthan.as_json(snapshot)))
	...
	try:
		...
		for fname in os.listdir(path):
			...
			for field in ["locals", "exception", "frames"]:
				data[field] = netmanthan.as_json(data[field])

			doc = netmanthan.new_doc("Error Snapshot")
			doc.update(data)
			doc.save()
...
def clear_old_snapshots():
	...
	netmanthan.db.delete(ErrorSnapshot, filters=(ErrorSnapshot.creation < (Now() - Interval(months=1))))
...
def get_error_snapshot_path():
	return netmanthan.get_site_path("error-snapshots")
	...
	def decorator_raise_error_on_no_output(func):
		...
		def wrapper_raise_error_on_no_output(*args, **kwargs):
			...
			if (not response) and raise_error:
				netmanthan.throw(error_message, error_type or Exception)
			return response
