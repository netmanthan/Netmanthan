...
def main():
	...
	commands.update({"get-netmanthan-commands": get_netmanthan_commands, "get-netmanthan-help": get_netmanthan_help})
	click.Group(commands=commands)(prog_name="bench")


def get_app_groups():
	"""Get all app groups, put them in main group "netmanthan" since bench is
	...
	ret = dict(netmanthan=click.group(name="netmanthan", commands=commands)(app_group))
	return ret
...
def app_group(ctx, site=False, force=False, verbose=False, profile=False):
	...
	if ctx.info_name == "netmanthan":
		ctx.info_name = ""


def get_sites(site_arg):
	if site_arg == "all":
		return netmanthan.utils.get_sites()
	elif site_arg:
		return [site_arg]
	elif os.environ.get("netmanthan_SITE"):
		return [os.environ.get("netmanthan_SITE")]
...
@click.command("get-netmanthan-commands")
def get_netmanthan_commands():
	commands = list(get_app_commands("netmanthan"))
...
@click.command("get-netmanthan-help")
def get_netmanthan_help():
	print(click.Context(get_app_groups()["netmanthan"]).get_help())


def get_apps():
	return netmanthan.get_all_apps(with_internal_apps=False, sites_path=".")


if __name__ == "__main__":
	if not netmanthan._dev_server:
