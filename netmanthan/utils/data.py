...
def get_first_day_of_the_week() -> str:
	return netmanthan.get_system_settings("first_day_of_the_week") or "Sunday"
...
def getdate(
	...
	try:
		...
		netmanthan.throw(
			netmanthan._("{} is not a valid date string.").format(netmanthan.bold(string_date)),
			title=netmanthan._("Invalid Date"),
		)
...
def add_to_date(
	...
	if isinstance(date, str):
		...
		try:
			...
			netmanthan.throw(netmanthan._("Please select a valid date filter"), title=netmanthan._("Invalid Date"))
...
def _get_system_timezone():
	return netmanthan.db.get_system_setting("time_zone") or "Asia/Kolkata"  # Default to India ?!


def get_system_timezone():
	if netmanthan.local.flags.in_test:
		return _get_system_timezone()

	return netmanthan.cache().get_value("time_zone", _get_system_timezone)
...
def now() -> str:
	...
	if netmanthan.flags.current_date:
		return (
			getdate(netmanthan.flags.current_date).strftime(DATE_FORMAT)
			+ " "
...
def get_user_date_format() -> str:
	...
	if getattr(netmanthan.local, "user_date_format", None) is None:
		netmanthan.local.user_date_format = netmanthan.db.get_default("date_format")

	return netmanthan.local.user_date_format or "yyyy-mm-dd"
...
def get_user_time_format() -> str:
	...
	if getattr(netmanthan.local, "user_time_format", None) is None:
		netmanthan.local.user_time_format = netmanthan.db.get_default("time_format")

	return netmanthan.local.user_time_format or "HH:mm:ss"
	...
	* dd/mm/yyyy
	...
	try:
		formatted_date = babel.dates.format_date(
			date, format_string, locale=(netmanthan.local.lang or "").replace("-", "_")
		)
	...
	* HH:mm
	...
	try:
		formatted_time = babel.dates.format_time(
			time_, format_string, locale=(netmanthan.local.lang or "").replace("-", "_")
		)
	except UnknownLocaleError:
	...
	* mm-dd-yyyy HH:mm
	...
	try:
		formatted_datetime = babel.dates.format_datetime(
			datetime, format_string, locale=(netmanthan.local.lang or "").replace("-", "_")
		)
	except UnknownLocaleError:
...
def validate_duration_format(duration):
	if not DURATION_PATTERN.match(duration):
		netmanthan.throw(
			netmanthan._("Value {0} must be in the valid duration format: d h m s").format(
				netmanthan.bold(duration)
			)
		)
...
def global_date_format(date, format="long"):
	...
	date = getdate(date)
	formatted_date = babel.dates.format_date(
		date, locale=(netmanthan.local.lang or "en").replace("-", "_"), format=format
	)
	return formatted_date
...
def cast_fieldtype(fieldtype, value, show_warning=True):
	if show_warning:
		message = (
			"Function `netmanthan.utils.data.cast_fieldtype` has been deprecated in favour"
			" of `netmanthan.utils.data.cast`. Use the newer util for safer type casting."
		)
...
def cast(fieldtype, value=None):
	"""Cast the value to the Python native object of the netmanthan fieldtype provided.
	...
	Mapping of Python types => netmanthan types:
...
def flt(
	...
	try:
		...
		if isinstance(e, netmanthan.InvalidRoundingMethod):
			raise
		num = 0.0
...
def cstr(s, encoding="utf-8"):
	return netmanthan.as_unicode(s, encoding)
...
def rounded(num, precision=0, rounding_method=None):
	...
	rounding_method = (
            rounding_method or netmanthan.get_system_settings("rounding_method") or "Banker's Rounding (legacy)"
	)
	...
	if rounding_method == "Banker's Rounding (legacy)":
		...
		netmanthan.throw(
			netmanthan._("Unknown Rounding Method: {}").format(rounding_method),
			exc=netmanthan.InvalidRoundingMethod,
		)
...
def round_based_on_smallest_currency_fraction(value, currency, precision=2):
	smallest_currency_fraction_value = flt(
		netmanthan.db.get_value("Currency", currency, "smallest_currency_fraction_value", cache=True)
	)
...
def fmt_money(
	...
	"""
	number_format = format or netmanthan.db.get_default("number_format") or "#,###.##"
	if precision is None:
		precision = cint(netmanthan.db.get_default("currency_precision")) or None
	...
	if decimal_str:
		...
		if precision > 2:
			if len(decimals) < 3:
				if currency:
					fraction = netmanthan.db.get_value("Currency", currency, "fraction_units", cache=True) or 100
	...
	if currency and netmanthan.defaults.get_global_default("hide_currency_symbol") != "Yes":
		symbol = netmanthan.db.get_value("Currency", currency, "symbol", cache=True) or currency
		symbol_on_right = netmanthan.db.get_value("Currency", currency, "symbol_on_right", cache=True)

		if symbol_on_right:
			amount = f"{amount} {netmanthan._(symbol)}"
		else:
			amount = f"{netmanthan._(symbol)} {amount}"
...
def money_in_words(
	...
	"""
	from netmanthan.utils import get_defaults
	...
	if not fraction_currency:
		fraction_currency = netmanthan.db.get_value("Currency", main_currency, "fraction", cache=True) or _(
			"Cent"
		)

	number_format = (
            netmanthan.db.get_value("Currency", main_currency, "number_format", cache=True)
            or netmanthan.db.get_default("number_format")
            or "#,###.##"
	)
...
def in_words(integer: int, in_million=True) -> str:
	...
	locale = "en_IN" if not in_million else netmanthan.local.lang
	integer = int(integer)
	try:
...
def get_thumbnail_base64_for_image(src):
	...
	from netmanthan import cache, safe_decode
	from netmanthan.core.doctype.file.utils import get_local_image

	if not src:
		netmanthan.throw(f"Invalid source for image: {src}")
	...
	def _get_base64():
		file_path = netmanthan.get_site_path("public", src.lstrip("/"))
...
def pdf_to_base64(filename: str) -> bytes | None:
	from netmanthan.utils.file_manager import get_file_path
...
def comma_or(some_list, add_quotes=True):
	return comma_sep(some_list, netmanthan._("{0} or {1}"), add_quotes)


def comma_and(some_list, add_quotes=True):
	return comma_sep(some_list, netmanthan._("{0} and {1}"), add_quotes)
...
def comma_sep(some_list, pattern, add_quotes=True):
	if isinstance(some_list, (list, tuple)):
		...
		if not some_list:
			...
			return pattern.format(", ".join(netmanthan._(s) for s in some_list[:-1]), some_list[-1])
	else:
		return some_list
...
def get_url(uri: str | None = None, full_address: bool = False) -> str:
	...
	host_name = netmanthan.local.conf.host_name or netmanthan.local.conf.hostname
	...
	if not host_name:
		...
		elif netmanthan.local.site:
			protocol = "http://"

			if netmanthan.local.conf.ssl_certificate:
				protocol = "https://"

			elif netmanthan.local.conf.wildcard:
				domain = netmanthan.local.conf.wildcard.get("domain")
				if (
					domain
					and netmanthan.local.site.endswith(domain)
					and netmanthan.local.conf.wildcard.get("ssl_certificate")
				):
					protocol = "https://"

			host_name = protocol + netmanthan.local.site

		else:
			host_name = netmanthan.db.get_single_value("Website Settings", "subdomain")
	...
	if not uri and full_address:
		uri = netmanthan.get_request_header("REQUEST_URI", "")

	port = netmanthan.conf.http_port or netmanthan.conf.webserver_port

	if (
		not (netmanthan.conf.restart_supervisor_on_update or netmanthan.conf.restart_systemd_on_update)
		and host_name
...
def get_host_name_from_request() -> str:
	if hasattr(netmanthan.local, "request") and netmanthan.local.request and netmanthan.local.request.host:
		protocol = (
			"https://" if "https" == netmanthan.get_request_header("X-Forwarded-Proto", "") else "http://"
		)
		return protocol + netmanthan.local.request.host
...
def get_filter(doctype: str, f: dict | list | tuple, filters_config=None) -> "netmanthan._dict":
	...
	"""
	from netmanthan.model import child_table_fields, default_fields, optional_fields
	...
	if not isinstance(f, (list, tuple)):
		netmanthan.throw(netmanthan._("Filter must be a tuple or list (in a list)"))
	...
	if len(f) == 3:
		...
		netmanthan.throw(
			netmanthan._("Filter must have 4 values (doctype, fieldname, operator, value): {0}").format(str(f))
		)

	f = netmanthan._dict(doctype=f[0], fieldname=f[1], operator=f[2], value=f[3])
	...
	if f.operator.lower() not in valid_operators:
		netmanthan.throw(netmanthan._("Operator must be one of {0}").format(", ".join(valid_operators)))
	...
	if f.doctype and (f.fieldname not in default_fields + optional_fields + child_table_fields):
		...
		meta = netmanthan.get_meta(f.doctype)
		if not meta.has_field(f.fieldname):

			# try and match the doctype name from child tables
			for df in meta.get_table_fields():
				if netmanthan.get_meta(df.options).has_field(f.fieldname):
					f.doctype = df.options
					break

	try:
		df = netmanthan.get_meta(f.doctype).get_field(f.fieldname)
	except netmanthan.exceptions.DoesNotExistError:
		df = None
...
def sanitize_column(column_name: str) -> None:
	...
	if netmanthan.db and netmanthan.db.db_type == "mariadb":
	...
	def _raise_exception():
		netmanthan.throw(_("Invalid field name {0}").format(column_name), netmanthan.DataError)
...
def to_markdown(html: str) -> str:
	...
	from netmanthan.core.utils import html2text
...
def generate_hash(*args, **kwargs) -> str:
	return netmanthan.generate_hash(*args, **kwargs)
...
def validate_json_string(string: str) -> None:
	try:
		...
		raise netmanthan.ValidationError
...
def get_user_info_for_avatar(user_id: str) -> _UserInfo:
	try:
		user = netmanthan.get_cached_doc("User", user_id)
...
def validate_python_code(
	...
	try:
		...
		msg = netmanthan._("{} Invalid python code on line {}").format(
			fieldname + ":" if fieldname else "", line_no + 1
		)
		msg += f"<br><pre>{error_line}</pre>"
		msg += f"<pre>{' ' * offset}^</pre>"

		netmanthan.throw(msg, title=netmanthan._("Syntax Error"))
	except Exception as e:
		netmanthan.msgprint(
			netmanthan._("{} Possibly invalid python code. <br>{}").format(fieldname + ": " or "", str(e)),
			indicator="orange",
		)
...
def format_timedelta(o: datetime.timedelta) -> str:
	...
	# but netmanthan doesnt - i think via babel : only allows 0..23 range for hour
