...
class SignupDisabledError(netmanthan.PermissionError):
	...


def get_oauth2_providers() -> dict[str, dict]:
	out = {}
	providers = netmanthan.get_all("Social Login Key", fields=["*"])
...
def get_oauth_keys(provider: str) -> dict[str, str]:
	...
	if keys := netmanthan.conf.get(f"{provider}_login"):
		return {"client_id": keys["client_id"], "client_secret": keys["client_secret"]}

	return {
		"client_id": netmanthan.db.get_value("Social Login Key", provider, "client_id"),
...
def get_oauth2_authorize_url(provider: str, redirect_to: str) -> str:
	...
	state = {
		"site": netmanthan.utils.get_url(),
		"token": netmanthan.generate_hash(),
...
def get_redirect_uri(provider: str) -> str:
	keys = netmanthan.conf.get(f"{provider}_login")
	...
	return netmanthan.utils.get_url(redirect_uri)
...
def get_info_via_oauth(
	...
	if not (info.get("email_verified") or info.get("email")):
		netmanthan.throw(_("Email not verified with {0}").format(provider.title()))
...
def login_oauth_user(
	...
	if not (state and state["token"]):
		netmanthan.respond_as_web_page(_("Invalid Request"), _("Token is missing"), http_status_code=417)
		return
	...
	try:
		...
		return netmanthan.respond_as_web_page(
			"Signup is Disabled",
	...
	netmanthan.local.login_manager.user = user
	netmanthan.local.login_manager.post_login()

	# because of a GET request!
	netmanthan.db.commit()

	if netmanthan.utils.cint(generate_login_token):
		login_token = netmanthan.generate_hash(length=32)
		netmanthan.cache().set_value(
			f"login_token:{login_token}", netmanthan.local.session.sid, expires_in_sec=120
		)

		netmanthan.response["login_token"] = login_token

	else:
		redirect_to = state.get("redirect_to")
		redirect_post_login(
			desk_user=netmanthan.local.response.get("message") == "Logged In",
			redirect_to=redirect_to,
			provider=provider,
		)
...
def get_user_record(user: str, data: dict) -> "User":
	try:
		return netmanthan.get_doc("User", user)
	except netmanthan.DoesNotExistError:
		if netmanthan.get_website_settings("disable_signup"):
			raise SignupDisabledError

	user: "User" = netmanthan.new_doc("User")
	...
	user.update(
		{
			...
			"gender": gender,
			"enabled": 1,
			"new_password": netmanthan.generate_hash(),
...
def update_oauth_user(user: str, data: dict, provider: str):
	...
	if not user.enabled:
		netmanthan.respond_as_web_page(_("Not Allowed"), _("User {0} is disabled").format(user.email))
		return False
	...
	if update_user_record:
		...
		if default_role := netmanthan.db.get_single_value("Portal Settings", "default_role"):
...
def redirect_post_login(
	...
	if not redirect_to:
		...
		redirect_to = netmanthan.utils.get_url(desk_uri if desk_user else "/me")

	netmanthan.local.response["location"] = redirect_to
