...
from netmanthan.boot import get_allowed_reports
from netmanthan.core.doctype.domain_settings.domain_settings import get_active_modules
from netmanthan.permissions import get_roles, get_valid_perms
from netmanthan.query_builder import DocType, Order
...
class UserPermissions:
	"""
	A user permission object can be accessed as `netmanthan.get_user()`
	"""

	def __init__(self, name=""):
		self.defaults = None
		self.name = name or netmanthan.session.get("user")
		self.roles = []
	...
	def setup_user(self):
		def get_user_doc():
			user = None
			try:
				user = netmanthan.get_doc("User", self.name).as_dict()
			except netmanthan.DoesNotExistError:
				pass
			except Exception as e:
				# install boo-boo
				if not netmanthan.db.is_table_missing(e):
					raise

			return user

		if not netmanthan.flags.in_install_db and not netmanthan.flags.in_test:
			user_doc = netmanthan.cache().hget("user_doc", self.name, get_user_doc)
			if user_doc:
				self.doc = netmanthan.get_doc(user_doc)
	...
	def build_doctype_map(self):
		...
		all_doctypes = netmanthan.get_all(
			"DocType",
			fields=[
				"name",
				"in_create",
	...
	def build_perm_map(self):
		...
		for r in get_valid_perms():
			...
			for k in netmanthan.permissions.rights:
	...
	def build_permissions(self):
		...
		self.build_perm_map()
		user_shared = netmanthan.share.get_shared_doctypes()
		no_list_view_link = []
		...
		self.shared = netmanthan.get_all(
		...
		if "System Manager" in self.get_roles():
			self.can_import += netmanthan.get_all("DocType", {"allow_import": 1}, pluck="name")
			self.can_import += netmanthan.get_all(
				"Property Setter",
				pluck="doc_type",
				filters={"property": "allow_import", "value": "1"},
			)

		netmanthan.cache().hset("can_import", netmanthan.session.user, self.can_import)

	def get_defaults(self):
		import netmanthan.defaults

		self.defaults = netmanthan.defaults.get_defaults(self.name)
	...
	def load_user(self):
		...
		d.name = self.name
		d.onboarding_status = netmanthan.parse_json(d.onboarding_status)
...
def get_fullname_and_avatar(user: str) -> _dict:
	first_name, last_name, avatar, name = netmanthan.db.get_value(
...
def get_system_managers(only_name: bool = False) -> list[str]:
	...
	system_managers = (
		...
		.where(
			...
			& (User.docstatus < 2)
			& (User.name.notin(netmanthan.STANDARD_USERS))
		)
		.select(*fields)
...
def add_role(user: str, role: str) -> None:
	netmanthan.get_doc("User", user).add_roles(role)
...
def is_website_user(username: str | None = None) -> str | None:
	return netmanthan.db.get_value("User", username or netmanthan.session.user, "user_type") == "Website User"
...
def is_system_user(username: str | None = None) -> str | None:
	return netmanthan.db.get_value(
		"User",
		{
			"email": username or netmanthan.session.user,
			"enabled": 1,
			"user_type": "System User",
		},
	)
