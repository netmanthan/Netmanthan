...
from netmanthan.utils import cint, strip_html_tags
...
def reset():
	...
	:return:
	"""
	netmanthan.db.delete("__global_search")
...
def get_doctypes_with_global_search(with_child_tables=True):
	...
	def _get():
		...
		for d in netmanthan.get_all("DocType", fields=["name", "module"], filters=filters):
			meta = netmanthan.get_meta(d.name)
			if len(meta.get_global_search_fields()) > 0:
				global_search_doctypes.append(d)

		installed_apps = netmanthan.get_installed_apps()
		module_app = netmanthan.local.module_app

		doctypes = [
			d.name
			for d in global_search_doctypes
			if module_app.get(netmanthan.scrub(d.module))
               and module_app[netmanthan.scrub(d.module)] in installed_apps
		]

		return doctypes

	return netmanthan.cache().get_value("doctypes_with_global_search", _get)
...
def rebuild_for_doctype(doctype):
	...
	"""
	if netmanthan.local.conf.get("disable_global_search"):
		return

	def _get_filters():
		filters = netmanthan._dict({"docstatus": ["!=", 2]})
	...
	if cint(meta.istable) == 1:
		parent_doctypes = netmanthan.get_all(
			"DocField",
			fields="parent",
			filters={"fieldtype": ["in", netmanthan.model.table_fields], "options": doctype},
		)
	...
	all_records = netmanthan.get_all(doctype, fields=fieldnames, filters=_get_filters())
		...
		if content:
			...
			try:
				if hasattr(get_controller(doctype), "is_website_published") and meta.allow_guest_to_view:
					d = netmanthan.get_doc(doctype, doc.name)
			...
			all_contents.append(
				{
					"doctype": netmanthan.db.escape(doctype),
					"name": netmanthan.db.escape(doc.name),
					"content": netmanthan.db.escape(" ||| ".join(content or "")),
					"published": published,
					"title": netmanthan.db.escape((title or "")[: int(netmanthan.db.VARCHAR_LEN)]),
					"route": netmanthan.db.escape((route or "")[: int(netmanthan.db.VARCHAR_LEN)]),
				}
			)
	if all_contents:
...
def delete_global_search_records_for_doctype(doctype):
	netmanthan.db.delete("__global_search", {"doctype": doctype})
...
def get_children_data(doctype, meta):
	...
	"""
	all_children = netmanthan._dict()
	child_search_fields = netmanthan._dict()

	for child in meta.get_table_fields():
		child_meta = netmanthan.get_meta(child.options)
		...
		if search_fields:
			...
			child_records = netmanthan.get_all(
				child.options, fields=child_fieldnames, filters={"docstatus": ["!=", 1], "parenttype": doctype}
			)

			for record in child_records:
				all_children.setdefault(record.parent, netmanthan._dict()).setdefault(child.options, []).append(
					record
				)
...
def update_global_search(doc):
	...
	for field in doc.meta.get_global_search_fields():
		if doc.get(field.fieldname) and field.fieldtype not in netmanthan.model.table_fields:
	...
	if content:
		...
		title = (cstr(doc.get_title()) or "")[: int(netmanthan.db.VARCHAR_LEN)]
...
def update_global_search_for_all_web_pages():
	if netmanthan.conf.get("disable_global_search"):
		return
...
def get_routes_to_index():
	...
	routes_to_index = []
	for app in apps:
		base = netmanthan.get_app_path(app, "www")
		path_to_index = netmanthan.get_app_path(app, "www")
...
def add_route_to_global_search(route):
	...
	netmanthan.local.no_cache = True
...
def get_formatted_value(value, field):
	...
	if getattr(field, "fieldtype", None) in ["Text", "Text Editor"]:
		value = unescape_html(netmanthan.safe_decode(value))
...
def sync_global_search():
	...
	:return:
	"""
	while netmanthan.cache().llen("global_search_queue") > 0:
		# rpop to follow FIFO
		# Last one should override all previous contents of same document
		value = json.loads(netmanthan.cache().rpop("global_search_queue").decode("utf-8"))
		sync_value(value)
...
def sync_value_in_queue(value):
	try:
		...
		netmanthan.cache().lpush("global_search_queue", json.dumps(value))
...
def delete_for_document(doc):
	...
	"""
	netmanthan.db.delete("__global_search", {"doctype": doc.doctype, "name": doc.name})
...
def search(text, start=0, limit=20, doctype=""):
	...
	"""
	from netmanthan.desk.doctype.global_search_settings.global_search_settings import (
		get_doctypes_for_global_search,
	)
	from netmanthan.query_builder.functions import Match
	...
	for doctype in allowed_doctypes:
		for index, r in enumerate(results):
			if r.doctype == doctype and r.rank > 0.0:
				try:
					meta = netmanthan.get_meta(r.doctype)
					if meta.image_field:
						r.image = netmanthan.db.get_value(r.doctype, r.name, meta.image_field)
				except Exception:
