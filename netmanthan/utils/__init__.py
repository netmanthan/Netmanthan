...
def get_fullname(user=None):
	...
	if not hasattr(netmanthan.local, "fullnames"):
		netmanthan.local.fullnames = {}

	if not netmanthan.local.fullnames.get(user):
		p = netmanthan.db.get_value("User", user, ["first_name", "last_name"], as_dict=True)
		if p:
			netmanthan.local.fullnames[user] = (
				" ".join(filter(None, [p.get("first_name"), p.get("last_name")])) or user
			)
		else:
			netmanthan.local.fullnames[user] = user

	return netmanthan.local.fullnames.get(user)
...
def get_email_address(user=None):
	...
	return netmanthan.db.get_value("User", user, "email")
...
def validate_phone_number_with_country_code(phone_number, fieldname):
	...
	try:
		...
		if not valid_number:
			netmanthan.throw(
				error_message.format(netmanthan.bold(phone_number), netmanthan.bold(fieldname)),
				title=error_title,
				exc=netmanthan.InvalidPhoneNumberError,
			)
...
def validate_phone_number(phone_number, throw=False):
	...
	if not match and throw:
		netmanthan.throw(
			netmanthan._("{0} is not a valid Phone Number").format(phone_number), netmanthan.InvalidPhoneNumberError
		)
...
def validate_name(name, throw=False):
	...
	if not match and throw:
		netmanthan.throw(netmanthan._("{0} is not a valid Name").format(name), netmanthan.InvalidNameError)
...
def validate_email_address(email_str, throw=False):
	...
	def _check(e):
		...
		if not _valid:
			if throw:
				invalid_email = netmanthan.utils.escape_html(e)
				netmanthan.throw(
					netmanthan._("{0} is not a valid Email Address").format(invalid_email),
					netmanthan.InvalidEmailAddressError,
				)
			return None
		else:
			return matched
...
def validate_url(txt, throw=False, valid_schemes=None):
	...
	if not is_valid and throw:
		netmanthan.throw(netmanthan._("'{0}' is not a valid URL").format(netmanthan.bold(txt)))
...
def has_gravatar(email):
	...
	if netmanthan.flags.in_import or netmanthan.flags.in_install or netmanthan.flags.in_test:
		# no gravatar if via upload
		# since querying gravatar for every item will be slow
		return ""

	hexdigest = hashlib.md5(netmanthan.as_unicode(email).encode("utf-8")).hexdigest()
...
def get_gravatar(email):
	from netmanthan.utils.identicon import Identicon
	...
	return Format(
		custom_var_printers=[
			...
			*[(variable_name, lambda *a, **kw: placeholder) for variable_name in blocklist],
			...
			(["_secret", netmanthan._dict, lambda *a, **kw: False], dict_printer),
		],
	)


def log(event, details):
	netmanthan.logger(event).info(details)
...
def get_defaults(key=None):
	...
	"""
	return netmanthan.db.get_defaults(key)
...
def set_default(key, val):
	...
	"""
	return netmanthan.db.set_default(key, val)
...
def get_path(*path, **kwargs):
	...
	if not base:
		base = netmanthan.local.site_path
	return os.path.join(base, *path)


def get_site_base_path():
	return netmanthan.local.site_path
...
def get_bench_path():
	return os.path.realpath(os.path.join(os.path.dirname(netmanthan.__file__), "..", "..", ".."))


def get_bench_id():
	return netmanthan.get_conf().get("bench_id", get_bench_path().strip("/").replace("/", "-"))


def get_site_id(site=None):
	return f"{site or netmanthan.local.site}@{get_bench_id()}"
...
def get_site_url(site):
	return f"http://{site}:{netmanthan.get_conf(site).webserver_port}"
...
def get_test_client() -> Client:
	"""Returns an test instance of the netmanthan WSGI"""
	from netmanthan.app import application

	return Client(application)


def get_hook_method(hook_name, fallback=None):
	method = netmanthan.get_hooks().get(hook_name)
	if method:
		method = netmanthan.get_attr(method[0])
		return method
	if fallback:
		return fallback
...
def call_hook_method(hook, *args, **kwargs):
	out = None
	for method_name in netmanthan.get_hooks(hook):
		out = out or netmanthan.get_attr(method_name)(*args, **kwargs)
...
def update_progress_bar(txt, i, l, absolute=False):
	...
	if not getattr(netmanthan.local, "request", None) or is_cli():
		lt = len(txt)
		try:
...
def get_html_format(print_path):
	...
	if os.path.exists(print_path):
		...
		for include_directive, path in INCLUDE_DIRECTIVE_PATTERN.findall(html_format):
			for app_name in netmanthan.get_installed_apps():
				include_path = netmanthan.get_app_path(app_name, *path.split(os.path.sep))
...
def get_sites(sites_path=None):
	if not sites_path:
		sites_path = getattr(netmanthan.local, "sites_path", None) or "."
...
def markdown(text, sanitize=True, linkify=True):
	html = text if is_html(text) else netmanthan.utils.md_to_html(text)
...
def get_site_info():
	from netmanthan.email.queue import get_emails_sent_this_month
	from netmanthan.utils.user import get_system_managers

	# only get system users
	users = netmanthan.get_all(
		"User",
		filters={"user_type": "System User", "name": ("not in", netmanthan.STANDARD_USERS)},
	...
	system_settings = netmanthan.db.get_singles_dict("System Settings")
	space_usage = netmanthan._dict((netmanthan.local.conf.limits or {}).get("space_usage", {}))
	...
	site_info = {
		...
		"last_logins": netmanthan.get_all("Activity Log", **kwargs),
	}

	# from other apps
	for method_name in netmanthan.get_hooks("get_site_info"):
		site_info.update(netmanthan.get_attr(method_name)(site_info) or {})

	# dumps -> loads to prevent datatype conflicts
	return json.loads(netmanthan.as_json(site_info))
...
def get_db_count(*args):
	...
	Example:
	        via terminal:
	                bench --site erpnext.local execute netmanthan.utils.get_db_count --args "['DocType', 'Communication']"
	"""
	db_count = {}
	for doctype in args:
		db_count[doctype] = netmanthan.db.count(doctype)

	return json.loads(netmanthan.as_json(db_count))
...
def call(fn, *args, **kwargs):
	...
	Pass a doctype or a series of doctypes to get the count of docs in them
	Parameters:
	        fn: netmanthan function to be called
	...
	Example:
	        via terminal:
	                bench --site erpnext.local execute netmanthan.utils.call --args '''["netmanthan.get_all", "Activity Log"]''' --kwargs '''{"fields": ["user", "creation", "full_name"], "filters":{"Operation": "Login", "Status": "Success"}, "limit": "10"}'''
	"""
	return json.loads(netmanthan.as_json(netmanthan.call(fn, *args, **kwargs)))
...
def get_safe_filters(filters):
	try:
		...
		if isinstance(filters, (int, float)):
			filters = netmanthan.as_unicode(filters)
...
def set_request(**kwargs):
	...
	netmanthan.local.request = Request(builder.get_environ())
...
def get_html_for_route(route):
	...
	html = netmanthan.safe_decode(response.get_data())
	return html
...
def get_build_version():
	try:
		return str(os.path.getmtime(os.path.join(netmanthan.local.sites_path, "assets/assets.json")))
	except OSError:
		# .build can sometimes not exist
		# this is not a major problem so send fallback
		return netmanthan.utils.random_string(8)


def get_assets_json():
	def _get_assets():
		# get merged assets.json and assets-rtl.json
		assets = netmanthan.parse_json(netmanthan.read_file("assets/assets.json"))

		if assets_rtl := netmanthan.read_file("assets/assets-rtl.json"):
			assets.update(netmanthan.parse_json(assets_rtl))

		return assets

	if not hasattr(netmanthan.local, "assets_json"):
		if not netmanthan.conf.developer_mode:
			netmanthan.local.assets_json = netmanthan.cache().get_value(
				"assets_json",
				_get_assets,
				shared=True,
			)
			...
			netmanthan.local.assets_json = _get_assets()

	return netmanthan.local.assets_json
...
def add_user_info(user, user_info):
	if user not in user_info:
		...
		user_info[user] = netmanthan._dict(
