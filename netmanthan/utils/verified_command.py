...
def get_secret():
	from netmanthan.utils.password import get_encryption_key

	return netmanthan.local.conf.get("secret") or get_encryption_key()


def verify_request():
	"""Verify if the incoming signed request if it is correct."""
	query_string = netmanthan.safe_decode(
        netmanthan.local.flags.signed_query_string or getattr(netmanthan.request, "query_string", None)
	)
	...
	if signature_string in query_string:
		...
		valid_method = netmanthan.request.method == "GET"
		valid_request_data = not (netmanthan.request.form or netmanthan.request.data)
...
def get_url(cmd, params, nonce=None, secret=None):
	...
	return netmanthan.utils.get_url("".join(["api/method/", cmd, "?", urlencode(params)]))


def get_signature(params, nonce, secret=None):
	params = "".join(netmanthan.utils.cstr(p) for p in params.values())
	if not secret:
		secret = netmanthan.local.conf.get("secret") or "secret"
