...
def get_change_log(user=None):
	...
	last_known_versions = netmanthan._dict(
		json.loads(netmanthan.db.get_value("User", user, "last_known_versions") or "{}")
	)
	...
	if "netmanthan" in current_versions:
		set_in_change_log("netmanthan", current_versions["netmanthan"], change_log)

	return change_log


def get_change_log_for_app(app, from_version, to_version):
	change_log_folder = os.path.join(netmanthan.get_app_path(app), "change_log")
		...
		if folder in major_version_folders:
			for file in os.listdir(os.path.join(change_log_folder, folder)):
				...
				if from_version < version <= to_version:
					...
					content = netmanthan.read_file(file_path)
...
def get_versions():
	...
	Example:
	        ...
	        {
	                "netmanthan": {
	                        "title": "netmanthan Framework",
	                        "version": "5.0.0"
	                }
	        }"""
	versions = {}
	for app in netmanthan.get_installed_apps(_ensure_on_bench=True):
		app_hooks = netmanthan.get_hooks(app_name=app)
		versions[app] = {
		...
		try:
			versions[app]["version"] = netmanthan.get_attr(app + ".__version__")
...
def check_for_update():
	updates = netmanthan._dict(major=[], minor=[], patch=[])
	apps = get_versions()
...
def check_release_on_github(app: str):
	"""
	Check the latest release for a given netmanthan application hosted on Github.

	Args:
	        app (str): The name of the netmanthan application.
...
def add_message_to_redis(update_json):
	...
	user_list = [x.name for x in netmanthan.get_all("User", filters={"enabled": True})]
	system_managers = [user for user in user_list if "System Manager" in netmanthan.get_roles(user)]
...
def show_update_popup():
	...
	if update_message:
		netmanthan.msgprint(update_message, title=_("New updates are available"), indicator="green")
