...
def request_cache(func: Callable) -> Callable:
	...
	netmanthan.local.request_cache. The cache only persists for the current request
	...
	def wrapper(*args, **kwargs):
		if not getattr(netmanthan.local, "initialised", None):
			return func(*args, **kwargs)
		if not hasattr(netmanthan.local, "request_cache"):
			netmanthan.local.request_cache = defaultdict(dict)
		...
		try:
			return netmanthan.local.request_cache[func][args_key]
		except KeyError:
			return_val = func(*args, **kwargs)
			netmanthan.local.request_cache[func][args_key] = return_val
			return return_val
...
def site_cache(ttl: int | None = None, maxsize: int | None = None) -> Callable:
	...
	netmanthan.utils.caching._SITE_CACHE. The cache persists on the parent process.
	...
	workers, use redis (netmanthan.cache API) instead.

	Usage:
	        from netmanthan.utils.caching import site_cache
	...
	def time_cache_wrapper(func: Callable = None) -> Callable:
		...
		def site_cache_wrapper(*args, **kwargs):
			if getattr(netmanthan.local, "initialised", None):
				...
				if hasattr(func, "maxsize") and len(_SITE_CACHE[func_key][netmanthan.local.site]) >= func.maxsize:
					_SITE_CACHE[func_key][netmanthan.local.site].pop(
						next(iter(_SITE_CACHE[func_key][netmanthan.local.site])), None
					)

				if func_call_key not in _SITE_CACHE[func_key][netmanthan.local.site]:
					_SITE_CACHE[func_key][netmanthan.local.site][func_call_key] = func(*args, **kwargs)

				return _SITE_CACHE[func_key][netmanthan.local.site][func_call_key]
...
def redis_cache(ttl: int | None = 3600, user: str | bool | None = None) -> Callable:
	...
	def wrapper(func: Callable = None) -> Callable:
		...
		def clear_cache():
			netmanthan.cache().delete_keys(func_key)
		...
		def redis_cache_wrapper(*args, **kwargs):
			...
			if netmanthan.cache().exists(func_call_key):
				return netmanthan.cache().get_value(func_call_key, user=user)
			else:
				val = func(*args, **kwargs)
				ttl = getattr(func, "ttl", 3600)
				netmanthan.cache().set_value(func_call_key, val, expires_in_sec=ttl, user=user)
				return val
