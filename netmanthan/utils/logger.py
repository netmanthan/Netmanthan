...
import netmanthan
from netmanthan.utils import get_sites

default_log_level = logging.WARNING if netmanthan._dev_server else logging.ERROR
...
def get_logger(
	...
	if allow_site is True:
		site = getattr(netmanthan.local, "site", None)
	...
	try:
		return netmanthan.loggers[logger_name]
	except KeyError:
		pass
	...
	logger.setLevel(netmanthan.log_level or default_log_level)
	...
	netmanthan.loggers[logger_name] = logger
...
class SiteContextFilter(logging.Filter):
	...
	def filter(self, record) -> bool:
		if "Form Dict" not in str(record.msg):
			site = getattr(netmanthan.local, "site", None)
			form_dict = sanitized_dict(getattr(netmanthan.local, "form_dict", None))
...
def set_log_level(level: int) -> None:
	...
	netmanthan.log_level = getattr(logging, (level or "").upper(), None) or default_log_level
	netmanthan.loggers = {}
