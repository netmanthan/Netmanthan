...
def inspect(var, render=True):
	from netmanthan.utils.jinja import get_jenv
...
def web_blocks(blocks):
	import netmanthan
	from netmanthan import _, _dict, throw
	from netmanthan.website.doctype.web_page.web_page import get_web_blocks_html
	...
	if not netmanthan.flags.web_block_scripts:
...
def include_script(path, preload=True):
	...
	if preload:
		...
		netmanthan.local.preload_assets["script"].append(path)
...
def include_style(path, rtl=None, preload=True):
	...
	if preload:
		...
		netmanthan.local.preload_assets["style"].append(path)

	return f'<link type="text/css" rel="stylesheet" href="{path}">'


def bundled_asset(path, rtl=None):
	from netmanthan.utils import get_assets_json
	from netmanthan.website.utils import abs_url
