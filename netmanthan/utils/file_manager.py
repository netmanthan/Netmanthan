...
import netmanthan
from netmanthan import _, conf
from netmanthan.query_builder.utils import DocType
from netmanthan.utils import call_hook_method, cint, cstr, encode, get_files_path, get_hook_method
...
def get_file_url(file_data_name):
	data = netmanthan.db.get_value("File", file_data_name, ["file_name", "file_url"], as_dict=True)
	return data.file_url or data.file_name


def upload():
	# get record details
	dt = netmanthan.form_dict.doctype
	dn = netmanthan.form_dict.docname
	file_url = netmanthan.form_dict.file_url
	filename = netmanthan.form_dict.filename
	netmanthan.form_dict.is_private = cint(netmanthan.form_dict.is_private)

	if not filename and not file_url:
		netmanthan.msgprint(_("Please select a file or url"), raise_exception=True)

	file_doc = get_file_doc()

	comment = {}
	if dt and dn:
		comment = netmanthan.get_doc(dt, dn).add_comment(
			"Attachment",
			_("added {0}").format(
...
def save_url(file_url, filename, dt, dn, folder, is_private, df=None):
	...
	# 	netmanthan.msgprint("URL must start with 'http://' or 'https://'")
	# 	return None, None

	file_url = unquote(file_url)
	file_size = netmanthan.form_dict.file_size
	...
	try:
		...
		return netmanthan.get_doc("File", f.duplicate_entry)
	return f


def get_uploaded_content():
	# should not be unicode when reading a file, hence using netmanthan.form
	if "filedata" in netmanthan.form_dict:
		if "," in netmanthan.form_dict.filedata:
			netmanthan.form_dict.filedata = netmanthan.form_dict.filedata.rsplit(",", 1)[1]
		netmanthan.uploaded_content = safe_b64decode(netmanthan.form_dict.filedata)
		netmanthan.uploaded_filename = netmanthan.form_dict.filename
		return netmanthan.uploaded_filename, netmanthan.uploaded_content
	else:
		netmanthan.msgprint(_("No file attached"))
		return None, None
...
def save_file(fname, content, dt, dn, folder=None, decode=False, is_private=0, df=None):
	...
	f = netmanthan.get_doc(file_data)
	f.flags.ignore_permissions = True
	try:
		f.insert()
	except netmanthan.DuplicateEntryError:
...
def get_file_data_from_hash(content_hash, is_private=0):
	for name in netmanthan.get_all(
		"File", {"content_hash": content_hash, "is_private": is_private}, pluck="name"
	):
		b = netmanthan.get_doc("File", name)
		return {k: b.get(k) for k in netmanthan.get_hooks()["write_file_keys"]}
	return False
...
def write_file(content, fname, is_private=0):
	...
	netmanthan.create_folder(file_path)
	# write the file
	if isinstance(content, str):
...
def remove_all(dt, dn, from_delete=False, delete_permanently=False):
	...
	try:
		for fid in netmanthan.get_all(
			"File", {"attached_to_doctype": dt, "attached_to_name": dn}, pluck="name"
		):
			if from_delete:
				# If deleting a doc, directly delete files
				netmanthan.delete_doc("File", fid, ignore_permissions=True, delete_permanently=delete_permanently)
			else:
...
def remove_file(
	...
	if not (attached_to_doctype and attached_to_name):
		attached = netmanthan.db.get_value(
	...
	if attached_to_doctype and attached_to_name and not from_delete:
		doc = netmanthan.get_doc(attached_to_doctype, attached_to_name)
		ignore_permissions = doc.has_permission("write") or False
		if netmanthan.flags.in_web_form:
			ignore_permissions = True
		if not file_name:
			file_name = netmanthan.db.get_value("File", fid, "file_name")
		comment = doc.add_comment("Attachment Removed", _("Removed {0}").format(file_name))
		netmanthan.delete_doc(
...
def delete_file(path):
	...
	if path:
		...
		if parts[0] == "files":
			path = netmanthan.utils.get_site_path("public", "files", parts[-1])

		else:
			path = netmanthan.utils.get_site_path("private", "files", parts[-1])
...
def get_file_name(fname, optional_suffix):
	...
	n_records = netmanthan.get_all("File", {"file_name": fname}, pluck="name")
...
def download_file(file_url):
	...
	Method : GET
	Endpoint : netmanthan.utils.file_manager.download_file
	URL Params : file_name = /path/to/file relative to site path
	"""
	file_doc = netmanthan.get_doc("File", {"file_url": file_url})
	...
	netmanthan.local.response.filecontent = filedata
...
def add_attachments(doctype, name, attachments):
	...
	for a in attachments:
		if isinstance(a, str):
			attach = netmanthan.db.get_value(
...
def is_safe_path(path: str) -> bool:
	...
	basedir = netmanthan.get_site_path()
