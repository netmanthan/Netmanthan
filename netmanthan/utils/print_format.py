...
from netmanthan.www.printview import validate_print_permission
...
def download_multi_pdf(
	...
	if not isinstance(doctype, dict):
		...
		netmanthan.local.response.filename = "{doctype}.pdf".format(
		...
		netmanthan.local.response.filename = f"{name}.pdf"

	netmanthan.local.response.filecontent = read_multi_pdf(output)
	netmanthan.local.response.type = "pdf"


def read_multi_pdf(output):
	# Get the content of the merged pdf files
	fname = os.path.join("/tmp", f"netmanthan-pdf-{netmanthan.generate_hash()}.pdf")
...
def download_pdf(
	...
	doc = doc or netmanthan.get_doc(doctype, name)
	validate_print_permission(doc)

	with print_language(language):
		pdf_file = netmanthan.get_print(
	...
	netmanthan.local.response.filecontent = pdf_file
	netmanthan.local.response.type = "pdf"
...
def report_to_pdf(html, orientation="Landscape"):
	...
	netmanthan.local.response.filename = "report.pdf"
	netmanthan.local.response.filecontent = get_pdf(html, {"orientation": orientation})
	netmanthan.local.response.type = "pdf"
...
def print_by_server(
	...
	print_settings = netmanthan.get_doc("Network Printer Settings", printer_setting)
	try:
		import cups
	except ImportError:
		netmanthan.throw(_("You need to install pycups to use this feature!"))
	...
	try:
		...
		if not file_path:
			file_path = os.path.join("/", "tmp", f"netmanthan-pdf-{netmanthan.generate_hash()}.pdf")
		...
		if (
			...
			netmanthan.throw(_("PDF generation failed"))
	except cups.IPPError:
		netmanthan.throw(_("Printing failed"))
