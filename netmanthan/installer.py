...
import netmanthan
from netmanthan.defaults import _clear_cache
from netmanthan.utils import cint, is_git_url
...
def _is_scheduler_enabled() -> bool:
	...
	try:
		netmanthan.connect()
		enable_scheduler = cint(netmanthan.db.get_single_value("System Settings", "enable_scheduler"))
	except Exception:
		pass
	finally:
		netmanthan.db.close()
...
def _new_site(
	...
	"""Install a new netmanthan site"""

	from netmanthan.utils import get_site_path, scheduler, touch_file
	...
	if not db_name:
		...
		db_name = "_" + hashlib.sha1(os.path.realpath(netmanthan.get_site_path()).encode()).hexdigest()[:16]
	...
	apps_to_install = (
            ["netmanthan"] + (netmanthan.conf.get("install_apps") or []) + (list(install_apps) or [])
	)
	...
	scheduler_status = "disabled" if netmanthan.utils.scheduler.is_scheduler_disabled() else "enabled"
...
def install_db(
	...
	from netmanthan.database import setup_database

	if not db_type:
		db_type = netmanthan.conf.db_type or "mariadb"
	...
	netmanthan.flags.in_install_db = True

	netmanthan.flags.root_login = root_login
	netmanthan.flags.root_password = root_password
	setup_database(force, source_sql, verbose, no_mariadb_socket)

	netmanthan.conf.admin_password = netmanthan.conf.admin_password or admin_password

	remove_missing_apps()

	netmanthan.db.create_auth_table()
	netmanthan.db.create_global_search_table()
	netmanthan.db.create_user_settings_table()

	netmanthan.flags.in_install_db = False
...
def find_org(org_repo: str) -> tuple[str, str]:
	...
	find_org()
	ref -> https://github.com/netmanthan/bench/blob/develop/bench/utils/__init__.py#L390
	...
	from netmanthan.exceptions import InvalidRemoteException

	for org in ["netmanthan", "erpnext"]:
...
def fetch_details_from_tag(_tag: str) -> tuple[str, str, str]:
	...
	ref -> https://github.com/netmanthan/bench/blob/develop/bench/utils/__init__.py#L403
...
def parse_app_name(name: str) -> str:
	...
	ref -> https://github.com/netmanthan/bench/blob/develop/bench/app.py#L114
...
def install_app(name, verbose=False, set_as_patched=True, force=False):
	...
	from netmanthan.model.sync import sync_for
	from netmanthan.modules.utils import sync_customizations
	from netmanthan.utils.fixtures import sync_fixtures

	netmanthan.flags.in_install = name
	netmanthan.flags.ignore_in_install = False

	netmanthan.clear_cache()
	app_hooks = netmanthan.get_hooks(app_name=name)
	installed_apps = netmanthan.get_installed_apps()
	...
	if name not in netmanthan.get_all_apps():
	...
	for before_install in app_hooks.before_install or []:
		out = netmanthan.get_attr(before_install)()
		if out is False:
			return

	for fn in netmanthan.get_hooks("before_app_install"):
		netmanthan.get_attr(fn)(name)
	...
	netmanthan.get_doc("Portal Settings", "Portal Settings").sync_menu()

	if set_as_patched:
		set_all_patches_as_completed(name)

	for after_install in app_hooks.after_install or []:
		netmanthan.get_attr(after_install)()

	for fn in netmanthan.get_hooks("after_app_install"):
		netmanthan.get_attr(fn)(name)
	...
	for after_sync in app_hooks.after_sync or []:
		netmanthan.get_attr(after_sync)()  #

	netmanthan.flags.in_install = False
...
def add_to_installed_apps(app_name, rebuild_website=True):
	...
	if app_name not in installed_apps:
		...
		netmanthan.db.set_global("installed_apps", json.dumps(installed_apps))
		netmanthan.db.commit()
...
def remove_app(app_name, dry_run=False, yes=False, no_backup=False, force=False):
	...
	app_hooks = netmanthan.get_hooks(app_name=app_name)

	# dont allow uninstall app if not installed unless forced
	if not force:
		if app_name not in netmanthan.get_installed_apps():
	...
	if not (dry_run or no_backup):
		from netmanthan.utils.backups import scheduled_backup

		print("Backing up...")
		scheduled_backup(ignore_files=True)

	netmanthan.flags.in_uninstall = True

	for before_uninstall in app_hooks.before_uninstall or []:
		netmanthan.get_attr(before_uninstall)()

	for fn in netmanthan.get_hooks("before_app_uninstall"):
		netmanthan.get_attr(fn)(app_name)

	modules = netmanthan.get_all("Module Def", filters={"app_name": app_name}, pluck="name")
	...
	for after_uninstall in app_hooks.after_uninstall or []:
		netmanthan.get_attr(after_uninstall)()

	for fn in netmanthan.get_hooks("after_app_uninstall"):
		netmanthan.get_attr(fn)(app_name)

	click.secho(f"Uninstalled App {app_name} from Site {site}", fg="green")
	netmanthan.flags.in_uninstall = False
...
def _delete_modules(modules: list[str], dry_run: bool) -> list[str]:
	...
	for module_name in modules:
		...
		for doctype in netmanthan.get_all(
			...
			if not dry_run:
				if doctype.issingle:
					netmanthan.delete_doc("DocType", doctype.name, ignore_on_trash=True)
				else:
		...
		if not dry_run:
			netmanthan.delete_doc("Module Def", module_name, ignore_on_trash=True, force=True)
...
def _delete_linked_documents(
	...
	for doctype, fieldname in doctype_linkfield_map.items():
		for record in netmanthan.get_all(doctype, filters={fieldname: module_name}, pluck="name"):
			print(f"* removing {doctype} '{record}'...")
			if not dry_run:
				netmanthan.delete_doc(doctype, record, ignore_on_trash=True, force=True)
...
def _get_module_linked_doctype_field_map() -> dict[str, str]:
	...
	linked_doctypes = netmanthan.get_all(
		"DocField",
		filters={"fieldtype": "Link", "options": "Module Def"},
		fields=["parent", "fieldname"],
	)
	existing_linked_doctypes = [d for d in linked_doctypes if netmanthan.db.exists("DocType", d.parent)]
...
def _delete_doctypes(doctypes: list[str], dry_run: bool) -> None:
	for doctype in set(doctypes):
		...
		if not dry_run:
			netmanthan.delete_doc("DocType", doctype, ignore_on_trash=True)
			netmanthan.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{doctype}`")
...
def set_all_patches_as_completed(app):
	from netmanthan.modules.patch_handler import get_patches_from_app

	patches = get_patches_from_app(app)
	for patch in patches:
		netmanthan.get_doc({"doctype": "Patch Log", "patch": patch}).insert(ignore_permissions=True)
	netmanthan.db.commit()


def init_singles():
	singles = netmanthan.get_all("DocType", filters={"issingle": True}, pluck="name")
	for single in singles:
		if netmanthan.db.get_singles_dict(single):
			continue

		try:
			doc = netmanthan.new_doc(single)
			doc.flags.ignore_mandatory = True
			doc.flags.ignore_validate = True
			doc.save()
		except (ImportError, netmanthan.DoesNotExistError):
...
def make_conf(
	...
	sites_path = netmanthan.local.sites_path
	netmanthan.destroy()
	netmanthan.init(site, sites_path=sites_path)


def make_site_config(
	db_name=None, db_password=None, site_config=None, db_type=None, db_host=None, db_port=None
):
	netmanthan.create_folder(os.path.join(netmanthan.local.site_path))
...
def update_site_config(key, value, validate=True, site_config_path=None):
	...
	from netmanthan.utils.synchronization import filelock
...
def _update_config_file(key: str, value, config_file: str):
	...
	if hasattr(netmanthan.local, "conf"):
		netmanthan.local.conf[key] = value


def get_site_config_path():
	return os.path.join(netmanthan.local.site_path, "site_config.json")
...
def make_site_dirs():
	for dir_path in [
		...
		path = netmanthan.get_site_path(dir_path)
		os.makedirs(path, exist_ok=True)


def add_module_defs(app, ignore_if_duplicate=False):
	modules = netmanthan.get_module_list(app)
	for module in modules:
		d = netmanthan.new_doc("Module Def")
		d.app_name = app
...
def remove_missing_apps():
	...
	apps = ("netmanthan_subscription", "shopping_cart")
	installed_apps = json.loads(netmanthan.db.get_global("installed_apps") or "[]")
	for app in apps:
...
def extract_sql_from_archive(sql_file_path):
	...
	"""
	from netmanthan.utils import get_bench_relative_path
...
def extract_files(site_name, file_path):
	...
	# Need to do netmanthan.init to maintain the site locals
	netmanthan.init(site=site_name)
	abs_site_path = os.path.abspath(netmanthan.get_site_path())
...
def is_downgrade(sql_file_path, verbose=False):
	...
	if netmanthan.conf.db_type not in (None, "mariadb"):
		return False
	...
	with open(sql_file_path) as f:
		for line in f:
			if head in line:
				# 'line' (str) format: ('2056588823','2020-05-11 18:21:31.488367','2020-06-12 11:49:31.079506','Administrator','Administrator',0,'Installed Applications','installed_applications','Installed Applications',1,'netmanthan','v10.1.71-74 (3c50d5e) (v10.x.x)','v10.x.x'),('855c640b8e','2020-05-11 18:21:31.488367','2020-06-12 11:49:31.079506','Administrator','Administrator',0,'Installed Applications','installed_applications','Installed Applications',2,'your_custom_app','0.0.1','master')
				line = line.strip().lstrip(head).rstrip(";").strip()
				app_rows = netmanthan.safe_eval(line)
				...
				# 'all_apps' (list) format: [('netmanthan', '12.x.x-develop ()', 'develop'), ('your_custom_app', '0.0.1', 'master')]
				...
				for app in all_apps:
					...
					if app_name == "netmanthan":
						try:
							current_version = Version(netmanthan.__version__)
						...
						if verbose and downgrade:
							print(f"Your site will be downgraded from netmanthan {backup_version} to {current_version}")
...
def partial_restore(sql_file_path, verbose=False):
	...
	if netmanthan.conf.db_type in (None, "mariadb"):
		from netmanthan.database.mariadb.setup_db import import_db_from_sql
	elif netmanthan.conf.db_type == "postgres":
		import warnings

		from netmanthan.database.postgres.setup_db import import_db_from_sql
...
def validate_database_sql(path, _raise=True):
	...
	if _raise and (missing_table or empty_file):
		raise netmanthan.InvalidDatabaseFile
