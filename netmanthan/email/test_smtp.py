...
class TestSMTP(netmanthanTestCase):
	...
	def test_get_email_account(self):
		existing_email_accounts = netmanthan.get_all(
		...
		for email_account in existing_email_accounts:
			netmanthan.db.set_value("Email Account", email_account["name"], unset_details)

		# remove mail_server config so that test@example.com is not created
		mail_server = netmanthan.conf.get("mail_server")
		del netmanthan.conf["mail_server"]

		netmanthan.local.outgoing_email_account = {}
		...
		netmanthan.conf["mail_server"] = mail_server
		for email_account in existing_email_accounts:
			set_details = {
			...
			netmanthan.db.set_value("Email Account", email_account["name"], set_details)
...
def create_email_account(email_id, password, enable_outgoing, default_outgoing=0, append_to=None):
	...
	email_account = netmanthan.new_doc("Email Account")
