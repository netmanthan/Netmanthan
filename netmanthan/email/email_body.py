...
class EMail:
	...
	def attach_file(self, n):
		...
		_file = netmanthan.get_doc("File", {"file_name": n})
	...
	def validate(self):
		...
		self.recipients = [strip(r) for r in self.recipients if r not in netmanthan.STANDARD_USERS]
		self.cc = [strip(r) for r in self.cc if r not in netmanthan.STANDARD_USERS]
		self.bcc = [strip(r) for r in self.bcc if r not in netmanthan.STANDARD_USERS]
	...
	def make(self):
		...
		for hook in netmanthan.get_hooks("make_email_body_message"):
			netmanthan.get_attr(hook)(self)
...
def get_formatted_html(
	...
	email_account=None,
	header=None,
	unsubscribe_link: netmanthan._dict | None = None,
	sender=None,
	...
	rendered_email = netmanthan.get_template("templates/emails/standard.html").render(
		{
...
def get_email_html(template, args, subject, header=None, with_container=False):
	...
	email = netmanthan.utils.jinja.get_email_from_template(template, args)
...
def inline_style_in_html(html):
	...
	css_files = netmanthan.get_hooks("email_css")
...
def get_message_id():
	...
	return email.utils.make_msgid(domain=netmanthan.local.site)
...
def get_footer(email_account, footer=None):
	...
	sender_address = netmanthan.db.get_default("email_footer_address")

	if sender_address:
		args.update({"sender_address": sender_address})

	if not cint(netmanthan.db.get_default("disable_standard_email_footer")):
		args.update({"default_mail_footer": netmanthan.get_hooks("default_mail_footer")})

	footer += netmanthan.utils.jinja.get_email_from_template("email_footer", args)[0]

	return footer


def replace_filename_with_cid(message):
	"""Replaces <img embed="assets/netmanthan/images/filename.jpg" ...> with
...
def get_filecontent_from_path(path):
	...
	if path.startswith("assets/"):
		...
		# public file
		full_path = netmanthan.get_site_path("public", path)
	elif path.startswith("private/files/"):
		# private file
		full_path = netmanthan.get_site_path(path)
	else:
		full_path = path
...
def get_header(header=None):
	...
	from netmanthan.utils.jinja import get_email_from_template
	...
	if not title:
		title = netmanthan.get_hooks("app_title")[-1]
