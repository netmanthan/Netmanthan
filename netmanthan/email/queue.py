...
from netmanthan.utils import cint, cstr, get_url, now_datetime
...
def get_emails_sent_this_month(email_account=None):
	...
	return netmanthan.db.sql(q, q_args)[0][0]
...
def get_unsubscribe_message(
	...
	return netmanthan._dict(html=html, text=text)
...
def get_unsubcribed_url(
	...
	# for test
	netmanthan.local.flags.signed_query_string = query_string
...
def unsubscribe(doctype, name, email):
	...
	if not netmanthan.flags.in_test and not verify_request():
		return
...
def flush(from_test=False):
	...
	from netmanthan.email.doctype.email_queue.email_queue import send_mail
	from netmanthan.utils.background_jobs import get_jobs

	# To avoid running jobs inside unit tests
	if netmanthan.are_emails_muted():
		msgprint(_("Emails are muted"))
		from_test = True

	if cint(netmanthan.db.get_default("suspend_email_queue")) == 1:
		return

	try:
		queued_jobs = set(get_jobs(site=netmanthan.local.site, key="job_name")[netmanthan.local.site])
	except Exception:
		queued_jobs = set()
	...
	for row in get_queue():
		try:
			...
			if job_name not in queued_jobs:
				...
				netmanthan.logger().debug(f"Not queueing job {job_name} because it is in queue already")
		except Exception:
			netmanthan.get_doc("Email Queue", row.name).log_error()
