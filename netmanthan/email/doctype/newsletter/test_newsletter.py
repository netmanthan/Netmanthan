...
import netmanthan
from netmanthan.email.doctype.newsletter.exceptions import (
	NewsletterAlreadySentError,
	NoRecipientFoundError,
)
from netmanthan.email.doctype.newsletter.newsletter import (
	Newsletter,
...
from netmanthan.utils import add_days, getdate
...
class TestNewsletterMixin:
	...
	def tearDown(self):
		...
		for newsletter in newsletters:
			...
			netmanthan.delete_doc("Newsletter", newsletter)
			netmanthan.db.delete("Newsletter Email Group", {"parent": newsletter})
			newsletters.remove(newsletter)

	def setup_email_group(self):
		if not netmanthan.db.exists("Email Group", "_Test Email Group"):
			netmanthan.get_doc({"doctype": "Email Group", "title": "_Test Email Group"}).insert()
		...
		for email in emails:
			...
			try:
				...
				netmanthan.db.update(doctype, email_filters, "unsubscribed", 0)
	...
	def send_newsletter(self, published=0, schedule_send=None) -> str | None:
		...
		netmanthan.db.delete("Newsletter")
	...
	def get_newsletter(**kwargs) -> "Newsletter":
		...
		similar_newsletters = netmanthan.get_all(doctype, newsletter_content, pluck="name")

		for similar_newsletter in similar_newsletters:
			netmanthan.delete_doc(doctype, similar_newsletter)

		newsletter = netmanthan.get_doc({"doctype": doctype, **newsletter_content, **kwargs})
		...
		attached_files = netmanthan.get_all(
			"File",
			{
		...
		for file in attached_files:
			netmanthan.delete_doc("File", file)

		return newsletter


class TestNewsletter(TestNewsletterMixin, netmanthanTestCase):
	def test_send(self):
		self.send_newsletter()

		email_queue_list = [netmanthan.get_doc("Email Queue", e.name) for e in netmanthan.get_all("Email Queue")]
	...
	def test_newsletter_send_test_email(self):
		...
		email_queue = netmanthan.get_all(
			"Email Queue",
			filters=[
				["reference_doctype", "=", "Newsletter"],
	...
	def test_send_scheduled_email_error_handling(self):
		...
		job_path = "netmanthan.email.doctype.newsletter.newsletter.Newsletter.queue_all"
		m = MagicMock(side_effect=netmanthan.OutgoingEmailError)

		with self.assertRaises(netmanthan.OutgoingEmailError):
