...
from netmanthan.email.doctype.email_group.email_group import add_subscribers
from netmanthan.rate_limiter import rate_limit
from netmanthan.utils.safe_exec import is_job_queued
from netmanthan.utils.verified_command import get_signed_params, verify_request
...
class Newsletter(WebsiteGenerator):
	...
	def get_sending_status(self):
		count_by_status = netmanthan.get_all(
			"Email Queue",
		...
		emails_queued = is_job_queued(
			job_name=netmanthan.utils.get_job_name("send_bulk_emails_for", self.doctype, self.name),
			queue="long",
		)
	...
	def send_test_email(self, email):
		test_emails = netmanthan.utils.validate_email_address(email, throw=True)
		self.send_newsletter(emails=test_emails)
		netmanthan.msgprint(_("Test email sent to {0}").format(email), alert=True)
	...
	def validate_newsletter_status(self):
		if self.email_sent:
			netmanthan.throw(_("Newsletter has already been sent"), exc=NewsletterAlreadySentError)

		if self.get("__islocal"):
			netmanthan.throw(_("Please save the Newsletter before sending"), exc=NewsletterNotSavedError)

	def validate_newsletter_recipients(self):
		if not self.newsletter_recipients:
			netmanthan.throw(_("Newsletter should have atleast one recipient"), exc=NoRecipientFoundError)
		self.validate_recipient_address()

	def validate_sender_address(self):
		"""Validate self.send_from is a valid email address or not."""
		if self.sender_email:
			netmanthan.utils.validate_email_address(self.sender_email, throw=True)
			self.send_from = (
	...
	def validate_recipient_address(self):
		...
		for recipient in self.newsletter_recipients:
			netmanthan.utils.validate_email_address(recipient, throw=True)

	def validate_publishing(self):
		if self.send_webview_link and not self.published:
			netmanthan.throw(_("Newsletter must be published to send webview link in email"))
	...
	def queue_all(self):
		...
		self.email_sent_at = netmanthan.utils.now()
	...
	def send_newsletter(self, emails: list[str]):
		...
		sender = self.send_from or netmanthan.utils.get_formatted_email(self.owner)
		args = self.as_dict()
		args["message"] = self.get_message()

		is_auto_commit_set = bool(netmanthan.db.auto_commit_on_many_writes)
		netmanthan.db.auto_commit_on_many_writes = not netmanthan.flags.in_test
		...
		netmanthan.db.auto_commit_on_many_writes = is_auto_commit_set

	def get_message(self) -> str:
		message = self.message
		if self.content_type == "Markdown":
			message = netmanthan.utils.md_to_html(self.message_md)
		if self.content_type == "HTML":
			message = self.message_html

		return netmanthan.render_template(message, {"doc": self.as_dict()})

	def get_recipients(self) -> list[str]:
		"""Get recipients from Email Group"""
		emails = netmanthan.get_all(
			"Email Group Member",
	...
	def get_email_groups(self) -> list[str]:
		...
		return [x.email_group for x in self.email_group] or netmanthan.get_all(
...
def confirmed_unsubscribe(email, group):
	...
	doc = netmanthan.get_doc("Email Group Member", {"email": email, "email_group": group})
...
def subscribe(email, email_group=None):  # noqa
	...
	api_endpoint = netmanthan.utils.get_url(
		"/api/method/netmanthan.email.doctype.newsletter.newsletter.confirm_subscription"
	)
	...
	email_confirmation_template = netmanthan.db.get_value(
	...
	if email_confirmation_template:
		...
		email_template = netmanthan.get_doc("Email Template", email_confirmation_template)
		email_subject = email_template.subject
		content = netmanthan.render_template(email_template.response, args)
	else:
...
def confirm_subscription(email, email_group=_("Website")):  # noqa
	...
	if not netmanthan.db.exists("Email Group", email_group):
		netmanthan.get_doc({"doctype": "Email Group", "title": email_group}).insert(ignore_permissions=True)
...
def send_scheduled_email():
	...
	scheduled_newsletter = netmanthan.get_all(
		"Newsletter",
		filters={
			"schedule_send": ("<=", netmanthan.utils.now_datetime()),
			"email_sent": False,
	...
	for newsletter_name in scheduled_newsletter:
		try:
			newsletter = netmanthan.get_doc("Newsletter", newsletter_name)
			...
			netmanthan.db.set_value("Newsletter", newsletter_name, "email_sent", 0)
