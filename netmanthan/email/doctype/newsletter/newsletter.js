...
netmanthan.ui.form.on("Newsletter", {
	refresh(frm) {
		let doc = frm.doc;
		let can_write = in_list(netmanthan.boot.user.can_write, doc.doctype);
		...
		if (frm.is_new() && !doc.sender_email) {
			let { fullname, email } = netmanthan.user_info(doc.owner);
	...
	send_emails(frm) {
		netmanthan.dom.freeze(__("Queuing emails..."));
		frm.call("send_emails").then(() => {
			frm.refresh();
			netmanthan.dom.unfreeze();
			netmanthan.show_alert(
				__("Queued {0} emails", [netmanthan.utils.shorten_number(frm.doc.total_recipients)])
			);
		});
	},

	schedule_send_dialog(frm) {
		let hours = netmanthan.utils.range(24);
	...
	update_schedule_message(frm) {
		if (!frm.doc.email_sent && frm.doc.schedule_send) {
			let datetime = netmanthan.datetime.global_date_format(frm.doc.schedule_send);
