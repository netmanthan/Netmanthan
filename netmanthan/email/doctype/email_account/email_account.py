...
import netmanthan
from netmanthan import _, are_emails_muted, safe_encode
from netmanthan.desk.form import assign_to
from netmanthan.email.doctype.email_domain.email_domain import EMAIL_DOMAIN_FIELDS
from netmanthan.email.receive import EmailServer, InboundMail, SentEmailInInboxError
from netmanthan.email.smtp import SMTPServer
from netmanthan.email.utils import get_port
from netmanthan.model.document import Document
from netmanthan.utils import cint, comma_or, cstr, parse_addr, validate_email_address
from netmanthan.utils.background_jobs import enqueue, get_jobs
from netmanthan.utils.jinja import render_template
...
def cache_email_account(cache_name):
	def decorator_cache_email_account(func):
		...
		def wrapper_cache_email_account(*args, **kwargs):
			if not hasattr(netmanthan.local, cache_name):
				setattr(netmanthan.local, cache_name, {})

			cached_accounts = getattr(netmanthan.local, cache_name)
...
class EmailAccount(Document):
	...
	def validate(self):
		...
		if self.login_id_is_different:
			if not self.login_id:
				netmanthan.throw(_("Login Id is required"))
		else:
			self.login_id = None

		# validate the imap settings
		if self.enable_incoming and self.use_imap and len(self.imap_folder) <= 0:
			netmanthan.throw(_("You need to set one IMAP folder for {0}").format(netmanthan.bold(self.email_id)))

		if netmanthan.local.flags.in_patch or netmanthan.local.flags.in_test:
			return
		...
		if not netmanthan.local.flags.in_install and not self.awaiting_password:
			if validate_oauth or self.password or self.smtp_server in ("127.0.0.1", "localhost"):
				...
				if self.enable_incoming or (self.enable_outgoing and not self.no_smtp_authentication):
					if not use_oauth:
						netmanthan.throw(_("Password is required or select Awaiting Password"))

		if self.notify_if_unreplied:
			if not self.send_notification_to:
				netmanthan.throw(_("{0} is mandatory").format(self.meta.get_label("send_notification_to")))
			...
			for folder in self.imap_folder:
				if folder.append_to:
					...
					if folder.append_to not in valid_doctypes:
						netmanthan.throw(_("Append To can be one of {0}").format(comma_or(valid_doctypes)))

	def validate_smtp_conn(self):
		if not self.smtp_server:
			netmanthan.throw(_("SMTP Server is required"))
	...
	def before_save(self):
		...
		if not self.enable_incoming and self.default_incoming:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					netmanthan.bold(_("Default Incoming")), netmanthan.bold(_("Enable Incoming"))
				)
			)
		if not self.enable_outgoing and self.default_outgoing:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					netmanthan.bold(_("Default Outgoing")), netmanthan.bold(_("Enable Outgoing"))
				)
			)
		if messages:
			if len(messages) == 1:
				(as_list, messages) = (0, messages[0])
			netmanthan.msgprint(messages, as_list=as_list, indicator="orange", title=_("Defaults Updated"))
	...
	def there_must_be_only_one_default(self):
		...
		for field in ("default_incoming", "default_outgoing"):
			...
			for email_account in netmanthan.get_all("Email Account", filters={field: 1}):
				if email_account.name == self.name:
					continue

				email_account = netmanthan.get_doc("Email Account", email_account.name)
				email_account.set(field, 0)
				email_account.save()

	@netmanthan.whitelist()
	def get_domain_values(self, domain: str):
		return netmanthan.db.get_value("Email Domain", domain, EMAIL_DOMAIN_FIELDS, as_dict=True)

	def get_incoming_server(self, in_receive=False, email_sync_rule="UNSEEN"):
		"""Returns logged in POP3/IMAP connection object."""
		if netmanthan.cache().get_value("workers:no-internet") == True:
			return None
		...
		if not args.get("host"):
			netmanthan.throw(_("{0} is required").format("Email Server"))

		email_server = EmailServer(netmanthan._dict(args))
	...
	def check_email_server_connection(self, email_server, in_receive):
		...
		try:
			...
			if in_receive and any(map(lambda t: t in message, all_error_codes)):
				...
				netmanthan.throw(cstr(e))

		except OSError:
			if in_receive:
				# timeout while connecting, see receive.py connect method
				description = netmanthan.message_log.pop() if netmanthan.message_log else "Socket Error"
				if test_internet():
					self.db_set("no_failed", self.no_failed + 1)
					if self.no_failed > 2:
						self.handle_incoming_connect_error(description=description)
				else:
					netmanthan.cache().set_value("workers:no-internet", True)
				return None
			else:
				raise
	...
	def _password(self):
		raise_exception = not (
                self.auth_method == "OAuth" or self.no_smtp_authentication or netmanthan.flags.in_test
		)
	...
	def from_record(cls, record):
		email_account = netmanthan.new_doc(cls.DOCTYPE)
		email_account.update(record)
		return email_account

	@classmethod
	def find(cls, name):
		return netmanthan.get_doc(cls.DOCTYPE, name)

	@classmethod
	def find_one_by_filters(cls, **kwargs):
		name = netmanthan.db.get_value(cls.DOCTYPE, kwargs)
	...
	def get_account_details_from_site_config(cls):
		if not netmanthan.conf.get("mail_server"):
			return {}
		...
		field_to_conf_name_map = {
			...
			"name": {"conf_names": ("email_sender_name",), "default": "netmanthan"},
		...
		for doc_field_name, d in field_to_conf_name_map.items():
			...
			value = [netmanthan.conf.get(k) for k in conf_names if netmanthan.conf.get(k)]
	...
	def set_failed_attempts_count(self, value):
		netmanthan.cache().set(f"{self.name}:email-account-failed-attempts", value)

	def get_failed_attempts_count(self):
		return cint(netmanthan.cache().get(f"{self.name}:email-account-failed-attempts"))
	...
	def receive(self):
		...
		for mail in inbound_mails:
			try:
				...
				if self.use_imap:
					self.handle_bad_emails(mail.uid, mail.raw_message, netmanthan.get_traceback())
				exceptions.append(netmanthan.get_traceback())
			else:
				netmanthan.db.commit()

		if exceptions:
			raise Exception(netmanthan.as_json(exceptions))
		...
		def process_mail(messages, append_to=None):
			for index, message in enumerate(messages.get("latest_messages", [])):
				...
				if self.email_sync_option != "UNSEEN" or seen_status != "SEEN":
					...
					mails.append(InboundMail(message, self, netmanthan.safe_decode(uid), seen_status, append_to))
	...
	def handle_bad_emails(self, uid, raw, reason):
		if cint(self.use_imap):
			...
			unhandled_email = netmanthan.get_doc(
				{
					"raw": raw,
					"uid": uid,
					"reason": reason,
	...
	def send_auto_reply(self, communication, email):
		...
		from netmanthan.core.doctype.communication.email import set_incoming_outgoing_accounts
		...
		if self.enable_auto_reply:
			...
			netmanthan.sendmail(
				...
				content=render_template(self.auto_reply_message or "", communication.as_dict())
                        or netmanthan.get_template("templates/emails/auto_reply.html").render(communication.as_dict()),
	...
	def on_trash(self):
		...
		Communication = netmanthan.qb.DocType("Communication")
		netmanthan.qb.update(Communication).set(Communication.email_account, "").where(
	...
	def after_rename(self, old, new, merge=False):
		netmanthan.db.set_value("Email Account", new, "email_account_name", new)
	...
	def mark_emails_as_read_unread(self, email_server=None, folder_name="INBOX"):
		...
		EmailFlagQ = netmanthan.qb.DocType("Email Flag Queue")
		flags = (
			netmanthan.qb.from_(EmailFlagQ)
			.select(EmailFlagQ.name, EmailFlagQ.communication, EmailFlagQ.uid, EmailFlagQ.action)
			.where(EmailFlagQ.is_completed == 0)
			.where(EmailFlagQ.email_account == netmanthan.db.escape(self.name))
		).run(as_dict=True)
		...
		if flags and uid_list:
			...
			EmailFlagQueue = netmanthan.qb.DocType("Email Flag Queue")
			netmanthan.qb.update(EmailFlagQueue).set(EmailFlagQueue.is_completed, 1).where(
	...
	def set_communication_seen_status(self, docnames, seen=0):
		...
		if not docnames:
			return
		Communication = netmanthan.qb.from_("Communication")
		netmanthan.qb.update(Communication).set(Communication.seen == seen).where(
	...
	def check_automatic_linking_email_account(self):
		if self.enable_automatic_linking:
			if not self.enable_incoming:
				netmanthan.throw(_("Automatic Linking can be activated only if Incoming is enabled."))

			if netmanthan.db.exists(
				"Email Account", {"enable_automatic_linking": 1, "name": ("!=", self.name)}
			):
				netmanthan.throw(_("Automatic Linking can be activated only for one Email Account."))
	...
	def get_oauth_token(self):
		if self.auth_method == "OAuth":
			connected_app = netmanthan.get_doc("Connected App", self.connected_app)
...
def get_append_to(
	...
	for dt in netmanthan.get_all("DocType", filters=filters, fields=["name", "email_append_to"]):
		email_append_to_list.append(dt.name)

	# Set Email Append To DocTypes set via Customize Form
	for dt in netmanthan.get_list(
...
def notify_unreplied():
	...
	for email_account in netmanthan.get_all(
		...
		if append_to:
			...
			for comm in netmanthan.get_all(
				"Communication",
				"name",
				filters=[
				...
				comm = netmanthan.get_doc("Communication", comm.name)

				if netmanthan.db.get_value(comm.reference_doctype, comm.reference_name, "status") == "Open":
...
def pull(now=False):
	...
	from netmanthan.integrations.doctype.connected_app.connected_app import has_token

	if netmanthan.cache().get_value("workers:no-internet") == True:
		if test_internet():
			netmanthan.cache().set_value("workers:no-internet", False)
		return

	doctype = netmanthan.qb.DocType("Email Account")
	email_accounts = (
	...
	for email_account in email_accounts:
		...
		if now:
			...
			queued_jobs = get_jobs(site=netmanthan.local.site, key="job_name")[netmanthan.local.site]
...
def pull_from_email_account(email_account):
	...
	email_account = netmanthan.get_doc("Email Account", email_account)
...
def setup_user_email_inbox(
	...
	from netmanthan.core.doctype.user.user import ask_pass_update
	...
	user_names = netmanthan.db.get_values("User", {"email": email_id}, as_dict=True)
	if not user_names:
		return
	...
	if update_user_email_settings:
		UserEmail = netmanthan.qb.DocType("User Email")
		netmanthan.qb.update(UserEmail).set(UserEmail.awaiting_password, (awaiting_password or 0)).set(
		...
		users = " and ".join([netmanthan.bold(user.get("name")) for user in user_names])
		netmanthan.msgprint(_("Enabled email inbox for user {0}").format(users))
	ask_pass_update()
...
def remove_user_email_inbox(email_account):
	...
	for user in users:
		doc = netmanthan.get_doc("User", user.get("name"))
...
def set_email_password(email_account, password):
	account = netmanthan.get_doc("Email Account", email_account)
