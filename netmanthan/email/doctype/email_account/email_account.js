...
function oauth_access(frm) {
	netmanthan.model.with_doc("Connected App", frm.doc.connected_app, () => {
		const connected_app = netmanthan.get_doc("Connected App", frm.doc.connected_app);
...
function set_default_max_attachment_size(frm) {
	if (frm.doc.__islocal && !frm.doc["attachment_limit"]) {
		netmanthan.call({
			method: "netmanthan.core.api.file.get_max_file_size",
			callback: function (r) {
				if (!r.exc) {
...
netmanthan.ui.form.on("Email Account", {
	service: function (frm) {
		$.each(netmanthan.email_defaults[frm.doc.service], function (key, value) {
			frm.set_value(key, value);
		});
		if (!frm.doc.use_imap) {
			$.each(netmanthan.email_defaults_pop[frm.doc.service], function (key, value) {
				frm.set_value(key, value);
			});
		}
	},
	...
	onload: function (frm) {
		...
		frm.set_query(
			"append_to",
			"netmanthan.email.doctype.email_account.email_account.get_append_to"
		);
		frm.set_query("append_to", "imap_folder", function () {
			return {
				query: "netmanthan.email.doctype.email_account.email_account.get_append_to",
			};
		});
	...
	refresh: function (frm) {
		...
		if (netmanthan.route_flags.delete_user_from_locals && netmanthan.route_flags.linked_user) {
			delete netmanthan.route_flags.delete_user_from_locals;
			delete locals["User"][netmanthan.route_flags.linked_user];
		}
	},
	...
	show_oauth_authorization_message(frm) {
		if (frm.doc.auth_method === "OAuth" && frm.doc.connected_app) {
			netmanthan.call({
				method: "netmanthan.integrations.doctype.connected_app.connected_app.has_token",
				args: {
	...
	domain: netmanthan.utils.debounce((frm) => {
		if (frm.doc.domain) {
			netmanthan.call({
	...
	email_sync_option: function (frm) {
		...
		if (frm.doc.email_sync_option == "ALL") {
			...
			netmanthan.confirm(msg, null, function () {
	...
	warn_autoreply_on_incoming: function (frm) {
		if (frm.doc.enable_incoming && frm.doc.enable_auto_reply && frm.doc.__islocal) {
			...
			netmanthan.confirm(msg, null, function () {
				...
				netmanthan.show_alert({ message: __("Disabled Auto Reply"), indicator: "blue" });
			});
		}
	},
});
