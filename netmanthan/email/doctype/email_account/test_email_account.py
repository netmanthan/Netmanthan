...
from netmanthan.email.doctype.email_account.email_account import notify_unreplied
from netmanthan.email.email_body import get_message_id
from netmanthan.email.receive import Email, InboundMail, SentEmailInInboxError
...
class TestEmailAccount(netmanthanTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
		email_account = netmanthan.get_doc("Email Account", "_Test Email Account 1")
	...
	def setUp(self):
		...
		netmanthan.flags.sent_mail = None
		netmanthan.db.delete("Email Queue")
		netmanthan.db.delete("Unhandled Email")
	...
	def test_incoming(self):
		...
		comm = netmanthan.get_doc("Communication", {"sender": "test_sender@example.com"})
		self.assertTrue("test_receiver@example.com" in comm.recipients)
		# check if todo is created
		self.assertTrue(netmanthan.db.get_value(comm.reference_doctype, comm.reference_name, "name"))
	...
	def test_incoming_with_attach(self):
		...
		existing_file = netmanthan.get_doc({"doctype": "File", "file_name": "erpnext-conf-14.png"})
		netmanthan.delete_doc("File", existing_file.name)
	...
	def test_outgoing(self):
		...
		mail = email.message_from_string(netmanthan.get_last_doc("Email Queue").message)
	...
	def test_sendmail(self):
		...
		sent_mail = email.message_from_string(netmanthan.safe_decode(netmanthan.flags.sent_mail))
	...
	def test_print_format(self):
		...
		sent_mail = email.message_from_string(netmanthan.get_last_doc("Email Queue").message)
	...
	def test_threading(self):
		...
		sent_name = make(
			...
			doctype="ToDo",
			name=netmanthan.get_last_doc("ToDo").name,
			send_email=True,
		)["name"]
		...
		sent = netmanthan.get_doc("Communication", sent_name)
	...
	def test_threading_by_message_id(self):
		...
		event = netmanthan.get_doc(dict(doctype="Event", subject="test-message")).insert()
		...
		last_mail = netmanthan.get_doc("Email Queue", dict(reference_name=event.name))
	...
	def test_handle_bad_emails(self):
		...
		self.assertTrue(netmanthan.db.get_value("Unhandled Email", {"message_id": message_id}))
	...
	def test_append_to(self):
		...
		self.assertTrue(netmanthan.db.exists(communication.reference_doctype, communication.reference_name))
	...
	def test_append_to_with_imap_folders(self):
		...
		for mail in mails:
			...
			if mail.append_to == "ToDo":
				...
				self.assertTrue(communication.reference_name)
				self.assertTrue(
					netmanthan.db.exists(communication.reference_doctype, communication.reference_name)
				)
			else:
				test_folder_mails += 1
	...
	@patch("netmanthan.email.receive.EmailServer.select_imap_folder", return_value=True)
	@patch("netmanthan.email.receive.EmailServer.logout", side_effect=lambda: None)
	def mocked_get_inbound_mails(
		email_account, messages={}, mocked_logout=None, mocked_select_imap_folder=None
	):
		from netmanthan.email.receive import EmailServer
...
class TestInboundMail(netmanthanTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
		email_account = netmanthan.get_doc("Email Account", "_Test Email Account 1")
	...
	def test_find_parent_communication_from_header(self):
		...
		communication = self.new_communication()
		mail_content = self.get_test_mail(fname="reply-4.raw").replace(
			"{{ message_id }}", f"<{communication.name}@{netmanthan.local.site}>"
		)
...
def cleanup(sender=None):
	...
	names = netmanthan.get_list("Communication", filters=filters, fields=["name"])
	for name in names:
		netmanthan.delete_doc_if_exists("Communication", name.name)
		netmanthan.delete_doc_if_exists("Communication Link", {"parent": name.name})
