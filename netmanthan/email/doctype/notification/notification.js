...
netmanthan.notification = {
	setup_fieldname_select: function (frm) {
		...
		netmanthan.model.with_doctype(frm.doc.document_type, function () {
			...
			let fields = netmanthan.get_doc("DocType", frm.doc.document_type).fields;
			let options = $.map(fields, function (d) {
				return in_list(netmanthan.model.no_value_type, d.fieldtype)
					? null
					: get_select_options(d);
			});
			...
			if (frm.doc.channel === "Email") {
				receiver_fields = $.map(fields, function (d) {
					...
					if (d.fieldtype == "Table") {
						let child_fields = netmanthan.get_doc("DocType", d.options).fields;
...
netmanthan.ui.form.on("Notification", {
	onload: function (frm) {
	...
	refresh: function (frm) {
		netmanthan.notification.setup_fieldname_select(frm);
		netmanthan.notification.setup_example_message(frm);
	...
	view_properties: function (frm) {
		netmanthan.route_options = { doc_type: frm.doc.document_type };
	...
	event: function (frm) {
		if (in_list(["Days Before", "Days After"], frm.doc.event)) {
			frm.add_custom_button(__("Get Alerts for Today"), function () {
				netmanthan.call({
					method: "netmanthan.email.doctype.notification.notification.get_documents_for_today",
					args: {
						notification: frm.doc.name,
					},
					callback: function (r) {
						if (r.message && r.message.length > 0) {
							netmanthan.msgprint(r.message.toString());
						} else {
							netmanthan.msgprint(__("No alerts for today"));
						}
					},
				});
			});
