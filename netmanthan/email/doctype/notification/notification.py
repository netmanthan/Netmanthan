...
from netmanthan.core.doctype.role.role import get_info_based_on_role, get_user_info
from netmanthan.core.doctype.sms_settings.sms_settings import send_sms
from netmanthan.desk.doctype.notification_log.notification_log import enqueue_create_notification
from netmanthan.integrations.doctype.slack_webhook_url.slack_webhook_url import send_slack_message
from netmanthan.model.document import Document
from netmanthan.modules.utils import export_module_json, get_doc_module
from netmanthan.utils import add_to_date, cast, is_html, nowdate, validate_email_address
...
class Notification(Document):
	...
	def validate(self):
		...
		if self.event in ("Days Before", "Days After") and not self.date_changed:
			netmanthan.throw(_("Please specify which date field must be checked"))

		if self.event == "Value Change" and not self.value_changed:
			netmanthan.throw(_("Please specify which value field must be checked"))

		self.validate_forbidden_types()
		self.validate_condition()
		self.validate_standard()
		netmanthan.cache().hdel("notifications", self.document_type)
...
"""
	...
	def validate_standard(self):
		if self.is_standard and self.enabled and not netmanthan.conf.developer_mode:
			netmanthan.throw(
				_("Cannot edit Standard Notification. To edit, please disable this and duplicate it")
			)

	def validate_condition(self):
		temp_doc = netmanthan.new_doc(self.document_type)
		if self.condition:
			try:
				netmanthan.safe_eval(self.condition, None, get_context(temp_doc.as_dict()))
			except Exception:
				netmanthan.throw(_("The Condition '{0}' is invalid").format(self.condition))

	def validate_forbidden_types(self):
		forbidden_document_types = ("Email Queue",)
		if self.document_type in forbidden_document_types or netmanthan.get_meta(self.document_type).istable:
			# currently notifications don't work on child tables as events are not fired for each record of child table

			netmanthan.throw(_("Cannot set Notification on Document Type {0}").format(self.document_type))
	...
	def get_documents_for_today(self):
		...
		for d in doc_list:
			doc = netmanthan.get_doc(self.document_type, d.name)

			if self.condition and not netmanthan.safe_eval(self.condition, None, get_context(doc)):
				continue
		...
		if self.set_property_after_alert:
			...
			try:
				if allow_update and not doc.flags.in_notification_update:
					...
					if doc.meta.get_field(fieldname).fieldtype in netmanthan.model.numeric_fieldtypes:
						value = netmanthan.utils.cint(value)
	...
	def create_system_notification(self, doc, context):
		...
		if "{" in subject:
			subject = netmanthan.render_template(self.subject, context)
		...
		notification_doc = {
			...
			"email_content": netmanthan.render_template(self.message, context),
	...
	def send_an_email(self, doc, context):
		...
		from netmanthan.core.doctype.communication.email import _make as make_communication
		...
		sender = None
		message = netmanthan.render_template(self.message, context)
	...
	def send_sms(self, doc, context):
		send_sms(
			...
			msg=netmanthan.render_template(self.message, context),
		)
	...
	def get_list_of_recipients(self, doc, context):
		...
		for recipient in self.recipients:
			if recipient.condition:
				if not netmanthan.safe_eval(recipient.condition, None, context):
					continue
	...
	def get_attachment(self, doc):
		...
		print_settings = netmanthan.get_doc("Print Settings", "Print Settings")
			...
			return [
				{
					...
					"lang": netmanthan.db.get_value("Print Format", self.print_format, "default_print_language")
	...
	def get_template(self):
		...
		def load_template(extn):
			template = ""
			template_path = os.path.join(os.path.dirname(module.__file__), netmanthan.scrub(self.name) + extn)
	...
	def load_standard_properties(self, context):
		...
		if not is_html(self.message):
			self.message = netmanthan.utils.md_to_html(self.message)
...
def get_documents_for_today(notification):
	notification = netmanthan.get_doc("Notification", notification)
...
def trigger_notifications(doc, method=None):
	if netmanthan.flags.in_import or netmanthan.flags.in_patch:
	...
	if method == "daily":
		...
		for d in doc_list:
			alert = netmanthan.get_doc("Notification", d.name)
...
def evaluate_alert(doc: Document, alert, event):
	...
	try:
		if isinstance(alert, str):
			alert = netmanthan.get_doc("Notification", alert)

		context = get_context(doc)

		if alert.condition:
			if not netmanthan.safe_eval(alert.condition, None, context):
				return

		if event == "Value Change" and not doc.is_new():
			if not netmanthan.db.has_column(doc.doctype, alert.value_changed):
		...
		message = _("Error while evaluating Notification {0}. Please fix your template.").format(
			netmanthan.utils.get_link_to_form("Notification", alert.name)
		)
		netmanthan.throw(message, title=_("Error in Notification"))
	except Exception as e:
		title = str(e)
		message = netmanthan.get_traceback()
		netmanthan.log_error(message=message, title=title)

		msg = f"<details><summary>{title}</summary>{message}</details>"
		netmanthan.throw(msg, title=_("Error in Notification"))
...
def get_context(doc):
	return {
		"doc": doc,
		"nowdate": nowdate,
		"netmanthan": netmanthan._dict(utils=get_safe_globals().get("netmanthan").get("utils")),
	}


def get_assignees(doc):
	assignees = []
	assignees = netmanthan.get_all(
		"ToDo",
...
def get_emails_from_template(template, context):
	...
	emails = netmanthan.render_template(template, context) if "{" in template else template
