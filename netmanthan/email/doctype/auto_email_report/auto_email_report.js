...
netmanthan.ui.form.on("Auto Email Report", {
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			frm.add_custom_button(__("Download"), function () {
				var w = window.open(
					netmanthan.urllib.get_full_url(
						"/api/method/netmanthan.email.doctype.auto_email_report.auto_email_report.download?" +
							"name=" +
			...
			frm.add_custom_button(__("Send Now"), function () {
				netmanthan.call({
					method: "netmanthan.email.doctype.auto_email_report.auto_email_report.send_now",
					args: { name: frm.doc.name },
					callback: function () {
						netmanthan.msgprint(__("Scheduled to send"));
					},
				});
			});
		} else {
			if (!frm.doc.user) {
				frm.set_value("user", netmanthan.session.user);
			}
			if (!frm.doc.email_to) {
				frm.set_value("email_to", netmanthan.session.user);
			}
		}
	},
	report: function (frm) {
	...
	fetch_report_filters(frm) {
		if (
			...
			netmanthan.call({
				method: "netmanthan.desk.query_report.get_script",
				args: {
					report_name: frm.doc.report,
				},
				callback: function (r) {
					netmanthan.dom.eval(r.message.script || "");
					frm.script_setup_for = frm.doc.report;
	...
	show_filters: async function (frm) {
		...
		wrapper.empty();
		let reference_report = netmanthan.query_reports[frm.doc.report];
		if (!reference_report || !reference_report.filters) {
			reference_report = await netmanthan.model.with_doc("Report", frm.doc.report);
		}
		if (
			...
			if (
				...
				if (frm.doc.filters) {
					...
					netmanthan.db.get_value("Report", frm.doc.report, "json", (r) => {
						if (r && r.json) {
				...
				report_filters = netmanthan.query_reports[frm.doc.reference_report].filters;
			} else {
				filters = JSON.parse(frm.doc.filters || "{}");
			...
			report_filters.forEach((f) => {
				...
				$("<td>" + netmanthan.format(filters[f.fieldname], f) + "</td>")
					.css(css)
					.appendTo(row);
			});
