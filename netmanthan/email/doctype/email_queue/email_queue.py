...
import netmanthan
from netmanthan import _, safe_encode, task
from netmanthan.core.utils import html2text
from netmanthan.email.doctype.email_account.email_account import EmailAccount
from netmanthan.email.email_body import add_attachment, get_email, get_formatted_html
from netmanthan.email.queue import get_unsubcribed_url, get_unsubscribe_message
from netmanthan.email.smtp import SMTPServer
from netmanthan.model.document import Document
from netmanthan.query_builder import DocType, Interval
...
class EmailQueue(Document):
	...
	def prevent_email_queue_delete(self):
		if netmanthan.session.user != "Administrator":
			netmanthan.throw(_("Only Administrator can delete Email Queue"))

	def get_duplicate(self, recipients):
		values = self.as_dict()
		del values["name"]
		duplicate = netmanthan.get_doc(values)
	...
	def new(cls, doc_data, ignore_permissions=False):
		...
		doc = netmanthan.new_doc(cls.DOCTYPE)
		doc.update(data)
	...
	def update_status(self, status, commit=False, **kwargs):
		...
		if self.communication:
			communication_doc = netmanthan.get_doc("Communication", self.communication)
	...
	def get_email_account(self):
		if self.email_account:
			return netmanthan.get_cached_doc("Email Account", self.email_account)
	...
	def can_send_now(self):
		if (
			netmanthan.are_emails_muted()
			or not self.is_to_be_sent()
			or cint(netmanthan.db.get_default("suspend_email_queue")) == 1
		):
			return False
	...
	def send(self, is_background_task: bool = False, smtp_server_instance: SMTPServer = None):
		...
		with SendMailContext(self, is_background_task, smtp_server_instance) as ctx:
			...
			if netmanthan.flags.in_test:
				netmanthan.flags.sent_mail = message
				return
	...
	def clear_old_logs(days=30):
		...
		"""
		days = days or 31
		email_queue = netmanthan.qb.DocType("Email Queue")
		email_recipient = netmanthan.qb.DocType("Email Queue Recipient")

		# Delete queue table
		(
			netmanthan.qb.from_(email_queue)
			.delete()
		...
		(
			netmanthan.qb.from_(email_recipient)
			.delete()
...
class SendMailContext:
	...
	def get_tracker_str(self) -> str:
		if netmanthan.conf.use_ssl and self.email_account_doc.track_email_status:
			tracker_url_html = f'<img src="{get_url()}/api/method/netmanthan.core.doctype.communication.email.mark_email_as_seen?name={self.queue_doc.communication}"/>'
	...
	def include_attachments(self, message):
		...
		for attachment in attachments:
			...
			if file_filters:
				_file = netmanthan.get_doc("File", file_filters)
				...
				print_format_file = netmanthan.attach_print(**attachment)
...
def retry_sending(name):
	doc = netmanthan.get_doc("Email Queue", name)
...
def toggle_sending(enable):
	...
	netmanthan.db.set_default("suspend_email_queue", 0 if sbool(enable) else 1)
...
def on_doctype_update():
	...
	netmanthan.db.add_index("Email Queue", ["message_id(140)"])


def get_email_retry_limit():
	return cint(netmanthan.db.get_system_setting("email_retry_limit")) or 3
...
class QueueBuilder:
	...
	def __init__(
		...
		:param unsubscribe_method: URL method for unsubscribe. Default is `/api/method/netmanthan.email.queue.unsubscribe`.
	...
	def unsubscribe_method(self):
		return self._unsubscribe_method or "/api/method/netmanthan.email.queue.unsubscribe"
	...
	def get_unsubscribed_user_emails(self):
		...
		if len(all_ids) > 0:
			unsubscribed = (
				netmanthan.qb.from_(EmailUnsubscribe)
			...
			for att in self._attachments:
				if att.get("fid") or att.get("file_url"):
					...
					if not att.get("lang", None):
						att["lang"] = netmanthan.local.lang
	...
	def process(self, send_now=False):
		...
		if not queue_separately:
			...
			if send_now and len(final_recipients) >= 1000:
				...
				send_now = False
			for recipients in netmanthan.utils.create_batch(final_recipients, 1000):
				netmanthan.enqueue(
					self.send_emails,
					queue_data=queue_data,
					final_recipients=recipients,
					job_name=netmanthan.utils.get_job_name(
						"send_bulk_emails_for", self.reference_doctype, self.reference_name
					),
					now=netmanthan.flags.in_test or send_now,
					queue="long",
				)
	...
	def as_dict(self, include_recipients=True):
		...
		except netmanthan.InvalidEmailAddressError:
