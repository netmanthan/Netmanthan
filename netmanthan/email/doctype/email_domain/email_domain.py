...
from netmanthan.email.receive import Timed_IMAP4, Timed_IMAP4_SSL, Timed_POP3, Timed_POP3_SSL
...
class EmailDomain(Document):
	def validate(self):
		...
		if netmanthan.local.flags.in_patch or netmanthan.local.flags.in_test or netmanthan.local.flags.in_install:
			return
	...
	def on_update(self):
		...
		for email_account in netmanthan.get_all("Email Account", filters={"domain": self.name}):
			try:
