...
import netmanthan
from netmanthan import _, safe_decode, safe_encode
from netmanthan.core.doctype.file import MaxFileSizeReachedError, get_random_filename
from netmanthan.email.oauth import Oauth
...
from netmanthan.utils.user import is_system_user
...
class EmailSizeExceededError(netmanthan.ValidationError):
	pass


class EmailTimeoutError(netmanthan.ValidationError):
	pass


class TotalSizeExceededError(netmanthan.ValidationError):
	pass


class LoginLimitExceeded(netmanthan.ValidationError):
	pass
...
class EmailServer:
	...
	def setup(self, args=None):
		# overrride
		self.settings = args or netmanthan._dict()
		...
		try:
			if cint(self.settings.use_ssl):
				self.imap = Timed_IMAP4_SSL(
					self.settings.host, self.settings.incoming_port, timeout=netmanthan.conf.get("pop_timeout")
				)
			else:
			...
			netmanthan.msgprint(_("Invalid Mail Server. Please rectify and try again."))
			raise
	...
	def connect_pop(self):
		...
		try:
			...
			netmanthan.log_error("POP: Unable to connect")
			...
			if self.is_temporary_system_problem(e):
				...
				netmanthan.msgprint(_("Invalid User Name or Support Password. Please rectify and try again."))
				raise
	...
	def get_messages(self, folder="INBOX"):
		...
		try:
			...
			# size limits
			self.total_size = 0
			self.max_email_size = cint(netmanthan.local.conf.get("max_email_size"))
	...
	def check_imap_uidvalidity(self, folder):
		...
		netmanthan.db.set_value("Email Account", self.settings.email_account, "uidnext", uidnext)
		...
		if not uid_validity or uid_validity != current_uid_validity:
			...
			netmanthan.qb.update(Communication).set(Communication.uid, -1).where(
			...
			if self.settings.use_imap:
				...
				IMAPFolder = netmanthan.qb.DocType("IMAP Folder")
				netmanthan.qb.update(IMAPFolder).set(IMAPFolder.uidvalidity, current_uid_validity).set(
					IMAPFolder.uidnext, uidnext
				).where(IMAPFolder.parent == self.settings.email_account_name).where(
					IMAPFolder.folder_name == folder
				).run()
			else:
				EmailAccount = netmanthan.qb.DocType("Email Account")
				netmanthan.qb.update(EmailAccount).set(EmailAccount.uidvalidity, current_uid_validity).set(
	...
	def retrieve_message(self, message_meta, msg_num=None):
		...
		try:
			...
			if self.has_login_limit_exceeded(e):
				...
				# log performs rollback and logs error in Error Log
				netmanthan.log_error("Unable to fetch email", self.make_error_msg(msg_num, incoming_mail))
				self.errors = True
	...
	def get_email_seen_status(self, uid, flag_string):
		...
		for flag in imaplib.ParseFlags(flag_string) or []:
			match = WORDS_PATTERN.search(netmanthan.as_unicode(flag))
...
class Email:
	...
	def set_from(self):
		...
		if _reply_to and not netmanthan.db.get_value("Email Account", {"email_id": _reply_to}, "email_id"):
	...
	def decode_email(self, email):
		...
		for part, encoding in decode_header(
			netmanthan.as_unicode(email).replace('"', " ").replace("'", " ")
		):
			if encoding:
	...
	def save_attachments_in_doc(self, doc):
		...
		for attachment in self.attachments:
			try:
				...
				pass
			except netmanthan.FileAlreadyAttachedException:
				pass
...
class InboundMail(Email):
	...
	def _build_communication_doc(self):
		...
		communication = netmanthan.get_doc(data)
	...
	def is_exist_in_system(self):
		...
		from netmanthan.core.doctype.communication.communication import Communication
	...
	def is_reply_to_system_sent_mail(self):
		...
		return self.is_reply() and netmanthan.local.site in self.in_reply_to
	...
	def match_record_by_subject_and_sender(self, doctype):
		...
		3. Find by matching subject alone (Special case)
		        ...
		        the system user is replying to via the common email account in netmanthan. This fix bypasses
		...
		if not record:
			...
			name = netmanthan.db.get_value(self.email_account.append_to, filters=filters)
			record = self.get_doc(doctype, name, ignore_error=True) if name else None
		return record

	def _create_reference_document(self, doctype):
		"""Create reference document if it does not exist in the system."""
		parent = netmanthan.new_doc(doctype)
		email_fileds = self.get_email_fields(doctype)

		if email_fileds.subject_field:
			parent.set(email_fileds.subject_field, netmanthan.as_unicode(self.subject)[:140])

		if email_fileds.sender_field:
			parent.set(email_fileds.sender_field, netmanthan.as_unicode(self.from_email))
		...
		try:
			...
			parent_name = netmanthan.db.get_value(
	...
	def get_doc(doctype, docname, ignore_error=False):
		try:
			return netmanthan.get_doc(doctype, docname)
	...
	def clean_subject(subject):
		...
		return netmanthan.as_unicode(strip(re.sub(regex, "", subject, 0, flags=re.IGNORECASE)))

	@staticmethod
	def get_email_fields(doctype):
		"""Returns Email related fields of a doctype."""
		fields = netmanthan._dict()
	...
	def get_document(self, doctype, name):
		"""Is same as netmanthan.get_doc but suppresses the DoesNotExist error."""
		try:
			return netmanthan.get_doc(doctype, name)
		except netmanthan.DoesNotExistError:
			return None
