...
class InvalidEmailCredentials(netmanthan.ValidationError):
	pass
...
def send(email, append_to=None, retry=1):
	...
	def _send(retry):
		...
		try:
			...
			netmanthan.throw(_("Invalid login or password"), title="Email Failed")
			raise
		except smtplib.SMTPRecipientsRefused:
			netmanthan.msgprint(_("Invalid recipient address"), title="Email Failed")
			raise
...
class SMTPServer:
	def __init__(
		...
		if not self.server:
			netmanthan.msgprint(
				...
				raise_exception=netmanthan.OutgoingEmailError,
			)
	...
	def session(self):
		...
		try:
			...
			if not _session:
				netmanthan.msgprint(
					_("Could not connect to outgoing email server"), raise_exception=netmanthan.OutgoingEmailError
				)
			...
			if self.use_oauth:
				...
				if res[0] != 235:
					netmanthan.msgprint(res[1], raise_exception=netmanthan.OutgoingEmailError)
			...
			netmanthan.throw(_("Invalid Outgoing Mail Server or Port"), title=_("Incorrect Configuration"))
