import netmanthan
from netmanthan.core.doctype.log_settings.log_settings import clear_log_table
from netmanthan.utils import add_to_date, today
...
def execute():
	...
	ref: https://github.com/netmanthan/netmanthan/issues/16971
	"""
...
def is_log_cleanup_stuck(doctype: str, retention: int) -> bool:
	...
	return bool(netmanthan.db.exists(doctype, {"modified": ("<", threshold)}))


def get_current_setting(fieldname):
	try:
		return netmanthan.db.get_single_value("Log Settings", fieldname)
	except Exception:
