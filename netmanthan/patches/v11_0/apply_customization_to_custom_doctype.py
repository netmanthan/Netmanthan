...
def execute():
	custom_doctypes = netmanthan.get_all("DocType", filters={"custom": 1})

	for doctype in custom_doctypes:
		property_setters = netmanthan.get_all(
			"Property Setter",
			filters={"doc_type": doctype.name, "doctype_or_field": "DocField"},
			fields=["name", "property", "value", "property_type", "field_name"],
		)

		custom_fields = netmanthan.get_all("Custom Field", filters={"dt": doctype.name}, fields=["*"])

		property_setter_map = {}

		for prop in property_setters:
			property_setter_map[prop.field_name] = prop
			netmanthan.db.delete("Property Setter", {"name": prop.name})

		meta = netmanthan.get_meta(doctype.name)
		...
		for cf in custom_fields:
			...
			if field:
				...
				df = netmanthan.new_doc("DocField", meta, "fields")
				df.update(cf)
				meta.fields.append(df)
			netmanthan.db.delete("Custom Field", {"name": cf.name})
