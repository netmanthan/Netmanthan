...
def execute():
	...
	reports = netmanthan.get_all("Report", {"report_type": "Report Builder"})

	for report_name in reports:
		settings = netmanthan.db.get_value("Report", report_name, "json")
		if not settings:
			continue

		settings = netmanthan._dict(json.loads(settings))
		...
		netmanthan.db.set_value("Report", report_name, "json", json.dumps(settings))
