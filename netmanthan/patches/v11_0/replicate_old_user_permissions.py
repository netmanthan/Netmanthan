...
import netmanthan
from netmanthan.permissions import get_valid_perms
from netmanthan.utils import cint


def execute():
	netmanthan.reload_doctype("User Permission")
	user_permissions = netmanthan.get_all("User Permission", fields=["allow", "name", "user"])
	...
	for key, doctype_to_skip in doctype_to_skip_map.items():
		if not doctype_to_skip:
			continue
		if not netmanthan.db.has_column("User Permission", "applicable_for") and netmanthan.db.has_column(
...
def get_linked_doctypes(doctype):
	from netmanthan.permissions import get_linked_doctypes

	linked_doctypes = get_linked_doctypes(doctype)
	child_doctypes = [d.options for d in netmanthan.get_meta(doctype).get_table_fields()]
