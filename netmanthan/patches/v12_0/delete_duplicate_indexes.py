...
def execute():
	...
	all_tables = netmanthan.db.get_tables()
	final_deletion_map = netmanthan._dict()

	for table in all_tables:
		indexes_to_keep_map = netmanthan._dict()
		indexes_to_delete = []
		index_info = netmanthan.db.sql(
		...
		for index in index_list:
			try:
				if is_clustered_index(table_name, index):
					continue
				netmanthan.db.sql_ddl(f"ALTER TABLE `{table_name}` DROP INDEX `{index}`")
