...
def execute():
	if not netmanthan.db.table_exists("List View Setting"):
		return
	if not netmanthan.db.exists("DocType", "List View Setting"):
		return

	netmanthan.reload_doc("desk", "doctype", "List View Settings")

	existing_list_view_settings = netmanthan.get_all(
		"List View Settings", as_list=True, order_by="modified"
	)
	for list_view_setting in netmanthan.get_all(
		"List View Setting",
