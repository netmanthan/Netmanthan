...
def execute():
	...
	"""
	netmanthan.reload_doctype("DocType Link")
	if netmanthan.db.has_table("Custom Link"):
		for custom_link in netmanthan.get_all("Custom Link", ["name", "document_type"]):
			netmanthan.db.sql(
				"update `tabDocType Link` set custom=1, parent=%s where parent=%s",
				(custom_link.document_type, custom_link.name),
			)

		netmanthan.delete_doc("DocType", "Custom Link")
