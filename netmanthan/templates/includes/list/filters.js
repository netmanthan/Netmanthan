function setup_list_filters() {
	...
	netmanthan.call('netmanthan.website.doctype.web_form.web_form.get_form_data', {
	...
	.then((r) => {
		...
		web_form_filters = web_form.web_form_fields.filter(df => {
			...
			const value = netmanthan.utils.get_query_params()[df.fieldname];
			const f = netmanthan.ui.form.make_control({
				df: {
					...
					is_filter: true,
					change: (e) => {
						const query_params = Object.assign(netmanthan.utils.get_query_params(), {
							[df.fieldname]: f.get_value()
						})
						const query_string = netmanthan.utils.make_query_string(query_params);
...
netmanthan.ready(setup_list_filters);
