...
from netmanthan.website.doctype.blog_settings.blog_settings import get_comment_limit
...
def add_comment(comment, comment_email, comment_by, reference_doctype, reference_name, route):
	if netmanthan.session.user == "Guest":
		...
		if reference_doctype == "Blog Post" and not netmanthan.db.get_single_value(
			"Blog Settings", "allow_guest_to_comment"
		):
			return

		if netmanthan.db.exists("User", comment_email):
			netmanthan.throw(_("Please login to post a comment."))

	if not comment.strip():
		netmanthan.msgprint(_("The comment cannot be empty"))
		return False

	if URLS_COMMENT_PATTERN.search(comment) or EMAIL_PATTERN.search(comment):
		netmanthan.msgprint(_("Comments cannot have links or email addresses"))
		return False

	doc = netmanthan.get_doc(reference_doctype, reference_name)
	...
	if doc.get("route"):
		url = f"{netmanthan.utils.get_request_site_address()}/{doc.route}#{comment.name}"
	else:
		url = f"{netmanthan.utils.get_request_site_address()}/app/{scrub(doc.doctype)}/{doc.name}#comment-{comment.name}"
	...
	if doc.doctype != "Blog Post" or doc.enable_email_notification:
		# notify creator
		creator_email = netmanthan.db.get_value("User", doc.owner, "email") or doc.owner
	...
	template = netmanthan.get_template("templates/includes/comments/comment.html")
