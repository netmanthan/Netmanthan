...
from netmanthan.website.doctype.blog_settings.blog_settings import get_like_limit
...
def like(reference_doctype, reference_name, like, route=""):
	like = netmanthan.parse_json(like)
	ref_doc = netmanthan.get_doc(reference_doctype, reference_name)
	...
	if like and ref_doc.enable_email_notification:
		...
		message = message + "<p><a href='{}/{}#likes' style='font-size: 80%'>{}</a></p>".format(
			netmanthan.utils.get_request_site_address(), ref_doc.route, _("View Blog Post")
		)

		# notify creator
		netmanthan.sendmail(
			recipients=netmanthan.db.get_value("User", ref_doc.owner, "email") or ref_doc.owner,
			subject=subject,
			message=message,
...
def add_like(reference_doctype, reference_name):
	...
	like = netmanthan.new_doc("Comment")
	...
	if user == "Guest":
		like.ip_address = netmanthan.local.request_ip
...
def delete_like(reference_doctype, reference_name):
	...
	netmanthan.db.delete("Comment", filters)
	return False
