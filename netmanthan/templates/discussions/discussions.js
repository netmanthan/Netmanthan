...
const setup_socket_io = () => {
	netmanthan.socketio.init(window.socketio_port || "9000");
	netmanthan.socketio.socket.on("publish_message", (data) => {
		publish_message(data);
	});
	netmanthan.socketio.socket.on("update_message", (data) => {
		update_message(data);
	});
	netmanthan.socketio.socket.on("delete_message", (data) => {
		delete_message(data);
	});
};
...
const publish_message = (data) => {
	...
	if ($(`.discussion-on-page[data-topic=${topic.name}]`).length) {
		...
		if (topic.owner == netmanthan.session.user) {
	...
	} else if (topic.owner == netmanthan.session.user && document_match_found) {
...
const submit_discussion = (e) => {
	...
	if (reply) {
		...
		netmanthan.call({
			method: "netmanthan.website.doctype.discussion_topic.discussion_topic.submit_discussion",
			args: {
...
const get_color_from_palette = (element) => {
	const palette = netmanthan.get_palette(element.attr("title"));
...
const perform_action = (e) => {
	...
	if (action === "edit") {
		...
		netmanthan.call({
			method: "netmanthan.website.doctype.discussion_reply.discussion_reply.delete_message",
			args: {
...
const hide_actions_on_conditions = (template, owner) => {
	...
	netmanthan.session.user != owner && $template.find(".dropdown").addClass("hide");
	return $template.prop("outerHTML");
};
