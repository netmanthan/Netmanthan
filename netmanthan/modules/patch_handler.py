...
def run_all(skip_failing: bool = False, patch_type: PatchType | None = None) -> None:
	...
	executed = set(netmanthan.get_all("Patch Log", fields="patch", pluck="patch"))

	netmanthan.flags.final_patches = []
	...
	for patch in netmanthan.flags.final_patches:
...
def get_all_patches(patch_type: PatchType | None = None) -> list[str]:
	...
	if patch_type and not isinstance(patch_type, PatchType):
		netmanthan.throw(f"Unsupported patch type specified: {patch_type}")
...
def get_patches_from_app(app: str, patch_type: PatchType | None = None) -> list[str]:
	...
	patches_txt = netmanthan.get_pymodule_path(app, "patches.txt")
	...
	try:
		...
		if patch_type.value in parser.sections():
			...
			netmanthan.throw(netmanthan._("Patch type {} not found in patches.txt").format(patch_type))
		...
		if not patch_type or patch_type == PatchType.pre_model_sync:
			return netmanthan.get_file_items(patches_txt)
...
def reload_doc(args):
	...
	run_single(method=netmanthan.modules.reload_doc, methodargs=args)
...
def execute_patch(patchmodule: str, method=None, methodargs=None):
	...
	if patchmodule.startswith("execute:"):
		...
		_patch = netmanthan.get_attr(patch)
	...
	print(
		f"Executing {patchmodule or methodargs} in {netmanthan.local.site} ({netmanthan.db.cur_db_name}){docstring}"
	)
	...
	try:
		if patchmodule:
			if patchmodule.startswith("finally:"):
				...
				netmanthan.flags.final_patches.append(patchmodule)
			else:
				if has_patch_file:
					_patch()
				else:
...
def update_patch_log(patchmodule):
	...
	netmanthan.get_doc({"doctype": "Patch Log", "patch": patchmodule}).insert(ignore_permissions=True)
...
def executed(patchmodule):
	...
	return netmanthan.db.get_value("Patch Log", {"patch": patchmodule})


def _patch_mode(enable):
	"""stop/start execution till patch is run"""
	netmanthan.local.flags.in_patch = enable
	netmanthan.db.commit()


def check_session_stopped():
	"""This function is deprecated. Use maintenance_mode in site config instead."""
	if netmanthan.db.get_global("__session_status") == "stop":
		netmanthan.msgprint(netmanthan.db.get_global("__session_status_message"))
