...
def export_module_json(doc, is_standard, module):
	...
	if not netmanthan.flags.in_import and getattr(netmanthan.get_conf(), "developer_mode", 0) and is_standard:
		...
		path = os.path.join(
			netmanthan.get_module_path(module), scrub(doc.doctype), scrub(doc.name), scrub(doc.name)
		)
...
def get_doc_module(module, doctype, name):
	...
	module_name = "{app}.{module}.{doctype}.{name}.{name}".format(
		app=netmanthan.local.module_app[scrub(module)],
		doctype=scrub(doctype),
		module=scrub(module),
		name=scrub(name),
	)
	return netmanthan.get_module(module_name)
...
def export_customizations(module, doctype, sync_on_migrate=0, with_permissions=0):
	...
	if not netmanthan.get_conf().developer_mode:
	...
	def add(_doctype):
		custom["custom_fields"] += netmanthan.get_all("Custom Field", fields="*", filters={"dt": _doctype})
		custom["property_setters"] += netmanthan.get_all(
			"Property Setter", fields="*", filters={"doc_type": _doctype}
		)
		custom["links"] += netmanthan.get_all("DocType Link", fields="*", filters={"parent": _doctype})

	add(doctype)

	if with_permissions:
		custom["custom_perms"] = netmanthan.get_all(
			"Custom DocPerm", fields="*", filters={"parent": doctype}
		)

	# also update the custom fields and property setters for all child tables
	for d in netmanthan.get_meta(doctype).get_table_fields():
	...
	if custom["custom_fields"] or custom["property_setters"] or custom["custom_perms"]:
		...
		with open(path, "w") as f:
			f.write(netmanthan.as_json(custom))

		netmanthan.msgprint(_("Customizations for <b>{0}</b> exported to:<br>{1}").format(doctype, path))
...
def sync_customizations(app=None):
	...
	for app_name in apps:
		for module_name in netmanthan.local.app_modules.get(app_name) or []:
			folder = netmanthan.get_app_path(app_name, module_name, "custom")
...
def sync_customizations_for_doctype(data, folder, filename: str = ""):
	...
	def sync(key, custom_doctype, doctype_fieldname):
		...
		def sync_single_doctype(doc_type):
			def _insert(data):
				if data.get(doctype_fieldname) == doc_type:
					...
					doc = netmanthan.get_doc(data)
					doc.db_insert()

			if custom_doctype != "Custom Field":
				netmanthan.db.delete(custom_doctype, {doctype_fieldname: doc_type})
				...
				for d in data[key]:
					field = netmanthan.db.get_value("Custom Field", {"dt": doc_type, "fieldname": d["fieldname"]})
					if not field:
		...
		for doc_type in doctypes:
			...
			if doc_type == doctype or not os.path.exists(
				os.path.join(folder, netmanthan.scrub(doc_type) + ".json")
			):
	...
	if update_schema and not netmanthan.db.get_value("DocType", doctype, "issingle"):
		netmanthan.db.updatedb(doctype)


def scrub(txt):
	return netmanthan.scrub(txt)
...
def get_module_path(module):
	...
	return netmanthan.get_module_path(module)
...
def reload_doc(module, dt=None, dn=None, force=False, reset_permissions=False):
	from netmanthan.modules.import_file import import_files
...
def export_doc(doctype, name, module=None):
	...
	from netmanthan.modules.export_file import write_document_file

	print(doctype, name)

	if not module:
		module = netmanthan.db.get_value("DocType", name, "module")
	write_document_file(netmanthan.get_doc(doctype, name), module)
...
def get_doctype_module(doctype) -> str:
	...
	def make_modules_dict():
		return dict(netmanthan.db.sql("select name, module from tabDocType"))

	doctype_module_map = netmanthan.cache().get_value("doctype_modules", make_modules_dict)

	if module_name := doctype_module_map.get(doctype):
		return module_name
	else:
		netmanthan.throw(_("DocType {} not found").format(doctype), exc=netmanthan.DoesNotExistError)
...
def load_doctype_module(doctype, module=None, prefix="", suffix=""):
	...
	try:
		if key not in doctype_python_modules:
			doctype_python_modules[key] = netmanthan.get_module(module_name)
...
def get_module_app(module: str) -> str:
	app = netmanthan.local.module_app.get(scrub(module))
	if app is None:
		netmanthan.throw(_("Module {} not found").format(module), exc=netmanthan.DoesNotExistError)
	return app
...
def get_app_publisher(module: str) -> str:
	app = get_module_app(module)
	if not app:
		netmanthan.throw(_("App not found"))
	app_publisher = netmanthan.get_hooks(hook="app_publisher", app_name=app)[0]
...
def make_boilerplate(template, doc, opts=None):
	...
	if not os.path.exists(target_file_path):
		...
		base_class = "Document"
		base_class_import = "from netmanthan.model.document import Document"
		if doc.get("is_tree"):
			base_class = "NestedSet"
			base_class_import = "from netmanthan.utils.nestedset import NestedSet"
	...
	def get_stats(args):
		...
		with open(target_file_path, "w") as target:
			with open(
				...
				target.write(
					netmanthan.as_unicode(
						netmanthan.utils.cstr(source.read()).format(
							app_publisher=app_publisher,
							year=netmanthan.utils.nowdate()[:4],
