netmanthan.pages["print"].on_page_load = function (wrapper) {
	netmanthan.ui.make_app_page({
		parent: wrapper,
	});

	let print_view = new netmanthan.ui.form.PrintView(wrapper);

	$(wrapper).bind("show", () => {
		const route = netmanthan.get_route();
		const doctype = route[1];
		const docname = route.slice(2).join("/");
		if (!netmanthan.route_options || !netmanthan.route_options.frm) {
			netmanthan.model.with_doc(doctype, docname, () => {
				let frm = { doctype: doctype, docname: docname };
				frm.doc = netmanthan.get_doc(doctype, docname);
				netmanthan.model.with_doctype(doctype, () => {
					frm.meta = netmanthan.get_meta(route[1]);
					print_view.show(frm);
				});
			});
		} else {
			print_view.frm = netmanthan.route_options.frm.doctype
				? netmanthan.route_options.frm
				: netmanthan.route_options.frm.frm;
			netmanthan.route_options.frm = null;
			print_view.show(print_view.frm);
		}
	});
};

netmanthan.ui.form.PrintView = class {
	constructor(wrapper) {
	...
	make() {
		this.print_wrapper = this.page.main.empty().html(
			`<div class="print-preview-wrapper"><div class="print-preview">
				${netmanthan.render_template("print_skeleton_loading")}
		...
		this.print_settings = netmanthan.model.get_doc(":Print Settings", "Print Settings");
		this.setup_menu();
		this.setup_toolbar();
		this.setup_sidebar();
	...
	add_sidebar_item(df, is_dynamic) {
		...
		let field = netmanthan.ui.form.make_control({
			df: df,
	...
	setup_menu() {
		...
		if (netmanthan.model.can_create("Print Format")) {
	...
	set_breadcrumbs() {
		netmanthan.breadcrumbs.add(this.frm.meta.module, this.frm.doctype);
	}
	...
	setup_additional_settings() {
		...
		netmanthan
			.xcall("netmanthan.printing.page.print.print.get_print_settings_to_show", {
	...
	edit_print_format() {
		...
		if (is_standard_but_editable) {
			netmanthan.set_route("Form", "Print Format", print_format.name);
			return;
		}
		if (is_custom_format) {
			if (print_format.print_format_builder_beta) {
				netmanthan.set_route("print-format-builder-beta", print_format.name);
			} else {
				netmanthan.set_route("print-format-builder", print_format.name);
			}
			return;
		}
		...
		netmanthan.prompt(
			...
			(data) => {
				...
				netmanthan.set_route("print-format-builder");
	...
	setup_keyboard_shortcuts() {
		this.wrapper.find(".print-toolbar a.btn-default").each((i, el) => {
			netmanthan.ui.keys.get_shortcut_group(this.frm.page).add($(el));
		});
	}
	...
	get_language_options() {
		return netmanthan.get_languages();
	}

	set_default_print_language() {
		let print_format = this.get_print_format();
		this.lang_code =
			this.frm.doc.language || print_format.default_print_language || netmanthan.boot.lang;
	...
	preview() {
		...
		this.get_print_html((out) => {
			...
			const print_height_inches = netmanthan.dom.pixel_to_inches(print_height);
	...
	setup_print_format_dom(out, $print_format) {
		...
		let base_url = netmanthan.urllib.get_base_url();
		let print_css = netmanthan.assets.bundled_asset(
			"print.bundle.css",
			netmanthan.utils.is_rtl(this.lang_code)
		);
		this.$print_format_body
			.find("html")
			.attr("dir", netmanthan.utils.is_rtl(this.lang_code) ? "rtl" : "ltr");
	...
	go_to_form_view() {
		...
		netmanthan.set_route("Form", this.frm.doctype, this.frm.docname);
	}
			...
			if (me.is_raw_printing()) {
				me.get_raw_commands(function (out) {
					netmanthan.ui.form
						...
						.then(netmanthan.ui.form.qz_success)
						.catch((err) => {
							netmanthan.ui.form.qz_fail(err);
						});
				});
			} else {
	...
	print_by_server() {
		...
		if (localStorage.getItem("network_printer")) {
			netmanthan.call({
				method: "netmanthan.utils.print_format.print_by_server",
				args: {
	...
	network_printer_setting_dialog(callback) {
		netmanthan.call({
			method: "netmanthan.printing.doctype.network_printer_settings.network_printer_settings.get_network_printer_settings",
			callback: function (r) {
				if (r.message) {
	...
	render_pdf() {
		...
		if (print_format.print_format_builder_beta) {
			...
			let w = window.open(`/api/method/netmanthan.utils.weasyprint.download_pdf?${params}`);
			if (!w) {
				netmanthan.msgprint(__("Please enable pop-ups"));
				return;
			}
		} else {
			this.render_page("/api/method/netmanthan.utils.print_format.download_pdf?");
		}
	}
	...
	get_print_html(callback) {
		...
		this._req = netmanthan.call({
			method: "netmanthan.www.printview.get_html_and_style",
			args: {
				doc: this.frm.doc,
	...
	get_raw_commands(callback) {
		...
		netmanthan.call({
			method: "netmanthan.www.printview.get_rendered_raw_commands",
			args: {
				doc: this.frm.doc,
				print_format: this.selected_format(),
				_lang: this.lang_code,
	...
	refresh_print_options() {
		this.print_formats = netmanthan.meta.get_print_formats(this.frm.doctype);
	...
	set_style(style) {
		netmanthan.dom.set_style(style || netmanthan.boot.print_css, "print-style");
	}
	...
	printer_setting_dialog() {
		...
		this.printer_list = [];
		netmanthan.ui.form.qz_get_printer_list().then((data) => {
			...
			if (!(this.printer_list && this.printer_list.length)) {
				netmanthan.throw(__("No Printer is Available."));
			}
		});
	}
};
