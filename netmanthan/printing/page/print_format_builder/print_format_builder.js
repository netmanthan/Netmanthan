netmanthan.pages["print-format-builder"].on_page_load = function (wrapper) {
	netmanthan.print_format_builder = new netmanthan.PrintFormatBuilder(wrapper);
	netmanthan.breadcrumbs.add("Setup", "Print Format");
};

netmanthan.pages["print-format-builder"].on_page_show = function (wrapper) {
	var route = netmanthan.get_route();
	if (route.length > 1) {
		netmanthan.model.with_doc("Print Format", route[1], function () {
			netmanthan.print_format_builder.print_format = netmanthan.get_doc("Print Format", route[1]);
			netmanthan.print_format_builder.refresh();
		});
	} else if (netmanthan.route_options) {
		if (netmanthan.route_options.make_new) {
			let { doctype, name, based_on, beta } = netmanthan.route_options;
			netmanthan.route_options = null;
			netmanthan.print_format_builder.setup_new_print_format(doctype, name, based_on, beta);
		} else {
			netmanthan.print_format_builder.print_format = netmanthan.route_options.doc;
			netmanthan.route_options = null;
			netmanthan.print_format_builder.refresh();
		}
	}
...
netmanthan.PrintFormatBuilder = class PrintFormatBuilder {
	constructor(parent) {
		this.parent = parent;
		this.make();
		this.refresh();
	...
	make() {
		this.page = netmanthan.ui.make_app_page({
			parent: this.parent,
		...
		this.page.main = $(
			'<div class="col-md-12 border print-format-builder-main netmanthan-card"></div>'
	...
	show_start() {
		this.page.main.html(netmanthan.render_template("print_format_builder_start", {}));
	...
	start_edit_print_format() {
		...
		var me = this;
		this.print_format_input = netmanthan.ui.form.make_control({
		...
		this.page.main.find(".btn-edit-print-format").on("click", function () {
			...
			if (!name) return;
			netmanthan.model.with_doc("Print Format", name, function (doc) {
				netmanthan.set_route("print-format-builder", name);
			});
		});
	}
	start_new_print_format() {
		var me = this;
		this.doctype_input = netmanthan.ui.form.make_control({
		...
		this.name_input = netmanthan.ui.form.make_control({
		...
		this.page.main.find(".btn-new-print-format").on("click", function () {
			...
			if (!(doctype && name)) {
				netmanthan.msgprint(__("Both DocType and Name required"));
				return;
			}
	...
	setup_new_print_format(doctype, name, based_on, beta) {
		netmanthan.call({
			method: "netmanthan.printing.page.print_format_builder.print_format_builder.create_custom_format",
			args: {
				doctype: doctype,
				name: name,
				based_on: based_on,
	...
	setup_print_format() {
		var me = this;
		netmanthan.model.with_doctype(this.print_format.doc_type, function (doctype) {
			me.meta = netmanthan.get_meta(me.print_format.doc_type);
			me.setup_sidebar();
			me.render_layout();
			...
			me.page.add_inner_button(__("Edit Properties"), function () {
				netmanthan.set_route("Form", "Print Format", me.print_format.name);
			});
		});
	}
	setup_sidebar() {
		...
		this.page.sidebar.html(
			$(netmanthan.render_template("print_format_builder_sidebar", { fields: fields }))
		);
	...
	render_layout() {
		...
		this.prepare_data();
		$(
			netmanthan.render_template("print_format_builder_layout", {
		...
		$.each(this.data, function (i, f) {
			...
			if (!f.name && f.fieldname) {
				...
				if (f.fieldname === "_custom_html") {
					...
					f = $.extend(
						netmanthan.meta.get_docfield(me.print_format.doc_type, f.fieldname) || {},
						f
					);
				}
			}
			...
			if (f.fieldtype === "Section Break") {
				...
				set_column();
			} else if (!in_list(netmanthan.model.layout_fields, f.fieldtype)) {
	...
	init_visible_columns(f) {
		f.visible_columns = [];
		$.each(netmanthan.get_meta(f.options).fields, function (i, _f) {
			if (
			...
			onAdd: function (evt) {
				...
				if (!$item.hasClass("print-format-builder-field")) {
					...
					if (fieldname === "_custom_html") {
						...
						var field = netmanthan.meta.get_docfield(me.print_format.doc_type, fieldname);
					}

					var html = netmanthan.render_template("print_format_builder_field", {
						field: field,
						me: me,
					});
	...
	setup_add_section() {
		...
		this.page.main.find(".print-format-builder-add-section").on("click", function () {
			...
			var $section = $(
				netmanthan.render_template("print_format_builder_section", {
					section: section,
					me: me,
				})
	...
	setup_column_selector() {
		...
		this.page.main.on("click", ".select-columns", function () {
			...
			var doc_fields = netmanthan.get_meta(doctype).fields;
			...
			// render checkboxes
			$(
				netmanthan.render_template("print_format_builder_column_selector", {
					fields: fields,
	...
	save_print_format() {
		...
		netmanthan.call({
			...
			callback: function (r) {
				...
				netmanthan.show_alert({ message: __("Saved"), indicator: "green" });
			},
		});
	}
};
