...
netmanthan.ui.form.on("Client Script", {
	setup(frm) {
	...
	refresh(frm) {
		if (frm.doc.dt && frm.doc.script) {
			frm.add_custom_button(__("Go to {0}", [frm.doc.dt]), () =>
				netmanthan.set_route("List", frm.doc.dt, "List")
			);
		}

		if (frm.doc.view == "Form") {
			frm.add_custom_button(__("Add script for Child Table"), () => {
				netmanthan.model.with_doctype(frm.doc.dt, () => {
					const child_tables = netmanthan.meta
			...
			if (!frm.is_new()) {
				frm.add_custom_button(__("Compare Versions"), () => {
					new netmanthan.ui.DiffView("Client Script", "script", frm.doc.name);
				});
			}
		}
	...
	dt(frm) {
		frm.toggle_display("view", !netmanthan.boot.single_types.includes(frm.doc.dt));
	...
	view(frm) {
		let has_form_boilerplate = frm.doc.script.includes("netmanthan.ui.form.on");
...
netmanthan.ui.form.on('Task',  'validate',  function(frm) {
...
netmanthan.ui.form.on('Sales Invoice',  {
    validate: function(frm) {
