...
import netmanthan
from netmanthan.core.doctype.doctype.doctype import InvalidFieldNameError
...
class TestCustomizeForm(netmanthanTestCase):
	def insert_custom_field(self):
		netmanthan.delete_doc_if_exists("Custom Field", "Event-custom_test_field")
		self.field = netmanthan.get_doc(
			{
				...
				"options": "\nCustom 1\nCustom 2\nCustom 3",
				"default": "Custom 3",
				"insert_after": netmanthan.get_meta("Event").fields[-1].fieldname,
			}
		).insert()

	def setUp(self):
		self.insert_custom_field()
		netmanthan.db.delete("Property Setter", dict(doc_type="Event"))
		netmanthan.db.commit()
		netmanthan.clear_cache(doctype="Event")

	def tearDown(self):
		netmanthan.delete_doc("Custom Field", self.field.name)
		netmanthan.db.commit()
		netmanthan.clear_cache(doctype="Event")

	def get_customize_form(self, doctype=None):
		d = netmanthan.get_doc("Customize Form")
		if doctype:
			d.doc_type = doctype
	...
	def test_fetch_to_customize(self):
		...
		self.assertEqual(len(d.get("fields")), len(netmanthan.get_doc("DocType", d.doc_type).fields) + 1)
	...
	def test_save_customization_custom_field_property(self):
		d = self.get_customize_form("Event")
		self.assertEqual(netmanthan.db.get_value("Custom Field", self.field.name, "reqd"), 0)

		custom_field = d.get("fields", {"fieldname": self.field.fieldname})[0]
		custom_field.reqd = 1
		custom_field.no_copy = 1
		d.run_method("save_customization")
		self.assertEqual(netmanthan.db.get_value("Custom Field", self.field.name, "reqd"), 1)
		self.assertEqual(netmanthan.db.get_value("Custom Field", self.field.name, "no_copy"), 1)
		...
		d.run_method("save_customization")
		self.assertEqual(netmanthan.db.get_value("Custom Field", self.field.name, "reqd"), 0)
		self.assertEqual(netmanthan.db.get_value("Custom Field", self.field.name, "no_copy"), 0)
	...
	def test_save_customization_new_field(self):
		...
		self.assertEqual(
			netmanthan.db.get_value("Custom Field", custom_field_name, "fieldtype"),
			"Data",
		)

		self.assertEqual(
			netmanthan.db.get_value("Custom Field", custom_field_name, "insert_after"),
			last_fieldname,
		)

		netmanthan.delete_doc("Custom Field", custom_field_name)
		self.assertEqual(netmanthan.db.get_value("Custom Field", custom_field_name), None)
	...
	def test_save_customization_remove_field(self):
		...
		self.assertEqual(netmanthan.db.get_value("Custom Field", custom_field.name), None)

		netmanthan.local.test_objects["Custom Field"] = []
	...
	def test_reset_to_defaults(self):
		...
		netmanthan.local.test_objects["Property Setter"] = []
	...
	def test_core_doctype_customization(self):
		self.assertRaises(netmanthan.ValidationError, self.get_customize_form, "User")
	...
	def test_custom_link(self):
		try:
			...
			event = netmanthan.get_meta("Event")
	...
	def test_custom_internal_links(self):
		...
		netmanthan.clear_cache()
		user_group = netmanthan.get_meta("User Group")
		...
		netmanthan.clear_cache()
		user_group = netmanthan.get_meta("Event")
		self.assertFalse(
	...
	def test_system_generated_fields(self):
		...
		custom_field = netmanthan.get_doc("Custom Field", {"dt": doctype, "fieldname": custom_field_name})
		...
		self.assertEqual(
			netmanthan.db.get_value("Property Setter", property_setter_filters, "value"), "Test Description"
		)
	...
	def test_custom_field_order(self):
		...
		field_order_property = json.loads(
			netmanthan.db.get_value("Property Setter", {"doc_type": "ToDo", "property": "field_order"}, "value")
		)

		self.assertEqual(field_order_property, [df.fieldname for df in netmanthan.get_meta("ToDo").fields])
