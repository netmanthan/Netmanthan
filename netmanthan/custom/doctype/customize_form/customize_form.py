...
from netmanthan.custom.doctype.property_setter.property_setter import delete_property_setter
from netmanthan.model import core_doctypes_list, no_value_fields
...
class CustomizeForm(Document):
	def on_update(self):
		netmanthan.db.delete("Singles", {"doctype": "Customize Form"})
		netmanthan.db.delete("Customize Form Field")
	...
	def validate_doctype(self, meta):
		...
		if self.doc_type in core_doctypes_list:
			netmanthan.throw(_("Core DocTypes cannot be customized."))

		if meta.issingle:
			netmanthan.throw(_("Single DocTypes cannot be customized."))

		if meta.custom:
			netmanthan.throw(_("Only standard DocTypes are allowed to be customized from Customize Form."))
	...
	def get_name_translation(self):
		...
		return netmanthan.get_value(
			"Translation",
			{"source_text": self.doc_type, "language": netmanthan.local.lang or "en"},
	...
	def set_name_translation(self):
		...
		if not self.label:
			if current:
				# clear translation
				netmanthan.delete_doc("Translation", current.name)
			return
			...
			netmanthan.get_doc(
				{
					...
					"language_code": netmanthan.local.lang or "en",
				}
			).insert()
			return

		if self.label != current.translated_text:
			netmanthan.db.set_value("Translation", current.name, "translated_text", self.label)
	...
	def save_customization(self):
		...
		if self.flags.update_db:
			netmanthan.db.updatedb(self.doc_type)

		if not hasattr(self, "hide_success") or not self.hide_success:
			netmanthan.msgprint(_("{0} updated").format(_(self.doc_type)), alert=True)
	...
	def allow_property_change(self, prop, meta_df, df):
		if prop == "fieldtype":
			...
			if not netmanthan.db.get_value(
			...
			(
                    netmanthan.db.get_value("DocField", {"parent": self.doc_type, "fieldname": df.fieldname}, "reqd")
                    == 1
			)
			and (df.get(prop) == 0)
		):
			...
			netmanthan.msgprint(_("You cannot unset 'Read Only' for field {0}").format(df.label))
			return False

		elif prop == "options" and df.get("fieldtype") not in ALLOWED_OPTIONS_CHANGE:
			netmanthan.msgprint(_("You can't set 'Options' for field {0}").format(df.label))
			return False

		elif prop == "translatable" and not supports_translation(df.get("fieldtype")):
			netmanthan.msgprint(_("You can't set 'Translatable' for field {0}").format(df.label))
			return False
	...
	def set_property_setters_for_actions_and_links(self, meta):
		...
		for doctype, fieldname, field_map in (
			...
			for i, d in enumerate(self.get(fieldname) or []):
				d.idx = i
				if netmanthan.db.exists(doctype, d.name) and not d.custom:
					# check property and apply property setter
					original = netmanthan.get_doc(doctype, d.name)
	...
	def update_order_property_setter(self, has_custom, fieldname):
		...
		if has_custom:
			...
			netmanthan.db.delete("Property Setter", dict(property=property_name, doc_type=self.doc_type))
	...
	def clear_removed_items(self, doctype, items):
		...
		"""
		if items:
			netmanthan.db.delete(doctype, dict(parent=self.doc_type, custom=1, name=("not in", items)))
		else:
			netmanthan.db.delete(doctype, dict(parent=self.doc_type, custom=1))
	...
	def update_custom_fields(self):
		for i, df in enumerate(self.get("fields")):
			...
			if not netmanthan.db.exists("Custom Field", {"dt": self.doc_type, "fieldname": df.fieldname}):
	...
	def add_custom_field(self, df, i):
		d = netmanthan.new_doc("Custom Field")
	...
	def update_in_custom_field(self, df, i):
		...
		custom_field = netmanthan.get_doc("Custom Field", meta_df[0].name)
		changed = False
		for prop in docfield_properties:
	...
	def delete_custom_fields(self):
		...
		for fieldname in fields_to_remove:
			...
			if not is_standard_or_system_generated_field(df):
				netmanthan.delete_doc("Custom Field", df.name)
	...
	def get_existing_property_value(self, property_name, fieldname=None):
		...
		if fieldname:
			...
			if netmanthan.db.has_column("DocType", property_name):
				property_value = netmanthan.db.get_value("DocType", self.doc_type, property_name)
			else:
				property_value = None
	...
	def validate_fieldtype_change(self, df, old_value, new_value):
		...
		if allowed:
			old_value_length = cint(netmanthan.db.type_map.get(old_value)[1])
			new_value_length = cint(netmanthan.db.type_map.get(new_value)[1])
	...
	def validate_fieldtype_length(self):
		for field in self.check_length_for_fieldtypes:
			df = field.get("df")
			max_length = cint(netmanthan.db.type_map.get(df.fieldtype)[1])
			fieldname = df.fieldname
			docs = netmanthan.db.sql(
				"""
			...
			label = df.label
			for doc in docs:
				links.append(netmanthan.utils.get_link_to_form(self.doc_type, doc.name))
			...
			if docs:
				netmanthan.throw(
					...
					).format(netmanthan.bold(label), links_str, netmanthan.bold(max_length)),
...
def reset_customization(doctype):
	...
	for setter in setters:
		netmanthan.delete_doc("Property Setter", setter)
	...
	for field in custom_fields:
		netmanthan.delete_doc("Custom Field", field)
