...
from netmanthan.utils.data import get_system_timezone
...
class OAuthWebRequestValidator(RequestValidator):
	...
	def validate_client_id(self, client_id, request, *args, **kwargs):
		...
		cli_id = netmanthan.db.get_value("OAuth Client", {"name": client_id})
		if cli_id:
			request.client = netmanthan.get_doc("OAuth Client", client_id).as_dict()
			return True
		else:
			return False
	...
	def validate_redirect_uri(self, client_id, redirect_uri, request, *args, **kwargs):
		...
		redirect_uris = netmanthan.db.get_value("OAuth Client", client_id, "redirect_uris").split(
			get_url_delimiter()
		)
	...
	def get_default_redirect_uri(self, client_id, request, *args, **kwargs):
		...
		redirect_uri = netmanthan.db.get_value("OAuth Client", client_id, "default_redirect_uri")
		return redirect_uri
	...
	def save_authorization_code(self, client_id, code, request, *args, **kwargs):
		...
		oac = netmanthan.new_doc("OAuth Authorization Code")
	...
	def authenticate_client(self, request, *args, **kwargs):
		...
		if request.client_id:
			oc = netmanthan.get_doc("OAuth Client", request.client_id)
		else:
			# Extract token, instantiate OAuth Bearer Token and use clientid from there.
			if "refresh_token" in netmanthan.form_dict:
				oc = netmanthan.get_doc(
					"OAuth Client",
					netmanthan.db.get_value(
						"OAuth Bearer Token",
						{"refresh_token": netmanthan.form_dict["refresh_token"]},
						"client",
					),
				)
			elif "token" in netmanthan.form_dict:
				oc = netmanthan.get_doc(
					"OAuth Client",
					netmanthan.db.get_value("OAuth Bearer Token", netmanthan.form_dict["token"], "client"),
				)
			else:
				oc = netmanthan.get_doc(
					"OAuth Client",
					netmanthan.db.get_value(
						"OAuth Bearer Token",
						netmanthan.get_request_header("Authorization").split(" ")[1],
						"client",
					),
				)
		try:
		...
		return netmanthan.session.user == user_id

	def authenticate_client_id(self, client_id, request, *args, **kwargs):
		cli_id = netmanthan.db.get_value("OAuth Client", client_id, "name")
		if not cli_id:
			# Don't allow public (non-authenticated) clients
			return False
		else:
			request["client"] = netmanthan.get_doc("OAuth Client", cli_id)
			return True
	...
	def validate_code(self, client_id, code, client, request, *args, **kwargs):
		...
		if code in checkcodes:
			request.scopes = netmanthan.db.get_value("OAuth Authorization Code", code, "scopes").split(
				get_url_delimiter()
			)
			request.user = netmanthan.db.get_value("OAuth Authorization Code", code, "user")
			code_challenge_method = netmanthan.db.get_value(
				"OAuth Authorization Code", code, "code_challenge_method"
			)
			code_challenge = netmanthan.db.get_value("OAuth Authorization Code", code, "code_challenge")

			if code_challenge and not request.code_verifier:
				if netmanthan.db.exists("OAuth Authorization Code", code):
					netmanthan.delete_doc("OAuth Authorization Code", code, ignore_permissions=True)
					netmanthan.db.commit()
				return False
	...
	def confirm_redirect_uri(self, client_id, code, redirect_uri, client, *args, **kwargs):
		saved_redirect_uri = netmanthan.db.get_value("OAuth Client", client_id, "default_redirect_uri")

		redirect_uris = netmanthan.db.get_value("OAuth Client", client_id, "redirect_uris")
	...
	def save_bearer_token(self, token, request, *args, **kwargs):
		...
		otoken = netmanthan.new_doc("OAuth Bearer Token")
		otoken.client = request.client["name"]
		try:
			otoken.user = (
				request.user
				if request.user
				else netmanthan.db.get_value(
					"OAuth Bearer Token",
			...
			otoken.user = netmanthan.session.user
		...
		default_redirect_uri = netmanthan.db.get_value(
	...
	def invalidate_authorization_code(self, client_id, code, request, *args, **kwargs):
		...
		netmanthan.db.set_value("OAuth Authorization Code", code, "validity", "Invalid")
		netmanthan.db.commit()
	...
	def validate_bearer_token(self, token, scopes, request):
		...
		otoken = netmanthan.get_doc("OAuth Bearer Token", token)
		...
		is_token_valid = (
                                 netmanthan.utils.datetime.datetime.utcnow().replace(tzinfo=pytz.utc) < token_expiration_utc
		) and otoken.status != "Revoked"
		client_scopes = netmanthan.db.get_value("OAuth Client", otoken.client, "scopes").split(
			get_url_delimiter()
		)
	...
	def get_original_scopes(self, refresh_token, request, *args, **kwargs):
		...
		# request.
		obearer_token = netmanthan.get_doc("OAuth Bearer Token", {"refresh_token": refresh_token})
	...
	def revoke_token(self, token, token_type_hint, request, *args, **kwargs):
		...
		if token_type_hint == "access_token":
			netmanthan.db.set_value("OAuth Bearer Token", token, "status", "Revoked")
		elif token_type_hint == "refresh_token":
			netmanthan.db.set_value("OAuth Bearer Token", {"refresh_token": token}, "status", "Revoked")
		else:
			netmanthan.db.set_value("OAuth Bearer Token", token, "status", "Revoked")
		netmanthan.db.commit()
	...
	def finalize_id_token(self, id_token, token, token_handler, request):
		# Check whether netmanthan server URL is set
		id_token_header = {"typ": "jwt", "alg": "HS256"}

		user = netmanthan.get_doc("User", request.user)
		...
		return netmanthan.safe_decode(id_token_encoded)

	def get_authorization_code_nonce(self, client_id, code, redirect_uri, request):
		if netmanthan.get_value("OAuth Authorization Code", code, "validity") == "Valid":
			return netmanthan.get_value("OAuth Authorization Code", code, "nonce")

		return None

	def get_authorization_code_scopes(self, client_id, code, redirect_uri, request):
		scope = netmanthan.get_value("OAuth Client", client_id, "scopes")
		if not scope:
			scope = []
		else:
	...
	def validate_id_token(self, token, scopes, request):
		try:
			id_token = netmanthan.get_doc("OAuth Bearer Token", token)
	...
	def validate_jwt_bearer_token(self, token, scopes, request):
		try:
			jwt = netmanthan.get_doc("OAuth Bearer Token", token)
	...
	def validate_silent_login(self, request):
		...
		- OpenIDConnectHybrid
		"""
		if netmanthan.session.user == "Guest" or request.prompt.lower() == "login":
			return False
		else:
			return True
	...
	def validate_user_match(self, id_token_hint, scopes, claims, request):
		...
		if id_token_hint:
			try:
				...
				client_id, client_secret = netmanthan.get_value(
					"OAuth Client",
					payload.get("aud"),
				...
				if payload.get("sub") and client_id and client_secret:
					user = netmanthan.db.get_value(
						"User Social Login",
						{"userid": payload.get("sub"), "provider": "netmanthan"},
						"parent",
					)
					...
					if verified_payload:
						return user.name == netmanthan.session.user

			except Exception:
				return False

		elif netmanthan.session.user != "Guest":
			return True
...
def delete_oauth2_data():
	...
	code_list = netmanthan.get_all("OAuth Authorization Code", filters={"validity": "Invalid"})
	token_list = netmanthan.get_all("OAuth Bearer Token", filters={"status": "Revoked"})
	if len(code_list) > 0:
		commit_code = True
	if len(token_list) > 0:
		commit_token = True
	for code in code_list:
		netmanthan.delete_doc("OAuth Authorization Code", code["name"])
	for token in token_list:
		netmanthan.delete_doc("OAuth Bearer Token", token["name"])
	if commit_code or commit_token:
		netmanthan.db.commit()


def get_client_scopes(client_id):
	scopes_string = netmanthan.db.get_value("OAuth Client", client_id, "scopes")
...
def get_userinfo(user):
	...
	if user.user_image:
		if netmanthan.utils.validate_url(user.user_image, valid_schemes=valid_url_schemes):
			picture = user.user_image
		else:
			picture = urljoin(netmanthan_server_url, user.user_image)

	userinfo = netmanthan._dict(
		{
			"sub": netmanthan.db.get_value(
				"User Social Login",
				{"parent": user.name, "provider": "netmanthan"},
				"userid",
			),
			...
			"email": user.email,
			"picture": picture,
			"roles": netmanthan.get_roles(user.name),
			"iss": netmanthan_server_url,
		}
	)
...
def generate_json_error_response(e):
	...
	netmanthan.local.response["http_status_code"] = getattr(e, "status_code", 500)
	return


def get_server_url():
	request_url = urlparse(netmanthan.request.url)
	request_url = f"{request_url.scheme}://{request_url.netloc}"
	return netmanthan.get_value("Social Login Key", "netmanthan", "base_url") or request_url
