...
class ParallelTestRunner:
	def __init__(self, app, site, build_number=1, total_builds=1, dry_run=False):
		self.app = app
		self.site = site
		self.build_number = netmanthan.utils.cint(build_number) or 1
		self.total_builds = netmanthan.utils.cint(total_builds)
		self.dry_run = dry_run
		self.setup_test_site()
		self.run_tests()

	def setup_test_site(self):
		netmanthan.init(site=self.site)
	...
	def before_test_setup(self):
		...
		for fn in netmanthan.get_hooks("before_tests", app_name=self.app):
			netmanthan.get_attr(fn)()

		test_module = netmanthan.get_module(f"{self.app}.tests")
	...
	def get_module(self, path, filename):
		app_path = netmanthan.get_pymodule_path(self.app)
...
class ParallelTestWithOrchestrator(ParallelTestRunner):
	...
	def __init__(self, app, site):
		...
		if not self.orchestrator_url:
			...
			click.echo("Pass public URL after hosting https://github.com/netmanthan/test-orchestrator")
			sys.exit(1)

		self.ci_build_id = os.environ.get("CI_BUILD_ID")
		self.ci_instance_id = os.environ.get("CI_INSTANCE_ID") or netmanthan.generate_hash(length=10)
