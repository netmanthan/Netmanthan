netmanthan.provide("netmanthan.core");

netmanthan.ui.form.on("Workflow", {
	onload: function (frm) {
	...
	refresh: function (frm) {
		if (frm.doc.document_type) {
			frm.add_custom_button(__("Go to {0} List", [frm.doc.document_type]), () => {
				netmanthan.set_route("List", frm.doc.document_type);
			});
		}
	...
	update_field_options: function (frm) {
		...
		if (doc.document_type) {
			const get_field_method =
				"netmanthan.workflow.doctype.workflow.workflow.get_fieldnames_for";
			netmanthan.xcall(get_field_method, { doctype: doc.document_type }).then((resp) => {
	...
	set_table_html: function (frm) {
		const promises = frm.states.map((r) => {
			...
			return netmanthan.utils.get_indicator_color(state).then((color) => {
				return `<tr>
				<td>
	...
	get_orphaned_states_and_count: function (frm) {
		...
		return netmanthan
			.xcall("netmanthan.workflow.doctype.workflow.workflow.get_workflow_state_count", {
		...
		if (frm.states && frm.states.length) {
			...
			$(frm.state_table)
				...
				.on("click", (e) => {
					...
					netmanthan.set_route("List", frm.doc.document_type, filters);
				});
		}
	},
});

netmanthan.ui.form.on("Workflow Document State", {
