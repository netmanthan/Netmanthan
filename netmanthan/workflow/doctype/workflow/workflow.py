...
class Workflow(Document):
	...
	def on_update(self):
		...
		netmanthan.clear_cache(doctype=self.document_type)

	def create_custom_field_for_workflow_state(self):
		netmanthan.clear_cache(doctype=self.document_type)
		meta = netmanthan.get_meta(self.document_type)
	...
	def validate_docstatus(self):
		def get_state(state):
			...
			netmanthan.throw(netmanthan._("{0} not a valid State").format(state))
		...
		for t in self.transitions:
			...
			if state.doc_status == "2":
				netmanthan.throw(
					netmanthan._("Cannot change state of Cancelled Document. Transition row {0}").format(t.idx)
				)

			if state.doc_status == "1" and next_state.doc_status == "0":
				netmanthan.throw(
					netmanthan._("Submitted Document cannot be converted back to draft. Transition row {0}").format(
						t.idx
					)
				)

			if state.doc_status == "0" and next_state.doc_status == "2":
				netmanthan.throw(netmanthan._("Cannot cancel before submitting. See Transition {0}").format(t.idx))
...
def get_fieldnames_for(doctype):
	netmanthan.has_permission(doctype=doctype, ptype="read", throw=True)
	return [
		f.fieldname for f in netmanthan.get_meta(doctype).fields if f.fieldname not in no_value_fields
	]
...
def get_workflow_state_count(doctype, workflow_state_field, states):
	netmanthan.has_permission(doctype=doctype, ptype="read", throw=True)
	states = netmanthan.parse_json(states)

	if workflow_state_field in netmanthan.get_meta(doctype).get_valid_columns():
