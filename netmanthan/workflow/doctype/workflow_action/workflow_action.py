...
from netmanthan.desk.form.utils import get_pdf_link
from netmanthan.desk.notifications import clear_doctype_notifications
...
from netmanthan.utils import get_datetime, get_url
from netmanthan.utils.background_jobs import enqueue
from netmanthan.utils.data import get_link_to_form
from netmanthan.utils.user import get_users_with_role
...
def on_doctype_update():
	...
	netmanthan.db.add_index("Workflow Action", ["reference_name", "reference_doctype", "status"])
...
def get_permission_query_conditions(user):
	...
	permitted_workflow_actions = (
		netmanthan.qb.from_(WorkflowAction)
	...
	""".format(
		permitted_workflow_actions=permitted_workflow_actions, user=netmanthan.db.escape(user)
	)


def has_permission(doc, user):

	user_roles = set(netmanthan.get_roles(user))
...
def confirm_action(doctype, docname, user, action):
	...
	logged_in_user = netmanthan.session.user
	...
	if logged_in_user == "Guest":
		netmanthan.set_user(logged_in_user)
...
def return_success_page(doc):
	netmanthan.respond_as_web_page(
		...
		_("{0}: {1} is set to state {2}").format(
			doc.get("doctype"), netmanthan.bold(doc.get("name")), netmanthan.bold(get_doc_workflow_state(doc))
		),
...
def return_link_expired_page(doc, doc_workflow_state):
	netmanthan.respond_as_web_page(
		...
		_("Document {0} has been set to state {1} by {2}").format(
			netmanthan.bold(doc.get("name")),
			netmanthan.bold(doc_workflow_state),
			netmanthan.bold(netmanthan.get_value("User", doc.get("modified_by"), "full_name")),
		),
		indicator_color="blue",
	)
...
def get_allowed_roles(user, workflow, workflow_state):
	user = user if user else netmanthan.session.user

	allowed_roles = netmanthan.get_all(
		"Workflow Transition",
		fields="allowed",
...
def update_completed_workflow_actions_using_role(user=None, workflow_action=None):
	...
	(
		netmanthan.qb.update(WorkflowAction)
...
def get_next_possible_transitions(workflow_name, state, doc=None):
	transitions = netmanthan.get_all(
		"Workflow Transition",
	...
	for transition in transitions:
		...
		for user in filtered_users:
			if not user_data_map.get(user):
				user_data_map[user] = netmanthan._dict(
					{
						"possible_actions": [],
						"email": netmanthan.db.get_value("User", user, "email"),
					}
				)
...
def create_workflow_actions_for_roles(roles, doc):
	workflow_action = netmanthan.get_doc(
		{
...
def get_workflow_action_url(action, doc, user):
	apply_action_method = (
		"/api/method/netmanthan.workflow.doctype.workflow_action.workflow_action.apply_action"
	)
...
def get_confirm_workflow_action_url(doc, action, user):
	confirm_action_method = (
		"/api/method/netmanthan.workflow.doctype.workflow_action.workflow_action.confirm_action"
	)
...
def get_common_email_args(doc):
	...
	if email_template:
		subject = netmanthan.render_template(email_template.subject, vars(doc))
		response = netmanthan.render_template(email_template.response, vars(doc))
	else:
	...
	common_args = {
		...
		"attachments": [netmanthan.attach_print(doctype, docname, file_name=docname, doc=doc)],
		"subject": subject,
		"message": response,
	}
	return common_args
...
def get_email_template(doc):
	...
	template_name = netmanthan.db.get_value(
	...
	if not template_name:
		return
	return netmanthan.get_doc("Email Template", template_name)


def get_state_optional_field_value(workflow_name, state):
	return netmanthan.get_cached_value(
