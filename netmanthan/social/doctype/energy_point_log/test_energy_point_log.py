...
import netmanthan
from netmanthan.desk.form.assign_to import add as assign_to
from netmanthan.desk.page.user_profile.user_profile import get_energy_points_heatmap_data
...
class TestEnergyPointLog(netmanthanTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def setUp(self):
		netmanthan.cache().delete_value("energy_point_rule_map")

	def tearDown(self):
		netmanthan.set_user("Administrator")
		netmanthan.db.delete("Energy Point Log")
		netmanthan.db.delete("Energy Point Rule")
		netmanthan.cache().delete_value("energy_point_rule_map")
	...
	def test_revert_points_on_cancelled_doc(self):
		...
		energy_point_logs = netmanthan.get_all("Energy Point Log")
		...
		energy_point_logs = netmanthan.get_all(
	...
	def test_allow_creation_of_new_log_if_the_previous_log_was_reverted(self):
		...
		log_name = netmanthan.db.exists("Energy Point Log", {"reference_name": created_todo.name})
		netmanthan.get_doc("Energy Point Log", log_name).revert("Just for test")
...
def create_energy_point_rule_for_todo(
	...
	name = "ToDo Closed"
	point_rule_exists = netmanthan.db.exists("Energy Point Rule", name)

	if point_rule_exists:
		return netmanthan.get_doc("Energy Point Rule", name)
