...
from netmanthan.cache_manager import clear_doctype_map, get_doctype_map
...
class AssignmentRule(Document):
	...
	def validate_document_types(self):
		if self.document_type == "ToDo":
			netmanthan.throw(
				_("Assignment Rule is not allowed on {0} document type").format(netmanthan.bold("ToDo"))
			)
		...
		if user:
			assign_to.add(
				dict(
					...
					name=doc.get("name"),
					description=netmanthan.render_template(self.description, doc),
	...
	def get_user_based_on_field(self, doc):
		...
		if netmanthan.db.exists("User", val):
			return val

	def safe_eval(self, fieldname, doc):
		try:
			if self.get(fieldname):
				return netmanthan.safe_eval(self.get(fieldname), None, doc)
		except Exception as e:
			# when assignment fails, don't block the document as it may be
			# a part of the email pulling
			netmanthan.msgprint(netmanthan._("Auto assignment failed: {0}").format(str(e)), indicator="orange")
	...
	def is_rule_not_applicable_today(self):
		today = netmanthan.flags.assignment_day or netmanthan.utils.get_weekday()
		assignment_days = self.get_assignment_days()
...
def reopen_closed_assignment(doc):
	...
	for todo in todo_list:
		todo_doc = netmanthan.get_doc("ToDo", todo)
...
def apply(doc=None, method=None, doctype=None, name=None):
	...
	assignment_rule_docs: list[AssignmentRule] = [
		netmanthan.get_cached_doc("Assignment Rule", d.get("name")) for d in assignment_rules
	]
	...
	if assignments:
		for assignment_rule in assignment_rule_docs:
			...
			if not new_apply:
				...
				if to_close_todos:
					# close todo status
					todos_to_close = netmanthan.get_all(
						"ToDo",
						filters={
							"reference_type": doc.doctype,
							"reference_name": doc.name,
					...
					for todo in todos_to_close:
						_todo = netmanthan.get_doc("ToDo", todo)
						_todo.status = "Closed"
...
def update_due_date(doc, state=None):
	...
	for rule in assignment_rules:
		rule_doc = netmanthan.get_cached_doc("Assignment Rule", rule.get("name"))
		...
		if field_updated:
			assignment_todos = netmanthan.get_all(
				"ToDo",
				filters={
...
def get_assignment_rules() -> list[str]:
	return netmanthan.get_all("Assignment Rule", filters={"disabled": 0}, pluck="document_type")
