...
import netmanthan
from netmanthan.automation.doctype.auto_repeat.auto_repeat import (
...
from netmanthan.utils import add_days, add_months, getdate, today
...
class TestAutoRepeat(netmanthanTestCase):
	def setUp(self):
		if not netmanthan.db.sql(
	...
	def test_daily_auto_repeat(self):
		...
		todo = netmanthan.get_doc(doc.reference_doctype, doc.reference_document)
		self.assertEqual(todo.auto_repeat, doc.name)

		new_todo = netmanthan.db.get_value(
			"ToDo", {"auto_repeat": doc.name, "name": ("!=", todo.name)}, "name"
		)

		new_todo = netmanthan.get_doc("ToDo", new_todo)
	...
	def monthly_auto_repeat(self, doctype, docname, start_date, end_date=None):
		...
		data = get_auto_repeat_entries(getdate(today()))
		create_repeated_entries(data)
		docnames = netmanthan.get_all(doc.reference_doctype, {"auto_repeat": doc.name})
		self.assertEqual(len(docnames), 1)

		doc = netmanthan.get_doc("Auto Repeat", doc.name)
	...
	def test_email_notification(self):
		...
		email_queue = netmanthan.db.exists(
	...
	def test_submit_on_creation(self):
		...
		submittable_doc = netmanthan.get_doc(dict(doctype=doctype, test="test submit on creation")).insert()
	...
	doc = netmanthan.get_doc(
		{
			...
			"reference_document": args.reference_document or netmanthan.db.get_value("ToDo", "name"),
...
def create_submittable_doctype(doctype, submit_perms=1):
	if netmanthan.db.exists("DocType", doctype):
		return
	else:
		doc = netmanthan.get_doc(
			{
