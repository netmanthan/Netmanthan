...
from netmanthan.contacts.address_and_contact import set_link_title
from netmanthan.core.doctype.dynamic_link.dynamic_link import deduplicate_dynamic_links
from netmanthan.model.document import Document
from netmanthan.model.naming import append_number_if_name_exists
from netmanthan.utils import cstr, has_gravatar
...
class Contact(Document):
	def autoname(self):
		...
		if netmanthan.db.exists("Contact", self.name):
	...
	def validate(self):
		...
		if self.get("sync_with_google_contacts") and not self.get("google_contacts"):
			netmanthan.throw(_("Select Google Contacts to which contact should be synced."))

		deduplicate_dynamic_links(self)

	def set_user(self):
		if not self.user and self.email_id:
			self.user = netmanthan.db.get_value("User", {"email": self.email_id})
	...
	def add_email(self, email_id, is_primary=0, autosave=False):
		if not netmanthan.db.exists("Contact Email", {"email_id": email_id, "parent": self.name}):
	...
	def add_phone(self, phone, is_primary_phone=0, is_primary_mobile_no=0, autosave=False):
		if not netmanthan.db.exists("Contact Phone", {"phone": phone, "parent": self.name}):
			self.append(
				"phone_nos",
				{
					"phone": phone,
	...
	def set_primary_email(self):
		...
		if len([email.email_id for email in self.email_ids if email.is_primary]) > 1:
			netmanthan.throw(_("Only one {0} can be set as primary.").format(netmanthan.bold("Email ID")))
	...
	def set_primary(self, fieldname):
		...
		if len(is_primary) > 1:
			netmanthan.throw(
				_("Only one {0} can be set as primary.").format(netmanthan.bold(netmanthan.unscrub(fieldname)))
			)
...
def invite_user(contact):
	contact = netmanthan.get_doc("Contact", contact)

	if not contact.email_id:
		netmanthan.throw(_("Please set Email Address"))
...
def update_contact(doc, method):
	...
	contact_name = netmanthan.db.get_value("Contact", {"email_id": doc.name})
	if contact_name:
...
def contact_query(doctype, txt, searchfield, start, page_len, filters):
	from netmanthan.desk.reportview import get_match_cond

	doctype = "Contact"
	if (
		not netmanthan.get_meta(doctype).get_field(searchfield)
		and searchfield not in netmanthan.db.DEFAULT_COLUMNS
	):
		return []
...
def get_contacts_linked_from(doctype, docname, fields=None):
	...
	link_fields = netmanthan.get_meta(doctype).get("fields", {"fieldtype": "Link", "options": "Contact"})
	if not link_fields:
		return []

	contact_names = netmanthan.get_value(doctype, docname, fieldname=[f.fieldname for f in link_fields])
	if not contact_names:
		return []

	return netmanthan.get_list("Contact", fields=fields, filters={"name": ("in", contact_names)})
