...
import netmanthan
from netmanthan import _, throw
...
from netmanthan.model.naming import make_autoname
...
class Address(Document):
	...
	def autoname(self):
		...
		if self.address_title:
			...
			if netmanthan.db.exists("Address", self.name):
	...
	def link_address(self):
		...
		if not self.links:
			contact_name = netmanthan.db.get_value("Contact", {"email_id": self.owner})
			if contact_name:
				contact = netmanthan.get_cached_doc("Contact", contact_name)
...
def get_address_display(address_dict: dict | str | None = None) -> str | None:
	...
	if not isinstance(address_dict, dict):
		address_dict = netmanthan.db.get_value("Address", address_dict, "*", as_dict=True, cache=True) or {}

	name, template = get_address_templates(address_dict)

	try:
		return netmanthan.render_template(template, address_dict)
	except TemplateSyntaxError:
		netmanthan.throw(_("There is an error in your Address Template {0}").format(name))
...
def get_territory_from_address(address):
	...
	if isinstance(address, str):
		address = netmanthan.get_cached_doc("Address", address)

	territory = None
	for fieldname in ("city", "state", "country"):
		if address.get(fieldname):
			territory = netmanthan.db.get_value("Territory", address.get(fieldname))
			if territory:
				break
...
def get_address_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by=None):
	from netmanthan.www.list import get_list
...
def has_website_permission(doc, ptype, user, verbose=False):
	...
	contact_name = netmanthan.db.get_value("Contact", {"email_id": netmanthan.session.user})
...
def get_address_templates(address):
	result = netmanthan.db.get_value(
		"Address Template", {"country": address.get("country")}, ["name", "template"]
	)

	if not result:
		result = netmanthan.db.get_value("Address Template", {"is_default": 1}, ["name", "template"])
...
def address_query(doctype, txt, searchfield, start, page_len, filters):
	...
	for fieldname, value in filters.items():
		if meta.get_field(fieldname) or fieldname in netmanthan.db.DEFAULT_COLUMNS:
			condition += f" and {fieldname}={netmanthan.db.escape(value)}"

	searchfields = meta.get_search_fields()

	if searchfield and (meta.get_field(searchfield) or searchfield in netmanthan.db.DEFAULT_COLUMNS):
...
def update_preferred_address(address, field):
	netmanthan.db.set_value("Address", address, field, 0)
