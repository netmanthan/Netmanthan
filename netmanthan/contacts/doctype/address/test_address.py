...
import netmanthan
from netmanthan.contacts.doctype.address.address import address_query, get_address_display
from netmanthan.tests.utils import netmanthanTestCase


class TestAddress(netmanthanTestCase):
	def test_template_works(self):
		if not netmanthan.db.exists("Address Template", "India"):
			netmanthan.get_doc({"doctype": "Address Template", "country": "India", "is_default": 1}).insert()

		if not netmanthan.db.exists("Address", "_Test Address-Office"):
			netmanthan.get_doc(
				{
		...
		address = netmanthan.get_list("Address")[0].name
		display = get_address_display(netmanthan.get_doc("Address", address).as_dict())
