...
from netmanthan.contacts.report.addresses_and_contacts.addresses_and_contacts import get_data
from netmanthan.tests.utils import netmanthanTestCase


def get_custom_linked_doctype():
	if bool(netmanthan.get_all("DocType", filters={"name": "Test Custom Doctype"})):
		return
...
def create_linked_address(link_list):
	if netmanthan.flags.test_address_created:
		return

	address = netmanthan.get_doc(
		{
			"doctype": "Address",
	...
	address.insert()
	netmanthan.flags.test_address_created = True

	return address.name


def create_linked_contact(link_list, address):
	if netmanthan.flags.test_contact_created:
		return
	...
	netmanthan.flags.test_contact_created = True


class TestAddressesAndContacts(netmanthanTestCase):
