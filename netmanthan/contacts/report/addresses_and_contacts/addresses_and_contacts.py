# Copyright (c) 2013, netmanthan Technologies Pvt. Ltd. and contributors
...
def get_reference_addresses_and_contact(reference_doctype, reference_name):
	data = []
	filters = None
	reference_details = netmanthan._dict()
	...
	reference_list = [
		d[0] for d in netmanthan.get_list(reference_doctype, filters=filters, fields=["name"], as_list=True)
	]

	for d in reference_list:
		reference_details.setdefault(d, netmanthan._dict())
...
def get_reference_details(reference_doctype, doctype, reference_list, reference_details):
	...
	records = netmanthan.get_list(doctype, filters=filters, fields=fields, as_list=True)
	temp_records = list()
	...
	if not reference_list:
		netmanthan.throw(_("No records present in {0}").format(reference_doctype))

	reference_details[reference_list[0]][netmanthan.scrub(doctype)] = temp_records
