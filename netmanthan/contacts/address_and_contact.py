...
def load_address_and_contact(doc, key=None):
	...
	from netmanthan.contacts.doctype.address.address import get_address_display, get_condensed_address
	...
	address_list = netmanthan.get_list("Address", filters=filters, fields=["*"], order_by="creation asc")
	...
	contact_list = netmanthan.get_list("Contact", filters=filters, fields=["*"])

	for contact in contact_list:
		contact["email_ids"] = netmanthan.get_all(
			"Contact Email",
			filters={"parenttype": "Contact", "parent": contact.name, "is_primary": 0},
			fields=["email_id"],
		)

		contact["phone_nos"] = netmanthan.get_all(
			"Contact Phone",
			filters={
				"parenttype": "Contact",
				"parent": contact.name,
		...
		if contact.address:
			address = netmanthan.get_doc("Address", contact.address)
...
def has_permission(doc, ptype, user):
	...
	for df in links.get("permitted_links") + links.get("not_permitted_links"):
		...
		if name and netmanthan.has_permission(doctype, ptype, doc=name):
			return True
...
def delete_contact_and_address(doctype: str, docname: str) -> None:
	for parenttype in ("Contact", "Address"):
		for name in netmanthan.get_all(
			...
			doc = netmanthan.get_doc(parenttype, name)
			if len(doc.links) == 1:
				doc.delete()
			else:
...
def filter_dynamic_link_doctypes(
	...
	_doctypes_from_df = netmanthan.get_all(
		"DocField",
		filters=filters,
		pluck="parent",
		distinct=True,
	...
	_doctypes_from_cdf = netmanthan.get_all(
...
def set_link_title(doc):
	...
	for link in doc.links:
		if not link.link_title:
			linked_doc = netmanthan.get_doc(link.link_doctype, link.link_name)
