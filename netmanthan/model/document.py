...
import netmanthan
from netmanthan import _, is_whitelisted, msgprint
from netmanthan.core.doctype.server_script.server_script_utils import run_server_script_for_doc_event
from netmanthan.desk.form.document_follow import follow_document
from netmanthan.integrations.doctype.webhook import run_webhooks
from netmanthan.model import optional_fields, table_fields
from netmanthan.model.base_document import BaseDocument, get_controller
from netmanthan.model.docstatus import DocStatus
from netmanthan.model.naming import set_new_name, validate_name
from netmanthan.model.utils import is_virtual_doctype
from netmanthan.model.workflow import set_workflow_state_on_action, validate_workflow
from netmanthan.utils import cstr, date_diff, file_lock, flt, get_datetime_str, now
from netmanthan.utils.data import get_absolute_url
from netmanthan.utils.global_search import update_global_search


def get_doc(*args, **kwargs):
	"""returns a netmanthan.model.Document object.
...
class Document(BaseDocument):
	...
	def load_from_db(self):
		...
		if not getattr(self, "_metaclass", False) and self.meta.issingle:
			single_doc = netmanthan.db.get_singles_dict(self.doctype, for_update=self.flags.for_update)
			if not single_doc:
				single_doc = netmanthan.new_doc(self.doctype, as_dict=True)
			...
			d = netmanthan.db.get_value(
				self.doctype, self.name, "*", as_dict=1, for_update=self.flags.for_update
			)
			if not d:
				netmanthan.throw(
					_("{0} {1} not found").format(_(self.doctype), self.name), netmanthan.DoesNotExistError
				)
		...
		for df in self._get_table_fields():
			...
			# During netmanthan is installed, the property "is_virtual" is not available in tabDocType, so
			# we need to filter those cases for the access to netmanthan.db.get_value() as it would crash otherwise.
	...
	def check_permission(self, permtype="read", permlevel=None):
		"""Raise `netmanthan.PermissionError` if not permitted"""
		if not self.has_permission(permtype):
			self.raise_no_permission_to(permtype)

	def has_permission(self, permtype="read", verbose=False) -> bool:
		"""
		Call `netmanthan.permissions.has_permission` if `ignore_permissions` flag isn't truthy
		...
		return netmanthan.permissions.has_permission(self.doctype, permtype, self)

	def raise_no_permission_to(self, perm_type):
		"""Raise `netmanthan.PermissionError`."""
		netmanthan.flags.error_message = (
			_("Insufficient Permission for {0}").format(self.doctype) + f" ({netmanthan.bold(_(perm_type))})"
		)
		raise netmanthan.PermissionError
	...
	def insert(
		...
		if not (
                netmanthan.flags.in_migrate or netmanthan.local.flags.in_install or netmanthan.flags.in_setup_wizard
		):
			if netmanthan.get_cached_value("User", netmanthan.session.user, "follow_created_documents"):
				follow_document(self.doctype, self.name, netmanthan.session.user)
		return self
	...
	def _save(self, ignore_permissions=None, ignore_version=None) -> "Document":
		...
		self.flags.ignore_version = netmanthan.flags.in_test if ignore_version is None else ignore_version
	...
	def update_child_table(self, fieldname, df=None):
		...
		if (
			...
			or netmanthan.get_meta(df.options).is_virtual == 1
		):
		...
		if rows:
			...
			deleted_rows = netmanthan.db.sql(
			...
			if len(deleted_rows) > 0:
				...
				netmanthan.db.delete(df.options, {"name": ("in", tuple(row[0] for row in deleted_rows))})
	...
	def set_new_name(self, force=False, set_name=None, set_child_names=True):
		"""Calls `netmanthan.naming.set_new_name` for parent and child docs."""
	...
	def update_single(self, d):
		...
		netmanthan.db.delete("Singles", {"doctype": self.doctype})
		...
		if self.doctype in netmanthan.db.value_cache:
			del netmanthan.db.value_cache[self.doctype]

	def set_user_and_timestamp(self):
		self._original_modified = self.modified
		self.modified = now()
		self.modified_by = netmanthan.session.user
		...
		if self.is_new() and not (
                netmanthan.flags.in_install or netmanthan.flags.in_patch or netmanthan.flags.in_migrate
		):
		...
		netmanthan.flags.currently_saving.append((self.doctype, self.name))
	...
	def _validate_non_negative(self):
		def get_msg(df):
			if self.get("parentfield"):
				return "{} {} #{}: {} {}".format(
					netmanthan.bold(_(self.doctype)),
					_("Row"),
					self.idx,
					_("Value cannot be negative for"),
					netmanthan.bold(_(df.label)),
				)
			else:
				return _("Value cannot be negative for {0}: {1}").format(
					_(df.parent), netmanthan.bold(_(df.label))
				)
		...
		for df in self.meta.get(
			...
			if flt(self.get(df.fieldname)) < 0:
				msg = get_msg(df)
				netmanthan.throw(msg, netmanthan.NonNegativeError, title=_("Negative Value"))
				...
				if fail:
					netmanthan.throw(
						_("Value cannot be changed for {0}").format(
							netmanthan.bold(self.meta.get_label(field.fieldname))
						),
						exc=netmanthan.CannotChangeConstantError,
					)
	...
	def apply_fieldlevel_read_permissions(self):
		...
		for table_field in self.meta.get_table_fields():
			all_fields += netmanthan.get_meta(table_field.options).fields or []
		...
		for table_field in self.meta.get_table_fields():
			for df in netmanthan.get_meta(table_field.options).fields or []:
	...
	def validate_higher_perm_levels(self):
		...
		if self.flags.ignore_permissions or netmanthan.flags.in_install:
			return
		...
		for df in self.meta.get_table_fields():
			high_permlevel_fields = netmanthan.get_meta(df.options).get_high_permlevel_fields()
			if high_permlevel_fields:
				for d in self.get(df.fieldname):
	...
	def get_permissions(self):
		if self.meta.istable:
			...
			permissions = netmanthan.get_meta(self.parenttype).permissions
		else:
	...
	def _set_defaults(self):
		...
		new_doc = netmanthan.new_doc(self.doctype, as_dict=True)
		self.update_if_missing(new_doc)

		# children
		for df in self.meta.get_table_fields():
			new_doc = netmanthan.new_doc(df.options, as_dict=True)
	...
	def check_if_latest(self):
		...
		if cstr(previous.modified) != cstr(self._original_modified):
			netmanthan.msgprint(
				...
				raise_exception=netmanthan.TimestampMismatchError,
			)
	...
	def check_docstatus_transition(self, to_docstatus):
		...
		if to_docstatus == DocStatus.draft():
			if self.docstatus.is_draft():
				...
				raise netmanthan.DocstatusTransitionError(
					_("Cannot change docstatus from 0 (Draft) to 2 (Cancelled)")
				)
			else:
				raise netmanthan.ValidationError(_("Invalid docstatus"), self.docstatus)
			...
			raise netmanthan.ValidationError(_("Cannot edit cancelled document"))
	...
	def _validate_mandatory(self):
		...
		if netmanthan.flags.print_messages:
			print(self.as_json().encode("utf-8"))

		raise netmanthan.MandatoryError(
	...
	def _validate_links(self):
		...
		if invalid_links:
			...
			netmanthan.throw(_("Could not find {0}").format(msg), netmanthan.LinkValidationError)

		if cancelled_links:
			msg = ", ".join(each[2] for each in cancelled_links)
			netmanthan.throw(_("Cannot link cancelled document: {0}").format(msg), netmanthan.CancelledLinkError)
	...
	def run_notifications(self, method):
		...
		if (
			(netmanthan.flags.in_import and netmanthan.flags.mute_emails)
		...
		from netmanthan.email.doctype.notification.notification import evaluate_alert
		...
		if self.flags.notifications is None:
			...
			self.flags.notifications = netmanthan.cache().hget(
	...
	def _rename(
		...
		"""Rename the document. Triggers netmanthan.rename_doc, then reloads."""
	...
	def load_doc_before_save(self, *, raise_exception: bool = False):
		...
		try:
			self._doc_before_save = netmanthan.get_doc(self.doctype, self.name, for_update=True)
	...
	def run_post_save_methods(self):
		...
		if (self.doctype, self.name) in netmanthan.flags.currently_saving:
			netmanthan.flags.currently_saving.remove((self.doctype, self.name))
	...
	def reset_seen(self):
		...
		if (
			...
			netmanthan.db.set_value(
				self.doctype, self.name, "_seen", json.dumps([netmanthan.session.user]), update_modified=False
			)
	...
	def notify_update(self):
		...
		if (
			...
			data = {"doctype": self.doctype, "name": self.name, "user": netmanthan.session.user}
			netmanthan.publish_realtime("list_update", data, after_commit=True)
	...
	def db_set(self, fieldname, value=None, update_modified=True, notify=False, commit=False):
		...
		:param commit: default False. run netmanthan.db.commit()
		"""
		...
		if update_modified and (self.doctype, self.name) not in netmanthan.flags.currently_saving:
			# don't update modified timestamp if called from post save methods
			# like on_update or on_submit
			self.set("modified", now())
			self.set("modified_by", netmanthan.session.user)
	...
	def db_get(self, fieldname):
		...
		return netmanthan.db.get_value(self.doctype, self.name, fieldname)

	def check_no_back_links_exist(self):
		"""Check if document links to any active document before Cancel."""
		from netmanthan.model.delete_doc import check_if_doc_is_dynamically_linked, check_if_doc_is_linked
	...
	def save_version(self):
		...
		if (
			...
			or netmanthan.flags.in_install
			or (not self._doc_before_save and netmanthan.flags.in_patch)
		):
			return

		doc_to_compare = self._doc_before_save
		if not doc_to_compare and (amended_from := self.get("amended_from")):
			doc_to_compare = netmanthan.get_doc(self.doctype, amended_from)
	...
	def hook(f):
		...
		def composer(self, *args, **kwargs):
			hooks = []
			method = f.__name__
			doc_events = netmanthan.get_doc_hooks()
			for handler in doc_events.get(self.doctype, {}).get(method, []) + doc_events.get("*", {}).get(
				method, []
			):
				hooks.append(netmanthan.get_attr(handler))
	...
	def validate_value(self, fieldname, condition, val2, doc=None, raise_exception=None):
		...
		if not netmanthan.compare(val1, condition, val2):
	...
	def validate_table_has_rows(self, parentfield, raise_exception=None):
		...
		if not (isinstance(self.get(parentfield), list) and len(self.get(parentfield)) > 0):
			...
			netmanthan.throw(
				_("Table {0} cannot be empty").format(label), raise_exception or netmanthan.EmptyTableError
			)
	...
	def add_comment(
		...
		out = netmanthan.get_doc(
			{
				"doctype": "Comment",
				"comment_type": comment_type,
				"comment_email": comment_email or netmanthan.session.user,
	...
	def add_seen(self, user=None):
		...
		if self.meta.track_seen and not netmanthan.flags.read_only:
			_seen = self.get("_seen") or []
			_seen = netmanthan.parse_json(_seen)

			if user not in _seen:
				_seen.append(user)
				netmanthan.db.set_value(self.doctype, self.name, "_seen", json.dumps(_seen), update_modified=False)
	...
	def add_viewed(self, user=None):
		...
		if hasattr(self.meta, "track_views") and self.meta.track_views:
			view_log = netmanthan.get_doc(
				{
					"doctype": "View Log",
					"viewed_by": netmanthan.session.user,
					"reference_doctype": self.doctype,
					"reference_name": self.name,
				}
			)
	...
	def get_document_share_key(self, expires_on=None, no_expiry=False):
		...
		existing_key = netmanthan.db.exists(
			"Document Share Key",
			{
				"reference_doctype": self.doctype,
		...
		if existing_key:
			doc = netmanthan.get_doc("Document Share Key", existing_key)
		else:
			doc = netmanthan.new_doc("Document Share Key")
	...
	def set_onload(self, key, value):
		if not self.get("__onload"):
			self.set("__onload", netmanthan._dict())
		self.get("__onload")[key] = value

	def get_onload(self, key=None):
		if not key:
			return self.get("__onload", netmanthan._dict())
	...
	def queue_action(self, action, **kwargs):
		...
		try:
			self.lock()
		except netmanthan.DocumentLockedError:
			netmanthan.throw(
		...
		return enqueue(
			"netmanthan.model.document.execute_action",
			...
			if timeout:
				for i in range(timeout):
					...
					if not file_lock.lock_exists(signature):
						...
						break
			if lock_exists:
				raise netmanthan.DocumentLockedError
		file_lock.create_lock(signature)
		netmanthan.local.locked_documents.append(self)

	def unlock(self):
		"""Delete the lock file for this document"""
		file_lock.delete_lock(self.get_signature())
		if self in netmanthan.local.locked_documents:
			netmanthan.local.locked_documents.remove(self)
		...
		if date_diff(self.get(to_date_field), self.get(from_date_field)) < 0:
			netmanthan.throw(
				_("{0} must be after {1}").format(
					netmanthan.bold(self.meta.get_label(to_date_field)),
					netmanthan.bold(self.meta.get_label(from_date_field)),
				),
				netmanthan.exceptions.InvalidDates,
			)

	def get_assigned_users(self):
		assigned_users = netmanthan.get_all(
			"ToDo",
			fields=["allocated_to"],
			filters={
	...
	def add_tag(self, tag):
		...
		from netmanthan.desk.doctype.tag.tag import DocTags
	...
	def deferred_insert(self) -> None:
		...
		from netmanthan.deferred_insert import deferred_insert
...
def execute_action(__doctype, __name, __action, **kwargs):
	...
	doc = netmanthan.get_doc(__doctype, __name)
	doc.unlock()
	try:
		...
		# add a comment (?)
		if netmanthan.local.message_log:
			msg = json.loads(netmanthan.local.message_log[-1]).get("message")
		else:
			msg = "<pre><code>" + netmanthan.get_traceback() + "</pre></code>"
