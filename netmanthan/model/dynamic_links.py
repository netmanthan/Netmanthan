...
def get_dynamic_link_map(for_delete=False):
	...
	"""
	if getattr(netmanthan.local, "dynamic_link_map", None) is None or netmanthan.flags.in_test:
		# Build from scratch
		dynamic_link_map = {}
		for df in get_dynamic_links():
			...
			if meta.issingle:
				...
				try:
					links = netmanthan.db.sql_list("""select distinct {options} from `tab{parent}`""".format(**df))
					for doctype in links:
						dynamic_link_map.setdefault(doctype, []).append(df)
				except netmanthan.db.TableMissingError:  # noqa: E722
					pass

		netmanthan.local.dynamic_link_map = dynamic_link_map
	return netmanthan.local.dynamic_link_map
...
def get_dynamic_links():
	...
	for query in dynamic_link_queries:
		df += netmanthan.db.sql(query, as_dict=True)
	return df
