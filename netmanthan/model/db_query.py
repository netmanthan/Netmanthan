...
from netmanthan.core.doctype.server_script.server_script_utils import get_server_script_map
from netmanthan.database.utils import DefaultOrderBy, FallBackDateTimeStr, NestedSetHierarchy
from netmanthan.model import get_permitted_fields, optional_fields
from netmanthan.model.meta import get_table_columns
from netmanthan.model.utils import is_virtual_doctype
from netmanthan.model.utils.user_settings import get_user_settings, update_user_settings
from netmanthan.query_builder.utils import Column
...
class DatabaseQuery:
	def __init__(self, doctype, user=None):
		...
		self.or_conditions = []
		self.fields = None
		self.user = user or netmanthan.session.user
	...
	def doctype_meta(self):
		if not hasattr(self, "_doctype_meta"):
			self._doctype_meta = netmanthan.get_meta(self.doctype)
	...
	def build_and_run(self):
		...
		if netmanthan.db.db_type == "postgres" and args.order_by and args.group_by:
	...
	def prepare_args(self):
		...
		for child in self.tables[1:]:
			...
			args.tables += f" {self.join} {child} on ({child}.parenttype = {netmanthan.db.escape(self.doctype)} and {child}.parent = {parent_name})"
	...
	def sanitize_fields(self):
		...
		def _raise_exception():
			netmanthan.throw(_("Use of sub-query or function is restricted"), netmanthan.DataError)
		...
		for field in self.fields:
			...
			if SUB_QUERY_PATTERN.match(field):
				...
				if function in blacklisted_functions:
					netmanthan.throw(
						_("Use of function {0} in field is restricted").format(function), exc=netmanthan.DataError
					)
			...
			if self.strict:
				if STRICT_FIELD_PATTERN.match(field):
					netmanthan.throw(_("Illegal SQL Query"))
	...
	def append_link_table(self, doctype, fieldname):
		...
		self.link_tables.append(
			netmanthan._dict(doctype=doctype, fieldname=fieldname, table_name=f"`tab{doctype}`")
		)
	...
	def _set_permission_map(self, doctype: str, parent_doctype: str | None = None):
		...
		val = netmanthan.has_permission(
			doctype,
			ptype=ptype,
			parent_doctype=parent_doctype or self.doctype,
		)
		if not val:
			netmanthan.flags.error_message = _("Insufficient Permission for {0}").format(netmanthan.bold(doctype))
			raise netmanthan.PermissionError(doctype)
	...
	def get_table_columns(self):
		...
		except netmanthan.db.TableMissingError:
			if self.ignore_ddl:
				return None
			else:
				raise
	...
	def apply_fieldlevel_read_permissions(self):
		...
		Note: Does not apply to `netmanthan.model.core_doctype_list`
		...
		Example:
		        ...
		        - Query: fields=["*"]
		        - Result: fields=["title", ...] // will also include netmanthan's meta field like `name`, `owner`, etc.
		"""
		if self.flags.ignore_permissions or not netmanthan.get_system_settings(
		...
		for i, field in enumerate(self.fields):
			...
			# labels / pseudo columns or netmanthan internals
			...
			# handle child / joined table fields
				...
				if wrap_grave_quotes(table) in self.query_tables:
					...
					raise netmanthan.PermissionError(ch_doctype)
	...
	def prepare_filter_condition(self, f):
		...
		from netmanthan.boot import get_additional_filters_from_hooks
		...
		if f.operator.lower() in NestedSetHierarchy:
			...
			lft, rgt = "", ""
			if f.value:
				lft, rgt = netmanthan.db.get_value(ref_doctype, f.value, ["lft", "rgt"])
			...
			fallback = "''"
			value = [netmanthan.db.escape((cstr(v.name) or "").strip(), percent=False) for v in result]
			if len(value):
				value = f"({', '.join(value)})"
			else:
				value = "('')"
			...
			fallback = "''"
			value = [netmanthan.db.escape((cstr(v) or "").strip(), percent=False) for v in values]
			if len(value):
				value = f"({', '.join(value)})"
			else:
				value = "('')"
			...
			if f.operator in (">", "<", ">=", "<=") and (f.fieldname in ("creation", "modified")):
				...
				value = netmanthan.db.format_date(f.value)
				fallback = "'0001-01-01'"

			elif (df and df.fieldtype == "Datetime") or isinstance(f.value, datetime):
				value = netmanthan.db.format_datetime(f.value)
				fallback = f"'{FallBackDateTimeStr}'"
			...
			if isinstance(f.value, Column):
				...
				quote = '"' if netmanthan.conf.db_type == "postgres" else "`"
				value = f"{tname}.{quote}{f.value.name}{quote}"

			# escape value
			elif escape and isinstance(value, str):
				value = f"{netmanthan.db.escape(value, percent=False)}"
		...
		if (
			...
			if f.operator.lower() == "like" and netmanthan.conf.get("db_type") == "postgres":
				f.operator = "ilike"
	...
	def build_match_conditions(self, as_condition=True) -> str | list:
		...
		if not self.user:
			self.user = netmanthan.session.user

		if not self.tables:
			self.extract_tables()

		role_permissions = netmanthan.permissions.get_role_permissions(self.doctype_meta, user=self.user)
		self.shared = netmanthan.share.get_shared(self.doctype, self.user)
		...
		if (
			...
			only_if_shared = True
			if not self.shared:
				netmanthan.throw(_("No permission to read {0}").format(_(self.doctype)), netmanthan.PermissionError)
			else:
			...
			if requires_owner_constraint(role_permissions):
				self.match_conditions.append(
					f"`tab{self.doctype}`.`owner` = {netmanthan.db.escape(self.user, percent=False)}"
				)

			# add user permission only if role has read perm
			elif role_permissions.get("read") or role_permissions.get("select"):
				# get user permissions
				user_permissions = netmanthan.permissions.get_user_permissions(self.user)
	...
	def get_share_condition(self):
		return (
			...
			+ f" in ({', '.join(netmanthan.db.escape(s, percent=False) for s in self.shared)})"
		)
	...
	def add_user_permissions(self, user_permissions):
		...
		for df in doctype_link_fields:
			...
			if user_permission_values:
				docs = []
				if netmanthan.get_system_settings("apply_strict_user_permissions"):
					condition = ""
				else:
				...
				if docs:
					values = ", ".join(netmanthan.db.escape(doc, percent=False) for doc in docs)
	...
	def get_permission_query_conditions(self):
		conditions = []
		condition_methods = netmanthan.get_hooks("permission_query_conditions", {}).get(self.doctype, [])
		if condition_methods:
			for method in condition_methods:
				c = netmanthan.call(netmanthan.get_attr(method), self.user)
				if c:
					conditions.append(c)

		permision_script_name = get_server_script_map().get("permission_query", {}).get(self.doctype)
		if permision_script_name:
			script = netmanthan.get_doc("Server Script", permision_script_name)
	...
	def validate_order_by_and_group_by(self, parameters: str):
		...
		if "select" in _lower and "from" in _lower:
			netmanthan.throw(_("Cannot use sub-query in order by"))
		...
		for field in parameters.split(","):
			...
			if full_field_name:
				...
				if tbl not in self.tables:
					if tbl.startswith("`"):
						tbl = tbl[4:-1]
					netmanthan.throw(_("Please select atleast 1 column from {0} to sort/group").format(tbl))

			if function in blacklisted_sql_functions:
				netmanthan.throw(_("Cannot use {0} in order/group by").format(field))
...
def check_parent_permission(parent, child_doctype):
	if parent:
		...
		if child_doctype and not (
                netmanthan.db.exists("DocField", {"parent": parent, "options": child_doctype})
                or netmanthan.db.exists("Custom Field", {"dt": parent, "options": child_doctype})
		):
			raise netmanthan.PermissionError

		if netmanthan.permissions.has_permission(parent):
			return
...
def has_any_user_permission_for_doctype(doctype, user, applicable_for):
	user_permissions = netmanthan.permissions.get_user_permissions(user=user)
...
def get_between_date_filter(value, df=None):
	...
	"""
	from_date = netmanthan.utils.nowdate()
	to_date = netmanthan.utils.nowdate()
	...
	if df and df.fieldtype == "Datetime":
		data = "'{}' AND '{}'".format(
			netmanthan.db.format_datetime(from_date),
			netmanthan.db.format_datetime(to_date),
		)
	else:
		data = f"'{netmanthan.db.format_date(from_date)}' AND '{netmanthan.db.format_date(to_date)}'"
...
def get_additional_filter_field(additional_filters_config, f, value):
	...
	f = netmanthan._dict(netmanthan.get_attr(additional_filter["get_field"])())
	if f.query_value:
		for option in f.options:
			option = netmanthan._dict(option)
