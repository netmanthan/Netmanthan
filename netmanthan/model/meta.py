...
from netmanthan.model.base_document import (
...
from netmanthan.utils import cast, cint, cstr
...
def get_meta(doctype, cached=True) -> "Meta":
	...
	if meta := netmanthan.cache().hget("doctype_meta", doctype):
		return meta

	meta = Meta(doctype)
	netmanthan.cache().hset("doctype_meta", doctype, meta)
	return meta
...
def get_table_columns(doctype):
	return netmanthan.db.get_table_columns(doctype)


def load_doctype_from_file(doctype):
	fname = netmanthan.scrub(doctype)
	with open(netmanthan.get_app_path("netmanthan", "core", "doctype", fname, fname + ".json")) as f:
...
class Meta(Document):
	...
	standard_set_once_fields = [
		netmanthan._dict(fieldname="creation", fieldtype="Datetime"),
		netmanthan._dict(fieldname="owner", fieldtype="Data"),
	]
	...
	def get_global_search_fields(self):
		...
		if getattr(self, "show_name_in_global_search", None):
			fields.append(netmanthan._dict(fieldtype="Data", fieldname="name", label="Name"))

		return fields

	def get_valid_columns(self):
		if not hasattr(self, "_valid_columns"):
			table_exists = netmanthan.db.table_exists(self.name)
	...
	def add_custom_fields(self):
		if not netmanthan.db.table_exists("Custom Field"):
			return

		custom_fields = netmanthan.db.get_values(
			"Custom Field",
	...
	def apply_property_setters(self):
		...
		"""
		if not netmanthan.db.table_exists("Property Setter"):
			return

		property_setters = netmanthan.db.get_values(
			"Property Setter",
	...
	def set_custom_permissions(self):
		...
		if netmanthan.flags.in_patch or netmanthan.flags.in_install:
			return

		if not self.istable and self.name not in ("DocType", "DocField", "DocPerm", "Custom DocPerm"):
			custom_perms = netmanthan.get_all(
				"Custom DocPerm",
				fields="*",
	...
	def get_fields_to_check_permissions(self, user_permission_doctypes):
		...
		if self.name in user_permission_doctypes:
			fields.append(netmanthan._dict({"label": "Name", "fieldname": "name", "options": self.name}))
	...
	def get_permitted_fieldnames(self, parenttype=None, *, user=None, permission_type="read"):
		...
		if not permission_type:
			permission_type = "select" if netmanthan.only_has_select_perm(self.name, user=user) else "read"
	...
	def get_permissions(self, parenttype=None):
		if self.istable and parenttype:
			...
			permissions = netmanthan.get_meta(parenttype).permissions
		else:
	...
	def get_dashboard_data(self):
		...
		implemented in other netmanthan applications via hooks.
		"""
		data = netmanthan._dict()
		if not self.custom:
			try:
				module = load_doctype_module(self.name, suffix="_dashboard")
				if hasattr(module, "get_data"):
					data = netmanthan._dict(module.get_data())
			except ImportError:
				pass
		...
		if not self.custom:
			for hook in netmanthan.get_hooks("override_doctype_dashboards", {}).get(self.name, []):
				data = netmanthan._dict(netmanthan.get_attr(hook)(data=data))
	...
	def get_web_template(self, suffix=""):
		...
		module_name = netmanthan.scrub(self.module)
		doctype = netmanthan.scrub(self.name)
		template_path = netmanthan.get_module_path(
...
def is_single(doctype):
	try:
		return netmanthan.db.get_value("DocType", doctype, "issingle")
	except IndexError:
		raise Exception("Cannot determine whether %s is single" % doctype)


def get_parent_dt(dt):
	if not netmanthan.is_table(dt):
		return ""

	return (
            netmanthan.db.get_value(
			"DocField",
			{"fieldtype": ("in", netmanthan.model.table_fields), "options": dt},
			"parent",
		)
            or ""
	)
...
def set_fieldname(field_id, fieldname):
	netmanthan.db.set_value("DocField", field_id, "fieldname", fieldname)
...
def get_field_currency(df, doc=None):
	...
	if not getattr(netmanthan.local, "field_currency", None):
		netmanthan.local.field_currency = netmanthan._dict()

	if not (
            netmanthan.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname)
            or (
                    doc.get("parent")
                    and netmanthan.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
		)
	):
		...
		if ":" in cstr(df.get("options")):
			...
			if len(split_opts) == 3 and doc.get(split_opts[1]):
				currency = netmanthan.get_cached_value(split_opts[0], doc.get(split_opts[1]), split_opts[2])
		else:
			...
			if doc.get("parenttype"):
				if currency:
					...
					if netmanthan.get_meta(doc.parenttype).has_field(df.get("options")):
						# only get_value if parent has currency field
						currency = netmanthan.db.get_value(doc.parenttype, doc.parent, df.get("options"))

		if currency:
			netmanthan.local.field_currency.setdefault((doc.doctype, ref_docname), netmanthan._dict()).setdefault(
				df.fieldname, currency
			)

	return netmanthan.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname) or (
            doc.get("parent")
            and netmanthan.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
	)
...
def get_field_precision(df, doc=None, currency=None):
	...
	from netmanthan.utils import get_number_format_info

	if df.precision:
		precision = cint(df.precision)

	elif df.fieldtype == "Currency":
		precision = cint(netmanthan.db.get_default("currency_precision"))
		if not precision:
			number_format = netmanthan.db.get_default("number_format") or "#,###.##"
			decimal_str, comma_str, precision = get_number_format_info(number_format)
	else:
		precision = cint(netmanthan.db.get_default("float_precision")) or 3
...
def get_default_df(fieldname):
	if fieldname in (default_fields + child_table_fields):
		if fieldname in ("creation", "modified"):
			return netmanthan._dict(fieldname=fieldname, fieldtype="Datetime")

		elif fieldname in ("idx", "docstatus"):
			return netmanthan._dict(fieldname=fieldname, fieldtype="Int")

		return netmanthan._dict(fieldname=fieldname, fieldtype="Data")
...
def trim_tables(doctype=None, dry_run=False, quiet=False):
	...
	for doctype in netmanthan.get_all("DocType", filters=filters, pluck="name"):
		try:
...
def trim_table(doctype, dry_run=True):
	netmanthan.cache().hdel("table_columns", f"tab{doctype}")
	ignore_fields = default_fields + optional_fields + child_table_fields
	columns = netmanthan.db.get_table_columns(doctype)
	fields = netmanthan.get_meta(doctype, cached=False).get_fieldnames_with_value()
	...
	if columns_to_remove and not dry_run:
		...
		netmanthan.db.sql_ddl(f"ALTER TABLE `tab{doctype}` {columns_to_remove}")
