...
from netmanthan.modules.patch_handler import _patch_mode
...
def sync_for(app_name, force=0, reset_permissions=False):
	...
	if app_name == "netmanthan":
		...
		netmanthan_PATH = netmanthan.get_app_path("netmanthan")
		...
		for core_module in [
			...
			files.append(os.path.join(netmanthan_PATH, "core", "doctype", core_module, f"{core_module}.json"))

		for custom_module in ["custom_field", "property_setter"]:
			files.append(
				os.path.join(netmanthan_PATH, "custom", "doctype", custom_module, f"{custom_module}.json")
			)

		for website_module in ["web_form", "web_template", "web_form_field", "portal_menu_item"]:
			files.append(
				os.path.join(netmanthan_PATH, "website", "doctype", website_module, f"{website_module}.json")
			)
		...
		for desk_module in [
			...
			files.append(os.path.join(netmanthan_PATH, "desk", "doctype", desk_module, f"{desk_module}.json"))

		for module_name, document_type in IMPORTABLE_DOCTYPES:
			file = os.path.join(netmanthan_PATH, module_name, "doctype", document_type, f"{document_type}.json")
			if file not in files:
				files.append(file)

	for module_name in netmanthan.local.app_modules.get(app_name) or []:
		folder = os.path.dirname(netmanthan.get_module(app_name + "." + module_name).__file__)
