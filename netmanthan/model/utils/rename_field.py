...
from netmanthan.utils.password import rename_password_field
...
def rename_field(doctype, old_fieldname, new_fieldname):
	...
	if not meta.issingle and not netmanthan.db.has_column(doctype, old_fieldname):
	...
	if new_field.fieldtype in table_fields:
		...
		if meta.issingle:
			...
			# copy field value
			netmanthan.db.sql(f"""update `tab{doctype}` set `{new_fieldname}`=`{old_fieldname}`""")
...
def update_reports(doctype, old_fieldname, new_fieldname):
	...
	for r in reports:
		...
		if report_dict.get("updated"):
			...
			netmanthan.db.sql("""update `tabReport` set `json`=%s where name=%s""", (new_val, r.name))


def update_users_report_view_settings(doctype, ref_fieldname, new_fieldname):
	user_report_cols = netmanthan.db.sql(
...
def update_user_settings(doctype, old_fieldname, new_fieldname):
	...
	user_settings = netmanthan.db.sql(
