...
def set_default(doc, key):
	...
	if not doc.is_default:
		netmanthan.db.set(doc, "is_default", 1)
...
def set_field_property(filters, key, value):
	...
	docs = [
		netmanthan.get_doc("DocType", d.parent)
		for d in netmanthan.get_all("DocField", fields=["parent"], filters=filters)
	]
...
class InvalidIncludePath(netmanthan.ValidationError):
	pass
...
def render_include(content):
	...
	for i in range(5):
		if "{% include" in content:
			...
			if not paths:
				netmanthan.throw(_("Invalid include path"), InvalidIncludePath)

			for path in paths:
				app, app_path = path.split("/", 1)
				with open(netmanthan.get_app_path(app, app_path), encoding="utf-8") as f:
					include = f.read()
...
def get_fetch_values(doctype, fieldname, value):
	...
	db_values = netmanthan.db.get_value(
...
def is_virtual_doctype(doctype: str):
	if netmanthan.db.has_column("DocType", "is_virtual"):
		return netmanthan.db.get_value("DocType", doctype, "is_virtual")
	return False
