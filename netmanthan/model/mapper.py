...
from netmanthan.model import child_table_fields, default_fields, table_fields
...
def make_mapped_doc(method, source_name, selected_children=None, args=None):
	...
	for hook in netmanthan.get_hooks("override_whitelisted_methods", {}).get(method, []):
	...
	if method not in netmanthan.whitelisted:
	...
	if args:
		netmanthan.flags.args = netmanthan._dict(json.loads(args))

	netmanthan.flags.selected_children = selected_children or None
...
def get_mapped_doc(
	...
	# main
	if not target_doc:
		target_doc = netmanthan.new_doc(table_maps[from_doctype]["doctype"])
	elif isinstance(target_doc, str):
		target_doc = netmanthan.get_doc(json.loads(target_doc))
	...
	if cached:
		source_doc = netmanthan.get_cached_doc(from_doctype, from_docname)
	else:
		source_doc = netmanthan.get_doc(from_doctype, from_docname)
		...
		for df in source_doc.meta.get_table_fields():
			...
			if table_map:
				for source_d in source_doc.get(df.fieldname):
					...
					if (
						netmanthan.flags.selected_children
						and (df.fieldname in netmanthan.flags.selected_children)
						and source_d.name not in netmanthan.flags.selected_children[df.fieldname]
					):
						continue
...
def map_fetch_fields(target_doc, df, no_copy_fields):
	...
	for fetch_df in target_doc.meta.get("fields", {"fetch_from": f"^{df.fieldname}."}):
		...
		if (
			...
			if not linked_doc:
				try:
					linked_doc = netmanthan.get_doc(df.options, target_doc.get(df.fieldname))
				except Exception:
					return
...
def map_child_doc(source_d, target_parent, table_map, source_parent=None):
	...
	target_d = netmanthan.new_doc(target_child_doctype, target_parent, target_parentfield)
