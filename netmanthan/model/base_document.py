...
from netmanthan.model.naming import set_new_name
from netmanthan.model.utils.link_count import notify_link_count
...
from netmanthan.utils.html_utils import unescape_html
...
def get_controller(doctype):
	...
	For `custom` type, returns `netmanthan.model.document.Document`.

	:param doctype: DocType name as string."""

	def _get_controller():
		from netmanthan.model.document import Document
		from netmanthan.utils.nestedset import NestedSet

		module_name, custom = netmanthan.db.get_value(
			"DocType", doctype, ("module", "custom"), cache=not netmanthan.flags.in_migrate
		) or ("Core", False)

		if custom:
			is_tree = netmanthan.db.get_value("DocType", doctype, "is_tree", ignore=True, cache=True)
			_class = NestedSet if is_tree else Document
		else:
			class_overrides = netmanthan.get_hooks("override_doctype_class")
			if class_overrides and class_overrides.get(doctype):
				import_path = class_overrides[doctype][-1]
				module_path, classname = import_path.rsplit(".", 1)
				module = netmanthan.get_module(module_path)
	...
	if netmanthan.local.dev_server or netmanthan.flags.in_migrate:
		return _get_controller()

	site_controllers = netmanthan.controllers.setdefault(netmanthan.local.site, {})
...
class BaseDocument:
	...
	@cached_property
	def meta(self):
		return netmanthan.get_meta(self.doctype)
	...
	def get_db_value(self, key):
		return netmanthan.db.get_value(self.doctype, self.name, key)
		...
		for fieldname in self.meta.get_valid_columns():
			...
			if df:
				if is_virtual_field:
					...
					if value is None:
						if (prop := getattr(type(self), fieldname, None)) and is_a_property(prop):
							...
							from netmanthan.utils.safe_exec import get_safe_globals

							value = netmanthan.safe_eval(
								code=options,
				...
				if isinstance(value, list) and df.fieldtype not in table_fields:
					netmanthan.throw(_("Value for {0} cannot be a list").format(_(df.label)))
	...
	def get_valid_columns(self) -> list[str]:
		if self.doctype not in netmanthan.local.valid_columns:
			if self.doctype in DOCTYPES_FOR_DOCTYPE:
				from netmanthan.model.meta import get_table_columns

				valid = get_table_columns(self.doctype)
			else:
				valid = self.meta.get_valid_columns()

			netmanthan.local.valid_columns[self.doctype] = valid

		return netmanthan.local.valid_columns[self.doctype]
	...
	def as_json(self):
		return netmanthan.as_json(self.as_dict())
	...
	def db_insert(self, ignore_if_duplicate=False):
		...
		if ignore_if_duplicate and netmanthan.db.db_type == "postgres":
			conflict_handler = "on conflict (name) do nothing"

		if not self.creation:
			self.creation = self.modified = now()
			self.created_by = self.modified_by = netmanthan.session.user
		...
		try:
			...
			if netmanthan.db.is_primary_key_violation(e):
				if self.meta.autoname == "hash":
					# hash collision? try again
					netmanthan.flags.retry_count = (netmanthan.flags.retry_count or 0) + 1
					if netmanthan.flags.retry_count > 5 and not netmanthan.flags.in_test:
						raise
					self.name = None
					self.db_insert()
					return
				...
				if not ignore_if_duplicate:
					netmanthan.msgprint(
						_("{0} {1} already exists").format(_(self.doctype), netmanthan.bold(self.name)),
						title=_("Duplicate Name"),
						indicator="red",
					)
					raise netmanthan.DuplicateEntryError(self.doctype, self.name, e)

			elif netmanthan.db.is_unique_key_violation(e):
				# unique constraint
	...
	def db_update(self):
		...
		try:
			...
			if netmanthan.db.is_unique_key_violation(e):
				self.show_unique_validation_message(e)
			else:
				raise
	...
	def show_unique_validation_message(self, e):
		if netmanthan.db.db_type != "postgres":
			...
			netmanthan.msgprint(_("{0} must be unique").format(label or fieldname))

		# this is used to preserve traceback
		raise netmanthan.UniqueValidationError(self.doctype, self.name, e)
	...
	def update_modified(self):
		...
		netmanthan.db.set_value(self.doctype, self.name, "modified", self.modified, update_modified=False)
		...
		for df in self.meta.get_link_fields() + self.meta.get(
			...
			if docname:
				if df.fieldtype == "Link":
					doctype = df.options
					if not doctype:
						netmanthan.throw(_("Options not set for link field {0}").format(df.fieldname))
				else:
					doctype = self.get(df.options)
					if not doctype:
						netmanthan.throw(_("{0} must be set first").format(self.meta.get_label(df.options)))
				...
				if not netmanthan.get_meta(doctype).get("is_virtual"):
					if not fields_to_fetch:
						# cache a single value type
						values = _dict(name=netmanthan.db.get_value(doctype, docname, "name", cache=True))
					else:
						values_to_fetch = ["name"] + [_df.fetch_from.split(".")[-1] for _df in fields_to_fetch]

						# don't cache if fetching other values too
						values = netmanthan.db.get_value(doctype, docname, values_to_fetch, as_dict=True)

				if getattr(netmanthan.get_meta(doctype), "issingle", 0):
					values.name = doctype

				if netmanthan.get_meta(doctype).get("is_virtual"):
					values = netmanthan.get_doc(doctype, docname).as_dict()
				...
				if values:
					...
					if not values.name:
						invalid_links.append((df.fieldname, docname, get_msg(df, docname)))
                            ...
                            and netmanthan.get_meta(doctype).is_submittable
                            and cint(netmanthan.db.get_value(doctype, docname, "docstatus")) == DocStatus.cancelled()
					):
	...
	def set_fetch_from_value(self, doctype, df, values):
		...
		if df.fieldtype in ["Small Text", "Text", "Data"]:
			from netmanthan.model.meta import get_default_df

			fetch_from_df = get_default_df(fetch_from_fieldname) or netmanthan.get_meta(doctype).get_field(
				fetch_from_fieldname
			)

			if not fetch_from_df:
				netmanthan.throw(
					_('Please check the value of "Fetch From" set for field {0}').format(netmanthan.bold(df.label)),
	...
	def _validate_selects(self):
		...
		for df in self.meta.get_select_fields():
			...
			if value not in options and not (netmanthan.flags.in_test and value.startswith("_T-")):
	...
	def _validate_data_fields(self):
		# data_field options defined in netmanthan.model.data_field_options
		for phone_field in self.meta.get_phone_fields():
			phone = self.get(phone_field.fieldname)
			netmanthan.utils.validate_phone_number_with_country_code(phone, phone_field.fieldname)
		...
		for data_field in self.meta.get_data_fields():
			...
			if data_field_options == "Email":
				if (self.owner in netmanthan.STANDARD_USERS) and (data in netmanthan.STANDARD_USERS):
					continue
				for email_address in netmanthan.utils.split_emails(data):
					netmanthan.utils.validate_email_address(email_address, throw=True)

			if data_field_options == "Name":
				netmanthan.utils.validate_name(data, throw=True)

			if data_field_options == "Phone":
				netmanthan.utils.validate_phone_number(data, throw=True)

			if data_field_options == "URL":
				if not data:
					continue

				netmanthan.utils.validate_url(data, throw=True)

	def _validate_constants(self):
		if netmanthan.flags.in_import or self.is_new() or self.flags.ignore_validate_constants:
			return

		constants = [d.fieldname for d in self.meta.get("fields", {"set_only_once": ("=", 1)})]
		if constants:
			values = netmanthan.db.get_value(self.doctype, self.name, constants, as_dict=True)
		...
		for fieldname in constants:
			...
			if self.get(fieldname) != value:
				netmanthan.throw(
					...
					netmanthan.CannotChangeConstantError,
				)
	...
	def _validate_length(self):
		...
		type_map = netmanthan.db.type_map
	...
	def _validate_code_fields(self):
		for field in self.meta.get_code_fields():
			...
			if language == "Python":
				netmanthan.utils.validate_python_code(code_string, fieldname=field.label, is_expression=False)

			elif language == "PythonExpression":
				netmanthan.utils.validate_python_code(code_string, fieldname=field.label)
	...
	def throw_length_exceeded_error(self, df, max_length, value):
		...
		netmanthan.throw(
			...
			netmanthan.CharacterLengthExceededError,
			title=_("Value too big"),
		)

	def _validate_update_after_submit(self):
		# get the full doc with children
		db_values = netmanthan.get_doc(self.doctype, self.name).as_dict()
				...
				if self_value != db_value:
					netmanthan.throw(
						_("{0} Not allowed to change {1} after submission from {2} to {3}").format(
							...
							netmanthan.bold(db_value),
							netmanthan.bold(self_value),
						),
						netmanthan.UpdateAfterSubmitError,
	...
	def _sanitize_content(self):
		...
		for fieldname, value in self.get_valid_dict(ignore_virtual=True).items():
			...
			value = netmanthan.as_unicode(value)
	...
	def _save_passwords(self):
		...
		from netmanthan.utils.password import remove_encrypted_password, set_encrypted_password
	...
	def get_formatted(
		...
		from netmanthan.utils.formatters import format_value
		...
		if (
			...
			currency = netmanthan.db.get_value("Currency", currency_value, cache=True)
	...
	def reset_values_if_no_permlevel_access(self, has_access_to, high_permlevel_fields):
		...
		if to_reset:
			if self.is_new():
				...
				ref_doc = netmanthan.new_doc(self.doctype)
			else:
	...
	def _extract_images_from_text_editor(self):
		from netmanthan.core.doctype.file.utils import extract_images_from_doc
...
def _filter(data, filters, limit=None):
	...
	for d in data:
		for f, fval in _filters.items():
			if not netmanthan.compare(getattr(d, f, None), fval[0], fval[1]):
				break
		else:
			out.append(d)
