...
from netmanthan.desk.doctype.tag.tag import delete_tags_for_document
...
from netmanthan.utils.file_manager import remove_all
from netmanthan.utils.global_search import delete_for_document
from netmanthan.utils.password import delete_all_passwords_for
...
def delete_doc(
	...
	# get from form
	if not doctype:
		doctype = netmanthan.form_dict.get("dt")
		name = netmanthan.form_dict.get("dn")
	...
	for name in names or []:
		if is_virtual:
			netmanthan.get_doc(doctype, name).delete()
			continue
		...
		if doctype == "DocType":
			if for_reload:
				...
				netmanthan.db.delete(
					"Custom Field", {"options": name, "fieldtype": ("in", netmanthan.model.table_fields)}
				)
				netmanthan.db.delete("__global_search", {"doctype": name})

			delete_from_table(doctype, name, ignore_doctypes, None)

			if (
				netmanthan.conf.developer_mode
				and not doc.custom
				and not (
                    for_reload or netmanthan.flags.in_migrate or netmanthan.flags.in_install or netmanthan.flags.in_uninstall
				)
			):
				try:
			...
			if not for_reload:
				...
				netmanthan.enqueue(
					"netmanthan.model.delete_doc.delete_dynamic_links",
					doctype=doc.doctype,
					name=doc.name,
...
def add_to_deleted_document(doc):
	...
	if doc.doctype != "Deleted Document" and netmanthan.flags.in_install != "netmanthan":
		netmanthan.get_doc(
			dict(
...
def delete_from_table(doctype: str, name: str, ignore_doctypes: list[str], doc):
	if doctype != "DocType" and doctype == name:
		netmanthan.db.delete("Singles", {"doctype": name})
	else:
		netmanthan.db.delete(doctype, {"name": name})
	if doc:
		child_doctypes = [
			d.options for d in doc.meta.get_table_fields() if netmanthan.get_meta(d.options).is_virtual == 0
		]
		...
		child_doctypes = netmanthan.get_all(
			"DocField",
			fields="options",
			filters={"fieldtype": ["in", netmanthan.model.table_fields], "parent": doctype},
			pluck="options",
		)

	child_doctypes_to_delete = set(child_doctypes) - set(ignore_doctypes)
	for child_doctype in child_doctypes_to_delete:
		netmanthan.db.delete(child_doctype, {"parenttype": doctype, "parent": name})
...
def check_permission_and_not_submitted(doc):
	...
	if (
		...
		and netmanthan.session.user != "Administrator"
		...
		netmanthan.msgprint(
			...
			raise_exception=netmanthan.PermissionError,
		)
...
def check_if_doc_is_linked(doc, method="Delete"):
	...
	"""
	from netmanthan.model.rename_doc import get_link_fields
	...
	for lf in link_fields:
		...
		if not issingle:
			...
			if netmanthan.get_meta(link_dt).istable:
				fields.extend(["parent", "parenttype"])

			for item in netmanthan.db.get_values(link_dt, {link_field: doc.name}, fields, as_dict=True):
				# available only in child table cases
				item_parent = getattr(item, "parent", None)
				linked_doctype = item.parenttype if item_parent else link_dt

				if linked_doctype in netmanthan.get_hooks("ignore_links_on_delete") or (
			...
			if netmanthan.db.get_value(link_dt, None, link_field) == doc.name:
				raise_link_exists_exception(doc, link_dt, link_dt)


def check_if_doc_is_dynamically_linked(doc, method="Delete"):
	"""Raise `netmanthan.LinkExistsError` if the document is dynamically linked"""
	for df in get_dynamic_link_map().get(doc.doctype, []):

		ignore_linked_doctypes = doc.get("ignore_linked_doctypes") or []

		if df.parent in netmanthan.get_hooks("ignore_links_on_delete") or (
			df.parent in ignore_linked_doctypes and method == "Cancel"
		):
			# don't check for communication and todo!
			continue

		meta = netmanthan.get_meta(df.parent)
		if meta.issingle:
			...
			for refdoc in netmanthan.db.sql(
...
def clear_timeline_references(link_doctype, link_name):
	netmanthan.db.delete("Communication Link", {"link_doctype": link_doctype, "link_name": link_name})
...
def delete_controllers(doctype, module):
	...
	dir_path = os.path.join(module_path, "doctype", netmanthan.scrub(doctype))
