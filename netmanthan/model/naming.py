...
from netmanthan.utils import cint, cstr, now_datetime
from netmanthan.utils.caching import redis_cache
...
class InvalidNamingSeriesError(netmanthan.ValidationError):
	pass
...
class NamingSeries:
	...
	def validate(self):
		if "." not in self.series:
			netmanthan.throw(
				_("Invalid naming series {}: dot (.) missing").format(netmanthan.bold(self.series)),
		...
		if not NAMING_SERIES_PATTERN.match(self.series):
			netmanthan.throw(
				...
				).format(netmanthan.bold(self.series)),
				exc=InvalidNamingSeriesError,
			)
	...
	def get_prefix(self) -> str:
		...
		if prefix is None:
			netmanthan.throw(_("Invalid Naming Series: {}").format(self.series))
	...
	def update_counter(self, new_count: int) -> None:
		...
		Series = netmanthan.qb.DocType("Series")
		prefix = self.get_prefix()

		# Initialize if not present in DB
		if netmanthan.db.get_value("Series", prefix, "name", order_by="name") is None:
			netmanthan.qb.into(Series).insert(prefix, 0).columns("name", "current").run()

		(
			netmanthan.qb.update(Series).set(Series.current, cint(new_count)).where(Series.name == prefix)
		).run()

	def get_current_value(self) -> int:
		prefix = self.get_prefix()
		return cint(netmanthan.db.get_value("Series", prefix, "current", order_by="name"))
...
def set_new_name(doc):
	...
	if autoname.lower() != "prompt" and not netmanthan.flags.in_import:
		doc.name = None

	if is_autoincremented(doc.doctype, meta):
		doc.name = netmanthan.db.get_next_sequence_val(doc.doctype)
		return
...
def _implicitly_auto_incremented(doctype) -> bool:
	...
	if netmanthan.db.db_type == "mariadb":
		...
		values = (netmanthan.db.db_name,)
	return netmanthan.db.sql(query, values)[0][0] == "bigint"
...
def set_name_from_naming_options(autoname, doc):
	...
	if _autoname.startswith("field:"):
		...
		if not doc.name:
			...
			netmanthan.throw(_("{0} is required").format(doc.meta.get_label(fieldname)))
...
def set_naming_from_document_naming_rule(doc):
	...
	"""
	from netmanthan.model.base_document import DOCTYPES_FOR_DOCTYPE
	...
	document_naming_rules = netmanthan.cache_manager.get_doctype_map(
	...
	for d in document_naming_rules:
		netmanthan.get_cached_doc("Document Naming Rule", d.name).apply(doc)
		if doc.name:
			break
...
def set_name_by_naming_series(doc):
	...
	if not doc.naming_series:
		netmanthan.throw(netmanthan._("Naming Series mandatory"))
...
def make_autoname(key="", doctype="", doc="", *, ignore_validate=False):
	...
	*Example:*
	...
	"""
	if key == "hash":
		return netmanthan.generate_hash(length=10)
...
def parse_naming_series(
	...
	for e in parts:
		...
		if e.startswith("#"):
			...
			part = netmanthan.get_attr(method[0])(doc, e)
		else:
			part = e
...
def has_custom_parser(e):
	...
	return netmanthan.get_hooks("naming_series_variables", {}).get(e)
...
def getseries(key, digits):
	# series created ?
	# Using netmanthan.qb as netmanthan.get_values does not allow order_by=None
	series = DocType("Series")
	current = (netmanthan.qb.from_(series).where(series.name == key).for_update().select("current")).run()

	if current and current[0][0] is not None:
		current = current[0][0]
		# yes, update it
		netmanthan.db.sql("UPDATE `tabSeries` SET `current` = `current` + 1 WHERE `name`=%s", (key,))
		current = cint(current) + 1
	else:
		# no, create it
		netmanthan.db.sql("INSERT INTO `tabSeries` (`name`, `current`) VALUES (%s, 1)", (key,))
		current = 1
...
def revert_series_if_last(key, name, doc=None):
	...
	*Example:*
	...
	series = DocType("Series")
	current = (
		netmanthan.qb.from_(series).where(series.name == prefix).for_update().select("current")
	).run()

	if current and current[0][0] == count:
		netmanthan.db.sql("UPDATE `tabSeries` SET `current` = `current` - 1 WHERE `name`=%s", prefix)


def get_default_naming_series(doctype: str) -> str | None:
	"""get default value for `naming_series` property"""
	naming_series_options = netmanthan.get_meta(doctype).get_naming_series_options()
...
def validate_name(doctype: str, name: int | str, case: str | None = None):
	...
	if not name:
		netmanthan.throw(_("No Name Specified for {0}").format(doctype))
	...
	if isinstance(name, int):
		if is_autoincremented(doctype):
			...
			netmanthan.db.set_next_sequence_val(doctype, name, is_val_used=True)
			return name

		netmanthan.throw(_("Invalid name type (integer) for varchar name column"), netmanthan.NameError)

	if name.startswith("New " + doctype):
		netmanthan.throw(
			_("There were some errors setting the name, please contact the administrator"), netmanthan.NameError
		)
	if case == "Title Case":
		name = name.title()
	if case == "UPPER CASE":
	...
	if not netmanthan.get_meta(doctype).get("issingle") and (doctype == name) and (name != "DocType"):
		netmanthan.throw(_("Name of {0} cannot be {1}").format(doctype, name), netmanthan.NameError)

	special_characters = "<>"
	if re.findall(f"[{special_characters}]+", name):
		message = ", ".join(f"'{c}'" for c in special_characters)
		netmanthan.throw(
			_("Name cannot contain special characters like {0}").format(message), netmanthan.NameError
		)
...
def append_number_if_name_exists(doctype, value, fieldname="name", separator="-", filters=None):
	...
	exists = netmanthan.db.exists(doctype, filters)

	regex = f"^{re.escape(value)}{separator}\\d+$"

	if exists:
		last = netmanthan.db.sql(
			...
			`{fieldname}` DESC LIMIT 1""".format(
				doctype=doctype, fieldname=fieldname, regex_character=netmanthan.db.REGEX_CHARACTER
			),
			regex,
		)
...
def _set_amended_name(doc):
	...
	if netmanthan.db.get_value(doc.doctype, doc.amended_from, "amended_from"):
...
def _prompt_autoname(autoname, doc):
	...
	if not doc.name:
		netmanthan.throw(_("Please set the document name"))
