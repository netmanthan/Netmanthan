...
import netmanthan
from netmanthan.website.utils import extract_title, get_frontmatter
...
def get_page_info_from_web_form(path):
	...
	rules, page_info = [], {}
	web_forms = netmanthan.get_all("Web Form", ["name", "route", "modified"], {"published": 1})
	for d in web_forms:
	...
	if end_point:
		if path.endswith("/list"):
			netmanthan.form_dict.is_list = True
		elif path.endswith("/new"):
			netmanthan.form_dict.is_new = True
		elif path.endswith("/edit"):
			netmanthan.form_dict.is_edit = True
		else:
			netmanthan.form_dict.is_read = True
		return page_info[end_point]
...
def evaluate_dynamic_routes(rules, path):
	...
	if hasattr(netmanthan.local, "request") and netmanthan.local.request.environ:
		urls = route_map.bind_to_environ(netmanthan.local.request.environ)
		try:
			...
			if args:
				...
				netmanthan.local.form_dict.update(args)
...
def get_pages(app=None):
	...
	def _build(app):
		...
		if app:
			...
			apps = netmanthan.local.flags.web_pages_apps or netmanthan.get_installed_apps()

		for app in apps:
			app_path = netmanthan.get_app_path(app)
	...
	return netmanthan.cache().get_value("website_pages", lambda: _build(app))
...
def get_pages_from_path(start, app, app_path):
	...
	if os.path.exists(start_path):
		for basepath, folders, files in os.walk(start_path):
			...
			if not "__init__.py" in files and netmanthan.conf.get("developer_mode"):
				open(os.path.join(basepath, "__init__.py"), "a").close()

			for fname in files:
				fname = netmanthan.utils.cstr(fname)
				if not "." in fname:
					continue
				...
				if extn in ("html", "xml", "js", "css", "md"):
					...
					# print netmanthan.as_json(pages[-1])
...
def setup_source(page_info):
	...
	if page_info.template.endswith((".md", ".html")):
		...
		if page_info.template.endswith(".md"):
			source = netmanthan.utils.md_to_html(source)
...
def get_base_template(path=None):
	...
	"""
	if not path:
		path = netmanthan.local.request.path

	base_template_map = netmanthan.get_hooks("base_template_map") or {}
...
def load_properties_from_controller(page_info):
	...
	module = netmanthan.get_module(page_info.controller)
	if not module:
		return
...
def get_doctypes_with_web_view():
	...
	def _get():
		...
		doctypes = netmanthan.get_hooks("website_generators")
		doctypes_with_web_view = netmanthan.get_all(
			"DocType", fields=["name", "module"], filters=dict(has_web_view=1)
		)
		module_app_map = netmanthan.local.module_app
		doctypes += [
			d.name
			for d in doctypes_with_web_view
			if module_app_map.get(netmanthan.scrub(d.module)) in installed_apps
		]
		return doctypes

	return netmanthan.cache().get_value("doctypes_with_web_view", _get)


def get_start_folders():
	return netmanthan.local.flags.web_pages_folders or ("www", "templates/pages")
