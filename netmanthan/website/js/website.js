...
netmanthan.provide("website");
netmanthan.provide("netmanthan.awesome_bar_path");
window.cur_frm = null;
...
$.extend(netmanthan, {
	...
	add_asset_to_head(link) {
		return new Promise((resolve) => {
			if (netmanthan._assets_loaded.includes(link)) return resolve();
			let el;
			...
			el.onload = () => {
				netmanthan._assets_loaded.push(link);
				resolve();
			};
		});
	},
	...
	call: function (opts) {
		...
		return $.ajax({
			...
			statusCode: opts.statusCode || {
				404: function () {
					netmanthan.msgprint(__("Not found"));
				},
				403: function () {
					netmanthan.msgprint(__("Not permitted"));
				},
				200: function (data) {
			...
			netmanthan.process_response(opts, data);
		});
	},
	...
	process_response: function (opts, data) {
		...
		if (data._server_messages) {
			...
			if (opts.error_msg) {
				...
				netmanthan.msgprint(server_messages);
			}
		}
	...
	send_message: function (opts, btn) {
		return netmanthan.call({
			type: "POST",
			method: "netmanthan.www.contact.send_message",
			btn: btn,
			args: opts,
	...
	has_permission: function (doctype, docname, perm_type, callback) {
		return netmanthan.call({
			type: "GET",
			method: "netmanthan.client.has_permission",
			no_spinner: true,
	...
	render_user: function () {
		if (netmanthan.is_user_logged_in()) {
			$(".btn-login-area").toggle(false);
			$(".logged-in").toggle(true);
			$(".user-image").attr("src", netmanthan.get_cookie("user_image"));

			$(".user-image-wrapper").html(
				netmanthan.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-sidebar").html(
				netmanthan.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-myaccount").html(
				netmanthan.avatar(null, "avatar-large", null, null, null, true)
			);
		}
	},
	freeze_count: 0,
	freeze: function (msg) {
		...
		netmanthan.freeze_count++;
	},
	unfreeze: function () {
		if (!netmanthan.freeze_count) return; // anything open?
		netmanthan.freeze_count--;
		if (!netmanthan.freeze_count) {
	...
	trigger_ready: function () {
		netmanthan.ready_events.forEach(function (fn) {
			fn();
		});
	},
	...
	bind_filters: function () {
		...
		$(".filter").each(function () {
			...
			var val = netmanthan.utils.get_url_arg(key).replace(/\+/g, " ");
	...
	bind_navbar_search: function () {
		netmanthan.get_navbar_search().on("keypress", function (e) {
			var val = $(this).val();
			if (e.which === 13 && val) {
				$(this).val("").blur();
				netmanthan.do_search(val);
				return false;
			}
		});
	},
	do_search: function (val) {
		var path =
			(netmanthan.awesome_bar_path && netmanthan.awesome_bar_path[location.pathname]) ||
			window.search_path ||
			location.pathname;
	...
	set_search_path: function (path) {
		netmanthan.awesome_bar_path[location.pathname] = path;
	},
	...
	is_user_logged_in: function () {
		return netmanthan.get_cookie("user_id") !== "Guest" && netmanthan.session.user !== "Guest";
	},
	...
	show_language_picker() {
		if (netmanthan.session.user === "Guest" && window.show_language_picker) {
			netmanthan
				.call("netmanthan.translate.get_all_languages", {
					with_language_name: true,
				})
				.then((res) => {
					let language_list = res.message;
					let language = netmanthan.get_cookie("preferred_language");
					let language_codes = [];
...
netmanthan.setup_search = function (target, search_scope) {
	...
	$input.on(
		...
		netmanthan.utils.debounce(() => {
			...
			netmanthan
				.call({
					method: "netmanthan.search.web_search",
					args: {
...
window.valid_email = function (id) {
	...
	// copied regex from netmanthan/utils.js validate_type
...
$(document).ready(function () {
	window.full_name = netmanthan.get_cookie("full_name");
	var logged_in = netmanthan.is_user_logged_in();
	$("#website-login").toggleClass("hide", logged_in ? true : false);
	$("#website-post-login").toggleClass("hide", logged_in ? false : true);
	$(".logged-in").toggleClass("hide", logged_in ? false : true);

	netmanthan.bind_navbar_search();

	// switch to app link
	if (netmanthan.get_cookie("system_user") === "yes" && logged_in) {
		netmanthan.add_switch_to_desk();
	}

	netmanthan.render_user();
	netmanthan.setup_lazy_images();
...
$(document).on("page-change", function () {
	...
	$.extend(netmanthan, netmanthan.get_cookies());
	netmanthan.session = { user: netmanthan.user_id };

	netmanthan.datetime.refresh_when();
	netmanthan.trigger_ready();
	netmanthan.bind_filters();
	netmanthan.highlight_code_blocks();
	netmanthan.add_link_to_headings();
	netmanthan.make_navbar_active();
	// scroll to hash
...
netmanthan.ready(function () {
	netmanthan.show_language_picker();
	netmanthan.setup_videos();
	netmanthan.socketio.init(window.socketio_port);
});
