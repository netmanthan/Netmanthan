...
from netmanthan.utils import cint, get_system_timezone, md_to_html
...
def can_cache(no_cache=False):
	if netmanthan.flags.force_website_cache:
		return True
	if netmanthan.conf.disable_website_cache or netmanthan.conf.developer_mode:
		return False
	if getattr(netmanthan.local, "no_cache", False):
		return False
	return not no_cache
...
def get_comment_list(doctype, name):
	comments = netmanthan.get_all(
		...
		or_filters=[["owner", "=", netmanthan.session.user], ["published", "=", 1]],
	)

	communications = netmanthan.get_all(
		"Communication",
		fields=[
			"name",
			"creation",
		...
		or_filters=[
			["recipients", "like", f"%{netmanthan.session.user}%"],
			["cc", "like", f"%{netmanthan.session.user}%"],
			["bcc", "like", f"%{netmanthan.session.user}%"],
		],
	)
...
def get_home_page():
	if netmanthan.local.flags.home_page and not netmanthan.flags.in_test:
		return netmanthan.local.flags.home_page
	...
	def _get_home_page():
		...
		if netmanthan.session.user != "Guest":
			# by role
			for role in netmanthan.get_roles():
				home_page = netmanthan.db.get_value("Role", role, "home_page")
				if home_page:
					break

			# portal default
			if not home_page:
				home_page = netmanthan.db.get_single_value("Portal Settings", "default_portal_home")
		...
		# global
		if not home_page:
			home_page = netmanthan.db.get_single_value("Website Settings", "home_page")

		if not home_page:
			home_page = "login" if netmanthan.session.user == "Guest" else "me"
	...
	return netmanthan.cache().hget("home_page", netmanthan.session.user, _get_home_page)


def get_home_page_via_hooks():
	home_page = None

	home_page_method = netmanthan.get_hooks("get_website_user_home_page")
	if home_page_method:
		home_page = netmanthan.get_attr(home_page_method[-1])(netmanthan.session.user)
	elif netmanthan.get_hooks("website_user_home_page"):
		home_page = netmanthan.get_hooks("website_user_home_page")[-1]

	if not home_page:
		role_home_page = netmanthan.get_hooks("role_home_page")
		if role_home_page:
			for role in netmanthan.get_roles():
	...
	if not home_page:
		home_page = netmanthan.get_hooks("home_page")
		if home_page:
...
def get_boot_data():
	return {
		"sysdefaults": {
			"float_precision": cint(netmanthan.get_system_settings("float_precision")) or 3,
			"date_format": netmanthan.get_system_settings("date_format") or "yyyy-mm-dd",
			"time_format": netmanthan.get_system_settings("time_format") or "HH:mm:ss",
		},
		"time_zone": {
			"system": get_system_timezone(),
			"user": netmanthan.db.get_value("User", netmanthan.session.user, "time_zone") or get_system_timezone(),
		},
	}


def is_signup_disabled():
	return netmanthan.db.get_single_value("Website Settings", "disable_signup", True)
...
def get_shade(color, percent=None):
	netmanthan.msgprint(_("get_shade method has been deprecated."))
	return color
...
def get_toc(route, url_prefix=None, app=None):
	...
	return netmanthan.get_template("templates/includes/full_index.html").render(
...
def get_full_index(route=None, app=None):
	...
	from netmanthan.website.router import get_pages

	if not netmanthan.local.flags.children_map:
		...
		children_map = netmanthan.cache().get_value("website_full_index", _build)

		netmanthan.local.flags.children_map = children_map

	return netmanthan.local.flags.children_map
...
def clear_cache(path=None):
	...
	for key in (
		...
		netmanthan.cache().delete_value(key)

	netmanthan.cache().delete_value("website_404")
	if path:
		netmanthan.cache().hdel("website_redirects", path)
		delete_page_cache(path)
	else:
		clear_sitemap()
		netmanthan.clear_cache("Guest")
		for key in (
			"portal_menu_items",
			"home_page",
			"website_route_rules",
	...
	for method in netmanthan.get_hooks("website_clear_cache"):
		netmanthan.get_attr(method)(path)
...
def get_sidebar_items(parent_sidebar, basepath=None):
	...
	hooks = netmanthan.get_hooks("look_for_sidebar_json")
	look_for_sidebar_json = hooks[0] if hooks else netmanthan.flags.look_for_sidebar
	...
	if not sidebar_items and parent_sidebar:
		sidebar_items = netmanthan.get_all(
...
def get_portal_sidebar_items():
	sidebar_items = netmanthan.cache().hget("portal_menu_items", netmanthan.session.user)
	if sidebar_items is None:
		sidebar_items = []
		roles = netmanthan.get_roles()
		portal_settings = netmanthan.get_doc("Portal Settings", "Portal Settings")
		...
		items_via_hooks = netmanthan.get_hooks("portal_menu_items")
		if items_via_hooks:
			for i in items_via_hooks:
				i["enabled"] = 1
			add_items(sidebar_items, items_via_hooks)

		netmanthan.cache().hset("portal_menu_items", netmanthan.session.user, sidebar_items)
...
def get_sidebar_items_from_sidebar_file(basepath, look_for_sidebar_json):
	...
	with open(sidebar_json_path) as sidebarfile:
		try:
			...
			netmanthan.throw("Invalid Sidebar JSON at " + sidebar_json_path)
...
def cache_html(func):
	...
	def cache_html_decorator(*args, **kwargs):
		if can_cache():
			html = None
			page_cache = netmanthan.cache().hget("website_page", args[0].path)
			if page_cache and netmanthan.local.lang in page_cache:
				html = page_cache[netmanthan.local.lang]
			if html:
				netmanthan.local.response.from_cache = True
				return html
		html = func(*args, **kwargs)
		context = args[0].context
		if can_cache(context.no_cache):
			page_cache = netmanthan.cache().hget("website_page", args[0].path) or {}
			page_cache[netmanthan.local.lang] = html
			netmanthan.cache().hset("website_page", args[0].path, page_cache)
...
def build_response(path, data, http_status_code, headers: dict | None = None):
	...
	response.headers["X-From-Cache"] = netmanthan.local.response.from_cache or False
...
def add_preload_for_bundled_assets(response):
	...
	for css in netmanthan.local.preload_assets["style"]:
		links.append(f"<{css}>; rel=preload; as=style")

	for js in netmanthan.local.preload_assets["script"]:
