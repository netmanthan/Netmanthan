...
from netmanthan.utils import get_datetime, now, quoted, strip_html
from netmanthan.utils.jinja import render_template
from netmanthan.utils.safe_exec import safe_exec
from netmanthan.website.doctype.website_slideshow.website_slideshow import get_slideshow
...
class WebPage(WebsiteGenerator):
	...
	def get_context(self, context):
		...
		if self.context_script:
			_locals = dict(context=netmanthan._dict())
	...
	def render_dynamic(self, context):
		...
		if is_jinja:
			netmanthan.flags.web_block_scripts = {}
			netmanthan.flags.web_block_styles = {}
			try:
	...
	def check_for_redirect(self, context):
		if "<!-- redirect:" in context.main_section:
			netmanthan.local.flags.redirect_location = (
	...
	def validate_dates(self):
		if self.end_date:
			if self.start_date and get_datetime(self.end_date) < get_datetime(self.start_date):
				netmanthan.throw(_("End Date cannot be before Start Date!"))
			...
			if self.published and now() > self.end_date:
				...
				netmanthan.msgprint(_("Clearing end date, as it cannot be in the past for published pages."))


def check_publish_status():
	# called via daily scheduler
	web_pages = netmanthan.get_all("Web Page", fields=["name", "published", "start_date", "end_date"])
	...
	for page in web_pages:
		...
		if page.published:
			...
			if (start_date and now_date < start_date) or (end_date and now_date > end_date):
				netmanthan.db.set_value("Web Page", page.name, "published", 0)
		else:
			# Publish pages that are inside the set date ranges
			if start_date:
				if not end_date or (end_date and now_date < end_date):
					netmanthan.db.set_value("Web Page", page.name, "published", 1)


def get_web_blocks_html(blocks):
	"""Converts a list of blocks into Raw HTML and extracts out their scripts for deduplication"""

	out = netmanthan._dict(html="", scripts={}, styles={})
	extracted_scripts = {}
	extracted_styles = {}
	for block in blocks:
		web_template = netmanthan.get_cached_doc("Web Template", block.web_template)
		rendered_html = netmanthan.render_template(
