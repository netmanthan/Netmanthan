...
test_records = netmanthan.get_test_records("Web Page")


class TestWebPage(netmanthanTestCase):
	def setUp(self):
		netmanthan.db.delete("Web Page")
		for t in test_records:
			netmanthan.get_doc(t).insert()
	...
	def test_dynamic_route(self):
		web_page = netmanthan.get_doc(
			dict(
				...
				content_type="HTML",
				dynamic_template=1,
				main_section_html="<div>{{ netmanthan.form_dict.doctype }}</div>",
			)
		).insert()
		try:
