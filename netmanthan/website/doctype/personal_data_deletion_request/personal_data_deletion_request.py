...
from netmanthan.utils import get_datetime, get_fullname, time_diff_in_hours
...
class PersonalDataDeletionRequest(Document):
	def __init__(self, *args, **kwargs):
		...
		self.user_data_fields = netmanthan.get_hooks("user_data_fields")
	...
	def autoname(self):
		from netmanthan.model.naming import set_name_from_naming_options
		...
		domain = netmanthan.local.site.replace("_", "-")
		...
		netmanthan.utils.validate_email_address(self.email, throw=True)

	def after_insert(self):
		self.send_verification_mail()

	def generate_url_for_confirmation(self):
		params = {"email": self.email, "name": self.name, "host_name": netmanthan.local.site}
		api = netmanthan.utils.get_url(
			"/api/method/netmanthan.website.doctype.personal_data_deletion_request"
	...
	def disable_user(self):
		user = netmanthan.get_doc("User", self.email)
		user.enabled = False
		user.save()
	...
	def send_verification_mail(self):
		...
		netmanthan.sendmail(
			...
			args={
				"email": self.email,
				"name": self.name,
				"host_name": netmanthan.utils.get_url(),
				"link": url,
			},
	...
	def notify_system_managers(self):
		...
		netmanthan.sendmail(
			...
			args={"user": self.email, "url": netmanthan.utils.get_url(self.get_url())},
	...
	def validate_data_anonymization(self):
		...
		if self.status != "Pending Approval":
			netmanthan.throw(_("This request has not yet been approved by the user."))
	...
	def generate_anonymization_dict(self, ref):
		...
		meta = netmanthan.get_meta(ref["doctype"])
	...
	def redact_doc(self, doc, ref):
		...
		if ref.get("rename") and doc["name"] != self.anon:
			netmanthan.rename_doc(ref["doctype"], doc["name"], self.anon, force=True, show_alert=False)
	...
	def _anonymize_data(self, email=None, anon=None, set_data=True, commit=False):
		...
		netmanthan.rename_doc("User", email, anon, force=True, show_alert=False)
	...
	def __redact_partial_match_data(self, doctype):
		...
		for df in netmanthan.get_meta(doctype["doctype"]).fields:
...
def process_data_deletion_request():
	auto_account_deletion = netmanthan.db.get_single_value("Website Settings", "auto_account_deletion")
	if auto_account_deletion < 1:
		return

	requests = netmanthan.get_all(
		"Personal Data Deletion Request", filters={"status": "Pending Approval"}, pluck="name"
	)

	for request in requests:
		doc = netmanthan.get_doc("Personal Data Deletion Request", request)
...
def confirm_deletion(email, name, host_name):
	...
	doc = netmanthan.get_doc("Personal Data Deletion Request", name)
	host_name = netmanthan.utils.get_url()
