...
from netmanthan.website.doctype.personal_data_download_request.personal_data_download_request import (
	get_user_data,
)


class TestRequestPersonalData(netmanthanTestCase):
	def setUp(self):
		create_user_if_not_exists(email="test_privacy@example.com")

	def tearDown(self):
		netmanthan.db.delete("Personal Data Download Request")

	def test_user_data_creation(self):
		user_data = json.loads(get_user_data("test_privacy@example.com"))
		contact_name = get_contact_name("test_privacy@example.com")
		expected_data = {"Contact": netmanthan.get_all("Contact", {"name": contact_name}, ["*"])}
	...
	def test_file_and_email_creation(self):
		netmanthan.set_user("test_privacy@example.com")
		download_request = netmanthan.get_doc(
		...
		file_count = netmanthan.db.count(
			"File",
			{
		...
		self.assertIn(netmanthan._("Download Your Data"), email_queue[0].message)

		netmanthan.db.delete("Email Queue")


def create_user_if_not_exists(email, first_name=None):
	netmanthan.delete_doc_if_exists("User", email)
	...
	user = netmanthan.get_doc(
		{
			...
			"birth_date": netmanthan.utils.now_datetime(),
		}
