...
from netmanthan.utils.verified_command import get_signed_params
...
class PersonalDataDownloadRequest(Document):
	...
	def generate_file_and_send_mail(self, personal_data):
		...
		file_link = (
                netmanthan.utils.get_url("/api/method/netmanthan.utils.file_manager.download_file")
                + "?"
                + get_signed_params({"file_url": f.file_url})
		)
		host_name = netmanthan.local.site
		netmanthan.sendmail(
...
def get_user_data(user):
	...
	hooks = netmanthan.get_hooks("user_data_fields")
	data = {}
	for hook in hooks:
		d = data.get(hook.get("doctype"), [])
		d += netmanthan.get_all(hook.get("doctype"), {hook.get("filter_by", "owner"): user}, ["*"])
		if d:
