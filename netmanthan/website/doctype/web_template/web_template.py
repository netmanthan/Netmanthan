...
from netmanthan.modules.export_file import get_module_path, scrub_dt_dn, write_document_file
...
class WebTemplate(Document):
	def validate(self):
		if self.standard and not (netmanthan.conf.developer_mode or netmanthan.flags.in_patch):
			netmanthan.throw(_("Enable developer mode to create a standard Web Template"))

		for field in self.fields:
			if not field.fieldname:
				field.fieldname = netmanthan.scrub(field.label)
	...
	def on_trash(self):
		if netmanthan.conf.developer_mode and self.standard:
	...
	def get_template_path(self):
		...
		file_name = netmanthan.scrub(self.name) + ".html"
	...
	def render(self, values=None):
		if not values:
			values = {}
		values = netmanthan.parse_json(values)
		values.update({"values": values})
		template = self.get_template(self.standard)

		return netmanthan.render_template(template, values)
