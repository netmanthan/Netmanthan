...
from netmanthan.utils import encode, get_request_site_address
from netmanthan.website.utils import get_boot_data
...
class WebsiteSettings(Document):
	...
	def validate_google_settings(self):
		if self.enable_google_indexing and not netmanthan.db.get_single_value("Google Settings", "enable"):
		...
		for idx, row in enumerate(self.route_redirects):
			try:
				...
				if not netmanthan.flags.in_migrate:
					netmanthan.throw(_("Invalid redirect regex in row #{}: {}").format(idx, str(e)))
	...
	def get_access_token(self):
		...
		if not self.indexing_refresh_token:
			button_label = netmanthan.bold(_("Allow API Indexing Access"))
...
def get_website_settings(context=None):
	...
	context = netmanthan._dict(context or {})
	settings: "WebsiteSettings" = netmanthan.get_cached_doc("Website Settings")
	...
	if context.disable_website_theme:
		context.theme = netmanthan._dict()

	else:
		from netmanthan.website.doctype.website_theme.website_theme import get_active_theme

		context.theme = get_active_theme() or netmanthan._dict()

	if not context.get("favicon"):
		context["favicon"] = "/assets/netmanthan/images/netmanthan-favicon.svg"
	...
	context.read_only_mode = netmanthan.flags.read_only
...
def get_auto_account_deletion():
	return netmanthan.db.get_single_value("Website Settings", "auto_account_deletion")
