...
from netmanthan.website.utils import (
	clear_cache,
	find_first_image,
	get_comment_list,
...
class BlogPost(WebsiteGenerator):
	...
	def make_route(self):
		if not self.route:
			return (
                    netmanthan.db.get_value("Blog Category", self.blog_category, "route")
                    + "/"
	...
	def validate(self):
		...
		if self.featured:
			if not self.meta_image:
				netmanthan.throw(_("A featured post must have a cover image"))
			self.reset_featured_for_other_blogs()

		self.set_read_time()

	def reset_featured_for_other_blogs(self):
		all_posts = netmanthan.get_all("Blog Post", {"featured": 1})
		for post in all_posts:
			netmanthan.db.set_value("Blog Post", post.name, "featured", 0)
	...
	def get_context(self, context):
		...
		context.enable_cta = not self.hide_cta and netmanthan.db.get_single_value(
			"Blog Settings", "show_cta_in_blog", cache=True
		)

		if self.blogger:
			context.blogger_info = netmanthan.get_doc("Blogger", self.blogger).as_dict()
		...
		context.category = netmanthan.db.get_value(
		...
		context.guest_allowed = netmanthan.db.get_single_value("Blog Settings", "allow_guest_to_comment")

	def fetch_cta(self):
		if netmanthan.db.get_single_value("Blog Settings", "show_cta_in_blog", cache=True):
			blog_settings = netmanthan.get_cached_doc("Blog Settings")
	...
	def fetch_social_links_info(self):
		if not netmanthan.db.get_single_value("Blog Settings", "enable_social_sharing", cache=True):
			return []

		url = netmanthan.local.site + "/" + self.route
	...
	def load_likes(self, context):
		...
		context.like_count = netmanthan.db.count("Comment", filters) or 0

		filters["comment_email"] = user

		if user == "Guest":
			filters["ip_address"] = netmanthan.local.request_ip

		context.like = netmanthan.db.count("Comment", filters) or 0
...
def get_list_context(context=None):
	...
	category = netmanthan.utils.escape_html(
        netmanthan.local.form_dict.blog_category or netmanthan.local.form_dict.category
	)
	if category:
	...
	elif netmanthan.local.form_dict.blogger:
		blogger = netmanthan.db.get_value("Blogger", {"name": netmanthan.local.form_dict.blogger}, "full_name")
		list_context.sub_title = _("Posts by {0}").format(blogger)
		list_context.title = blogger

	elif netmanthan.local.form_dict.txt:
		list_context.sub_title = _('Filtered by "{0}"').format(sanitize_html(netmanthan.local.form_dict.txt))
	...
	blog_settings = netmanthan.get_doc("Blog Settings").as_dict(no_default_fields=True)
...
def get_blog_categories():
	...
	post, category = netmanthan.qb.DocType("Blog Post"), netmanthan.qb.DocType("Blog Category")
	return (
		netmanthan.qb.from_(category)
		.select(category.name, category.route, category.title)
		.where(
			(category.published == 1)
			& ExistsCriterion(
				netmanthan.qb.from_(post)
				.select("name")
...
def get_blog_category(route):
	return netmanthan.db.get_value("Blog Category", {"name": route}, "title") or route
...
def get_blog_list(
	...
	if filters and filters.get("blogger"):
		conditions.append("t1.blogger=%s" % netmanthan.db.escape(filters.get("blogger")))

	if category:
		conditions.append("t1.blog_category=%s" % netmanthan.db.escape(category))

	if txt:
		conditions.append(
			'(t1.content like {0} or t1.title like {0}")'.format(netmanthan.db.escape("%" + txt + "%"))
		)

	if conditions:
		netmanthan.local.no_cache = 1
	...
	posts = netmanthan.db.sql(query, as_dict=1)
	...
	for post in posts:
		...
		post.category = netmanthan.db.get_value(
