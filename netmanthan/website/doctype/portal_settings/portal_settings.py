...
class PortalSettings(Document):
	...
	def sync_menu(self):
		...
		dirty = False
		for item in netmanthan.get_hooks("standard_portal_menu_items"):
			if item.get("role") and not netmanthan.db.exists("Role", item.get("role")):
				netmanthan.get_doc({"doctype": "Role", "role_name": item.get("role"), "desk_access": 0}).insert()
	...
	def remove_deleted_doctype_items(self):
		existing_doctypes = set(netmanthan.get_list("DocType", pluck="name"))
