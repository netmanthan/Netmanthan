netmanthan.ui.form.on("Web Form", {
	setup: function () {
		netmanthan.meta.docfield_map["Web Form Field"].fieldtype.formatter = (value) => {
			const prefix = {
				"Page Break": "--red-600",
		...
		netmanthan.meta.docfield_map["Web Form Field"].fieldname.formatter = (value) => {
			if (!value) return;
			return netmanthan.unscrub(value);
		};

		netmanthan.meta.docfield_map["Web Form List Column"].fieldname.formatter = (value) => {
			if (!value) return;
			return netmanthan.unscrub(value);
		};
	},
	...
	refresh: function (frm) {
		...
		frm.get_field("is_standard").toggle(netmanthan.boot.developer_mode);

		if (frm.doc.is_standard && !netmanthan.boot.developer_mode) {
			frm.set_read_only();
			frm.disable_save();
		}
	...
	validate: function (frm) {
		...
		if (!frm.doc.web_form_fields) {
			...
			netmanthan.throw(__("Atleast one field is required in Web Form Fields Table"));
		}
		...
		if (page_break_count >= 10) {
			netmanthan.throw(__("There can be only 9 Page Break fields in a Web Form"));
		}
	},
	...
	add_get_fields_button(frm) {
		frm.add_custom_button(__("Get Fields"), () => {
			let webform_fieldtypes = netmanthan.meta
...
netmanthan.ui.form.on("Web Form List Column", {
	fieldname: function (frm, doctype, name) {
		let doc = netmanthan.get_doc(doctype, name);
		let df = netmanthan.meta.get_docfield(frm.doc.doc_type, doc.fieldname);
		if (!df) return;
...
netmanthan.ui.form.on("Web Form Field", {
...
function get_fields_for_doctype(doctype) {
	return new Promise((resolve) => netmanthan.model.with_doctype(doctype, resolve)).then(() => {
		return netmanthan.meta.get_docfields(doctype).filter((df) => {
			return (
