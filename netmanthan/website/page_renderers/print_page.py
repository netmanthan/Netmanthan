...
class PrintPage(TemplatePage):
	...
	def can_render(self):
		...
		if len(parts) == 2:
			if netmanthan.db.exists("DocType", parts[0], True) and netmanthan.db.exists(parts[0], parts[1], True):
				return True
	...
	def render(self):
		parts = self.path.split("/", 1)
		netmanthan.form_dict.doctype = parts[0]
		netmanthan.form_dict.name = parts[1]
