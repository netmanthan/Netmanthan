...
import netmanthan
from netmanthan.website.page_renderers.base_template_page import BaseTemplatePage
from netmanthan.website.router import get_base_template, get_page_info
...
class TemplatePage(BaseTemplatePage):
	...
	def set_template_path(self):
		...
		for app in netmanthan.get_installed_apps(netmanthan_last=True):
	...
	def get_index_path_options(search_path):
		return (
			netmanthan.as_unicode(f"{search_path}{d}") for d in ("", ".html", ".md", "/index.html", "/index.md")
		)
	...
	def update_context(self):
		...
		self.context.build_version = netmanthan.utils.get_build_version()

		if self.pymodule_name:
			self.pymodule = netmanthan.get_module(self.pymodule_name)
		...
		self.context.developer_mode = netmanthan.conf.developer_mode
	...
	def render_template(self):
		if self.template_path.endswith("min.js"):
			...
			html = netmanthan.render_template(self.source, self.context, safe_render=safe_render)
	...
	def get_raw_template(self):
		return netmanthan.get_jloader().get_source(netmanthan.get_jenv(), self.context.template)[0]
	...
	def convert_from_markdown(self):
		if self.template_path.endswith(".md"):
			self.source = netmanthan.utils.md_to_html(self.source)
	...
	def set_standard_path(self, path):
		self.app = "netmanthan"
		self.app_path = netmanthan.get_app_path("netmanthan")
		self.path = path
	...
	def set_user_info(self):
		from netmanthan.utils.user import get_fullname_and_avatar

		info = get_fullname_and_avatar(netmanthan.session.user)
