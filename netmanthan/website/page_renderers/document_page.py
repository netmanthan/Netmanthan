...
class DocumentPage(BaseTemplatePage):
	...
	def search_in_doctypes_with_web_view(self):
		for doctype in get_doctypes_with_web_view():
			...
			try:
				self.docname = netmanthan.db.get_value(doctype, filters, "name")
				if self.docname:
	...
	@cache_html
	def get_html(self):
		self.doc = netmanthan.get_doc(self.doctype, self.docname)
		self.init_context()
		self.update_context()
		self.post_process_context()
		html = netmanthan.get_template(self.template_path).render(self.context)
		return html
