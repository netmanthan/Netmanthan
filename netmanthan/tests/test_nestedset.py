...
from netmanthan.utils.nestedset import (
...
class TestNestedSet(netmanthanTestCase):
	...
	def test_basic_tree(self):
		...
		min_lft = 1
		max_rgt = netmanthan.qb.from_("Test Tree DocType").select(Max(Field("rgt"))).run(pluck=True)[0]

		for record in records:
			lft, rgt, parent_test_tree_doctype = netmanthan.db.get_value(
				"Test Tree DocType",
			...
			if parent_test_tree_doctype:
				parent_lft, parent_rgt = netmanthan.db.get_value(
	...
	def test_recursion(self):
		leaf_node = netmanthan.get_doc("Test Tree DocType", {"some_fieldname": "Parent 2"})
	...
	def test_move_group_into_another(self):
		old_lft, old_rgt = netmanthan.db.get_value("Test Tree DocType", "Parent 2", ["lft", "rgt"])
		...
		# after move
		new_lft, new_rgt = netmanthan.db.get_value("Test Tree DocType", "Parent 2", ["lft", "rgt"])
	...
	def test_move_leaf_into_another_group(self):
		child_2 = netmanthan.get_doc("Test Tree DocType", "Child 2")

		# assert that child 2 is not already under parent 1
		parent_lft_old, parent_rgt_old = netmanthan.db.get_value(
		...
		parent_lft_new, parent_rgt_new = netmanthan.db.get_value(
			"Test Tree DocType", "Parent 2", ["lft", "rgt"]
		)
	...
	def test_delete_leaf(self):
		...
		child_1 = netmanthan.get_doc("Test Tree DocType", "Child 1")
		child_1.delete()
		records.remove(el)

		self.test_basic_tree()

		n = netmanthan.new_doc("Test Tree DocType")
		n.update(el)
		n.insert()
		records.append(el)
	...
	def test_delete_group(self):
		...
		with self.assertRaises(NestedSetChildExistsError):
			netmanthan.delete_doc("Test Tree DocType", "Parent 1")
	...
	def test_rename_nestedset(self):
		...
		# Rename doctype
		netmanthan.rename_doc("DocType", doctype.name, "Test " + random_string(10), force=True)

	def test_merge_groups(self):
		global records
		el = {"some_fieldname": "Parent 2", "parent_test_tree_doctype": "Root Node", "is_group": 1}
		netmanthan.rename_doc("Test Tree DocType", "Parent 2", "Parent 1", merge=True)
		records.remove(el)
		self.test_basic_tree()
	...
	def test_merge_leaf_into_group(self):
		with self.assertRaises(NestedSetInvalidMergeError):
			netmanthan.rename_doc("Test Tree DocType", "Child 1", "Parent 1", merge=True)

	def test_merge_group_into_leaf(self):
		with self.assertRaises(NestedSetInvalidMergeError):
			netmanthan.rename_doc("Test Tree DocType", "Parent 1", "Child 1", merge=True)
