...
UI_TEST_USER = "netmanthan@example.com"


def whitelist_for_tests(fn):
	if netmanthan.request and not (netmanthan.flags.in_test or getattr(netmanthan.local, "dev_server", 0)):
		netmanthan.throw("Cannot run UI tests. Use a development server with `bench start`")

	return netmanthan.whitelist()(fn)
...
def create_if_not_exists(doc):
	...
	for doc in docs:
		...
		filters = doc.copy()
		filters.pop("doctype")
		name = netmanthan.db.exists(doc.doctype, filters)
		if not name:
			d = netmanthan.get_doc(doc)
...
@whitelist_for_tests
def clear_notes():
	for note in netmanthan.get_all("Note", pluck="name"):
		netmanthan.delete_doc("Note", note, force=True)
	...
	doc = netmanthan.get_doc(
		{
			...
			"communication_date": netmanthan.utils.now_datetime(),
		}
	)
	doc.insert()
	return doc
...
@whitelist_for_tests
def setup_workflow():
	from netmanthan.workflow.doctype.workflow.test_workflow import create_todo_workflow
...
def create_contact_phone_nos_records():
	if netmanthan.get_all("Contact", {"first_name": "Test Contact"}):
		return

	doc = netmanthan.new_doc("Contact")
...
def create_contact_records():
	if netmanthan.get_all("Contact", {"first_name": "Test Form Contact 1"}):
		return
...
def create_multiple_todo_records():
	if netmanthan.get_all("ToDo", {"description": "Multiple ToDo 1"}):
		return

	values = [(f"100{i}", f"Multiple ToDo {i}") for i in range(1, 1002)]

	netmanthan.db.bulk_insert("ToDo", fields=["name", "description"], values=set(values))


def insert_contact(first_name, phone_number):
	doc = netmanthan.get_doc({"doctype": "Contact", "first_name": first_name})
...
def create_form_tour():
	if netmanthan.db.exists("Form Tour", {"name": "Test Form Tour"}):
		return

	tour = netmanthan.get_doc(
		{
			"doctype": "Form Tour",
...
def create_web_page(title, route, single_thread):
	web_page = netmanthan.db.exists("Web Page", {"route": route})
	if web_page:
		return web_page
	web_page = netmanthan.get_doc(
	...
	web_page.append(
		"page_blocks",
		{
			...
			"web_template_values": netmanthan.as_json(
				{
					"title": "Discussions",
...
def update_webform_to_multistep():
	if not netmanthan.db.exists("Web Form", "update-profile-duplicate"):
		doc = netmanthan.get_doc("Web Form", "edit-profile")
		_doc = netmanthan.copy_doc(doc)
...
def insert_doctype_with_child_table_record(name):
	if netmanthan.get_all(name, {"title": "Test Grid Search"}):
		return
...
def insert_translations():
	...
	for doc in translation:
		if not netmanthan.db.exists("doc"):
			netmanthan.get_doc(doc).insert()


@whitelist_for_tests
def create_blog_post():

	blog_category = netmanthan.get_doc(
		{"name": "general", "doctype": "Blog Category", "title": "general"}
	).insert(ignore_if_duplicate=True)

	blogger = netmanthan.get_doc(
		{
...
def create_test_user(username=None):
	...
	if netmanthan.db.exists("User", name):
		return

	user = netmanthan.new_doc("User")
	user.email = name
	user.first_name = "netmanthan"
	user.new_password = netmanthan.local.conf.admin_password
	...
	all_roles = set(netmanthan.get_all("Role", pluck="name"))
...
def setup_tree_doctype():
	netmanthan.delete_doc_if_exists("DocType", "Custom Tree")
	...
	if not netmanthan.db.exists("Custom Tree", "All Trees"):
		netmanthan.get_doc({"doctype": "Custom Tree", "tree": "All Trees"}).insert()


@whitelist_for_tests
def setup_image_doctype():
	netmanthan.delete_doc_if_exists("DocType", "Custom Image")
...
@whitelist_for_tests
def setup_inbox():
	netmanthan.db.delete("User Email")
...
def setup_default_view(view, force_reroute=None):
	netmanthan.delete_doc_if_exists("Property Setter", "Event-main-default_view")
	netmanthan.delete_doc_if_exists("Property Setter", "Event-main-force_re_route_to_default_view")
...
@whitelist_for_tests
def create_note():
	if not netmanthan.db.exists("Note", "Routing Test"):
		netmanthan.get_doc({"doctype": "Note", "title": "Routing Test"}).insert()


@whitelist_for_tests
def create_kanban():
	if not netmanthan.db.exists("Custom Field", "Note-kanban"):
		netmanthan.get_doc(
			{
				"is_system_generated": 0,
				"dt": "Note",
	...
	if not netmanthan.db.exists("Kanban Board", "_Note _Kanban"):
		netmanthan.get_doc(
			{
...
def create_todo_with_attachment_limit(description):
	...
	return netmanthan.get_doc({"doctype": "ToDo", "description": description}).insert()


@whitelist_for_tests
def create_admin_kanban():
	if not netmanthan.db.exists("Kanban Board", "Admin Kanban"):
		netmanthan.get_doc(
			{
				"doctype": "Kanban Board",
				"name": "Admin Kanban",
