...
import netmanthan
from netmanthan.model.base_document import BaseDocument
from netmanthan.utils import cint

datetime_like_types = (datetime.datetime, datetime.date, datetime.time, datetime.timedelta)


class netmanthanTestCase(unittest.TestCase):
	"""Base test class for netmanthan tests.
	...
	def setUpClass(cls) -> None:
		cls.TEST_SITE = getattr(netmanthan.local, "site", None) or cls.TEST_SITE
		cls.ADMIN_PASSWORD = netmanthan.get_conf(cls.TEST_SITE).admin_password
		# flush changes done so far to avoid flake
		netmanthan.db.commit()
		if cls.SHOW_TRANSACTION_COMMIT_WARNINGS:
			netmanthan.db.add_before_commit(_commit_watcher)

		# enqueue teardown actions (executed in LIFO order)
		cls.addClassCleanup(_restore_thread_locals, copy.deepcopy(netmanthan.local.flags))
	...
	# --- netmanthan Framework specific assertions
	...
	def assertQueryCount(self, count):
		...
		def _sql_with_count(*args, **kwargs):
			...
			queries.append(netmanthan.db.last_query)
			return ret

		try:
			orig_sql = netmanthan.db.sql
			netmanthan.db.sql = _sql_with_count
			yield
			self.assertLessEqual(len(queries), count, msg="Queries executed: " + "\n\n".join(queries))
		finally:
			netmanthan.db.sql = orig_sql
	...
	def assertRowsRead(self, count):
		...
		def _sql_with_count(*args, **kwargs):
			...
			rows_read += cint(netmanthan.db._cursor.rowcount)
			return ret
...
def _rollback_db():
	netmanthan.local.before_commit = []
	netmanthan.local.rollback_observers = []
	netmanthan.db.value_cache = {}
	netmanthan.db.rollback()


def _restore_thread_locals(flags):
	netmanthan.local.flags = flags
	netmanthan.local.error_log = []
	netmanthan.local.message_log = []
	netmanthan.local.debug_log = []
	netmanthan.local.realtime_log = []
	netmanthan.local.conf = netmanthan._dict(netmanthan.get_site_config())
	netmanthan.local.cache = {}
	netmanthan.local.lang = "en"
	netmanthan.local.preload_assets = {"style": [], "script": []}
...
def change_settings(doctype, settings_dict):
	...
	try:
		settings = netmanthan.get_doc(doctype)
		# remember setting
		...
		netmanthan.db.value_cache[settings] = {}
...
def patch_hooks(overridden_hoooks):
	get_hooks = netmanthan.get_hooks
	...
	with patch.object(netmanthan, "get_hooks", patched_hooks):
		yield
