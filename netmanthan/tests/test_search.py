...
from netmanthan.desk.search import get_names_for_mentions, search_link, search_widget
from netmanthan.tests.utils import netmanthanTestCase
from netmanthan.utils import set_request
from netmanthan.website.serve import get_response


class TestSearch(netmanthanTestCase):
	def setUp(self):
	...
	def test_search_field_sanitizer(self):
		...
		result = netmanthan.response["results"][0]
	...
	def test_only_enabled_in_mention(self):
		...
		netmanthan.delete_doc("User", email)
	...
	def test_link_field_order(self):
		...
		result = netmanthan.response["results"]
	...
	def test_link_search_in_foreign_language(self):
		try:
			...
			output = netmanthan.response["values"]
	...
	def test_validate_and_sanitize_search_inputs(self):
		...
		# should not fail if function is called via netmanthan.call with extra arguments
		args = ("Random DocType", "Random", "email", "2", "10", dict())
		kwargs = {"as_dict": False}
		self.assertListEqual(netmanthan.call("netmanthan.tests.test_search.get_data", *args, **kwargs), [])

		# should not fail if query has @ symbol in it
		search_link("User", "user@random", searchfield="name")
		self.assertListEqual(netmanthan.response["results"], [])
	...
	def test_reference_doctype(self):
		...
		search_link(
			...
			query="netmanthan.tests.test_search.query_with_reference_doctype",
		)
		self.assertListEqual(netmanthan.response["results"], [])
...
def setup_test_link_field_order(TestCase):
	...
	# Create Tree doctype
	if not netmanthan.db.exists("DocType", TestCase.tree_doctype_name):
		TestCase.tree_doc = netmanthan.get_doc(
			{
				"doctype": "DocType",
		...
		TestCase.tree_doc = netmanthan.get_doc("DocType", TestCase.tree_doctype_name)

	# Create root for the tree doctype
	if not netmanthan.db.exists(TestCase.tree_doctype_name, {"random": TestCase.parent_doctype_name}):
		netmanthan.get_doc(
...
class TestWebsiteSearch(netmanthanTestCase):
