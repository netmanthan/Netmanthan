...
from netmanthan.translate import (
	extract_javascript,
...
first_lang, second_lang, third_lang, fourth_lang, fifth_lang = choices(
	...
	netmanthan.get_all("Language", pluck="name", filters=[["name", "not like", "en%"]]),
	k=5,
)


class TestTranslate(netmanthanTestCase):
	...
	def tearDown(self):
		netmanthan.form_dict.pop("_lang", None)
		if self._testMethodName in self.guest_sessions_required:
			netmanthan.set_user("Administrator")

	def test_extract_message_from_file(self):
		data = netmanthan.translate.get_messages_from_file(translation_string_file)
		exp_filename = "apps/netmanthan/netmanthan/tests/translation_test_file.txt"
	...
	def test_translation_with_context(self):
		try:
			netmanthan.local.lang = "fr"
			self.assertEqual(_("Change"), "Changement")
			self.assertEqual(_("Change", context="Coins"), "la monnaie")
		finally:
			netmanthan.local.lang = "en"

	def test_request_language_resolution_with_form_dict(self):
		"""Test for netmanthan.translate.get_language

		Case 1: netmanthan.form_dict._lang is set
		"""

		netmanthan.form_dict._lang = first_lang

		with patch.object(netmanthan.translate, "get_preferred_language_cookie", return_value=second_lang):
	...
	def test_request_language_resolution_with_cookie(self):
		...
		Case 2: netmanthan.form_dict._lang is not set, but preferred_language cookie is
		"""

		with patch.object(netmanthan.translate, "get_preferred_language_cookie", return_value="fr"):
	...
	def test_guest_request_language_resolution_with_cookie(self):
		...
		Case 3: netmanthan.form_dict._lang is not set, but preferred_language cookie is [Guest User]
		"""
	...
	def test_guest_request_language_resolution_with_request_header(self):
		...
		Case 4: netmanthan.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is [Guest User]
		"""
	...
	def test_request_language_resolution_with_request_header(self):
		...
		Case 5: netmanthan.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is
		"""
	...
	def test_load_all_translate_files(self):
		...
		verify_translation_files("netmanthan")
	...
	def test_python_extractor(self):
		...
		code = textwrap.dedent(
			...
			netmanthan._("attr")
			_("name")
			netmanthan._("attr with", context="attr context")
...
def verify_translation_files(app):
	...
	translations_dir = Path(netmanthan.get_app_path(app)) / "translations"
