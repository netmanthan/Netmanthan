...
import netmanthan
from netmanthan.integrations.oauth2 import encode_params
...
class TestOAuth20(unittest.TestCase):
	def setUp(self):
		...
		client = netmanthan.get_all("OAuth Client", fields=["*"])[0]
		...
		# Set netmanthan server URL reqired for id_token generation
		try:
			netmanthan_login_key = netmanthan.get_doc("Social Login Key", "netmanthan")
		...
		netmanthan_login_key.base_url = netmanthan.utils.get_url()
	...
	def test_login_using_authorization_code(self):
		...
		try:
			session.get(
				get_full_url("/api/method/netmanthan.integrations.oauth2.authorize"),
		...
		token_response = requests.post(
			get_full_url("/api/method/netmanthan.integrations.oauth2.get_token"),
	...
	def test_revoke_token(self):
		client = netmanthan.get_doc("OAuth Client", self.client_id)
		...
		revoke_token_response = requests.post(
			get_full_url("/api/method/netmanthan.integrations.oauth2.revoke_token"),
			headers=self.form_header,
	...
	def test_login_using_implicit_token(self):
		oauth_client = netmanthan.get_doc("OAuth Client", self.client_id)
	...
	def test_openid_code_id_token(self):
		...
		nonce = netmanthan.generate_hash()
...
def check_valid_openid_response(access_token=None):
	...
	openid_response = requests.get(
		get_full_url("/api/method/netmanthan.integrations.oauth2.openid_profile"), headers=headers
	)
...
def get_full_url(endpoint):
	...
	return urljoin(netmanthan.utils.get_url(), endpoint)


def update_client_for_auth_code_grant(client_id):
	client = netmanthan.get_doc("OAuth Client", client_id)
	client.grant_type = "Authorization Code"
	client.response_type = "Code"
	client.save()
	netmanthan.db.commit()
