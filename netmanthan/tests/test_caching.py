...
from netmanthan.utils.caching import redis_cache, request_cache, site_cache
...
@site_cache
def ping() -> str:
	register_with_external_service(netmanthan.local.site)
	return netmanthan.local.site
...
class TestCachingUtils(netmanthanTestCase):
	def test_request_cache(self):
		...
		acceptable_args = [
			...
			range(10),
			{"abc": "test-key"},
			netmanthan.get_last_doc("DocType"),
			netmanthan._dict(),
		]
...
class TestSiteCache(netmanthanAPITestCase):
...
class TestRedisCache(netmanthanAPITestCase):
