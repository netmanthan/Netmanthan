...
from netmanthan.database.utils import DefaultOrderBy
...
from netmanthan.permissions import add_user_permission, clear_user_permissions_for_doctype
from netmanthan.query_builder import Column
...
def enable_permlevel_restrictions():
	netmanthan.db.set_single_value("System Settings", "apply_perm_level_on_api_calls", 1)
	yield
	netmanthan.db.set_single_value("System Settings", "apply_perm_level_on_api_calls", 0)


class TestReportview(netmanthanTestCase):
	def setUp(self):
	...
	def test_child_table_field_syntax(self):
		note = netmanthan.get_doc(
			doctype="Note",
			title=f"Test {netmanthan.utils.random_string(8)}",
			content="test",
	...
	def test_child_table_join(self):
		netmanthan.delete_doc_if_exists("DocType", "Parent DocType 1")
		netmanthan.delete_doc_if_exists("DocType", "Parent DocType 2")
		netmanthan.delete_doc_if_exists("DocType", "Child DocType")
		# child table
		netmanthan.get_doc(
			{
				"doctype": "DocType",
		...
		# clear records
		netmanthan.db.delete("Parent DocType 1")
		netmanthan.db.delete("Parent DocType 2")
		netmanthan.db.delete("Child DocType")
		...
		# test query
		results1 = netmanthan.get_all("Parent DocType 1", fields=["name", "child.title as child_title"])
		results2 = netmanthan.get_all("Parent DocType 2", fields=["name", "child.title as child_title"])
	...
	def test_build_match_conditions(self):
		...
		test2user = netmanthan.get_doc("User", "test2@example.com")
	...
	def test_none_filter(self):
		query = netmanthan.qb.get_query("DocType", fields="name", filters={"restrict_to_domain": None})
	...
	def test_query_fields_sanitizer(self):
		...
		with self.assertRaises(netmanthan.DataError):
			DatabaseQuery("DocType").execute(
				fields=["name", "issingle", "if (issingle=1, (select name from tabUser), count(name))"],
				limit_start=0,
				limit_page_length=1,
	...
	def test_nested_permission(self):
		...
		from netmanthan.core.page.permission_manager.permission_manager import update
	...
	def test_of_not_of_descendant_ancestors(self):
		...
		data = netmanthan.get_all("Nested DocType", {"name": ("descendants of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)

		data = netmanthan.get_all("Nested DocType", {"name": ("descendants of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = netmanthan.get_all("Nested DocType", {"name": ("ancestors of", "Level 2 A")})
		...
		data = netmanthan.get_all("Nested DocType", {"name": ("ancestors of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = netmanthan.get_all("Nested DocType", {"name": ("not descendants of", "Level 2 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		...
		data = netmanthan.get_all("Nested DocType", {"name": ("not descendants of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		...
		data = netmanthan.get_all("Nested DocType", {"name": ("not ancestors of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = netmanthan.get_all("Nested DocType", {"name": ("not ancestors of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = netmanthan.get_all("Nested DocType", {"name": ("ancestors of", "Root")})
		self.assertTrue(len(data) == 0)
		self.assertTrue(
			len(netmanthan.get_all("Nested DocType", {"name": ("not ancestors of", "Root")}))
			== len(netmanthan.get_all("Nested DocType"))
		)
	...
	def test_is_set_is_not_set(self):
		...
		netmanthan.db.set_value("DocType", "Property Setter", "autoname", None, update_modified=False)
	...
	def test_prepare_select_args(self):
		# netmanthan.get_all inserts modified field into order_by clause
	...
	def test_column_comparison(self):
		...
		users_unedited = netmanthan.get_all(
			"User",
			filters={"creation": Column("modified")},
			fields=["name", "creation", "modified"],
			limit=1,
		)
		users_edited = netmanthan.get_all(
			"User",
			filters={"creation": ("!=", Column("modified"))},
			fields=["name", "creation", "modified"],
			limit=1,
	...
	def test_reportview_get_permlevel_system_users(self):
		with setup_patched_blog_post(), setup_test_user(set_user=True):
			...
			response = execute_cmd("netmanthan.desk.reportview.get")
	...
	def test_cast_name(self):
		...
		netmanthan.delete_doc_if_exists("DocType", "autoinc_dt_test")
	...
	def test_fieldname_starting_with_int(self):
		...
		netmanthan.delete_doc_if_exists("DocType", "dt_with_int_named_fieldname")
		netmanthan.delete_doc_if_exists("DocType", "table_dt")
		...
		dt_data = netmanthan.get_doc({"doctype": "dt_with_int_named_fieldname", "1field": "10"}).insert(
			ignore_permissions=True
		)
	...
	def test_permission_query_condition(self):
		from netmanthan.desk.doctype.dashboard_settings.dashboard_settings import create_dashboard_settings
		...
		dashboard_settings = netmanthan.db.sql(
			"""
				SELECT name
	...
	def test_virtual_doctype(self):
		...
		with patch("netmanthan.controllers", new={netmanthan.local.site: {"Virtual DocType": VirtualDocType}}):
			VirtualDocType.get_list = MagicMock()

			netmanthan.get_all("Virtual DocType", filters={"name": "test"}, fields=["name"], limit=1)
	...
	def test_coalesce_with_in_ops(self):
		self.assertNotIn("ifnull", netmanthan.get_all("User", {"name": ("in", ["a", "b"])}, run=0))
		self.assertIn("ifnull", netmanthan.get_all("User", {"name": ("in", ["a", None])}, run=0))
		self.assertIn("ifnull", netmanthan.get_all("User", {"name": ("in", ["a", ""])}, run=0))
		self.assertIn("ifnull", netmanthan.get_all("User", {"name": ("in", [])}, run=0))
		self.assertIn("ifnull", netmanthan.get_all("User", {"name": ("not in", ["a"])}, run=0))
		self.assertIn("ifnull", netmanthan.get_all("User", {"name": ("not in", [])}, run=0))
		self.assertIn("ifnull", netmanthan.get_all("User", {"name": ("not in", [""])}, run=0))
...
class TestReportView(netmanthanTestCase):
	...
	def test_get_count(self):
		...
		list_filter_response = execute_cmd("netmanthan.desk.reportview.get_count")
		netmanthan.local.form_dict = netmanthan._dict(
			{"doctype": "DocType", "filters": {"show_title_field_in_link": 1}, "distinct": "true"}
		)
		dict_filter_response = execute_cmd("netmanthan.desk.reportview.get_count")
		...
		child_filter_response = execute_cmd("netmanthan.desk.reportview.get_count")
		current_value = netmanthan.db.sql(
	...
	def test_reportview_get_aggregation(self):
		...
		netmanthan.local.request = netmanthan._dict(method="GET")
		netmanthan.local.form_dict = netmanthan._dict(
			{
				"doctype": "DocType",
...
def add_child_table_to_blog_post():
	child_table = netmanthan.get_doc(
		{
			"doctype": "DocType",
			"istable": 1,
			"custom": 1,
...
def create_nested_doctype():
	if netmanthan.db.exists("DocType", "Nested DocType"):
		return
...
def create_nested_doctype_records():
	...
	for r in records:
		d = netmanthan.new_doc("Nested DocType")
		d.update(r)
