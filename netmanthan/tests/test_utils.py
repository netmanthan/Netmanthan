...
import netmanthan
from netmanthan.installer import parse_app_name
...
from netmanthan.utils.data import (
	add_to_date,
	cast,
	cstr,
...
from netmanthan.utils.dateutils import get_dates_from_timegrain
from netmanthan.utils.diff import _get_value_from_version, get_version_diff, version_query
from netmanthan.utils.image import optimize_image, strip_exif_data
from netmanthan.utils.response import json_handler
from netmanthan.utils.synchronization import LockTimeoutError, filelock


class TestFilters(netmanthanTestCase):
...
class TestMoney(netmanthanTestCase):
...
class TestDataManipulation(netmanthanTestCase):
	def test_scrub_urls(self):
		html = """
			...
			<div class="more-info">
				...
				<img src="/assets/netmanthan/test.jpg">
			</div>
			<div style="background-image: url('/assets/netmanthan/bg.jpg')">
		...
		self.assertTrue(f'<img src="{url}/assets/netmanthan/test.jpg">' in html)
		self.assertTrue(
			f"style=\"background-image: url('{url}/assets/netmanthan/bg.jpg') !important\"" in html
		)
		self.assertTrue('<a href="mailto:test@example.com">email</a>' in html)


class TestFieldCasting(netmanthanTestCase):
...
class TestMathUtils(netmanthanTestCase):
	def test_floor(self):
...
class TestHTMLUtils(netmanthanTestCase):
	def test_clean_email_html(self):
		from netmanthan.utils.html_utils import clean_email_html
	...
	def test_sanitize_html(self):
		from netmanthan.utils.html_utils import sanitize_html
...
class TestValidationUtils(netmanthanTestCase):
	def test_valid_url(self):
		...
		self.assertTrue(validate_url("http://netmanthan.io", throw=True))
		...
		self.assertRaises(netmanthan.ValidationError, validate_url, "netmanthan", throw=True)

		# Scheme validation
		self.assertFalse(validate_url("https://google.com", valid_schemes="http"))
		self.assertTrue(validate_url("ftp://netmanthan.cloud", valid_schemes=["https", "ftp"]))
		self.assertFalse(
			validate_url("bolo://netmanthan.io", valid_schemes=("http", "https", "ftp", "ftps"))
		)
		self.assertRaises(
			netmanthan.ValidationError, validate_url, "gopher://netmanthan.io", valid_schemes="https", throw=True
		)
	...
	def test_valid_email(self):
		...
		# Valid addresses
		self.assertTrue(validate_email_address("someone@netmanthan.com"))
		self.assertTrue(validate_email_address("someone@netmanthan.com, anyone@netmanthan.io"))
		...
		# Invalid with throw
		self.assertRaises(
			netmanthan.InvalidEmailAddressError, validate_email_address, "someone.com", throw=True
		)


class TestImage(netmanthanTestCase):
	def test_strip_exif_data(self):
		original_image = Image.open("../apps/netmanthan/netmanthan/tests/data/exif_sample_image.jpg")
		original_image_content = open(
			"../apps/netmanthan/netmanthan/tests/data/exif_sample_image.jpg", mode="rb"
		).read()
	...
	def test_optimize_image(self):
		image_file_path = "../apps/netmanthan/netmanthan/tests/data/sample_image_for_optimization.jpg"
...
class TestPythonExpressions(netmanthanTestCase):
	...
	def test_validation_for_bad_python_expression(self):
		...
		for expr in invalid_expressions:
			self.assertRaises(netmanthan.ValidationError, validate_python_code, expr)


class TestDiffUtils(netmanthanTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
		cls.doc = netmanthan.get_doc(doctype="Client Script", dt="Client Script", name="test_client_script")
		cls.doc.insert()
		cls.doc.script = "2;"
...
class TestDateUtils(netmanthanTestCase):
	def test_first_day_of_week(self):
		# Monday as start of the week
		with patch.object(netmanthan.utils.data, "get_first_day_of_the_week", return_value="Monday"):
			self.assertEqual(
				netmanthan.utils.get_first_day_of_week("2020-12-25"), netmanthan.utils.getdate("2020-12-21")
			)
			self.assertEqual(
				netmanthan.utils.get_first_day_of_week("2020-12-20"), netmanthan.utils.getdate("2020-12-14")
			)

		# Sunday as start of the week
		self.assertEqual(
			netmanthan.utils.get_first_day_of_week("2020-12-25"), netmanthan.utils.getdate("2020-12-20")
		)
		self.assertEqual(
			netmanthan.utils.get_first_day_of_week("2020-12-21"), netmanthan.utils.getdate("2020-12-20")
		)

	def test_last_day_of_week(self):
		self.assertEqual(
			netmanthan.utils.get_last_day_of_week("2020-12-24"), netmanthan.utils.getdate("2020-12-26")
		)
		self.assertEqual(
			netmanthan.utils.get_last_day_of_week("2020-12-28"), netmanthan.utils.getdate("2021-01-02")
		)
...
class TestResponse(netmanthanTestCase):
	def test_json_handler(self):
		...
		GOOD_OBJECT = {
			...
			"doc": [
				netmanthan.get_doc("System Settings"),
			],
			"iter": [
				{1, 2, 3},
				(1, 2, 3),
...
class TestTimeDeltaUtils(netmanthanTestCase):
...
class TestXlsxUtils(netmanthanTestCase):
	def test_unescape(self):
		from netmanthan.utils.xlsxutils import handle_html
...
class TestLinkTitle(netmanthanTestCase):
	def test_link_title_doctypes_in_boot_info(self):
		...
		prop_setter = netmanthan.get_doc(
			{
		...
		from netmanthan.boot import get_link_title_doctypes
	...
	def test_link_titles_on_getdoc(self):
		...
		link_titles = netmanthan.local.response["_link_titles"]
...
class TestAppParser(netmanthanTestCase):
	def test_app_name_parser(self):
		bench_path = get_bench_path()
		netmanthan_app = os.path.join(bench_path, "apps", "netmanthan")
		self.assertEqual("netmanthan", parse_app_name(netmanthan_app))
		self.assertEqual("healthcare", parse_app_name("healthcare"))
		self.assertEqual("healthcare", parse_app_name("https://github.com/netmanthan/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("git@github.com:netmanthan/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("netmanthan/healthcare@develop"))


class TestIntrospectionMagic(netmanthanTestCase):
	...
	def test_get_newargs(self):
		...
		self.assertEqual(netmanthan.get_newargs(f, safe_kwargs), safe_kwargs)

		unsafe_args = dict(safe_kwargs)
		unsafe_args.update({"ignore_permissions": True, "flags": {"ignore_mandatory": True}})
		self.assertEqual(netmanthan.get_newargs(f, unsafe_args), safe_kwargs)
	...
	def test_strip_off_kwargs_when_not_supported(self):
		...
		self.assertEqual(netmanthan.get_newargs(f, args), {"b": 1})

		# No args
		self.assertEqual(netmanthan.get_newargs(lambda: None, args), {})


class TestLocks(netmanthanTestCase):
...
class TestMiscUtils(netmanthanTestCase):
	...
	def test_get_all_sites(self):
		self.assertIn(netmanthan.local.site, get_sites())
...
class TestTBSanitization(netmanthanTestCase):
	def test_traceback_sanitzation(self):
		try:
			password = "42"
			args = {"password": "42", "pwd": "42", "safe": "safe_value"}
			args = netmanthan._dict({"password": "42", "pwd": "42", "safe": "safe_value"})
			raise Exception
		except Exception:
			traceback = netmanthan.get_traceback(with_context=True)
...
class TestRounding(netmanthanTestCase):
