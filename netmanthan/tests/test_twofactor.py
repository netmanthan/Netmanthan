...
import netmanthan
from netmanthan.auth import HTTPRequest, get_login_attempt_tracker, validate_ip_address
...
from netmanthan.utils import cint, set_request

from . import get_system_setting, update_system_settings


class TestTwoFactor(netmanthanTestCase):
	...
	def setUp(self):
		...
		self.login_manager = netmanthan.local.login_manager
		self.user = self.login_manager.user
		update_system_settings({"allow_consecutive_login_attempts": 2})

	def tearDown(self):
		netmanthan.local.response["verification"] = None
		netmanthan.local.response["tmp_id"] = None
		disable_2fa()
		netmanthan.clear_cache(user=self.user)
	...
	def test_authenticate_for_2factor(self):
		"""Verification obj and tmp_id should be set in netmanthan.local."""
		authenticate_for_2factor(self.user)
		verification_obj = netmanthan.local.response["verification"]
		tmp_id = netmanthan.local.response["tmp_id"]
		self.assertTrue(verification_obj)
		self.assertTrue(tmp_id)
		for k in ["_usr", "_pwd", "_otp_secret"]:
			self.assertTrue(netmanthan.cache().get(f"{tmp_id}{k}"), f"{k} not available")
	...
	def test_two_factor_is_enabled(self):
		...
		# Scenario 3
		enable_2fa()
		user = netmanthan.get_doc("User", self.user)
		user.restrict_ip = netmanthan.local.request_ip
		user.save()
		self.assertTrue(should_run_2fa(self.user))
	...
	def test_confirm_otp_token(self):
		...
		netmanthan.flags.otp_expiry = 2
		authenticate_for_2factor(self.user)
		tmp_id = netmanthan.local.response["tmp_id"]
		otp = "wrongotp"
		with self.assertRaises(netmanthan.AuthenticationError):
		...
		netmanthan.flags.otp_expiry = None
		if netmanthan.flags.tests_verbose:
	...
	def test_render_string_template(self):
		...
		args = {"issuer_name": "netmanthan Technologies"}
		_str = "Verification Code from {{issuer_name}}"
		_str = netmanthan.render_template(_str, args)
		self.assertEqual(_str, "Verification Code from netmanthan Technologies")
	...
	def test_otp_attempt_tracker(self):
		...
		with self.assertRaises(netmanthan.SecurityException):
			confirm_otp_token(self.login_manager, otp=otp, tmp_id=tmp_id)
...
def create_http_request():
	...
	enable_2fa()
	netmanthan.form_dict["usr"] = "test@example.com"
	netmanthan.form_dict["pwd"] = "Eastern_43A1W"
	netmanthan.local.form_dict["cmd"] = "login"
...
def enable_2fa(bypass_two_factor_auth=0, bypass_restrict_ip_check=0):
	...
	system_settings = netmanthan.get_doc("System Settings")
...
def toggle_2fa_all_role(state=None):
	...
	all_role = netmanthan.get_doc("Role", "All")
