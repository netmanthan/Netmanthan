...
from netmanthan.utils.password import check_password, decrypt, encrypt, passlibctx, update_password


class TestPassword(netmanthanTestCase):
	def setUp(self):
		netmanthan.delete_doc("Email Account", "Test Email Account Password")
		netmanthan.delete_doc("Email Account", "Test Email Account Password-new")
	...
	def make_email_account(self, name="Test Email Account Password"):
		if not netmanthan.db.exists("Email Account", name):
			...
			return netmanthan.get_doc("Email Account", name)
	...
	def test_hashed_password(self, user="test@example.com"):
		...
		self.assertRaises(netmanthan.AuthenticationError, check_password, user, new_password)
	...
	def test_password_on_rename_user(self):
		...
		netmanthan.rename_doc(doc.doctype, old_name, new_name)

		new_doc = netmanthan.get_doc(doc.doctype, new_name)
		self.assertEqual(new_doc.get_password(), password)
		self.assertTrue(not get_password_list(doc))

		netmanthan.rename_doc(doc.doctype, new_name, old_name)
