...
from netmanthan.utils import get_site_url, get_test_client

try:
	_site = netmanthan.local.site
except Exception:
	_site = None
...
class ThreadWithReturnValue(Thread):
	...
	def run(self):
		if self._target is not None:
			with patch("netmanthan.app.get_site_name", return_value=_site):
				header_patch = patch("netmanthan.get_request_header", new=patch_request_header)
...
class netmanthanAPITestCase(netmanthanTestCase):
	...
	def sid(self) -> str:
		if not getattr(self, "_sid", None):
			...
			self._sid = netmanthan.session.sid
...
class TestResourceAPI(netmanthanAPITestCase):
	DOCTYPE = "ToDo"
	GENERATED_DOCUMENTS = []
	...
	def setUpClass(cls):
		super().setUpClass()
		for _ in range(10):
			doc = netmanthan.get_doc({"doctype": "ToDo", "description": netmanthan.mock("paragraph")}).insert()
	...
	def tearDownClass(cls):
		for name in cls.GENERATED_DOCUMENTS:
			netmanthan.delete_doc_if_exists(cls.DOCTYPE, name)
		netmanthan.db.commit()
	...
	def test_get_list_dict(self):
		...
		json = netmanthan._dict(response.json)
		self.assertEqual(response.status_code, 200)
	...
	def test_create_document(self):
		...
		data = {"description": netmanthan.mock("paragraph"), "sid": self.sid}
	...
	def test_update_document(self):
		...
		generated_desc = netmanthan.mock("paragraph")
		data = {"description": generated_desc, "sid": self.sid}
		random_doc = choice(self.GENERATED_DOCUMENTS)
		desc_before_update = netmanthan.db.get_value(self.DOCTYPE, random_doc, "description")
	...
	def test_delete_document(self):
		...
		non_existent_doc = netmanthan.generate_hash(length=12)
	...
	def test_run_doc_method(self):
		...
		if response.status_code == 403:
			...
			self.assertEqual(
				response.json.get("exception"), "netmanthan.exceptions.PermissionError: Not permitted"
			)
...
class TestMethodAPI(netmanthanAPITestCase):
	...
	def test_version(self):
		...
		self.assertEqual(Version(json.message), Version(netmanthan.__version__))
	...
	def test_get_user_info(self):
		# test 3: test for /api/method/netmanthan.realtime.get_user_info
		response = self.get(f"{self.METHOD_PATH}/netmanthan.realtime.get_user_info")
		self.assertEqual(response.status_code, 200)
		self.assertIsInstance(response.json, dict)
	...
	def test_auth_cycle(self):
		...
		response = self.get("/api/method/netmanthan.auth.get_logged_user")
...
class TestReadOnlyMode(netmanthanAPITestCase):
	...
	def test_blocked_writes(self):
		response = self.post(self.REQ_PATH, {"description": netmanthan.mock("paragraph"), "sid": self.sid})
		self.assertEqual(response.status_code, 503)
		self.assertEqual(response.json["exc_type"], "InReadOnlyMode")


class TestWSGIApp(netmanthanAPITestCase):
	def test_request_hooks(self):
		self.addCleanup(lambda: _test_REQ_HOOK.clear())

		with patch_hooks(
			{
				"before_request": ["netmanthan.tests.test_api.before_request"],
				"after_request": ["netmanthan.tests.test_api.after_request"],
			}
		):
