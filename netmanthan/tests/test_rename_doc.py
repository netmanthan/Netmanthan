...
from netmanthan.exceptions import DoesNotExistError, ValidationError
from netmanthan.model.base_document import get_controller
from netmanthan.model.rename_doc import (
	bulk_rename,
	get_fetch_fields,
	update_document_title,
	update_linked_doctypes,
)
from netmanthan.modules.utils import get_doc_path
from netmanthan.tests.utils import netmanthanTestCase
from netmanthan.utils import add_to_date, now
...
def patch_db(endpoints: list[str] = None):
	...
	for point in endpoints:
		x = patch(f"netmanthan.db.{point}", new=lambda: True)
		patched_endpoints.append(x)

	savepoint = "SAVEPOINT_for_test_bulk_rename"
	netmanthan.db.savepoint(save_point=savepoint)
	try:
...
class TestRenameDoc(netmanthanTestCase):
	@classmethod
	def setUpClass(self):
		"""Setting Up data for the tests defined under TestRenameDoc"""
		# set developer_mode to rename doc controllers
		super().setUpClass()
		self._original_developer_flag = netmanthan.conf.developer_mode
		netmanthan.conf.developer_mode = 1
	...
	def tearDownClass(self):
		...
		netmanthan.delete_doc_if_exists("DocType", "Renamed Doc")
		netmanthan.db.sql_ddl("drop table if exists `tabRenamed Doc`")

		# delete the documents created
		for docname in self.available_documents:
			netmanthan.delete_doc(self.test_doctype, docname)

		for dt in self.doctype.values():
			if netmanthan.db.exists("DocType", dt):
				netmanthan.delete_doc("DocType", dt)
				netmanthan.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{dt}`")

		# reset original value of developer_mode conf
		netmanthan.conf.developer_mode = self._original_developer_flag

	def setUp(self):
		netmanthan.flags.link_fields = {}
		if self._testMethodName == "test_doc_rename_method":
			self.property_setter = netmanthan.get_doc(
				{
	...
	def test_rename_doc(self):
		"""Rename an existing document via netmanthan.rename_doc"""
		old_name = choice(self.available_documents)
		new_name = old_name + ".new"
		self.assertEqual(new_name, netmanthan.rename_doc(self.test_doctype, old_name, new_name, force=True))
		self.available_documents.remove(old_name)
		self.available_documents.append(new_name)

	def test_merging_docs(self):
		"""Merge two documents via netmanthan.rename_doc"""
		first_todo, second_todo = sample(self.available_documents, 2)

		second_todo_doc = netmanthan.get_doc(self.test_doctype, second_todo)
		second_todo_doc.priority = "High"
		second_todo_doc.save()

		merged_todo = netmanthan.rename_doc(
			self.test_doctype, first_todo, second_todo, merge=True, force=True
		)
		merged_todo_doc = netmanthan.get_doc(self.test_doctype, merged_todo)
		self.available_documents.remove(first_todo)

		with self.assertRaises(DoesNotExistError):
			netmanthan.get_doc(self.test_doctype, first_todo)
	...
	def test_rename_controllers(self):
		...
		self.assertNotEqual(get_controller(self.doctype.old), netmanthan.model.document.Document)

		old_doctype_path = get_doc_path("Custom", "DocType", self.doctype.old)

		# rename doc via wrapper API accessible via /desk
		netmanthan.rename_doc("DocType", self.doctype.old, self.doctype.new)

		# check if database and controllers are updated
		self.assertTrue(netmanthan.db.exists("DocType", self.doctype.new))
		self.assertFalse(netmanthan.db.exists("DocType", self.doctype.old))
		self.assertFalse(os.path.exists(old_doctype_path))

	def test_rename_doctype(self):
		"""Rename DocType via netmanthan.rename_doc"""
		from netmanthan.core.doctype.doctype.test_doctype import new_doctype

		if not netmanthan.db.exists("DocType", "Rename This"):
			new_doctype(
				"Rename This",
				fields=[
					{
		...
		to_rename_record = netmanthan.get_doc(
			{"doctype": "Rename This", "linked_to_doctype": "Rename This"}
		).insert()

		# Rename doctype
		self.assertEqual(
			"Renamed Doc", netmanthan.rename_doc("DocType", "Rename This", "Renamed Doc", force=True)
		)

		# Test if Doctype value has changed in Link field
		linked_to_doctype = netmanthan.db.get_value(
		...
		new_name = "ToDo"
		self.assertEqual(new_name, netmanthan.rename_doc("Renamed Doc", old_name, new_name, force=True))

	def test_update_document_title_api(self):
		test_doctype = "Module Def"
		test_doc = netmanthan.get_doc(
			{
				"doctype": test_doctype,
				"module_name": f"Test-test_update_document_title_api-{netmanthan.generate_hash()}",
				"custom": True,
			}
		)
		...
		doc_before = netmanthan.get_doc(test_doctype, dn)
		return_value = update_document_title(doctype=dt, docname=dn, new_name=new_name)
		doc_after = netmanthan.get_doc(test_doctype, return_value)
	...
	def test_bulk_rename(self):
		...
		with patch_db(["commit", "rollback"]), patch("netmanthan.enqueue") as enqueue:
			...
			enqueue.assert_called_with(
				"netmanthan.utils.global_search.rebuild_for_doctype",
				doctype=self.test_doctype,
			)
	...
	def test_deprecated_utils(self):
		...
		with redirect_stdout(stdout), patch_db(["set_value"]):
			...
			self.assertTrue("Function netmanthan.model.rename_doc.get_fetch_fields" in stdout.getvalue())

			update_linked_doctypes("User", "ToDo", "str", "str")
			self.assertTrue("Function netmanthan.model.rename_doc.update_linked_doctypes" in stdout.getvalue())

	def test_doc_rename_method(self):
		name = choice(self.available_documents)
		new_name = f"{name}-{netmanthan.generate_hash(length=4)}"
		doc = netmanthan.get_doc(self.test_doctype, name)
		doc.rename(new_name, merge=netmanthan.db.exists(self.test_doctype, new_name))
	...
	def test_parenttype(self):
		...
		parent_a_instance = netmanthan.get_doc(
			doctype=parent_a.name, test_table=[{"some_fieldname": "x"}], name="XYZ"
		).insert()

		parent_b_instance = netmanthan.get_doc(
