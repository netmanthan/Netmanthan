...
import netmanthan
import netmanthan.commands.site
import netmanthan.commands.utils
import netmanthan.recorder
from netmanthan.installer import add_to_installed_apps, remove_app
from netmanthan.query_builder.utils import db_type_is
from netmanthan.tests.test_query_builder import run_only_if
from netmanthan.tests.utils import netmanthanTestCase, timeout
from netmanthan.utils import add_to_date, get_bench_path, get_bench_relative_path, now
from netmanthan.utils.backups import BackupGenerator, fetch_latest_backups
from netmanthan.utils.jinja_globals import bundled_asset
from netmanthan.utils.scheduler import enable_scheduler, is_scheduler_inactive

_result: Result | None = None
TEST_SITE = "commands-site-O4PN2QKA.test"  # added random string tag to avoid collisions
CLI_CONTEXT = netmanthan._dict(sites=[TEST_SITE])
...
def missing_in_backup(doctypes: list, file: os.PathLike) -> list:
	...
	"""
	predicate = 'COPY public."tab{}"' if netmanthan.conf.db_type == "postgres" else "CREATE TABLE `tab{}`"
...
@contextmanager
def maintain_locals():
	pre_site = netmanthan.local.site
	pre_flags = netmanthan.local.flags.copy()
	pre_db = netmanthan.local.db

	try:
		yield
	finally:
		post_site = getattr(netmanthan.local, "site", None)
		if not post_site or post_site != pre_site:
			netmanthan.init(site=pre_site)
			netmanthan.local.db = pre_db
			netmanthan.local.flags.update(pre_flags)
...
def cli(cmd: Command, args: list | None = None):
	with maintain_locals():
		...
		patch_ctx = patch("netmanthan.commands.pass_context", pass_test_context)
...
class BaseTestCommands(netmanthanTestCase):
	@classmethod
	...
	def execute(self, command, kwargs=None):
		...
		site = {"site": netmanthan.local.site}
		cmd_input = None
		if kwargs:
	...
	def setup_test_site(cls):
		cmd_config = {
			"test_site": TEST_SITE,
			"admin_password": netmanthan.conf.admin_password,
			"root_login": netmanthan.conf.root_login,
			"root_password": netmanthan.conf.root_password,
			"db_type": netmanthan.conf.db_type,
		}
...
class TestCommands(BaseTestCommands):
	def test_execute(self):
		...
		self.execute("bench --site {site} execute netmanthan.db.get_database_size")
		self.assertEqual(self.returncode, 0)
		self.assertIsInstance(float(self.stdout), float)

		# test 2: execute a command expecting an errored output as local won't exist
		self.execute("bench --site {site} execute netmanthan.local.site")
		...
		self.execute("""bench --site {site} execute netmanthan.bold --kwargs '{{"text": "DocType"}}'""")
		self.assertEqual(self.returncode, 0)
		self.assertEqual(self.stdout[1:-1], netmanthan.bold(text="DocType"))
	...
	def test_restore(self):
		...
		self.execute(
			"bench --site {test_site} execute netmanthan.utils.backups.fetch_latest_backups",
			site_data,
		)
		...
		self.execute(
			"bench --site {test_site} execute" " netmanthan.utils.backups.fetch_latest_backups --kwargs {kw}",
			site_data,
		)
		site_data.update({"database": json.loads(self.stdout)["database"]})
		self.execute("bench --site {test_site} restore {database}", site_data)
	...
	def test_partial_restore(self):
			...
			netmanthan.get_doc(
				{
					...
					"description": netmanthan.mock("paragraph"),
				}
			).insert()
		netmanthan.db.commit()
		todo_count = netmanthan.db.count("ToDo")
		...
		netmanthan.db.sql_ddl("DROP TABLE IF EXISTS `tabToDo`")
		netmanthan.db.commit()

		self.execute("bench --site {site} partial-restore {path}", {"path": db_path})
		self.assertEqual(self.returncode, 0)
		self.assertEqual(netmanthan.db.count("ToDo"), todo_count)

	def test_recorder(self):
		netmanthan.recorder.stop()

		self.execute("bench --site {site} start-recording")
		netmanthan.local.cache = {}
		self.assertEqual(netmanthan.recorder.status(), True)

		self.execute("bench --site {site} stop-recording")
		netmanthan.local.cache = {}
		self.assertEqual(netmanthan.recorder.status(), False)
	...
	def test_list_apps(self):
		...
		doctype = netmanthan.get_single("Installed Applications").installed_applications
		if doctype:
			installed_apps = {x.app_name for x in doctype}
		else:
			installed_apps = set(netmanthan.get_installed_apps())
	...
	def test_netmanthan_site_env(self):
		os.putenv("netmanthan_SITE", netmanthan.local.site)
		self.execute("bench execute netmanthan.ping")
		self.assertEqual(self.returncode, 0)
	...
	def test_set_password(self):
		...
		original_password = netmanthan.conf.admin_password or "admin"
	...
	@skipIf(
		not (
                netmanthan.conf.root_password and netmanthan.conf.admin_password and netmanthan.conf.db_type == "mariadb"
		),
	...
	def test_bench_drop_site_should_archive_site(self):
		...
		self.execute(
			...
			f"--admin-password {netmanthan.conf.admin_password} "
			f"--mariadb-root-password {netmanthan.conf.root_password} "
			f"--db-type {netmanthan.conf.db_type or 'mariadb'} "
		)
		self.assertEqual(self.returncode, 0)

		self.execute(f"bench drop-site {site} --force --root-password {netmanthan.conf.root_password}")
		self.assertEqual(self.returncode, 0)
	...
	def test_force_install_app(self):
		...
		# set admin password in site_config as when netmanthan force installs, we don't have the conf
		self.execute(f"bench --site {TEST_SITE} set-config admin_password {netmanthan.conf.admin_password}")

		# try installing the netmanthan_docs app again on test site
		self.execute(f"bench --site {TEST_SITE} install-app {app_name}")
		self.assertIn(f"{app_name} already installed", self.stdout)
		self.assertEqual(self.returncode, 0)

		# force install netmanthan_docs app on the test site
	...
	def test_set_global_conf(self):
		...
		value = "42"
		self.execute(f"bench set-config {key} {value} -g")
		conf = netmanthan.get_site_config()
...
class TestBackups(BaseTestCommands):
	...
	site_backup_path = netmanthan.utils.get_site_path("private", "backups")
	...
	def test_backup_fails_with_exit_code(self):
		...
		odb = BackupGenerator(
			...
			netmanthan.conf.db_name,
            netmanthan.conf.db_password + "INCORRECT PASSWORD",
			db_host=netmanthan.db.host,
			db_port=netmanthan.db.port,
			db_type=netmanthan.conf.db_type,
		)
	...
	def test_clear_log_table(self):
		d = netmanthan.get_doc(doctype="Error Log", title="Something").insert()
		d.db_set("modified", "2010-01-01", update_modified=False)
		netmanthan.db.commit()

		tables_before = netmanthan.db.get_tables(cached=False)

		self.execute("bench --site {site} clear-log-table --days=30 --doctype='Error Log'")
		self.assertEqual(self.returncode, 0)
		netmanthan.db.commit()

		self.assertFalse(netmanthan.db.exists("Error Log", d.name))
		tables_after = netmanthan.db.get_tables(cached=False)
	...
	def test_backup_only_specific_doctypes(self):
		"""Take a backup with (include) backup options set in the site config `netmanthan.conf.backup.includes`"""
		self.execute(
	...
	def test_backup_excluding_specific_doctypes(self):
		"""Take a backup with (exclude) backup options set (`netmanthan.conf.backup.excludes`, `--exclude`)"""
		# test 1: take a backup with netmanthan.conf.backup.excludes
		self.execute(
	...
	def test_selective_backup_priority_resolution(self):
		"""Take a backup with conflicting backup options set (`netmanthan.conf.excludes`, `--include`)"""
		self.execute(
	...
	def test_dont_backup_conf(self):
		"""Take a backup ignoring netmanthan.conf.backup settings (with --ignore-backup-conf option)"""
...
class TestRemoveApp(netmanthanTestCase):
	def test_delete_modules(self):
		from netmanthan.installer import (
			_delete_doctypes,
			_delete_modules,
			_get_module_linked_doctype_field_map,
		)

		test_module = netmanthan.new_doc("Module Def")

		test_module.update({"module_name": "RemoveThis", "app_name": "netmanthan"})
		test_module.save()

		module_def_linked_doctype = netmanthan.get_doc(
			{
				"doctype": "DocType",
		...
		self.assertFalse(netmanthan.db.exists("Module Def", test_module.module_name))
		self.assertFalse(netmanthan.db.exists("DocType", module_def_linked_doctype.name))

	def test_dry_run(self):
		"""Check if dry run in not destructive."""

		# nothing to assert, if this fails rest of the test suite will crumble.
		remove_app("netmanthan", dry_run=True, yes=True, no_backup=True)


class TestSiteMigration(BaseTestCommands):
	def test_migrate_cli(self):
		with cli(netmanthan.commands.site.migrate) as result:
...
class TestAddNewUser(BaseTestCommands):
	def test_create_user(self):
		...
		netmanthan.db.rollback()
		self.assertEqual(self.returncode, 0)
		user = netmanthan.get_doc("User", "test@gmail.com")
...
class TestBenchBuild(BaseTestCommands):
	def test_build_assets_size_check(self):
		with cli(netmanthan.commands.utils.build, "--force --production") as result:
			self.assertEqual(result.exit_code, 0)
			self.assertEqual(result.exception, None)
		...
		for chunk in default_bundle:
			abs_path = Path.cwd() / netmanthan.local.sites_path / bundled_asset(chunk)[1:]
...
class TestCommandUtils(netmanthanTestCase):
	def test_bench_helper(self):
		from netmanthan.utils.bench_helper import get_app_groups

		app_groups = get_app_groups()
		self.assertIn("netmanthan", app_groups)
		self.assertIsInstance(app_groups["netmanthan"], click.Group)
...
class TestSchedulerCLI(BaseTestCommands):
	...
	def test_scheduler_status(self):
		...
		parsed_output = netmanthan.parse_json(self.stdout)
		self.assertEqual(self.returncode, 0)
