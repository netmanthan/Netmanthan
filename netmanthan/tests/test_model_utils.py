...
import netmanthan
from netmanthan.model import core_doctypes_list, get_permitted_fields, is_default_field
from netmanthan.model.utils import get_fetch_values
from netmanthan.tests.utils import netmanthanTestCase


class TestModelUtils(netmanthanTestCase):
	def test_get_fetch_values(self):
		...
		full_name = netmanthan.db.get_value("User", user, "full_name")
	...
	def test_get_permitted_fields(self):
		...
		todo_all_columns = netmanthan.get_meta("ToDo").get_valid_columns()
		...
		with set_user("Guest"):
			...
			picked_doctype_all_columns = netmanthan.get_meta(picked_doctype).get_valid_columns()
		...
		with set_user("Administrator"):
			...
			child_all_fields = netmanthan.get_meta("Installed Application").get_valid_columns()
...
def set_user(user: str):
	past_user = netmanthan.session.user or "Administrator"
	netmanthan.set_user(user)
	yield
	netmanthan.set_user(past_user)
