...
import netmanthan
from netmanthan.app import make_form_dict
from netmanthan.desk.doctype.note.note import Note
from netmanthan.model.naming import make_autoname, parse_naming_series, revert_series_if_last
from netmanthan.tests.utils import netmanthanTestCase
from netmanthan.utils import cint, now_datetime, set_request
...
class TestDocument(netmanthanTestCase):
	def test_get_return_empty_list_for_table_field_if_none(self):
		d = netmanthan.get_doc({"doctype": "User"})
	...
	def test_load_single(self):
		d = netmanthan.get_doc("Website Settings", "Website Settings")
	...
	def test_insert(self):
		...
		self.assertEqual(netmanthan.db.get_value("Event", d.name, "subject"), "test-doc-test-event 1")
	...
	def test_insert_with_child(self):
		...
		d.insert()
		self.assertTrue(d.name.startswith("EV"))
		self.assertEqual(netmanthan.db.get_value("Event", d.name, "subject"), "test-doc-test-event 2")
	...
	def test_update(self):
		...
		self.assertEqual(netmanthan.db.get_value(d.doctype, d.name, "subject"), "subject changed")
	...
	def test_mandatory(self):
		...
		netmanthan.delete_doc_if_exists("User", "test_mandatory@example.com", 1)
		...
		self.assertRaises(netmanthan.MandatoryError, d.insert)

		d.set("first_name", "Test Mandatory")
		d.insert()
		self.assertEqual(netmanthan.db.get_value("User", d.name), d.name)
	...
	def test_text_editor_field(self):
		...
		except netmanthan.MandatoryError:
			self.fail("Text Editor false positive mandatory error")

	def test_conflict_validation(self):
		d1 = self.test_insert()
		d2 = netmanthan.get_doc(d1.doctype, d1.name)
		d1.save()
		self.assertRaises(netmanthan.TimestampMismatchError, d2.save)

	def test_conflict_validation_single(self):
		d1 = netmanthan.get_doc("Website Settings", "Website Settings")
		d1.home_page = "test-web-page-1"

		d2 = netmanthan.get_doc("Website Settings", "Website Settings")
	...
	def test_permission(self):
		...
		self.assertRaises(netmanthan.PermissionError, self.test_insert)
		netmanthan.set_user("Administrator")

	def test_permission_single(self):
		netmanthan.set_user("Guest")
		d = netmanthan.get_doc("Website Settings", "Website Settings")
		self.assertRaises(netmanthan.PermissionError, d.save)
		netmanthan.set_user("Administrator")

	def test_link_validation(self):
		netmanthan.delete_doc_if_exists("User", "test_link_validation@example.com", 1)
		...
		self.assertRaises(netmanthan.LinkValidationError, d.insert)
	...
	def test_validate(self):
		...
		self.assertRaises(netmanthan.ValidationError, d.validate)
		self.assertRaises(netmanthan.ValidationError, d.run_method, "validate")
		self.assertRaises(netmanthan.ValidationError, d.save)
	...
	def test_update_after_submit(self):
		...
		self.assertRaises(netmanthan.UpdateAfterSubmitError, d.validate_update_after_submit)
	...
	def test_varchar_length(self):
		...
		self.assertRaises(netmanthan.CharacterLengthExceededError, d.save)
	...
	def test_naming_series(self):
		...
		for series in data:
			...
			old_current = netmanthan.db.get_value("Series", prefix, "current", order_by="name")

			revert_series_if_last(series, name)
			new_current = cint(netmanthan.db.get_value("Series", prefix, "current", order_by="name"))

			self.assertEqual(cint(old_current) - 1, new_current)

	def test_non_negative_check(self):
		netmanthan.delete_doc_if_exists("Currency", "netmanthan Coin", 1)

		d = netmanthan.get_doc(
			{"doctype": "Currency", "currency_name": "netmanthan Coin", "smallest_currency_fraction_value": -1}
		)

		self.assertRaises(netmanthan.NonNegativeError, d.insert)

		d.set("smallest_currency_fraction_value", 1)
		d.insert()
		self.assertEqual(netmanthan.db.get_value("Currency", d.name), d.name)
	...
	def test_get_formatted(self):
		...
		netmanthan.delete_doc_if_exists("Currency", "INR", 1)
		...
		d = netmanthan.get_doc({"doctype": "Test Formatted", "currency": 100000})
	...
	def test_virtual_fields(self):
		...
		netmanthan.db.delete("Custom Field", {"dt": "Note", "fieldname": "age"})
		...
		def patch_note(class_=None):
			return patch("netmanthan.controllers", new={netmanthan.local.site: {"Note": class_ or CustomTestNote}})

		@contextmanager
		def customize_note(with_options=False):
			options = (
				"netmanthan.utils.now_datetime() - netmanthan.utils.get_datetime(doc.creation)" if with_options else ""
			)
		...
		with patch_note():
			doc = netmanthan.get_last_doc("Note")
	...
	def test_doc_events(self):
		...
		for doctype, doc_hooks in netmanthan.get_doc_hooks().items():
			for _, hooks in doc_hooks.items():
				for hook in hooks:
					try:
						netmanthan.get_attr(hook)
	...
	def test_error_on_saving_new_doc_with_name(self):
		...
		doc = netmanthan.get_doc(
			{
				"doctype": "ToDo",
				"description": "this should raise netmanthan.DoesNotExistError",
				"name": "lets-trick-doc-save",
			}
		)

		self.assertRaises(netmanthan.DoesNotExistError, doc.save)


class TestDocumentWebView(netmanthanTestCase):
	def get(self, path, user="Guest"):
		netmanthan.set_user(user)
		set_request(method="GET", path=path)
		make_form_dict(netmanthan.local.request)
	...
	def test_web_view_link_authentication(self):
		...
		# expire the key
		document_key_doc = netmanthan.get_doc("Document Share Key", {"key": document_key})
