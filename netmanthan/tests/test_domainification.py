...
from netmanthan.core.page.permission_manager.permission_manager import get_roles_and_doctypes
from netmanthan.desk.doctype.desktop_icon.desktop_icon import (
	add_user_icon,
...
class TestDomainification(netmanthanTestCase):
	def setUp(self):
		# create test domain
	...
	def tearDown(self):
		netmanthan.db.delete("Role", {"name": "_Test Role"})
		netmanthan.db.delete("Has Role", {"role": "_Test Role"})
		netmanthan.db.delete("Domain", {"name": ("in", ("_Test Domain 1", "_Test Domain 2"))})
		netmanthan.delete_doc("DocType", "Test Domainification")
		self.remove_from_active_domains(remove_all=True)
	...
	def add_active_domain(self, domain):
		...
		domain_settings = netmanthan.get_doc("Domain Settings", "Domain Settings")
	...
	def new_domain(self, domain):
		# create new domain
		netmanthan.get_doc({"doctype": "Domain", "domain": domain}).insert()
	...
	def test_active_domains(self):
		self.assertTrue("_Test Domain 1" in netmanthan.get_active_domains())
		self.assertFalse("_Test Domain 2" in netmanthan.get_active_domains())

		self.add_active_domain("_Test Domain 2")
		self.assertTrue("_Test Domain 2" in netmanthan.get_active_domains())

		self.remove_from_active_domains("_Test Domain 1")
		self.assertTrue("_Test Domain 1" not in netmanthan.get_active_domains())
	...
	def test_doctype_and_role_domainification(self):
		...
		test_role = netmanthan.get_doc({"doctype": "Role", "role_name": "_Test Role"}).insert()
	...
	def test_module_def_for_domainification(self):
		...
		test_module_def = netmanthan.get_doc("Module Def", "Contacts")
