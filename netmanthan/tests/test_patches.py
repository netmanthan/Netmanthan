...
import netmanthan
from netmanthan.modules import patch_handler
...
execute:netmanthan.db.updatedb("Item")
execute:netmanthan.function(arg="1")
...
class TestPatches(netmanthanTestCase):
	def test_patch_module_names(self):
		netmanthan.flags.final_patches = []
		netmanthan.flags.in_install = True
		for patchmodule in patch_handler.get_all_patches():
			if patchmodule.startswith("execute:"):
				...
				self.assertTrue(netmanthan.get_attr(patchmodule.split(maxsplit=1)[0] + ".execute"))

		netmanthan.flags.in_install = False

	def test_get_patch_list(self):
		pre = patch_handler.get_patches_from_app("netmanthan", patch_handler.PatchType.pre_model_sync)
		post = patch_handler.get_patches_from_app("netmanthan", patch_handler.PatchType.post_model_sync)
		all_patches = patch_handler.get_patches_from_app("netmanthan")
	...
	def test_all_patches_are_marked_completed(self):
		all_patches = patch_handler.get_patches_from_app("netmanthan")
		finished_patches = netmanthan.db.count("Patch Log")

		self.assertGreaterEqual(finished_patches, len(all_patches))


class TestPatchReader(netmanthanTestCase):
	def get_patches(self):
		return (
			patch_handler.get_patches_from_app("netmanthan"),
			patch_handler.get_patches_from_app("netmanthan", patch_handler.PatchType.pre_model_sync),
			patch_handler.get_patches_from_app("netmanthan", patch_handler.PatchType.post_model_sync),
		)
...
def check_patch_files(app):
	...
	patch_dir = Path(netmanthan.get_app_path(app)) / "patches"
	...
	for file in patch_dir.glob("**/*.py"):
		...
		try:
			patch_module = netmanthan.get_module(module)
