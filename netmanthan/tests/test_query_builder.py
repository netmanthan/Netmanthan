...
import netmanthan
from netmanthan.query_builder import Case
from netmanthan.query_builder.builder import Function
from netmanthan.query_builder.custom import ConstantColumn
from netmanthan.query_builder.functions import (
	Cast_,
	Coalesce,
	CombineDatetime,
	Date,
...
def run_only_if(dbtype: db_type_is) -> Callable:
	return unittest.skipIf(db_type_is(netmanthan.conf.db_type) != dbtype, f"Only runs for {dbtype.value}")


@run_only_if(db_type_is.MARIADB)
class TestCustomFunctionsMariaDB(netmanthanTestCase):
	...
	def test_constant_column(self):
		query = netmanthan.qb.from_("DocType").select("name", ConstantColumn("John").as_("User"))
		self.assertEqual(query.get_sql(), "SELECT `name`,'John' `User` FROM `tabDocType`")

	def test_timestamp(self):
		note = netmanthan.qb.DocType("Note")
		self.assertEqual(
		...
		todo = netmanthan.qb.DocType("ToDo")
		select_query = (
	...
	def test_time(self):
		...
		select_query = netmanthan.qb.from_(note).select(
			CombineDatetime(note.posting_date, note.posting_time)
		)
	...
	def test_cast(self):
		...
		self.assertEqual(
			netmanthan.qb.from_("red").from_(note).select("other", Cast_(note.name, "varchar")).get_sql(),
	...
	def test_round(self):
		...
		query = netmanthan.qb.from_(note).select(Round(note.price))
		self.assertEqual("select round(`price`,0) from `tabnote`", str(query).lower())

		query = netmanthan.qb.from_(note).select(Round(note.price, 3))
		self.assertEqual("select round(`price`,3) from `tabnote`", str(query).lower())

	def test_truncate(self):
		note = netmanthan.qb.DocType("Note")
		query = netmanthan.qb.from_(note).select(Truncate(note.price, 3))
		self.assertEqual("select truncate(`price`,3) from `tabnote`", str(query).lower())


@run_only_if(db_type_is.POSTGRES)
class TestCustomFunctionsPostgres(netmanthanTestCase):
	def test_concat(self):
...
class TestBuilderBase:
	def test_adding_tabs(self):
		self.assertEqual("tabNotes", netmanthan.qb.DocType("Notes").get_sql())
		self.assertEqual("__Auth", netmanthan.qb.DocType("__Auth").get_sql())
		self.assertEqual("Notes", netmanthan.qb.Table("Notes").get_sql())

	def test_run_patcher(self):
		query = netmanthan.qb.from_("ToDo").select("*").limit(1)
	...
	def test_agg_funcs(self):
		netmanthan.db.truncate("Communication")
		sample_data = {
		...
		netmanthan.get_doc(sample_data).insert()
		sample_data["rating"] = 3
		netmanthan.get_doc(sample_data).insert()
		sample_data["rating"] = 4
		netmanthan.get_doc(sample_data).insert()
		self.assertEqual(netmanthan.qb.max("Communication", "rating"), 4)
		self.assertEqual(netmanthan.qb.min("Communication", "rating"), 1)
		self.assertAlmostEqual(netmanthan.qb.avg("Communication", "rating"), 2.666, places=2)
		self.assertEqual(netmanthan.qb.sum("Communication", "rating"), 8.0)
		netmanthan.db.rollback()


class TestParameterization(netmanthanTestCase):
	def test_where_conditions(self):
		DocType = netmanthan.qb.DocType("DocType")
		query = netmanthan.qb.from_(DocType).select(DocType.name).where(DocType.owner == "Administrator' --")
	...
	def test_set_conditions(self):
		DocType = netmanthan.qb.DocType("DocType")
		query = netmanthan.qb.update(DocType).set(DocType.value, "some_value")
	...
	def test_case(self):
		DocType = netmanthan.qb.DocType("DocType")
		query = netmanthan.qb.from_(DocType).select(
			Case()
	...
	def test_case_in_update(self):
		DocType = netmanthan.qb.DocType("DocType")
		query = netmanthan.qb.update(DocType).set(
			"parent",
			Case()
			.when(DocType.search_fields == "value", "other_value")
			.when(Coalesce(DocType.search_fields == "subject_in_function"), "true_value")
	...
	def test_named_parameter_wrapper(self):
		from netmanthan.query_builder.terms import NamedParameterWrapper
...
@run_only_if(db_type_is.MARIADB)
class TestBuilderMaria(netmanthanTestCase, TestBuilderBase):
	def test_adding_tabs_in_from(self):
		self.assertEqual("SELECT * FROM `tabNotes`", netmanthan.qb.from_("Notes").select("*").get_sql())
		self.assertEqual("SELECT * FROM `__Auth`", netmanthan.qb.from_("__Auth").select("*").get_sql())

	def test_get_qb_type(self):
		from netmanthan.query_builder import get_query_builder

		qb = get_query_builder(netmanthan.db.db_type)
		self.assertEqual("SELECT * FROM `tabDocType`", qb().from_("DocType").select("*").get_sql())


@run_only_if(db_type_is.POSTGRES)
class TestBuilderPostgres(netmanthanTestCase, TestBuilderBase):
	def test_adding_tabs_in_from(self):
		self.assertEqual('SELECT * FROM "tabNotes"', netmanthan.qb.from_("Notes").select("*").get_sql())
		self.assertEqual('SELECT * FROM "__Auth"', netmanthan.qb.from_("__Auth").select("*").get_sql())

	def test_replace_tables(self):
		info_schema = netmanthan.qb.Schema("information_schema")
		self.assertEqual(
			'SELECT * FROM "pg_stat_all_tables"',
			netmanthan.qb.from_(info_schema.tables).select("*").get_sql(),
		)

	def test_replace_fields_post(self):
		self.assertEqual("relname", netmanthan.qb.Field("table_name").get_sql())
