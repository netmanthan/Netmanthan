...
import netmanthan
from netmanthan.email.doctype.email_account.test_email_account import TestEmailAccount
...
class TestEmail(netmanthanTestCase):
	def setUp(self):
	...
	def test_send_after(self):
		...
		from netmanthan.email.queue import flush
	...
	def test_flush(self):
		...
		self.assertTrue("test1@example.com" in queue_recipients)
		self.assertEqual(len(queue_recipients), 2)
		self.assertTrue("Unsubscribe" in netmanthan.safe_decode(netmanthan.flags.sent_mail))
	...
	def test_cc_footer(self):
		...
		self.assertTrue(
            ...
            in netmanthan.safe_decode(netmanthan.flags.sent_mail)
		)

		# check for email tracker
		self.assertTrue("mark_email_as_seen" in netmanthan.safe_decode(netmanthan.flags.sent_mail))
		netmanthan.conf.use_ssl = False
	...
	def test_expose(self):
		...
		email_obj = email.message_from_string(netmanthan.safe_decode(netmanthan.flags.sent_mail))
		for part in email_obj.walk():
			...
			if content:
				...
				netmanthan.local.flags.signed_query_string = re.search(
					r"(?<=/api/method/netmanthan.email.queue.unsubscribe\?).*(?=" + eol + ")", content.decode()
				).group(0)
	...
	def test_expired(self):
		self.test_email_queue()
		netmanthan.db.sql("UPDATE `tabEmail Queue` SET `modified`=(NOW() - INTERVAL '8' day)")

		from netmanthan.email.queue import set_expiry_for_email_queue
	...
	def test_sender(self):
		def _patched_assertion(email_account, assertion):
			with patch.object(QueueBuilder, "get_outgoing_email_account", return_value=email_account):
				...
				email_queue_sender = netmanthan.db.get_value("Email Queue", {"status": "Sent"}, "sender")
	...
	def test_unsubscribe(self):
		from netmanthan.email.queue import unsubscribe
		...
		email_queue = netmanthan.db.get_value("Email Queue", {"status": "Not Sent"})
		queue_recipients = [
			r.recipient
			for r in netmanthan.db.sql(
				"""select recipient from `tabEmail Queue Recipient`
		...
		netmanthan.get_doc("Email Queue", email_queue).send()
		self.assertTrue("Unsubscribe" in netmanthan.safe_decode(netmanthan.flags.sent_mail))
	...
	def test_image_parsing(self):
		...
		netmanthan.db.delete("Communication", {"sender": "sukh@yyy.com"})

		with open(netmanthan.get_app_path("netmanthan", "tests", "data", "email_with_image.txt")) as raw:
			messages = {
...
class TestVerifiedRequests(netmanthanTestCase):
