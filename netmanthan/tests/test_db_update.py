...
class TestDBUpdate(netmanthanTestCase):
	def test_db_update(self):
		doctype = "User"
		netmanthan.reload_doctype("User", force=True)
		netmanthan.model.meta.trim_tables("User")
		...
		table_columns = netmanthan.db.get_table_columns_description(f"tab{doctype}")
		...
		for field_def in field_defs:
			...
			fallback_default = (
				"0" if field_def.get("fieldtype") in netmanthan.model.numeric_fieldtypes else "NULL"
			)
	...
	def test_index_and_unique_constraints(self):
		...
		netmanthan.db.add_index(doctype, ["email_signature(200)"])
		netmanthan.db.updatedb(doctype)
	...
	def test_unique_index_on_install(self):
		...
		for dt in netmanthan.get_all("DocType", {"is_virtual": 0, "issingle": 0}, pluck="name"):
			doctype = netmanthan.get_meta(dt)
...
def get_fieldtype_from_def(field_def):
	fieldtuple = netmanthan.db.type_map.get(field_def.fieldtype, ("", 0))
...
def get_other_fields_meta(meta):
	...
	optional_fields = netmanthan.db.OPTIONAL_COLUMNS
	if meta.track_seen:
		optional_fields.append("_seen")

	child_table_fields_map = {}
	if meta.istable:
		child_table_fields_map.update({field: ("Data", 0) for field in netmanthan.db.CHILD_TABLE_COLUMNS})
	...
	field_map = [
		netmanthan._dict({"fieldname": field, "fieldtype": _type, "length": _length})
