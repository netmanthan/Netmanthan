...
import netmanthan
from netmanthan import get_hooks
from netmanthan.tests.utils import netmanthanTestCase
from netmanthan.utils import set_request
from netmanthan.website.page_renderers.static_page import StaticPage
from netmanthan.website.serve import get_response, get_response_content
from netmanthan.website.utils import build_response, clear_website_cache, get_home_page


class TestWebsite(netmanthanTestCase):
	def setUp(self):
		netmanthan.set_user("Guest")

	def tearDown(self):
		netmanthan.db.delete("Access Log")
	...
	def test_home_page(self):
		...
		netmanthan.db.set_value("Role", "home-page-test", "home_page", "home-page-test")
		netmanthan.set_user("test-user-for-home-page@example.com")
		self.assertEqual(get_home_page(), "home-page-test")

		netmanthan.set_user("Administrator")
		netmanthan.db.set_value("Role", "home-page-test", "home_page", "")

		# home page via portal settings
		netmanthan.db.set_value("Portal Settings", None, "default_portal_home", "test-portal-home")

		netmanthan.set_user("test-user-for-home-page@example.com")
		netmanthan.cache().hdel("home_page", netmanthan.session.user)
		self.assertEqual(get_home_page(), "test-portal-home")

		netmanthan.db.set_value("Portal Settings", None, "default_portal_home", "")
		clear_website_cache()

		# home page via website settings
		netmanthan.db.set_value("Website Settings", None, "home_page", "contact")
		self.assertEqual(get_home_page(), "contact")

		netmanthan.db.set_value("Website Settings", None, "home_page", None)
		clear_website_cache()
		...
		with patch.object(
			...
			"get_hooks",
			patched_get_hooks(
				"get_website_user_home_page", ["netmanthan.www._test._test_home_page.get_website_user_home_page"]
			),
		):
			self.assertEqual(get_home_page(), "_test/_test_folder")

		clear_website_cache()
		with patch.object(netmanthan, "get_hooks", patched_get_hooks("website_user_home_page", ["login"])):
			self.assertEqual(get_home_page(), "login")

		clear_website_cache()
		with patch.object(netmanthan, "get_hooks", patched_get_hooks("home_page", ["about"])):
			self.assertEqual(get_home_page(), "about")

		clear_website_cache()
		with patch.object(
			netmanthan, "get_hooks", patched_get_hooks("role_home_page", {"home-page-test": ["home-page-test"]})
		):
			self.assertEqual(get_home_page(), "home-page-test")
	...
	def test_app(self):
		...
		netmanthan.local.session_obj = None
	...
	def test_redirect(self):
		import netmanthan.hooks

		netmanthan.set_user("Administrator")

		netmanthan.hooks.website_redirects = [
		...
		delattr(netmanthan.hooks, "website_redirects")
	...
	def test_custom_page_renderer(self):
		...
		def patched_get_hooks(*args, **kwargs):
			...
			if args and args[0] == "page_renderer":
				return_value = ["netmanthan.tests.test_website.CustomPageRenderer"]
			return return_value

		with patch.object(netmanthan, "get_hooks", patched_get_hooks):
	...
	def test_printview_page(self):
		netmanthan.db.value_cache[("DocType", "Language", "name")] = (("Language",),)
		netmanthan.set_user("Administrator")
	...
	def test_json_sidebar_data(self):
		netmanthan.flags.look_for_sidebar = False
		content = get_response_content("/_test/_test_folder/_test_page")
		self.assertNotIn("Test Sidebar", content)
		clear_website_cache()
		netmanthan.flags.look_for_sidebar = True
		content = get_response_content("/_test/_test_folder/_test_page")
	...
	def test_safe_render(self):
		...
		self.assertNotIn("netmanthan.exceptions.ValidationError: Illegal template", content)

	def test_never_render(self):
		from pathlib import Path
		from random import choices

		WWW = Path(netmanthan.get_app_path("netmanthan")) / "www"
	...
	def test_resolve_class(self):
		from netmanthan.utils.jinja_globals import resolve_class

		context = netmanthan._dict(primary=True)
		...
		self.assertEqual(
			netmanthan.render_template(content, context), '<a class="btn btn-default btn-primary">Test</a>'
		)
	...
	def test_app_include(self):
		...
		with patch.object(netmanthan, "get_hooks", patched_get_hooks):
			netmanthan.set_user("Administrator")
			netmanthan.hooks.app_include_js.append("test_app_include.js")
			netmanthan.hooks.app_include_css.append("test_app_include.css")
			netmanthan.conf.update({"app_include_js": ["test_app_include_via_site_config.js"]})
			netmanthan.conf.update({"app_include_css": ["test_app_include_via_site_config.css"]})
			...
			delattr(netmanthan.local, "request")
			netmanthan.set_user("Guest")
