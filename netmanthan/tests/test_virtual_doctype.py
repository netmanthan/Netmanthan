...
import netmanthan
import netmanthan.modules.utils
from netmanthan.core.doctype.doctype.test_doctype import new_doctype
from netmanthan.desk.form.save import savedocs
...
class VirtualDoctypeTest(Document):
	...
	def get_list(args):
		...
		return [netmanthan._dict(doc) for name, doc in data.items()]
...
class TestVirtualDoctypes(netmanthanTestCase):
	@classmethod
	def setUpClass(cls):
		netmanthan.flags.allow_doctype_export = True
		cls.addClassCleanup(netmanthan.flags.pop, "allow_doctype_export", None)

		vdt = new_doctype(name=TEST_DOCTYPE_NAME, is_virtual=1, custom=0).insert()
		cls.addClassCleanup(vdt.delete)

		patch_virtual_doc = patch(
			"netmanthan.controllers", new={netmanthan.local.site: {TEST_DOCTYPE_NAME: VirtualDoctypeTest}}
		)
	...
	def test_insert_update_and_load_from_desk(self):
		...
		docname = netmanthan.response.docs[0]["name"]

		doc = netmanthan.get_doc(TEST_DOCTYPE_NAME, docname)
	...
	def test_multiple_doc_insert_and_get_list(self):
		doc1 = netmanthan.get_doc(doctype=TEST_DOCTYPE_NAME, some_fieldname="first").insert()
		doc2 = netmanthan.get_doc(doctype=TEST_DOCTYPE_NAME, some_fieldname="second").insert()
	...
	def test_delete_doc(self):
		doc = netmanthan.get_doc(doctype=TEST_DOCTYPE_NAME, some_fieldname="data").insert()

		netmanthan.delete_doc(doc.doctype, doc.name)
