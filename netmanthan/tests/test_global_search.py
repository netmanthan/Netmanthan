...
from netmanthan.utils import global_search, now_datetime


class TestGlobalSearch(netmanthanTestCase):
	def setUp(self):
		update_global_search_doctypes()
		global_search.setup_global_search_table()
		self.assertTrue("__global_search" in netmanthan.db.get_tables())
		doctype = "Event"
		global_search.reset()
	...
	def tearDown(self):
		netmanthan.db.delete("Property Setter", {"doc_type": "Event"})
	...
	def test_update_doc(self):
		...
		event = netmanthan.get_doc("Event", netmanthan.get_all("Event")[0].name)
	...
	def test_delete_doc(self):
		self.insert_test_events()
		event_name = netmanthan.get_all("Event")[0].name
		event = netmanthan.get_doc("Event", event_name)
		...
		netmanthan.delete_doc("Event", event_name)
		global_search.sync_global_search()
		netmanthan.db.commit()
	...
	def test_insert_child_table(self):
		...
		for text in phrases:
			doc = netmanthan.get_doc({"doctype": "Event", "subject": text, "starts_on": now_datetime()})
			doc.insert()
