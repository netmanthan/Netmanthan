...
class TestFixtureImport(netmanthanTestCase):
	def create_new_doctype(self, DocType: str) -> None:
		file = netmanthan.get_app_path("netmanthan", "custom", "fixtures", f"{DocType}.json")
	...
	def insert_dummy_data_and_export(self, DocType: str, dummy_name_list: list[str]) -> str:
		for name in dummy_name_list:
			doc = netmanthan.get_doc({"doctype": DocType, "member_name": name})
			doc.insert()
	...
	def test_fixtures_import(self):
		self.assertFalse(netmanthan.db.exists("DocType", "temp_doctype"))
		...
		netmanthan.db.truncate("temp_doctype")
		...
		self.assertEqual(netmanthan.db.count("temp_doctype"), len(dummy_name_list))

		data = netmanthan.get_all("temp_doctype", "member_name")
		netmanthan.db.truncate("temp_doctype")
	...
	def test_singles_fixtures_import(self):
		self.assertFalse(netmanthan.db.exists("DocType", "temp_singles"))
		...
		singles_doctype = netmanthan.qb.DocType("Singles")
		truncate_query = (
			netmanthan.qb.from_(singles_doctype).delete().where(singles_doctype.doctype == "temp_singles")
		)
		truncate_query.run()
		...
		data = netmanthan.db.get_single_value("temp_singles", "member_name")
		truncate_query.run()
