...
def build_missing_files():
	...
	for type in ["css", "js"]:
		folder = os.path.join(sites_path, "assets", "netmanthan", "dist", type)
	...
	assets_json = netmanthan.read_file("assets/assets.json")
	if assets_json:
		assets_json = netmanthan.parse_json(assets_json)

		for bundle_file, output_file in assets_json.items():
			if not output_file.startswith("/assets/netmanthan"):
				continue
		...
		if missing_assets:
			...
			files_to_build = ["netmanthan/" + name for name in missing_assets]
			bundle(build_mode, files=files_to_build)
	else:
		# no assets.json, run full build
		bundle(build_mode, apps="netmanthan")


def get_assets_link(netmanthan_head) -> str:
	import requests

	tag = getoutput(
		r"cd ../apps/netmanthan && git show-ref --tags -d | grep %s | sed -e 's,.*"
		r" refs/tags/,,' -e 's/\^{}//'" % netmanthan_head
	)

	if tag:
		# if tag exists, download assets from github release
		url = f"https://github.com/netmanthan/netmanthan/releases/download/{tag}/assets.tar.gz"
	else:
		url = f"http://assets.netmanthanframework.com/{netmanthan_head}.tar.gz"

	if not requests.head(url):
		reference = f"Release {tag}" if tag else f"Commit {netmanthan_head}"
		raise AssetsDontExistError(f"Assets for {reference} don't exist")

	return url


def fetch_assets(url, netmanthan_head):
	click.secho("Retrieving assets...", fg="yellow")

	prefix = mkdtemp(prefix="netmanthan-assets-", suffix=netmanthan_head)
	assets_archive = download_file(url, prefix)

	if not assets_archive:
		raise AssetsNotDownloadedError(f"Assets could not be retrived from {url}")

	click.echo(click.style("✔", fg="green") + f" Downloaded netmanthan assets from {url}")
...
def setup_assets(assets_archive):
	...
	with tarfile.open(assets_archive) as tar:
		for file in tar:
			if not file.isdir():
				dest = "." + file.name.replace("./netmanthan-bench/sites", "")
...
def download_netmanthan_assets(verbose=True):
	"""Downloads and sets up netmanthan assets if they exist based on the current
	commit HEAD.
	Returns True if correctly setup else returns False.
	"""
	netmanthan_head = getoutput("cd ../apps/netmanthan && git rev-parse HEAD")

	if not netmanthan_head:
		return False

	try:
		url = get_assets_link(netmanthan_head)
		assets_archive = fetch_assets(url, netmanthan_head)
...
def setup():
	...
	for app in netmanthan.get_all_apps(True):
		try:
			pymodules.append(netmanthan.get_module(app))
		except ImportError:
			pass
	app_paths = [os.path.dirname(pymodule.__file__) for pymodule in pymodules]
	assets_path = os.path.join(netmanthan.local.sites_path, "assets")
...
def bundle(
	...
	netmanthan_app_path = netmanthan.get_app_path("netmanthan", "..")
	netmanthan.commands.popen(command, cwd=netmanthan_app_path, env=get_node_env(), raise_err=True)
...
def watch(apps=None):
	...
	live_reload = netmanthan.utils.cint(os.environ.get("LIVE_RELOAD", netmanthan.conf.live_reload))
	...
	check_node_executable()
	netmanthan_app_path = netmanthan.get_app_path("netmanthan", "..")
	netmanthan.commands.popen(command, cwd=netmanthan_app_path, env=get_node_env())
...
def generate_assets_map():
	...
	for app_name in netmanthan.get_all_apps():
		app_doc_path = None

		pymodule = netmanthan.get_module(app_name)
...
def html_to_js_template(path, content):
	"""returns HTML template content as Javascript code, adding it to `netmanthan.templates`"""
	return """netmanthan.templates["{key}"] = '{content}';\n""".format(
