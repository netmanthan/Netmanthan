...
def add_docshare(
	...
	if share_name:
		doc = netmanthan.get_doc("DocShare", share_name)
	else:
		doc = netmanthan.new_doc("DocShare")
		doc.update(
	...
	if netmanthan.get_cached_value("User", user, "follow_shared_documents"):
...
def remove(doctype, name, user, flags=None):
	share_name = netmanthan.db.get_value(
		"DocShare", {"user": user, "share_name": name, "share_doctype": doctype}
	)

	if share_name:
		netmanthan.delete_doc("DocShare", share_name, flags=flags)
...
def set_docshare_permission(doctype, name, user, permission_to, value=1, everyone=0, flags=None):
	...
	if not share_name:
		...
		share = netmanthan.get_doc("DocShare", share_name)
		if flags:
...
def get_shared(doctype, user=None, rights=None, *, filters=None, limit=None):
	...
	shared_docs = netmanthan.get_all(
		"DocShare",
		fields=["share_name"],
...
def get_shared_doctypes(user=None):
	...
	if not user:
		user = netmanthan.session.user
	table = netmanthan.qb.DocType("DocShare")
	query = (
...
def check_share_permission(doctype, name):
	...
	if not netmanthan.has_permission(doctype, ptype="share", doc=name):
		netmanthan.throw(
			_("No permission to {0} {1} {2}").format("share", _(doctype), name), netmanthan.PermissionError
		)
...
def notify_assignment(shared_by, doctype, doc_name, everyone, notify=0):
	...
	reference_user = get_fullname(netmanthan.session.user)
	notification_message = _("{0} shared a document {1} {2} with you").format(
		netmanthan.bold(reference_user), netmanthan.bold(_(doctype)), get_title_html(title)
	)
