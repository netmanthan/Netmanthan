...
class TestFullTextSearch(netmanthanTestCase):
	def setUp(self):
		index = get_index()
		index.build()
		self.index = index
	...
	def test_update_index(self):
		...
		# Update new doc
		self.index.update_index({"name": "sw/netmanthanbooks", "content": """DesktopAccounting"""})

		res = self.index.search("DesktopAccounting")
		self.assertEqual(res[0], "sw/netmanthanbooks")
...
def get_index():
	return TestWrapper("test_netmanthan_index")
...
def get_documents():
	...
	docs.append(
		{
			...
			netmanthan Technologies Pvt. Ltd. and is built on MariaDB database system using a Python based server-side framework.
	...
	docs.append(
		{
			"name": "sw/netmanthan",
			"content": """netmanthan Framework is a full-stack web framework, that includes everything you need to build and
