...
netmanthan.ui.form.on("Workspace", {
	setup: function () {
		netmanthan.meta.get_field("Workspace Link", "only_for").no_default = true;
	},
	...
	refresh: function (frm) {
		...
		let url = `/app/${
			frm.doc.public
				? netmanthan.router.slug(frm.doc.title)
				: "private/" + netmanthan.router.slug(frm.doc.title)
		}`;
		frm.sidebar
		...
		if (
			...
			(frm.doc.public &&
				!frm.has_perm("write") &&
				!netmanthan.user.has_role("Workspace Manager"))
		) {
