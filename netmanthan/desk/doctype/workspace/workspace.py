...
from netmanthan.desk.desktop import save_new_widget
from netmanthan.desk.utils import validate_route_conflict
from netmanthan.model.document import Document
from netmanthan.model.rename_doc import rename_doc
from netmanthan.modules.export_file import delete_folder, export_to_files


class Workspace(Document):
	def validate(self):
		if self.public and not is_workspace_manager() and not disable_saving_as_public():
			netmanthan.throw(_("You need to be Workspace Manager to edit this document"))
		...
		try:
			...
			netmanthan.throw(_("Content data shoud be a list"))

	def clear_cache(self):
		super().clear_cache()
		if self.for_user:
			netmanthan.cache().hdel("bootinfo", self.for_user)
		else:
	...
	def on_update(self):
		...
		if netmanthan.conf.developer_mode and self.public:
			if self.module:
	...
	def after_delete(self):
		...
		if self.module and netmanthan.conf.developer_mode:
			delete_folder(self.module, "Workspace", self.title)

	@staticmethod
	def get_module_wise_workspaces():
		workspaces = netmanthan.get_all(
			"Workspace",
			fields=["name", "module"],
		...
		for link in self.links:
			...
			if link.type == "Card Break":
				if card_links and (
                        ...
                        or current_card.get("only_for") == netmanthan.get_system_settings("country")
				):
...
def disable_saving_as_public():
	return (
            ...
            or netmanthan.flags.in_uninstall
...
def get_report_type(report):
	report_type = netmanthan.get_value("Report", report, "report_type")
...
def new_page(new_page):
	...
	doc = netmanthan.new_doc("Workspace")
...
def save_page(title, public, new_widgets, blocks):
	public = netmanthan.parse_json(public)

	filters = {"public": public, "label": title}

	if not public:
		filters = {"for_user": netmanthan.session.user, "label": title + "-" + netmanthan.session.user}
	pages = netmanthan.get_all("Workspace", filters=filters)
	if pages:
		doc = netmanthan.get_doc("Workspace", pages[0])
...
def update_page(name, title, icon, parent, public):
	public = netmanthan.parse_json(public)
	doc = netmanthan.get_doc("Workspace", name)
	...
	if doc:
		...
		if doc.public != public:
			doc.sequence_id = netmanthan.db.count("Workspace", {"public": public}, cache=True)
			doc.public = public
		doc.for_user = "" if public else doc.for_user or netmanthan.session.user
		...
		child_docs = netmanthan.get_all(
			"Workspace", filters={"parent_page": doc.title, "public": doc.public}
		)
		if child_docs:
			for child in child_docs:
				child_doc = netmanthan.get_doc("Workspace", child.name)
				child_doc.parent_page = doc.title
				if child_doc.public != public:
					child_doc.public = public
				child_doc.for_user = "" if public else child_doc.for_user or netmanthan.session.user
...
def hide_unhide_page(page_name: str, is_hidden: bool):
	page = netmanthan.get_doc("Workspace", page_name)

	if page.get("public") and not is_workspace_manager():
		netmanthan.throw(
			_("Need Workspace Manager role to hide/unhide public workspaces"), netmanthan.PermissionError
		)

	if not page.get("public") and page.get("for_user") != netmanthan.session.user:
		netmanthan.throw(_("Cannot update private workspace of other users"), netmanthan.PermissionError)
...
def duplicate_page(page_name, new_page):
	...
	old_doc = netmanthan.get_doc("Workspace", page_name)
	doc = netmanthan.copy_doc(old_doc)
	...
	doc.module = ""
	if not doc.public:
		doc.for_user = doc.for_user or netmanthan.session.user
...
def delete_page(page):
	...
	if netmanthan.db.exists("Workspace", page.get("name")):
		netmanthan.get_doc("Workspace", page.get("name")).delete(ignore_permissions=True)
...
def sort_pages(sb_public_items, sb_private_items):
	...
	workspace_private_pages = get_page_list(["name", "title"], {"for_user": netmanthan.session.user})
	...
	for seq, d in enumerate(pages):
		for page in workspace_pages:
			if page.title == d.get("title"):
				doc = netmanthan.get_doc("Workspace", page.name)
...
def last_sequence_id(doc):
	doc_exists = netmanthan.db.exists(
...
def get_page_list(fields, filters):
	return netmanthan.get_all("Workspace", fields=fields, filters=filters, order_by="sequence_id asc")


def is_workspace_manager():
	return "Workspace Manager" in netmanthan.get_roles()
