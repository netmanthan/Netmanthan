...
def after_doctype_insert():
	netmanthan.db.add_unique("Desktop Icon", ("module_name", "owner", "standard"))
...
def get_desktop_icons(user=None):
	...
	user_icons = netmanthan.cache().hget("desktop_icons", user)
	...
	if not user_icons:
		...
		blocked_doctypes = netmanthan.get_all(
			"DocType",
		...
		standard_icons = netmanthan.get_all("Desktop Icon", fields=fields, filters={"standard": 1})
		...
		netmanthan.cache().hset("desktop_icons", user, user_icons)
...
def add_user_icon(_doctype, _report=None, label=None, link=None, type="link", standard=0):
	...
	icon_name = netmanthan.db.exists(
		"Desktop Icon", {"standard": standard, "link": link, "owner": netmanthan.session.user}
	)

	if icon_name:
		if netmanthan.db.get_value("Desktop Icon", icon_name, "hidden"):
			# if it is hidden, unhide it
			netmanthan.db.set_value("Desktop Icon", icon_name, "hidden", 0)
			clear_desktop_icons_cache()

	else:
		idx = (
                netmanthan.db.sql("select max(idx) from `tabDesktop Icon` where owner=%s", netmanthan.session.user)[0][
				0
			]
                or netmanthan.db.sql("select count(*) from `tabDesktop Icon` where standard=1")[0][0]
		)

		if not netmanthan.db.get_value("Report", _report):
			_report = None
			userdefined_icon = netmanthan.db.get_value(
		...
		module_icon = netmanthan.get_value(
			"Desktop Icon",
		...
		except netmanthan.UniqueValidationError as e:
			netmanthan.throw(_("Desktop Icon already exists"))
	...
	for i, module_name in enumerate(new_order):
		if module_name not in ("Explore",):
			if user:
				...
				name = netmanthan.db.get_value("Desktop Icon", {"standard": 1, "module_name": module_name})
				if name:
					icon = netmanthan.get_doc("Desktop Icon", name)
				else:
...
def set_desktop_icons(visible_list, ignore_duplicate=True):
	...
	if not int(netmanthan.defaults.get_defaults().setup_complete or 0):
		netmanthan.db.delete("Desktop Icon", {"standard": 0})

	# set standard as blocked and hidden if setting first active domain
	if not netmanthan.flags.keep_desktop_icons:
		netmanthan.db.sql("update `tabDesktop Icon` set blocked=0, hidden=1 where standard=1")

	# set as visible if present, or add icon
	for module_name in visible_list:
		name = netmanthan.db.get_value("Desktop Icon", {"module_name": module_name})
		if name:
			netmanthan.db.set_value("Desktop Icon", name, "hidden", 0)
		else:
			if netmanthan.db.exists("DocType", module_name):
				try:
...
def set_hidden(module_name, user=None, hidden=1):
	...
	if user:
		...
		if hidden and icon.custom:
			netmanthan.delete_doc(icon.doctype, icon.name, ignore_permissions=True)
			return

		# hidden by user
		icon.db_set("hidden", hidden)
	else:
		icon = netmanthan.get_doc("Desktop Icon", {"standard": 1, "module_name": module_name})
...
def get_all_icons():
	return [
		d.module_name
		for d in netmanthan.get_all("Desktop Icon", filters={"standard": 1}, fields=["module_name"])
	]


def clear_desktop_icons_cache(user=None):
	netmanthan.cache().hdel("desktop_icons", user or netmanthan.session.user)
	netmanthan.cache().hdel("bootinfo", user or netmanthan.session.user)
...
def get_user_copy(module_name, user=None):
	...
	desktop_icon_name = netmanthan.db.get_value(
		"Desktop Icon", {"module_name": module_name, "owner": user, "standard": 0}
	)

	if desktop_icon_name:
		return netmanthan.get_doc("Desktop Icon", desktop_icon_name)
	else:
...
def make_user_copy(module_name, user):
	...
	standard_name = netmanthan.db.get_value("Desktop Icon", {"module_name": module_name, "standard": 1})

	if not standard_name:
		netmanthan.throw(_("{0} not found").format(module_name), netmanthan.DoesNotExistError)

	original = netmanthan.get_doc("Desktop Icon", standard_name)

	desktop_icon = netmanthan.get_doc(
...
def sync_from_app(app):
	...
	try:
		modules = netmanthan.get_attr(app + ".config.desktop.get_data")() or {}
	except ImportError:
		return []
	...
	for i, m in enumerate(modules_list):
		...
		if desktop_icon_name:
			desktop_icon = netmanthan.get_doc("Desktop Icon", desktop_icon_name)
		else:
			# new icon
			desktop_icon = netmanthan.get_doc(
		...
		try:
			desktop_icon.save()
		except netmanthan.exceptions.UniqueValidationError:
			pass
...
def update_icons(hidden_list, user=None):
	...
	netmanthan.msgprint(netmanthan._("Updated"), indicator="green", title=_("Success"), alert=True)


def get_context(context):
	context.icons = get_user_icons(netmanthan.session.user)
	context.user = netmanthan.session.user

	if "System Manager" in netmanthan.get_roles():
		context.users = netmanthan.get_all(
			"User",
...
def get_module_icons(user=None):
	...
	if not user:
		icons = netmanthan.get_all("Desktop Icon", fields="*", filters={"standard": 1}, order_by="idx")
	else:
		netmanthan.cache().hdel("desktop_icons", user)
		icons = get_user_icons(user)

	for icon in icons:
		icon.value = netmanthan.db.escape(_(icon.label or icon.module_name))
...
def get_user_icons(user):
	...
	from netmanthan.boot import get_allowed_pages
