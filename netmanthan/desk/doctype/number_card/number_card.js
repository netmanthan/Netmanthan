...
netmanthan.ui.form.on("Number Card", {
	...
	create_add_to_dashboard_button: function (frm) {
		frm.add_custom_button("Add Card to Dashboard", () => {
			const dialog = netmanthan.dashboard_utils.get_add_to_dashboard_dialog(
				frm.doc.name,
				"Number Card",
				"netmanthan.desk.doctype.number_card.number_card.add_card_to_dashboard"
			);

			if (!frm.doc.name) {
				netmanthan.msgprint(__("Please create Card first"));
			} else {
				dialog.show();
			}
		});
...
[{
	...
	fieldtype: "Link",
	options: "Company",
	default: netmanthan.defaults.get_user_default("Company"),
	reqd: 1
},
{
	fieldname: "account",
...
<code>
{
	...
	filters_config: function (frm) {
		frm.filters = eval(frm.doc.filters_config);
		const filter_values = netmanthan.report_utils.get_filter_values(frm.filters);
		...
		if (doctype) {
			netmanthan.model.with_doctype(doctype, () => {
				netmanthan.get_meta(doctype).fields.map((df) => {
					if (netmanthan.model.numeric_fieldtypes.includes(df.fieldtype)) {
	...
	set_report_filters: function (frm) {
		...
		if (report_name) {
			netmanthan.report_utils.get_report_filters(report_name).then((filters) => {
				if (filters) {
					frm.filters = filters;
					const filter_values = netmanthan.report_utils.get_filter_values(filters);
	...
	render_dynamic_filters_table(frm) {
		if (!netmanthan.boot.developer_mode || !frm.doc.is_standard || frm.doc.type == "Custom") {
			return;
		}
