...
class Note(Document):
	...
	def validate(self):
		if self.notify_on_login and not self.expire_notification_on:
			...
			self.expire_notification_on = netmanthan.utils.add_days(self.creation, 7)
...
def mark_as_seen(note):
	note = netmanthan.get_doc("Note", note)
	if netmanthan.session.user not in [d.user for d in note.seen_by]:
		note.append("seen_by", {"user": netmanthan.session.user})
...
def get_permission_query_conditions(user):
	...
	return f"""(`tabNote`.public=1 or `tabNote`.owner={netmanthan.db.escape(user)})"""
