...
netmanthan.ui.form.on("Notification Log", {
	...
	open_reference_document: function (frm) {
		...
		netmanthan.set_route("Form", dt, dn);
	},
	...
	set_attachment: function (frm) {
		...
		$wrapper.find(".attached-file-link").click(() => {
			const w = window.open(
				netmanthan.urllib.get_full_url(`/api/method/netmanthan.utils.print_format.download_pdf?
