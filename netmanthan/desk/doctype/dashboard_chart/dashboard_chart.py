...
from netmanthan.utils import cint, get_datetime, getdate, has_common, now_datetime, nowdate
from netmanthan.utils.dashboard import cache_source
from netmanthan.utils.data import format_date
from netmanthan.utils.dateutils import (
...
def get_permission_query_conditions(user):
	...
	allowed_reports = [netmanthan.db.escape(report) for report in get_allowed_report_names()]
	allowed_modules = [
...
def has_permission(doc, ptype, user):
	...
	if doc.chart_type == "Report":
		...
		allowed_doctypes = netmanthan.permissions.get_doctypes_with_read()
...
def get(
	...
	if chart_name:
		chart = netmanthan.get_doc("Dashboard Chart", chart_name)
	else:
		chart = netmanthan._dict(netmanthan.parse_json(chart))
	...
	filters = netmanthan.parse_json(filters) or netmanthan.parse_json(chart.filters_json)
	if not filters:
		filters = []
...
def create_dashboard_chart(args):
	...
	doc = netmanthan.new_doc("Dashboard Chart")
	...
	if netmanthan.db.exists("Dashboard Chart", args.chart_name):
...
def add_chart_to_dashboard(args):
	...
	dashboard_link = netmanthan.new_doc("Dashboard Chart Link")
	dashboard_link.chart = args.chart_name or args.name

	if args.set_standard and dashboard.is_standard:
		chart = netmanthan.get_doc("Dashboard Chart", dashboard_link.chart)
		chart.is_standard = 1
...
def get_charts_for_user(doctype, txt, searchfield, start, page_len, filters):
	or_filters = {"owner": netmanthan.session.user, "is_public": 1}
...
class DashboardChart(Document):
	def on_update(self):
		netmanthan.cache().delete_key(f"chart-data:{self.name}")
	...
	def validate(self):
		if not netmanthan.conf.developer_mode and self.is_standard:
			netmanthan.throw(_("Cannot edit Standard charts"))
	...
	def check_required_field(self):
		if not self.document_type:
			netmanthan.throw(_("Document type is required to create a dashboard chart"))

		if (
			self.document_type
			and netmanthan.get_meta(self.document_type).istable
			and not self.parent_document_type
		):
			netmanthan.throw(_("Parent document type is required to create a dashboard chart"))

		if self.chart_type == "Group By":
			if not self.group_by_based_on:
				netmanthan.throw(_("Group By field is required to create a dashboard chart"))
			if self.group_by_type in ["Sum", "Average"] and not self.aggregate_function_based_on:
				netmanthan.throw(_("Aggregate Function field is required to create a dashboard chart"))
		else:
			if not self.based_on:
				netmanthan.throw(_("Time series based on is required to create a dashboard chart"))

	def check_document_type(self):
		if netmanthan.get_meta(self.document_type).issingle:
			netmanthan.throw(_("You cannot create a dashboard chart from single DocTypes"))
