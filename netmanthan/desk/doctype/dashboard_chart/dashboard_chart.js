...
netmanthan.ui.form.on("Dashboard Chart", {
	setup: function (frm) {
	...
	before_save: function (frm) {
		...
		static_filters = netmanthan.dashboard_utils.remove_common_static_filter_values(
			static_filters,
			dynamic_filters
		);
	...
	refresh: function (frm) {
		...
		frm.is_disabled = !netmanthan.boot.developer_mode && frm.doc.is_standard;
		...
		frm.add_custom_button("Add Chart to Dashboard", () => {
			const dialog = netmanthan.dashboard_utils.get_add_to_dashboard_dialog(
				frm.doc.name,
				"Dashboard Chart",
				"netmanthan.desk.doctype.dashboard_chart.dashboard_chart.add_chart_to_dashboard"
			);

			if (!frm.doc.chart_name) {
				netmanthan.msgprint(__("Please create chart first"));
			} else {
				dialog.show();
			}
		});
	...
	set_heatmap_year_options: function (frm) {
		if (frm.doc.type == "Heatmap") {
			netmanthan.db.get_doc("System Settings").then((doc) => {
				const creation_date = doc.creation;
				frm.set_df_property(
					"heatmap_year",
					"options",
					netmanthan.dashboard_utils.get_years_since_creation(creation_date)
				);
			});
		}
	},
	...
	set_chart_report_filters: function (frm) {
		...
		if (report_name) {
			if (frm.doc.filters_json.length > 2) {
				...
				netmanthan.report_utils.get_report_filters(report_name).then((filters) => {
					if (filters) {
						frm.chart_filters = filters;
						let filter_values = netmanthan.report_utils.get_filter_values(filters);
	...
	set_chart_field_options: function (frm) {
		...
		if (frm.doc.dynamic_filters_json && frm.doc.dynamic_filters_json.length > 2) {
			filters = netmanthan.dashboard_utils.get_all_filters(frm.doc);
		}
		netmanthan
			.xcall("netmanthan.desk.query_report.run", {
			...
			.then((data) => {
				...
				if (!frm.doc.use_report_chart) {
					if (data.result.length) {
						frm.field_options = netmanthan.report_utils.get_field_options_from_report(
							data.columns,
							data
						);
						frm.set_df_property(
						...
						if (!frm.field_options.numeric_fields.length) {
							...
							let y_field_df = netmanthan.meta.get_docfield(
	...
	update_options: function (frm) {
		...
		if (doctype) {
			netmanthan.model.with_doctype(doctype, () => {
				...
				netmanthan.get_meta(doctype).fields.map((df) => {
	...
	show_filters: function (frm) {
		frm.chart_filters = [];
		netmanthan.dashboard_utils.get_filters_for_chart_type(frm.doc).then((filters) => {
			if (filters) {
				frm.chart_filters = filters;
			}
	...
	render_filters_table: function (frm) {
		...
		table.on("click", () => {
			frm.is_disabled && netmanthan.throw(__("Cannot edit filters for standard charts"));
			...
			netmanthan.dashboards.filters_dialog = dialog;

			if (is_document_type) {
				frm.filter_group = new netmanthan.ui.FilterGroup({
			...
			if (frm.doc.chart_type == "Report") {
				...
				netmanthan.query_reports[frm.doc.report_name] &&
					netmanthan.query_reports[frm.doc.report_name].onload &&
					netmanthan.query_reports[frm.doc.report_name].onload(netmanthan.query_report);
			}
	...
	render_dynamic_filters_table(frm) {
		...
		let fields = netmanthan.dashboard_utils.get_fields_for_dynamic_filter_dialog(
			is_document_type,
			filters,
			frm.dynamic_filters
		);
	...
	set_parent_document_type: async function (frm) {
		...
		let doc_is_table =
			document_type &&
			(await netmanthan.db.get_value("DocType", document_type, "istable")).message.istable;

		frm.set_df_property("parent_document_type", "hidden", !doc_is_table);

		if (document_type && doc_is_table) {
			let parents = await netmanthan.xcall(
				"netmanthan.desk.doctype.dashboard_chart.dashboard_chart.get_parent_doctypes",
