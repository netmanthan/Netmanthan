...
from netmanthan.contacts.doctype.contact.contact import get_default_contact
...
class Event(Document):
	def validate(self):
		...
		if (
			...
			netmanthan.throw(_("Daily Events should finish on the Same Day."))

		if self.sync_with_google_calendar and not self.google_calendar:
			netmanthan.throw(_("Select Google Calendar to which event should be synced."))
	...
	def on_trash(self):
		...
		if communications:
			for communication in communications:
				netmanthan.delete_doc_if_exists("Communication", communication.name)
	...
	def sync_communication(self):
		if self.event_participants:
			for participant in self.event_participants:
				...
				comms = netmanthan.get_all("Communication", filters=filters, fields=["name"])

				if comms:
					for comm in comms:
						communication = netmanthan.get_doc("Communication", comm.name)
						self.update_communication(participant, communication)
				else:
					meta = netmanthan.get_meta(participant.reference_doctype)
	...
	def update_communication(self, participant, communication):
		...
		communication.sender_full_name = netmanthan.utils.get_fullname(self.owner)
		communication.reference_doctype = self.doctype
		communication.reference_name = self.name
	...
	def set_participants_email(self):
		for participant in self.event_participants:
			...
			participant.email = (
				netmanthan.get_value("Contact", participant_contact, "email_id") if participant_contact else None
			)


@netmanthan.whitelist()
def delete_communication(event, reference_doctype, reference_docname):
	deleted_participant = netmanthan.get_doc(reference_doctype, reference_docname)
	...
	comms = netmanthan.get_list("Communication", filters=filters, fields=["name"])

	if comms:
		deletion = []
		for comm in comms:
			delete = netmanthan.get_doc("Communication", comm.name).delete()
...
def send_event_digest():
	...
	for user in users:
		...
		if events:
			netmanthan.set_user_lang(user.name, user.language)
			...
			netmanthan.sendmail(
				...
				subject=netmanthan._("Upcoming Events for Today"),
				template="upcoming_events",
				args={
					"events": events,
				},
				header=[netmanthan._("Events in Today's Calendar"), "blue"],
			)
...
def delete_events(ref_type, ref_name, delete_event=False):
	participations = netmanthan.get_all(
		"Event Participants",
	...
	if participations:
		for participation in participations:
			if delete_event:
				netmanthan.delete_doc("Event", participation.parent, for_reload=True)
			else:
				total_participants = netmanthan.get_all(
					"Event Participants", filters={"parenttype": "Event", "parent": participation.parent}
				)

				if len(total_participants) <= 1:
					netmanthan.db.delete("Event", {"name": participation.parent})
					netmanthan.db.delete("Event Participants", {"name": participation.name})


# Close events if ends_on or repeat_till is less than now_datetime
def set_status_of_events():
	events = netmanthan.get_list(
	...
	for event in events:
		if (event.ends_on and getdate(event.ends_on) < getdate(nowdate())) or (
			...
			netmanthan.db.set_value("Event", event.name, "status", "Closed")
