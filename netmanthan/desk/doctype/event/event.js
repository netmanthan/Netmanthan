...
netmanthan.ui.form.on("Event", {
	...
	refresh: function (frm) {
		if (frm.doc.event_participants) {
			frm.doc.event_participants.forEach((value) => {
				frm.add_custom_button(
					...
					function () {
						netmanthan.set_route("Form", value.reference_doctype, value.reference_docname);
					},
					__("Participants")
				);
			});
		...
		frm.add_custom_button(
			__("Add Contacts"),
			function () {
				new netmanthan.desk.eventParticipants(frm, "Contact");
			},
			__("Add Participants")
		);
		...
		if (frm.doc.google_meet_link && netmanthan.datetime.now_date() <= ends_on_date) {
...
netmanthan.ui.form.on("Event Participants", {
	event_participants_remove: function (frm, cdt, cdn) {
		if (cdt && !cdn.includes("New Event Participants")) {
			netmanthan.call({
				type: "POST",
				method: "netmanthan.desk.doctype.event.event.delete_communication",
				args: {
					event: frm.doc,
...
netmanthan.desk.eventParticipants = class eventParticipants {
