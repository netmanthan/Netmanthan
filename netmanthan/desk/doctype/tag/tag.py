...
from netmanthan.utils import unique
...
def check_user_tags(dt):
	...
	try:
		doctype = DocType(dt)
		netmanthan.qb.from_(doctype).select(doctype._user_tags).limit(1).run()
	except Exception as e:
		if netmanthan.db.is_column_missing(e):
			DocTags(dt).setup()
...
def add_tags(tags, dt, docs, color=None):
	"adds a new tag to a record, and creates the Tag master"
	tags = netmanthan.parse_json(tags)
...
def get_tagged_docs(doctype, tag):
	netmanthan.has_permission(doctype, throw=True)
	doctype = DocType(doctype)
	return (netmanthan.qb.from_(doctype).where(doctype._user_tags.like(tag)).select(doctype.name)).run()


@netmanthan.whitelist()
def get_tags(doctype, txt):
	tag = netmanthan.get_list("Tag", filters=[["name", "like", f"%{txt}%"]])
...
class DocTags:
	...
	def get_tag_fields(self):
		...
		return netmanthan.db.get_value("DocType", self.dt, "tag_fields")

	def get_tags(self, dn):
		"""returns tag for a particular item"""
		return (netmanthan.db.get_value(self.dt, dn, "_user_tags", ignore=1) or "").strip()
	...
	def add(self, dn, tag):
		...
		if not tag in tl:
			tl.append(tag)
			if not netmanthan.db.exists("Tag", tag):
				netmanthan.get_doc({"doctype": "Tag", "name": tag}).insert(ignore_permissions=True)
			self.update(dn, tl)
	...
	def update(self, dn, tl):
		...
		try:
			...
			doc = netmanthan.get_doc(self.dt, dn)
...
def delete_tags_for_document(doc):
	...
	"""
	if not netmanthan.db.table_exists("Tag Link"):
		return

	netmanthan.db.delete("Tag Link", {"document_type": doc.doctype, "document_name": doc.name})
...
def update_tags(doc, tags):
	...
	existing_tags = [
		tag.tag
		for tag in netmanthan.get_list(
...
def get_tags_list_for_awesomebar():
	return netmanthan.get_list("Tag", pluck="name", order_by=None)
