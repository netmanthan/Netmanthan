...
def get_notifications():
	...
	if netmanthan.flags.in_install or not netmanthan.db.get_single_value("System Settings", "setup_complete"):
		return out
	...
	for name in groups:
		count = cache.hget("notification_count:" + name, netmanthan.session.user)
			...
			if d in notification_count:
				...
				try:
					if isinstance(condition, dict):
						result = netmanthan.get_list(
							d, fields=["count(*) as count"], filters=condition, ignore_ifnull=True
						)[0].count
					else:
						result = netmanthan.get_attr(condition)()

				except netmanthan.PermissionError:
					netmanthan.clear_messages()
					pass
					# netmanthan.msgprint("Permission Error in notifications for {0}".format(d))
					...
					netmanthan.cache().hset("notification_count:" + d, netmanthan.session.user, result)
...
def clear_notification_config(user):
	netmanthan.cache().hdel("notification_config", user)


def delete_notification_count_for(doctype):
	netmanthan.cache().delete_key("notification_count:" + doctype)
...
def get_notification_info():
	...
	conditions = {}
	module_doctypes = {}
	doctype_info = dict(netmanthan.db.sql("""select name, module from tabDocType"""))
...
def get_notification_config():
	...
	return netmanthan.cache().hget("notification_config", user, _get)
...
def get_open_count(doctype, name, items=None):
	...
	if netmanthan.flags.in_migrate or netmanthan.flags.in_install:
		return {"count": []}
	...
	if not meta.custom:
		module = netmanthan.get_meta_module(doctype)
...
def notify_mentions(ref_doctype, ref_name, content):
	if ref_doctype and ref_name and content:
		...
		sender_fullname = get_fullname(netmanthan.session.user)
		...
		notification_message = _("""{0} mentioned you in a comment in {1} {2}""").format(
			netmanthan.bold(sender_fullname), netmanthan.bold(ref_doctype), get_title_html(title)
		)
...
def extract_mentions(txt):
	...
	for mention in soup.find_all(class_="mention"):
		if mention.get("data-is-group") == "true":
			try:
				user_group = netmanthan.get_cached_doc("User Group", mention["data-id"])
