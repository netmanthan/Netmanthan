...
from netmanthan.core.utils import ljust_list
...
from netmanthan.permissions import get_role_permissions
...
def get_report_doc(report_name):
	...
	if not netmanthan.has_permission(doc.ref_doctype, "report"):
		netmanthan.throw(
	...
	if doc.disabled:
		netmanthan.throw(_("Report {0} is disabled").format(report_name))
...
def generate_report_result(
	...
	return {
		...
		"status": None,
		"execution_time": netmanthan.cache().hget("report_execution_time", report.name) or 0,
	}
...
def background_enqueue_run(report_name, filters=None, user=None):
	from netmanthan.core.doctype.prepared_report.prepared_report import make_prepared_report
...
def get_script(report_name):
	...
	module = report.module or netmanthan.db.get_value("DocType", report.ref_doctype, "module")

	is_custom_module = netmanthan.get_cached_value("Module Def", module, "custom")
	...
	if not script:
		script = "netmanthan.query_reports['%s']={}" % report_name

	return {
		"script": render_include(script),
		"html_format": html_format,
		"execution_time": netmanthan.cache().hget("report_execution_time", report_name) or 0,
...
def get_reference_report(report):
	if report.report_type != "Custom Report":
		return report
	reference_report = netmanthan.get_doc("Report", report.reference_report)
...
def run(
	...
	if not netmanthan.has_permission(report.ref_doctype, "report"):
		netmanthan.msgprint(
		...
		if key in custom_column_data:
			for row in result:
				...
				if isinstance(link_field, str):
					link_field = netmanthan._dict({"fieldname": link_field, "names": []})
...
def get_prepared_report_result(report, filters, dn=None, user=None):
	from netmanthan.core.doctype.prepared_report.prepared_report import get_completed_prepared_report
	...
	doc = netmanthan.get_doc("Prepared Report", dn) if dn else None
	if doc:
		try:
			...
			netmanthan.msgprint(_("Prepared report render failed"))
			doc = None
...
def export_query():
	...
	if data.get("report_name"):
		...
		netmanthan.permissions.can_export(
			netmanthan.get_cached_value("Report", report_name, "ref_doctype"),
			raise_exception=True,
		)

	file_format_type = data.get("file_format_type")
	custom_columns = netmanthan.parse_json(data.get("custom_columns", "[]"))
	...
	if file_format_type == "Excel":
		...
		netmanthan.response["filename"] = _(report_name) + ".xlsx"
		netmanthan.response["filecontent"] = xlsx_file.getvalue()
		netmanthan.response["type"] = "binary"


def format_duration_fields(data: netmanthan._dict) -> None:
...
def get_data_for_custom_field(doctype, field, names=None):
	...
	if not netmanthan.has_permission(doctype, "read"):
		netmanthan.throw(_("Not Permitted to read {0}").format(doctype), netmanthan.PermissionError)

	filters = {}
	if names:
		if isinstance(names, (str, bytearray)):
			names = netmanthan.json.loads(names)
		filters.update({"name": ["in", names]})

	value_map = netmanthan._dict(
		netmanthan.get_list(doctype, filters=filters, fields=["name", field], as_list=1)
	)
	return value_map
...
def save_report(reference_report, report_name, columns, filters):
	...
	if docname:
		report = netmanthan.get_doc("Report", docname)
		...
		report.save()
		netmanthan.msgprint(_("Report updated successfully"))

		return docname
	else:
		new_report = netmanthan.get_doc(
			{
				"doctype": "Report",
				"report_name": report_name,
		...
		netmanthan.msgprint(_("{0} saved successfully").format(new_report.name))
...
def get_filtered_data(ref_doctype, columns, data, user):
	...
	shared = netmanthan.share.get_shared(ref_doctype, user)
	columns_dict = get_columns_dict(columns)

	role_permissions = get_role_permissions(netmanthan.get_meta(ref_doctype), user)
			...
			for match_filters in filter_list:
				...
				for dt, idx in linked_doctypes.items():
					...
					if (
						...
						and netmanthan.db.exists(dt, cell_value)
					):
						match = False
						break
...
def get_column_as_dict(col):
	...
	if isinstance(col, str):
		...
		col_dict["fieldname"] = netmanthan.scrub(col[0])

	# dict
	else:
		col_dict.update(col)
		if "fieldname" not in col_dict:
			col_dict["fieldname"] = netmanthan.scrub(col_dict["label"])
...
def get_user_match_filters(doctypes, user):
	...
	for dt in doctypes:
		filter_list = netmanthan.desk.reportview.build_match_conditions(dt, user, False)
		if filter_list:
