...
from netmanthan.model import child_table_fields, default_fields, get_permitted_fields, optional_fields
...
from netmanthan.utils import add_user_info, cstr, format_duration
...
def get_list():
	...
	if is_virtual_doctype(args.doctype):
		...
		# uncompressed (refactored from netmanthan.model.db_query.get_list)
		data = execute(**args)
...
def get_form_params():
	...
	data = netmanthan._dict(netmanthan.local.form_dict)
	clean_params(data)
	validate_args(data)
	return data
	...
	if isinstance(filters, list):
		...
		for condition in filters:
			if len(condition) == 3:
				...
				if is_standard(fieldname):
					continue
				meta = netmanthan.get_meta(condition[0])
...
def setup_group_by(data):
	...
	if data.group_by and data.aggregate_function:
		if data.aggregate_function.lower() not in ("count", "sum", "avg"):
			netmanthan.throw(_("Invalid aggregate function"))

		if netmanthan.db.has_column(data.aggregate_on_doctype, data.aggregate_on_field):
			data.fields.append(
...
def raise_invalid_field(fieldname):
	netmanthan.throw(_("Field not permitted in query") + f": {fieldname}", netmanthan.DataError)
...
def get_meta_and_docfield(fieldname, data):
	...
	meta = netmanthan.get_meta(parenttype)
...
def update_wildcard_field_param(data):
	if (isinstance(data.fields, str) and data.fields == "*") or (
		...
		if netmanthan.get_system_settings("apply_perm_level_on_api_calls"):
			...
			data.fields = netmanthan.db.get_table_columns(data.doctype)
		return True
...
def get_parenttype_and_fieldname(field, data):
	if "." in field:
		...
		if parenttype.startswith("`tab"):
			...
			parenttype = netmanthan.get_meta(data.doctype).get_field(parenttype).options
	else:
...
def compress(data, args=None):
	...
	from netmanthan.desk.query_report import add_total_row
	...
	if args.get("add_total_row"):
		meta = netmanthan.get_meta(args.doctype)
...
def save_report(name, doctype, report_settings):
	...
	if netmanthan.db.exists("Report", name):
		report = netmanthan.get_doc("Report", name)
		if report.is_standard == "Yes":
			netmanthan.throw(_("Standard Reports cannot be edited"))

		if report.report_type != "Report Builder":
			netmanthan.throw(_("Only reports of type Report Builder can be edited"))

		if report.owner != netmanthan.session.user and not report.has_permission("write"):
			netmanthan.throw(_("Insufficient Permissions for editing Report"), netmanthan.PermissionError)
	else:
		report = netmanthan.new_doc("Report")
	...
	netmanthan.msgprint(
		_("Report {0} saved").format(netmanthan.bold(report.name)),
		indicator="green",
		alert=True,
	)
	return report.name
...
def delete_report(name):
	...
	report = netmanthan.get_doc("Report", name)
	if report.is_standard == "Yes":
		netmanthan.throw(_("Standard Reports cannot be deleted"))

	if report.report_type != "Report Builder":
		netmanthan.throw(_("Only reports of type Report Builder can be deleted"))

	if report.owner != netmanthan.session.user and not report.has_permission("delete"):
		netmanthan.throw(_("Insufficient Permissions for deleting Report"), netmanthan.PermissionError)

	report.delete(ignore_permissions=True)
	netmanthan.msgprint(
		_("Report {0} deleted").format(netmanthan.bold(report.name)),
		indicator="green",
		alert=True,
	)
...
def export_query():
	...
	title = netmanthan.form_dict.title
	netmanthan.form_dict.pop("title", None)
	...
	netmanthan.permissions.can_export(doctype, raise_exception=True)

	if "selected_items" in form_params:
		si = json.loads(netmanthan.form_dict.get("selected_items"))
	...
	if file_format_type == "CSV":
		...
		for r in data:
			...
			writer.writerow([handle_html(netmanthan.as_unicode(v)) if isinstance(v, str) else v for v in r])

		f.seek(0)
		netmanthan.response["result"] = cstr(f.read())
		netmanthan.response["type"] = "csv"
		netmanthan.response["doctype"] = title
		...
		netmanthan.response["filename"] = _(title) + ".xlsx"
		netmanthan.response["filecontent"] = xlsx_file.getvalue()
		netmanthan.response["type"] = "binary"
...
def get_labels(fields, doctype):
	...
	for key in fields:
		...
		if parenttype == doctype and fieldname == "name":
			...
			df = netmanthan.get_meta(parenttype).get_field(fieldname)
...
def delete_items():
	...
	items = sorted(json.loads(netmanthan.form_dict.get("items")), reverse=True)
	doctype = netmanthan.form_dict.get("doctype")

	if len(items) > 10:
		netmanthan.enqueue("netmanthan.desk.reportview.delete_bulk", doctype=doctype, items=items)
	else:
...
def delete_bulk(doctype, items):
	for i, d in enumerate(items):
		try:
			netmanthan.delete_doc(doctype, d)
			if len(items) >= 5:
...
def get_stats(stats, doctype, filters=None):
	...
	try:
		db_columns = netmanthan.db.get_table_columns(doctype)
	except (netmanthan.db.InternalError, netmanthan.db.ProgrammingError):
	...
	for column in columns:
		...
		try:
			...
			if column == "_user_tags":
				...
				no_tag_count = netmanthan.get_list(
					doctype,
					fields=[column, "count(*)"],
		...
		except netmanthan.db.SQLError:
			pass
		except netmanthan.db.InternalError as e:
			# raised when _user_tags column is added on the fly
			pass
