...
from netmanthan.model.meta import is_single
...
class SubmittableDocumentTree:
	def __init__(self, doctype: str, name: str):
		...
		* Node is referenced(linked) by many other documents and those are the child nodes.
		...
		NOTE: child document is a property of child node (not same as netmanthan child docs of a table field).
		"""
	...
	def get_submittable_doctypes(self) -> list[str]:
		...
		if not self._submittable_doctypes:
			self._submittable_doctypes = netmanthan.get_all(
...
def get_references_across_doctypes_by_dynamic_link_field(
	...
	for doctype, fieldname, doctype_fieldname in links:
		try:
			...
			for linked_to in netmanthan.get_all(doctype, pluck=doctype_fieldname, filters=filters, distinct=1):
				if linked_to:
					links_by_doctype[linked_to].append(
						{"doctype": doctype, "fieldname": fieldname, "doctype_fieldname": doctype_fieldname}
					)
		except netmanthan.db.ProgrammingError:
			# TODO: FIXME
			continue
	return links_by_doctype
...
def get_referencing_documents(
	...
	if not link_info.get("is_child"):
		...
		return {from_table: netmanthan.get_all(from_table, filters, pluck="name", order_by=None)}
	...
	for parent, rows in itertools.groupby(res, key=lambda row: row["parenttype"]):
		...
		documents[parent].extend(
            netmanthan.get_all(parent, filters=filters, pluck="name", order_by=None) or []
		)
	return documents
...
def cancel_all_linked_docs(docs, ignore_doctypes_on_cancel_all=None):
	...
	for i, doc in enumerate(docs, 1):
		if validate_linked_doc(doc, ignore_doctypes_on_cancel_all):
			linked_doc = netmanthan.get_doc(doc.get("doctype"), doc.get("name"))
			linked_doc.cancel()
		netmanthan.publish_progress(percent=i / len(docs) * 100, title=_("Cancelling documents"))
...
def validate_linked_doc(docinfo, ignore_doctypes_on_cancel_all=None):
	...
	if not netmanthan.get_meta(docinfo.get("doctype")).is_submittable:
		return False
...
def get_exempted_doctypes():
	...
	for doctypes in netmanthan.get_hooks("auto_cancel_exempted_doctypes"):
...
def get_linked_docs(doctype: str, name: str, linkinfo: dict | None = None) -> dict[str, list]:
	...
	for dt, link in linkinfo.items():
		...
		link["doctype"] = dt
		try:
			link_meta_bundle = netmanthan.desk.form.load.get_meta_bundle(dt)
		except Exception as e:
			if isinstance(e, netmanthan.DoesNotExistError):
				if netmanthan.local.message_log:
					netmanthan.local.message_log.pop()
			continue
		...
		if not linkmeta.get("issingle"):
			fields = [
				...
				for d in linkmeta.get(
					...
					{
						"in_list_view": 1,
						"fieldtype": ["not in", ("Image", "HTML", "Button") + netmanthan.model.table_fields],
					},
				)
			...
			try:
				if link.get("filters"):
					ret = netmanthan.get_all(doctype=dt, fields=fields, filters=link.get("filters"), order_by=None)
					...
					me = netmanthan.db.get_value(doctype, name, ["parenttype", "parent"], as_dict=True, order_by=None)
					if me and me.parenttype == dt:
						ret = netmanthan.get_all(
...
def get_linked_doctypes(doctype, without_ignore_user_permissions_enabled=False):
	...
	if without_ignore_user_permissions_enabled:
		...
		return netmanthan.cache().hget("linked_doctypes", doctype, lambda: _get_linked_doctypes(doctype))
...
def _get_linked_doctypes(doctype, without_ignore_user_permissions_enabled=False):
	...
	links = netmanthan.get_all("DocField", fields=["parent as dt"], filters=filters)
	links += netmanthan.get_all("Custom Field", fields=["dt"], filters=filters)
...
def get_linked_fields(doctype, without_ignore_user_permissions_enabled=False):
	...
	# find links of parents
	links = netmanthan.get_all("DocField", fields=["parent", "fieldname"], filters=filters, as_list=1)
	links += netmanthan.get_all(
	...
	table_doctypes = netmanthan.get_all(
	...
	for parent, options in netmanthan.get_all(
...
def get_dynamic_linked_fields(doctype, without_ignore_user_permissions_enabled=False):
	...
	for df in links:
		...
		is_child = netmanthan.get_meta(df.doctype).istable
		possible_link = netmanthan.get_all(
			df.doctype,
