...
from netmanthan.build import scrub_html_template
from netmanthan.model.meta import Meta
from netmanthan.model.utils import render_include
from netmanthan.modules import get_module_path, load_doctype_module, scrub
from netmanthan.utils import get_html_format
...
def get_meta(doctype, cached=True):
	...
	if cached and not netmanthan.conf.developer_mode:
		meta = netmanthan.cache().hget("doctype_form_meta", doctype)
		if not meta:
			meta = FormMeta(doctype)
			netmanthan.cache().hset("doctype_form_meta", doctype, meta)
	else:
		meta = FormMeta(doctype)

	if netmanthan.local.lang != "en":
		meta.set_translations(netmanthan.local.lang)
...
class FormMeta(Meta):
	...
	def add_code(self):
		...
		system_country = netmanthan.get_system_settings("country")
...
"""
	...
	def add_search_fields(self):
		...
		for df in self.get("fields", {"fieldtype": "Link", "options": ["!=", "[Select]"]}):
			if df.options:
				try:
					search_fields = netmanthan.get_meta(df.options).search_fields
	...
	def _show_missing_doctype_msg(self, df):
		...
		msg = _("Field {0} is referring to non-existing doctype {1}.").format(
			netmanthan.bold(df.fieldname), netmanthan.bold(df.options)
		)
		...
		netmanthan.throw(msg, title=_("Missing DocType"))

	def add_linked_document_type(self):
		for df in self.get("fields", {"fieldtype": "Link"}):
			if df.options:
				try:
					df.linked_document_type = netmanthan.get_meta(df.options).document_type
				except netmanthan.DoesNotExistError:
					self._show_missing_doctype_msg(df)

	def load_print_formats(self):
		print_formats = netmanthan.db.sql(
	...
	def load_workflows(self):
		...
		if workflow_name and netmanthan.db.exists("Workflow", workflow_name):
			workflow = netmanthan.get_doc("Workflow", workflow_name)
			workflow_docs.append(workflow)

			for d in workflow.get("states"):
				workflow_docs.append(netmanthan.get_doc("Workflow State", d.state))
		...
		if not self.custom:
			...
			if hasattr(module, "form_grid_templates"):
				for key, path in module.form_grid_templates.items():
					templates[key] = get_html_format(netmanthan.get_app_path(app, path))

				self.set("__form_grid_templates", templates)

	def set_translations(self, lang):
		from netmanthan.translate import extract_messages_from_code, make_dict_from_messages

		self.set("__messages", netmanthan.get_lang_dict("doctype", self.name))
...
def get_code_files_via_hooks(hook, name):
	...
	for app_name in netmanthan.get_installed_apps():
		code_hook = netmanthan.get_hooks(hook, default={}, app_name=app_name)
		if not code_hook:
			continue
		...
		for file in files:
			path = netmanthan.get_app_path(app_name, *file.strip("/").split("/"))
