...
from netmanthan.utils import get_url_to_form


@netmanthan.whitelist()
def update_follow(doctype, doc_name, following):
	if following:
		return follow_document(doctype, doc_name, netmanthan.session.user)
	else:
		return unfollow_document(doctype, doc_name, netmanthan.session.user)
...
def follow_document(doctype, doc_name, user):
	...
	if (not netmanthan.get_meta(doctype).track_changes) or user == "Administrator":
		return

	if not netmanthan.db.get_value("User", user, "document_follow_notify", ignore=True, cache=True):
		return

	if not is_document_followed(doctype, doc_name, user):
		doc = netmanthan.new_doc("Document Follow")
...
def unfollow_document(doctype, doc_name, user):
	...
	if doc:
		netmanthan.delete_doc("Document Follow", doc[0].name)
		return 1
	return 0
...
def get_user_list(frequency):
	...
	User = DocType("User")
	return (
		netmanthan.qb.from_(DocumentFollow)
		.join(User)
...
def get_version(doctype, doc_name, frequency, user):
	...
	if version:
		for v in version:
			change = netmanthan.parse_json(v.data)
			time = netmanthan.utils.format_datetime(v.modified, "hh:mm a")
			timeline_items = []
			if change.changed:
...
def get_comments(doctype, doc_name, frequency, user):
	...
	for comment in comments:
		...
		time = netmanthan.utils.format_datetime(comment.modified, "hh:mm a")
		timeline.append(
			{
...
def _get_filters(frequency, user):
	...
	if frequency == "Weekly":
		filters += [
			["modified", ">", netmanthan.utils.add_days(netmanthan.utils.nowdate(), -7)],
			["modified", "<", netmanthan.utils.nowdate()],
		]

	elif frequency == "Daily":
		filters += [
			["modified", ">", netmanthan.utils.add_days(netmanthan.utils.nowdate(), -1)],
			["modified", "<", netmanthan.utils.nowdate()],
		]

	elif frequency == "Hourly":
		filters += [
			["modified", ">", netmanthan.utils.add_to_date(netmanthan.utils.now_datetime(), hours=-1)],
			["modified", "<", netmanthan.utils.now_datetime()],
		]
