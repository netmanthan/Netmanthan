...
def _toggle_like(doctype, name, add, user=None):
	...
	try:
		liked_by = netmanthan.db.get_value(doctype, name, "_liked_by")
		...
		if add == "Yes":
			if user not in liked_by:
				...
				if netmanthan.get_cached_value("User", user, "follow_liked_documents"):
		...
		netmanthan.db.set_value(doctype, name, "_liked_by", json.dumps(liked_by), update_modified=False)
