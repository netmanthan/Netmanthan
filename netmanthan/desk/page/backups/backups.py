...
from netmanthan.utils import cint, get_site_path, get_url
from netmanthan.utils.data import convert_utc_to_system_timezone
...
def get_scheduled_backup_limit():
	backup_limit = netmanthan.db.get_singles_value("System Settings", "backup_limit")
...
def schedule_files_backup(user_email):
	...
	queued_jobs = get_jobs(site=netmanthan.local.site, queue="long")
	method = "netmanthan.desk.page.backups.backups.backup_files_and_notify_user"

	if method not in queued_jobs[netmanthan.local.site]:
		enqueue(
			"netmanthan.desk.page.backups.backups.backup_files_and_notify_user",
			queue="long",
			user_email=user_email,
		)
		netmanthan.msgprint(_("Queued for backup. You will receive an email with the download link"))
	else:
		netmanthan.msgprint(
...
def backup_files_and_notify_user(user_email=None):
	from netmanthan.utils.backups import backup
