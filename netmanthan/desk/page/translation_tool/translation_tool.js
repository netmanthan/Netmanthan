netmanthan.pages["translation-tool"].on_page_load = function (wrapper) {
	...
	netmanthan.translation_tool = new TranslationTool(page);
};
...
class TranslationTool {
	constructor(page) {
		...
		this.wrapper.append(netmanthan.render_template("translation_tool"));
	...
	setup_language_filter() {
		let languages = Object.keys(netmanthan.boot.lang_dict).map((language_label) => {
			let value = netmanthan.boot.lang_dict[language_label];
			return {
		...
		if (translation_language || netmanthan.boot.lang !== "en") {
			language_selector.set_value(translation_language || netmanthan.boot.lang);
		} else {
			netmanthan.prompt(
				{
	...
	fetch_messages() {
		netmanthan.dom.freeze(__("Fetching..."));
		return netmanthan
			.xcall("netmanthan.translate.get_messages", {
			...
			<div
				...
				<div class="bold ellipsis">
					<span class="indicator ${this.get_indicator_color(message)}">
						<span>${netmanthan.utils.escape_html(message.source_text)}</span>
					</span>
				</div>
			</div>
		`;
	...
	get_additional_info(source_id) {
		netmanthan.dom.freeze("Fetching...");
		return netmanthan
			.xcall("netmanthan.translate.get_source_additional_info", {
				source: source_id,
				language: this.page.fields_dict["language"].get_value(),
			})
			.finally(netmanthan.dom.unfreeze);
	}

	make_edit_form(translation, { contributions, positions }) {
		if (!this.form) {
			this.form = new netmanthan.ui.FieldGroup({
				fields: [
					{
						fieldtype: "HTML",
						fieldname: "header",
	...
	show_confirmation_dialog() {
		this.confirmation_dialog = new netmanthan.ui.Dialog({
			fields: [
				{
					label: __("Language"),
					fieldname: "language",
	...
	create_translations() {
		netmanthan.dom.freeze(__("Submitting..."));
		return netmanthan
			.xcall("netmanthan.core.doctype.translation.translation.create_translations", {
	...
	setup_local_contributions() {
		// TODO: Refactor
		netmanthan
			.xcall("netmanthan.translate.get_contributions", {
				language: this.language,
			})
			.then((messages) => {
				let template = (message) => `
	...
	show_translation_status_modal(e, $el) {
		...
		netmanthan.xcall("netmanthan.translate.get_contribution_status", { message_id }).then((doc) => {
	...
	get_code_url(path, line_no, app) {
		...
		return `https://github.com/netmanthan/${app}/blob/develop/${code_path}#L${line_no}`;
	}
}
