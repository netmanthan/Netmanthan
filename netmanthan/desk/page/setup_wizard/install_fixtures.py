...
def update_genders():
	...
	for record in records:
		netmanthan.get_doc(record).insert(ignore_permissions=True, ignore_if_duplicate=True)
...
def update_salutations():
	...
	for record in records:
		doc = netmanthan.new_doc(record.get("doctype"))
		doc.update(record)
...
def add_unsubscribe():
	...
	for unsubscribe in email_unsubscribe:
		if not netmanthan.get_all("Email Unsubscribe", filters=unsubscribe):
			doc = netmanthan.new_doc("Email Unsubscribe")
