...
import netmanthan
from netmanthan.geo.country_info import get_country_info
from netmanthan.translate import get_messages_for_boot, send_translations, set_default_language
from netmanthan.utils import cint, strip
...
def get_setup_stages(args):
	...
	# App setup stage functions should not include netmanthan.db.commit
	# That is done by netmanthan after successful completion of all stages
	stages = [
		{
...
def setup_complete(args):
	...
	is_background_task = netmanthan.conf.get("trigger_site_setup_in_background")
...
def process_setup_stages(stages, user_input, is_background_task=False):
	...
	try:
		netmanthan.flags.in_setup_wizard = True
		current_task = None
		...
		if not is_background_task:
			return {"status": "ok"}
		netmanthan.publish_realtime("setup_task", {"status": "ok"}, user=netmanthan.session.user)
	finally:
		netmanthan.flags.in_setup_wizard = False
...
def run_setup_success(args):
	for hook in netmanthan.get_hooks("setup_wizard_success"):
		netmanthan.get_attr(hook)(args)
	install_fixtures.install()


def get_stages_hooks(args):
	stages = []
	for method in netmanthan.get_hooks("setup_wizard_stages"):
		stages += netmanthan.get_attr(method)(args)
	return stages


def get_setup_complete_hooks(args):
	stages = []
	for method in netmanthan.get_hooks("setup_wizard_complete"):
		stages.append(
			{
				"status": "Executing method",
				"fail_msg": "Failed to execute method",
				"tasks": [
					{"fn": netmanthan.get_attr(method), "args": args, "fail_msg": "Failed to execute method"}
				],
			}
		)
	return stages
...
def handle_setup_exception(args):
	...
	if args:
		traceback = netmanthan.get_traceback()
		print(traceback)
		for hook in netmanthan.get_hooks("setup_wizard_exception"):
			netmanthan.get_attr(hook)(traceback, args)
...
def update_system_settings(args):
	...
	system_settings = netmanthan.get_doc("System Settings", "System Settings")
	system_settings.update(
		{
			...
			"float_precision": 3,
			"date_format": netmanthan.db.get_value("Country", args.get("country"), "date_format"),
			"time_format": netmanthan.db.get_value("Country", args.get("country"), "time_format"),
			"number_format": number_format,
			"enable_scheduler": 1 if not netmanthan.flags.in_test else 0,
...
def update_user_name(args):
	...
	if args.get("email"):
		if netmanthan.db.exists("User", args.get("email")):
			# running again
			return

		args["name"] = args.get("email")

        _mute_emails, netmanthan.flags.mute_emails = netmanthan.flags.mute_emails, True
		...
		doc.insert()
		netmanthan.flags.mute_emails = _mute_emails
		update_password(args.get("email"), args.get("password"))

	elif first_name:
		args.update({"name": netmanthan.session.user, "first_name": first_name, "last_name": last_name})
	...
	if args.get("attach_user"):
		...
		if len(attach_user) == 3:
			...
			netmanthan.db.set_value("User", args.get("name"), "user_image", fileurl)
...
def add_all_roles_to(name):
	user = netmanthan.get_doc("User", name)
	for role in netmanthan.db.sql("""select name from tabRole"""):
		if role[0] not in [
			"Administrator",
			"Guest",
			"All",
...
def disable_future_access():
	netmanthan.db.set_default("desktop:home_page", "workspace")
	netmanthan.db.set_single_value("System Settings", "setup_complete", 1)

	# Enable onboarding after install
	netmanthan.db.set_single_value("System Settings", "enable_onboarding", 1)

	if not netmanthan.flags.in_test:
		# remove all roles and add 'Administrator' to prevent future access
		page = netmanthan.get_doc("Page", "setup-wizard")
		page.roles = []
...
@netmanthan.whitelist()
def load_languages():
	language_codes = netmanthan.db.sql(
	...
	return {
		"default_language": netmanthan.db.get_value("Language", netmanthan.local.lang, "language_name")
                            or netmanthan.local.lang,
		"languages": sorted(netmanthan.db.sql_list("select language_name from tabLanguage order by name")),
...
@netmanthan.whitelist()
def load_country():
	from netmanthan.sessions import get_geo_ip_country

	return get_geo_ip_country(netmanthan.local.request_ip) if netmanthan.local.request_ip else None


@netmanthan.whitelist()
def load_user_details():
	return {
		"full_name": netmanthan.cache().hget("full_name", "signup"),
		"email": netmanthan.cache().hget("email", "signup"),
	}
...
def email_setup_wizard_exception(traceback, args):
	if not netmanthan.conf.setup_wizard_exception_email:
		return
...
- **User:** {user}""".format(
		...
		args="\n".join(pretty_args),
		user=netmanthan.session.user,
		headers=netmanthan.request.headers if netmanthan.request else "[no request]",
	)

	netmanthan.sendmail(
		recipients=netmanthan.conf.setup_wizard_exception_email,
		sender=netmanthan.session.user,
		subject=f"Setup failed: {netmanthan.local.site}",
		message=message,
		delayed=False,
	)
...
def get_language_code(lang):
	return netmanthan.db.get_value("Language", {"language_name": lang})


def enable_twofactor_all_roles():
	all_role = netmanthan.get_doc("Role", {"role_name": "All"})
	all_role.two_factor_auth = True
	all_role.save(ignore_permissions=True)


def make_records(records, debug=False):
	from netmanthan import _dict
...
def show_document_insert_error():
	...
	netmanthan.log_error("Exception during Setup")
