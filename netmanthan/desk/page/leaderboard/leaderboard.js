netmanthan.pages["leaderboard"].on_page_load = (wrapper) => {
	netmanthan.leaderboard = new Leaderboard(wrapper);

	$(wrapper).bind("show", () => {
		// Get which leaderboard to show
		let doctype = netmanthan.get_route()[1];
		netmanthan.leaderboard.show_leaderboard(doctype);
	});
};
...
class Leaderboard {
	...
	get_leaderboard_config() {
		...
		netmanthan
			.xcall("netmanthan.desk.page.leaderboard.leaderboard.get_leaderboard_config")
			.then((config) => {
				...
				const _initial_doctype = netmanthan.get_route()[1] || this.doctypes[0];
	...
	setup_leaderboard_fields() {
		this.company_select = this.page.add_field({
			...
			fieldtype: "Link",
			options: "Company",
			default: netmanthan.defaults.get_default("company"),
			reqd: 1,
			change: (e) => {
				this.make_request();
			},
		...
		this.type_select = this.page.add_select(
			...
			this.options.selected_filter.map((d) => {
				return { label: __(netmanthan.model.unscrub(d)), value: d };
			})
		);
	...
	create_date_range_field() {
		let timespan_field = $(this.parent).find(
			`.netmanthan-control[data-original-title="${__("Timespan")}"]`
		);
		this.date_range_field = $(`<div class="from-date-field"></div>`)
			.insertAfter(timespan_field)
			.hide();

		let date_field = netmanthan.ui.form.make_control({
			df: {
				fieldtype: "DateRange",
				fieldname: "selected_date_range",
				placeholder: __("Date Range"),
				default: [netmanthan.datetime.month_start(), netmanthan.datetime.now_date()],
	...
	render_selected_doctype() {
		this.$sidebar_list.on("click", "li", (e) => {
			...
			this.company_select.set_value(
				netmanthan.defaults.get_default("company") || this.company_select.get_value()
			);
			...
			netmanthan.set_route("leaderboard", this.options.selected_doctype);
			this.make_request();
		});
	}
	...
	make_request() {
		netmanthan.model.with_doctype(this.options.selected_doctype, () => {
	...
	get_leaderboard(notify) {
		...
		if (!company && !this.leaderboard_config[this.options.selected_doctype].company_disabled) {
			notify(this, null);
			netmanthan.show_alert(__("Please select Company"));
			return;
		}
		netmanthan
			...
			.then((r) => {
				...
				netmanthan.utils.make_chart(".leaderboard-graph", custom_options);
	...
	get_leaderboard_data(me, res) {
		if (res && res.message.length) {
			...
			netmanthan.utils.setup_search($(me.parent), ".list-item-container", ".list-id");
		} else {
			me.$graph_area.hide();
	...
	render_list_header() {
		const _selected_filter = this.options.selected_filter.map((i) => netmanthan.model.unscrub(i));
		const fields = ["rank", "name", this.options.selected_filter_item];
		const filters = fields
			.map((filter) => {
				const col = __(netmanthan.model.unscrub(filter));
	...
	render_message() {
		...
		let html = `<div class="leaderboard-empty-state ${display_class}">
			<div class="no-result text-center">
				<img src="/assets/netmanthan/images/ui-states/search-empty-state.svg"
					alt="Empty State"
					class="null-state"
				>
	...
	get_item_html(item, index) {
		...
		const link = `/app/${netmanthan.router.slug(this.options.selected_doctype)}/${item.name}`;
	...
	get_sidebar_item(item, icon) {
		let icon_html = icon ? netmanthan.utils.icon(icon, "md") : "";
	...
	get_date_range() {
		...
		let current_date = netmanthan.datetime.now_date();
		let date_range_map = {
			"this week": [netmanthan.datetime.week_start(), netmanthan.datetime.week_end()],
			"this month": [netmanthan.datetime.month_start(), netmanthan.datetime.month_end()],
			"this quarter": [netmanthan.datetime.quarter_start(), netmanthan.datetime.quarter_end()],
			"this year": [netmanthan.datetime.year_start(), netmanthan.datetime.year_end()],
			"last week": [netmanthan.datetime.add_days(current_date, -7), current_date],
			"last month": [netmanthan.datetime.add_months(current_date, -1), current_date],
			"last quarter": [netmanthan.datetime.add_months(current_date, -3), current_date],
			"last year": [netmanthan.datetime.add_months(current_date, -12), current_date],
			"all time": null,
			"select date range": this.selected_date_range || [
				netmanthan.datetime.month_start(),
				current_date,
			],
		};
