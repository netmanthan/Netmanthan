"""
netmanthanClient is a library that helps you connect with other netmanthan systems
"""
import base64
import json
...
class netmanthanException(Exception):
	pass


class netmanthanClient:
	def __init__(
		self,
		url,
		username=None,
		...
		api_key=None,
		api_secret=None,
		netmanthan_authorization_source=None,
	):
		import requests
		...
		self.netmanthan_authorization_source = netmanthan_authorization_source
	...
	def setup_key_authentication_headers(self):
		if self.api_key and self.api_secret:
			...
			if self.netmanthan_authorization_source:
				auth_source = {"netmanthan-Authorization-Source": self.netmanthan_authorization_source}
	...
	def insert(self, doc):
		...
		res = self.session.post(
			...
			data={"data": netmanthan.as_json(doc)},
			verify=self.verify,
			headers=self.headers,
		)
		return netmanthan._dict(self.post_process(res))
	...
	def insert_many(self, docs):
		...
		return self.post_request({"cmd": "netmanthan.client.insert_many", "docs": netmanthan.as_json(docs)})
	...
	def update(self, doc):
		...
		res = self.session.put(
			url, data={"data": netmanthan.as_json(doc)}, verify=self.verify, headers=self.headers
		)
		return netmanthan._dict(self.post_process(res))
	...
	def bulk_update(self, docs):
		...
		return self.post_request({"cmd": "netmanthan.client.bulk_update", "docs": netmanthan.as_json(docs)})
	...
	def delete(self, doctype, name):
		...
		return self.post_request({"cmd": "netmanthan.client.delete", "doctype": doctype, "name": name})

	def submit(self, doc):
		"""Submit remote document

		:param doc: dict or Document object to be submitted remotely"""
		return self.post_request({"cmd": "netmanthan.client.submit", "doc": netmanthan.as_json(doc)})
	...
	def get_value(self, doctype, fieldname=None, filters=None):
		...
		return self.get_request(
			{
				"cmd": "netmanthan.client.get_value",
				"doctype": doctype,
				"fieldname": fieldname or "name",
				"filters": netmanthan.as_json(filters),
			}
		)
	...
	def set_value(self, doctype, docname, fieldname, value):
		...
		return self.post_request(
			{
				"cmd": "netmanthan.client.set_value",
				"doctype": doctype,
				"name": docname,
				"fieldname": fieldname,
				"value": value,
	...
	def cancel(self, doctype, name):
		...
		return self.post_request({"cmd": "netmanthan.client.cancel", "doctype": doctype, "name": name})
	...
	def rename_doc(self, doctype, old_name, new_name):
		...
		params = {
			"cmd": "netmanthan.client.rename_doc",
			"doctype": doctype,
			"old_name": old_name,
			"new_name": new_name,
		}
	...
	def migrate_doctype(
		...
		for doc in docs:
			...
			if doctype != "User" and not netmanthan.db.exists("User", doc.get("owner")):
				netmanthan.get_doc(
					{
						"doctype": "User",
						"email": doc.get("owner"),
			...
			doc["doctype"] = doctype
			new_doc = netmanthan.get_doc(doc)
			new_doc.insert()
	...
	def migrate_single(self, doctype):
		...
		doc.modified = netmanthan.db.get_single_value(doctype, "modified")
	...
	def post_process(self, response):
		...
		if rjson and ("exc" in rjson) and rjson["exc"]:
			try:
				...
				exc = "netmanthanClient Request Failed\n\n" + exc
			except Exception:
				exc = rjson["exc"]

			raise netmanthanException(exc)
		if "message" in rjson:
...
class netmanthanOAuth2Client(netmanthanClient):
