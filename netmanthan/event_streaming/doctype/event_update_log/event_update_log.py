...
import netmanthan
from netmanthan.model import no_value_fields, table_fields
...
class EventUpdateLog(Document):
	def after_insert(self):
		...
		enqueued_method = (
			"netmanthan.event_streaming.doctype.event_consumer.event_consumer.notify_event_consumers"
		)
		jobs = get_jobs()
...
def notify_consumers(doc, event):
	...
	if netmanthan.flags.in_install or netmanthan.flags.in_migrate:
		return
...
def check_doctype_has_consumers(doctype: str) -> bool:
	...
	return bool(netmanthan.cache().hget(ENABLED_DOCTYPES_CACHE_KEY, doctype, fetch_from_db))
...
def get_update(old, new, for_child=False):
	...
	out = netmanthan._dict(changed={}, added={}, removed={}, row_changed={})
...
def make_event_update_log(doc, update_type):
	...
	if update_type != "Delete":
		...
		data = netmanthan.as_json(doc) if not doc.get("diff") else netmanthan.as_json(doc.diff)
	else:
		data = None
...
def is_consumer_uptodate(update_log, consumer):
	...
	prev_log_consumers = netmanthan.get_all(
...
def mark_consumer_read(update_log_name, consumer_name):
	...
	"""
	update_log = netmanthan.get_doc("Event Update Log", update_log_name)
...
def get_update_logs_for_consumer(event_consumer, doctypes, last_update):
	...
	if isinstance(doctypes, str):
		doctypes = netmanthan.parse_json(doctypes)

	from netmanthan.event_streaming.doctype.event_consumer.event_consumer import has_consumer_access

	consumer = netmanthan.get_doc("Event Consumer", event_consumer)
	docs = netmanthan.get_list(
		doctype="Event Update Log",
