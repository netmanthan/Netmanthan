...
from netmanthan.event_streaming.doctype.event_producer.event_producer import pull_from_node
...
class TestEventProducer(netmanthanTestCase):
	def setUp(self):
	...
	def test_insert(self):
		...
		self.assertTrue(netmanthan.db.exists("ToDo", producer_doc.name))
	...
	def test_update(self):
		...
		self.pull_producer_data()
		local_doc = netmanthan.get_doc(producer_doc.doctype, producer_doc.name)
	...
	def test_delete(self):
		...
		self.pull_producer_data()
		self.assertFalse(netmanthan.db.exists("ToDo", producer_doc.name))

	def test_child_table_sync_with_dependencies(self):
		producer = get_remote_site()
		producer_user = netmanthan._dict(
			doctype="User",
		...
		netmanthan.db.delete("User", {"email": producer_user.email})
		producer_user = producer.insert(producer_user)

		producer_note = netmanthan._dict(
		...
		netmanthan.db.delete("Note", {"title": producer_note.title})
		producer_note = producer.insert(producer_note)

		self.pull_producer_data()
		self.assertTrue(netmanthan.db.exists("User", producer_user.name))
		if self.assertTrue(netmanthan.db.exists("Note", producer_note.name)):
			local_note = netmanthan.get_doc("Note", producer_note.name)
	...
	def test_dynamic_link_dependencies_synced(self):
		...
		event_producer = netmanthan.get_doc("Event Producer", producer_url, for_update=True)
		event_producer.producer_doctypes = []
		event_producer.append("producer_doctypes", {"ref_doctype": "ToDo", "use_same_name": 1})
		event_producer.save()

		producer_link_doc = netmanthan._dict(doctype="Note", title="Test Dynamic Link 1")

		delete_on_remote_if_exists(producer, "Note", {"title": producer_link_doc.title})
		netmanthan.db.delete("Note", {"title": producer_link_doc.title})
		producer_link_doc = producer.insert(producer_link_doc)
		producer_doc = netmanthan._dict(
			doctype="ToDo",
		...
		self.assertTrue(netmanthan.db.exists("Note", producer_link_doc.name))
		self.assertEqual(
			producer_link_doc.name, netmanthan.db.get_value("ToDo", producer_doc.name, "reference_name")
		)
	...
	def test_conditional_events(self):
		...
		# Add Condition
		event_producer = netmanthan.get_doc("Event Producer", producer_url)
		...
		# Make test doc
		producer_note1 = netmanthan._dict(doctype="Note", public=0, title="test conditional sync")
		...
		self.assertFalse(netmanthan.db.exists("Note", producer_note1.name))
		...
		# it should sync now
		self.assertTrue(netmanthan.db.exists("Note", producer_note1.name))
		local_note = netmanthan.get_doc("Note", producer_note1.name)
	...
	def test_conditional_events_with_cmd(self):
		...
		note_producer_entry.condition = (
			"cmd: netmanthan.event_streaming.doctype.event_producer.test_event_producer.can_sync_note"
		)
		event_producer.save()

		# Make test doc
		producer_note1 = netmanthan._dict(doctype="Note", public=0, title="test conditional sync cmd")
		delete_on_remote_if_exists(producer, "Note", {"title": producer_note1["title"]})
		producer_note1 = producer.insert(producer_note1)
	...
	def test_event_sync_log(self):
		...
		self.pull_producer_data()
		self.assertTrue(netmanthan.db.exists("Event Sync Log", {"docname": producer_doc.name}))
	...
	def test_mapping(self):
		...
		producer_note = netmanthan._dict(doctype="Note", title="Test Mapping", content="Test Mapping")
		delete_on_remote_if_exists(producer, "Note", {"title": producer_note.title})
		producer_note = producer.insert(producer_note)
		self.pull_producer_data()
		# check inserted
		self.assertTrue(netmanthan.db.exists("ToDo", {"description": producer_note.content}))
		...
		self.pull_producer_data()
		# check delete
		self.assertFalse(netmanthan.db.exists("ToDo", {"description": producer_note.content}))
	...
	def test_inner_mapping(self):
		...
		self.assertTrue(netmanthan.db.exists("Role", {"role_name": producer_note.title}))
		# check doc inserted
		self.assertTrue(netmanthan.db.exists("ToDo", {"description": producer_note.content}))
...
def get_mapping(mapping_name, local, remote, field_map):
	name = netmanthan.db.exists("Document Type Mapping", mapping_name)
	if name:
		doc = netmanthan.get_doc("Document Type Mapping", name)
	else:
		doc = netmanthan.new_doc("Document Type Mapping")
...
def create_event_producer(producer_url):
	if netmanthan.db.exists("Event Producer", producer_url):
		event_producer = netmanthan.get_doc("Event Producer", producer_url)
	...
	response = producer_site.post_api(
		"netmanthan.core.doctype.user.user.generate_keys", params={"user": "Administrator"}
	)
	...
	event_producer = netmanthan.new_doc("Event Producer")
	event_producer.producer_doctypes = []
...
def connect():
	def _connect():
		return netmanthanClient(url=producer_url, username="Administrator", password="admin", verify=False)
