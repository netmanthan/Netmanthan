...
from netmanthan.utils.data import get_link_to_form, get_url
...
class EventProducer(Document):
	...
	def validate(self):
		self.validate_event_subscriber()
		if netmanthan.flags.in_test:
			for entry in self.producer_doctypes:
				entry.status = "Approved"

	def validate_event_subscriber(self):
		if not netmanthan.db.get_value("User", self.user, "api_key"):
			netmanthan.throw(
				_("Please generate keys for the Event Subscriber User {0} first.").format(
					netmanthan.bold(get_link_to_form("User", self.user))
				)
			)

	def on_update(self):
		if not self.incoming_change:
			if netmanthan.db.exists("Event Producer", self.name):
				if not self.api_key or not self.api_secret:
					netmanthan.throw(_("Please set API Key and Secret on the producer and consumer sites first."))
				else:
	...
	def on_trash(self):
		last_update = netmanthan.db.get_value("Event Producer Last Update", dict(event_producer=self.name))
		if last_update:
			netmanthan.delete_doc("Event Producer Last Update", last_update)

	def check_url(self):
		valid_url_schemes = ("http", "https")
		netmanthan.utils.validate_url(self.producer_url, throw=True, valid_schemes=valid_url_schemes)
	...
	def set_last_update(self, last_update):
		last_update_doc_name = netmanthan.db.get_value(
	...
	def get_request_data(self):
		...
		for entry in self.producer_doctypes:
			if entry.has_mapping:
				...
				dt = netmanthan.db.get_value("Document Type Mapping", entry.mapping, "remote_doctype")
			else:
				dt = entry.ref_doctype
			consumer_doctypes.append({"doctype": dt, "condition": entry.condition})

		user_key = netmanthan.db.get_value("User", self.user, "api_key")
	...
	def update_event_consumer(self):
		if self.is_producer_online():
			...
			event_consumer = netmanthan._dict(event_consumer)
			if event_consumer:
				...
				for entry in self.producer_doctypes:
					if entry.has_mapping:
						# if mapping, subscribe to remote doctype on consumer's site
						ref_doctype = netmanthan.db.get_value("Document Type Mapping", entry.mapping, "remote_doctype")
					else:
	...
	def is_producer_online(self):
		...
		while retry > 0:
			...
			retry -= 1
			time.sleep(5)
		netmanthan.throw(_("Failed to connect to the Event Producer site. Retry after some time."))


def get_producer_site(producer_url):
	"""create a netmanthanClient object for event producer site"""
	producer_doc = netmanthan.get_doc("Event Producer", producer_url)
	producer_site = netmanthanClient(
		url=producer_url,
...
def pull_producer_data():
	...
	if response.status_code == 200:
		for event_producer in netmanthan.get_all("Event Producer"):
...
def pull_from_node(event_producer):
	...
	event_producer = netmanthan.get_doc("Event Producer", event_producer)
...
def get_config(event_config):
	...
	for entry in event_config:
		if entry.status == "Approved":
			if entry.has_mapping:
				(mapped_doctype, mapping) = netmanthan.db.get_value(
...
def sync(update, producer_site, event_producer, in_retry=False):
	...
	try:
		...
		if in_retry:
			...
			return "Failed"
		log_event_sync(update, event_producer.name, "Failed", netmanthan.get_traceback())
...
def set_insert(update, producer_site, event_producer):
	...
	if netmanthan.db.get_value(update.ref_doctype, update.docname):
		# doc already created
		return
	doc = netmanthan.get_doc(update.data)
...
def set_update(update, producer_site):
	...
	if local_doc:
		data = netmanthan._dict(update.data)
...
def update_row_added(local_doc, added):
	...
	for tablename, rows in added.items():
		...
		for child in rows:
			child_doc = netmanthan.get_doc(child)
...
def get_updates(producer_site, last_update, doctypes):
	...
	docs = producer_site.post_request(
		{
			"cmd": "netmanthan.event_streaming.doctype.event_update_log.event_update_log.get_update_logs_for_consumer",
			"event_consumer": get_url(),
			"doctypes": netmanthan.as_json(doctypes),
			"last_update": last_update,
		}
	)
	return [netmanthan._dict(d) for d in (docs or [])]
...
def get_local_doc(update):
	...
	try:
		if not update.use_same_name:
			return netmanthan.get_doc(update.ref_doctype, {"remote_docname": update.docname})
		return netmanthan.get_doc(update.ref_doctype, update.docname)
...
def sync_dependencies(document, producer_site):
	...
	def sync_child_table_dependencies(doc, table_fields, producer_site):
		for df in table_fields:
			...
			for entry in child_table:
				...
				if child_doc:
					child_doc = netmanthan._dict(child_doc)
					set_dependencies(child_doc, netmanthan.get_meta(entry.doctype).get_link_fields(), producer_site)
	...
	def sync_dynamic_link_dependencies(doc, dl_fields, producer_site):
		for df in dl_fields:
			...
			if docname and not check_dependency_fulfilled(linked_doctype, docname):
				...
				netmanthan.get_doc(master_doc).insert(set_name=docname)
	...
	def set_dependencies(doc, link_fields, producer_site):
		for df in link_fields:
			...
			if docname and not check_dependency_fulfilled(linked_doctype, docname):
				master_doc = producer_site.get_doc(linked_doctype, docname)
				try:
					master_doc = netmanthan.get_doc(master_doc)
	...
	def check_dependency_fulfilled(linked_doctype, docname):
		return netmanthan.db.exists(linked_doctype, docname)
...
def sync_mapped_dependencies(dependencies, producer_site):
	...
	for entry in dependencies:
		doc = netmanthan._dict(json.loads(entry[1]))
		docname = netmanthan.db.exists(doc.doctype, doc.name)
		if not docname:
			doc = netmanthan.get_doc(doc).insert(set_child_names=False)
...
def log_event_sync(update, event_producer, sync_status, error=None):
	...
	doc = netmanthan.new_doc("Event Sync Log")
	...
	doc.data = netmanthan.as_json(update.data)
	doc.use_same_name = update.use_same_name
	doc.mapping = update.mapping if update.mapping else None
	if update.use_same_name:
		doc.docname = update.docname
	else:
		doc.docname = netmanthan.db.get_value(update.ref_doctype, {"remote_docname": update.docname}, "name")
	if error:
		doc.error = error
	doc.insert()
...
def get_mapped_update(update, producer_site):
	...
	mapping = netmanthan.get_doc("Document Type Mapping", update.mapping)
	if update.update_type == "Create":
		doc = netmanthan._dict(json.loads(update.data))
...
def new_event_notification(producer_url):
	...
	enqueued_method = "netmanthan.event_streaming.doctype.event_producer.event_producer.pull_from_node"
	jobs = get_jobs()
	if not jobs or enqueued_method not in jobs[netmanthan.local.site]:
		netmanthan.enqueue(enqueued_method, queue="default", **{"event_producer": producer_url})


@netmanthan.whitelist()
def resync(update):
	"""Retry syncing update if failed"""
	update = netmanthan._dict(json.loads(update))
	producer_site = get_producer_site(update.event_producer)
	event_producer = netmanthan.get_doc("Event Producer", update.event_producer)
	if update.mapping:
		update = get_mapped_update(update, producer_site)
		update.data = json.loads(update.data)
