...
export default {
	...
	mounted() {
		if (!this.letterhead && netmanthan.boot.sysdefaults.letter_head) {
			this.set_letterhead(netmanthan.boot.sysdefaults.letter_head);
		}
	...
	methods: {
		toggle_edit_letterhead() {
			...
			if (!this.control) {
				this.control = netmanthan.ui.form.make_control({
