...
export default {
	...
	mounted() {
		this.doc_select = netmanthan.ui.form.make_control({
		...
		this.preview_type = netmanthan.ui.form.make_control({
	...
	methods: {
		...
		get_default_docname() {
			return netmanthan.db.get_list(this.doctype, { limit: 1 }).then(doc => {
	...
	computed: {
		...
		url() {
			...
			let url =
				this.type == "PDF"
					? `/api/method/netmanthan.utils.weasyprint.download_pdf`
					: "/printpreview";
