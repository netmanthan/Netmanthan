	...
	methods: {
		edit_html() {
			let d = new netmanthan.ui.Dialog({
				...
				primary_action: ({ html }) => {
					html = netmanthan.dom.remove_script_and_style(html);
		...
		get_all_columns() {
			let meta = netmanthan.get_meta(this.df.options);
			let more_columns = [
				{
					label: __("Sr No."),
					value: "idx"
			...
			return more_columns.concat(
				meta.fields
					.map(tf => {
						if (netmanthan.model.no_value_type.includes(tf.fieldtype)) {
							return;
						}
						return {
							label: tf.label,
		...
		get_column_to_add(fieldname) {
			...
			return {
				...netmanthan.meta.get_docfield(this.df.options, fieldname),
				width: 10
			};
		},
