...
export default {
	...
	mounted() {
		let method =
			"netmanthan.printing.page.print_format_builder_beta.print_format_builder_beta.get_google_fonts";
	...
	methods: {
		...
		clone_field(df) {
			...
			if (cloned.custom) {
				...
				cloned.fieldname += "_" + netmanthan.utils.get_random(8);
			}
			return cloned;
		}
	},
		...
		print_templates() {
			...
			for (let template of templates) {
				...
				if (template.field) {
					...
					df = {
						label: template.name,
						fieldname: netmanthan.scrub(template.name)
					};
				}
				out.push({
...
.control-font :deep(.netmanthan-control[data-fieldname="font"] label) {
	display: none;
}
</style>
