netmanthan.ui.OnboardingTour = class OnboardingTour {
	constructor() {
		this.driver_steps = [];
	...
	init_driver() {
		this.driver = new netmanthan.Driver({
			className: "netmanthan-driver",
			allowClose: false,
			padding: 10,
			...
			opacity: 0.5,
			onHighlighted: (step) => {
				netmanthan.ui.next_form_tour = step.options.step_info?.next_form_tour;
				const wait_for_node = setInterval(() => {
					...
					if (step.popover.closeBtnNode) {
						step.popover.closeBtnNode.onclick = () => {
							...
							!netmanthan.boot.user.onboarding_status[this.tour.name] &&
								(netmanthan.boot.user.onboarding_status[this.tour.name] = {});
							netmanthan.boot.user.onboarding_status[this.tour.name].is_complete = true;
							if (!this.driver.hasNextStep()) {
								netmanthan.boot.user.onboarding_status[
									this.tour.name
								].all_steps_completed = true;
							}

							netmanthan.call({
								method: "netmanthan.desk.doctype.form_tour.form_tour.update_user_status",
								args: {
									value: JSON.stringify(netmanthan.boot.user.onboarding_status),
				...
				if ($input) netmanthan.utils.sleep(200).then(() => $input.focus());
			},
		});
	}

	async init({ tour_name, start_step }) {
		this.tour = await netmanthan.db.get_doc("Form Tour", tour_name);
		this.init_driver();
		this.build_steps();
	...
	get_step(step_info, on_next) {
		...
		return {
			...
			popover: {
				title,
				description,
				position: netmanthan.router.slug(position || "Bottom"),
			},
			onNext: on_next,
			step_info: step_info,
			showButtons,
...
netmanthan.ui.init_onboarding_tour = () => {
	...
	typeof netmanthan.boot.onboarding_tours == "undefined" && netmanthan.boot.onboarding_tours == [];
	typeof netmanthan.boot.user.onboarding_status == "undefined" &&
		netmanthan.boot.user.onboarding_status == {};
	let route = netmanthan.router.current_route;
	if (route[0] === "") return;
	...
	if (route[0] != "dashboard-view") {
		netmanthan.boot.onboarding_tours &&
			netmanthan.boot.onboarding_tours.forEach((tour) => {
	...
	matching_tours = matching_tours.filter((tour) => {
		if (netmanthan.boot.user.onboarding_status[tour[0]]?.is_complete == true) return false;
		return true;
	});
	matching_tours = matching_tours.map((tour) => {
		if (netmanthan.boot.user.onboarding_status[tour[0]]?.steps_complete != undefined) {
			tour.push(netmanthan.boot.user.onboarding_status[tour[0]].steps_complete);
		}
		return tour;
	});
	if (matching_tours.length == 0) return;
	let current_tour = matching_tours.find(
		(tour) => tour[0] == netmanthan.ui.currentTourInstance?.tour?.name
	);
	let next_tour = matching_tours.find((tour) => tour[0] == netmanthan.ui.next_form_tour);
	if (current_tour) {
		...
		netmanthan.ui.next_form_tour = undefined;
	} else {
	...
	if (!tour_name) return;
	if (netmanthan.ui.currentTourInstance?.driver) {
		netmanthan.ui.currentTourInstance.driver_steps = [];
		netmanthan.ui.currentTourInstance.driver.reset(true);
		netmanthan.ui.currentTourInstance.update_driver_steps();
	}
	const tour = (netmanthan.ui.currentTourInstance = new netmanthan.ui.OnboardingTour());
	...
	const wait_for_data = setInterval(() => {
		...
		if (document.body.getAttribute("data-ajax-state") === "complete") {
			netmanthan.utils.sleep(500).then(() => {
				tour.init({
					tour_name,
					start_step,
				});
