...
export default class GoogleDrivePicker {
	...
	loadGapi() {
		...
		if (!netmanthan.boot.user.google_drive_token) {
	...
	handleAuthResult(authResult) {
		...
		if (authResult && !authResult.error) {
			netmanthan.boot.user.google_drive_token = authResult.access_token;
			this.createPicker();
		} else {
			let error = error_map[authResult.error] || __("Google Authentication Error");
			netmanthan.throw(error);
		}
	}
	...
	createPicker() {
		...
		if (this.pickerApiLoaded && netmanthan.boot.user.google_drive_token) {
			...
			this.picker = new google.picker.PickerBuilder()
				...
				.setOAuthToken(netmanthan.boot.user.google_drive_token)
				.addView(this.view)
				.setLocale(netmanthan.boot.lang)
