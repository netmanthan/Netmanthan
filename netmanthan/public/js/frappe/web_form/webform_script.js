...
netmanthan.ready(function () {
	let web_form_doc = netmanthan.web_form_doc;
	let reference_doc = netmanthan.reference_doc;
	...
	function show_login_prompt() {
		if (netmanthan.session.user != "Guest" || !web_form_doc.login_required) return;
		const login_required = new netmanthan.ui.Dialog({
	...
	function show_form() {
		...
		$(".file-size").each(function () {
			$(this).text(netmanthan.form.formatters.FileSize($(this).text()));
		});
	}
