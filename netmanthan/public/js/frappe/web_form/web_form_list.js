...
netmanthan.provide("netmanthan.web_form_list");

export default class WebFormList {
	constructor(opts) {
		Object.assign(this, opts);
		netmanthan.web_form_list = this;
	...
	make_filters() {
		...
		netmanthan
			.call("netmanthan.website.doctype.web_form.web_form.get_web_form_filters", {
			...
			.then((response) => {
				...
				fields.forEach((field) => {
					...
					let input = netmanthan.ui.form.make_control({
						df: {
	...
	fetch_data() {
		let args = {
			method: "netmanthan.www.list.get_list_data",
			args: {
	...
	more() {
		...
		this.fetch_data().then((res) => {
			if (res.message.length === 0) {
				netmanthan.msgprint(__("No more items to display"));
			}
	...
	make_table_body() {
		if (this.data.length) {
			...
			this.wrapper.empty();
			netmanthan.has_permission(this.doctype, "", "create", () => {
	...
	append_rows(row_data) {
		...
		row_data.forEach((data_item) => {
			...
			let row = new netmanthan.ui.WebFormListRow({
				row: $row_element,
				doc: data_item,
	...
	make_actions() {
		...
		netmanthan.has_permission(this.doctype, "", "delete", () => {
	...
	delete_rows() {
		...
		netmanthan
			.call({
				type: "POST",
				method: "netmanthan.website.doctype.web_form.web_form.delete_multiple",
				args: {
					web_form_name: this.web_form_name,
...
netmanthan.ui.WebFormListRow = class WebFormListRow {
	...
	make_row() {
		...
		this.columns.forEach((field) => {
			let formatter = netmanthan.form.get_formatter(field.fieldtype);
			let value =
