...
netmanthan.provide("netmanthan.web_form");
import EventEmitterMixin from "../../netmanthan/event_emitter";

export default class WebForm extends netmanthan.ui.FieldGroup {
	constructor(opts) {
		super();
		Object.assign(this, opts);
		netmanthan.web_form = this;
		netmanthan.web_form.events = {};
		Object.assign(netmanthan.web_form.events, EventEmitterMixin);
	...
	make() {
		...
		netmanthan.init_client_script && netmanthan.init_client_script();
		this.setup_listeners();
		netmanthan.web_form.events.trigger("after_load");
	...
	set_default_values() {
		...
		let values = netmanthan.utils.get_query_params();
		delete values.new;
	...
	render_progress_dots() {
		...
		for (let i = 0; i <= this.page_breaks.length; i++) {
			let $dot = $(`<div class="slide-step">
				...
				<div class="slide-step-complete">${netmanthan.utils.icon("tick", "xs")}</div>
	...
	is_next_section_empty(section) {
		...
		let visible_controls = _section.find(".netmanthan-control:not(.hide-control)");
	...
	save() {
		...
		// Save
		window.saving = true;
		netmanthan.form_dirty = false;

		netmanthan.call({
			type: "POST",
			method: "netmanthan.website.doctype.web_form.web_form.accept",
			args: {
				data: this.doc,
				web_form: this.name,
				for_payment,
			...
			callback: (response) => {
				...
				if (!response.exc) {
					...
					netmanthan.web_form.events.trigger("after_save");
					...
					if (is_new && (response.message.attachment || response.message.file)) {
						netmanthan.call({
							type: "POST",
							method: "netmanthan.handler.upload_file",
							args: {
	...
	handle_success(data) {
		...
		if (this.success_url) {
			netmanthan.utils.setup_timer(5, 0, $(".time"));
			setTimeout(() => {
