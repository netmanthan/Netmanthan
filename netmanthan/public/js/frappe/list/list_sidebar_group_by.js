...
netmanthan.views.ListGroupBy = class ListGroupBy {
	constructor(opts) {
		$.extend(this, opts);
		this.make_wrapper();
	...
	render_group_by_items() {
		let get_item_html = (fieldname) => {
			...
			if (fieldname === "assigned_to") {
				...
				label = netmanthan.meta.get_label(this.doctype, fieldname);
				let docfield = netmanthan.meta.get_docfield(this.doctype, fieldname);
				if (!docfield) {
					return;
				}
			...
			return `<li class="group-by-field list-link">
					...
					href="#" onclick="return false;">
						...
						<span>${netmanthan.utils.icon("select", "xs")}</span>
					</a>
	...
	setup_search($dropdown) {
		netmanthan.utils.setup_search($dropdown, ".group-by-item", ".group-by-value", "data-name");
	}
	...
	get_group_by_count(field) {
		...
		return netmanthan.call("netmanthan.desk.listview.get_group_by_count", args).then((r) => {
			let field_counts = r.message || [];
			field_counts = field_counts.filter((f) => f.count !== 0);
			let current_user = field_counts.find((f) => f.name === netmanthan.session.user);
			field_counts = field_counts.filter(
				(f) => !["Guest", "Administrator", netmanthan.session.user].includes(f.name)
			);
			// Set netmanthan.session.user on top of the list
	...
	get_dropdown_html(field, fieldtype, applied = false) {
		...
		if (label === netmanthan.session.user) {
			label = __("Me");
		...
		let applied_html = applied
			? `<span class="applied"> ${netmanthan.utils.icon("tick", "xs")} </span>`
			: "";
