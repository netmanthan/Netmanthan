...
netmanthan.views.ListViewSelect = class ListViewSelect {
	constructor(opts) {
		$.extend(this, opts);
	...
	set_current_view() {
		...
		const view_name = netmanthan.utils.to_title_case(route[2] || "");
		if (route.length > 2 && netmanthan.views.view_modes.includes(view_name)) {
	...
	setup_views() {
		const views = {
			...
			Report: {
				...
				current_view_handler: () => {
					...
					if (netmanthan.get_route().length > 3) {
						default_action = {
			...
			Calendar: {
				condition: netmanthan.views.calendar[this.doctype],
			...
			Inbox: {
				condition: this.doctype === "Communication" && netmanthan.boot.email_accounts.length,
				action: () => this.set_route("inbox"),
				current_view_handler: () => {
					const accounts = this.get_email_accounts();
					let default_action;
					if (has_common(netmanthan.user_roles, ["System Manager", "Administrator"])) {
						default_action = {
							label: __("New Email Account"),
							action: () => netmanthan.new_doc("Email Account"),
						};
					}
			...
			Tree: {
				condition:
					netmanthan.treeview_settings[this.doctype] ||
					netmanthan.get_meta(this.doctype).is_tree,
		...
		netmanthan.views.view_modes.forEach((view) => {
	...
	setup_kanban_switcher(kanbans) {
		...
		if (can_create) {
			this.page.add_custom_menu_item(
				...
				() => netmanthan.views.KanbanView.show_kanban_dialog(this.doctype),
				true
			);
		}
	}
	...
	get_page_name() {
		return netmanthan.utils.to_title_case(netmanthan.get_route().slice(-1)[0] || "");
	}
	...
	get_reports() {
		...
		var reports =
			Object.values(netmanthan.boot.user.all_reports).sort((a, b) =>
	...
	setup_kanban_boards() {
			...
			netmanthan.db.get_value(
				...
				(board) => {
					if (!$.isEmptyObject(board)) {
						netmanthan.set_route("list", doctype, "kanban", board.name);
					} else {
						netmanthan.views.KanbanView.show_kanban_dialog(doctype);
					}
				}
			);
		}
		...
		const last_opened_kanban =
			netmanthan.model.user_settings[this.doctype]["Kanban"]?.last_kanban_board;
		if (!last_opened_kanban) {
			fetch_kanban_board(this.doctype);
		} else {
			netmanthan.db.exists("Kanban Board", last_opened_kanban).then((exists) => {
				if (exists) {
					netmanthan.set_route("list", this.doctype, "kanban", last_opened_kanban);
				} else {
					fetch_kanban_board(this.doctype);
				}
			});
	...
	get_calendars() {
		...
		return netmanthan.db
			...
			.then((result) => {
				...
				if (netmanthan.views.calendar[this.doctype]) {
	...
	get_email_accounts() {
		let accounts_to_add = [];
		let accounts = netmanthan.boot.email_accounts;
	...
	slug() {
		return netmanthan.router.slug(netmanthan.router.doctype_layout || this.doctype);
	}
};
