export default class BulkOperations {
	constructor({ doctype }) {
		if (!doctype) netmanthan.throw(__("Doctype required"));
	...
	print(docs) {
		...
		const is_submittable = netmanthan.model.is_submittable(this.doctype);
		...
		if (invalid_docs.length > 0) {
			netmanthan.msgprint(__("You selected Draft or Cancelled documents"));
			return;
		}

		if (valid_docs.length === 0) {
			netmanthan.msgprint(__("Select atleast 1 record for printing"));
			return;
		}
		...
		const dialog = new netmanthan.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					fieldname: "print_sel",
					options: netmanthan.meta.get_print_formats(this.doctype),
					default: netmanthan.get_meta(this.doctype).default_print_format,
				},
				{
					fieldtype: "Select",
					label: __("Page Size"),
					fieldname: "page_size",
					options: netmanthan.meta.get_print_sizes(),
		...
		dialog.set_primary_action(__("Print"), (args) => {
			if (!args) return;
			const default_print_format = netmanthan.get_meta(this.doctype).default_print_format;
			...
			if (args.page_size === "Custom") {
				if (args.page_height === 0 || args.page_width === 0) {
					netmanthan.throw(__("Page height and width cannot be zero"));
				}
			...
			const w = window.open(
				"/api/method/netmanthan.utils.print_format.download_multi_pdf?" +
					"doctype=" +
	...
	get_letterhead_options() {
		...
		netmanthan.call({
			method: "netmanthan.client.get_list",
			args: {
				doctype: "Letter Head",
	...
	delete(docnames, done = null) {
		netmanthan
			.call({
				method: "netmanthan.desk.reportview.delete_items",
				freeze: true,
				freeze_message:
					docnames.length <= 10
	...
	assign(docnames, done) {
		if (docnames.length > 0) {
			const assign_to = new netmanthan.ui.form.AssignToDialog({
				obj: this,
				method: "netmanthan.desk.form.assign_to.add_multiple",
				doctype: this.doctype,
				docname: docnames,
				bulk_assign: true,
				re_assign: true,
			...
			netmanthan.msgprint(__("Select records for assignment"));
		}
	}
	...
	apply_assignment_rule(docnames, done) {
		if (docnames.length > 0) {
			netmanthan
				.call("netmanthan.automation.doctype.assignment_rule.assignment_rule.bulk_apply", {
					doctype: this.doctype,
					docnames: docnames,
				})
				.then(() => done());
	...
	submit_or_cancel(docnames, action = "submit", done = null) {
		...
		netmanthan
			.call({
				method: "netmanthan.desk.doctype.bulk_update.bulk_update.submit_cancel_or_update_docs",
				args: {
					doctype: this.doctype,
					action: action,
					docnames: docnames,
			...
			.then((r) => {
				...
				if (failed.length < docnames.length) {
					netmanthan.utils.play_sound(action);
					if (done) done();
				}
			});
	}
			...
			primary_action: ({ value }) => {
				...
				netmanthan
					...
					.then((r) => {
						...
						dialog.hide();
						netmanthan.show_alert(__("Updated successfully"));
					});
			},
		...
		const dialog = new netmanthan.ui.Dialog({
			...
			fields: [
				{
					...
					get_data: function (txt) {
						return netmanthan.db.get_link_options("Tag", txt);
					},
				},
			],
			...
			primary_action: () => {
				...
				if (args && args.tags) {
					...
					netmanthan.call({
						method: "netmanthan.desk.doctype.tag.tag.add_tags",
						args: {
							tags: args.tags,
							dt: this.doctype,
							docs: docnames,
	...
	export(doctype, docnames) {
		netmanthan.require("data_import_tools.bundle.js", () => {
			const data_exporter = new netmanthan.data_import.DataExporter(
				doctype,
				"Insert New Records"
			);
