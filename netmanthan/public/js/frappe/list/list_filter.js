...
export default class ListFilter {
	constructor({ wrapper, doctype }) {
		...
		this.can_add_global = netmanthan.user.has_role(["System Manager", "Administrator"]);
		this.filters = [];
		this.make();
		this.bind();
		this.refresh();
	...
	make() {
		...
		this.filter_input = netmanthan.ui.form.make_control({
			df: {
				fieldtype: "Data",
		...
		this.is_global_input = netmanthan.ui.form.make_control({
			df: {
				fieldtype: "Check",
				label: __("Is Global"),
			},
	...
	filter_template(filter) {
		return `<div class="list-link filter-pill list-sidebar-button btn btn-default" data-name="${
			...
			<a class="remove">${netmanthan.utils.icon("close")}</a>
		</div>`;
	}
	...
	save_filter(filter_name) {
		return netmanthan.db.insert({
			doctype: "List Filter",
			reference_doctype: this.list_view.doctype,
			filter_name,
			for_user: this.is_global_input.get_value() ? "" : netmanthan.session.user,
	...
	remove_filter(name) {
		if (!name) return;
		return netmanthan.db.delete_doc("List Filter", name);
	}
