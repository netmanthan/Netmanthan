...
netmanthan.views.BaseList = class BaseList {
	constructor(opts) {
	...
	setup_defaults() {
		...
		this.page_title = this.page_title || netmanthan.router.doctype_layout || __(this.doctype);
		this.meta = netmanthan.get_meta(this.doctype);
		this.settings = netmanthan.listview_settings[this.doctype] || {};
		this.user_settings = netmanthan.get_user_settings(this.doctype);

		this.start = 0;
		this.page_length = 20;
		this.data = [];
		this.method = "netmanthan.desk.reportview.get";

		this.can_create = netmanthan.model.can_create(this.doctype);
		this.can_write = netmanthan.model.can_write(this.doctype);
	...
	get_list_view_settings() {
		return netmanthan
			.call("netmanthan.desk.listview.get_list_settings", {
	...
	async set_fields() {
		let fields = [].concat(netmanthan.model.std_fields_list, this.meta.title_field);
	...
	_add_field(fieldname, doctype) {
		...
		const is_valid_field =
			netmanthan.model.std_fields_list.includes(fieldname) ||
			netmanthan.meta.has_field(doctype, fieldname) ||
			fieldname === "_seen";
	...
	set_stats() {
		...
		this.workflow_state_fieldname = netmanthan.workflow.get_state_fieldname(this.doctype);
		if (this.workflow_state_fieldname) {
			if (!netmanthan.workflow.workflows[this.doctype]["override_status"]) {
	...
	fetch_meta() {
		return netmanthan.model.with_doctype(this.doctype);
	}
	...
	setup_page() {
		...
		!this.hide_card_layout && this.page.main.addClass("netmanthan-card");
	...
	setup_view_menu() {
		...
		if (netmanthan.boot.desk_settings.view_switcher && !this.meta.force_re_route_to_default_view) {
			/* @preserve
			...
			this.views_list = new netmanthan.views.ListViewSelect({
				doctype: this.doctype,
	...
	set_breadcrumbs() {
		netmanthan.breadcrumbs.add(this.meta.module, this.doctype);
	}

	setup_side_bar() {
		if (this.hide_sidebar || !netmanthan.boot.desk_settings.list_sidebar) return;
		this.list_sidebar = new netmanthan.views.ListSidebar({
			doctype: this.doctype,
			stats: this.stats,
	...
	setup_list_wrapper() {
		this.$netmanthan_list = $('<div class="netmanthan-list">').appendTo(this.page.main);
	}
	...
	setup_result_area() {
		...
		this.$netmanthan_list.append(this.$result);
	}
	...
	setup_no_result_area() {
		...
		`).hide();
		this.$netmanthan_list.append(this.$no_result);
	}

	setup_freeze_area() {
		this.$freeze = $('<div class="freeze"></div>').hide();
		this.$netmanthan_list.append(this.$freeze);
	}
	...
	setup_paging_area() {
		...
		this.$netmanthan_list.append(this.$paging_area);
	...
	get_fields() {
		...
		return this.fields.map((f) => netmanthan.model.get_full_column_name(f[0], f[1]));
	}

	get_group_by() {
		let name_field = this.fields && this.fields.find((f) => f[0] == "name");
		if (name_field) {
			return netmanthan.model.get_full_column_name(name_field[0], name_field[1]);
		}
		return null;
	}
	...
	refresh() {
		...
		return netmanthan.call(args).then((r) => {
			// render
			this.prepare_data(r);
	...
	prepare_data(r) {
		...
		Object.assign(netmanthan.boot.user_info, data.user_info);
		delete data.user_info;

		data = !Array.isArray(data) ? netmanthan.utils.dict(data.keys, data.values) : data;
...
class FilterArea {
	...
	clear(refresh = true) {
		...
		return netmanthan.run_serially(promises).then(() => {
			this.trigger_refresh = true;
		});
	}
			...
			doctype_fields
				.filter(
					(df) =>
						...
						(df.in_standard_filter && netmanthan.model.is_value_type(df.fieldtype))
				)
				.map((df) => {
	...
	make_filter_list() {
		$(`<div class="filter-selector">
			<div class="btn-group">
				...
				<button class="btn btn-default btn-sm filter-x-button" title="${__("Clear all filters")}">
					<span class="filter-icon">
						${netmanthan.utils.icon("filter-x")}
					</span>
				</button>
			</div>
		...
		this.filter_list = new netmanthan.ui.FilterGroup({
...
netmanthan.views.is_valid = (view_mode) => netmanthan.views.view_modes.includes(view_mode);
