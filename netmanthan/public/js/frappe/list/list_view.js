...
netmanthan.views.ListView = class ListView extends netmanthan.views.BaseList {
	static load_last_view() {
		const route = netmanthan.get_route();
		const doctype = route[1];

		if (route.length === 2) {
			const user_settings = netmanthan.get_user_settings(doctype);
			const last_view = user_settings.last_view;
			netmanthan.set_route(
				"list",
				netmanthan.router.doctype_layout || doctype,
				netmanthan.views.is_valid(last_view) ? last_view.toLowerCase() : "list"
			);
			return true;
		}
		return false;
	...
	constructor(opts) {
		super(opts);
		this.show();
		this.debounced_refresh = netmanthan.utils.debounce(
	...
	has_permissions() {
		const can_read = netmanthan.perm.has_perm(this.doctype, 0, "read");
		return can_read;
	}
	...
	check_permissions() {
		if (!this.has_permissions()) {
			netmanthan.set_route("");
			netmanthan.throw(__("Not permitted to view {0}", [this.doctype]));
		}
	}
	...
	validate_filters(filters) {
		...
		valid_fields = valid_fields.concat(netmanthan.model.std_fields_list);
	...
	show_restricted_list_indicator_if_applicable() {
		const match_rules_list = netmanthan.perm.get_match_rules(this.doctype);
		if (match_rules_list.length) {
			this.restricted_list = $(
				`<button class="btn btn-xs restricted-button flex align-center">
					${netmanthan.utils.icon("restriction", "xs")}
				</button>`
			)
	...
	show_restrictions(match_rules_list = []) {
		netmanthan.msgprint(
			netmanthan.render_template("list_view_permission_restrictions", {
	...
	async set_fields() {
		...
		await Promise.all(
			fields.map((f) => {
				return new Promise((resolve) => {
					const df =
						typeof f === "string" ? netmanthan.meta.get_docfield(this.doctype, f) : f;
					if (
						df &&
						df.fieldtype == "Link" &&
						netmanthan.boot.link_title_doctypes.includes(df.options)
					) {
						netmanthan.model.with_doctype(df.options, () => {
							const meta = netmanthan.get_meta(df.options);
		...
		this.fields.forEach((f) => {
			const df = netmanthan.meta.get_docfield(f[1], f[0]);
	...
	patch_refresh_and_load_lib() {
		...
		this.refresh = netmanthan.utils.throttle(this.refresh, 1000);
		this.load_lib = new Promise((resolve) => {
			if (this.required_libs) {
				netmanthan.require(this.required_libs, resolve);
			} else {
				resolve();
			}
		});
	...
	set_primary_action() {
		if (this.can_create && !netmanthan.boot.read_only) {
			const doctype_name = __(netmanthan.router.doctype_layout) || __(this.doctype);
	...
	make_new_doc() {
		...
		this.filter_area.get().forEach((f) => {
			if (f[2] === "=" && netmanthan.model.is_non_std_field(f[1])) {
				options[f[1]] = f[3];
			}
		});
		netmanthan.new_doc(doctype, options);
	}
	...
	setup_columns() {
		...
		const get_df = netmanthan.meta.get_docfield.bind(null, this.doctype);
		...
		if (netmanthan.has_indicator(this.doctype)) {
			// indicator
			this.columns.push({
				type: "Status",
			});
		...
		this.columns = this.columns.concat(
			fields_in_list_view
				.filter((df) => {
					if (netmanthan.has_indicator(this.doctype) && df.fieldname === "status") {
						return false;
					}
					if (!df.in_list_view) {
						return false;
	...
	get_no_result_message() {
		...
		let empty_state_image =
			...
			"/assets/netmanthan/images/ui-states/list-empty-state.svg";
	...
	before_refresh() {
		if (netmanthan.route_options && this.filter_area) {
	...
	before_render() {
		...
		netmanthan.model.user_settings.save(this.doctype, "last_view", this.view_name);
	...
	get_header_html() {
		...
		let subject_html = `
			...
			<span class="level-item list-liked-by-me hidden-xs">
				<span title="${__("Likes")}">${netmanthan.utils.icon("heart", "sm", "like-icon")}</span>
			</span>
		...
		const $columns = this.columns
			.map((col) => {
				let classes = [
					...
					netmanthan.model.is_numeric_field(col.df) ? "text-right" : "",
				].join(" ");
	...
	get_column_html(col, doc) {
		...
		let translated_doctypes = netmanthan.boot?.translated_doctypes || [];
		...
		const format = () => {
			if (df.fieldtype === "Code") {
				...
				return netmanthan.format(value, df, null, doc);
			}
		};
			...
			if (strip_html_required) {
				...
				_value =
					typeof value_display === "string"
						? netmanthan.utils.escape_html(value_display)
						: value_display;
			}
			...
			if (df.fieldtype === "Image") {
				html = df.options
					...
					: `<div class="missing-image small">
						${netmanthan.utils.icon("restriction")}
					</div>`;
			} else if (df.fieldtype === "Select") {
				html = `<span class="filterable indicator-pill ${netmanthan.utils.guess_colour(
					_value
				)} ellipsis"
				...
				html = `<a class="filterable ellipsis"
					data-filter="${fieldname},=,${netmanthan.utils.escape_html(value)}">
					${format()}
				</a>`;
			}

			return `<span class="ellipsis"
				title="${__(label)}: ${netmanthan.utils.escape_html(_value)}">
				${html}
			</span>`;
		};
		...
		const css_class = [
			"list-row-col ellipsis",
			class_map[col.type],
			netmanthan.model.is_numeric_field(df) ? "text-right" : "",
		].join(" ");
	...
	get_tags_html(user_tags, limit, colored = false) {
		let get_tag_html = (tag) => {
			...
			if (tag) {
				if (colored) {
					color = netmanthan.get_palette(tag);
	...
	get_meta_html(doc) {
		...
		if (assigned_users.length) {
			assigned_to = `<div class="list-assignments">
					${netmanthan.avatar_group(assigned_users, 3, { filterable: true })[0].outerHTML}
				</div>`;
		}

		const comment_count = `<span class="comment-count">
				${netmanthan.utils.icon("small-message")}
	...
	get_form_link(doc) {
		...
		return `/app/${encodeURIComponent(
			netmanthan.router.slug(netmanthan.router.doctype_layout || this.doctype)
		)}/${encodeURIComponent(cstr(doc.name))}`;
	}

	get_seen_class(doc) {
		return JSON.parse(doc._seen || "[]").includes(netmanthan.session.user) ? "" : "bold";
	}

	get_like_html(doc) {
		const liked_by = JSON.parse(doc._liked_by || "[]");
		const heart_class = liked_by.includes(netmanthan.session.user)
			? "liked-by liked"
			: "not-liked";
		const title = liked_by.map((u) => netmanthan.user_info(u).fullname).join(", ");

		const div = document.createElement("div");
		div.innerHTML = `
			<span class="like-action ${heart_class}">
				${netmanthan.utils.icon("heart", "sm", "like-icon")}
			</span>
	...
	get_indicator_html(doc, show_workflow_state) {
		const indicator = netmanthan.get_indicator(doc, this.doctype, show_workflow_state);
	...
	get_indicator_dot(doc) {
		const indicator = netmanthan.get_indicator(doc, this.doctype);
		if (!indicator) return "";
	...
	get_image_url(doc) {
		...
		if (window.cordova && !netmanthan.utils.is_url(url)) {
			url = netmanthan.base_url + url;
		}
		return url || null;
	}
	...
	setup_filterable() {
		...
		this.$result.on("click", ".filterable", (e) => {
			...
			const filters_to_apply = filters.map((f) => {
				f = f.split(",");
				if (f[2] === "Today") {
					f[2] = netmanthan.datetime.get_today();
				} else if (f[2] == "User") {
					f[2] = netmanthan.session.user;
				}
	...
	setup_list_click() {
		this.$result.on("click", ".list-row, .image-view-header, .file-header", (e) => {
			...
			if (link) {
				netmanthan.set_route(link.pathname);
				return false;
			}
		});
	}
	...
	setup_like() {
		this.$result.on("click", ".like-action", netmanthan.ui.click_toggle_like);
	...
	setup_realtime_updates() {
		...
		netmanthan.socketio.doctype_subscribe(this.doctype);
		netmanthan.realtime.off("list_update");
		netmanthan.realtime.on("list_update", (data) => {
	...
	disable_realtime_updates() {
		netmanthan.socketio.doctype_unsubscribe(this.doctype);
	...
	process_document_refreshes() {
		...
		netmanthan.call(call_args).then(({ message }) => {
			if (!message) return;
			const data = netmanthan.utils.dict(message.keys, message.values);
	...
	save_view_user_settings(obj) {
		return netmanthan.model.user_settings.save(this.doctype, this.view_name, obj);
	}

	on_update() {}

	update_url_with_filters() {
		if (netmanthan.get_route_str() == this.page_name && !this.report_name) {
	...
	get_menu_items() {
		...
		if (netmanthan.model.can_import(doctype, null, this.meta)) {
			items.push({
				label: __("Import", null, "Button in list view menu"),
				action: () =>
					netmanthan.set_route("list", "data-import", {
		...
		if (netmanthan.model.can_set_user_permissions(doctype)) {
			items.push({
		...
		if (
			netmanthan.model.can_create("Custom Field") &&
			netmanthan.model.can_create("Property Setter")
		) {
			items.push({
				...
				action: () => {
					if (!this.meta) return;
					if (this.meta.custom) {
						netmanthan.set_route("form", "doctype", doctype);
					} else if (!this.meta.custom) {
						netmanthan.set_route("form", "customize-form", {
							doc_type: doctype,
						});
					}
				},
		...
		if (netmanthan.user.has_role("System Manager") && netmanthan.boot.developer_mode === 1) {
			// edit doctype
			items.push({
				label: __("Edit DocType", null, "Button in list view menu"),
				action: () => netmanthan.set_route("form", "doctype", doctype),
				standard: true,
			});
		}
	...
	show_list_settings() {
		netmanthan.model.with_doctype(this.doctype, () => {
			new ListSettings({
				listview: this,
				doctype: this.doctype,
				settings: this.list_view_settings,
				meta: netmanthan.get_meta(this.doctype),
			});
		});
	}
	...
	get_workflow_action_menu_items() {
		...
		if (netmanthan.model.has_workflow(this.doctype)) {
			const actions = netmanthan.workflow.get_all_transition_actions(this.doctype);
			actions.forEach((action) => {
				workflow_actions.push({
					...
					action: () => {
						...
						netmanthan
							.xcall("netmanthan.model.workflow.bulk_workflow_approval", {
	...
	toggle_workflow_actions() {
		if (!netmanthan.model.has_workflow(this.doctype)) return;
		...
		netmanthan
			.xcall("netmanthan.model.workflow.get_common_transition_actions", {
				docs: checked_items,
				doctype: this.doctype,
			})
			.then((actions) => {
	...
	get_actions_menu_items() {
		...
		const is_field_editable = (field_doc) => {
			return (
				field_doc.fieldname &&
				netmanthan.model.is_value_type(field_doc) &&
		...
		const bulk_edit = () => {
			return {
				...
				action: () => {
					...
					netmanthan.meta.get_docfields(doctype).forEach((field_doc) => {
		...
		// bulk printing
		if (netmanthan.model.can_print(doctype)) {
			actions_menu_items.push(bulk_printing());
		}

		// bulk submit
		if (
			netmanthan.model.is_submittable(doctype) &&
			has_submit_permission(doctype) &&
			!netmanthan.model.has_workflow(doctype)
		) {
			actions_menu_items.push(bulk_submit());
		}

		// bulk cancel
		if (netmanthan.model.can_cancel(doctype) && !netmanthan.model.has_workflow(doctype)) {
			actions_menu_items.push(bulk_cancel());
		}

		// bulk delete
		if (netmanthan.model.can_delete(doctype)) {
	...
	parse_filters_from_route_options() {
		...
		for (let field in netmanthan.route_options) {
			let doctype = null;
			let value = netmanthan.route_options[field];
			...
			if (!doctype) {
				doctype = netmanthan.meta.get_doctype_for_field(this.doctype, field);
			}
...
netmanthan.get_list_view = (doctype) => {
	let route = `List/${doctype}/List`;
	return netmanthan.views.list_view[route];
};
