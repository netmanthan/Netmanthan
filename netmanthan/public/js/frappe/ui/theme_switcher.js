...
netmanthan.ui.ThemeSwitcher = class ThemeSwitcher {
	constructor() {
		this.setup_dialog();
		this.refresh();
	}
	...
	bind_events() {
		this.dialog.$wrapper.on("keydown", (e) => {
			...
			const key = netmanthan.ui.keys.get_key(e);
			let increment_by;
	...
	toggle_theme(theme) {
		...
		netmanthan.show_alert(__("Theme Changed"), 3);

		netmanthan.xcall("netmanthan.core.doctype.user.user.switch_theme", {
			theme: toTitle(theme),
		});
	}
...
netmanthan.ui.add_system_theme_switch_listener = () => {
	netmanthan.ui.dark_theme_media_query.addEventListener("change", () => {
		netmanthan.ui.set_theme();
	});
};

netmanthan.ui.dark_theme_media_query = window.matchMedia("(prefers-color-scheme: dark)");

netmanthan.ui.set_theme = (theme) => {
	const root = document.documentElement;
	let theme_mode = root.getAttribute("data-theme-mode");
	if (!theme) {
		if (theme_mode === "automatic") {
			theme = netmanthan.ui.dark_theme_media_query.matches ? "dark" : "light";
		}
	}
