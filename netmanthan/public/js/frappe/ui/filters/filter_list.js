netmanthan.ui.FilterGroup = class {
	constructor(opts) {
		$.extend(this, opts);
		this.filters = [];
		window.fltr = this;
	...
	add_filters(filters) {
		...
		return netmanthan.run_serially(promises).then(() => this.update_filters());
	}
	...
	_push_new_filter(doctype, fieldname, condition, value, hidden = false) {
		...
		let filter = new netmanthan.ui.Filter(args);
	...
	filter_exists(filter_value) {
		...
		this.filters
			...
			.map((f) => {
				...
				let equal = netmanthan.utils.arrays_equal;
