...
netmanthan.ui.Tags = class {
	...
	bind() {
		...
		const select_tag = function () {
			const tagValue = netmanthan.utils.xss_sanitise(me.$input.val());
			me.addTag(tagValue);
			me.$input.val("");
		};
	...
	removeTag(label) {
		label = netmanthan.utils.xss_sanitise(label);
	...
	get_tag(label) {
		let $tag = netmanthan.get_data_pill(label, label, (target, pill_wrapper) => {
