// netmanthan.ui.Capture
// Author - Achilles Rasquinha <achilles@netmanthan.io>
 ...
 *
 * @example
 * netmanthan._.get_data_uri(video)
 * // returns "data:image/pngbase64,..."
 */
netmanthan._.get_data_uri = (element) => {
...
/**
 * @description netmanthan's Capture object.
 *
 * @example
 * const capture = netmanthan.ui.Capture()
 * capture.show()
 *
 ...
 */
netmanthan.ui.Capture = class {
	constructor(options = {}) {
		this.options = netmanthan.ui.Capture.OPTIONS;
	...
	set_options(options) {
		this.options = { ...netmanthan.ui.Capture.OPTIONS, ...options };
	...
	show() {
		...
		if (cint(netmanthan.boot.sysdefaults.force_web_capture_mode_for_uploads)) {
			this.show_for_desktop();
		} else if (netmanthan.is_mobile()) {
	...
	build_dialog() {
		...
		me.$template = $(netmanthan.ui.Capture.TEMPLATE);
	...
	show_for_desktop() {
		...
		this.render_stream()
			...
			.catch((err) => {
				if (me.options.error) {
					netmanthan.show_alert(netmanthan.ui.Capture.ERR_MESSAGE, 3);
				}
			...
			images += `
				<div class="mt-1 p-1 rounded col-md-3 col-sm-4 col-xs-4" data-idx="${idx}">
					<span class="capture-remove-btn" data-idx="${idx}">
						${netmanthan.utils.icon("close", "lg")}
					</span>
	...
	setup_take_photo_action() {
		...
		this.dialog.set_primary_action(__("Take Photo"), () => {
			const data_url = netmanthan._.get_data_uri(me.video);
...
netmanthan.ui.Capture.OPTIONS = {
	title: __("Camera"),
	animate: false,
	error: false,
};
netmanthan.ui.Capture.ERR_MESSAGE = __("Unable to load camera.");
netmanthan.ui.Capture.TEMPLATE = `
<div class="netmanthan-capture">
	<div class="embed-responsive embed-responsive-16by9 fc-stream-container">
		<video class="fc-stream embed-responsive-item">${netmanthan.ui.Capture.ERR_MESSAGE}</video>
	</div>
