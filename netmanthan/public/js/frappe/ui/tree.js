...
netmanthan.ui.Tree = class {
	constructor({
		...
		if (!icon_set) {
			this.icon_set = {
				open: netmanthan.utils.icon("folder-open", "md"),
				closed: netmanthan.utils.icon("folder-normal", "md"),
				leaf: netmanthan.utils.icon("primitive-dot", "xs"),
			};
		}
	...
	get_all_nodes(value, is_root, label) {
		...
		return new Promise((resolve) => {
			netmanthan.call({
				method: "netmanthan.desk.treeview.get_all_nodes",
				args: args,
				callback: (r) => {
	...
	on_node_click(node) {
		this.expand_node(node);
		netmanthan.dom.activate(this.wrapper, node.$tree_link, "tree-link");
