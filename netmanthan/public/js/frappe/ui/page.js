 ...
 *
 * @returns {netmanthan.ui.Page}
 */

/**
 * @typedef {Object} netmanthan.ui.Page
 */

netmanthan.ui.make_app_page = function (opts) {
	opts.parent.page = new netmanthan.ui.Page(opts);
	return opts.parent.page;
};

netmanthan.ui.pages = {};

netmanthan.ui.Page = class Page {
	constructor(opts) {
		$.extend(this, opts);
		...
		this.make();
		netmanthan.ui.pages[netmanthan.get_route_str()] = this;
	}
	...
	load_lib(callback) {
		netmanthan.require(this.required_libs, callback);
	}

	add_main_section() {
		$(netmanthan.render_template("page", {})).appendTo(this.wrapper);
	...
	setup_page() {
		...
		this.card_layout && this.main.addClass("netmanthan-card");
	...
	setup_sidebar_toggle() {
		...
		if (this.disable_sidebar_toggle || !sidebar_wrapper.length) {
			...
			sidebar_toggle.click(() => {
				if (netmanthan.utils.is_xs() || netmanthan.utils.is_sm()) {
	...
	update_sidebar_icon() {
		...
		sidebar_toggle_icon.html(
			netmanthan.utils.icon(is_sidebar_visible ? "sidebar-collapse" : "sidebar-expand", "md")
		);
	}
	...
	add_action_icon(icon, click, css_class = "", tooltip_label) {
		...
		button.click(click);
		button
			.attr("title", __(tooltip_label || netmanthan.unscrub(icon)))
	...
	get_icon_label(icon, label) {
		...
		return `${icon ? netmanthan.utils.icon(icon_name, size) : ""} <span class="hidden-xs"> ${__(
			label
		)} </span>`;
	}
	...
	set_action(btn, opts) {
		...
		netmanthan.ui.keys.get_shortcut_group(this).add(btn, text_span.length ? text_span : btn);
	}
	...
	add_dropdown_item({
		...
		if (icon) {
			$icon = `<span class="menu-item-icon">${netmanthan.utils.icon(icon)}</span>`;
		}
		...
		if (shortcut) {
			...
			`);
			netmanthan.ui.keys.add_shortcut(shortcut_obj);
		} else {
			$li = $(`
				<li>
					<a class="grey-link dropdown-item" href="#" onClick="return false;">
			...
			$group = $(
				`<div class="inner-group-button" data-label="${encodeURIComponent(label)}">
					<button type="button" class="btn btn-default ellipsis" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
						${label}
						${netmanthan.utils.icon("select", "xs")}
					</button>
	...
	set_title(title, icon = null, strip = true, tab_title = "") {
		...
		this.title = title;
		netmanthan.utils.set_title(tab_title || title);
		if (icon) {
			title = `${netmanthan.utils.icon(icon)} ${title}`;
		}
	...
	add_button(label, click, opts) {
		...
		let button = $(`<button
			class="btn ${opts.btn_class || "btn-default"} ${opts.btn_size || "btn-sm"} ellipsis">
				${opts.icon ? netmanthan.utils.icon(opts.icon) : ""}
				${label}
		</button>`);
	...
	add_dropdown_button(parent, label, click, icon) {
		netmanthan.ui.toolbar.add_dropdown_button(parent, label, click, icon);
	}
	...
	add_field(df, parent) {
		...
		var f = netmanthan.ui.form.make_control({
			df: df,
