netmanthan.ui.TagEditor = class TagEditor {
	constructor(opts) {
		/* docs:
		Arguments
	...
	setup_tags() {
		...
		this.tags = new netmanthan.ui.Tags({
			parent: this.wrapper,
			placeholder: __("Add a tag ..."),
			onTagAdd: (tag) => {
				if (me.initialized && !me.refreshing) {
					return netmanthan.call({
						method: "netmanthan.desk.doctype.tag.tag.add_tag",
			...
			onTagRemove: (tag) => {
				if (!me.refreshing) {
					return netmanthan.call({
						method: "netmanthan.desk.doctype.tag.tag.remove_tag",
						args: me.get_args(tag),
						callback: function (r) {
	...
	setup_awesomplete() {
		...
		$input.on("input", function (e) {
			...
			netmanthan.call({
				method: "netmanthan.desk.doctype.tag.tag.get_tags",
				args: {
