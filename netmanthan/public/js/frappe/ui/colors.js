...
netmanthan.ui.color = {
	get: function (color_name, shade) {
		...
		return netmanthan.ui.color_map;
	},
	get_color: function (color_name) {
		const color_names = Object.keys(netmanthan.ui.color_map);
		if (color_names.includes(color_name)) {
			return netmanthan.ui.color_map[color_name];
		} else {
	...
	all: function () {
		return Object.values(netmanthan.ui.color_map).reduce((acc, curr) => acc.concat(curr), []);
	},
	names: function () {
		return Object.keys(netmanthan.ui.color_map);
	},
	...
	get_color_name: function (hex) {
		for (const key in netmanthan.ui.color_map) {
			const colors = netmanthan.ui.color_map[key];
...
netmanthan.ui.color_map = netmanthan.ui.color.get_color_map();
