...
netmanthan.ui.is_liked = function (doc) {
	var liked = netmanthan.ui.get_liked_by(doc);
	return liked.indexOf(netmanthan.session.user) === -1 ? false : true;
};

netmanthan.ui.get_liked_by = function (doc) {
...
netmanthan.ui.toggle_like = function ($btn, doctype, name, callback) {
	var add = $btn.hasClass("not-liked") ? "Yes" : "No";
	// disable click
	$btn.css("pointer-events", "none");

	netmanthan.call({
		method: "netmanthan.desk.like.toggle_like",
		quiet: true,
		args: {
			doctype: doctype,
			name: name,
			...
			if (!r.exc) {
				...
				if (doc) {
					var liked_by = JSON.parse(doc._liked_by || "[]"),
						idx = liked_by.indexOf(netmanthan.session.user);
					if (add === "Yes") {
						if (idx === -1) liked_by.push(netmanthan.session.user);
					} else {
						if (idx !== -1) {
...
netmanthan.ui.click_toggle_like = function () {
	var $btn = $(this);
	...
	netmanthan.ui.toggle_like($btn, doctype, name, function () {
		if (not_liked) {
...
netmanthan.ui.setup_like_popover = ($parent, selector, check_not_liked = true) => {
	if (netmanthan.dom.is_touchscreen()) {
		return;
	}
			...
			content: () => {
				...
				liked_by.forEach((user) => {
					...
					liked_by_list.append(`
						<li data-user=${user}>${netmanthan.avatar(user, "avatar-xs")}
							<span>${netmanthan.user.full_name(user)}</span>
						</li>
					`);
				});
				...
				liked_by_list.children("li").click((ev) => {
					...
					netmanthan.set_route(link_base + user);
				});
