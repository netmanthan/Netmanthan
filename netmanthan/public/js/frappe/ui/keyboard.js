...
netmanthan.provide("netmanthan.ui.keys.handlers");

netmanthan.ui.keys.setup = function () {
	$(window).on("keydown", function (e) {
		var key = netmanthan.ui.keys.get_key(e);
		if (netmanthan.ui.keys.handlers[key]) {
			var out = null;
			for (var i = 0, l = netmanthan.ui.keys.handlers[key].length; i < l; i++) {
				var handler = netmanthan.ui.keys.handlers[key][i];
...
netmanthan.ui.keys.standard_shortcuts = standard_shortcuts;
netmanthan.ui.keys.add_shortcut = ({
	...
	netmanthan.ui.keys.off(shortcut, page);
	// attach new handler
	netmanthan.ui.keys.on(shortcut, handler);
...
netmanthan.ui.keys.show_keyboard_shortcut_dialog = () => {
	if (netmanthan.ui.keys.is_dialog_shown) return;
		...
		let html = shortcuts
			...
			.map((shortcut) => {
				let shortcut_label = shortcut.shortcut
					.split("+")
					.map(netmanthan.utils.to_title_case)
					.join("+");
				if (netmanthan.utils.is_mac()) {
	...
	let dialog = new netmanthan.ui.Dialog({
		...
		on_hide() {
			netmanthan.ui.keys.is_dialog_shown = false;
		},
	});
	...
	dialog.show();
	netmanthan.ui.keys.is_dialog_shown = true;
};

netmanthan.ui.keys.get_key = function (e) {
	var keycode = e.keyCode || e.which;
	var key = netmanthan.ui.keys.key_map[keycode] || String.fromCharCode(keycode);
...
netmanthan.ui.keys.on = function (key, handler) {
	if (!netmanthan.ui.keys.handlers[key]) {
		netmanthan.ui.keys.handlers[key] = [];
	}
	netmanthan.ui.keys.handlers[key].push(handler);
};

netmanthan.ui.keys.off = function (key, page) {
	let handlers = netmanthan.ui.keys.handlers[key];
	if (!handlers || handlers.length === 0) return;
	netmanthan.ui.keys.handlers[key] = handlers.filter((h) => {
...
netmanthan.ui.keys.add_shortcut({
	shortcut: "ctrl+s",
	action: function (e) {
		netmanthan.app.trigger_primary_action();
		e.preventDefault();
		return false;
	},
...
netmanthan.ui.keys.on("escape", function (e) {
	handle_escape_key();
});

netmanthan.ui.keys.on("esc", function (e) {
	handle_escape_key();
});

netmanthan.ui.keys.on("enter", function (e) {
	if (window.cur_dialog && cur_dialog.confirm_dialog) {
		cur_dialog.get_primary_btn().trigger("click");
	}
});

netmanthan.ui.keys.on("ctrl+down", function (e) {
	var grid_row = netmanthan.ui.form.get_open_grid_form();
	grid_row &&
...
netmanthan.ui.keys.on("ctrl+up", function (e) {
	var grid_row = netmanthan.ui.form.get_open_grid_form();
	grid_row &&
		grid_row.toggle_view(false, function () {
			grid_row.open_prev();
...
"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter, i) => {
	netmanthan.ui.keys.key_map[65 + i] = letter;
});

// keyCode map
netmanthan.ui.keyCode = {
	ESCAPE: 27,
	LEFT: 37,
	RIGHT: 39,
	UP: 38,
