...
netmanthan.ui.open_dialogs = [];

netmanthan.ui.Dialog = class Dialog extends netmanthan.ui.FieldGroup {
	constructor(opts) {
		super();
		this.display = false;
		this.is_dialog = true;
	...
	make() {
		this.$wrapper = netmanthan.get_modal("", "");
		...
		this.$wrapper
			.on("hide.bs.modal", function () {
				...
				if (netmanthan.ui.open_dialogs[netmanthan.ui.open_dialogs.length - 1] === me) {
					netmanthan.ui.open_dialogs.pop();
					if (netmanthan.ui.open_dialogs.length) {
						window.cur_dialog =
							netmanthan.ui.open_dialogs[netmanthan.ui.open_dialogs.length - 1];
					} else {
						window.cur_dialog = null;
					}
				}
			...
			.on("shown.bs.modal", function () {
				...
				netmanthan.ui.open_dialogs.push(me);
				me.focus_on_first_input();
				me.hide_scrollbar(true);
				me.on_page_show && me.on_page_show();
				$(document).trigger("netmanthan.ui.Dialog:shown");
	...
	toggle_minimize() {
		...
		this.get_minimize_btn().html(netmanthan.utils.icon(icon));
