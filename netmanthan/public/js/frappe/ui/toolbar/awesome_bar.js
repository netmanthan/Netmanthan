...
netmanthan.provide("netmanthan.tags");

netmanthan.search.AwesomeBar = class AwesomeBar {
	setup(element) {
		var me = this;
			...
			netmanthan.utils.debounce(function (e) {
				...
				if (txt && txt.length > 1) {
					...
					me.options = me.options.concat(
						me.deduplicate(netmanthan.search.utils.get_recent_pages(txt || ""))
					);
					me.options = me.options.concat(netmanthan.search.utils.get_frequent_links());
				}
				me.add_help();
		...
		$input.on("awesomplete-select", function (e) {
			...
			if (item.route_options) {
				netmanthan.route_options = item.route_options;
			}
			...
			if (item.onclick) {
				...
				if (event.ctrlKey || event.metaKey) {
					netmanthan.open_in_new_tab = true;
				}
				netmanthan.set_route(item.route);
			}
			$input.val("");
			$input.trigger("blur");
		});
		...
		netmanthan.search.utils.setup_recent();
	}
	...
	add_help() {
		this.options.push({
			...
			onclick: function () {
				...
				netmanthan.msgprint(txt, __("Search Help"));
			},
		});
	}
	...
	build_options(txt) {
		var options = netmanthan.search.utils
			.get_creatables(txt)
			.concat(
				netmanthan.search.utils.get_search_in_list(txt),
				netmanthan.search.utils.get_doctypes(txt),
				netmanthan.search.utils.get_reports(txt),
				netmanthan.search.utils.get_pages(txt),
				netmanthan.search.utils.get_workspaces(txt),
				netmanthan.search.utils.get_dashboards(txt),
				netmanthan.search.utils.get_recent_pages(txt || ""),
				netmanthan.search.utils.get_executables(txt)
			);
		if (txt.charAt(0) === "#") {
			options = netmanthan.tags.utils.get_tags(txt);
		}
	...
	make_global_search(txt) {
		...
		// 		<span class="ellipsis">Search for ${netmanthan.utils.xss_sanitise(txt).bold()}</span>
		// 		<kbd>↵</kbd>
		// 	</span>
		// `);

		// search_text.click(() => {
		// 	netmanthan.searchdialog.search.init_search(txt, "global_search");
		// });
		...
		this.options.push({
			label: `
				<span class="flex justify-between text-medium">
					<span class="ellipsis">${__("Search for {0}", [netmanthan.utils.xss_sanitise(txt).bold()])}</span>
					<kbd>↵</kbd>
				</span>
			`,
	...
	make_search_in_current(txt) {
		...
		if (route[0] === "List" && txt.indexOf(" in") === -1) {
			...
			var meta = netmanthan.get_meta(netmanthan.container.page.list_view.doctype);
		...
		if (first == parseInt(first) || first === "(" || first === "=") {
			...
			try {
				...
				this.options.push({
					...
					default: "Calculator",
					onclick: function () {
						netmanthan.msgprint(formatted_value, __("Result"));
					},
				});
			} catch (e) {
				// pass
	...
	make_random(txt) {
		if (txt.toLowerCase().includes("random")) {
			this.options.push({
				...
				value: netmanthan.utils.get_random(16),
				onclick: function () {
					netmanthan.msgprint(netmanthan.utils.get_random(16), __("Result"));
				},
			});
		}
	}
