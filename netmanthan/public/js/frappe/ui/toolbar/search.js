...
netmanthan.search.SearchDialog = class {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	make() {
		this.search_dialog = new netmanthan.ui.Dialog({
			minimizable: true,
			size: "large",
		});
		this.set_header();
		...
		this.searches = {
			global_search: {
				...
				get_results: (keywords, callback) => {
					let start = 0,
						limit = 100;
					let results = netmanthan.search.utils.get_nav_results(keywords);
					netmanthan.search.utils.get_global_results(keywords, start, limit).then(
						(global_results) => {
			...
			tags: {
				...
				get_results: (keywords, callback) => {
					var results = netmanthan.search.utils.get_nav_results(keywords);
					netmanthan.tags.utils.get_tag_results(keywords).then(
						(global_results) => {
							results = results.concat(global_results);
							callback(results, keywords);
						},
	...
	render_data(result_sets) {
		let $search_results = $(netmanthan.render_template("search")).addClass("hide");
	...
	render_result(type, result) {
		...
		if (result.image !== undefined) {
			let avatar_html = netmanthan.get_avatar("avatar-medium", result.label, result.image);
	...
	handle_result_click(result, $result) {
		if (result.route_options) {
			netmanthan.route_options = result.route_options;
		}
		$result.on("click", () => {
			...
			if (result.onclick) {
				...
				netmanthan.set_route(result.route);
