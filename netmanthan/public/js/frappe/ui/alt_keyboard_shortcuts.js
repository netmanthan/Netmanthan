netmanthan.provide("netmanthan.ui.keys");

let shortcut_groups = new WeakMap();
let shortcut_group_list = [];
netmanthan.ui.keys.shortcut_groups = shortcut_groups;

netmanthan.ui.keys.get_shortcut_group = (parent) => {
	// parent must be an object
	if (!shortcut_groups.has(parent)) {
		shortcut_groups.set(parent, new netmanthan.ui.keys.AltShortcutGroup());
	}
...
netmanthan.ui.keys.bind_shortcut_group_event = () => {
	...
	$(document).on("keydown", (e) => {
		let key = (netmanthan.ui.keys.key_map[e.which] || "").toLowerCase();
...
netmanthan.ui.keys.AltShortcutGroup = class AltShortcutGroup {
	constructor() {
		this.shortcuts_dict = {};
	...
	is_taken(letter) {
		let is_in_global_shortcut = netmanthan.ui.keys.standard_shortcuts
			.filter((s) => !s.page)
