...
netmanthan.data_import.ImportPreview = class ImportPreview {
	...
	prepare_columns() {
		this.columns = this.preview_data.columns.map((col, i) => {
			...
			if (col.skip_import) {
				...
				return {
					id: netmanthan.utils.get_random(6),
	...
	setup_styles() {
		...
		this.datatable.style.setStyle(`svg.import-success`, {
			width: "16px",
			fill: netmanthan.ui.color.get_color_shade("green", "dark"),
		});
		...
		this.datatable.style.setStyle(row_classes, {
			pointerEvents: "none",
			backgroundColor: netmanthan.ui.color.get_color_shade("gray", "extra-light"),
			color: netmanthan.ui.color.get_color_shade("gray", "dark"),
		});
	}
	...
	show_column_warning(_, $target) {
		...
		netmanthan.utils.scroll_to($warning, true, 30);
	}
...
function get_fields_as_options(doctype, column_map) {
	let keys = [doctype];
	netmanthan.meta.get_table_fields(doctype).forEach((df) => {
	...
	return [].concat(
		...keys.map((key) => {
			return column_map[key].map((df) => {
				...
				if (doctype !== key) {
					let table_field = netmanthan.meta.get_docfield(doctype, key);
