...
export default class NumberCardWidget extends Widget {
	...
	make_card() {
		netmanthan.model.with_doc("Number Card", this.number_card_name || this.name).then((card) => {
			if (!card) {
	...
	create_number_card() {
		this.set_doc_args();
		return netmanthan
			.xcall("netmanthan.desk.doctype.number_card.number_card.create_number_card", {
				args: this.card_doc,
			})
			.then((doc) => {
				this.name = doc.name;
	...
	set_route() {
		...
		const route = netmanthan.utils.generate_route({
			name: name,
		...
		if (is_document_type) {
			...
			netmanthan.route_options = filters.reduce((acc, filter) => {
	...
	get_settings(type) {
		...
		const settings_map = {
			...
			"Document Type": {
				method: "netmanthan.desk.doctype.number_card.number_card.get_result",
				args: {
					doc: this.card_doc,
					filters: this.filters,
				},
	...
	get_filters() {
		const filters = netmanthan.dashboard_utils.get_all_filters(this.card_doc);
		return filters;
	}
	...
	render_card() {
		...
		netmanthan.run_serially([() => this.render_number(), () => this.render_stats()]);
	}
	...
	get_number() {
		return netmanthan.xcall(this.settings.method, this.settings.args).then((res) => {
	...
	get_number_for_doctype_card(res) {
		...
		if (this.card_doc.function !== "Count") {
			return netmanthan.model.with_doctype(this.card_doc.document_type, () => {
				const based_on_df = netmanthan.meta.get_docfield(
	...
	get_number_for_report_card(res) {
		...
		this.number = netmanthan.report_utils.get_result_of_fn(this.card_doc.report_function, vals);
		this.set_formatted_number(col);
	}

	set_formatted_number(df) {
		const default_country = netmanthan.sys_defaults.country;
		const shortened_number = netmanthan.utils.shorten_number(this.number, default_country, 5);
		let number_parts = shortened_number.split(" ");

		const symbol = number_parts[1] || "";
		const formatted_number = $(netmanthan.format(number_parts[0], df)).text();
		...
		return this.get_percentage_stats().then(() => {
			if (this.percentage_stat == 0 || this.percentage_stat == undefined) {
				...
				caret_html = `<span class="indicator-pill-round green">
						${netmanthan.utils.icon("arrow-up-right", "xs")}
					</span>`;
				color_class = "green-stat";
			} else {
				caret_html = `<span class="indicator-pill-round red">
						${netmanthan.utils.icon("arrow-down-left", "xs")}
					</span>`;
	...
	get_percentage_stats() {
		return netmanthan
			.xcall("netmanthan.desk.doctype.number_card.number_card.get_percentage_difference", {
				doc: this.card_doc,
				filters: this.filters,
				result: this.number,
			})
			.then((res) => {
				if (res !== undefined) {
					this.percentage_stat = netmanthan.utils.shorten_number(res);
				}
			});
	}
		...
		let actions = [
			...
			},
			{
				...
				handler: () => {
					...
					netmanthan.set_route("Form", "Number Card", number_card);
				},
			},
		];
