class WidgetDialog {
		...
		this.dialog = new netmanthan.ui.Dialog({
			...
			primary_action: (data) => {
				...
				if (!this.editing && !data.name) {
					data.name = `${this.type}-${this.label}-${netmanthan.utils.get_random(20)}`;
				}
	...
	get_title() {
		...
		let label = (action = action + " " + netmanthan.model.unscrub(this.type));
		return __(label);
	}
...
class QuickListDialog extends WidgetDialog {
	...
	generate_filter_from_json() {
		if (this.values && this.values.quick_list_filter) {
			this.filters = netmanthan.utils.get_filter_from_json(
	...
	process_data(data) {
		if (this.filter_group) {
			...
			data.quick_list_filter = netmanthan.utils.get_filter_as_json(filters);
		}
...
class ShortcutDialog extends WidgetDialog {
				...
				onchange: () => {
					if (this.dialog.get_value("type") == "DocType") {
						this.dialog.fields_dict.link_to.get_query = () => {
							return {
								query: "netmanthan.core.report.permitted_documents_for_user.permitted_documents_for_user.query_doctypes",
								filters: {
			...
			},
			{
				...
				onchange: () => {
					...
					if (doctype && this.dialog.get_value("type") == "DocType") {
						netmanthan.model.with_doctype(doctype, async () => {
							let meta = netmanthan.get_meta(doctype);

							if (doctype && netmanthan.boot.single_types.includes(doctype)) {
								this.hide_filters();
							} else if (doctype) {
							...
							if (netmanthan.boot.calendars.includes(doctype)) views.push("Calendar");

							const response = await netmanthan.db.get_value(
								"Kanban Board",
			...
			},
			{
				...
				depends_on: (state) => {
					if (this.dialog) {
						...
						let is_single = netmanthan.boot.single_types.includes(doctype);
	...
	process_data(data) {
		if (this.dialog.get_value("type") == "DocType" && this.filter_group) {
			let filters = this.filter_group.get_filters();
			data.stats_filter = netmanthan.utils.get_filter_as_json(filters);
		}

		data.label = data.label ? data.label : netmanthan.model.unscrub(data.link_to);
...
class NumberCardDialog extends WidgetDialog {
			...
			return [
				{
					...
					get_query: () => {
						return {
							query: "netmanthan.desk.doctype.number_card.number_card.get_cards_for_user",
							filters: {
	...
	set_aggregate_function_fields() {
		...
		if (this.document_type && netmanthan.get_meta(this.document_type)) {
			netmanthan.get_meta(this.document_type).fields.map((df) => {
		...
		return [
			{
				...
				get_query: () => {
					return {
						query: "netmanthan.desk.doctype.custom_html_block.custom_html_block.get_custom_blocks_for_user",
					};
				},
			},
		];
