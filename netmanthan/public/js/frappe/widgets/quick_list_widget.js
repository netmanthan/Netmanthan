...
export default class QuickListWidget extends Widget {
	...
	setup_add_new_button() {
		...
		this.add_new_button.on("click", () => {
			netmanthan.set_route(
				netmanthan.utils.generate_route({
					type: "doctype",
	...
	setup_refresh_list_button() {
		this.refresh_list = $(
			`<div class="refresh-list btn btn-xs pull-right" title="${__("Refresh List")}">
				${netmanthan.utils.icon("refresh", "sm")}
			</div>`
		);
	...
	setup_filter(doctype) {
		...
		this.filters = netmanthan.utils.get_filter_from_json(this.quick_list_filter, doctype);
	...
	setup_filter_dialog() {
		...
		this.dialog = new netmanthan.ui.Dialog({
			...
			primary_action: function () {
				...
				me.quick_list_filter = netmanthan.utils.get_filter_as_json(filters);
	...
	setup_quick_list_item(doc) {
		const indicator = netmanthan.get_indicator(doc, this.document_type);
			...
			<div class="quick-list-item">
				<div class="ellipsis left">
					...
					<div class="timestamp text-muted">
						${netmanthan.datetime.prettyDate(doc.modified)}
					</div>
				</div>
			</div>
		`);
		...
		$(`<div class="right-arrow">${netmanthan.utils.icon("right", "xs")}</div>`).appendTo(
			$quick_list_item
		);
		...
		$quick_list_item.click((e) => {
			...
			netmanthan.set_route(`${netmanthan.utils.get_form_link(this.document_type, doc.name)}`);
		});
	...
	set_body() {
		...
		netmanthan.model.with_doctype(this.document_type, () => {
			let fields = ["name"];

			// get name of title field
			if (!this.title_field_name) {
				let meta = netmanthan.get_meta(this.document_type);
			...
			this.has_status_field = netmanthan.meta.has_field(this.document_type, "status");
			...
			if (this.has_status_field) {
				...
				let workflow_fieldname = netmanthan.workflow.get_state_fieldname(this.document_type);
				workflow_fieldname && fields.push(workflow_fieldname);
			}

			fields.push("modified");

			let quick_list_filter = netmanthan.utils.get_filter_from_json(this.quick_list_filter);

			let args = {
				method: "netmanthan.desk.reportview.get",
				args: {
	...
	set_footer() {
		...
		let filters = netmanthan.utils.get_filter_from_json(this.quick_list_filter);

		let route = netmanthan.utils.generate_route({ type: "doctype", name: this.document_type });
		this.see_all_button = $(`
		...
		this.see_all_button.click((e) => {
			...
			if (filters) {
				netmanthan.route_options = filters;
			}
