...
export default class OnboardingWidget extends Widget {
	async refresh() {
		netmanthan.utils.load_video_player();
	...
	add_step(step, index) {
		...
		let $step = $(`<a class="onboarding-step ${status}">
				<div class="step-title">
					...
					<div class="step-index step-skipped">${netmanthan.utils.icon("tick", "xs")}</div>
					<div class="step-index step-complete">${netmanthan.utils.icon("tick", "xs")}</div>
	...
	show_step(step) {
		...
		let actions = {
			...
			"Create Entry": (step) => {
				if (step.is_complete) {
					netmanthan.set_route(`/app/List/${step.reference_document}`);
				} else {
		...
		const set_description = () => {
			let content = step.description
				? netmanthan.markdown(step.description)
				: `<h1>${__(step.title)}</h1>`;

			if (step.action === "Create Entry") {
				// add a secondary action to view list
				content += `<p>
					<a href='/app/${netmanthan.router.slug(step.reference_document)}'>
		...
		const toggle_video = () => {
			...
			let plyr = new netmanthan.Plyr(video[0], {
				hideControls: true,
				resetOnEnd: true,
			});
	...
	go_to_page(step) {
		...
		netmanthan.set_route(step.path).then(() => {
			let message =
			...
			let msg_dialog = netmanthan.msgprint({
				message: message,
				title: title,
				primary_action: {
					action: () => {
	...
	open_report(step) {
		let route = netmanthan.utils.generate_route({
		...
		let current_route = netmanthan.get_route();

		netmanthan.set_route(route).then(() => {
			let msg_dialog = netmanthan.msgprint({
				message: __(step.report_description),
				title: __(step.reference_report),
				primary_action: {
					action: () => {
						netmanthan.set_route(current_route).then(() => {
							this.mark_complete(step);
						});
						msg_dialog.hide();
					},
			...
			netmanthan.msg_dialog.custom_onhide = () => this.mark_complete(step);
		});
	}
	...
	show_form_tour(step) {
		let route;
		if (step.is_single) {
			route = netmanthan.router.slug(step.reference_document);
		} else {
			route = `${netmanthan.router.slug(step.reference_document)}/new`;
		}

		let current_route = netmanthan.get_route();

		netmanthan.route_hooks = {};
		netmanthan.route_hooks.after_load = (frm) => {
			const on_finish = () => {
				let msg_dialog = netmanthan.msgprint({
	...
	update_settings(step) {
		...
		netmanthan.route_hooks.after_save = (frm) => {
			let success = false;
			let args = {};
			...
			if (success) {
				...
				args.secondary_action = {
					action: () => netmanthan.set_route(current_route),
					label: __("Go Back"),
				};
			...
			netmanthan.msgprint(args);
			netmanthan.msg_dialog.custom_onhide = () => custom_onhide();
		};

		netmanthan.set_route("Form", step.reference_document);
	}
	...
	async create_entry(step) {
		...
		if (step.is_submittable) {
			netmanthan.route_hooks.after_save = () => {
				netmanthan.msgprint({
					message: __("Submit this document to complete this step."),
					title: __("Document Saved"),
				});
			};
			netmanthan.route_hooks.after_submit = callback;
		} else {
			netmanthan.route_hooks.after_save = callback;
		}

		netmanthan.set_route("Form", step.reference_document, docname);
	}

	show_quick_entry(step) {
		let current_route = netmanthan.get_route_str();
		netmanthan.ui.form.make_quick_entry(
			step.reference_document,
			() => {
				if (netmanthan.get_route_str() != current_route) {
					let success_dialog = netmanthan.msgprint({
						message: __("Let's take you back to onboarding"),
						title: __("Document Saved"),
						primary_action: {
							action: () => {
	...
	show_video(step) {
		netmanthan.help.show_video(step.video_url, step.title);
		this.mark_complete(step);
	}
	...
	update_step_status(step, status, value, callback) {
		...
		netmanthan
			.call("netmanthan.desk.desktop.update_onboarding_step", {
				name: step.name,
				field: status,
				value: value,
			})
	...
	show_success() {
		...
		let success_state_image =
			this.success_state_image || "/assets/netmanthan/images/ui-states/success-color.png";
		let documentation = "";
		if (this.docs_url) {
			documentation = __(
	...
	is_dismissed() {
		...
		if (Object.keys(dismissed).includes(this.title)) {
			...
			let today = new Date();
			let diff = netmanthan.datetime.get_hour_diff(today, last_hidden);
			return diff < 24;
		}
		return false;
	}
	...
	set_actions() {
		...
		dismiss.on("click", () => {
			let dismissed = JSON.parse(localStorage.getItem("dismissed-onboarding") || "{}");
			dismissed[this.title] = netmanthan.datetime.now_datetime();

			localStorage.setItem("dismissed-onboarding", JSON.stringify(dismissed));
			this.delete(true, true);
			this.widget.closest(".ce-block").hide();
			netmanthan.telemetry.capture("dismissed_" + netmanthan.scrub(this.title), "netmanthan_onboarding");
		});
		...
		return netmanthan.model
			...
			.then((onboarding_doc) => {
				if (onboarding_doc) {
					...
					const method =
						"netmanthan.desk.doctype.onboarding_step.onboarding_step.get_onboarding_steps";
					return netmanthan
	...
	async get_first_document(doctype) {
		const { message } = await netmanthan.db.get_value(
			"Form Tour",
		...
		if (message.first_document) {
			await netmanthan.db.get_list(doctype, { order_by: "creation" }).then((res) => {
