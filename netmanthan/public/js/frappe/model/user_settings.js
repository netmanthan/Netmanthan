netmanthan.provide("netmanthan.model.user_settings");

$.extend(netmanthan.model.user_settings, {
	get: function (doctype) {
		return netmanthan
			.call("netmanthan.model.utils.user_settings.get", { doctype })
			.then((r) => JSON.parse(r.message || "{}"));
	},
	save: function (doctype, key, value) {
		if (netmanthan.session.user === "Guest") return Promise.resolve();

		const old_user_settings = netmanthan.model.user_settings[doctype] || {};
	...
	remove: function (doctype, key) {
		var user_settings = netmanthan.model.user_settings[doctype] || {};
	...
	update: function (doctype, user_settings) {
		...
		return netmanthan.call({
			method: "netmanthan.model.utils.user_settings.save",
			args: {
				doctype: doctype,
				user_settings: user_settings,
			},
			callback: function (r) {
				netmanthan.model.user_settings[doctype] = r.message;
			},
		});
	},
});

netmanthan.get_user_settings = function (doctype, key) {
	var settings = netmanthan.model.user_settings[doctype] || {};
	if (key) {
