...
$.extend(netmanthan.model, {
	...
	init: function () {
		...
		netmanthan.realtime.on("doc_update", function (data) {
			var doc = locals[data.doctype] && locals[data.doctype][data.name];

			if (doc) {
				// current document is dirty, show message if its not me
				if (
					netmanthan.get_route()[0] === "Form" &&
					cur_frm.doc.doctype === doc.doctype &&
					cur_frm.doc.name === doc.name
				) {
					if (data.modified !== cur_frm.doc.modified && !netmanthan.ui.form.is_saving) {
						if (!cur_frm.is_dirty()) {
					...
					if (!doc.__unsaved) {
						...
						netmanthan.model.remove_from_locals(doc.doctype, doc.name);
					} else {
	...
	is_value_type: function (fieldtype) {
		...
		return netmanthan.model.no_value_type.indexOf(fieldtype) === -1;
	},

	is_non_std_field: function (fieldname) {
		return ![...netmanthan.model.std_fields_list, ...netmanthan.model.child_table_field_list].includes(
			fieldname
		);
	},
	...
	get_std_field: function (fieldname, ignore = false) {
		var docfield = $.map(
			[].concat(netmanthan.model.std_fields).concat(netmanthan.model.std_fields_table),
			function (d) {
		...
		if (!docfield.length) {
			...
			if (ignore) {
				...
				netmanthan.msgprint(__("Unknown Column: {0}", [fieldname]));
			}
		}
		return docfield[0];
	},
	...
	set_in_localstorage: function (doctype, docs) {
		try {
			...
			netmanthan.model.clear_local_storage();
			localStorage["_doctype:" + doctype] = JSON.stringify(docs);
		}
	},
	...
	with_doctype: function (doctype, callback, async) {
		if (locals.DocType[doctype]) {
			...
			let cached_docs = netmanthan.model.get_from_localstorage(doctype);
			...
			return netmanthan.call({
				method: "netmanthan.desk.form.load.getdoctype",
				type: "GET",
				args: {
					doctype: doctype,
					with_parent: 1,
				...
				callback: function (r) {
					if (r.exc) {
						netmanthan.msgprint(__("Unable to load: {0}", [__(doctype)]));
						throw "No doctype";
					}
					if (r.message == "use_cache") {
						netmanthan.model.sync(cached_doc);
					} else {
						netmanthan.model.set_in_localstorage(doctype, r.docs);
					}
					netmanthan.model.init_doctype(doctype);

					if (r.user_settings) {
						// remember filters and other settings from last view
						netmanthan.model.user_settings[doctype] = JSON.parse(r.user_settings);
						netmanthan.model.user_settings[doctype].updated_on = moment().toString();
					}
					callback && callback(r);
				},
			});
	...
	init_doctype: function (doctype) {
		...
		if (meta.__templates) {
			$.extend(netmanthan.templates, meta.__templates);
		}
	},
	...
	with_doc: function (doctype, name, callback) {
		return new Promise((resolve) => {
			...
			if (
				...
				resolve(netmanthan.get_doc(doctype, name));
			} else {
				return netmanthan.call({
					method: "netmanthan.desk.form.load.getdoc",
					type: "GET",
					args: {
						doctype: doctype,
						name: name,
	...
	get_docinfo: function (doctype, name) {
		return (netmanthan.model.docinfo[doctype] && netmanthan.model.docinfo[doctype][name]) || null;
	},

	set_docinfo: function (doctype, name, key, value) {
		if (netmanthan.model.docinfo[doctype] && netmanthan.model.docinfo[doctype][name]) {
			netmanthan.model.docinfo[doctype][name][key] = value;
		}
	},

	get_shared: function (doctype, name) {
		return netmanthan.model.get_docinfo(doctype, name).shared;
	},

	get_server_module_name: function (doctype) {
		var dt = netmanthan.model.scrub(doctype);
		var module = netmanthan.model.scrub(locals.DocType[doctype].module);
		var app = netmanthan.boot.module_app[module];
	...
	can_create: function (doctype) {
		return netmanthan.boot.user.can_create.indexOf(doctype) !== -1;
	},

	can_select: function (doctype) {
		if (netmanthan.boot.user) {
			return netmanthan.boot.user.can_select.indexOf(doctype) !== -1;
		}
	},

	can_read: function (doctype) {
		if (netmanthan.boot.user) {
			return netmanthan.boot.user.can_read.indexOf(doctype) !== -1;
		}
	},

	can_write: function (doctype) {
		return netmanthan.boot.user.can_write.indexOf(doctype) !== -1;
	},

	can_get_report: function (doctype) {
		return netmanthan.boot.user.can_get_report.indexOf(doctype) !== -1;
	},

	can_delete: function (doctype) {
		if (!doctype) return false;
		return netmanthan.boot.user.can_delete.indexOf(doctype) !== -1;
	},

	can_cancel: function (doctype) {
		if (!doctype) return false;
		return netmanthan.boot.user.can_cancel.indexOf(doctype) !== -1;
	},

	has_workflow: function (doctype) {
		return netmanthan.get_list("Workflow", { document_type: doctype, is_active: 1 }).length;
	},
	...
	is_single: function (doctype) {
		if (!doctype) return false;
		return netmanthan.boot.single_types.indexOf(doctype) != -1;
	},
	...
	can_import: function (doctype, frm, meta = null) {
		...
		if (netmanthan.user_roles.includes("System Manager")) return true;

		if (frm) return frm.perm[0].import === 1;
		return netmanthan.boot.user.can_import.indexOf(doctype) !== -1;
	},
	...
	can_export: function (doctype, frm) {
		...
		return netmanthan.boot.user.can_export.indexOf(doctype) !== -1;
	},

	can_print: function (doctype, frm) {
		if (frm) return frm.perm[0].print === 1;
		return netmanthan.boot.user.can_print.indexOf(doctype) !== -1;
	},

	can_email: function (doctype, frm) {
		if (frm) return frm.perm[0].email === 1;
		return netmanthan.boot.user.can_email.indexOf(doctype) !== -1;
	},

	can_share: function (doctype, frm) {
		let disable_sharing = cint(netmanthan.sys_defaults.disable_document_sharing);

		if (disable_sharing && netmanthan.session.user !== "Administrator") {
			return false;
		}
		...
		return netmanthan.boot.user.can_share.indexOf(doctype) !== -1;
	},
	...
	can_set_user_permissions: function (doctype, frm) {
		...
		return netmanthan.boot.user.can_set_user_permissions.indexOf(doctype) !== -1;
	},

	has_value: function (dt, dn, fn) {
		// return true if property has value
		var val = locals[dt] && locals[dt][dn] && locals[dt][dn][fn];
		var df = netmanthan.meta.get_docfield(dt, fn, dn);

		if (netmanthan.model.table_fields.includes(df.fieldtype)) {
			var ret = false;
	...
	get_list: function (doctype, filters) {
		...
		return netmanthan.utils.filter_dict(docsdict, filters);
	},
	...
	get_value: function (doctype, filters, fieldname, callback) {
		if (callback) {
			...
			if (
				...
				var l = netmanthan.get_list(doctype, filters);
	...
	set_value: function (
		...
		$.each(to_update, (key, value) => {
			if (doc && doc[key] !== value) {
				...
				doc[key] = value;
				tasks.push(() => netmanthan.model.trigger(key, value, doc, skip_dirty_trigger));
			} else {
	...
	on: function (doctype, fieldname, fn) {
		...
		*/
		/* example: netmanthan.model.on("Customer", "age", function(fieldname, value, doc) {
		  if(doc.age < 16) {
		   	netmanthan.msgprint("Warning, Customer must atleast be 16 years old.");
		    raise "CustomerAgeError";
		  }
		}) */
		netmanthan.provide("netmanthan.model.events." + doctype);
		if (!netmanthan.model.events[doctype][fieldname]) {
			netmanthan.model.events[doctype][fieldname] = [];
		}
		netmanthan.model.events[doctype][fieldname].push(fn);
	},
		...
		function enqueue_events(events) {
			...
			for (const fn of events) {
				...
				tasks.push(() => {
					...
					// or use the default promise netmanthan.after_ajax
					if (return_value && return_value.then) {
						return return_value;
					} else {
						return netmanthan.after_server_call();
					}
				});
			}
		}

		if (netmanthan.model.events[doc.doctype]) {
			enqueue_events(netmanthan.model.events[doc.doctype][fieldname]);
			enqueue_events(netmanthan.model.events[doc.doctype]["*"]);
		}
	...
	get_doc: function (doctype, name) {
		...
		if ($.isPlainObject(name)) {
			var doc = netmanthan.get_list(doctype, name);
	...
	get_children: function (doctype, parent, parentfield, filters) {
		if ($.isPlainObject(doctype)) {
			...
			var doc = netmanthan.get_doc(doctype, parent);
		}

		var children = doc[parentfield] || [];
		if (filters) {
			return netmanthan.utils.filter_dict(children, filters);
		} else {
			return children;
		}
	},
	...
	remove_from_locals: function (doctype, name) {
		...
		if (netmanthan.views.formview[doctype]) {
			delete netmanthan.views.formview[doctype].frm.opendocs[name];
		}
	},
	...
	get_no_copy_list: function (doctype) {
		...
		var docfields = netmanthan.get_doc("DocType", doctype).fields || [];
	...
	delete_doc: function (doctype, docname, callback) {
		let title = docname;
		const title_field = netmanthan.get_meta(doctype).title_field;
		if (netmanthan.get_meta(doctype).autoname == "hash" && title_field) {
			const value = netmanthan.model.get_value(doctype, docname, title_field);
			if (value) {
				title = `${value} (${docname})`;
			}
		}
		netmanthan.confirm(__("Permanently delete {0}?", [title.bold()]), function () {
			return netmanthan.call({
				method: "netmanthan.client.delete",
				args: {
					doctype: doctype,
					name: docname,
				},
				...
				callback: function (r, rt) {
					if (!r.exc) {
						netmanthan.utils.play_sound("delete");
						netmanthan.model.clear_doc(doctype, docname);
	...
	round_floats_in: function (doc, fieldnames) {
		if (!fieldnames) {
			fieldnames = netmanthan.meta.get_fieldnames(doc.doctype, doc.parent, {
	...
	validate_missing: function (doc, fieldname) {
		if (!doc[fieldname]) {
			netmanthan.throw(
				__("Please specify") +
					": " +
					__(netmanthan.meta.get_label(doc.doctype, fieldname, doc.parent || doc.name))
			);
		}
	},
	...
	is_numeric_field: function (fieldtype) {
		...
		return netmanthan.model.numeric_fieldtypes.includes(fieldtype);
	},

	set_default_views_for_doctype(doctype, frm) {
		netmanthan.model.with_doctype(doctype, () => {
			let meta = netmanthan.get_meta(doctype);
			let default_views = ["List", "Report", "Dashboard", "Kanban"];

			if (meta.is_calendar_and_gantt && netmanthan.views.calendar[doctype]) {
			...
			if (doctype === "Communication" && netmanthan.boot.email_accounts.length) {
...
// legacy
netmanthan.get_doc = netmanthan.model.get_doc;
netmanthan.get_children = netmanthan.model.get_children;
netmanthan.get_list = netmanthan.model.get_list;
