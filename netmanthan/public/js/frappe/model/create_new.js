...
$.extend(netmanthan.model, {
	...
	get_new_doc: function (doctype, parent_doc, parentfield, with_mandatory_children) {
		...
		var doc = {
			docstatus: 0,
			doctype: doctype,
			name: netmanthan.model.get_new_name(doctype),
			__islocal: 1,
			__unsaved: 1,
			owner: netmanthan.session.user,
		};
		netmanthan.model.set_default_values(doc, parent_doc);
		...
		if (parent_doc) {
			...
			netmanthan.provide("netmanthan.model.docinfo." + doctype + "." + doc.name);
		}

		netmanthan.model.add_to_locals(doc);

		if (with_mandatory_children) {
			netmanthan.model.create_mandatory_children(doc);
		}
		...
		if (netmanthan.route_options && netmanthan.route_options.name_field) {
			var meta = netmanthan.get_meta(doctype);
			// set title field / name as name
			if (meta.autoname && meta.autoname.indexOf("field:") !== -1) {
				doc[meta.autoname.substr(6)] = netmanthan.route_options.name_field;
			} else if (meta.title_field) {
				doc[meta.title_field] = netmanthan.route_options.name_field;
			}

			delete netmanthan.route_options.name_field;
		}

		// set route options
		if (netmanthan.route_options && !doc.parent) {
			$.each(netmanthan.route_options, function (fieldname, value) {
				var df = netmanthan.meta.has_field(doctype, fieldname);
				if (
					df &&
	...
	make_new_doc_and_get_name: function (doctype, with_mandatory_children) {
		return netmanthan.model.get_new_doc(doctype, null, null, with_mandatory_children).name;
	},

	get_new_name: function (doctype) {
		var cnt = netmanthan.model.new_name_count;
		if (!cnt[doctype]) cnt[doctype] = 0;
		cnt[doctype]++;
		return netmanthan.router.slug(`new-${doctype}-${cnt[doctype]}`);
	},

	set_default_values: function (doc, parent_doc) {
		var doctype = doc.doctype;
		var docfields = netmanthan.meta.get_docfields(doctype);
		var updated = [];
		for (var fid = 0; fid < docfields.length; fid++) {
			var f = docfields[fid];
			if (!in_list(netmanthan.model.no_value_type, f.fieldtype) && doc[f.fieldname] == null) {
				if (f.no_default) continue;
				var v = netmanthan.model.get_default_value(f, doc, parent_doc);
				if (v) {
	...
	create_mandatory_children: function (doc) {
		var meta = netmanthan.get_meta(doc.doctype);
		if (meta && meta.istable) return;

		// create empty rows for mandatory table fields
		netmanthan.meta.get_docfields(doc.doctype).forEach(function (df) {
			if (df.fieldtype === "Table" && df.reqd) {
				netmanthan.model.add_child(doc, df.fieldname);
			}
		});
	},
	...
	get_default_value: function (df, doc, parent_doc) {
		var user_default = "";
		var user_permissions = netmanthan.defaults.get_user_permissions();
		let allowed_records = [];
		let default_doc = null;
		let value = null;
		if (user_permissions) {
			({ allowed_records, default_doc } = netmanthan.perm.filter_allowed_docs_for_doctype(
		...
		if (df.fieldtype === "Link" && df.options !== "User") {
			...
			if (has_user_permissions && default_doc) {
				...
				if (!df.ignore_user_permissions) {
					var user_defaults = netmanthan.defaults.get_user_defaults(df.options);
				...
				if (!user_default) {
					user_default = netmanthan.defaults.get_user_default(df.fieldname);
				}
				...
				if (
					...
					user_default = netmanthan.boot.user.last_selected_values[df.options];
				}
		...
		if (!value || df["default"]) {
			...
			if (default_val == "__user" || default_val.toLowerCase() == "user") {
				value = netmanthan.session.user;
			} else if (default_val == "user_fullname") {
				value = netmanthan.session.user_fullname;
			} else if (default_val == "Today") {
				value = netmanthan.datetime.get_today();
			} else if ((default_val || "").toLowerCase() === "now") {
				if (df.fieldtype == "Time") {
					value = netmanthan.datetime.now_time();
				} else {
					// datetime fields are stored in system tz
					value = netmanthan.datetime.system_datetime();
				}
			} else if (default_val[0] === ":") {
				var boot_doc = netmanthan.model.get_default_from_boot_docs(df, doc, parent_doc);
	...
	get_default_from_boot_docs: function (df, doc, parent_doc) {
		...
		if (netmanthan.get_list(df["default"]).length > 0) {
			var ref_fieldname = df["default"].slice(1).toLowerCase().replace(" ", "_");
			var ref_value = parent_doc
				? parent_doc[ref_fieldname]
				: netmanthan.defaults.get_user_default(ref_fieldname);
			var ref_doc = ref_value ? netmanthan.get_doc(df["default"], ref_value) : null;
	...
	add_child: function (parent_doc, doctype, parentfield, idx) {
		...
		if (arguments.length === 2) {
			parentfield = doctype;
			doctype = netmanthan.meta.get_field(parent_doc.doctype, parentfield).options;
		}

		// create row doc
		idx = idx ? idx - 0.1 : (parent_doc[parentfield] || []).length + 1;

		var child = netmanthan.model.get_new_doc(doctype, parent_doc, parentfield);
		child.idx = idx;
	...
	copy_doc: function (doc, from_amend, parent_doc, parentfield) {
		...
		var newdoc = netmanthan.model.get_new_doc(doc.doctype, parent_doc, parentfield);

		for (var key in doc) {
			// dont copy name and blank fields
			var df = netmanthan.meta.get_docfield(doc.doctype, key);
			...
			if (
				...
				if (netmanthan.model.table_fields.includes(df.fieldtype)) {
					for (var i = 0, j = value.length; i < j; i++) {
						var d = value[i];
						netmanthan.model.copy_doc(d, from_amend, newdoc, df.fieldname);
					}
				} else {
					newdoc[key] = doc[key];
				}
		...
		var user = netmanthan.session.user;
	...
	open_mapped_doc: function (opts) {
		...
		return netmanthan.call({
			type: "POST",
			method: "netmanthan.model.mapper.make_mapped_doc",
			args: {
				method: opts.method,
			...
			callback: function (r) {
				if (!r.exc) {
					...
					netmanthan.set_route("Form", r.message.doctype, r.message.name);
				}
			},
		});
	},
...
netmanthan.create_routes = {};
netmanthan.new_doc = function (doctype, opts, init_callback) {
	if (doctype === "File") {
	...
	return new Promise((resolve) => {
		if (opts && $.isPlainObject(opts)) {
			netmanthan.route_options = opts;
		}
		netmanthan.model.with_doctype(doctype, function () {
			if (netmanthan.create_routes[doctype]) {
				netmanthan.set_route(netmanthan.create_routes[doctype]).then(() => resolve());
			} else {
				netmanthan.ui.form
