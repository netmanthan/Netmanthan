...
netmanthan.provide("netmanthan.dom");
...
netmanthan.dom = {
	...
	get_unique_id: function () {
		const id = "unique-" + netmanthan.dom.id_count;
		netmanthan.dom.id_count++;
		return id;
	},
	set_unique_id: function (ele) {
		...
		var id = "unique-" + netmanthan.dom.id_count;
		$ele.attr("id", id);
		netmanthan.dom.id_count++;
		return id;
	},
	...
	add: function (parent, newtag, className, cs, innerHTML, onclick) {
		if (parent && parent.substr) parent = netmanthan.dom.by_id(parent);
	...
	freeze: function (msg, css_class) {
		...
		netmanthan.dom.freeze_count++;
	},
	unfreeze: function () {
		if (!netmanthan.dom.freeze_count) return; // anything open?
		netmanthan.dom.freeze_count--;
		if (!netmanthan.dom.freeze_count) {
	...
	scroll_to_section(section_name) {
		setTimeout(() => {
			...
			if (section.length) {
				...
				netmanthan.ui.scroll(section.parent().parent());
			}
		}, 200);
	},
...
netmanthan.ellipsis = function (text, max) {
	if (!max) max = 20;
	text = cstr(text);
	if (text.length > max) {
...
netmanthan.run_serially = function (tasks) {
...
netmanthan.load_image = (src, onload, onerror, preprocess = () => {}) => {
...
netmanthan.timeout = (seconds) => {
	return new Promise((resolve) => {
		setTimeout(() => resolve(), seconds * 1000);
	});
};

netmanthan.scrub = function (text, spacer = "_") {
	return text.replace(/ /g, spacer).toLowerCase();
};

netmanthan.unscrub = function (txt) {
	return netmanthan.model.unscrub(txt);
};

netmanthan.get_data_pill = (label, target_id = null, remove_action = null, image = null) => {
	...
	if (remove_action) {
		let remove_btn = $(`
			<span class="remove-btn cursor-pointer">
				${netmanthan.utils.icon("close", "sm")}
			</span>
		`).click(() => {
...
netmanthan.get_modal = function (title, content) {
				...
				<div class="modal-header">
					...
					<div class="modal-actions">
						<button class="btn btn-modal-minimize btn-link hide">
							${netmanthan.utils.icon("collapse")}
						</button>
						<button class="btn btn-modal-close btn-link" data-dismiss="modal">
							${netmanthan.utils.icon("close-alt", "sm", "close-alt")}
						</button>
					</div>
				</div>
...
netmanthan.is_online = function () {
	if (netmanthan.boot.developer_mode == 1) {
...
netmanthan.create_shadow_element = function (wrapper, html, css, js) {
	let random_id = "custom-block-" + netmanthan.utils.get_random(5).toLowerCase();
	...
	class CustomBlock extends HTMLElement {
		constructor() {
			...
			div.innerHTML = netmanthan.dom.remove_script_and_style(html);

			// link global desk css
			let link = document.createElement("link");
			link.rel = "stylesheet";
			link.href = netmanthan.assets.bundled_asset("desk.bundle.css");
