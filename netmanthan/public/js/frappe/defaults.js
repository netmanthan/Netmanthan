...
netmanthan.defaults = {
	get_user_default: function (key) {
		let defaults = netmanthan.boot.user.defaults;
		let d = defaults[key];
		if (!d && netmanthan.defaults.is_a_user_permission_key(key)) {
			d = defaults[netmanthan.model.scrub(key)];
		...
		if (!netmanthan.defaults.in_user_permission(key, d)) {
			return;
		}
	...
	get_user_defaults: function (key) {
		var defaults = netmanthan.boot.user.defaults;
		var d = defaults[key];

		if (netmanthan.defaults.is_a_user_permission_key(key)) {
			if (d && $.isArray(d) && d.length === 1) {
				// Use User Permission value when only when it has a single value
				d = d[0];
			} else {
				d = defaults[key] || defaults[netmanthan.model.scrub(key)];
			}
		}
		if (!$.isArray(d)) d = [d];
		...
		d.filter((item) => {
			if (netmanthan.defaults.in_user_permission(key, item)) {
				return item;
			}
		});
		return d;
	...
	get_global_default: function (key) {
		var d = netmanthan.sys_defaults[key];
		if ($.isArray(d)) d = d[0];
		return d;
	},
	...
	set_user_default_local: function (key, value) {
		netmanthan.boot.user.defaults[key] = value;
	},
	get_default: function (key) {
		...
		if (netmanthan.defaults.is_a_user_permission_key(key)) {
			if (value && $.isArray(value) && value.length === 1) {
				...
				value = defaults[netmanthan.model.scrub(key)];
			}
		}

		if (!netmanthan.defaults.in_user_permission(key, value)) {
			return;
		}
	...
	is_a_user_permission_key: function (key) {
		return key.indexOf(":") === -1 && key !== netmanthan.model.scrub(key);
	},

	in_user_permission: function (key, value) {
		let user_permission = this.get_user_permissions()[netmanthan.model.unscrub(key)];
	...
	update_user_permissions: function () {
		const method = "netmanthan.core.doctype.user_permission.user_permission.get_user_permissions";
		netmanthan.call(method).then((r) => {
			if (r.message) {
	...
	load_user_permission_from_boot: function () {
		if (netmanthan.boot.user.user_permissions) {
			this._user_permissions = Object.assign({}, netmanthan.boot.user.user_permissions);
		} else {
