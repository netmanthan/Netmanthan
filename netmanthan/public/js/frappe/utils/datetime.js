...
netmanthan.provide("netmanthan.datetime");

netmanthan.defaultDateFormat = "YYYY-MM-DD";
netmanthan.defaultTimeFormat = "HH:mm:ss";
netmanthan.defaultDatetimeFormat = netmanthan.defaultDateFormat + " " + netmanthan.defaultTimeFormat;
moment.defaultFormat = netmanthan.defaultDateFormat;

netmanthan.provide("netmanthan.datetime");

$.extend(netmanthan.datetime, {
	convert_to_user_tz: function (date, format) {
		...
		let date_obj = null;
		if (netmanthan.boot.time_zone && netmanthan.boot.time_zone.system && netmanthan.boot.time_zone.user) {
			date_obj = moment
				.tz(date, netmanthan.boot.time_zone.system)
				.clone()
				.tz(netmanthan.boot.time_zone.user);
		} else {
			date_obj = moment(date);
		}

		return format === false ? date_obj : date_obj.format(netmanthan.defaultDatetimeFormat);
	},
	...
	convert_to_system_tz: function (date, format) {
		...
		if (netmanthan.boot.time_zone && netmanthan.boot.time_zone.system && netmanthan.boot.time_zone.user) {
			date_obj = moment
				.tz(date, netmanthan.boot.time_zone.user)
				.clone()
				.tz(netmanthan.boot.time_zone.system);
		} else {
			date_obj = moment(date);
		}
	...
	is_system_time_zone: function () {
		if (netmanthan.boot.time_zone && netmanthan.boot.time_zone.system && netmanthan.boot.time_zone.user) {
			return (
				moment().tz(netmanthan.boot.time_zone.system).utcOffset() ===
				moment().tz(netmanthan.boot.time_zone.user).utcOffset()
			);
		}
	...
	is_timezone_same: function () {
		return netmanthan.datetime.is_system_time_zone();
	},

	str_to_obj: function (d) {
		return moment(d, netmanthan.defaultDatetimeFormat)._d;
	},
	...
	obj_to_user: function (d) {
		return moment(d).format(netmanthan.datetime.get_user_date_fmt().toUpperCase());
	},
	...
	get_user_time_fmt: function () {
		return (netmanthan.sys_defaults && netmanthan.sys_defaults.time_format) || "HH:mm:ss";
	},

	get_user_date_fmt: function () {
		return (netmanthan.sys_defaults && netmanthan.sys_defaults.date_format) || "yyyy-mm-dd";
	},
	...
	str_to_user: function (val, only_time = false, only_date = false) {
		if (!val) return "";
		const user_date_fmt = netmanthan.datetime.get_user_date_fmt().toUpperCase();
		const user_time_fmt = netmanthan.datetime.get_user_time_fmt();
		let user_format = user_time_fmt;

		if (only_time) {
			let date_obj = moment(val, netmanthan.defaultTimeFormat);
			return date_obj.format(user_format);
		} else if (only_date) {
			let date_obj = moment(val, netmanthan.defaultDateFormat);
			return date_obj.format(user_date_fmt);
		} else {
			let date_obj = moment.tz(val, netmanthan.boot.time_zone.system);
			if (typeof val !== "string" || val.indexOf(" ") === -1) {
				user_format = user_date_fmt;
			} else {
				user_format = user_date_fmt + " " + user_time_fmt;
			}
			return date_obj.clone().tz(netmanthan.boot.time_zone.user).format(user_format);
		}
	},
	...
	user_to_str: function (val, only_time = false) {
		var user_time_fmt = netmanthan.datetime.get_user_time_fmt();
		if (only_time) {
			return moment(val, user_time_fmt).format(netmanthan.defaultTimeFormat);
		}

		var user_fmt = netmanthan.datetime.get_user_date_fmt().toUpperCase();
	...
	user_to_obj: function (d) {
		return netmanthan.datetime.str_to_obj(netmanthan.datetime.user_to_str(d));
	},
	...
	now_date: function (as_obj = false) {
		return netmanthan.datetime._date(netmanthan.defaultDateFormat, as_obj);
	},

	now_time: function (as_obj = false) {
		return netmanthan.datetime._date(netmanthan.defaultTimeFormat, as_obj);
	},

	now_datetime: function (as_obj = false) {
		return netmanthan.datetime._date(netmanthan.defaultDatetimeFormat, as_obj);
	},

	system_datetime: function (as_obj = false) {
		return netmanthan.datetime._date(netmanthan.defaultDatetimeFormat, as_obj, true);
	},

	_date: function (format, as_obj = false, system_time = false) {
		let time_zone = netmanthan.boot.time_zone?.system || netmanthan.sys_defaults.time_zone;
		...
		if (!system_time) {
			time_zone = netmanthan.boot.time_zone?.user || time_zone;
		}
		let date = moment.tz(time_zone);

		return as_obj ? netmanthan.datetime.moment_to_date_obj(date) : date.format(format);
	},
	...
	nowdate: function () {
		return netmanthan.datetime.now_date();
	},
	...
	validate: function (d) {
		return moment(
			d,
			[netmanthan.defaultDateFormat, netmanthan.defaultDatetimeFormat, netmanthan.defaultTimeFormat],
			true
		).isValid();
	},

	get_first_day_of_the_week_index() {
		const first_day_of_the_week = netmanthan.sys_defaults.first_day_of_the_week || "Sunday";
...
Object.defineProperties(window, {
	dateutil: {
		get: function () {
			console.warn(
				"Please use `netmanthan.datetime` instead of `dateutil`. It will be deprecated soon."
			);
			return netmanthan.datetime;
		},
		configurable: true,
	},
	date: {
		get: function () {
			console.warn(
				"Please use `netmanthan.datetime` instead of `date`. It will be deprecated soon."
			);
			return netmanthan.datetime;
		},
		configurable: true,
	...
	get_today: {
		get: function () {
			console.warn(
				"Please use `netmanthan.datetime.get_today` instead of `get_today`. It will be deprecated soon."
			);
			return netmanthan.datetime.get_today;
		},
		configurable: true,
	},
});
