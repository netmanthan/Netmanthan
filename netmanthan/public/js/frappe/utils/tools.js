...
netmanthan.provide("netmanthan.tools");

netmanthan.tools.downloadify = function (data, roles, title) {
	if (roles && roles.length && !has_common(roles, roles)) {
		netmanthan.msgprint(
			__("Export not allowed. You need {0} role to export.", [netmanthan.utils.comma_or(roles)])
		);
		return;
	}

	var filename = title + ".csv";
	var csv_data = netmanthan.tools.to_csv(data);
...
netmanthan.markdown = function (txt) {
	if (!netmanthan.md2html) {
		netmanthan.md2html = new showdown.Converter();
	}
	...
	return netmanthan.md2html.makeHtml(txt);
};

netmanthan.tools.to_csv = function (data) {
	var res = [];
