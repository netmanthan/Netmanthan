...
netmanthan.avatar = function (
	...
	if (user) {
		// desk
		user_info = netmanthan.user_info(user);
	} else {
		// website
		let full_name = title || netmanthan.get_cookie("full_name");
		user_info = {
			image: image_url === null ? netmanthan.get_cookie("user_image") : image_url,
			fullname: full_name,
			abbr: netmanthan.get_abbr(full_name),
			color: netmanthan.get_palette(full_name),
		};
	}
...
netmanthan.get_avatar = function (css_class, title, image_url = null, remove_color, data_attributes) {
	if (!css_class) {
	...
	if (image_url) {
		...
		let abbr = netmanthan.get_abbr(title);
		let style = "";
		if (!remove_color) {
			let color = netmanthan.get_palette(title);
...
netmanthan.avatar_group = function (users, limit = 4, options = {}) {
	...
	let html = display_users
		.map((user) =>
			netmanthan.avatar(user, "avatar-small " + css_class, null, null, false, options.filterable)
		)
		.join("");
	if (extra_users.length === 1) {
		html += netmanthan.avatar(
			extra_users[0],
		...
		html = `
			...
			<span class="avatar avatar-small ${css_class}">
				<div class="avatar-frame standard-image avatar-extra-count"
					title="${extra_users.map((u) => netmanthan.user_info(u).fullname).join(", ")}">
					+${extra_users.length}
				</div>
			</span>
		`;
		...
		avatar_action_html = `
			<span class="avatar avatar-small">
				<div class="avatar-frame avatar-action">
					${netmanthan.utils.icon(options.action_icon, "sm")}
				</div>
			</span>
		`;
	}
...
netmanthan.ui.scroll = function (element, animate, additional_offset) {
...
netmanthan.get_palette = function (txt) {
	if (!txt) return netmanthan.palette[8]; // breaks when undefined
	var idx = cint((parseInt(md5(txt).substr(4, 2), 16) + 1) / 5.33);
	return netmanthan.palette[idx % 8];
};

netmanthan.get_abbr = function (txt, max_length) {
	if (!txt) return "";
	var abbr = "";
...
netmanthan.gravatars = {};
netmanthan.get_gravatar = function (email_id, size = 0) {
	var param = size ? "s=" + size : "d=retro";
	if (!netmanthan.gravatars[email_id]) {
		// TODO: check if gravatar exists
		netmanthan.gravatars[email_id] =
			"https://secure.gravatar.com/avatar/" + md5(email_id) + "?" + param;
	}
	return netmanthan.gravatars[email_id];
};
...
netmanthan.get_cookie = function getCookie(name) {
	return netmanthan.get_cookies()[name];
};

netmanthan.get_cookies = function getCookies() {
...
netmanthan.is_mobile = function () {
	return $(document).width() < 768;
};

netmanthan.utils.xss_sanitise = function (string, options) {
...
netmanthan.utils.sanitise_redirect = (url) => {
	const is_external = (() => {
		return (url) => {
			function domain(url) {
	...
	url = netmanthan.utils.strip_url(url);

	return is_external(url)
		? ""
		: sanitise_javascript(netmanthan.utils.xss_sanitise(url, { strategies: ["js"] }));
};

netmanthan.utils.strip_url = (url) => {
...
netmanthan.utils.new_auto_repeat_prompt = function (frm) {
	const fields = [
		{
			fieldname: "frequency",
			fieldtype: "Select",
		...
		},
		{
			...
			label: __("Start Date"),
			reqd: 1,
			default: netmanthan.datetime.nowdate(),
		},
		{
			fieldname: "end_date",
			fieldtype: "Date",
	...
	netmanthan.prompt(
		...
		function (values) {
			netmanthan.call({
				method: "netmanthan.automation.doctype.auto_repeat.auto_repeat.make_auto_repeat",
				args: {
...
netmanthan.utils.get_page_view_count = function (route) {
	return netmanthan.call("netmanthan.website.doctype.web_page_view.web_page_view.get_page_view_count", {
		path: route,
	});
};
