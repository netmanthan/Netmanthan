function prettyDate(date, mini) {
	...
	if (typeof date == "string") {
		date = netmanthan.datetime.convert_to_user_tz(date);
		date = new Date(
			(date || "")
				.replace(/-/g, "/")
				.replace(/[TZ]/g, " ")
	...
	let diff = (new Date(netmanthan.datetime.now_datetime()).getTime() - date.getTime()) / 1000;
...
window.comment_when = function (datetime, mini) {
	var timestamp = netmanthan.datetime.str_to_user ? netmanthan.datetime.str_to_user(datetime) : datetime;
	return (
		'<span class="netmanthan-timestamp ' +
		(mini ? " mini" : "") +
		'" data-timestamp="' +
		datetime +
		'" title="' +
...
netmanthan.datetime.comment_when = comment_when;
netmanthan.datetime.prettyDate = prettyDate;

netmanthan.datetime.refresh_when = function () {
	if (jQuery) {
		$(".netmanthan-timestamp").each(function () {
