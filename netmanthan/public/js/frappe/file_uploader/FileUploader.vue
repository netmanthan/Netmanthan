				...
				<div class="mt-2 text-center">
					...
					<button v-if="google_drive_settings.enabled" class="btn btn-file-upload" @click="show_google_drive_picker">
						<svg width="30" height="30">
							<image href="/assets/netmanthan/icons/social/google_drive.svg" width="30" height="30"/>
						</svg>
...
export default {
	...
	created() {
		...
		if (netmanthan.user_id !== "Guest") {
			netmanthan.call({
				// method only available after login
				method: "netmanthan.integrations.doctype.google_settings.google_settings.get_file_picker_settings",
				callback: (resp) => {
					if (!resp.exc) {
		...
		if (this.restrictions.max_file_size == null) {
			netmanthan.call('netmanthan.core.api.file.get_max_file_size')
				.then(res => {
					this.restrictions.max_file_size = Number(res.message);
				});
		}
		if (this.restrictions.max_number_of_files == null && this.doctype) {
			this.restrictions.max_number_of_files = netmanthan.get_meta(this.doctype)?.max_attachments;
		}
	},
	watch: {
	...
	methods: {
		...
		upload_via_file_browser() {
			...
			if (!selected_file.value) {
				netmanthan.msgprint(__('Click on a file to select it.'));
		...
		upload_via_web_link() {
			...
			if (!file_url) {
				netmanthan.msgprint(__('Invalid URL'));
				this.close_dialog = true;
				return Promise.reject();
			}
		...
		return_as_dataurl() {
			let promises = this.files.map(file =>
				netmanthan.dom.file_to_base64(file.file_obj)
					.then(dataurl => {
				...
				xhr.onreadystatechange = () => {
					if (xhr.readyState == XMLHttpRequest.DONE) {
						if (xhr.status === 200) {
							...
							netmanthan.request.cleanup({}, error);
						}
					}
				}
		...
		capture_image() {
			const capture = new netmanthan.ui.Capture({
				animate: false,
				error: true
			});
			capture.show();
			capture.submit(data_urls => {
				data_urls.forEach(data_url => {
					let filename = `capture_${netmanthan.datetime.now_datetime().replaceAll(/[: -]/g, '_')}.png`;
