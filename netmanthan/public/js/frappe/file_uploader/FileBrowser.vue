...
export default {
	...
	methods: {
		...
		get_files_in_folder(folder, start) {
			return netmanthan
				.call("netmanthan.core.api.file.get_files_in_folder", {
					folder,
					start,
		...
		search_by_name: netmanthan.utils.debounce(function() {
			if (this.search_text === "") {
			...
			netmanthan
				.call(
					"netmanthan.core.api.file.get_files_by_search_text",
					{
		...
		make_file_node(file) {
			...
			let label = netmanthan.utils.file_name_ellipsis(filename, 40);
			return {
				label: label,
				filename: filename,
