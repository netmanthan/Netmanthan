...
netmanthan.provide("netmanthan.model.docinfo");
...
netmanthan.ui.form.Controller = class FormController {
	constructor(opts) {
		$.extend(this, opts);
	}
};

netmanthan.ui.form.Form = class netmanthanForm {
	constructor(doctype, parent, in_form, doctype_layout_name) {
		...
		this.sections = [];
		this.grids = [];
		this.cscript = new netmanthan.ui.form.Controller({ frm: this });
		this.events = {};
		this.fetch_dict = {};
		this.parent = parent;
		this.doctype_layout = netmanthan.get_doc("DocType Layout", doctype_layout_name);
		this.undo_manager = new UndoManager({ frm: this });
		this.setup_meta(doctype);
		this.debounced_reload_doc = netmanthan.utils.debounce(this.reload_doc.bind(this), 1000);
	...
	setup_meta() {
		this.meta = netmanthan.get_doc("DocType", this.doctype);

		if (this.meta.istable) {
			this.meta.in_dialog = 1;
		}

		this.perm = netmanthan.perm.get_perm(this.doctype); // for create
	...
	setup() {
		this.fields = [];
		this.fields_dict = {};
		this.state_fieldname = netmanthan.workflow.get_state_fieldname(this.doctype);
		...
		this.toolbar = new netmanthan.ui.form.Toolbar({
			frm: this,
			page: this.page,
		});
		...
		this.script_manager = new netmanthan.ui.form.ScriptManager({
			frm: this,
		});
		this.script_manager.setup();
		this.watch_model_updates();

		if (!this.meta.hide_toolbar && netmanthan.boot.desk_settings.timeline) {
			// this.footer_tab = new netmanthan.ui.form.Tab(this.layout, {
			// 	label: __("Activity"),
			// 	fieldname: 'timeline'
			// });

			this.footer = new netmanthan.ui.form.Footer({
				frm: this,
	...
	setup_std_layout() {
		...
		// layout
		this.layout = new netmanthan.ui.form.Layout({
			parent: this.body,
		...
		this.dashboard = new netmanthan.ui.form.Dashboard(dashboard_parent, this);

		this.tour = new netmanthan.ui.form.FormTour({
			frm: this,
		});

		// workflow state
		this.states = new netmanthan.ui.form.States({
			frm: this,
		});
	}
	...
	watch_model_updates() {
		...
		// on table fields
		var table_fields = netmanthan.get_children("DocType", me.doctype, "fields", {
			fieldtype: ["in", netmanthan.model.table_fields],
		});

		// using $.each to preserve df via closure
		$.each(table_fields, function (i, df) {
			netmanthan.model.on(df.options, "*", function (fieldname, value, doc) {
	...
	setup_file_drop() {
		...
		this.$wrapper.on("dragenter dragover", false).on("drop", function (e) {
			...
			if (me.doc.__islocal) {
				netmanthan.msgprint(__("Please save before attaching."));
				throw "attach error";
			}

			new netmanthan.ui.FileUploader({
				doctype: me.doctype,
				docname: me.docname,
				frm: me,
		...
		this.fields.map((field) => {
			if (field.df.fieldtype === "Markdown Editor") {
				this.set_df_property(field.df.fieldname, "autocompletions", () => {
					...
					return attachments
						.filter((file) => netmanthan.utils.is_image_file(file.file_url))
						.map((file) => {
							return {
	...
	refresh(docname) {
		...
		if (this.docname) {
			...
			// set the doc
			this.doc = netmanthan.get_doc(this.doctype, this.docname);

			// check permissions
			this.fetch_permissions();
			if (!this.has_read_permission()) {
				netmanthan.show_not_permitted(__(this.doctype) + " " + __(cstr(this.docname)));
				return;
			}
			...
			// read only (workflow)
			this.read_only = netmanthan.workflow.is_read_only(this.doctype, this.docname);
			if (this.read_only) {
				this.set_read_only(true);
				netmanthan.show_alert(__("This form is not editable due to a Workflow."));
			}
	...
	setup_doctype_actions() {
		if (this.meta.actions) {
			for (let action of this.meta.actions) {
				netmanthan.ui.form.on(this.doctype, "refresh", () => {
					if (!this.is_new()) {
						if (!action.hidden) {
	...
	execute_action(action) {
		if (typeof action === "string") {
			...
			if (typeof action === "string") {
				netmanthan.throw(`Action ${action} not found`);
			}
		}
		if (action.action_type === "Server Action") {
			return netmanthan.xcall(action.action, { doc: this.doc }).then((doc) => {
				if (doc.doctype) {
					// document is returned by the method,
					// apply the changes locally and refresh
					netmanthan.model.sync(doc);
					this.refresh();
				}
			...
			return netmanthan.set_route(action.action);
		}
	}
	...
	initialize_new_doc() {
		...
		this.script_manager.trigger("before_load", this.doctype, this.docname).then(() => {
			...
			netmanthan.after_ajax(function () {
				me.trigger_link_fields();
			});

			netmanthan.breadcrumbs.add(me.meta.module, me.doctype);
		});
	...
	render_form(switched) {
		if (!this.meta.istable) {
			...
			if (netmanthan.boot.desk_settings.form_sidebar) {
				this.sidebar = new netmanthan.ui.form.Sidebar({
					frm: this,
					page: this.page,
				});
				this.sidebar.make();
	...
	run_after_load_hook() {
		if (netmanthan.route_hooks.after_load) {
			let route_callback = netmanthan.route_hooks.after_load;
			delete netmanthan.route_hooks.after_load;
	...
	refresh_header(switched) {
		...
		if (!this.meta.in_dialog || this.in_form) {
			netmanthan.utils.set_title(this.meta.issingle ? this.doctype : this.docname);
		}
	...
	validate_and_save(save_action, callback, btn, on_error, resolve, reject) {
		...
		var after_save = function (r) {
			...
			if (!r.exc) {
				if (["Save", "Update", "Amend"].indexOf(save_action) !== -1) {
					netmanthan.utils.play_sound("click");
				}

				me.script_manager.trigger("after_save");

				if (netmanthan.route_hooks.after_save) {
					let route_callback = netmanthan.route_hooks.after_save;
					delete netmanthan.route_hooks.after_save;
		...
		if (save_action != "Update") {
			// validate
			netmanthan.validated = true;
			netmanthan
				.run_serially([
					() => this.script_manager.trigger("validate"),
					() => this.script_manager.trigger("before_save"),
					() => {
						if (!netmanthan.validated) {
							fail();
							return;
						}

						netmanthan.ui.form.save(me, save_action, after_save, btn);
					},
				])
				.catch(fail);
		} else {
						...
						me.save(
							...
							function (r) {
								if (r.exc) {
									...
									netmanthan.utils.play_sound("submit");
									callback && callback();
									me.script_manager
										.trigger("on_submit")
										.then(() => resolve(me))
										.then(() => {
											if (netmanthan.route_hooks.after_submit) {
												let route_callback =
													netmanthan.route_hooks.after_submit;
												delete netmanthan.route_hooks.after_submit;
												route_callback(me);
											}
										});
								}
	...
	savecancel(btn, callback, on_error) {
		...
		netmanthan
			.call({
				method: "netmanthan.desk.form.linked_with.get_submitted_linked_docs",
				args: {
	...
	_cancel_all(r, btn, callback, on_error) {
		...
		for (let doctype of doctypes) {
			if (!me.ignore_doctypes_on_cancel_all.includes(doctype)) {
				let docnames = links
					...
					.map((link) => netmanthan.utils.get_form_link(link.doctype, link.name, true))
					.join(", ");
		...
		let can_cancel = links.every((link) => netmanthan.model.can_cancel(link.doctype));
		if (can_cancel) {
		...
		let d = new netmanthan.ui.Dialog(
			{
				...
				fields: [
					{
						fieldtype: "HTML",
						options: `<p class="netmanthan-confirm-message">${confirm_message}</p>`,
					},
				],
			},
			() => me.handle_save_fail(btn, on_error)
		...
		if (can_cancel) {
			d.set_primary_action(__("Cancel All"), () => {
				d.hide();
				netmanthan.call({
					method: "netmanthan.desk.form.linked_with.cancel_all_linked_docs",
					args: {
						docs: links,
			...
			me.script_manager.trigger("before_cancel").then(() => {
				...
				var after_cancel = function (r) {
					if (r.exc) {
						...
						netmanthan.utils.play_sound("cancel");
						me.refresh();
						callback && callback();
						me.script_manager.trigger("after_cancel");
					}
				};
				netmanthan.ui.form.save(me, "cancel", after_cancel, btn);
			});
		};
	...
	savetrash() {
		...
		netmanthan.model.delete_doc(this.doctype, this.docname, function () {
			window.history.back();
		});
	}
	...
	amend_doc() {
		if (!this.fields_dict["amended_from"]) {
			netmanthan.msgprint(__('"amended_from" field must be present to do an amendment.'));
			return;
		}

		netmanthan
			.xcall("netmanthan.client.is_document_amended", {
			...
			.then((is_amended) => {
				...
				var fn = function (newdoc) {
					...
					if (me.fields_dict && me.fields_dict["amendment_date"])
						newdoc.amendment_date = netmanthan.datetime.obj_to_str(new Date());
				};
				this.copy_doc(fn, 1);
				netmanthan.utils.play_sound("click");
			});
	...
	validate_form_action(action, resolve) {
		...
		var perms = netmanthan.perm.get_perm(this.doc.doctype)[0];
		...
		if (
			(netmanthan.workflow.is_read_only(this.doctype, this.docname) &&
	...
	show_submit_message() {
		if (
			...
			!this.is_dirty() &&
			!this.is_new() &&
			!netmanthan.model.has_workflow(this.doctype) && // show only if no workflow
	...
	fetch_permissions() {
		...
		this.perm = netmanthan.perm.get_perm(dt, this.doc);
	}
	...
	check_doctype_conflict(docname) {
		if (this.doctype == "DocType" && docname == "DocType") {
			netmanthan.msgprint(__("Allowing DocType, DocType. Be careful!"));
		} else if (this.doctype == "DocType") {
			if (netmanthan.views.formview[docname] || netmanthan.pages["List/" + docname]) {
				window.location.reload();
				//	netmanthan.msgprint(__("Cannot open {0} when its instance is open", ['DocType']))
				// throw 'doctype open conflict'
			}
		} else {
			if (
				netmanthan.views.formview.DocType &&
				netmanthan.views.formview.DocType.frm.opendocs[this.doctype]
			) {
				window.location.reload();
				//	netmanthan.msgprint(__("Cannot open instance when its {0} is open", ['DocType']))
				// throw 'doctype open conflict'
			}
		}
	}
	...
	rename_notify(dt, old, name) {
		...
		// cleanup
		if (this && this.opendocs[old] && netmanthan.meta.docfield_copy[dt]) {
			// delete docfield copy
			netmanthan.meta.docfield_copy[dt][name] = netmanthan.meta.docfield_copy[dt][old];
			delete netmanthan.meta.docfield_copy[dt][old];
		}
		...
		netmanthan.re_route[netmanthan.router.get_sub_path()] = `${encodeURIComponent(
			netmanthan.router.slug(this.doctype)
		)}/${encodeURIComponent(name)}`;
		!netmanthan._from_link && netmanthan.set_route("Form", this.doctype, name);
	}
	...
	print_doc() {
		...
		netmanthan.set_route("print", this.doctype, this.doc.name);
	}

	navigate_records(prev) {
		let filters, sort_field, sort_order;
		let list_view = netmanthan.get_list_view(this.doctype);
		if (list_view) {
			filters = list_view.get_filters_for_args();
			sort_field = list_view.sort_by;
			sort_order = list_view.sort_order;
		} else {
			let list_settings = netmanthan.get_user_settings(this.doctype)["List"];
			if (list_settings) {
		...
		netmanthan.call("netmanthan.desk.form.utils.get_next", args).then((r) => {
			if (r.message) {
				netmanthan.set_route("Form", this.doctype, r.message);
	...
	rename_doc() {
		netmanthan.model.rename_doc(this.doctype, this.docname, () => this.refresh_header());
	}
	...
	email_doc(message) {
		new netmanthan.views.CommunicationComposer({
			doc: this.doc,
			frm: this,
	...
	copy_doc(onload, from_amend) {
		...
		var newdoc = netmanthan.model.copy_doc(this.doc, from_amend);
	...
	reload_doc() {
		...
		if (!this.doc.__islocal) {
			netmanthan.model.remove_from_locals(this.doctype, this.docname);
			return netmanthan.model.with_doc(this.doctype, this.docname, () => {
				this.refresh();
			});
		}
	}
	...
	has_perm(ptype) {
		return netmanthan.perm.has_perm(this.doctype, 0, ptype, this.doc);
	}
	...
	get_docinfo() {
		return netmanthan.model.docinfo[this.doctype][this.docname];
	}
	...
	set_footnote(txt) {
		this.footnote_area = netmanthan.utils.set_footnote(this.footnote_area, this.body, txt);
	}
	...
	scroll_to_element() {
		if (netmanthan.route_options && netmanthan.route_options.scroll_to) {
			var scroll_to = netmanthan.route_options.scroll_to;
			delete netmanthan.route_options.scroll_to;
			...
			if (selector.length) {
				netmanthan.utils.scroll_to(selector);
			}
		} else if (window.location.hash) {
			if ($(window.location.hash).length) {
				netmanthan.utils.scroll_to(window.location.hash, true, 200, null, null, true);
			} else {
	...
	show_success_action() {
		...
		const success_action = new netmanthan.ui.form.SuccessAction(this);
		success_action.show();
	}
	...
	set_currency_labels(fields_list, currency, parentfield) {
		...
		$.each(fields_list, function (i, fname) {
			var docfield = netmanthan.meta.docfield_map[doctype][fname];
			if (docfield) {
	...
	field_map(fnames, fn) {
		...
		for (var i = 0, l = fnames.length; i < l; i++) {
			...
			var field = netmanthan.meta.get_docfield(this.doctype, fieldname, this.docname);
			if (field) {
				fn(field);
	...
	get_docfield(fieldname1, fieldname2) {
		if (fieldname2) {
			...
			return netmanthan.meta.get_docfield(doctype, fieldname2, this.docname);
		} else {
			// for parent
			return netmanthan.meta.get_docfield(this.doctype, fieldname1, this.docname);
		}
	}
	...
	set_df_property(fieldname, property, value, docname, table_field, table_row_name = null) {
		...
		if (!docname || !table_field) {
			...
			const filtered_fields = netmanthan.utils.filter_dict(grid.docfields, {
				fieldname: table_field,
			});
			if (filtered_fields.length) {
				df = netmanthan.meta.get_docfield(
	...
	get_files() {
		return this.attachments
			? netmanthan.utils.sort(this.attachments.get_attachments(), "file_name", "string")
			: [];
	}
	...
	clear_table(fieldname) {
		netmanthan.model.clear_table(this.doc, fieldname);
	}

	add_child(fieldname, values) {
		var doc = netmanthan.model.add_child(
			this.doc,
			netmanthan.meta.get_docfield(this.doctype, fieldname).options,
			fieldname
		);
		if (values) {
	...
	set_value(field, value, if_missing, skip_dirty_trigger = false) {
		...
		var _set = function (f, v) {
			...
			if (fieldobj) {
				if (!if_missing || !netmanthan.model.has_value(me.doctype, me.doc.name, f)) {
					if (
						netmanthan.model.table_fields.includes(fieldobj.df.fieldtype) &&
						$.isArray(v)
					) {
						// set entire child table from specified array as value
						netmanthan.model.clear_table(me.doc, fieldobj.df.fieldname);

						const standard_fields = [
							...netmanthan.model.std_fields_list,
							...netmanthan.model.child_table_field_list,
						];
						v.forEach((d, idx) => {
							let child = netmanthan.model.add_child(
								me.doc,
								fieldobj.df.options,
						...
						return netmanthan.model.set_value(
							me.doctype,
							me.doc.name,
							f,
							v,
				...
				netmanthan.msgprint(__("Field {0} not found.", [f]));
				throw "frm.set_value";
			}
		};
		...
		if (typeof field == "string") {
			...
			return netmanthan.run_serially(tasks);
		}
	}
	...
	call(opts, args, callback) {
		...
		if (!opts.doc) {
			if (opts.method.indexOf(".") === -1)
				opts.method = netmanthan.model.get_server_module_name(me.doctype) + "." + opts.method;
				...
				if ($.isPlainObject(r.message)) {
					if (opts.child) {
						...
						var std_field_list = ["doctype"]
							.concat(netmanthan.model.std_fields_list)
							.concat(netmanthan.model.child_table_field_list);
	...
	set_read_only() {
		const docperms = netmanthan.perm.get_perm(this.doc.doctype);
	...
	get_formatted(fieldname) {
		return netmanthan.format(
			this.doc[fieldname],
			netmanthan.meta.get_docfield(this.doctype, fieldname, this.docname),
			{ no_icon: true },
			this.doc
		);
	}
	...
	open_grid_row() {
		return netmanthan.ui.form.get_open_grid_form();
	}
	...
	get_selected() {
		...
		var selected = {},
			me = this;
		netmanthan.meta.get_table_fields(this.doctype).forEach(function (df) {
	...
	set_indicator_formatter(fieldname, get_color, get_text) {
		...
		var doctype;
		if (netmanthan.meta.docfield_map[this.doctype][fieldname]) {
			doctype = this.doctype;
		} else {
			netmanthan.meta.get_table_fields(this.doctype).every(function (df) {
				if (netmanthan.meta.docfield_map[df.options][fieldname]) {
					doctype = df.options;
					return false;
				} else {
					return true;
		...
		netmanthan.meta.docfield_map[doctype][fieldname].formatter = function (
			value,
			df,
			options,
			doc
			...
			if (value) {
				...
				if (get_text) {
					label = get_text(doc);
				} else if (netmanthan.form.link_formatters[df.options]) {
					label = netmanthan.form.link_formatters[df.options](value, doc);
				} else {
					label = value;
				}
				...
				return `
						<a class="indicator ${get_color(doc || {})}"
							href="/app/${netmanthan.router.slug(df.options)}/${escaped_name}"
							data-doctype="${df.options}"
							data-name="${netmanthan.utils.escape_html(value)}">
							${label}
						</a>
					`;
			} else {
	...
	can_create(doctype) {
		...
		if (!netmanthan.model.can_create(doctype)) {
			return false;
		}
	...
	make_new(doctype) {
		...
		if (this.make_methods && this.make_methods[doctype]) {
			...
			netmanthan.model.with_doctype(doctype, function () {
				let new_doc = netmanthan.model.get_new_doc(doctype, null, null, true);

				// set link fields (if found)
				me.set_link_field(doctype, new_doc);

				netmanthan.ui.form.make_quick_entry(doctype, null, null, new_doc);
				// netmanthan.set_route('Form', doctype, new_doc.name);
			});
		}
	}
	...
	set_link_field(doctype, new_doc) {
		let me = this;
		netmanthan.get_meta(doctype).fields.forEach(function (df) {
	...
	update_in_all_rows(table_fieldname, fieldname, value) {
		...
		netmanthan.model
			...
			.filter((child) => !netmanthan.model.has_value(child.doctype, child.name, fieldname))
			.forEach((child) =>
				netmanthan.model.set_value(child.doctype, child.name, fieldname, value)
			);
	}
	...
	scroll_to_field(fieldname, focus = true) {
		...
		// scroll to input
		netmanthan.utils.scroll_to($el, true, 15);
		...
		let control_element = $el.closest(".netmanthan-control");
	...
	setup_docinfo_change_listener() {
		...
		if (this.doc && !this.is_new()) {
			netmanthan.socketio.doc_subscribe(doctype, docname);
		}
		netmanthan.realtime.off("docinfo_update");
		netmanthan.realtime.on("docinfo_update", ({ doc, key, action = "update" }) => {
			if (
			...
			let doc_list = netmanthan.model.docinfo[doctype][docname][key] || [];
			let docindex = doc_list.findIndex((old_doc) => {
				return old_doc.name === doc.name;
			});

			if (action === "add") {
				netmanthan.model.docinfo[doctype][docname][key].push(doc);
			}
			if (docindex > -1) {
				if (action === "update") {
					netmanthan.model.docinfo[doctype][docname][key].splice(docindex, 1, doc);
				}
				if (action === "delete") {
					netmanthan.model.docinfo[doctype][docname][key].splice(docindex, 1);
				}
			}
			...
			if (
				!(
					...
					doc.doctype === "Comment" &&
					doc.owner === netmanthan.session.user
				)
			) {
	...
	set_fields_as_options(
		...
		return new Promise((resolve) => {
			netmanthan.model.with_doctype(reference_doctype, () => {
				netmanthan.get_meta(reference_doctype).fields.map((df) => {
					filter_function(df) &&
	...
	get_involved_users() {
		...
		return involved_users
			.uniqBy((u) => u)
			.filter((user) => !["Administrator", netmanthan.session.user].includes(user))
			.filter(Boolean);
	}
};

netmanthan.validated = 0;
// Proxy for netmanthan.validated
Object.defineProperty(window, "validated", {
	get: function () {
		console.warn(
			"Please use `netmanthan.validated` instead of `validated`. It will be deprecated soon."
		); // eslint-disable-line
		return netmanthan.validated;
	},
	set: function (value) {
		console.warn(
			"Please use `netmanthan.validated` instead of `validated`. It will be deprecated soon."
		); // eslint-disable-line
		netmanthan.validated = value;
		return netmanthan.validated;
	},
});
