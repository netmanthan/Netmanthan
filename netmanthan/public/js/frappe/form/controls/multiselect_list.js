netmanthan.ui.form.ControlMultiSelectList = class ControlMultiSelectList extends (
	netmanthan.ui.form.ControlData
) {
	...
	set_selectable_items(options) {
		let html = options
			.map((option) => {
				...
				<div class="multiselect-check">${netmanthan.utils.icon("tick", "xs")}</div>
			</li>`;
			})
			.join("");
		if (!html) {
