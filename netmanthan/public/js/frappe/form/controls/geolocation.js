...
netmanthan.ui.form.ControlGeolocation = class ControlGeolocation extends netmanthan.ui.form.ControlData {
	...
	set_disp_area(value) {
		...
		if (this.frm) {
			...
			$(document).on("netmanthan.ui.Dialog:shown", () => {
				this.make_map(value);
			});
		}
	}
	...
	bind_leaflet_map() {
		...
		this.map.setView(netmanthan.utils.map_defaults.center, netmanthan.utils.map_defaults.zoom);
	...
	bind_leaflet_draw_control() {
		if (
			!netmanthan.perm.has_perm(this.doctype, this.df.permlevel, "write", this.doc) ||
			this.df.read_only
		) {
			return;
		}
	...
	get_leaflet_controls() {
		return new L.Control.Draw({
			...
			draw: {
				polyline: {
					shapeOptions: {
						color: netmanthan.ui.color.get("blue"),
						weight: 10,
					},
				},
				polygon: {
					...
					drawError: {
						color: netmanthan.ui.color.get("orange"), // Color the shape will turn when intersects
