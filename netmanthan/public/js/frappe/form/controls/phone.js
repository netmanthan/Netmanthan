...
netmanthan.ui.form.ControlPhone = class ControlPhone extends netmanthan.ui.form.ControlData {
	async make_input() {
	...
	async setup_country_codes() {
		...
		if (data) {
			...
			const data = await netmanthan.xcall("netmanthan.geo.country_info.get_country_timezone_info");
	...
	input_events() {
		this.$input.keydown((e) => {
			...
			if ([netmanthan.ui.keyCode.BACKSPACE].includes(key_code)) {
	...
	setup_country_code_picker() {
		...
		if (!this.selected_icon.length) {
			this.selected_icon = $(
				`<div class="selected-phone">${netmanthan.utils.icon("down", "sm")}</div>`
			);
	...
	get_country_flag(country) {
		...
		return netmanthan.utils.flag(code);
	}
