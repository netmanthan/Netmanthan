...
netmanthan.ui.form.ControlIcon = class ControlIcon extends netmanthan.ui.form.ControlData {
	make_input() {
	...
	get_all_icons() {
		netmanthan.symbols = [];
		$("#netmanthan-symbols > symbol[id]").each(function () {
			this.id.includes("icon-") && netmanthan.symbols.push(this.id.replace("icon-", ""));
		});
	}
	...
	make_icon_input() {
		...
		if (!this.selected_icon) {
			this.selected_icon = $(
				`<div class="selected-icon">${netmanthan.utils.icon("folder-normal", "md")}</div>`
			);
