netmanthan.provide("netmanthan.phone_call");

netmanthan.ui.form.ControlData = class ControlData extends netmanthan.ui.form.ControlInput {
	...
	make_input() {
		...
		this.$input.on("paste", (e) => {
			...
			if (maxlength && pasted_data.length > maxlength) {
				...
				if (this.frm && netmanthan.model.can_write(this.frm.doctype)) {
					let doctype_edit_link = null;
					if (this.frm.meta.custom) {
						doctype_edit_link = netmanthan.utils.get_form_link(
							"DocType",
							this.frm.doctype,
							true,
							__("this form")
		...
		this.$wrapper.find(".control-input").append(
			`<span class="link-btn">
				<a class="btn-open no-decoration" title="${__("Open Link")}" target="_blank">
					${netmanthan.utils.icon("link-url", "sm")}
				</a>
			</span>`
		);
		...
		if (this.df.with_copy_button) {
			this.$wrapper
				...
				.append(
					...
					${netmanthan.utils.icon("clipboard", "sm")}
				</button>`
				)
				.find(".action-btn")
				.click(() => {
					netmanthan.utils.copy_to_clipboard(this.value);
				});
		}
	}
	...
	bind_change_event() {
		...
		if (this.constructor.trigger_change_on_input_event && !this.in_grid()) {
			...
			this.$input.on("input", netmanthan.utils.debounce(change_handler, 500));
		}
	}
	setup_autoname_check() {
		if (!this.df.parent) return;
		this.meta = netmanthan.get_meta(this.df.parent);
		if (
			this.meta &&
			((this.meta.autoname &&
	...
	validate(v) {
		...
		if (this.df.options == "Phone") {
			...
			var email_list = netmanthan.utils.split_emails(v);
			if (!email_list) {
				return "";
			} else {
...
netmanthan.ui.form.ControlReadOnly = netmanthan.ui.form.ControlData;
