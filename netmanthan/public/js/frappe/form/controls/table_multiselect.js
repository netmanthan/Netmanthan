netmanthan.ui.form.ControlTableMultiSelect = class ControlTableMultiSelect extends (
	netmanthan.ui.form.ControlLink
) {
	make_input() {
		super.make_input();
		...
		this.$input_area.on("click", ".btn-link-to-form", (e) => {
			...
			const value = decodeURIComponent($value.data().value);
			const link_field = this.get_link_field();
			netmanthan.set_route("Form", link_field.options, value);
		});
		this.$input.on("keydown", (e) => {
			// if backspace key pressed on empty input, delete last value
			if (e.keyCode == netmanthan.ui.keyCode.BACKSPACE && e.target.value === "") {
	...
	parse(value, label) {
		...
		if (value) {
			if (this.frm) {
				const new_row = netmanthan.model.add_child(
					this.frm.doc,
					this.df.options,
					this.df.fieldname
				);
			...
			netmanthan.utils.add_link_title(link_field.options, value, label);
		}
	...
	get_pill_html(value) {
		...
		const pill_name = netmanthan.utils.get_link_title(link_field.options, value) || value;
		return `
			<button class="data-pill btn tb-selected-value" data-value="${encoded_value}">
				<span class="btn-link-to-form">${__(pill_name)}</span>
				<span class="btn-remove">${netmanthan.utils.icon("close")}</span>
			</button>
		`;
	}
	get_options() {
	...
	get_link_field() {
		if (!this._link_field) {
			const meta = netmanthan.get_meta(this.df.options);
