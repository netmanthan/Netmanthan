netmanthan.ui.form.Control = class BaseControl {
	constructor(opts) {
		$.extend(this, opts);
		this.make();

		// if developer_mode=1, show fieldname as tooltip
		if (netmanthan.boot.user && netmanthan.boot.developer_mode === 1 && this.$wrapper) {
	...
	make_wrapper() {
		this.$wrapper = $("<div class='netmanthan-control'></div>").appendTo(this.parent);
	...
	get_status(explain) {
		...
		var status = netmanthan.perm.get_field_display_status(
			this.df,
			netmanthan.model.get_doc(this.doctype, this.docname),
		...
		let value = netmanthan.model.get_value(this.doctype, this.docname, this.df.fieldname);
	...
	show_translatable_button(value) {
		...
		if (
			...
			!netmanthan.model.can_write("Translation") ||
			!value
		)
			return;

		// Disable translation in website
		if (!netmanthan.views || !netmanthan.views.TranslationManager) return;
		...
		$(translation_btn)
			...
			.on("click", () => {
				if (!this.doc.__islocal) {
					new netmanthan.views.TranslationManager({
						df: this.df,
