...
netmanthan.ui.form.ControlTextEditor = class ControlTextEditor extends netmanthan.ui.form.ControlCode {
	make_wrapper() {
		super.make_wrapper();
	}
	...
	bind_events() {
		this.quill.on(
			"text-change",
			netmanthan.utils.debounce((delta, oldDelta, source) => {
		...
		$(this.quill.root).on("keydown", (e) => {
			const key = netmanthan.ui.keys && netmanthan.ui.keys.get_key(e);
	...
	get_mention_options() {
		...
		return {
			...
			source: netmanthan.utils.debounce(async function (search_term, renderList) {
				let method =
					me.mention_search_method || "netmanthan.desk.search.get_names_for_mentions";
				let values = await netmanthan.xcall(method, {
					search_term,
				});
			...
			renderItem(item) {
				...
				return `${value} ${item.is_group ? netmanthan.utils.icon("users") : ""}`;
			},
		};
	}
	...
	parse(value) {
		...
		return netmanthan.dom.remove_script_and_style(value);
	}
