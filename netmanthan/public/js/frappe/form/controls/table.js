...
netmanthan.ui.form.ControlTable = class ControlTable extends netmanthan.ui.form.Control {
	make() {
		super.make();
		...
		this.$wrapper.on("paste", ":text", (e) => {
			...
			let data = netmanthan.utils.csv_to_array(pasted_data, "\t");
	...
	get_field(field_name) {
		...
		this.grid.meta.fields.some((field) => {
			if (netmanthan.model.no_value_type.includes(field.fieldtype)) {
				return false;
			}
