netmanthan.ui.form.ControlCurrency = class ControlCurrency extends netmanthan.ui.form.ControlFloat {
	...
	get_precision() {
		...
		if (typeof this.df.precision != "number" && !this.df.precision) {
			if (netmanthan.boot.sysdefaults.currency_precision) {
				this.df.precision = netmanthan.boot.sysdefaults.currency_precision;
			} else {
