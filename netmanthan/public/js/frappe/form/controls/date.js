netmanthan.ui.form.ControlDate = class ControlDate extends netmanthan.ui.form.ControlData {
	...
	set_formatted_input(value) {
		...
		if (should_refresh) {
			this.datepicker.selectDate(netmanthan.datetime.str_to_obj(value));
		}
	}
	set_date_options() {
		// webformTODO:
		let sysdefaults = netmanthan.boot.sysdefaults;

		let lang = "en";
		netmanthan.boot.user && (lang = netmanthan.boot.user.language);
		...
		this.date_format = netmanthan.defaultDateFormat;
		this.datepicker_options = {
			...
			firstDay: netmanthan.datetime.get_first_day_of_the_week_index(),
			onSelect: () => {
	...
	get_now_date() {
		return netmanthan.datetime
			.convert_to_system_tz(netmanthan.datetime.now_date(true), false)
			.toDate();
	}
	set_t_for_today() {
		var me = this;
		this.$input.on("keydown", function (e) {
			if (e.which === 84) {
				...
				if (me.df.fieldtype == "Date") {
					me.set_value(netmanthan.datetime.nowdate());
				}
				if (me.df.fieldtype == "Datetime") {
					me.set_value(netmanthan.datetime.now_datetime());
				}
				if (me.df.fieldtype == "Time") {
					me.set_value(netmanthan.datetime.now_time());
				}
				return false;
			}
		});
	...
	parse(value) {
		if (value) {
			...
			return netmanthan.datetime.user_to_str(value, false, true);
		}
	}
	format_for_input(value) {
		if (value) {
			return netmanthan.datetime.str_to_user(value, false, true);
		}
		return "";
	}
	validate(value) {
		if (value && !netmanthan.datetime.validate(value)) {
			let sysdefaults = netmanthan.sys_defaults;
			let date_format =
				sysdefaults && sysdefaults.date_format ? sysdefaults.date_format : "yyyy-mm-dd";
			netmanthan.msgprint(__("Date {0} must be in format: {1}", [value, date_format]));
			return "";
		}
		return value;
	}
