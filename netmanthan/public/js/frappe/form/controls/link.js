...
netmanthan.ui.form.recent_link_validations = {};

netmanthan.ui.form.ControlLink = class ControlLink extends netmanthan.ui.form.ControlData {
		...
		$(`<div class="link-field ui-front" style="position: relative;">
			...
			<span class="link-btn">
				<a class="btn-open no-decoration" title="${__("Open Link")}">
					${netmanthan.utils.icon("arrow-right", "xs")}
				</a>
			</span>
		...
		this.$input.on("focus", function () {
			setTimeout(function () {
				if (me.$input.val() && me.get_options()) {
					...
					me.$link.toggle(true);
					me.$link_open.attr("href", netmanthan.utils.get_form_link(doctype, name));
				}
	...
	get_reference_doctype() {
		...
		if (this.doctype) return this.doctype;
			...
			return netmanthan.get_route && netmanthan.get_route()[0] === "List"
				? netmanthan.get_route()[1]
				: null;
		}
	}
	setup_buttons() {
	...
	is_translatable() {
		return in_list(netmanthan.boot?.translated_doctypes || [], this.get_options());
	}
	is_title_link() {
		return in_list(netmanthan.boot?.link_title_doctypes || [], this.get_options());
	}
	async set_link_title(value) {
		...
		const link_title =
			netmanthan.utils.get_link_title(doctype, value) ||
			(await netmanthan.utils.fetch_link_title(doctype, value));
	...
	parse_validate_and_set_in_model(value, e, label) {
		...
		if (label) {
			...
			netmanthan.utils.add_link_title(this.df.options, value, label);
		}
	...
	new_doc() {
		...
		if (df && df.get_route_options_for_new_doc) {
			netmanthan.route_options = df.get_route_options_for_new_doc(this);
		} else {
			netmanthan.route_options = {};
		}

		// partially entered name field
		netmanthan.route_options.name_field = this.get_label_value();

		// reference to calling link
		netmanthan._from_link = netmanthan.utils.deep_clone(this);
		netmanthan._from_link_scrollY = $(document).scrollTop();

		netmanthan.ui.form.make_quick_entry(doctype, (doc) => {
	...
	setup_awesomeplete() {
		...
		this.awesomplete = new Awesomplete(me.input, {
			...
			item: function (item) {
				...
				return $("<li></li>")
					...
					.html(`<a><p title="${netmanthan.utils.escape_html(_label)}">${html}</p></a>`)
					.get(0);
			},
			sort: function () {
				return 0;
		...
		this.$input.on(
			"input",
			netmanthan.utils.debounce(function (e) {
					...
					callback: function (r) {
						...
						if (!me.df.only_select) {
							...
							let custom__link_options =
								netmanthan.ui.form.ControlLink.link_options &&
								netmanthan.ui.form.ControlLink.link_options(me);
		...
		this.$input.on("awesomplete-select", function (e) {
			...
			if (me.df.remember_last_selected_value) {
				netmanthan.boot.user.last_selected_values[me.df.options] = item.value;
			}
	...
	toggle_href(doctype) {
		if (netmanthan.model.can_select(doctype) && !netmanthan.model.can_read(doctype)) {
	...
	get_filter_description(filters) {
		...
		function get_filter_description(filter) {
			...
			let docfield = netmanthan.meta.get_docfield(doctype, fieldname);
			let label = docfield ? docfield.label : netmanthan.model.unscrub(fieldname);
	...
	set_custom_query(args) {
		const is_valid_value = (value, key) => {
			...
			if (this.frm) {
				let field = netmanthan.meta.get_docfield(this.frm.doctype, key);
	...
	validate_link_and_fetch(df, options, docname, value) {
		...
		function update_dependant_fields(response) {
			...
			for (const [target_field, source_field] of Object.entries(fetch_map)) {
				...
				let target_df = netmanthan.meta.get_docfield(df.parent, target_field);
				let target_value = netmanthan.model.get_value(df.parent, docname, target_field);
				if (target_df?.fetch_if_empty && target_value) {
					continue;
				} else {
					netmanthan.model.set_value(
						df.parent,
						docname,
						target_field,
						field_value,
		...
		if (value) {
			return netmanthan
				.xcall("netmanthan.client.validate_link", {
					doctype: options,
					docname: value,
