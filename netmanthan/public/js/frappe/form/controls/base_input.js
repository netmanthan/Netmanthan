netmanthan.ui.form.ControlInput = class ControlInput extends netmanthan.ui.form.Control {
	...
	make_wrapper() {
		if (this.only_input) {
			this.$wrapper = $('<div class="form-group netmanthan-control">').appendTo(this.parent);
		} else {
			this.$wrapper = $(
	...
	refresh_input() {
		...
		if (me.disp_status != "None") {
			// refresh value
			if (me.frm) {
				me.value = netmanthan.model.get_value(me.doctype, me.docname, me.df.fieldname);
			} else if (me.doc) {
				...
				if (is_fetch_from_read_only) {
					$(me.disp_area).attr(
						...
						__(
							...
							me.df.fetch_from.split(".").map((value) => __(netmanthan.unscrub(value)))
						)
					);
				}
			}
	...
	set_disp_area(value) {
		...
		if (this.df.fieldtype === "Data") {
			value = netmanthan.utils.escape_html(value);
		}
		let doc = this.doc || (this.frm && this.frm.doc);
		let display_value = netmanthan.format(value, this.df, { no_icon: true, inline: true }, doc);
