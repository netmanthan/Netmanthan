...
netmanthan.ui.form.Toolbar = class Toolbar {
	constructor(opts) {
		$.extend(this, opts);
		this.refresh();
	...
	set_title() {
		if (this.frm.is_new()) {
			...
			if (
				...
				this.page.$sub_title_area.on("click", (event) => {
					...
					netmanthan.utils.copy_to_clipboard(this.frm.docname);
				});
			}
		} else {
		...
		if (this.frm.meta.title_field) {
			netmanthan.utils.set_title(title + " - " + this.frm.docname);
		}
	...
	rename_document_title(input_name, input_title, merge = false) {
		...
		let rename_document = () => {
			return netmanthan
				.xcall("netmanthan.model.rename_doc.update_document_title", {
					doctype,
					docname,
					name: input_name,
					title: input_title,
				...
				.then((new_docname) => {
					...
					if (input_name && new_docname == docname) {
						netmanthan.socketio.doc_subscribe(doctype, input_name);
						netmanthan.realtime.on("doc_update", (data) => {
	...
	set_indicator() {
		var indicator = netmanthan.get_indicator(this.frm.doc);
		if (
	...
	make_viewers() {
		...
		this.frm.viewers = new netmanthan.ui.form.FormViewers({
			frm: this.frm,
	...
	make_menu_items() {
		...
		const is_submittable = netmanthan.model.is_submittable(this.frm.doc.doctype);

		const print_settings = netmanthan.model.get_doc(":Print Settings", "Print Settings");
		...
		if (
			...
			if (netmanthan.model.can_print(null, me.frm) && !this.frm.meta.issingle) {
		...
		// email
		if (netmanthan.model.can_email(null, me.frm) && me.frm.doc.docstatus < 2) {
			this.page.add_menu_item(
				__("Email"),
				function () {
					me.frm.email_doc();
		...
		// duplicate
		if (in_list(netmanthan.boot.user.can_create, me.frm.doctype) && !me.frm.meta.allow_copy) {
			this.page.add_menu_item(
				__("Duplicate"),
				function () {
					me.frm.copy_doc();
		...
		this.page.add_menu_item(
			...
			function () {
				netmanthan.utils.copy_to_clipboard(JSON.stringify(me.frm.doc));
			},
			true
		);
		...
		if (
			...
			netmanthan.model.can_delete(me.frm.doctype)
		) {
			this.page.add_menu_item(
				__("Delete"),
				function () {
		...
		if (this.can_repeat()) {
			this.page.add_menu_item(
				__("Repeat"),
				function () {
					netmanthan.utils.new_auto_repeat_prompt(me.frm);
				},
				true
			);
		}
		...
		if (p[CREATE] && !this.frm.meta.issingle && !this.frm.meta.in_create) {
			this.page.add_menu_item(
				...
				function () {
					netmanthan.new_doc(me.frm.doctype, true);
				},
				true,
				{
					shortcut: "Ctrl+B",
	...
	make_customize_buttons() {
		...
		if (netmanthan.model.can_create("DocType")) {
			if (netmanthan.boot.developer_mode === 1 && !is_doctype_form) {
				// edit doctype
				this.page.add_menu_item(
					__("Edit DocType"),
					() => {
						netmanthan.set_route("Form", "DocType", this.frm.doctype);
					},
					true
				);
			}
	...
	has_workflow() {
		if (this._has_workflow === undefined)
			this._has_workflow = netmanthan.get_list("Workflow", {
	...
	show_linked_with() {
		if (!this.frm.linked_with) {
			this.frm.linked_with = new netmanthan.ui.form.LinkedWith({
				frm: this.frm,
			});
		}
	...
	set_page_actions(status) {
		...
		if (status === "Edit") {
			...
			if (this.has_workflow()) {
				netmanthan
					.xcall("netmanthan.model.workflow.can_cancel_document", {
