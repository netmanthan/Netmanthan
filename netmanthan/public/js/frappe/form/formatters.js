...
netmanthan.provide("netmanthan.form.formatters");

netmanthan.form.link_formatters = {};

netmanthan.form.formatters = {
	...
	_apply_custom_formatter: function (value, df) {
		...
		example:
			netmanthan.meta.docfield_map[df.parent][df.fieldname].formatter = (value) => {
		...
		*/
		...
		if (df) {
			const std_df =
				netmanthan.meta.docfield_map[df.parent] &&
				netmanthan.meta.docfield_map[df.parent][df.fieldname];
	...
	Data: function (value, df) {
		...
		return netmanthan.form.formatters._apply_custom_formatter(value, df);
	},
	Autocomplete: function (value, df) {
		return __(netmanthan.form.formatters["Data"](value, df));
	},
	...
	Float: function (value, docfield, options, doc) {
		...
		var precision =
			docfield.precision ||
			cint(netmanthan.boot.sysdefaults && netmanthan.boot.sysdefaults.float_precision) ||
			null;
		if (docfield.options && docfield.options.trim()) {
			...
			return netmanthan.form.formatters.Currency(value, docfield, options, doc);
		} else {
			// show 1.000000 as 1
			...
			return netmanthan.form.formatters._right(format_number(value, null, precision), options);
		}
	},
	Int: function (value, docfield, options) {
		return netmanthan.form.formatters._right(value == null ? "" : cint(value), options);
	},
	Percent: function (value, docfield, options) {
		const precision =
			...
			cint(netmanthan.boot.sysdefaults && netmanthan.boot.sysdefaults.float_precision) ||
			2;
		return netmanthan.form.formatters._right(flt(value, precision) + "%", options);
	},
	...
	Currency: function (value, docfield, options, doc) {
		var currency = netmanthan.meta.get_field_currency(docfield, doc);
		...
		if (typeof docfield.precision == "number") {
			...
			precision = cint(
				docfield.precision || netmanthan.boot.sysdefaults.currency_precision || 2
			);
		}
		...
		if (precision > 2) {
			...
			if (decimals.length < 3 || decimals.length < precision) {
				const fraction =
					netmanthan.model.get_value(":Currency", currency, "fraction_units") || 100; // if not set, minimum 2.
		...
		if (options && options.only_value) {
			...
			return netmanthan.form.formatters._right(value, options);
		}
	},
	...
	Link: function (value, docfield, options, doc) {
		...
		let link_title = netmanthan.utils.get_link_title(doctype, value);
		...
		if (netmanthan.form.link_formatters[doctype]) {
			// don't apply formatters in case of composite (parent field of same type)
			if (doc && doctype !== doc.doctype) {
				value = netmanthan.form.link_formatters[doctype](value, doc, docfield);
			}
		}
		...
		if (docfield && docfield.link_onclick) {
			...
			if (netmanthan.model.can_read(doctype)) {
				const a = document.createElement("a");
				a.href = `/app/${encodeURIComponent(
					netmanthan.router.slug(doctype)
	...
	Date: function (value) {
		if (!netmanthan.datetime.str_to_user) {
			return value;
		}
		if (value) {
			value = netmanthan.datetime.str_to_user(value, false, true);
			// handle invalid date
	...
	DateRange: function (value) {
		if (Array.isArray(value)) {
			return __("{0} to {1}", [
				netmanthan.datetime.str_to_user(value[0]),
				netmanthan.datetime.str_to_user(value[1]),
			]);
		} else {
			return value || "";
		}
	...
	Datetime: function (value) {
		if (value) {
			return moment(netmanthan.datetime.convert_to_user_tz(value)).format(
				netmanthan.boot.sysdefaults.date_format.toUpperCase() +
					" " +
					(netmanthan.boot.sysdefaults.time_format || "HH:mm:ss")
			);
		} else {
			return "";
		}
	...
	Text: function (value, df) {
		if (value) {
			...
			if (!match) {
				value = netmanthan.utils.replace_newlines(value);
			}
		}

		return netmanthan.form.formatters.Data(value, df);
	},
	Time: function (value) {
		if (value) {
			value = netmanthan.datetime.str_to_user(value, true);
		}
	...
	Duration: function (value, docfield) {
		if (value) {
			let duration_options = netmanthan.utils.get_duration_options(docfield);
			value = netmanthan.utils.get_formatted_duration(value, duration_options);
		}
	...
	SmallText: function (value) {
		return netmanthan.form.formatters.Text(value);
	},
	TextEditor: function (value) {
		let formatted_value = netmanthan.form.formatters.Text(value);
	...
	WorkflowState: function (value) {
		var workflow_state = netmanthan.get_doc("Workflow State", value);
		if (workflow_state) {
			return repl(
	...
	TableMultiSelect: function (rows, df, options) {
		...
		const formatted_values = rows.map((row) => {
			...
			return netmanthan.format(value, link_field, options, row);
		});
	...
	Icon: (value) => {
		return value
			? `<div>
			<div class="selected-icon">${netmanthan.utils.icon(value, "md")}</div>
...
netmanthan.form.get_formatter = function (fieldtype) {
	if (!fieldtype) fieldtype = "Data";
	return netmanthan.form.formatters[fieldtype.replace(/ /g, "")] || netmanthan.form.formatters.Data;
};

netmanthan.format = function (value, df, options, doc) {
	...
	var formatter = df.formatter || netmanthan.form.get_formatter(fieldtype);

	var formatted = formatter(value, df, options, doc);

	if (typeof formatted == "string") formatted = netmanthan.dom.remove_script_and_style(formatted);

	return formatted;
};

netmanthan.get_format_helper = function (doc) {
	var helper = {
		get_formatted: function (fieldname) {
			var df = netmanthan.meta.get_docfield(doc.doctype, fieldname);
			if (!df) {
				console.log("fieldname not found: " + fieldname);
			}
			return netmanthan.format(doc[fieldname], df, { inline: 1 }, doc);
		},
	};
	$.extend(helper, doc);
	return helper;
...
netmanthan.form.link_formatters["User"] = function (value, doc, docfield) {
