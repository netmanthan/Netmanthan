...
netmanthan.ui.form.Dashboard = class FormDashboard {
	...
	setup_dashboard_sections() {
		...
		this.heatmap_area = this.make_section({
			...
			is_dashboard_section: 1,
			body_html: `
				<div id="heatmap-${netmanthan.model.scrub(this.frm.doctype)}" class="heatmap"></div>
	...
	refresh() {
		this.reset();
		if (this.frm.doc.__islocal || !netmanthan.boot.desk_settings.dashboard) {
			return;
		}
	...
	render_links() {
		...
		$(netmanthan.render_template("form_links", this.data)).appendTo(transactions_area_body);
	...
	render_report_links() {
		...
		if (this.data.reports && this.data.reports.length) {
			$(netmanthan.render_template("report_links", this.data)).appendTo(parent);
			// bind reports
	...
	open_report($link) {
		...
		netmanthan.provide("netmanthan.route_options");
		netmanthan.route_options[fieldname] = this.frm.doc.name;
		netmanthan.set_route("query-report", report);
	}
	...
	open_document_list($link, show_open) {
		...
		if (
			...
			if (names.length) {
				netmanthan.route_options = { name: ["in", names] };
			} else {
				return false;
			}
		} else if (this.data.fieldname) {
			netmanthan.route_options = this.get_document_filter(doctype);
			if (show_open && netmanthan.ui.notifications) {
				netmanthan.ui.notifications.show_open_count_list(doctype);
			}
		}

		netmanthan.set_route("List", doctype, "List");
	}
	...
	set_open_count() {
		...
		let method = this.data.method || "netmanthan.desk.notifications.get_open_count";
		netmanthan.call({
			type: "GET",
			method: method,
			args: {
	...
	// heatmap
	render_heatmap() {
		this.heatmap = new netmanthan.Chart("#heatmap-" + netmanthan.model.scrub(this.frm.doctype), {
			type: "heatmap",
	...
	render_graph(args) {
		...
		this.chart = new netmanthan.Chart(".form-graph", args);
		if (!this.chart) {
			this.hide();
		}
	}
