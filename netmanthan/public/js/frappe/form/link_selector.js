...
netmanthan.ui.form.LinkSelector = class LinkSelector {
	constructor(opts) {
		...
		if (this.doctype != "[Select]") {
			netmanthan.model.with_doctype(this.doctype, function (r) {
				me.make();
			});
		} else {
			this.make();
				...
				if (r.values.length) {
					...
					$(
						'<p><br><span class="text-muted">' +
							__("No Results") +
							"</span>" +
							(netmanthan.model.can_create(me.doctype)
	...
	set_in_grid(value) {
		return new Promise((resolve) => {
			if (this.qty_fieldname) {
					...
					(data) => {
						...
						if (!updated) {
							...
							netmanthan.run_serially([
								...
								() => netmanthan.timeout(0.1),
								() => {
									let args = {};
									args[this.fieldname] = value;
									args[this.qty_fieldname] = data.qty;
									return netmanthan.model.set_value(d.doctype, d.name, args);
								},
								() => netmanthan.show_alert(__("Added {0} ({1})", [value, data.qty])),
								() => resolve(),
							]);
						}
					},
				...
				netmanthan.model.set_value(d.doctype, d.name, this.fieldname, value).then(() => {
					netmanthan.show_alert(__("{0} {1} added", [this.dynamic_link_reference, value]));
					resolve();
				});
			} else {
				let d = this.target.add_new_row();
				netmanthan.model.set_value(d.doctype, d.name, this.fieldname, value).then(() => {
					netmanthan.show_alert(__("{0} added", [value]));
					resolve();
				});
			}
		});
...
netmanthan.link_search = function (doctype, args, callback, btn) {
	if (!args) {
		args = {
			txt: "",
		};
