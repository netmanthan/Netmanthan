...
netmanthan.provide("netmanthan.ui.form.handlers");
...
netmanthan.ui.form.get_event_handler_list = function (doctype, fieldname) {
	if (!netmanthan.ui.form.handlers[doctype]) {
		netmanthan.ui.form.handlers[doctype] = {};
	}
	if (!netmanthan.ui.form.handlers[doctype][fieldname]) {
		netmanthan.ui.form.handlers[doctype][fieldname] = [];
	}
	return netmanthan.ui.form.handlers[doctype][fieldname];
};

netmanthan.ui.form.on = netmanthan.ui.form.on_change = function (doctype, fieldname, handler) {
	var add_handler = function (fieldname, handler) {
		var handler_list = netmanthan.ui.form.get_event_handler_list(doctype, fieldname);
...
netmanthan.ui.form.off = function (doctype, fieldname, handler) {
	var handler_list = netmanthan.ui.form.get_event_handler_list(doctype, fieldname);
...
netmanthan.ui.form.trigger = function (doctype, fieldname) {
	cur_frm.script_manager.trigger(fieldname, doctype);
};

netmanthan.ui.form.ScriptManager = class ScriptManager {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	trigger(event_name, doctype, name) {
		...
		this.frm.selected_doc = netmanthan.get_doc(doctype, name);
	...
	get_handlers(event_name, doctype) {
		...
		if (netmanthan.ui.form.handlers[doctype] && netmanthan.ui.form.handlers[doctype][event_name]) {
			$.each(netmanthan.ui.form.handlers[doctype][event_name], function (i, fn) {
	...
	setup() {
		...
		$.each(this.frm.fields, function (i, field) {
			...
			if (netmanthan.model.table_fields.includes(field.df.fieldtype)) {
				$.each(
					netmanthan.meta.get_docfields(field.df.options, me.frm.docname),
					function (i, df) {
						setup_add_fetch(df);
					}
				);
		...
		// css
		doctype.__css && netmanthan.dom.set_style(doctype.__css);

		this.trigger("setup");
	}
	log_error(caller, e) {
		netmanthan.show_alert({ message: __("Error in Client Script."), indicator: "error" });
