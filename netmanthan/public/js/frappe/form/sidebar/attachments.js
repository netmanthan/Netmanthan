...
netmanthan.ui.form.Attachments = class Attachments {
	constructor(opts) {
		$.extend(this, opts);
	...
	make() {
		...
		this.parent.find(".explore-btn").click(() => {
			...
			netmanthan.set_route("List", "File", {
	...
	add_attachment(attachment) {
		...
		if (netmanthan.model.can_write(this.frm.doctype, this.frm.name)) {
			remove_action = function (target_id) {
				netmanthan.confirm(__("Are you sure you want to delete the attachment?"), function () {
		...
		const icon = `<a href="/app/file/${fileid}">
				${netmanthan.utils.icon(attachment.is_private ? "lock" : "unlock", "sm ml-0")}
			</a>`;

		$(`<li class="attachment-row">`)
			.append(netmanthan.get_data_pill(file_label, fileid, remove_action, icon))
	...
	remove_attachment(fileid, callback) {
		...
		return netmanthan.call({
			method: "netmanthan.desk.form.utils.remove_attach",
			type: "DELETE",
			args: {
				fid: fileid,
				dt: me.frm.doctype,
			...
			callback: function (r, rt) {
				if (r.exc) {
					if (!r._server_messages) netmanthan.msgprint(__("There were errors"));
					return;
				}
