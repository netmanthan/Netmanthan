...
netmanthan.ui.form.AssignTo = class AssignTo {
	constructor(opts) {
		$.extend(this, opts);
	...
	render(assignments) {
		...
		let avatar_group = netmanthan.avatar_group(assigned_users, 5, {
			align: "left",
			overlap: true,
		});
		...
		avatar_group.click(() => {
			new netmanthan.ui.form.AssignmentDialog({
	...
	add() {
		...
		if (this.frm.is_new()) {
			netmanthan.throw(__("Please save the document before assignment"));
			return;
		}

		if (!me.assign_to) {
			me.assign_to = new netmanthan.ui.form.AssignToDialog({
	...
	remove(owner) {
		if (this.frm.is_new()) {
			netmanthan.throw(__("Please save the document before removing assignment"));
			return;
		}

		return netmanthan
			.xcall("netmanthan.desk.form.assign_to.remove", {
...
netmanthan.ui.form.AssignToDialog = class AssignToDialog {
	constructor(opts) {
		$.extend(this, opts);
	...
	make() {
		...
		me.dialog = new netmanthan.ui.Dialog({
	...
	assign_to_me() {
		...
		if (me.dialog.get_value("assign_to_me")) {
			assign_to.push(netmanthan.session.user);
		}
		...
		return [
			...
			},
			{
				...
				get_data: function (txt) {
					return netmanthan.db.get_link_options("User", txt, {
...
netmanthan.ui.form.AssignmentDialog = class {
	constructor(opts) {
		this.frm = opts.frm;
	...
	make() {
		...
		this.assignment_list.removeClass("netmanthan-control");
	...
	add_assignment(assignment) {
		return netmanthan
			.xcall("netmanthan.desk.form.assign_to.add", {
				doctype: this.frm.doctype,
				name: this.frm.docname,
	...
	remove_assignment(assignment) {
		return netmanthan.xcall("netmanthan.desk.form.assign_to.remove", {
			doctype: this.frm.doctype,
			name: this.frm.docname,
			assign_to: assignment,
		});
	...
	close_assignment(assignment) {
		return netmanthan.xcall("netmanthan.desk.form.assign_to.close", {
			doctype: this.frm.doctype,
			name: this.frm.docname,
			assign_to: assignment,
		});
	...
	get_assignment_row(assignment) {
		const row = $(`
			<div class="dialog-assignment-row" data-user="${assignment}">
				<div class="assignee">
					${netmanthan.avatar(assignment)}
					${netmanthan.user.full_name(assignment)}
				</div>
		...
		if (assignment === netmanthan.session.user || this.frm.perm[0].write) {
			btn_group.append(`
				<button type="button" class="btn btn-default remove-btn" title="${__("Cancel")}">
				${netmanthan.utils.icon("close")}
				</button>
			`);
