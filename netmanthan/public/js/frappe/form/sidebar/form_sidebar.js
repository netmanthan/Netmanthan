...
netmanthan.ui.form.Sidebar = class {
	constructor(opts) {
		$.extend(this, opts);
	}

	make() {
		var sidebar_content = netmanthan.render_template("form_sidebar", {
		...
		netmanthan.ui.form.setup_user_image_event(this.frm);
	...
	bind_events() {
		...
		this.comments.on("click", function () {
			netmanthan.utils.scroll_to(me.frm.footer.wrapper.find(".comment-box"), true);
		});

		this.like_icon.on("click", function () {
			netmanthan.ui.toggle_like(me.like_wrapper, me.frm.doctype, me.frm.doc.name, function () {
				me.refresh_like();
			});
		});
	}
	...
	setup_keyboard_shortcuts() {
		...
		netmanthan.ui.keys.get_shortcut_group(this.page).add(assignment_link);
	}
	...
	refresh() {
		if (this.frm.doc.__islocal) {
			...
			this.sidebar
				...
				.html(
					__(
						"{0} edited this {1}",
						[
							netmanthan.user.full_name(this.frm.doc.modified_by).bold(),
			...
			this.sidebar
				...
				.html(
					__(
						"{0} created this {1}",
						[
							netmanthan.user.full_name(this.frm.doc.owner).bold(),
			...
			netmanthan.ui.form.set_user_image(this.frm);
		}
	}
			...
			netmanthan.call({
				...
				callback: function (res) {
					...
					me.sidebar.find(".auto-repeat-status").on("click", function () {
						netmanthan.set_route("Form", "Auto Repeat", me.frm.doc.auto_repeat);
					});
				},
			});
		}
	...
	make_tags() {
		...
		this.frm.tags = new netmanthan.ui.TagEditor({
			parent: tags_parent,
	...
	make_attachments() {
		var me = this;
		this.frm.attachments = new netmanthan.ui.form.Attachments({
	...
	make_assignments() {
		this.frm.assign_to = new netmanthan.ui.form.AssignTo({
	...
	make_shared() {
		this.frm.shared = new netmanthan.ui.form.Share({
			frm: this.frm,
	...
	make_like() {
		...
		netmanthan.ui.setup_like_popover(this.sidebar.find(".form-stats-likes"), ".like-icon");
	}
	...
	make_follow() {
		...
		this.follow_button.on("click", () => {
			...
			netmanthan
				.call("netmanthan.desk.form.document_follow.update_follow", {
					doctype: this.frm.doctype,
					doc_name: this.frm.doc.name,
					following: !is_followed,
				})
				.then(() => {
					netmanthan.model.set_docinfo(
						this.frm.doctype,
						this.frm.doc.name,
	...
	refresh_like() {
		...
		const liked = netmanthan.ui.is_liked(this.frm.doc);
		this.like_wrapper
	...
	make_review() {
		...
		if (netmanthan.boot.energy_points_enabled && !this.frm.is_new()) {
			this.frm.reviews = new netmanthan.ui.form.Review({
	...
	reload_docinfo(callback) {
		netmanthan.call({
			method: "netmanthan.desk.form.load.get_docinfo",
			args: {
				doctype: this.frm.doctype,
