...
netmanthan.ui.form.DocumentFollow = class DocumentFollow {
	constructor(opts) {
		$.extend(this, opts);
		if (!netmanthan.boot.user.document_follow_notify) {
	...
	render_sidebar() {
		...
		const document_follow_enabled = netmanthan.boot.user.document_follow_notify;
		const document_can_be_followed = netmanthan.get_meta(this.frm.doctype).track_changes;
		if (
			netmanthan.session.user === "Administrator" ||
	...
	bind_events() {
		this.follow_document_link.on("click", () => {
			...
			netmanthan.call({
				method: "netmanthan.desk.form.document_follow.follow_document",
				args: {
		...
		this.unfollow_document_link.on("click", () => {
			...
			netmanthan.call({
				method: "netmanthan.desk.form.document_follow.unfollow_document",
				args: {
					doctype: this.frm.doctype,
					doc_name: this.frm.doc.name,
					user: netmanthan.session.user,
	...
	get_followed_user() {
		...
		return new Promise((resolve) => {
			netmanthan
				.call({
					method: "netmanthan.desk.form.document_follow.get_follow_users",
					args: {
						doctype: this.frm.doctype,
						doc_name: this.frm.doc.name,
					},
				...
				.then((r) => {
					...
					for (var d in r.message) {
						...
						if (this.count_others < 4) {
							html += netmanthan.avatar(r.message[d].user, "avatar-small");
						}
