...
netmanthan.ui.form.Share = class Share {
	constructor(opts) {
		$.extend(this, opts);
	...
	render_sidebar() {
		...
		this.shares.append(netmanthan.avatar_group(shared_users, 5, { align: "left", overlap: true }));
	}
	show() {
		var me = this;
		...
		netmanthan.call({
			method: "netmanthan.share.get_users",
			args: {
	...
	render_shared(shared) {
		...
		$(
			netmanthan.render_template("set_sharing", {
				frm: this.frm,
				shared: this.shared,
				everyone: everyone,
			})
		...
		if (netmanthan.model.can_share(null, this.frm)) {
	...
	make_user_input() {
		// make add-user input
		this.dialog.share_with = netmanthan.ui.form.make_control({
	...
	set_edit_share_events() {
		...
		$(d.body)
			...
			.on("click", function () {
				...
				netmanthan.call({
					method: "netmanthan.share.set_permission",
					args: {
						doctype: me.frm.doctype,
						name: me.frm.doc.name,
						user: user,
