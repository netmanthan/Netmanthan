...
netmanthan.provide("netmanthan.success_action");

netmanthan.ui.form.SuccessAction = class SuccessAction {
	constructor(form) {
		this.form = form;
		this.load_setting();
	}
	...
	load_setting() {
		this.setting = netmanthan.boot.success_action.find(
	...
	show_alert() {
		netmanthan.db.get_list(this.form.doctype, { limit: 2 }).then((result) => {
	...
	get default_actions() {
		return {
			new: {
				label: __("New"),
				action: (frm) => netmanthan.new_doc(frm.doctype),
			},
			print: {
				label: __("Print"),
			...
			list: {
				label: __("View All"),
				action: (frm) => {
					netmanthan.set_route("List", frm.doctype);
				},
			},
		};
	}
