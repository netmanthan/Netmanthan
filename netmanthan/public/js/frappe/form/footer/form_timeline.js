...
class FormTimeline extends BaseTimeline {
		...
		if (this.frm.meta.allow_events_in_timeline) {
			let create_event = () => {
				const args = {
					...
					txt: netmanthan.markdown(this.frm.comment_box.get_value()),
				};
				return new netmanthan.views.InteractionComposer(args);
			};
	...
	setup_document_email_link() {
		...
		if (doc_info.document_email) {
			...
			this.document_email_link_wrapper.find(".document-email-link").on("click", (e) => {
				...
				netmanthan.utils.copy_to_clipboard(text);
			});
		}
	}
	...
	render_timeline_items() {
		...
		netmanthan.utils.bind_actions_with_object(this.timeline_items_wrapper, this);
	}

	set_document_info() {
		// TODO: handle creation via automation
		const creation = comment_when(this.frm.doc.creation);
		let creation_message = netmanthan.utils.is_current_user(this.frm.doc.owner)
		...
		let modified_message = netmanthan.utils.is_current_user(this.frm.doc.modified_by)
		...
		if (this.frm.doc.route && cint(netmanthan.boot.website_tracking_enabled)) {
			let route = this.frm.doc.route;
			netmanthan.utils.get_page_view_count(route).then((res) => {
	...
	get_user_link(user) {
		const user_display_text = (netmanthan.user_info(user).fullname || "").bold();
		return netmanthan.utils.get_form_link("User", user, true, user_display_text);
	}
	...
	get_view_timeline_contents() {
		...
		(this.doc_info.views || []).forEach((view) => {
			...
			let view_message = netmanthan.utils.is_current_user(view.owner)
	...
	async get_more_communication_timeline_contents() {
		...
		let response = await netmanthan.call({
			method: "netmanthan.desk.form.load.get_communications",
			args: {
	...
	get_communication_timeline_content(doc, allow_reply = true) {
		doc._url = netmanthan.utils.get_form_link("Communication", doc.name);
		...
		doc.content = netmanthan.dom.remove_script_and_style(doc.content);
		let communication_content = $(netmanthan.render_template("timeline_message_box", { doc }));
		if (allow_reply) {
	...
	get_comment_timeline_content(doc) {
		doc.content = netmanthan.dom.remove_script_and_style(doc.content);
		const comment_content = $(netmanthan.render_template("timeline_message_box", { doc }));
		...
		(this.doc_info.milestones || []).forEach((milestone_log) => {
			milestone_timeline_contents.push({
				...
				content: __("{0} changed {1} to {2}", [
					...
					netmanthan.meta.get_label(this.frm.doctype, milestone_log.track_field),
	...
	get_custom_timeline_contents() {
		...
		(this.doc_info.additional_timeline_content || []).forEach((custom_item) => {
			custom_timeline_contents.push({
				...
				content:
					custom_item.content ||
					netmanthan.render_template(custom_item.template, custom_item.template_data),
			});
		});
	...
	get_energy_point_timeline_contents() {
		...
		(this.doc_info.energy_point_logs || []).forEach((log) => {
			...
			energy_point_timeline_contents.push({
				...
				content: netmanthan.energy_points.format_form_log(log),
			});
		});
	...
	setup_reply(communication_box, communication_doc) {
		...
		let reply = $(`<a class="action-btn reply">${netmanthan.utils.icon("reply", "md")}</a>`).click(
			() => {
				this.compose_mail(communication_doc);
			}
		);
		let reply_all = $(
			`<a class="action-btn reply-all">${netmanthan.utils.icon("reply-all", "md")}</a>`
		).click(() => {
	...
	compose_mail(communication_doc = null, reply_all = false) {
		const args = {
			...
			frm: this.frm,
			recipients:
				communication_doc && communication_doc.sender != netmanthan.session.user_email
		...
		if (this.frm.doctype === "Communication") {
			...
			const comment_value = netmanthan.markdown(this.frm.comment_box.get_value());
			args.message = strip_html(comment_value) ? comment_value : "";
		}

		new netmanthan.views.CommunicationComposer(args);
	}
	...
	setup_comment_actions(comment_wrapper, doc) {
		...
		if (
			netmanthan.model.can_delete("Comment") &&
			(netmanthan.session.user == doc.owner || netmanthan.user.has_role("System Manager"))
		) {
			const delete_option = $(`
				<li>
		...
		let edit_button = $();
		let current_user = netmanthan.session.user;
	...
	make_editable(container) {
		return netmanthan.ui.form.make_control({
			parent: container,
			df: {
				fieldtype: "Comment",
				fieldname: "comment",
	...
	update_comment(name, content) {
		return netmanthan
			.xcall("netmanthan.desk.form.utils.update_comment", { name, content })
			.then(() => {
	...
	delete_comment(comment_name) {
		netmanthan.confirm(__("Delete comment?"), () => {
			return netmanthan
				.xcall("netmanthan.client.delete", {
					doctype: "Comment",
					name: comment_name,
				})
				.then(() => {
	...
	copy_link(ev) {
		let doc_link = netmanthan.urllib.get_full_url(
			netmanthan.utils.get_form_link(this.frm.doctype, this.frm.docname)
		);
		let element_id = $(ev.currentTarget).closest(".timeline-content").attr("id");
		netmanthan.utils.copy_to_clipboard(`${doc_link}#${element_id}`);
	}
}
