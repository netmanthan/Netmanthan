...
function get_version_timeline_content(version_doc, frm) {
	...
	if (!$.isEmptyObject(updater_reference)) {
		...
		if (doctype && docname) {
			updater_reference_link = netmanthan.utils.get_form_link(doctype, docname, true, label);
		} else {
	...
	if (data.changed && data.changed.length) {
		...
		data.changed.every(function (p) {
			if (p[0] === "docstatus") {
				...
				const df = netmanthan.meta.get_docfield(frm.doctype, p[0], frm.docname);
				if (df && !df.hidden) {
					const field_display_status = netmanthan.perm.get_field_display_status(
						df,
						null,
						frm.perm
					);
	...
	if (data.row_changed && data.row_changed.length) {
		...
		data.row_changed.every(function (row) {
			row[3].every(function (p) {
				...
				if (df && !df.hidden) {
					var field_display_status = netmanthan.perm.get_field_display_status(
						df,
						null,
						frm.perm
					);
					...
					if (field_display_status === "Read" || field_display_status === "Write") {
						parts.push(
							__("{0} from {1} to {2} in row #{3}", [
								netmanthan.meta.get_label(frm.fields_dict[row[0]].grid.doctype, p[0]),
								format_content_for_timeline(p[1]),
								format_content_for_timeline(p[2]),
								row[1],
							])
	...
	["added", "removed"].forEach(function (key) {
		if (data[key] && data[key].length) {
			let parts = (data[key] || []).map(function (p) {
				var df = netmanthan.meta.get_docfield(frm.doctype, p[0], frm.docname);
				if (df && !df.hidden) {
					var field_display_status = netmanthan.perm.get_field_display_status(
						df,
						null,
					...
					if (field_display_status === "Read" || field_display_status === "Write") {
						return __(netmanthan.meta.get_label(frm.doctype, p[0]));
					}
				}
			});
...
function get_version_comment(version_doc, text) {
	...
	if (text.includes("<a")) {
		...
		try {
			...
			if (unlinked_content) {
				version_comment += netmanthan.utils.get_form_link(
					"Version",
					version_doc.name,
					true,
					unlinked_content
	...
	return netmanthan.utils.get_form_link("Version", version_doc.name, true, text);
}
...
function format_content_for_timeline(content) {
	...
	// escapes HTML
	// and makes it bold
	content = netmanthan.ellipsis(content, 40) || '""';
	content = netmanthan.utils.escape_html(content);
	return content.bold();
}
