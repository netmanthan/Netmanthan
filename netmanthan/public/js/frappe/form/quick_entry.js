...
netmanthan.quick_edit = function (doctype, name) {
	netmanthan.db.get_doc(doctype, name).then((doc) => {
		netmanthan.ui.form.make_quick_entry(doctype, null, null, doc);
	});
};

netmanthan.ui.form.make_quick_entry = (doctype, after_insert, init_callback, doc, force) => {
	var trimmed_doctype = doctype.replace(/ /g, "");
	var controller_name = "QuickEntryForm";

	if (netmanthan.ui.form[trimmed_doctype + "QuickEntryForm"]) {
		controller_name = trimmed_doctype + "QuickEntryForm";
	}

	netmanthan.quick_entry = new netmanthan.ui.form[controller_name](
		doctype,
		after_insert,
		init_callback,
		doc,
	...
	return netmanthan.quick_entry.setup();
};

netmanthan.ui.form.QuickEntryForm = class QuickEntryForm {
	...
	check_quick_entry_doc() {
		if (!this.doc) {
			this.doc = netmanthan.model.get_new_doc(this.doctype, null, null, true);
		}
	}
	...
	render_dialog() {
		...
		this.dialog.onhide = () => (netmanthan.quick_entry = null);
		this.dialog.show();
	...
	insert() {
		...
		return new Promise((resolve) => {
			me.update_doc();
			netmanthan.call({
				method: "netmanthan.client.save",
				args: {
					doc: me.dialog.doc,
				},
				callback: function (r) {
					if (netmanthan.model.is_submittable(me.doctype)) {
						...
						me.dialog.hide();
						// delete the old doc
						netmanthan.model.clear_doc(me.dialog.doc.doctype, me.dialog.doc.name);
						me.dialog.doc = r.message;
						if (netmanthan._from_link) {
							netmanthan.ui.form.update_calling_link(me.dialog.doc);
						} else {
							if (me.after_insert) {
	...
	submit(doc) {
		var me = this;
		netmanthan.call({
			method: "netmanthan.client.submit",
			args: {
				doc: doc,
			},
			callback: function (r) {
	...
	open_form_if_not_list() {
		...
		if (route && !(route[0] === "List" && route[1] === doc.doctype)) {
			netmanthan.run_serially([() => netmanthan.set_route("Form", doc.doctype, doc.name)]);
		}
	}
	...
	open_doc(set_hooks) {
		...
		if (set_hooks && this.after_insert) {
			netmanthan.route_options = netmanthan.route_options || {};
			netmanthan.route_options.after_save = (frm) => {
				this.after_insert(frm);
			};
		}
		netmanthan.set_route("Form", this.doctype, this.doc.name);
	}
