...
netmanthan.ui.form.Layout = class Layout {
	constructor(opts) {
		this.views = {};
		this.pages = [];
		this.tabs = [];
	...
	get_doctype_fields() {
		...
		if (this.doctype_layout) {
			...
			fields = fields.concat(
				netmanthan.meta.sort_docfields(netmanthan.meta.docfield_map[this.doctype])
			);
		}
	...
	get_fields_from_layout() {
		...
		for (let f of this.doctype_layout.fields) {
			const docfield = copy_dict(netmanthan.meta.docfield_map[this.doctype][f.fieldname]);
	...
	init_field(df, parent, render = false) {
		const fieldobj = netmanthan.ui.form.make_control({
			df: df,
	...
	make_page(df) {
		...
		this.fold_btn = head.find(".btn-fold").on("click", function () {
			...
			if (page.hasClass("hide")) {
				...
				netmanthan.utils.scroll_to($(this), true, 30);
				me.folded = false;
			} else {
	...
	is_numeric_field_active() {
		const control = $(document.activeElement).closest(".netmanthan-control");
	...
	refresh_sections() {
		...
		this.wrapper.find(".form-section:not(.hide-control)").each(function () {
			...
			if (section.find(".netmanthan-control:not(.hide-control)").length) {
	...
	attach_doc_and_docfields(refresh) {
		...
		for (let i = 0, l = this.fields_list.length; i < l; i++) {
			...
			if (me.doc) {
				...
				fieldobj.df =
					netmanthan.meta.get_docfield(me.doc.doctype, fieldobj.df.fieldname, me.doc.name) ||
					fieldobj.df;
			}
		...
		for (let i = start_idx + 1, len = fields.length; i < len; i++) {
			...
			if (this.is_visible(field)) {
				if (field.df.fieldtype === "Table") {
					...
					return true;
				} else if (!in_list(netmanthan.model.no_value_type, field.df.fieldtype)) {
