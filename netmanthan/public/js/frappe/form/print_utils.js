netmanthan.ui.get_print_settings = function (pdf, callback, letter_head, pick_columns) {
	var print_settings = locals[":Print Settings"]["Print Settings"];

	var default_letter_head =
		locals[":Company"] && netmanthan.defaults.get_default("company")
			? locals[":Company"][netmanthan.defaults.get_default("company")]["default_letter_head"]
			: "";
	...
	var columns = [
		...
		},
		{
			...
			options: Object.keys(netmanthan.boot.letter_heads),
	...
	return netmanthan.prompt(
		...
		function (data) {
			...
			if (data.letter_head) {
				data.letter_head = netmanthan.boot.letter_heads[print_settings.letter_head];
			}
			callback(data);
		},
		__("Print Settings")
...
netmanthan.ui.form.qz_connect = function () {
	return new Promise(function (resolve, reject) {
		netmanthan.ui.form.qz_init().then(() => {
			if (qz.websocket.isActive()) {
				// if already active, resolve immediately
				// netmanthan.show_alert({message: __('QZ Tray Connection Active!'), indicator: 'green'});
				resolve();
			} else {
				// try to connect once before firing the mimetype launcher
...
netmanthan.ui.form.qz_init = function () {
	...
	return new Promise((resolve) => {
		if (typeof qz === "object" && typeof qz.version === "string") {
			...
			let qz_required_assets = [
				"/assets/netmanthan/node_modules/js-sha256/build/sha256.min.js",
				"/assets/netmanthan/node_modules/qz-tray/qz-tray.js",
			];
			netmanthan.require(qz_required_assets, () => {
			...
			// note 'netmanthan.require' does not have callback on fail. Hence, any failure cannot be communicated to the user.
		}
	});
};

netmanthan.ui.form.qz_get_printer_list = function () {
	// returns the list of printers that are available to the QZ Tray
	return netmanthan.ui.form
		.qz_connect()
		.then(function () {
...
netmanthan.ui.form.qz_success = function () {
...
netmanthan.ui.form.qz_fail = function (e) {
	// notify qz errors
	netmanthan.show_alert(
		{
