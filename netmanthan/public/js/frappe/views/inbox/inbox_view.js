/**
 * netmanthan.views.InboxView
 */

netmanthan.provide("netmanthan.views");

netmanthan.views.InboxView = class InboxView extends netmanthan.views.ListView {
	static load_last_view() {
		const route = netmanthan.get_route();
		if (!route[3] && netmanthan.boot.email_accounts.length) {
			let email_account;
			if (netmanthan.boot.email_accounts[0].email_id == "All Accounts") {
				email_account = "All Accounts";
			} else {
				email_account = netmanthan.boot.email_accounts[0].email_account;
			}
			netmanthan.set_route("List", "Communication", "Inbox", email_account);
			return true;
		...
		function is_valid(email_account) {
			return netmanthan.boot.email_accounts.find((d) => d.email_account === email_account);
		}
	}
	...
	setup_defaults() {
		...
		this.email_account = netmanthan.get_route()[3];
	...
	get_seen_class(doc) {
		const seen =
			Boolean(doc.seen) || JSON.parse(doc._seen || "[]").includes(netmanthan.session.user)
				? ""
				: "bold";
		return seen;
	}
	...
	get_meta_html(email) {
		...
		if (email.reference_doctype && email.reference_doctype !== this.doctype) {
			link = `<a class="text-muted grey"
				href="${netmanthan.utils.get_form_link(email.reference_doctype, email.reference_name)}"
	...
	get_inbox_filters() {
		...
		if (email_account === "Sent") {
			...
			filters = default_filters.concat([
				...
				["Communication", "email_account", "in", netmanthan.boot.all_accounts, true],
			]);
		} else {
			var op = "=";
			if (email_account == "All Accounts") {
				op = "in";
				email_account = netmanthan.boot.all_accounts;
			}
	...
	get_no_result_message() {
		...
		} else if (!email_account && !netmanthan.boot.email_accounts.length) {
		...
		const html = netmanthan.model.can_create(args.doctype)
			? `<p>${args.msg}</p>
			<p>
	...
	make_new_doc() {
		if (!this.email_account && !netmanthan.boot.email_accounts.length) {
			netmanthan.route_options = {
				email_id: netmanthan.session.user_email,
			};
			netmanthan.new_doc("Email Account");
		} else {
