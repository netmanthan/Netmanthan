...
netmanthan.provide("netmanthan.interaction_settings");

netmanthan.views.InteractionComposer = class InteractionComposer {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	get_event_categories() {
		let me = this;
		netmanthan.model.with_doctype("Event", () => {
			let categories = netmanthan.meta
	...
	setup_attach() {
		...
		attach.find(".add-more-attachments a").on("click", () => new netmanthan.ui.FileUploader(args));
		this.render_attach();
	}
	...
	create_interaction(btn, form_values, selected_attachments) {
		...
		if (!("owner" in interaction_values)) {
			interaction_values["owner"] = netmanthan.session.user;
		}
		if (!("assigned_by" in interaction_values) && interaction_values["doctype"] == "ToDo") {
			interaction_values["assigned_by"] = netmanthan.session.user;
		}
	...
	add_attachments(doc, attachments) {
		netmanthan.call({
			method: "netmanthan.utils.file_manager.add_attachments",
			args: {
				doctype: doc.doctype,
				name: doc.name,
