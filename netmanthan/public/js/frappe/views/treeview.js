...
netmanthan.provide("netmanthan.treeview_settings");
netmanthan.provide("netmanthan.views.trees");
window.cur_tree = null;

netmanthan.views.TreeFactory = class TreeFactory extends netmanthan.views.Factory {
	make(route) {
		netmanthan.model.with_doctype(route[1], function () {
			var options = {
				doctype: route[1],
				meta: netmanthan.get_meta(route[1]),
			};

			if (
				!netmanthan.treeview_settings[route[1]] &&
				!netmanthan.meta.get_docfield(route[1], "is_group")
			) {
				netmanthan.msgprint(__("Tree view is not available for {0}", [route[1]]));
				return false;
			}
			$.extend(options, netmanthan.treeview_settings[route[1]] || {});
			netmanthan.views.trees[options.doctype] = new netmanthan.views.TreeView(options);
		});
	}
	...
	on_show() {
		...
		let treeview = netmanthan.views.trees[route[1]];
...
netmanthan.views.TreeView = class TreeView {
	constructor(opts) {
		var me = this;
		...
		this.get_tree_nodes = me.opts.get_tree_nodes || "netmanthan.desk.treeview.get_children";
	...
	get_permissions() {
		this.can_read = netmanthan.model.can_read(this.doctype);
		this.can_create =
			netmanthan.boot.user.can_create.indexOf(this.doctype) !== -1 ||
			netmanthan.boot.user.in_create.indexOf(this.doctype) !== -1;
		this.can_write = netmanthan.model.can_write(this.doctype);
		this.can_delete = netmanthan.model.can_delete(this.doctype);
	}
	make_page() {
		var me = this;
		this.parent = netmanthan.container.add_page(this.page_name);
		netmanthan.ui.make_app_page({ parent: this.parent, single_column: true });
		...
		this.page.main.addClass("netmanthan-card");
	...
	make_filters() {
		var me = this;
		netmanthan.treeview_settings.filters = [];
		$.each(this.opts.filters || [], function (i, filter) {
			if (netmanthan.route_options && netmanthan.route_options[filter.fieldname]) {
				filter.default = netmanthan.route_options[filter.fieldname];
			}
	...
	make_tree() {
		...
		this.tree = new netmanthan.ui.Tree({
			parent: this.body,
			label: use_label,
	...
	rebuild_tree() {
		...
		netmanthan.call({
			method: "netmanthan.utils.nestedset.rebuild_tree",
			args: {
				doctype: me.doctype,
	...
	select_node(node) {
		...
		if (this.opts.view_template) {
			...
			$(
				netmanthan.render_template(me.opts.view_template, {
					data: node.data,
					doctype: me.doctype,
				})
		...
		var toolbar = [
			{
				...
				click: function (node) {
					netmanthan.set_route("Form", me.doctype, node.label);
				},
			},
			{
				label: __("Add Child"),
			...
			},
			{
				...
				condition: function (node) {
					...
					if (me.doctype && netmanthan.get_meta(me.doctype)) {
						if (!netmanthan.get_meta(me.doctype).allow_rename) allow_rename = false;
					}
					return !node.is_root && me.can_write && allow_rename;
				},
				click: function (node) {
					netmanthan.model.rename_doc(me.doctype, node.label, function (new_name) {
			...
			},
			{
				...
				click: function (node) {
					netmanthan.model.delete_doc(me.doctype, node.label, function () {
						node.parent.remove();
					});
				},
				btnClass: "hidden-xs",
	...
	new_node() {
		...
		if (!(node && node.expandable)) {
			netmanthan.msgprint(__("Select a group node first."));
			return;
		}
		...
		d.set_primary_action(__("Create New"), function () {
			...
			d.hide();
			netmanthan.dom.freeze(__("Creating {0}", [me.doctype]));

			$.extend(args, v);
			return netmanthan.call({
				method: me.opts.add_tree_node || "netmanthan.desk.treeview.add_node",
				args: args,
				callback: function (r) {
					if (!r.exc) {
	...
	print_tree() {
		if (!netmanthan.model.can_print(this.doctype)) {
			netmanthan.msgprint(__("You are not allowed to print this report"));
			return false;
		}
		var tree = $(".tree:visible").html();
		var me = this;
		netmanthan.ui.get_print_settings(false, function (print_settings) {
			var title = __(me.docname || me.doctype);
			netmanthan.render_tree({ title: title, tree: tree, print_settings: print_settings });
			netmanthan.call({
				method: "netmanthan.core.doctype.access_log.access_log.make_access_log",
				args: {
					doctype: me.doctype,
	...
	set_menu_item() {
		...
		this.menu_items = [
			{
				label: __("View List"),
				action: function () {
					netmanthan.set_route("List", me.doctype);
				},
			},
			{
				label: __("Print"),
		...
		if (
			...
			netmanthan.meta.has_field(me.doctype, "lft") &&
			netmanthan.meta.has_field(me.doctype, "rgt")
		) {
