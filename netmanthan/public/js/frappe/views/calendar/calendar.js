...
netmanthan.provide("netmanthan.views.calendar");
netmanthan.provide("netmanthan.views.calendars");

netmanthan.views.CalendarView = class CalendarView extends netmanthan.views.ListView {
	static load_last_view() {
		const route = netmanthan.get_route();
		if (route.length === 3) {
			const doctype = route[1];
			const user_settings = netmanthan.get_user_settings(doctype)["Calendar"] || {};
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.calendar_settings = netmanthan.views.calendar[this.doctype] || {};
			this.calendar_name = netmanthan.get_route()[3];
		});
	}
	...
	render() {
		...
		this.load_lib
			...
			.then((options) => {
				this.calendar = new netmanthan.views.Calendar(options);
			});
	}
	...
	get_calendar_preferences() {
		...
		return new Promise((resolve) => {
			if (calendar_name === "default") {
				Object.assign(options, netmanthan.views.calendar[this.doctype]);
				resolve(options);
			} else {
				netmanthan.model.with_doc("Calendar View", calendar_name, () => {
					const doc = netmanthan.get_doc("Calendar View", calendar_name);
					if (!doc) {
						...
						netmanthan.set_route("List", this.doctype, "Calendar", "default");
						return;
					}
	...
	get required_libs() {
		let assets = [
			"assets/netmanthan/js/lib/fullcalendar/fullcalendar.min.css",
			"assets/netmanthan/js/lib/fullcalendar/fullcalendar.min.js",
		];
		let user_language = netmanthan.boot.user.language;
		if (user_language && user_language !== "en") {
			assets.push("assets/netmanthan/js/lib/fullcalendar/locale-all.js");
		}
		return assets;
	}
};

netmanthan.views.Calendar = class Calendar {
	constructor(options) {
		$.extend(this, options);
	...
	make_page() {
		...
		$.each(netmanthan.boot.calendars, function (i, doctype) {
			if (netmanthan.model.can_read(doctype)) {
				me.page.add_menu_item(__(doctype), function () {
					netmanthan.set_route("List", doctype, "Calendar");
				});
			}
		});
	...
	make() {
		...
		this.footnote_area = netmanthan.utils.set_footnote(
			this.footnote_area,
			this.$wrapper,
	...
	set_css() {
		...
		this.$wrapper
			...
			.attr("class", "")
			.html(netmanthan.utils.icon("left"));
		this.$wrapper
			.find(".fc-next-button span")
			.attr("class", "")
			.html(netmanthan.utils.icon("right"));

		this.$wrapper.find(".fc-today-button").prepend(netmanthan.utils.icon("today"));
	...
	get_system_datetime(date) {
		date._offset = moment(date).tz(netmanthan.sys_defaults.time_zone)._offset;
		return netmanthan.datetime.convert_to_system_tz(date);
	}
	setup_options(defaults) {
		var me = this;
		defaults.meridiem = "false";
		this.cal_options = {
			locale: netmanthan.boot.user.language || "en",
			header: {
			...
			events: function (start, end, timezone, callback) {
				return netmanthan.call({
					method: me.get_events_method || "netmanthan.desk.calendar.get_events",
					type: "GET",
			...
			eventClick: function (event) {
				...
				if (netmanthan.model.can_read(doctype)) {
					netmanthan.set_route("Form", doctype, event.name);
				}
			},
			...
			select: function (startDate, endDate, jsEvent, view) {
				...
				var event = netmanthan.model.get_new_doc(me.doctype);
				...
				netmanthan.set_route("Form", me.doctype, event.name);
			},
	...
	prepare_events(events) {
		...
		return (events || []).map((d) => {
			d.id = d.name;
			d.editable = netmanthan.model.can_write(d.doctype || me.doctype);
			...
			if (d.convertToUserTz) {
				d.start = netmanthan.datetime.convert_to_user_tz(d.start);
				d.end = netmanthan.datetime.convert_to_user_tz(d.end);
			}

			// show event on single day if start or end date is invalid
			if (!netmanthan.datetime.validate(d.start) && d.end) {
				d.start = netmanthan.datetime.add_days(d.end, -1);
			}

			if (d.start && !netmanthan.datetime.validate(d.end)) {
				d.end = netmanthan.datetime.add_days(d.start, 1);
			}

			me.fix_end_date_for_event_render(d);
			me.prepare_colors(d);

			d.title = netmanthan.utils.html2text(d.title);
	...
	prepare_colors(d) {
		...
		if (this.get_css_class) {
			...
			if (color_name.startsWith("#")) {
				color_name = netmanthan.ui.color.validate_hex(color_name) ? color_name : "blue";
			}

			d.backgroundColor = netmanthan.ui.color.get(color_name, "extra-light");
			d.textColor = netmanthan.ui.color.get(color_name, "dark");
		} else {
			color = d.color;
			if (!netmanthan.ui.color.validate_hex(color) || !color) {
				color = netmanthan.ui.color.get("blue", "extra-light");
			}
			d.backgroundColor = color;
			d.textColor = netmanthan.ui.color.get_contrast_color(color);
		}
		return d;
	}
	update_event(event, revertFunc) {
		var me = this;
		netmanthan.model.remove_from_locals(me.doctype, event.name);
		return netmanthan.call({
			method: me.update_event_method || "netmanthan.desk.calendar.update_event",
			args: me.get_update_args(event),
			callback: function (r) {
				if (r.exc) {
					netmanthan.show_alert(__("Unable to update event"));
					revertFunc();
				}
			},
			error: function () {
