...
import DataTable from "netmanthan-datatable";

netmanthan.provide("netmanthan.widget.utils");
netmanthan.provide("netmanthan.views");
netmanthan.provide("netmanthan.query_reports");

netmanthan.standard_pages["query-report"] = function () {
	var wrapper = netmanthan.container.add_page("query-report");
	...
	netmanthan.query_report = new netmanthan.views.QueryReport({
		parent: wrapper,
	});

	$(wrapper).bind("show", function () {
		netmanthan.query_report.show();
	});
};

netmanthan.views.QueryReport = class QueryReport extends netmanthan.views.BaseList {
	show() {
	...
	init() {
		...
		this.init_promise = netmanthan.run_serially(tasks);
		return this.init_promise;
	}
	...
	setup_defaults() {
		...
		this.refresh = netmanthan.utils.throttle(this.refresh, 300);
	...
	update_url_with_filters() {
		if (netmanthan.get_route_str() == this.page_name) {
	...
	get_no_result_message() {
		return `<div class="msg-box no-border">
			<div>
				<img src="/assets/netmanthan/images/ui-states/list-empty-state.svg" alt="Generic Empty State" class="null-state">
			</div>
	...
	setup_events() {
		netmanthan.realtime.on("report_generated", (data) => {
			...
			if (data.report_name) {
				...
				if (data.name == this.prepared_report_doc_name) {
					...
					netmanthan.show_alert({ message: alert_message, indicator: "orange" });
				}
			}
		});
	...
	load() {
		if (netmanthan.get_route().length < 2) {
			this.toggle_nothing_to_show(true);
			return;
		}

		let route_options = {};
		route_options = Object.assign(route_options, netmanthan.route_options);

		if (this.report_name !== netmanthan.get_route()[1]) {
			// different report
			this.load_report(route_options);
		} else if (netmanthan.has_route_options()) {
	...
	add_card_button_to_toolbar() {
		if (!netmanthan.model.can_create("Number Card")) return;
	...
	add_chart_buttons_to_toolbar(show) {
		if (!netmanthan.model.can_create("Dashboard Chart")) return;
		if (show) {
	...
	add_card_to_dashboard() {
		let field_options = netmanthan.report_utils.get_field_options_from_report(
			this.columns,
			this.raw_data
		);
		const dashboard_field = netmanthan.dashboard_utils.get_dashboard_link_field();
		const set_standard = netmanthan.boot.developer_mode;

		const dialog = new netmanthan.ui.Dialog({
	...
	add_chart_to_dashboard() {
		if (this.chart_fields || this.chart_options) {
			...
			netmanthan.msgprint(__("Please Set Chart"));
		}
	}
	...
	create_number_card(values, dashboard_name, card_name, set_standard) {
		...
		this.add_to_dashboard(
			"netmanthan.desk.doctype.number_card.number_card.create_report_number_card",
			args,
			dashboard_name,
			card_name,
			"Number Card"
	...
	create_dashboard_chart(chart_args, dashboard_name, chart_name, set_standard) {
		let args = {
			...
			chart_type: "Report",
			report_name: this.report_name,
			type: chart_args.chart_type || netmanthan.model.unscrub(chart_args.type),
		...
		this.add_to_dashboard(
			"netmanthan.desk.doctype.dashboard_chart.dashboard_chart.create_report_chart",
			args,
			dashboard_name,
			chart_name,
			"Dashboard Chart"
	...
	add_to_dashboard(method, args, dashboard_name, name, doctype) {
		netmanthan.xcall(method, { args: args }).then(() => {
			let message;
			if (dashboard_name) {
			...
			netmanthan.msgprint(message, __("New {0} Created", [__(doctype)]));
		});
	}
	...
	refresh_report(route_options) {
		...
		return netmanthan.run_serially([
			() => this.setup_filters(),
	...
	get_report_doc() {
		return netmanthan.model
			.with_doc("Report", this.report_name)
			.then((doc) => {
				this.report_doc = doc;
			})
			.then(() => netmanthan.model.with_doctype(this.report_doc.ref_doctype));
	}

	get_report_settings() {
		return new Promise((resolve, reject) => {
			if (netmanthan.query_reports[this.report_name]) {
				this.report_settings = netmanthan.query_reports[this.report_name];
				resolve();
			} else {
				netmanthan
					.xcall("netmanthan.desk.query_report.get_script", {
						report_name: this.report_name,
					})
					.then((settings) => {
						netmanthan.dom.eval(settings.script || "");
						netmanthan.after_ajax(() => {
							...
							netmanthan.query_reports[this.report_name] = this.report_settings;
	...
	get_local_report_settings(custom_report_name) {
		let report_script_name =
			this.report_doc.report_type === "Custom Report"
				...
				: this.report_name;
		return netmanthan.query_reports[report_script_name] || {};
	}
	...
	setup_progress_bar() {
		...
		this.interval = setInterval(function () {
			seconds_elapsed += 1;
			netmanthan.show_progress(__("Preparing Report"), seconds_elapsed, execution_time);
		}, 1000);
	}
	...
	evaluate_depends_on_value(expression, filter_label) {
		...
		if (doc) {
			if (typeof expression === "boolean") {
				...
				try {
					out = netmanthan.utils.eval(expression.substr(5), { doc });
				} catch (e) {
					netmanthan.throw(
	...
	set_route_filters(route_options) {
		if (!route_options) route_options = netmanthan.route_options;
		...
		if (route_options) {
			...
			return netmanthan.run_serially(promises);
		}
	}
	...
	refresh(have_filters_changed) {
		...
		return new Promise((resolve) => {
			this.last_ajax = netmanthan.call({
				method: "netmanthan.desk.query_report.run",
				type: "GET",
				args: {
					report_name: this.report_name,
					filters: filters,
			...
			.then((r) => {
				...
				if (data.result && data.result.length) {
					...
					if (this.chart_options) {
						...
						this.$chart.empty();
						if (this.chart_fields) {
							this.chart_options = netmanthan.report_utils.make_chart_options(
								this.columns,
								this.raw_data,
								this.chart_fields
							);
	...
	render_summary(data) {
		data.forEach((summary) => {
			netmanthan.utils.build_summary_item(summary).appendTo(this.$summary);
		});
	...
	get_query_params() {
		const query_string = netmanthan.utils.get_query_string(netmanthan.get_route_str());
		const query_params = netmanthan.utils.get_query_params(query_string);
		return query_params;
	}
	...
	add_prepared_report_buttons(doc) {
		if (doc) {
			this.page.add_inner_button(__("Download Report"), function () {
				window.open(
					netmanthan.urllib.get_full_url(
						"/api/method/netmanthan.core.doctype.prepared_report.prepared_report.download_attachment?" +
							"dn=" +
			...
			let pretty_diff = netmanthan.datetime.comment_when(doc.report_end_time);
			const days_old = netmanthan.datetime.get_day_diff(
				netmanthan.datetime.now_datetime(),
				doc.report_end_time
			);
			if (days_old > 1) {
		...
		this.primary_action_map = {
			...
			Edit: {
				label: __("Edit"),
				click: () => {
					netmanthan.set_route(netmanthan.get_route());
				},
			},
			Rebuild: {
				label: __("Rebuild"),
	...
	show_warning_or_generate_report() {
		netmanthan
			.xcall(
				"netmanthan.core.doctype.prepared_report.prepared_report.get_reports_in_queued_state",
				{
			...
			.then((reports) => {
				...
				if (reports.length) {
					...
					this.prepared_report_dialog = netmanthan.warn(
					...
					netmanthan.utils.bind_actions_with_object(
	...
	delete_old_queued_reports() {
		...
		netmanthan
			.xcall("netmanthan.core.doctype.prepared_report.prepared_report.delete_prepared_reports", {
	...
	generate_background_report() {
		...
		if (!missing_mandatory.length) {
			...
			return new Promise((resolve) =>
				netmanthan.call({
					method: "netmanthan.desk.query_report.background_enqueue_run",
					args: {
						report_name: this.report_name,
						filters: filters,
					},
				...
				let alert_message =
					...
					`</a>`;
				netmanthan.show_alert({ message: alert_message, indicator: "orange" }, 10);
				this.toggle_nothing_to_show(true);
			});
		}
	}
	...
	render_datatable() {
		...
		if (
			...
			let datatable_options = {
				...
				data: data,
				inlineFilters: true,
				language: netmanthan.boot.lang,
				translations: netmanthan.utils.datatable.get_translations(),
				treeView: this.tree_report,
				layout: "fixed",
				cellHeight: 33,
				showTotalRow: this.raw_data.add_total_row && !this.report_settings.tree,
				direction: netmanthan.utils.is_rtl() ? "rtl" : "ltr",
				hooks: {
					columnTotal: netmanthan.utils.report_column_total,
				},
			};
	...
	get_chart_options(data) {
		...
		options.axisOptions = {
			...
			numberFormatter: netmanthan.utils.format_chart_axis_number,
		};
		options.height = 280;
		return options;
	}
	...
	render_chart(options) {
		this.$chart.empty();
		this.$chart.show();
		this.chart = new netmanthan.Chart(this.$chart[0], options);
	}
	...
	open_create_chart_dialog() {
		...
		function preview_chart() {
			...
			if (values.x_field && values.y_fields.length) {
				let options = netmanthan.report_utils.make_chart_options(
					me.columns,
					me.raw_data,
					values
				);
				...
				wrapper.empty();
				new netmanthan.Chart(wrapper[0], options);
	...
	prepare_columns(columns) {
		return columns.map((column) => {
			column = netmanthan.report_utils.prepare_field_from_column(column);
			...
			if (column.fieldtype === "Date") {
				compareFn = (cell, keyword) => {
					...
					const keywordValue = netmanthan.datetime.user_to_obj(keyword);
					const cellValue = netmanthan.datetime.str_to_obj(cell.content);
	...
	set_breadcrumbs() {
		...
		const ref_doctype = netmanthan.get_meta(this.report_doc.ref_doctype);
		netmanthan.breadcrumbs.add(ref_doctype.module);
	}

	make_access_log(method, file_format) {
		netmanthan.call("netmanthan.core.doctype.access_log.access_log.make_access_log", {
	...
	pdf_report(print_settings) {
		const base_url = netmanthan.urllib.get_base_url();
		const print_css = netmanthan.boot.print_css;
		const landscape = print_settings.orientation == "Landscape";
		...
		const content = netmanthan.render_template(template, {
			title: __(this.report_name),
			subtitle: filters_html,
		...
		const html = netmanthan.render_template("print_template", {
			title: __(this.report_name),
			content: content,
			base_url: base_url,
			print_css: print_css,
			...
			landscape: landscape,
			columns: columns,
			lang: netmanthan.boot.lang,
			layout_direction: netmanthan.utils.is_rtl() ? "rtl" : "ltr",
			can_use_smaller_font: this.report_doc.is_standard === "Yes" && custom_format ? 0 : 1,
		});
		...
		netmanthan.render_pdf(html, print_settings);
	}
	...
	get_filters_html_for_print() {
		...
		return Object.keys(applied_filters)
			.map((fieldname) => {
				const docfield = netmanthan.query_report.get_filter(fieldname).df;
				const value = applied_filters[fieldname];
				return `<h6>${__(docfield.label)}: ${netmanthan.format(value, docfield)}</h6>`;
			})
			.join("");
	}
	...
	export_report() {
		...
		this.export_dialog = netmanthan.prompt(
			...
			({ file_format, include_indentation }) => {
				...
				if (file_format === "CSV") {
					...
					netmanthan.tools.downloadify(out, null, this.report_name);
				} else {
					let filters = this.get_filter_values(true);
					if (netmanthan.urllib.get_dict("prepared_report_name")) {
						filters = Object.assign(
							netmanthan.urllib.get_dict("prepared_report_name"),
							filters
						);
					}
					...
					const args = {
						cmd: "netmanthan.desk.query_report.export_query",
						report_name: this.report_name,
					...
					open_url_post(netmanthan.request.url, args);
				}
			},
		...
		return rows.map((row) => {
			...
			return row.slice(standard_column_count).map((cell, i) => {
				if (cell.column.fieldtype === "Duration") {
					cell.content = netmanthan.utils.get_formatted_duration(cell.content);
				}
	...
	get_menu_items() {
		let items = [
			...
			{
				label: __("Edit"),
				action: () => netmanthan.set_route("Form", "Report", this.report_name),
				condition: () => netmanthan.user.is_report_manager(),
				standard: true,
			},
			{
				label: __("Print"),
				action: () => {
					let dialog = netmanthan.ui.get_print_settings(
						false,
				...
				condition: () => netmanthan.model.can_print(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("PDF"),
			...
			},
			{
				...
				condition: () => netmanthan.model.can_export(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("Setup Auto Email"),
				action: () =>
					netmanthan.set_route("List", "Auto Email Report", { report: this.report_name }),
				standard: true,
			},
			{
				label: __("Add Column"),
				action: () => {
					let d = new netmanthan.ui.Dialog({
						...
						fields: [
							{
								...
								change: () => {
									...
									netmanthan.model.with_doctype(doctype, () => {
										let options = netmanthan.meta
											.get_docfields(doctype)
											.filter(netmanthan.model.is_value_type)
											.map((df) => ({
												label: df.label,
												value: df.fieldname,
											}));
						...
						primary_action: (values) => {
							...
							let df = netmanthan.meta.get_docfield(values.doctype, values.field);
							...
							netmanthan.call({
								method: "netmanthan.desk.query_report.get_data_for_custom_field",
								args: {
									field: values.field,
									doctype: values.doctype,
									names: Array.from(
			...
			},
			{
				...
				condition: () => netmanthan.model.can_set_user_permissions("Report"),
				standard: true,
			},
		];

		if (netmanthan.user.is_report_manager()) {
			items.push({
				label: __("Save"),
				action: () => {
					let d = new netmanthan.ui.Dialog({
						...
						primary_action: (values) => {
							netmanthan.call({
								method: "netmanthan.desk.query_report.save_report",
								args: {
								...
								callback: function (r) {
									this.show_save = false;
									d.hide();
									netmanthan.set_route("query-report", r.message);
								},
							});
						},
					});
...
Object.defineProperty(netmanthan, "query_report_filters_by_name", {
	get() {
		console.warn(
			"[Query Report] netmanthan.query_report_filters_by_name is deprecated. Please use the new api: netmanthan.query_report.get_filter_value(fieldname) and netmanthan.query_report.set_filter_value(fieldname, value)"
		);
		return null;
	},
});
