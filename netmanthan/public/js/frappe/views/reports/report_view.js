...
netmanthan.views.ReportView = class ReportView extends netmanthan.views.ListView {
	get view_name() {
		return "Report";
	}
	...
	setup_events() {
		...
		netmanthan.realtime.on("list_update", (data) => this.on_update(data));
	}
	...
	setup_sort_selector() {
		this.sort_selector = new netmanthan.ui.SortSelector({
		...
		this.group_by_control = new netmanthan.ui.GroupBy(this);
	...
	before_refresh() {
		if (this.report_doc) {
			// don't parse netmanthan.route_options if this is a Custom Report
	...
	set_dirty_state_for_custom_report() {
		...
		if (!netmanthan.utils.deep_equal(current_settings, report_settings)) {
	...
	prepare_data(r) {
		let data = r.message || {};
		data = netmanthan.utils.dict(data.keys, data.values);
	...
	on_update(data) {
		if (this.doctype === data.doctype && data.name) {
			...
			const flash_row = data.user !== netmanthan.session.user;
	...
	setup_charts() {
		...
		for (let col of this.columns) {
			...
			// numeric values in y
			if (col.docfield && netmanthan.model.numeric_fieldtypes.includes(col.docfield.fieldtype)) {
				y_fields.push({
					label: col.content,
					fieldname: col.id,
					value: col.id,
			...
			fields: [
				...
				},
				{
					...
					default: defaults.chart_type
						? netmanthan.utils.to_title_case(defaults.chart_type)
						: "Bar",
				},
			],
			primary_action: (data) => {
	...
	make_chart() {
		...
		this.chart = new netmanthan.Chart(this.$charts_wrapper.find(".charts-inner-wrapper")[0], {
			...
			tooltipOptions: {
				formatTooltipY: (value) => {
					return netmanthan.format(value, get_df(this.chart_args.y_axes[0]), {
	...
	render_editing_input(colIndex, value, parent) {
		...
		if (col.docfield.fieldtype === "Text Editor") {
			...
			// make control
			control = netmanthan.ui.form.make_control({
				df: col.docfield,
				parent: parent,
				render_input: true,
			});
	...
	is_editable(df, data) {
		if (
			df &&
			netmanthan.model.can_write(this.doctype) &&
			...
			netmanthan.model.is_non_std_field(df.fieldname)
		)
			return true;
		return false;
	}
	...
	set_default_fields() {
		...
		const fields = this.meta.fields.filter((df) => {
			return (
				...
				netmanthan.perm.has_perm(this.doctype, df.permlevel, "read") &&
				netmanthan.model.is_value_type(df.fieldtype) &&
				!df.report_hide
			);
		});
	...
	get_fields() {
		let fields = this.fields.map((f) => {
			let column_name = netmanthan.model.get_full_column_name(f[0], f[1]);
			if (f[1] !== this.doctype) {
				// child table field
		...
		const cdt_name_fields = this.get_unique_cdt_in_view().map(
			(cdt) => netmanthan.model.get_full_column_name("name", cdt) + " as " + `'${cdt}:name'`
		);
	...
	add_currency_column(fieldname, doctype, col_index) {
		...
		const df = netmanthan.meta.get_docfield(doctype, fieldname);
		if (
			df &&
			df.fieldtype === "Currency" &&
			df.options &&
			!df.options.includes(":") &&
			netmanthan.meta.has_field(doctype, df.options)
		) {
			...
			const field_label = netmanthan.meta.get_label(doctype, field[0]);
			netmanthan.show_alert(
	...
	remove_column_from_datatable(column) {
		...
		if (field[0] === "name") {
			this.refresh();
			netmanthan.throw(__("Cannot remove ID field"));
		}
	...
	get_columns_for_picker() {
		...
		const standard_fields_filter = (df) => !in_list(netmanthan.model.no_value_type, df.fieldtype);

		let doctype_fields = netmanthan.meta
			.get_docfields(this.doctype)
			.filter(standard_fields_filter);

		// filter out docstatus field from picker
		let std_fields = netmanthan.model.std_fields.filter((df) => df.fieldname !== "docstatus");

		// add status field derived from docstatus, if status is not a standard field
		let has_status_values = false;

		if (this.data) {
			has_status_values = netmanthan.get_indicator(this.data[0], this.doctype);
		}

		if (!netmanthan.meta.has_field(this.doctype, "status") && has_status_values) {
			doctype_fields = [
				{
					label: __("Status"),
					fieldname: "docstatus",
		...
		const table_fields = netmanthan.meta.get_table_fields(this.doctype);

		table_fields.forEach((df) => {
			const cdt = df.options;
			const child_table_fields = netmanthan.meta
				.get_docfields(cdt)
				.filter(standard_fields_filter);
	...
	get_dialog_fields() {
		...
		const table_fields = netmanthan.meta.get_table_fields(this.doctype).filter((df) => !df.hidden);
		...
		for (let f of this.fields) {
			...
			if (f[0] !== "docstatus") {
				...
				if (
					...
					!netmanthan.meta.has_field(this.doctype, "status")
				) {
	...
	build_column(c) {
		...
		let docfield = netmanthan.meta.docfield_map[doctype || this.doctype][fieldname];
		...
		if (!docfield) {
			docfield = netmanthan.model.get_std_field(fieldname, true);
			...
			if (docfield) {
				...
				if (fieldname == "docstatus" && !netmanthan.meta.has_field(this.doctype, "status")) {
		...
		const editable =
			netmanthan.model.is_non_std_field(fieldname) &&
			!docfield.read_only &&
			!docfield.is_virtual;

		const align = (() => {
			const is_numeric = netmanthan.model.is_numeric_field(docfield);
			if (is_numeric) {
				return "right";
			}
		...
		return {
			...
			format: (value, row, column, data) => {
				...
				return netmanthan.format(value, column.docfield, { always_show_decimals: true }, doc);
			},
		};
	}
		...
		if (this.add_totals_row) {
			...
			const totals_row = this.columns.map((col, i) => {
				return {
					...
					format: (value) => {
						let formatted_value = netmanthan.format(
							value,
							col.docfield,
							{
								always_show_decimals: true,
	...
	format_total_cell(formatted_value, df) {
		...
		if (netmanthan.model.is_numeric_field(df.docfield)) {
	...
	build_row(d) {
		return this.columns.map((col) => {
			...
			if (col.field === "docstatus" && !netmanthan.meta.has_field(this.doctype, "status")) {
				// get status from docstatus
				let status = netmanthan.get_indicator(d, this.doctype);
				if (status) {
	...
	save_report(save_type) {
		const _save_report = (name) => {
			...
			return netmanthan.call({
				method: "netmanthan.desk.reportview.save_report",
				args: {
					name: name,
					doctype: this.doctype,
				...
				callback: (r) => {
					if (r.exc) {
						netmanthan.msgprint(__("Report was not saved (there were errors)"));
						return;
					}
					if (r.message != this.report_name) {
						// Rerender the reports dropdown,
						// so that this report is included in the dropdown as well.
						netmanthan.boot.user.all_reports[r.message] = {
							ref_doctype: this.doctype,
							report_type: "Report Builder",
							title: r.message,
						};

						netmanthan.set_route("List", this.doctype, "Report", r.message);
						return;
					}
	...
	delete_report() {
		return netmanthan.call({
			method: "netmanthan.desk.reportview.delete_report",
	...
	get_report_doc() {
		return new Promise((resolve) => {
			netmanthan.model.with_doc("Report", this.report_name, () => {
				resolve(netmanthan.get_doc("Report", this.report_name));
			});
		});
	}
		...
		return (
			...
			filters
				.map((f) => {
					...
					const docfield = netmanthan.meta.get_docfield(doctype, fieldname);
					const label = `<b>${__(netmanthan.meta.get_label(doctype, fieldname))}</b>`;
					switch (condition) {
						case "=":
							return __("{0} is equal to {1}", [
								label,
								netmanthan.format(value, docfield),
							]);
						case "!=":
						...
						case "Between":
							return __("{0} is between {1} and {2}", [
								__(label),
								netmanthan.format(value[0], docfield),
								netmanthan.format(value[1], docfield),
							]);
						case "Timespan":
						...
						case "in":
							return __("{0} is one of {1}", [
								__(label),
								netmanthan.utils.comma_or(
									value.map((v) => netmanthan.format(v, docfield))
								),
							]);
						case "not in":
	...
	get_columns_totals(data) {
		...
		this.columns.forEach((col, i) => {
			const totals = data.reduce((totals, d) => {
				if (col.id in d && netmanthan.model.is_numeric_field(col.docfield)) {
					totals += flt(d[col.id]);
					return totals;
				}
			}, 0);
	...
	report_menu_items() {
		let items = [
			...
			},
			{
				...
				action: () => {
					...
					netmanthan.ui.get_print_settings(false, (print_settings) => {
			...
			},
			{
				...
				action: () => {
					...
					netmanthan.utils.setup_search(d.$body, ".unit-checkbox", ".label-area");
					d.show();
				},
			},
		];

		if (netmanthan.model.can_export(this.doctype)) {
			items.push({
				label: __("Export"),
				action: () => {
					...
					const d = new netmanthan.ui.Dialog({
						...
						primary_action: (data) => {
							args.cmd = "netmanthan.desk.reportview.export_query";
		...
		items.push({
			...
			action: () => {
				if (this.report_name) {
					...
					netmanthan.msgprint(__("Please save the report first"));
				}
			},
		});
		...
		const can_edit_or_delete = (action) => {
			...
			return (
				...
				(netmanthan.model[method]("Report") || this.report_doc.owner === netmanthan.session.user)
			);
		};
		...
		if (can_edit_or_delete("delete")) {
			items.push({
				label: __("Delete"),
				action: () =>
					netmanthan.confirm("Are you sure you want to delete this report?", () =>
						this.delete_report()
					),
		...
		// user permissions
		if (this.report_name && netmanthan.model.can_set_user_permissions("Report")) {
			items.push({
				...
				action: () => {
					...
					netmanthan.set_route("List", "User Permission", args);
				},
			});
		}
