netmanthan.provide("netmanthan.report_utils");

netmanthan.report_utils = {
	make_chart_options: function (
		...
		let datasets = y_fields.map((y_field) => ({
			name: netmanthan.model.unscrub(y_field),
		...
		function get_options(fields) {
			return fields.map((field) => {
				if (field.fieldname) {
					...
					field = netmanthan.report_utils.prepare_field_from_column(field);
					return { label: field.label, value: field.fieldname };
				}
			});
		}
	...
	get_report_filters: function (report_name) {
		if (netmanthan.query_reports[report_name]) {
			let filters = netmanthan.query_reports[report_name].filters;
		...
		return netmanthan
			...
			.then((r) => {
				netmanthan.dom.eval(r.script || "");
				return netmanthan.after_ajax(() => {
					if (
						netmanthan.query_reports[report_name] &&
						!netmanthan.query_reports[report_name].filters &&
						r.filters
					) {
						return (netmanthan.query_reports[report_name].filters = r.filters);
					}
					return (
						netmanthan.query_reports[report_name] &&
						netmanthan.query_reports[report_name].filters
