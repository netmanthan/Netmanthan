...
netmanthan.last_edited_communication = {};
const separator_element = "<div>---</div>";

netmanthan.views.CommunicationComposer = class {
	constructor(opts) {
		$.extend(this, opts);
		if (!this.doc) {
	...
	make() {
		...
		this.dialog = new netmanthan.ui.Dialog({
	...
	get_fields() {
		const fields = [
			...
			{
				fieldtype: "Button",
				label: netmanthan.utils.icon("down", "xs"),
			...
			},
			{
				...
				fieldname: "content",
				onchange: netmanthan.utils.debounce(this.save_as_draft.bind(this), 300),
			},
			{
				fieldtype: "Button",
			...
			{ fieldtype: "Section Break" },
			{
				...
				default: netmanthan.boot.user.send_me_a_copy,
			},
			{
		...
		const email_accounts = netmanthan.boot.email_accounts.filter((account) => {
			return (
	...
	toggle_more_options(show_options) {
		...
		const label = netmanthan.utils.icon(show_options ? "up-line" : "down", "xs");
	...
	setup_multiselect_queries() {
		["recipients", "cc", "bcc"].forEach((field) => {
			this.dialog.fields_dict[field].get_data = () => {
				...
				netmanthan.call({
					method: "netmanthan.email.get_contact_list",
					args: { txt },
					callback: (r) => {
	...
	setup_email_template() {
		...
		function add_template() {
			...
			netmanthan.call({
				method: "netmanthan.email.doctype.email_template.email_template.get_email_template",
				args: {
		...
		netmanthan.utils.add_select_group_button(clear_and_add_template, email_template_actions);
	}
	...
	get_last_edited_communication(clear) {
		if (!netmanthan.last_edited_communication[this.doctype]) {
			netmanthan.last_edited_communication[this.doctype] = {};
		}

		if (clear || !netmanthan.last_edited_communication[this.doctype][this.key]) {
			netmanthan.last_edited_communication[this.doctype][this.key] = {};
		}

		return netmanthan.last_edited_communication[this.doctype][this.key];
	}
	...
	async set_values() {
		...
		const subject = this.subject ? netmanthan.utils.html2text(this.subject) : "";
	...
	setup_print_language() {
		...
		$(fields.language_sel.input).empty().add_options(netmanthan.get_languages());
	...
	setup_print() {
		...
		if (this.frm) {
			const print_formats = netmanthan.meta.get_print_formats(this.frm.meta.name);
	...
	setup_attach() {
		...
		$(`
			...
			<p class='add-more-attachments'>
				<button class='btn btn-xs btn-default'>
					${netmanthan.utils.icon("small-add", "xs")}&nbsp;
					${__("Add Attachment")}
				</button>
			</p>
		...
		attach
			...
			.on("click", () => new netmanthan.ui.FileUploader(args));
			...
			if (files.length) {
				$.each(files, (i, f) => {
					...
					if (!attachment_rows.find(`[data-file-name="${f.name}"]`).length) {
						f.file_url = netmanthan.urllib.get_full_url(f.file_url);
	...
	get_attachment_row(attachment, checked) {
		return $(`<p class="checkbox flex">
			...
			<a href="${attachment.file_url}" target="_blank" class="btn-linkF">
				${netmanthan.utils.icon("link-url")}
			</a>
		</p>`);
	}
	...
	setup_email() {
		...
		$(fields.send_me_a_copy.input).on("click", () => {
			...
			netmanthan.db.set_value("User", netmanthan.session.user, "send_me_a_copy", val);
			netmanthan.boot.user.send_me_a_copy = val;
		});
	}
	...
	send_email(btn, form_values, selected_attachments, print_html, print_format) {
		...
		if (!form_values.recipients) {
			netmanthan.msgprint(__("Enter Email Recipient(s)"));
			return;
		}
		...
		if (this.frm && !netmanthan.model.can_email(this.doc.doctype, this.frm)) {
			netmanthan.msgprint(__("You are not allowed to send emails related to this document"));
			return;
		}

		return netmanthan.call({
			method: "netmanthan.core.doctype.communication.email.make",
			args: {
				...
				sender_full_name: form_values.sender ? netmanthan.user.full_name() : undefined,
			...
			callback(r) {
				if (!r.exc) {
					netmanthan.utils.play_sound("email");

					if (r.message["emails_not_sent_to"]) {
						netmanthan.msgprint(
							__("Email not sent to {0} (unsubscribed / disabled)", [
								netmanthan.utils.escape_html(r.message["emails_not_sent_to"]),
							])
						);
					}
	...
	is_print_letterhead_checked() {
		if (this.frm && $(this.frm.wrapper).find(".form-print-wrapper").is(":visible")) {
			...
			return (
				netmanthan.model.get_doc(":Print Settings", "Print Settings") || { with_letterhead: 1 }
			).with_letterhead
				? 1
				: 0;
		}
	...
	async get_signature(sender_email) {
		let signature = netmanthan.boot.user.email_signature;
		...
		if (!signature) {
			...
			const email_accounts = await netmanthan.db.get_list("Email Account", {
				filters: filters,
		...
		if (!netmanthan.utils.is_html(signature)) {
	...
	get_earlier_reply() {
		...
		const communication_date = netmanthan.datetime.global_date_format(
	...
	html2text(html) {
		...
		const text = netmanthan.utils.html2text(html);
		return text.replace(/\n{3,}/g, "\n\n");
	}
};
