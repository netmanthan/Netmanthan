...
netmanthan.views.Container = class Container {
	...
	constructor() {
		...
		$(document).on("page-change", function () {
			...
			var route_str = netmanthan.get_route_str();
			$("body").attr("data-route", route_str);
			$("body").attr("data-sidebar", me.has_sidebar() ? 1 : 0);
		});

		$(document).bind("rename", function (event, dt, old_name, new_name) {
			netmanthan.breadcrumbs.rename(dt, old_name, new_name);
		});
	}
	add_page(label) {
	...
	change_to(label) {
		...
		if (label.tagName) {
			...
			var page = netmanthan.pages[label];
		}
		if (!page) {
		...
		this.page._route = netmanthan.router.get_sub_path();
		$(this.page).trigger("show");
		!this.page.disable_scroll_to_top && netmanthan.utils.scroll_to(0);
	...
	has_sidebar() {
		var flag = 0;
		var route_str = netmanthan.get_route_str();
		// check in netmanthan.ui.pages
		flag = netmanthan.ui.pages[route_str] && !netmanthan.ui.pages[route_str].single_column;

		// sometimes netmanthan.ui.pages is updated later,
