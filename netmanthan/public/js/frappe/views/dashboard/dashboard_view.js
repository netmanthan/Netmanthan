...
netmanthan.views.DashboardView = class DashboardView extends netmanthan.views.ListView {
	get view_name() {
		return "Dashboard";
	}
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.dashboard_settings =
				netmanthan.get_user_settings(this.doctype)["dashboard_settings"] || null;
		});
	}
	...
	render_empty_state() {
		...
		const empty_state_image = "/assets/netmanthan/images/ui-states/list-empty-state.svg";
	...
	save_dashboard_customization() {
		...
		netmanthan.model.user_settings.save(
			this.doctype,
			"dashboard_settings",
			this.dashboard_settings
		);
	...
	reset_dashboard_customization() {
		netmanthan.confirm(__("Are you sure you want to reset all customizations?"), () => {
			this.dashboard_settings = null;
			netmanthan.model.user_settings
		...
		const dialog = new netmanthan.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					get_query: () => {
						return {
							query: "netmanthan.desk.doctype.dashboard_chart.dashboard_chart.get_charts_for_user",
							filters: {
								document_type: this.doctype,
							},
						};
			...
			primary_action: (values) => {
				...
				if (chart.new_or_existing == "New Chart") {
					...
					netmanthan
						.xcall(
							"netmanthan.desk.doctype.dashboard_chart.dashboard_chart.create_dashboard_chart",
							{ args: chart }
						)
						.then((doc) => {
	...
	get_field_options() {
		...
		netmanthan.get_meta(this.doctype).fields.map((df) => {
