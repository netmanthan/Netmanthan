...
netmanthan.views.FileView = class FileView extends netmanthan.views.ListView {
	static load_last_view() {
		const route = netmanthan.get_route();
		if (route.length === 2) {
			const view_user_settings = netmanthan.get_user_settings("File", "File");
			netmanthan.set_route(
				"List",
				"File",
				view_user_settings.last_folder || netmanthan.boot.home_folder
			);
			return true;
		}
	...
	setup_view() {
		...
		this.page.add_button(__("Toggle Grid View"), () => {
			netmanthan.views.FileView.grid_view = !netmanthan.views.FileView.grid_view;
			this.refresh();
		});
	}
	...
	get_args() {
		...
		if (netmanthan.views.FileView.grid_view) {
			Object.assign(args, {
		...
		const items = [
			{
				label: __("Home"),
				action: () => {
					netmanthan.set_route("List", "File", "Home");
				},
			},
			{
				label: __("New Folder"),
					...
					netmanthan.prompt(
						...
						(values) => {
							if (values.value.indexOf("/") > -1) {
								netmanthan.throw(__("Folder name should not include '/' (slash)"));
							}
							const data = {
							...
							netmanthan.call({
								method: "netmanthan.core.api.file.create_new_folder",
								args: data,
							});
						},
			...
			},
			{
				...
				action: () => {
					new netmanthan.ui.FileUploader({
						...
						on_success: (file) => {
							netmanthan.show_alert(__("Unzipping files..."));
							netmanthan
								.call("netmanthan.core.api.file.unzip_file", {
									name: file.name,
								})
								.then((r) => {
									if (r.message) {
										netmanthan.show_alert(__("Unzipped {0} files", [r.message]));
									}
								});
						},
					});
	...
	add_file_action_buttons() {
		this.$cut_button = this.page
			.add_button(__("Cut"), () => {
				netmanthan.file_manager.cut(this.get_checked_items(), this.current_folder);
				this.$checks.parents(".file-wrapper").addClass("cut");
			})
			.hide();

		this.$paste_btn = this.page
			.add_button(__("Paste"), () => netmanthan.file_manager.paste(this.current_folder))
			.hide();

		this.page.add_actions_menu_item(__("Export as zip"), () => {
			let docnames = this.get_checked_items(true);
			if (docnames.length) {
				open_url_post("/api/method/netmanthan.core.api.file.zip_files", {
	...
	set_fields() {
		this.fields = this.meta.fields
			.filter((df) => netmanthan.model.is_value_type(df.fieldtype) && !df.hidden)
			.map((df) => df.fieldname)
	...
	prepare_datum(d) {
		...
		if (d.is_folder) {
			...
			type = "folder";
		} else if (netmanthan.utils.is_image_file(d.file_name)) {
			icon_class = "image";
			type = "image";
		} else {
			icon_class = "file";
		...
		d.subject_html = `
			${netmanthan.utils.icon(icon_class)}
			<span>${title}</span>
	...
	before_render() {
		super.before_render();
		netmanthan.model.user_settings.save("File", "grid_view", netmanthan.views.FileView.grid_view);
	...
	get_header_html() {
		...
		let header_selector_html = !netmanthan.views.FileView.grid_view
			? `<input class="level-item list-check-all hidden-xs" type="checkbox" title="${__(
					"Select All"
			  )}">`
			: "";

		let header_columns_html = !netmanthan.views.FileView.grid_view
	...
	get_creation_date(file) {
		...
		let created_on;
		if (date === netmanthan.datetime.now_date()) {
			created_on = comment_when(file.creation);
		} else {
			created_on = netmanthan.datetime.str_to_user(date);
		}
		return created_on;
	}
	...
	get_left_html(file) {
		...
		const file_size = file.file_size ? netmanthan.form.formatters.FileSize(file.file_size) : "";
		...
		this.$result.on("drop", (e) => {
			...
			if (dataTransfer.files && dataTransfer.files.length > 0) {
				...
				if ($el.parents(".folders").length !== 0) {
					...
					netmanthan.file_manager.paste(folder_name);
					netmanthan.show_alert(`File ${file_name} moved to ${folder_name}`);
				}
			}
		});
	}
	...
	toggle_cut_paste_buttons() {
		const hide_paste_btn =
			!netmanthan.file_manager.can_paste ||
			netmanthan.file_manager.old_folder === this.current_folder;
...
netmanthan.views.FileView.grid_view = netmanthan.get_user_settings("File").grid_view || false;
