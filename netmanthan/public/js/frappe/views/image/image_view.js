/**
 * netmanthan.views.ImageView
 */
netmanthan.provide("netmanthan.views");

netmanthan.views.ImageView = class ImageView extends netmanthan.views.ListView {
	get view_name() {
		return "Image";
	}
	...
	item_html(item) {
		...
		const escaped_title = netmanthan.utils.escape_html(title);
		...
		const expand_button_html = item._image_url
			? `<div class="zoom-view" data-name="${encoded_name}">
				${netmanthan.utils.icon("expand", "xs")}
			</div>`
			: "";
	...
	get_attached_images() {
		return netmanthan
			.call({
				method: "netmanthan.core.api.file.get_attached_images",
				args: {
...
netmanthan.views.GalleryView = class GalleryView {
	constructor(opts) {
		$.extend(this, opts);
		var me = this;
	...
	prepare() {
		...
		if (this.pswp_root.length === 0) {
			var pswp = netmanthan.render_template("photoswipe_dom");
	...
	load_lib() {
		return new Promise((resolve) => {
			var asset_dir = "assets/netmanthan/js/lib/photoswipe/";
			netmanthan.require(
				[
