...
netmanthan.standard_pages["Workspaces"] = function () {
	var wrapper = netmanthan.container.add_page("Workspaces");
	...
	netmanthan.workspace = new netmanthan.views.Workspace(wrapper);
	$(wrapper).bind("show", function () {
		netmanthan.workspace.show();
	});
};

netmanthan.views.Workspace = class Workspace {
	constructor(wrapper) {
		this.wrapper = $(wrapper);
		this.page = wrapper.page;
		this.blocks = netmanthan.workspace_block.blocks;
	...
	async setup_pages(reload) {
		...
		if (this.all_pages) {
			...
			for (let page of this.all_pages) {
				netmanthan.workspaces[netmanthan.router.slug(page.name)] = {
					title: page.title,
					public: page.public,
				};
			}
	...
	get_pages() {
		return netmanthan.xcall("netmanthan.desk.desktop.get_workspace_sidebar_items");
	}
			...
			>
				<div class="desk-sidebar-item standard-sidebar-item ${item.selected ? "selected" : ""}">
					<a
						href="/app/${
							item.public
								? netmanthan.router.slug(item.title)
								: "private/" + netmanthan.router.slug(item.title)
						}"
						class="item-anchor ${item.is_editable ? "" : "block-click"}" title="${__(item.title)}"
					>
						<span class="sidebar-item-icon" item-icon=${item.icon || "folder-normal"}>${netmanthan.utils.icon(
	...
	make_sidebar() {
		...
		this.sidebar.find(".selected").length &&
			!netmanthan.dom.is_element_in_viewport(this.sidebar.find(".selected")) &&
	...
	build_sidebar_section(title, root_pages) {
		...
		let $title = $(`<div class="standard-sidebar-label">
			<span>${netmanthan.utils.icon("small-down", "xs")}</span>
	...
	append_item(item, container) {
		let is_current_page =
			netmanthan.router.slug(item.title) == netmanthan.router.slug(this.get_page_to_show().name) &&
	...
	add_drop_icon(item, sidebar_control, item_container) {
		...
		let $drop_icon = $(
			`<span class="drop-icon hidden">${netmanthan.utils.icon(drop_icon, "sm")}</span>`
	...
	show() {
		...
		if (!netmanthan.router.current_route[0]) {
			netmanthan.router.current_route = !page.public
	...
	get_data(page) {
		return netmanthan
			.call("netmanthan.desk.desktop.get_desktop_page", {
				page: page,
			})
			.then((data) => {
				...
				return netmanthan.dashboard_utils.get_dashboard_settings().then((settings) => {
					if (settings) {
	...
	get_page_to_show() {
		...
		let page =
			(netmanthan.get_route()[1] == "private" ? netmanthan.get_route()[2] : netmanthan.get_route()[1]) ||
			default_page.name;
		let is_public = netmanthan.get_route()[1]
			? netmanthan.get_route()[1] != "private"
			: default_page.public;
	...
	async show_page(page) {
		...
		if (this.all_pages.length) {
			...
			if (this.pages && this.pages[current_page.name]) {
				...
				await netmanthan.after_ajax(() => this.get_data(current_page));
			}
	...
	setup_customization_buttons(page) {
		...
		this.page.set_secondary_action(__("Discard"), async () => {
			...
			this.reload();
			netmanthan.show_alert({ message: __("Customizations Discarded"), indicator: "info" });
		});

		if (page.name && this.has_access) {
			this.page.add_inner_button(__("Settings"), () => {
				netmanthan.set_route(`workspace/${page.name}`);
			});
		}
	}
	...
	add_sidebar_actions(item, sidebar_control, is_new) {
		if (!item.is_editable) {
			...
			netmanthan.utils.add_custom_button(
				netmanthan.utils.icon("duplicate", "sm"),
			...
			netmanthan.utils.add_custom_button(
				netmanthan.utils.icon("unhide", "sm"),
	...
	edit_page(item) {
		...
		const d = new netmanthan.ui.Dialog({
			...
			primary_action: (values) => {
				values.title = netmanthan.utils.escape_html(values.title);
				...
				netmanthan.call({
					method: "netmanthan.desk.doctype.workspace.workspace.update_page",
					args: {
						name: old_item.name,
						title: values.title,
						icon: values.icon || "",
					...
					callback: function (res) {
						if (res.message) {
							...
							netmanthan.show_alert({ message: message, indicator: "green" });
						}
					},
				});
				...
				if (this.make_page_selected) {
					...
					let route = pre_url + netmanthan.router.slug(values.title);
	...
	update_sidebar(old_item, new_item) {
		...
		if (is_title_changed || is_section_changed) {
			if (new_item.is_public) {
				...
				let user = netmanthan.session.user;
	...
	update_cached_values(old_item, new_item, duplicate, new_page) {
		...
		// update netmanthan.workspaces
		if (netmanthan.workspaces[netmanthan.router.slug(old_item.name)] || new_page) {
			!duplicate && delete netmanthan.workspaces[netmanthan.router.slug(old_item.name)];
			if (new_item) {
				netmanthan.workspaces[netmanthan.router.slug(new_item.name)] = { title: new_item.title };
			}
		}
	...
	add_settings_button(item, sidebar_control) {
		...
		if (this.is_item_deletable(item)) {
			this.dropdown_list.push({
				...
				icon: netmanthan.utils.icon("delete-active", "sm"),
		...
		let $button = $(`
			<div class="btn btn-secondary btn-xs setting-btn dropdown-btn" title="${__("Setting")}">
				${netmanthan.utils.icon("dot-horizontal", "xs")}
			</div>
	...
	is_item_deletable(item) {
		...
		if (
			!item.public ||
			(item.public && (!item.module || (item.module && netmanthan.boot.developer_mode)))
		)
			return true;
		return false;
	}
	...
	delete_page(page) {
		netmanthan.confirm(
			...
			() => {
				netmanthan.call({
					method: "netmanthan.desk.doctype.workspace.workspace.delete_page",
					args: { page: page },
					callback: function (res) {
						if (res.message) {
	...
	duplicate_page(page) {
		...
		const d = new netmanthan.ui.Dialog({
			...
			primary_action: (values) => {
				...
				d.hide();
				netmanthan.call({
					method: "netmanthan.desk.doctype.workspace.workspace.duplicate_page",
					args: {
						page_name: page.name,
						new_page: values,
					},
				...
				new_page.name = values.title + (new_page.public ? "" : "-" + netmanthan.session.user);
				new_page.label = new_page.name;
				new_page.icon = values.icon;
				new_page.parent_page = values.parent || "";
				new_page.for_user = new_page.public ? "" : netmanthan.session.user;
	...
	hide_unhide_workspace(page, event, hide) {
		...
		netmanthan.call({
			method: "netmanthan.desk.doctype.workspace.workspace." + method,
			args: {
				page_name: page.name,
			},
			callback: (r) => {
	...
	update_sorted_sidebar() {
		if (this.sorted_public_items || this.sorted_private_items) {
			netmanthan.call({
				method: "netmanthan.desk.doctype.workspace.workspace.sort_pages",
				args: {
				...
				callback: function (res) {
					if (res.message) {
						...
						netmanthan.show_alert({ message: __(message), indicator: "green" });
					}
				},
			});
		}
	...
	initialize_new_page() {
		...
		const d = new netmanthan.ui.Dialog({
			...
			primary_action: (values) => {
				...
				let name = values.title + (values.is_public ? "" : "-" + netmanthan.session.user);
				let blocks = [
					{
						type: "header",
				...
				let new_page = {
					...
					title: values.title,
					public: values.is_public || 0,
					for_user: values.is_public ? "" : netmanthan.session.user,
					icon: values.icon,
				...
				this.editor
					...
					.then(async () => {
						...
						netmanthan.call({
							method: "netmanthan.desk.doctype.workspace.workspace.new_page",
							args: {
								new_page: new_page,
							},
							callback: function (res) {
						...
						let route = pre_url + netmanthan.router.slug(new_page.title);
						netmanthan.set_route(route);
	...
	validate_page(new_page, old_page) {
		...
		if (netmanthan.router.doctype_route_exist(netmanthan.router.slug(new_page.title))) {
	...
	initialize_editorjs(blocks) {
		this.tools = {
			...
			HeaderSize: netmanthan.workspace_block.tunes["header_size"],
		};
	...
	save_page(page) {
		...
		return this.editor
			...
			.then((outputData) => {
				...
				page.content = JSON.stringify(blocks);
				netmanthan.call({
					method: "netmanthan.desk.doctype.workspace.workspace.save_page",
					args: {
						title: page.title,
	...
	create_page_skeleton() {
		...
		this.body.prepend(netmanthan.render_template("workspace_loading_skeleton"));
	...
	create_sidebar_skeleton() {
		...
		$(netmanthan.render_template("workspace_sidebar_loading_skeleton")).insertBefore(this.sidebar);
	...
	register_awesomebar_shortcut() {
		"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter) => {
			...
			netmanthan.ui.keys.add_shortcut({ shortcut: letter, ...default_shortcut });
			netmanthan.ui.keys.add_shortcut({ shortcut: `shift+${letter}`, ...default_shortcut });
		});
	}
};
