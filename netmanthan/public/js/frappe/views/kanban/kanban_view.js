...
netmanthan.views.KanbanView = class KanbanView extends netmanthan.views.ListView {
	static load_last_view() {
		const route = netmanthan.get_route();
		if (route.length === 3) {
			const doctype = route[1];
			const user_settings = netmanthan.get_user_settings(doctype)["Kanban"] || {};
			if (!user_settings.last_kanban_board) {
				return new netmanthan.views.KanbanView({ doctype: doctype });
			}
	...
	show() {
		netmanthan.views.KanbanView.get_kanbans(this.doctype).then((kanbans) => {
			if (!kanbans.length) {
				return netmanthan.views.KanbanView.show_kanban_dialog(this.doctype, true);
			} else if (kanbans.length && netmanthan.get_route().length !== 4) {
				return netmanthan.views.KanbanView.show_kanban_dialog(this.doctype, true);
			} else {
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.board_name = netmanthan.get_route()[3] || get_board_name() || null;
	...
	set_board_perms_and_push_menu_items() {
		...
		return netmanthan.call({
			method: "netmanthan.client.get_doc_permissions",
			args: {
	...
	push_menu_items() {
		...
		if (this.board_perms.delete) {
			this.menu_items.push({
				...
				action: () => {
					netmanthan.confirm(__("Are you sure you want to proceed?"), () => {
						netmanthan.db.delete_doc("Kanban Board", this.board_name).then(() => {
							netmanthan.show_alert(`Kanban Board ${this.board_name} deleted.`);
							netmanthan.set_route("List", this.doctype, "List");
						});
					});
				},
			});
	...
	get_board() {
		return netmanthan.db.get_doc("Kanban Board", this.board_name).then((board) => {
			this.board = board;
	...
	save_kanban_board_filters() {
		...
		netmanthan.db.set_value("Kanban Board", this.board_name, "filters", filters).then((r) => {
			if (r.exc) {
	...
	render() {
		...
		if (!this.kanban) {
			this.kanban = new netmanthan.views.KanbanBoard({
	...
	get_card_meta() {
		var meta = netmanthan.get_meta(this.doctype);
		// preserve route options erased by new doc
		let route_options = { ...netmanthan.route_options };
		var doc = netmanthan.model.get_new_doc(this.doctype);
		netmanthan.route_options = route_options;
		var title_field = null;
		var quick_entry = false;

		if (this.meta.title_field) {
			title_field = netmanthan.meta.get_field(this.doctype, this.meta.title_field);
		}
		...
		if (
			mandatory.some((df) => netmanthan.model.table_fields.includes(df.fieldtype)) ||
			mandatory.length > 1
		) {
			quick_entry = true;
		}
		...
		if (!title_field) {
			title_field = netmanthan.meta.get_field(this.doctype, "name");
		}
...
netmanthan.views.KanbanView.get_kanbans = function (doctype) {
	let kanbans = [];

	return get_kanban_boards().then((kanban_boards) => {
		if (kanban_boards) {
			kanban_boards.forEach((board) => {
				let route = `/app/${netmanthan.router.slug(board.reference_doctype)}/view/kanban/${
					board.name
				}`;
	...
	function get_kanban_boards() {
		return netmanthan
			.call("netmanthan.desk.doctype.kanban_board.kanban_board.get_kanban_boards", { doctype })
			.then((r) => r.message);
	}
};

netmanthan.views.KanbanView.show_kanban_dialog = function (doctype) {
	let dialog = new_kanban_dialog();
	dialog.show();

	function make_kanban_board(board_name, field_name, project) {
		return netmanthan.call({
			method: "netmanthan.desk.doctype.kanban_board.kanban_board.quick_kanban_board",
			args: {
				doctype,
				board_name,
				field_name,
			...
			callback: function (r) {
				var kb = r.message;
				if (kb.filters) {
					netmanthan.provide("netmanthan.kanban_filters");
					netmanthan.kanban_filters[kb.kanban_board_name] = kb.filters;
				}
				netmanthan.set_route("List", doctype, "Kanban", kb.kanban_board_name);
			},
		});
	}
	...
	function new_kanban_dialog() {
		...
		const select_fields = netmanthan.get_meta(doctype).fields.filter((df) => {
		...
		let primary_action = () => {
			if (to_save) {
				...
				make_kanban_board(values.board_name, values.field_name, values.project).then(
					() => dialog.hide(),
					(err) => netmanthan.msgprint(err)
				);
			} else {
				netmanthan.set_route("Form", "Customize Form", { doc_type: doctype });
			}
		};

		return new netmanthan.ui.Dialog({
			title: dialog_title,
