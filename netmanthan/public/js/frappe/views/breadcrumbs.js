...
netmanthan.breadcrumbs = {
	...
	add(module, doctype, type) {
		...
		this.all[netmanthan.breadcrumbs.current_page()] = obj;
		this.update();
	},

	current_page() {
		return netmanthan.get_route_str();
	},

	update() {
		var breadcrumbs = this.all[netmanthan.breadcrumbs.current_page()];
		...
		if (breadcrumbs.type === "Custom") {
			...
			// form / print
			let view = netmanthan.get_route()[0];
	...
	get last_route() {
		return netmanthan.route_history.slice(-2)[0];
	},
	...
	set_workspace_breadcrumb(breadcrumbs) {
		...
		if (
			...
			(breadcrumbs.module_info.blocked ||
				!netmanthan.visible_modules.includes(breadcrumbs.module_info.module))
		) {
			return;
		}

		this.append_breadcrumb_element(
			`/app/${netmanthan.router.slug(breadcrumbs.workspace)}`,
	...
	set_workspace(breadcrumbs) {
		...
		if (this.last_route?.[0] == "Workspaces") {
			...
			if (
				breadcrumbs.module &&
				netmanthan.boot.module_wise_workspaces[breadcrumbs.module]?.includes(last_workspace)
			) {
		...
		if (breadcrumbs.module) {
			...
			breadcrumbs.module_info = netmanthan.get_module(breadcrumbs.module);

			// set workspace
			if (
				breadcrumbs.module_info &&
				netmanthan.boot.module_wise_workspaces[breadcrumbs.module]
			) {
				breadcrumbs.workspace = netmanthan.boot.module_wise_workspaces[breadcrumbs.module][0];
			}
		}
	},
	...
	set_list_breadcrumb(breadcrumbs) {
		...
		const doctype_meta = netmanthan.get_doc("DocType", doctype);
		if (
			(doctype === "User" && !netmanthan.user.has_role("System Manager")) ||
			doctype_meta?.issingle
		) {
			// no user listview for non-system managers and single doctypes
		} else {
			let route;
			const doctype_route = netmanthan.router.slug(netmanthan.router.doctype_layout || doctype);
			if (doctype_meta?.is_tree) {
				let view = netmanthan.model.user_settings[doctype].last_view || "Tree";
	...
	set_form_breadcrumb(breadcrumbs, view) {
		const doctype = breadcrumbs.doctype;
		const docname = netmanthan.get_route().slice(2).join("/");
		let form_route = `/app/${netmanthan.router.slug(doctype)}/${docname}`;
		...
		if (view === "form") {
			...
			last_crumb.click((event) => {
				...
				netmanthan.utils.copy_to_clipboard(last_crumb.text());
			});
		}
	},
	...
	set_dashboard_breadcrumb(breadcrumbs) {
		const doctype = breadcrumbs.doctype;
		const docname = netmanthan.get_route()[1];
		let dashboard_route = `/app/${netmanthan.router.slug(doctype)}/${docname}`;
		$(`<li><a href="${dashboard_route}">${__(docname)}</a></li>`).appendTo(this.$breadcrumbs);
	},

	setup_modules() {
		if (!netmanthan.visible_modules) {
			netmanthan.visible_modules = $.map(netmanthan.boot.allowed_workspaces, (m) => {
				return m.module;
			});
		}
	},
	...
	rename(doctype, old_name, new_name) {
		...
		delete netmanthan.breadcrumbs.all[old_route_str];
		this.update();
	},
