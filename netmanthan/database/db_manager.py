...
class DbManager:
	...
	def restore_database(target, source, user, password):
		...
		from netmanthan.utils import make_esc
		...
		command = (
			...
			+ ("-P{port}" if netmanthan.db.port else "")
			+ " {target} {source}"
		)
		command = command.format(
			...
			host=esc(netmanthan.db.host),
			target=esc(target),
			source=source,
			port=netmanthan.db.port,
		)

		os.system(command)
		netmanthan.cache().delete_keys("")  # Delete all keys associated with this site.
