...
from netmanthan.database.utils import (
	DefaultOrderBy,
	EmptyQueryValues,
	FallBackDateTimeStr,
	LazyMogrify,
...
from netmanthan.exceptions import DoesNotExistError, ImplicitCommitError
from netmanthan.model.utils.link_count import flush_local_link_count
from netmanthan.query_builder.functions import Count
from netmanthan.utils import cast as cast_fieldtype
from netmanthan.utils import cint, get_datetime, get_table_name, getdate, now, sbool
from netmanthan.utils.deprecations import deprecated, deprecation_warning
...
class Database:
	...
	class InvalidColumnName(netmanthan.ValidationError):
		pass
	...
	def __init__(
		...
		self.setup_type_map()
		self.host = host or netmanthan.conf.db_host or "127.0.0.1"
		self.port = port or netmanthan.conf.db_port or ""
		self.user = user or netmanthan.conf.db_name
		self.db_name = netmanthan.conf.db_name
		self._conn = None

		if ac_name:
			self.user = ac_name or netmanthan.conf.db_name

		if use_default:
			self.user = netmanthan.conf.db_name

		self.transaction_writes = 0
		self.auto_commit_on_many_writes = 0

		self.password = password or netmanthan.conf.db_password
		self.value_cache = {}
		self.logger = netmanthan.logger("database")
	...
	def connect(self):
		...
		netmanthan.local.rollback_observers = []
	...
	def sql(
		...
		Examples:
		        ...
		        netmanthan.db.sql("select name from tabCustomer", as_dict=True)

		        # return names beginning with a
		        netmanthan.db.sql("select name from tabCustomer where name like %s", "a%")

		        # values as dict
		        netmanthan.db.sql("select name from tabCustomer where name like %(name)s and owner=%(owner)s",
		...
		try:
			...
			if self.is_syntax_error(e):
				netmanthan.errprint(f"Syntax error in query:\n{query} {values or ''}")

			elif self.is_deadlocked(e):
				raise netmanthan.QueryDeadlockError(e) from e

			elif self.is_timedout(e):
				raise netmanthan.QueryTimeoutError(e) from e
				...
				netmanthan.throw(
					...
					exc=netmanthan.InReadOnlyMode,
				)

			# TODO: added temporarily
			elif self.db_type == "postgres":
				traceback.print_stack()
				netmanthan.errprint(f"Error in query:\n{e}")
				raise
		...
		if debug:
			time_end = time()
			netmanthan.errprint(f"Execution time: {time_end - time_start:.2f} sec")
	...
	def _log_query(
		...
		if netmanthan.conf.allow_tests and netmanthan.cache().get_value("flag_print_sql"):
		...
		if debug:
			...
			netmanthan.errprint(_query)

		if netmanthan.conf.logging == 2:
			_query = _query or str(mogrified_query)
			netmanthan.log(f"#### query\n{_query}\n####")
	...
	def mogrify(self, query: Query, values: QueryValues):
		...
		try:
			...
			if isinstance(values, dict):
				return query % {k: netmanthan.db.escape(v) if isinstance(v, str) else v for k, v in values.items()}
			elif isinstance(values, (list, tuple)):
				return query % tuple(netmanthan.db.escape(v) if isinstance(v, str) else v for v in values)
			return query, values
	...
	def explain_query(self, query, values=None):
		...
		netmanthan.errprint("--- query explain ---")
		try:
			self._cursor.execute(f"EXPLAIN {query}", values)
		except Exception as e:
			netmanthan.errprint(f"error in query explain: {e}")
		else:
			netmanthan.errprint(json.dumps(self.fetch_as_dict(), indent=1))
			netmanthan.errprint("--- query explain end ---")
	...
	def sql_list(self, query, values=(), debug=False, **kwargs):
		...
		Example:
		        ...
		        doctypes = netmanthan.db.sql_list("select name from DocType")
		"""
		...
		if query[:6].lower() in ("update", "insert", "delete"):
			...
			if self.transaction_writes > self.MAX_WRITES_PER_TRANSACTION:
				if self.auto_commit_on_many_writes:
					...
					raise netmanthan.TooManyWritesError(msg)
	...
	def fetch_as_dict(self, formatted=0, as_utf8=0) -> list[netmanthan._dict]:
		...
		if not as_utf8:
			return [netmanthan._dict(zip(keys, row)) for row in result]
		...
		for r in result:
			...
			ret.append(netmanthan._dict(zip(keys, values)))
		return ret

	@staticmethod
	def clear_db_table_cache(query):
		if query and is_query_type(query, ("drop", "create")):
			netmanthan.cache().delete_key("db_tables")
	...
	def get_value(
		...
		*,
		...
		Example:
		        ...
		        netmanthan.db.get_value("Customer", {"name": ("like a%")})

		        # return last login of **User** `test@example.com`
		        netmanthan.db.get_value("User", "test@example.com", "last_login")

		        last_login, last_ip = netmanthan.db.get_value("User", "test@example.com",
		                ["last_login", "last_ip"])

		        # returns default date_format
		        netmanthan.db.get_value("System Settings", None, "date_format")
		"""
	...
	def get_values(
		...
		*,
		...
		Example:
		        ...
		        # return first customer starting with a
		        customers = netmanthan.db.get_values("Customer", {"name": ("like a%")})

		        # return last login of **User** `test@example.com`
		        user = netmanthan.db.get_values("User", "test@example.com", "*")[0]
		"""
		out = None
		...
		if isinstance(filters, list):
			...
			if (filters is not None) and (filters != doctype or doctype == "DocType"):
				try:
					...
					if ignore and (netmanthan.db.is_missing_column(e) or netmanthan.db.is_table_missing(e)):
						# table or column not found, return None
						out = None
					elif (not ignore) and netmanthan.db.is_table_missing(e):
	...
	def get_singles_dict(self, doctype, debug=False, *, for_update=False, cast=False):
		...
		Example:
		        ...
		        account_settings = netmanthan.db.get_singles_dict("Accounts Settings")
		"""
		queried_result = netmanthan.qb.get_query(
			"Singles",
		...
		if not cast:
			return netmanthan._dict(queried_result)
	...
	def get_all(*args, **kwargs):
		return netmanthan.get_all(*args, **kwargs)

	@staticmethod
	def get_list(*args, **kwargs):
		return netmanthan.get_list(*args, **kwargs)
	...
	def set_single_value(
		...
		**kwargs,
		...
		Example:
		        ...
		        company = netmanthan.db.set_single_value("System Settings", "deny_multiple_sessions", True)
		"""
	...
	def get_single_value(self, doctype, fieldname, cache=True):
		...
		Example:
		        ...
		        company = netmanthan.db.get_single_value('Global Defaults', 'default_company')
		"""
		...
		df = netmanthan.get_meta(doctype).get_field(fieldname)

		if not df:
			netmanthan.throw(
				_("Invalid field name: {0}").format(netmanthan.bold(fieldname)), self.InvalidColumnName
			)
	...
	def _get_value_for_many_names(
		...
		*,
		...
		if names := list(filter(None, names)):
			return netmanthan.qb.get_query(
				doctype,
				fields=field,
				filters=names,
				order_by=order_by,
	...
	def set_value(
		...
		**Warning:** this function will not call Document events and should be avoided in normal cases.
		...
		if update_modified:
			...
			modified_by = modified_by or netmanthan.session.user
		...
		if is_single_doctype:
			...
			query = (
				netmanthan.qb.into("Singles").columns("doctype", "field", "value").insert(*singles_data)
			).run(debug=debug)
			netmanthan.clear_document_cache(dt, dt)
			...
			if isinstance(dn, str):
				netmanthan.clear_document_cache(dt, dn)
			else:
				# TODO: Fix this; doesn't work rn - gavin@netmanthan.io
				# netmanthan.cache().hdel_keys(dt, "document_cache")
				# Workaround: clear all document caches
				netmanthan.cache().delete_value("document_cache")
	...
	def touch(self, doctype, docname):
		...
		netmanthan.qb.update(DocType).set(DocType.modified, modified).where(DocType.name == docname).run()
		return modified
	...
	def set_temp(value):
		...
		netmanthan.cache().hset("temp", key, value)
		return key

	@staticmethod
	def get_temp(key):
		"""Return the temperory value and delete it."""
		return netmanthan.cache().hget("temp", key)
	...
	def set_default(key, val, parent="__default", parenttype=None):
		...
		netmanthan.defaults.set_default(key, val, parent, parenttype)
	...
	def add_default(key, val, parent="__default", parenttype=None):
		...
		netmanthan.defaults.add_default(key, val, parent, parenttype)

	@staticmethod
	def get_defaults(key=None, parent="__default"):
		"""Get all defaults"""
		defaults = netmanthan.defaults.get_defaults_for(parent)
		if not key:
			return defaults
		...
		return defaults.get(netmanthan.scrub(key))

	def begin(self, *, read_only=False):
		read_only = read_only or netmanthan.flags.read_only
		mode = "READ ONLY" if read_only else ""
		self.sql(f"START TRANSACTION {mode}")

	def commit(self):
		"""Commit current transaction. Calls SQL `COMMIT`."""
		for method in netmanthan.local.before_commit:
			netmanthan.call(method[0], *(method[1] or []), **(method[2] or {}))
	...
	def add_before_commit(self, method, args=None, kwargs=None):
		netmanthan.local.before_commit.append([method, args, kwargs])

	@staticmethod
	def flush_realtime_log():
		for args in netmanthan.local.realtime_log:
			netmanthan.realtime.emit_via_redis(*args)

		netmanthan.local.realtime_log = []

	def savepoint(self, save_point):
		"""Savepoints work as a nested transaction.

		Changes can be undone to a save point by doing netmanthan.db.rollback(save_point)
	...
	def rollback(self, *, save_point=None):
		...
		if save_point:
			...
			self.sql("rollback")
			self.begin()
			for obj in dict.fromkeys(netmanthan.local.rollback_observers):
				if hasattr(obj, "on_rollback"):
					obj.on_rollback()
			netmanthan.local.rollback_observers = []

			netmanthan.local.realtime_log = []
			netmanthan.flags.enqueue_after_commit = []
	...
	def a_row_exists(self, doctype):
		...
		return netmanthan.get_all(doctype, limit=1, order_by=None, as_list=True)
	...
	def count(self, dt, filters=None, debug=False, cache=False, distinct: bool = True):
		...
		if cache and not filters:
			cache_count = netmanthan.cache().get_value(f"doctype:count:{dt}")
			if cache_count is not None:
				return cache_count
		count = netmanthan.qb.get_query(
			table=dt,
			filters=filters,
			fields=Count("*"),
			distinct=distinct,
		...
		if not filters and cache:
			netmanthan.cache().set_value(f"doctype:count:{dt}", count, expires_in_sec=86400)
		return count
	...
	def get_creation_count(self, doctype, minutes):
		...
		from netmanthan.utils import now_datetime
	...
	def get_db_table_columns(self, table) -> list[str]:
		...
		columns = netmanthan.cache().hget("table_columns", table)
		if columns is None:
			information_schema = netmanthan.qb.Schema("information_schema")

			columns = (
				netmanthan.qb.from_(information_schema.columns)
			...
			if columns:
				netmanthan.cache().hset("table_columns", table, columns)
	...
	def get_system_setting(self, key):
		return netmanthan.get_system_settings(key)
	...
	def is_column_missing(e):
		return netmanthan.db.is_missing_column(e)

	def get_descendants(self, doctype, name):
		"""Return descendants of the group node in tree"""
		from netmanthan.utils.nestedset import get_descendants_of
	...
	def log_touched_tables(self, query):
		if is_query_type(query, ("insert", "delete", "update", "alter", "drop", "rename")):
			...
			if netmanthan.flags.touched_tables is None:
				netmanthan.flags.touched_tables = set()
			netmanthan.flags.touched_tables.update(tables)
	...
	def bulk_insert(self, doctype, fields, values, ignore_duplicates=False, *, chunk_size=10_000):
		...
		for start_index in range(0, len(values), chunk_size):
			query = netmanthan.qb.into(table)
	...
	def create_sequence(self, *args, **kwargs):
		from netmanthan.database.sequence import create_sequence

		return create_sequence(*args, **kwargs)

	def set_next_sequence_val(self, *args, **kwargs):
		from netmanthan.database.sequence import set_next_val

		set_next_val(*args, **kwargs)

	def get_next_sequence_val(self, *args, **kwargs):
		from netmanthan.database.sequence import get_next_val
...
def enqueue_jobs_after_commit():
	...
	if netmanthan.flags.enqueue_after_commit and len(netmanthan.flags.enqueue_after_commit) > 0:
		for job in netmanthan.flags.enqueue_after_commit:
			q = get_queue(job.get("queue"), is_async=job.get("is_async"))
			q.enqueue_call(
				execute_job,
				timeout=job.get("timeout"),
				kwargs=job.get("queue_args"),
				failure_ttl=netmanthan.conf.get("rq_job_failure_ttl") or RQ_JOB_FAILURE_TTL,
				result_ttl=netmanthan.conf.get("rq_results_ttl") or RQ_RESULTS_TTL,
...
def savepoint(catch: type | tuple[type, ...] = Exception):
	...
	try:
		...
		netmanthan.db.release_savepoint(savepoint)
...
def get_query_execution_timeout() -> int:
	...
	if not netmanthan.conf.get("enable_db_statement_timeout"):
		return 0

	# Zero means no timeout, which is the default value in db.
	timeout = 0
	with suppress(Exception):
		if getattr(netmanthan.local, "request", None):
			timeout = netmanthan.conf.http_timeout or 300
