...
from netmanthan.database.schema import DBTable, get_definition
from netmanthan.model import log_types
from netmanthan.utils import cint, flt


class PostgresTable(DBTable):
	def create(self):
		varchar_len = netmanthan.db.VARCHAR_LEN
		...
		if (
			...
			netmanthan.db.create_sequence(self.doctype, check_not_exists=True, cache=netmanthan.db.SEQUENCE_CACHE)
	...
	def create_indexes(self):
		...
		if create_index_query:
			# nosemgrep
			netmanthan.db.sql(create_index_query)
	...
	def alter(self):
		...
		for col in self.set_default:
			...
			if col.fieldtype in ("Check", "Int"):
				...
				col_default = f"{netmanthan.db.escape(col.default)}"
		...
		try:
			if query:
				...
				# nosemgrep
				netmanthan.db.sql(final_alter_query)
			if create_contraint_query:
				# nosemgrep
				netmanthan.db.sql(create_contraint_query)
			if drop_contraint_query:
				# nosemgrep
				netmanthan.db.sql(drop_contraint_query)
		except Exception as e:
			# sanitize
			if netmanthan.db.is_duplicate_fieldname(e):
				netmanthan.throw(str(e))
			elif netmanthan.db.is_duplicate_entry(e):
