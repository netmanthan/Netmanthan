...
class DBTable:
	def __init__(self, doctype, meta=None):
		...
		self.meta = meta or netmanthan.get_meta(doctype, False)
	...
	def sync(self):
		...
		if self.is_new():
			...
			netmanthan.cache().hdel("table_columns", self.table_name)
			self.alter()
	...
	def get_column_definitions(self):
		column_list = [] + netmanthan.db.DEFAULT_COLUMNS
		ret = []
	...
	def get_index_definitions(self):
		...
		for key, col in self.columns.items():
			if (
				col.set_index
				and not col.unique
				and col.fieldtype in netmanthan.db.type_map
				and netmanthan.db.type_map.get(col.fieldtype)[0] not in ("text", "longtext")
			):
	...
	def get_columns_from_docfields(self):
		...
		if not self.meta.get("istable"):
			for fieldname in netmanthan.db.OPTIONAL_COLUMNS:
	...
	def validate(self):
		...
		columns = [
			netmanthan._dict({"fieldname": f, "fieldtype": "Data"}) for f in netmanthan.db.STANDARD_VARCHAR_COLUMNS
		]
		if self.meta.get("istable"):
			columns += [
				netmanthan._dict({"fieldname": f, "fieldtype": "Data"}) for f in netmanthan.db.CHILD_TABLE_COLUMNS
			]
		...
		for col in columns:
			if len(col.fieldname) >= 64:
				netmanthan.throw(
					_("Fieldname is limited to 64 characters ({0})").format(netmanthan.bold(col.fieldname))
				)

			if "varchar" in netmanthan.db.type_map.get(col.fieldtype, ()):

				# validate length range
				new_length = cint(col.length) or cint(netmanthan.db.VARCHAR_LEN)
				if not (1 <= new_length <= 1000):
					netmanthan.throw(_("Length of {0} should be between 1 and 1000").format(col.fieldname))
				...
				if cint(current_length) != cint(new_length):
					try:
						...
						if netmanthan.db.is_missing_column(e):
					...
					if max_length and max_length[0][0] and max_length[0][0] > new_length:
						...
						netmanthan.msgprint(info_message)

	def is_new(self):
		return self.table_name not in netmanthan.db.get_tables()

	def setup_table_columns(self):
		# TODO: figure out a way to get key data
		for c in netmanthan.db.get_table_columns_description(self.table_name):
...
class DbColumn:
	...
	def get_definition(self, for_modification=False):
		...
		if self.fieldtype in ("Check", "Int"):
			...
			self.default
			and (self.default not in netmanthan.db.DEFAULT_SHORTCUTS)
			and not cstr(self.default).startswith(":")
		):
			column_def += f" default {netmanthan.db.escape(self.default)}"
	...
	if special_characters := SPECIAL_CHAR_PATTERN.findall(n):
		...
		netmanthan.throw(
			_("Fieldname {0} cannot have special characters like {1}").format(
				netmanthan.bold(cstr(n)), special_characters
			),
			netmanthan.db.InvalidColumnName,
		)
	return n


def validate_column_length(fieldname):
	if len(fieldname) > netmanthan.db.MAX_COLUMN_LENGTH:
		netmanthan.throw(_("Fieldname is limited to 64 characters ({0})").format(fieldname))


def get_definition(fieldtype, precision=None, length=None):
	d = netmanthan.db.type_map.get(fieldtype)
	...
	if fieldtype == "Int" and length and length > 11:
		...
		d = netmanthan.db.type_map.get("Long Int")
	...
	if size:
		...
		if length:
			if coltype == "varchar":
				...
				# NOTE: this will only be applicable for mariadb as netmanthan implements int
...
def add_column(
	...
	if column_name in netmanthan.db.get_table_columns(doctype):
		# already exists
		return
