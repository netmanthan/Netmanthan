...
from netmanthan.database.schema import DBTable
...
class MariaDBTable(DBTable):
	...
	def alter(self):
		...
		for col in self.add_index:
			...
			if not netmanthan.db.get_column_index(self.table_name, col.fieldname, unique=False):
		...
		for col in {*self.drop_index, *self.drop_unique}:
			...
			if unique_constraint_changed and not col.unique:
				if unique_index := netmanthan.db.get_column_index(self.table_name, col.fieldname, unique=True):
			...
			if index_constraint_changed and not col.set_index:
				if index_record := netmanthan.db.get_column_index(self.table_name, col.fieldname, unique=False):
