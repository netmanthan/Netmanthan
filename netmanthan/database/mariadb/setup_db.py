...
import netmanthan
from netmanthan.database.db_manager import DbManager
...
def get_mariadb_variables():
	return netmanthan._dict(netmanthan.db.sql("show variables"))
...
def setup_database(force, source_sql, verbose, no_mariadb_socket=False):
	netmanthan.local.session = netmanthan._dict({"user": "Administrator"})

	db_name = netmanthan.local.conf.db_name
	...
	dbman.create_user(db_name, netmanthan.conf.db_password, **dbman_kwargs)
	if verbose:
...
def setup_help_database(help_db_name):
	dbman = DbManager(get_root_connection(netmanthan.flags.root_login, netmanthan.flags.root_password))
...
def drop_user_and_database(db_name, root_login, root_password):
	netmanthan.local.db = get_root_connection(root_login, root_password)
	dbman = DbManager(netmanthan.local.db)
	dbman.drop_database(db_name)
...
def bootstrap_database(db_name, verbose, source_sql=None):
	...
	netmanthan.connect(db_name=db_name)
	if "tabDefaultValue" not in netmanthan.db.get_tables(cached=False):
...
def import_db_from_sql(source_sql=None, verbose=False):
	...
	db_name = netmanthan.conf.db_name
	if not source_sql:
		source_sql = os.path.join(os.path.dirname(__file__), "framework_mariadb.sql")
	DbManager(netmanthan.local.db).restore_database(db_name, source_sql, db_name, netmanthan.conf.db_password)
	if verbose:
...
def check_database_settings():
	...
	if not result:
		print(
			...
			).format(site=netmanthan.local.site, sep2="\n\n", sep="\n")
		)
...
def check_compatible_versions():
	try:
		...
		if version_tuple < (10, 3):
			click.secho(
				f"Warning: MariaDB version {version} is less than 10.3 which is not supported by netmanthan",
				fg="yellow",
			)
		elif version_tuple >= (10, 9):
			click.secho(
				f"Warning: MariaDB version {version} is more than 10.8 which is not yet tested with netmanthan Framework.",
				fg="yellow",
			)
	except Exception:
		click.secho(
