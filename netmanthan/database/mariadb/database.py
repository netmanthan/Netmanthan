...
from netmanthan.database.mariadb.schema import MariaDBTable
from netmanthan.utils import UnicodeWithAttrs, cstr, get_datetime, get_table_name
...
class MariaDBConnectionUtil:
	...
	def get_connection_settings(self) -> dict:
		...
		if self.user not in (netmanthan.flags.root_login, "root"):
			conn_settings["database"] = self.user

		if self.port:
			conn_settings["port"] = int(self.port)

		if netmanthan.conf.local_infile:
			conn_settings["local_infile"] = netmanthan.conf.local_infile

		if netmanthan.conf.db_ssl_ca and netmanthan.conf.db_ssl_cert and netmanthan.conf.db_ssl_key:
			conn_settings["ssl"] = {
				"ca": netmanthan.conf.db_ssl_ca,
				"cert": netmanthan.conf.db_ssl_cert,
				"key": netmanthan.conf.db_ssl_key,
			}
...
class MariaDBDatabase(MariaDBConnectionUtil, MariaDBExceptionUtil, Database):
	...
	def escape(s, percent=True):
		...
		# Also, given we're promoting use of the PyPika builder via netmanthan.qb, the use
		# of this method should be limited.

		# pymysql expects unicode argument to escape_string with Python 3
		s = netmanthan.as_unicode(escape_string(netmanthan.as_unicode(s)), "utf-8").replace("`", "\\`")
	...
	def get_tables(self, cached=True):
		...
		if cached:
			tables = netmanthan.cache().get_value("db_tables")
			to_query = not tables
		...
		if to_query:
			...
			netmanthan.cache().set_value("db_tables", tables)
