...
import netmanthan
from netmanthan.utils import get_request_site_address

CALLBACK_METHOD = "/api/method/netmanthan.integrations.google_oauth.callback"
_SCOPES = {
...
_DOMAIN_CALLBACK_METHODS = {
	"mail": "netmanthan.email.oauth.authorize_google_access",
	"contacts": "netmanthan.integrations.doctype.google_contacts.google_contacts.authorize_access",
	"drive": "netmanthan.integrations.doctype.google_drive.google_drive.authorize_access",
	"indexing": "netmanthan.website.doctype.website_settings.google_indexing.authorize_access",
}
...
class GoogleOAuth:
	...
	def __init__(self, domain: str, validate: bool = True):
		self.google_settings = netmanthan.get_single("Google Settings")
	...
	def validate_google_settings(self):
		...
		if not self.google_settings.enable:
			netmanthan.throw(netmanthan._("Please enable {} before continuing.").format(google_settings))

		if not (self.google_settings.client_id and self.google_settings.client_secret):
			netmanthan.throw(netmanthan._("Please update {} before continuing.").format(google_settings))
...
def handle_response(
	...
	if "error" in response:
		netmanthan.log_error(
			netmanthan._(error_title), netmanthan._(response.get("error_description", error_message))
		)

		if raise_err:
			netmanthan.throw(netmanthan._(error_title), GoogleAuthenticationError, netmanthan._(error_message))
...
@netmanthan.whitelist(methods=["GET"])
def callback(state: str, code: str = None, error: str = None) -> None:
	"""Common callback for google integrations.
	Invokes functions using `netmanthan.get_attr` and also adds required (keyworded) arguments
	along with committing and redirecting us back to netmanthan site."""
	...
	if not error:
		if (domain := state.pop("domain")) in _DOMAIN_CALLBACK_METHODS:
			...
			netmanthan.get_attr(_DOMAIN_CALLBACK_METHODS[domain])(**state)
