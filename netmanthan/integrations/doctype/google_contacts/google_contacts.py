...
class GoogleContacts(Document):
	def validate(self):
		if not netmanthan.db.get_single_value("Google Settings", "enable"):
			netmanthan.throw(_("Enable Google API in Google Settings."))

	def get_access_token(self):
		if not self.refresh_token:
			button_label = netmanthan.bold(_("Allow Google Contacts Access"))
...
def authorize_access(g_contact, reauthorize=False, code=None):
	...
	oauth_code = (
		netmanthan.db.get_value("Google Contacts", g_contact, "authorization_code") if not code else code
	)
	oauth_obj = GoogleOAuth("contacts")
...
def get_google_contacts_object(g_contact):
	...
	"""
	account = netmanthan.get_doc("Google Contacts", g_contact)
	oauth_obj = GoogleOAuth("contacts")
...
def sync(g_contact=None):
	...
	google_contacts = netmanthan.get_list("Google Contacts", filters=filters)
...
def sync_contacts_from_google_contacts(g_contact):
	...
	netmanthan.db.set_value("Google Contacts", account.name, "last_sync_on", netmanthan.utils.now_datetime())

	for idx, connection in enumerate(results):
		netmanthan.publish_realtime(
			"import_google_contacts", dict(progress=idx + 1, total=len(results)), user=netmanthan.session.user
		)
...
def insert_contacts_to_google_contacts(doc, method=None):
	...
	"""
	if (
		not netmanthan.db.exists("Google Contacts", {"name": doc.google_contacts})
	...
	try:
		...
		netmanthan.db.set_value("Contact", doc.name, "google_contacts_id", contact.get("resourceName"))
	except HttpError as err:
		netmanthan.msgprint(
...
def update_contacts_to_google_contacts(doc, method=None):
	...
	try:
		...
		netmanthan.msgprint(_("Contact Synced with Google Contacts."))
	except HttpError as err:
		netmanthan.msgprint(
