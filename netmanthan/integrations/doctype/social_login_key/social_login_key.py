...
class BaseUrlNotSetError(netmanthan.ValidationError):
	pass


class AuthorizeUrlNotSetError(netmanthan.ValidationError):
	pass


class AccessTokenUrlNotSetError(netmanthan.ValidationError):
	pass


class RedirectUrlNotSetError(netmanthan.ValidationError):
	pass


class ClientIDNotSetError(netmanthan.ValidationError):
	pass


class ClientSecretNotSetError(netmanthan.ValidationError):
	pass


class SocialLoginKey(Document):
	def autoname(self):
		self.name = netmanthan.scrub(self.provider_name)

	def validate(self):
		self.set_icon()
		if self.custom_base_url and not self.base_url:
			netmanthan.throw(_("Please enter Base URL"), exc=BaseUrlNotSetError)
		if not self.authorize_url:
			netmanthan.throw(_("Please enter Authorize URL"), exc=AuthorizeUrlNotSetError)
		if not self.access_token_url:
			netmanthan.throw(_("Please enter Access Token URL"), exc=AccessTokenUrlNotSetError)
		if not self.redirect_url:
			netmanthan.throw(_("Please enter Redirect URL"), exc=RedirectUrlNotSetError)
	...
	def set_icon(self):
		...
		if self.provider_name in icon_map:
			...
			self.icon = f"/assets/netmanthan/icons/social/{icon_file}"
	...
	def get_social_login_provider(self, provider, initialize=False):
		...
		providers["Office 365"] = {
			...
			"redirect_url": "/api/method/netmanthan.integrations.oauth2_logins.login_via_office365",
			"api_endpoint": None,
		...
		providers["GitHub"] = {
			...
			"redirect_url": "/api/method/netmanthan.integrations.oauth2_logins.login_via_github",
			"api_endpoint": "user",
			"api_endpoint_args": None,
		...
		providers["Google"] = {
			...
			"redirect_url": "/api/method/netmanthan.integrations.oauth2_logins.login_via_google",
		...
		providers["Facebook"] = {
			...
			"redirect_url": "/api/method/netmanthan.integrations.oauth2_logins.login_via_facebook",
		...
		providers["netmanthan"] = {
			"provider_name": "netmanthan",
			"enable_social_login": 1,
			"custom_base_url": 1,
			"icon": "/assets/netmanthan/images/netmanthan-favicon.svg",
			"redirect_url": "/api/method/netmanthan.integrations.oauth2_logins.login_via_netmanthan",
			"api_endpoint": "/api/method/netmanthan.integrations.oauth2.openid_profile",
			"api_endpoint_args": None,
			"authorize_url": "/api/method/netmanthan.integrations.oauth2.authorize",
			"access_token_url": "/api/method/netmanthan.integrations.oauth2.get_token",
			"auth_url_data": json.dumps({"response_type": "code", "scope": "openid"}),
		}
		...
		providers["Salesforce"] = {
			...
			"redirect_url": "/api/method/netmanthan.integrations.oauth2_logins.login_via_salesforce",
		...
		providers["fairlogin"] = {
			...
			"custom_base_url": 0,
			"icon": "fa fa-key",
			"redirect_url": "/api/method/netmanthan.integrations.oauth2_logins.login_via_fairlogin",
