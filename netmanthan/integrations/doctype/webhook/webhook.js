...
netmanthan.webhook = {
	set_fieldname_select: (frm) => {
		if (frm.doc.webhook_doctype) {
			netmanthan.model.with_doctype(frm.doc.webhook_doctype, () => {
				// get doctype fields
				let fields = $.map(
					netmanthan.get_doc("DocType", frm.doc.webhook_doctype).fields,
					(d) => {
						if (
							netmanthan.model.no_value_type.includes(d.fieldtype) &&
							!netmanthan.model.table_fields.includes(d.fieldtype)
						) {
							return null;
				...
				// add meta fields
				for (let field of netmanthan.model.std_fields) {
...
netmanthan.ui.form.on("Webhook", {
	refresh: (frm) => {
		netmanthan.webhook.set_fieldname_select(frm);
	},

	request_structure: (frm) => {
		netmanthan.webhook.set_request_headers(frm);
	},
...
netmanthan.ui.form.on("Webhook Data", {
	fieldname: (frm, cdt, cdn) => {
		...
		if (!df.length) {
			...
			df = netmanthan.model.std_fields.filter((field) => field.fieldname == row.fieldname);
		}
