...
import netmanthan
from netmanthan.integrations.doctype.webhook.webhook import (
	enqueue_webhook,
	get_webhook_data,
	get_webhook_headers,
)
...
def get_test_webhook(config):
	wh = netmanthan.get_doc(config).insert()
	wh.reload()
	try:
		yield wh
	finally:
...
class TestWebhook(netmanthanTestCase):
	@classmethod
	def setUpClass(cls):
		# delete any existing webhooks
		netmanthan.db.delete("Webhook")
		# Delete existing logs if any
		netmanthan.db.delete("Webhook Request Log")
		super().setUpClass()
	...
	def setUp(self):
		...
		if netmanthan.db.exists("Webhook", webhook_fields):
			self.webhook = netmanthan.get_doc("Webhook", webhook_fields)
		else:
			self.webhook = netmanthan.new_doc("Webhook")
			self.webhook.update(webhook_fields)

		# create a User document
		self.user = netmanthan.new_doc("User")
		self.user.first_name = netmanthan.mock("name")
		self.user.email = netmanthan.mock("email")
		self.user.save()

		# Create another test user specific to this test
		self.test_user = netmanthan.new_doc("User")
	...
	def test_webhook_trigger_with_enabled_webhooks(self):
		...
		netmanthan.flags.webhooks = None

		# Insert the user to db
		self.test_user.insert()

		self.assertTrue("User" in netmanthan.flags.webhooks)
		# only 1 hook (enabled) must be queued
		self.assertEqual(len(netmanthan.flags.webhooks.get("User")), 1)
		self.assertTrue(self.test_user.email in netmanthan.flags.webhooks_executed)
		self.assertEqual(
			netmanthan.flags.webhooks_executed.get(self.test_user.email)[0], self.sample_webhooks[0].name
		)
	...
	def test_validate_doc_events(self):
		...
		self.assertRaises(netmanthan.ValidationError, self.webhook.save)
	...
	def test_webhook_req_log_creation(self):
		if not netmanthan.db.get_value("User", "user2@integration.webhooks.test.com"):
			user = netmanthan.get_doc(
				{"doctype": "User", "email": "user2@integration.webhooks.test.com", "first_name": "user2"}
			).insert()
		else:
			user = netmanthan.get_doc("User", "user2@integration.webhooks.test.com")

		webhook = netmanthan.get_doc("Webhook", {"webhook_doctype": "User"})
		enqueue_webhook(user, webhook)

		self.assertTrue(netmanthan.get_all("Webhook Request Log", pluck="name"))
	...
	def test_webhook_with_array_body(self):
		...
		with get_test_webhook(wh_config) as wh:
			doc = netmanthan.new_doc("Note")
			doc.title = "Test Webhook Note"

			enqueue_webhook(doc, wh)
			log = netmanthan.get_last_doc("Webhook Request Log")
