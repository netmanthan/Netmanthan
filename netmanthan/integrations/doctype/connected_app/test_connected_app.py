...
import netmanthan
from netmanthan.integrations.doctype.social_login_key.test_social_login_key import (
...
def get_connected_app():
	...
	connected_app = netmanthan.new_doc(doctype)
	connected_app.provider_name = "netmanthan"
...
def get_oauth_client():
	oauth_client = netmanthan.new_doc("OAuth Client")
...
class TestConnectedApp(netmanthanTestCase):
	def setUp(self):
		"""Set up a Connected App that connects to our own oAuth provider.

		netmanthan comes with it's own oAuth2 provider that we can test against. The
		...
		netmanthan's "Authorization URL" and "Access Token URL" (actually they're
	...
	def test_web_application_flow(self):
		...
		resp = oauth2_session.get(urljoin(self.base_url, "/api/method/netmanthan.auth.get_logged_user"))
	...
	def tearDown(self):
		...
		if getattr(self, "oauth_client", None):
			tokens = netmanthan.get_all("OAuth Bearer Token", filters={"client": self.oauth_client.name})
			for token in tokens:
				doc = netmanthan.get_doc("OAuth Bearer Token", token.name)
				doc.delete()

			codes = netmanthan.get_all("OAuth Authorization Code", filters={"client": self.oauth_client.name})
			for code in codes:
				doc = netmanthan.get_doc("OAuth Authorization Code", code.name)
				doc.delete()
