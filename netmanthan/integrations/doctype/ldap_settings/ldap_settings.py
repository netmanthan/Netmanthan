...
import netmanthan
from netmanthan import _, safe_encode
from netmanthan.model.document import Document
from netmanthan.twofactor import authenticate_for_2factor, confirm_otp_token, should_run_2fa
...
class LDAPSettings(Document):
	...
	def connect_to_ldap(self, base_dn, password, read_only=True) -> ldap3.Connection:
		try:
			...
			netmanthan.throw(msg, title=_("LDAP Not Installed"))
		except LDAPInvalidCredentialsResult:
			netmanthan.throw(_("Invalid username or password"))
		except Exception as ex:
			netmanthan.throw(_(str(ex)))

	@staticmethod
	def get_ldap_client_settings() -> dict:
		# return the settings to be used on the client side.
		result = {"enabled": False}
		ldap = netmanthan.get_cached_doc("LDAP Settings")
		if ldap.enabled:
			result["enabled"] = True
			result["method"] = "netmanthan.integrations.doctype.ldap_settings.ldap_settings.login"
		return result
	...
	def create_or_update_user(self, user_data: dict, groups: list = None):
		...
		if netmanthan.db.exists("User", user_data["email"]):
			user = netmanthan.get_doc("User", user_data["email"])
		...
		if self.default_user_type == "System User":
			...
			role = netmanthan.db.get_value("User Type", user.user_type, "role")
	...
	def authenticate(self, username: str, password: str):
		if not self.enabled:
			netmanthan.throw(_("LDAP is not enabled."))
		...
		try:
			...
			netmanthan.throw(_("Please use a valid LDAP search filter"), title=_("Misconfigured"))
		...
		if conn.search(
			...
			if conn.entries and conn.entries[0]:
				...
				if conn.modify(entry_dn, changes=changes):
					if logout_sessions:
						from netmanthan.sessions import clear_sessions

						clear_sessions(user=user, force=True)
					netmanthan.msgprint(_("Password changed successfully."))
				else:
					netmanthan.throw(_("Failed to change password."))
			else:
				netmanthan.throw(_("No Entry for the User {0} found within LDAP!").format(user))
		else:
			netmanthan.throw(_("No LDAP User found for email: {0}").format(user))
	...
	def convert_ldap_entry_to_dict(self, user_entry: Entry):
		...
		if isinstance(email, list):
			...
			for e in email:
				if netmanthan.db.exists("User", e):
					email = e
					break
			else:
...
def login():
	# LDAP LOGIN LOGIC
	args = netmanthan.form_dict
	ldap: LDAPSettings = netmanthan.get_doc("LDAP Settings")

	user = ldap.authenticate(netmanthan.as_unicode(args.usr), netmanthan.as_unicode(args.pwd))

	netmanthan.local.login_manager.user = user.name
	if should_run_2fa(user.name):
		authenticate_for_2factor(user.name)
		if not confirm_otp_token(netmanthan.local.login_manager):
			return False
