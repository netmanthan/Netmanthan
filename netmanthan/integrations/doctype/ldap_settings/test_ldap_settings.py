...
import netmanthan
from netmanthan.exceptions import MandatoryError, ValidationError
...
class LDAP_TestCase:
	...
	def mock_ldap_connection(f):
		...
		def wrapped(self, *args, **kwargs):
			...
			with mock.patch(
				"netmanthan.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.connect_to_ldap",
				...
				netmanthan.get_doc(localdoc).save()
	...
	def clean_test_users():
		with contextlib.suppress(Exception):
			netmanthan.get_doc("User", "posix.user1@unit.testing").delete()
		with contextlib.suppress(Exception):
			netmanthan.get_doc("User", "posix.user2@unit.testing").delete()
		with contextlib.suppress(Exception):
			netmanthan.get_doc("User", "website_ldap_user@test.com").delete()

	@classmethod
	def setUpClass(cls):
		cls.clean_test_users()
		# Save user data for restoration in tearDownClass()
		cls.user_ldap_settings = netmanthan.get_doc("LDAP Settings")
		...
		user = netmanthan.get_doc(cls.user1doc)
		...
		user = netmanthan.get_doc(cls.user2doc)
		user.insert(ignore_permissions=True)
	...
	def tearDownClass(cls):
		with contextlib.suppress(Exception):
			netmanthan.get_doc("LDAP Settings").delete()
	...
	def test_connect_to_ldap(self):
		...
		prevent_connection_parameters = {
			"mode": {
				"IP_V4_ONLY": "Locks the user to IPv4 without netmanthan providing a way to configure",
				"IP_V6_ONLY": "Locks the user to IPv6 without netmanthan providing a way to configure",
			},
			"auto_bind": {
	...
	def test_update_user_fields(self):
		...
		test_user = netmanthan.get_doc("User", test_user_data["email"])
		self.test_class.update_user_fields(test_user, test_user_data)
		updated_user = netmanthan.get_doc("User", test_user_data["email"])
		...
		self.assertIn(self.test_class.default_role, netmanthan.get_roles(updated_user.name))
	...
	def test_create_website_user(self):
		...
		new_user = netmanthan.get_doc("User", new_test_user_data["email"])
	...
	def test_sync_roles(self):
		if self.TEST_LDAP_SERVER.lower() == "openldap":
			test_user_data = {
				"posix.user1": [
					...
					"Administrators",
					"default_role",
					"netmanthan_default_all",
					"netmanthan_default_guest",
				],
				"posix.user2": [
					"Users",
					"Group3",
		...
		role_to_group_map = {
			...
			"All": "netmanthan_default_all",
			"Guest": "netmanthan_default_guest",
		}

		# re-create user1 to ensure clean
		netmanthan.get_doc("User", "posix.user1@unit.testing").delete()
		user = netmanthan.get_doc(self.user1doc)
		user.insert(ignore_permissions=True)

		for test_user in test_user_data:
			test_user_doc = netmanthan.get_doc("User", f"{test_user}@unit.testing")
			test_user_roles = netmanthan.get_roles(f"{test_user}@unit.testing")

			self.assertTrue(
				len(test_user_roles) == 2, "User should only be a part of the All and Guest roles"
			)  # check default netmanthan roles

			self.test_class.sync_roles(test_user_doc, test_user_data[test_user])  # update user roles

			netmanthan.get_doc("User", f"{test_user}@unit.testing")
			updated_user_roles = netmanthan.get_roles(f"{test_user}@unit.testing")
	...
	def test_create_or_update_user(self):
		...
		netmanthan.get_doc("User", f"{test_user}@unit.testing").delete()

		with self.assertRaises(
			netmanthan.exceptions.DoesNotExistError
		):  # ensure user deleted so function can be tested
			netmanthan.get_doc("User", f"{test_user}@unit.testing")

		with mock.patch(
			"netmanthan.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.update_user_fields"
		) as update_user_fields_method:
			with mock.patch(
				"netmanthan.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.sync_roles"
	...
	def test_authenticate(self):
		with mock.patch(
			"netmanthan.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.fetch_ldap_groups"
		...
		for username, password in enumerate(invalid_users):
			with self.assertRaises(netmanthan.exceptions.ValidationError) as display_massage:
