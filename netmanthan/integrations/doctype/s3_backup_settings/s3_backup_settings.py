...
class S3BackupSettings(Document):
	def validate(self):
		...
		try:
			...
			bucket_name = netmanthan.bold(self.bucket)
			if error_code == "403":
...
def take_backup():
	...
	enqueue(
		"netmanthan.integrations.doctype.s3_backup_settings.s3_backup_settings.take_backups_s3",
		queue="long",
		timeout=1500,
	)
...
def take_backups_if(freq):
	if cint(netmanthan.db.get_single_value("S3 Backup Settings", "enabled")):
		if netmanthan.db.get_single_value("S3 Backup Settings", "frequency") == freq:
			take_backups_s3()
...
def notify():
	error_message = netmanthan.get_traceback()
	send_email(False, "Amazon S3", "S3 Backup Settings", "notify_email", error_message)


def backup_to_s3():
	from netmanthan.utils import get_backups_path
	from netmanthan.utils.backups import new_backup

	doc = netmanthan.get_single("S3 Backup Settings")
	bucket = doc.bucket
