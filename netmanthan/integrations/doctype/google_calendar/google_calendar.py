...
from netmanthan.integrations.google_oauth import GoogleOAuth
...
from netmanthan.utils.password import set_encrypted_password
...
class GoogleCalendar(Document):
	def validate(self):
		...
		if not google_settings.enable:
			netmanthan.throw(_("Enable Google API in Google Settings."))

		if not google_settings.client_id or not google_settings.client_secret:
			netmanthan.throw(_("Enter Client Id and Client Secret in Google Settings."))
	...
	def get_access_token(self):
		...
		if not self.refresh_token:
			button_label = netmanthan.bold(_("Allow Google Calendar Access"))
			raise netmanthan.ValidationError(_("Click on {0} to generate Refresh Token.").format(button_label))
...
def authorize_access(g_calendar, reauthorize=None):
	...
	"""
	google_settings = netmanthan.get_doc("Google Settings")
	google_calendar = netmanthan.get_doc("Google Calendar", g_calendar)

	redirect_uri = (
		get_request_site_address(True)
		+ "?cmd=netmanthan.integrations.doctype.google_calendar.google_calendar.google_callback"
	)

	if not google_calendar.authorization_code or reauthorize:
		netmanthan.cache().hset("google_calendar", "google_calendar", google_calendar.name)
		...
		try:
			...
			netmanthan.local.response["location"] = "/app/Form/{}/{}".format(
				quote("Google Calendar"), quote(google_calendar.name)
			)

			netmanthan.msgprint(_("Google Calendar has been configured."))
...
def google_callback(code=None):
	...
	"""
	google_calendar = netmanthan.cache().hget("google_calendar", "google_calendar")
	netmanthan.db.set_value("Google Calendar", google_calendar, "authorization_code", code)
	netmanthan.db.commit()
...
def sync(g_calendar=None):
	...
	google_calendars = netmanthan.get_list("Google Calendar", filters=filters)
...
def get_google_calendar_object(g_calendar):
	...
	"""
	google_settings = netmanthan.get_doc("Google Settings")
	account = netmanthan.get_doc("Google Calendar", g_calendar)
	...
	try:
		if account.google_calendar_id:
			...
			calendar = {
				...
				"timeZone": netmanthan.db.get_single_value("System Settings", "time_zone"),
			}
...
def sync_events_from_google_calendar(g_calendar, method=None):
	...
	while True:
		try:
			...
			if err.resp.status == 410:
				...
				netmanthan.msgprint(msg, title="Invalid Sync Token", indicator="blue")
			else:
				netmanthan.throw(msg)
	...
	for idx, event in enumerate(results):
		netmanthan.publish_realtime(
			"import_google_calendar", dict(progress=idx + 1, total=len(results)), user=netmanthan.session.user
		)
		...
		if event.get("status") == "confirmed":
			...
			if not netmanthan.db.exists("Event", {"google_calendar_event_id": event.get("id")}):
			...
			netmanthan.get_doc(
				{
					...
					"reference_name": netmanthan.db.get_value(
						"Event",
						{
							"google_calendar_id": account.google_calendar_id,
							"google_calendar_event_id": event.get("id"),
...
def insert_event_to_calendar(account, event, recurrence=None):
	"""
	Inserts event in netmanthan Calendar during Sync
	"""
	calendar_event = {
		"doctype": "Event",
	...
	netmanthan.get_doc(calendar_event).insert(ignore_permissions=True)


def update_event_in_calendar(account, event, recurrence=None):
	"""
	Updates Event in netmanthan Calendar if any existing Google Calendar Event is updated
	"""
	calendar_event = netmanthan.get_doc("Event", {"google_calendar_event_id": event.get("id")})
...
def insert_event_in_google_calendar(doc, method=None):
	...
	"""
	if (
		not netmanthan.db.exists("Google Calendar", {"name": doc.google_calendar})
	...
	try:
		...
		netmanthan.msgprint(_("Event Synced with Google Calendar."))
	except HttpError as err:
		netmanthan.throw(
...
def update_event_in_google_calendar(doc, method=None):
	"""
	Updates Events in Google Calendar if any existing event is modified in netmanthan Calendar
	"""
...
def delete_event_from_google_calendar(doc, method=None):
	"""
	Delete Events from Google Calendar if netmanthan Event is deleted.
	"""

	if not netmanthan.db.exists("Google Calendar", {"name": doc.google_calendar}):
		return
