...
netmanthan.ui.form.on("Google Calendar", {
	refresh: function (frm) {
		...
		netmanthan.realtime.on("import_google_calendar", (data) => {
			if (data.progress) {
		...
		if (frm.doc.refresh_token) {
			frm.add_custom_button(__("Sync Calendar"), function () {
				...
				netmanthan
					.call({
						method: "netmanthan.integrations.doctype.google_calendar.google_calendar.sync",
						args: {
	...
	authorize_google_calendar_access: function (frm) {
		...
		netmanthan.call({
			method: "netmanthan.integrations.doctype.google_calendar.google_calendar.authorize_access",
			args: {
				g_calendar: frm.doc.name,
