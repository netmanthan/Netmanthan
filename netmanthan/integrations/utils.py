...
from netmanthan.utils import get_request_session
...
def make_request(method, url, auth=None, headers=None, data=None):
	...
	try:
		...
		netmanthan.flags.integration_request = s.request(method, url, data=data, auth=auth, headers=headers)
		netmanthan.flags.integration_request.raise_for_status()

		if netmanthan.flags.integration_request.headers.get("content-type") == "text/plain; charset=utf-8":
			return parse_qs(netmanthan.flags.integration_request.text)

		return netmanthan.flags.integration_request.json()
...
def create_request_log(
	...
	**kwargs,
	...
	integration_request = netmanthan.get_doc(
		{
...
def get_json(obj):
	return obj if isinstance(obj, str) else netmanthan.as_json(obj, indent=1)
