...
const html_plugin = require("./netmanthan-html");
const rtlcss = require("rtlcss");
const postCssPlugin = require("@netmanthan/esbuild-plugin-postcss2").default;
...
const argv = yargs
	...
	.option("skip_netmanthan", {
		type: "boolean",
		description: "Skip building netmanthan assets",
	})
	.option("files", {
		type: "string",
	...
	.example("node esbuild --apps netmanthan,erpnext", "Run build only for netmanthan and erpnext")
	.example(
		"node esbuild --files netmanthan/website.bundle.js,netmanthan/desk.bundle.js",
		"Run build only for specified bundles"
	)
	.version(false).argv;

const APPS = (!argv.apps ? app_list : argv.apps.split(",")).filter(
	(app) => !(argv.skip_netmanthan && app == "netmanthan")
);
...
function get_files_to_build(files) {
	// files: ['netmanthan/website.bundle.js', 'erpnext/main.bundle.js']
	let include_patterns = [];
	let ignore_patterns = [];
